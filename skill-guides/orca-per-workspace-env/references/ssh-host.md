...
Load this when the recipe connects over SSH instead of starting `manta serve`, and when the user has
explicitly asked for `checkoutMode: provisioned-root`.

SSH mode is a different shape, not the Manta-server templates relabeled. `create` runs no
`manta serve` and emits no `pairingCode`. Manta connects over its SSH relay, brings up the git and
filesystem providers, and imports the repo. The script only readies the host and prints the SSH
details Manta dials.

## The result shape

Manta rejects anything else. Required fields only; add optionals from the next section as the
network needs them.
...
These describe how the user's desktop reaches the box; there is no `manta serve` URL in SSH mode.
...
- `relayGracePeriodSeconds` bounds how long Manta keeps the SSH relay alive after the workspace
...
and resume. Manta still disconnects and reconnects its own SSH relay on sleep, wake, and delete, which
...
`MANTA_REPO_BRANCH`. Use `MANTA_REPO_REF` to fetch the selected source, but create `MANTA_REPO_BRANCH`
at the exact `MANTA_REPO_REF_HEAD` commit, because resolving the symbolic ref again can race with an
upstream update. `MANTA_REPO_URL` and `MANTA_REPO_REF` are a matched fetch pair, and the URL is the
remote Manta resolved the base ref against, which is not necessarily named `origin` on the desktop.
Fetch from the URL the pair supplies:

```bash
[ -n "${MANTA_REPO_REF_HEAD:-}" ] || { echo "missing pinned source commit" >&2; exit 1; }
git fetch "$MANTA_REPO_URL" "$MANTA_REPO_REF"
