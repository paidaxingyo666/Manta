...
rejects `--from`. Omit `--terminal` inside a Manta terminal, where Manta resolves
...
```text
MANTA orchestration gate-create --task <task_id> --question "<decision>" --options <json_array> --json
MANTA orchestration gate-resolve --id <gate_id> --resolution "<choice>" --json
MANTA orchestration gate-list --task <task_id> --json
```
