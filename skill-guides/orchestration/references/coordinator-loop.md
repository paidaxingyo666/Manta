...
```text
MANTA orchestration task-create --spec "<dependent work>" --deps <json_array> --json
MANTA orchestration task-list --ready --brief --json
```
...
```text
MANTA orchestration worker-start --task <task_id> --worktree current --agent claude --model sonnet --json
...
connected worker server must advertise launch-preference support before Manta
...
MANTA orchestration worker-start --task <next_task_id> --terminal <agent_terminal_handle> --json
```
