...
```text
MANTA orchestration worker-list --run <run_id> --json
...
Continue with its top-level cursor, which is pinned to that source. If Manta
...
operation identity so Manta can replay, join, or recover it instead of starting a
duplicate. Ask what happened first:

```text
MANTA orchestration request-show --request <request_id> --json
```
...
Manta restarted before recording its outcome; replay the original command with
...
```text
MANTA orchestration worker-start --task <task_id> --retry-of <dispatch_id> --worktree <explicit_placement> --agent <agent> --json
```
...
```text
MANTA orchestration worker-stop --dispatch <dispatch_id> --json
MANTA orchestration worker-abandon --dispatch <dispatch_id> --json
```
