...
description: >-
  ...
  lightweight terminal prompts, shell commands, Manta worktree management,
  reading or waiting on terminals, and the Manta embedded browser. Use Computer
  Use for external browser windows, webviews, Manta app UI, or desktop UI
  outside Manta's embedded browser only when the task requires OS/window-level
  control such as focus, menus, dialogs, coordinates, or screenshots. Use
  `orca-cli` for Manta's embedded pages and a page-automation tool such as
  Playwright or CDP for external pages.
---

# Manta Inter-Agent Orchestration

Orchestration is Manta's structured coordination layer for agent messages, task ownership, dispatch state, and worker completion tracking.
...
If a task says to use Manta orchestration, the coordinator must create or bind a Run, create the Task with `manta orchestration task-create`, then attach the worker with either the preferred `manta orchestration worker-start` composition or the low-level `manta orchestration dispatch --inject` path.

Do not substitute non-Manta subagent tools, generic agent-spawn APIs, or chat-only parallel worker features. Those may create useful workers, but they do not create Manta task/dispatch provenance, injected lifecycle preambles, `worker_done` authority, or decision gates.

Before claiming a worker was orchestrated, verify the task/dispatch exists:

```bash
manta orchestration task-list --json
manta orchestration dispatch-show --task <task_id> --json
```

If the work was accidentally run outside Manta orchestration, say so plainly. To repair provenance, rerun or revalidate the needed work through a fresh Manta terminal plus injected dispatch; do not retroactively describe the external worker as orchestrated.
...
- `manta status --json` should show a running runtime.
- `manta` must be on PATH (`manta-ide` on Linux).
- The orchestration experimental feature must be enabled in Settings > Experimental.
- `manta orchestration` commands are RPC calls to the running Manta runtime.

## Contract Migration

Manta adopts a live pre-update orchestration assignment into an ordinary Run. Adoption preserves the existing agent process, PTY/session, terminal handle, tab/leaf/pane, worktree or folder workspace, Task, and Dispatch; it never restarts or replaces the worker. The retired scheduler is not revived, and a newly created attempt uses the current grammar.
...
```bash
manta orchestration run-use --id <adopted_run_id> --takeover-legacy --json
manta orchestration check --run <adopted_run_id> --json
```
...
New orchestration messages and tasks belong to one explicitly bound Run. A Run is only a durable namespace and coordinator inbox; it never schedules or places workers. Lifecycle authority comes from the active Dispatch, and terminal handles remain routing metadata rather than durable identity. Send `worker_done` and `heartbeat` from the worker's own terminal; Manta routes them to that Dispatch's Run.
...
- Do not use `manta orchestration dispatch --inject` for full handoffs. It injects a coordinator preamble that tells the worker to send `worker_done`, heartbeat, and `ask` messages, then end its turn under the original terminal's dispatch lifecycle.
- Do not run `manta orchestration task-create`, `manta orchestration dispatch --inject`, or `manta orchestration check --wait` for full handoffs. Do not peek at terminal output after prompt delivery to monitor progress.
...
```bash
manta orchestration send --subject <text> [--to <run:id|dispatch:id|legacy_handle>] [--from <handle>] [--body <text>] [--type <type>] [--priority <level>] [--thread-id <id>] [--payload <json>] [--json]
manta orchestration check [--terminal <handle>] [--ack <delivery_id>] [--peek|--all] [--types <type,...>] [--format] [--wait] [--timeout-ms <n>] [--json]
manta orchestration reply --id <msg_id> --body <text> [--from <handle>] [--json]
manta orchestration ask (--question <text>|--resume <msg_id>) [--options <csv>] [--timeout-ms <n>] [--from <handle>] [--json]
manta orchestration inbox [--limit <n>] [--json]
```

Rules:

- Omit `--from` unless impersonating another terminal; Manta auto-resolves it from the current terminal.
...
- Use `dispatch:<id>` for coordinator guidance to one supervised worker. Manta routes that stable address locally or through the connected-server relay; do not substitute a remote terminal handle.
- Terminal handles remain appropriate for low-level pre-Dispatch messaging. Prefer `agentTerminalHandle` from the create response, fall back to `startupTerminal.handle` for older runtimes, then re-resolve with `manta terminal list --worktree ... --json` if missing or stale. Continue with the replacement handle only; never dual-send to old and new handles.
...
- `manta orchestration check --peek --format --json` returns locally formatted unread mail without consuming it; it never writes to terminal input or remotely wakes another terminal. Use `orchestration dispatch --inject` to deliver a tracked task, or `terminal send` when an existing agent needs a free-form prompt.
- While supervising workers manually, use `check --wait --types worker_done,escalation,question --timeout-ms <n>` instead of sleep/poll loops. Process the whole Delivery, reply to `question` messages with `manta orchestration reply --id <msg_id> --body <answer> --json`, then acknowledge and keep waiting.
...
- `heartbeat` is also Dispatch-scoped. Include both IDs and omit `--to` so Manta uses the owning Run; use `status` for broad progress updates.
...
```bash
manta orchestration run-create --objective <text> --json
manta orchestration task-create --spec <text> [--deps <json_array>] [--parent <task_id>] [--json]
manta orchestration task-list [--status <status>] [--ready] [--brief] [--json]
manta orchestration task-update --id <task_id> --status <status> [--result <json>] [--json]
manta orchestration dispatch --task <task_id> --to <handle> [--from <handle>] [--inject] [--json]
manta orchestration dispatch-show --task <task_id> [--json]
```
...
- If the target is a bare shell, omit `--inject`, dispatch for tracking if needed, then send the prompt manually with `manta terminal send --terminal <handle> --text <prompt> --enter --json`.
...
Use `worker-start` for the normal supervised path. It composes the existing worktree, terminal, readiness, and dispatch primitives while returning exact created/reused effects. Agents still choose placement and concurrency; Manta does not schedule workers or infer conflicts.
...
manta orchestration task-create --spec "<worker A task>" --json
manta orchestration task-create --spec "<worker B task>" --json
manta orchestration worker-start --task <task_a> --worktree current --agent codex --json
manta orchestration worker-start --task <task_b> --worktree current --agent claude --json
```
...
```bash
manta orchestration worker-start --task <task_id> --worktree current --agent claude --model opus --effort high --json
```

`--effort` requires `--model`, and neither option can combine with `--terminal`. A connected worker server must advertise launch-preference support before Manta forwards either option.

For a new worktree, setup runs by default and agent-first creation reuses the returned startup agent terminal:

```bash
manta orchestration worker-start --task <task_id> --worktree new-child --name <name> --agent codex --setup run --json
# Independent/top-level:
manta orchestration worker-start --task <task_id> --worktree new-top-level --name <name> --agent codex --setup run --json
```
...
To run the worker on another connected Manta server, add `--on <saved-environment>`. The Run and Tasks remain authoritative on the current server; later commands route by Dispatch ID, so never repeat `--on`:

```bash
# Mac Run home -> Windows worker (the reverse is identical from a Windows Run home)
manta orchestration worker-start --task <task_id> --on windows --worktree new-top-level --repo <exact_remote_repo_selector> --name <name> --agent codex --setup run --json
manta orchestration worker-show --dispatch <dispatch_id> --json
manta orchestration worker-read --dispatch <dispatch_id> --limit 50 --json
manta orchestration send --to dispatch:<dispatch_id> --subject "Follow-up" --body "<attempt-specific guidance>" --json
```
...
`orchestration check` receives it even when the Dispatch is on another connected Manta server.

`worker-read` defaults to `--source auto`: Manta returns the exact hook-reported Codex, Claude, OpenClaude, or Grok transcript when it can prove the worker session, otherwise it returns bounded terminal output with `source: "terminal"` and a typed `fallbackReason`. Continue with the returned top-level `cursor`; it stays pinned to that exact source. If Manta reports `source_changed`, start a fresh read without the old cursor. Never supply or guess a provider session ID or transcript path.
...
manta orchestration worker-release --dispatch <dispatch_id> --json
# Acknowledge only after every message and required release decision is handled:
manta orchestration check --ack <delivery_id> --wait --types worker_done,escalation,question --timeout-ms 900000 --json
```

After processing each accepted `worker_done`, choose the terminal's next owner before you acknowledge the Delivery or wait again. If the same exact agent has an immediate follow-up Task, read the `worker.agent_terminal_handle` field of `worker-show --dispatch <dispatch_id> --json`, then run `manta orchestration worker-start --task <next_task_id> --terminal <handle> --json` so Manta transfers cleanup ownership to the new Dispatch. Otherwise run `manta orchestration worker-release --dispatch <dispatch_id> --json`.

Run `worker-release` after both succeeded and failed `worker_done` reports unless the user explicitly asked to keep that worker live. Release is post-completion cleanup, not cancellation: Manta first preserves inspectable output, then closes only the exact agent terminal owned by that settled Dispatch. Reused or pre-existing terminals, setup terminals, coordinators, active workers, user-taken-over terminals, and identities Manta cannot prove are retained. If the user explicitly asks to keep the live terminal for debugging, record that exception with `manta orchestration worker-retain --dispatch <dispatch_id> --json` instead of silently skipping cleanup. When the user is finished, the same Dispatch can be passed to `worker-release`, which clears the requested retention and releases the terminal.
...
Workers report exactly once using the IDs and capability injected by Manta; they do not supply Run/server/terminal identity:

```bash
manta orchestration send --type worker_done --subject "<status>" --body "<what changed, findings, and what remains>" --task-id <task_id> --dispatch-id <dispatch_id> --outcome succeeded --files-modified "path/a,path/b" --json
...
```bash
manta orchestration ask --question "<question>" --options "yes,no" --timeout-ms 600000 --json
manta orchestration ask --resume <message_id> --timeout-ms 600000 --json
# Coordinator:
manta orchestration reply --id <message_id> --body "<answer>" --json
```

Recovery is conditional, never a fixed destructive sequence:

- The response was lost and named no Dispatch: run `manta orchestration request-show --request <request_id> --json` first. It is read-only. `completed` means the mutation already took effect. `pending` means the original mutation is still running or Manta restarted before recording its outcome. For either state, replaying the original command with `--retry-request <request_id>` reuses the same operation identity so Manta can replay, join, or safely recover it without starting a separate duplicate. `absent` means this runtime holds no receipt under your caller identity and is not proof that nothing happened; inspect the affected state before deciding whether to retry.
...
```bash
manta orchestration gate-create --task <task_id> --question <text> [--options <json_array>] [--json]
manta orchestration gate-resolve --id <gate_id> --resolution <text> [--json]
manta orchestration gate-list [--task <task_id>] [--status <status>] [--json]
```
...
Recovery only: `manta orchestration reset --tasks|--messages|--all --json` clears the selected local orchestration database state. Do not run it during active coordination unless explicitly abandoning that state.
...
Do not run `manta orchestration task-create`, `manta orchestration dispatch --inject`, or `manta orchestration check --wait` for full handoffs. `task-create` is also forbidden because it records coordinator-owned tracking state; if a task row is needed, the user asked for supervised orchestration. Do not create a `taskId`/`dispatchId`, inject a lifecycle preamble, wait for completion, or read the worker terminal after prompt delivery except to avoid losing the initial prompt.

New top-level worktree handoff:

```bash
manta worktree create --name <task-name> --no-parent --agent codex --prompt "<task brief>" --setup run --json
```

Before creating a new worktree from an active feature branch, decide and state whether the desired Manta lineage is child or top-level. Use child worktree lineage only when the new work is conceptually stacked under or dependent on the active worktree. For independent repo-wide fixes, standalone feature work, or unrelated follow-up tasks, create a top-level worktree with `--no-parent`.
...
`manta worktree create --agent codex --prompt ...` launches the known Codex agent but does not accept Codex-specific `--model` or `-c model_reasoning_effort=...` arguments. When the user asks for a specific Codex model or effort, create the independent worktree first, launch Codex with the requested command in that worktree, wait only for TUI readiness if prompt delivery would otherwise race startup, send the prompt, and stop.
...
Use the exact full `<repo-id>::<path>` worktree id returned by `manta worktree create --json`; a bare repo id cannot target the new worktree.

```bash
manta worktree create --name <task-name> --no-parent --setup run --json
manta terminal create --worktree id:<newFullWorktreeId> --title <task-name> --command 'codex --model gpt-5.5 -c model_reasoning_effort="xhigh"' --json
...
`--no-parent` only controls Manta lineage; it does not choose the Git base. If the work should start from the repo default base, omit `--base-branch` so Manta uses that default, or explicitly pass the repo default base (`origin/main`, `origin/master`, or the `manta repo show --repo <selector> --json` value); never base it on the current feature branch unless the user explicitly asks for stacked work or "branch from current". Put current-branch context in the prompt instead.
...
```bash
manta terminal create --worktree active --title <task-name> --command "codex" --json
manta terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
...
When a new worktree is allowed, use child lineage for isolated work that is stacked under or dependent on the active worktree, and use `--no-parent` when it is not stacked. Decide the Git base separately: `--no-parent` makes the worktree top-level in Manta, while omitted `--base-branch` uses the repo default base.
...
```bash
manta worktree create --name <task-name> --agent codex --setup run --json
...
Use `manta worktree create --prompt ...` or `manta terminal send ...` for full handoffs or untracked/lightweight prompts. Those paths do not attach `taskId`/`dispatchId`; the worker should not send lifecycle messages unless the prompt supplies a live orchestration preamble.
...
```bash
manta terminal list [--worktree <selector>] [--include-visual-layouts] [--json]
manta terminal create [--worktree <selector>] [--title <text>] [--command <cmd>] [--json]
manta terminal split --terminal <handle> [--direction horizontal|vertical] [--command <cmd>] [--json]
manta terminal wait --terminal <handle> --for tui-idle --timeout-ms <n> --json
manta terminal read --terminal <handle> --json
manta terminal send --terminal <handle> --text <text> --enter --json
```

If an older CLI rejects `worktree create --agent`, create the worktree normally, then run `manta terminal create --worktree <selector> --command "codex" --json` or `--command "claude"`.
...
- Workers with a valid live preamble must send `worker_done` exactly once from their own terminal with an explicit `--outcome succeeded` or `--outcome failed`:
  `manta orchestration send --type worker_done --subject "<short status>" --body "<3-sentence summary: what you did, what you found, what's left>" --task-id <task_id> --dispatch-id <dispatch_id> --outcome succeeded --files-modified "path/a" --report-path "<optional>" --json`
- A failed outcome is still a terminal report, but Manta records both the Dispatch and Task as failed. Never encode failure only in the subject/body.
...
- For long tasks, send heartbeat/status only when the preamble asks for it, including both IDs:
  `manta orchestration send --type heartbeat --subject "alive" --payload '{"taskId":"<task_id>","dispatchId":"<dispatch_id>","phase":"implementing"}' --json`
...
```bash
manta terminal create --worktree active --title login-css-worker --command "claude" --json
manta terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
manta orchestration task-create --spec "Fix the login button CSS" --json
manta orchestration dispatch --task <task_id> --to <handle> --inject --json
manta orchestration check --wait --types worker_done,escalation,question --timeout-ms 900000 --json
```
