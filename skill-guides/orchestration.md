...
Orchestration is Manta's structured coordination layer. It records who owns work,
...
3. Read coordinator follow-ups at each natural checkpoint — before starting a
   ...
   `MANTA orchestration check --terminal <your_handle> --json`.
...
omit it inside the coordinator's own Manta terminal. It returns the bound Run's
...
`MANTA orchestration worker-list --include-remote --json` (defaults to the bound
...
below, run `MANTA skills get orchestration --reference references/<file>.md` and
read only that document; `--references` lists the names. If the CLI rejects
`--reference`, run `MANTA skills get orchestration --full` once instead: it
