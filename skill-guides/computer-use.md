...
name: computer-use
description: >-
  Drives the GUI of a visible local app window through `manta computer`: accessibility
  ...
  for Manta's embedded browser (`manta-cli`).
---

# Computer Use

Use this skill to drive a visible app window through `manta computer`. Prefer a programmatic path (shell, filesystem, git, HTTP, existing CLIs) whenever it can complete the task; use this skill only when a visible window needs GUI control those cannot reach. Do not use it for Manta's embedded browser (`manta-cli`).
