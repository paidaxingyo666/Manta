...
name: computer-use
description: >-
  Use Manta's computer-use CLI to inspect and operate local desktop app windows
  ...
  accessibility actions. Also use for browser windows, webviews, Manta app UI,
  or other desktop UI. Triggers include "computer use", "manta computer", "read
...
Use this skill for desktop UI through `manta computer`. When the requested target is a website or web app, operate the desktop browser app/window that contains the page.
...
MANTA computer get-app-state --app com.spotify.client --json
MANTA computer click --app com.spotify.client --element-index 42 --json
```
...
```text
MANTA computer get-app-state --app com.microsoft.edgemac --json
MANTA computer get-app-state --app Spotify --json
MANTA computer get-app-state --app pid:12345 --json
```
...
MANTA computer capabilities --json
MANTA computer list-apps --json
MANTA computer list-windows --app <app> --json
MANTA computer get-app-state --app <app> --json
MANTA computer get-app-state --app <app> --restore-window --json
MANTA computer click --app <app> --element-index <index> --json
MANTA computer click --app <app> --x 100 --y 100 --json
MANTA computer click --app <app> --x 100 --y 100 --modifiers CmdOrCtrl+Shift --json
MANTA computer click --app <app> --element-index <index> --mouse-button right --json
MANTA computer click --app <app> --element-index <index> --mouse-button middle --json
MANTA computer perform-secondary-action --app <app> --element-index <index> --action <name> --json
MANTA computer set-value --app <app> --element-index <index> --value "text" --json
MANTA computer type-text --app <app> --text "text" --json
MANTA computer press-key --app <app> --key Return --json
MANTA computer hotkey --app <app> --key CmdOrCtrl+A --json
MANTA computer paste-text --app <app> --text "text" --json
MANTA computer scroll --app <app> (--element-index <index> | --x <x> --y <y>) --direction down --json
MANTA computer drag --app <app> --from-element-index <index> --to-element-index <index> --json
MANTA computer drag --app <app> --from-x 100 --from-y 100 --to-x 300 --to-y 300 --json
```
...
```bash
printf '%s' "$TEXT" | MANTA computer set-value --app <app> --element-index <index> --value-stdin --json
```
...
```text
MANTA computer get-app-state --app com.microsoft.edgemac --restore-window --json
MANTA computer set-value --app com.microsoft.edgemac --element-index <addressBarIndex> --value "test123" --json
MANTA computer press-key --app com.microsoft.edgemac --key Return --json
```
...
- `app_not_found`: run `list-apps` and retry with the bundle ID. If the target is a web app such as Gmail, choose the desktop browser app/window that contains it; do not retry `MANTA computer ... --app Gmail` unchanged because `manta computer` app selectors refer to desktop apps, not website names.
...
- `screenshot_failed`: use `--no-screenshot` if tree state is enough; if the message names Screen Recording or screenshots permission, run `MANTA computer permissions --id screenshots --json`.
...
Confirm Manta status unless already checked, then run `MANTA computer capabilities --json`. For website or web-app targets such as Gmail, identify the desktop browser app/window that contains the page, then get that target app state with `MANTA computer get-app-state --app <app> --json`.
