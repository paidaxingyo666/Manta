...
Inside the lifecycle scripts the placeholder does not apply: `manta serve` written there runs on
...
done. Never create a Manta workspace except for the step-10 test the user asked for. Do not create
...
- **Never snapshot a machine on which the Manta runtime has already run.** The first `manta serve`
  ...
  dedicated to Manta runtime data, never `/`, the home directory, or an ancestor of home. Refuse
