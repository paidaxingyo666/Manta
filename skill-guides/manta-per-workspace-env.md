...
Manta is a **thin wrapper**: you guide, detect, and scaffold; you never own the user's cloud account,
...
**The one branch that shapes everything — connection mode:** **Manta-server** (`create` runs `manta serve`
in the env and emits a `pairingCode`; §7c/§7f) vs **SSH** (`create` runs no server and emits a
`connection.type:"ssh"` block Manta dials into; §7g/§7h). Settle this first — it changes the `create`
output shape and half the templates.

Keep Manta's checkout behavior unchanged by default: omit `checkoutMode`, emit schema version 1, and
let Manta create a linked worktree. Only use `checkoutMode: provisioned-root` when the user explicitly
...
**Quick-start (happy path):** interview the user (connection mode Manta-server vs SSH, provider, agent CLI,
git auth — §1.2) + read the provider's CLI docs → scaffold `scripts/manta-vm/` from §7 → run the
base-snapshot script, then the auth script (you invoke these by hand; not via `manta.yaml`) → wire
`environmentRecipes` in `manta.yaml` → `manta vm recipe doctor <id> --json` (free) → then the `--provision`
...
a long time, or need the user at the keyboard. Never create a Manta workspace or commit unless asked.

1. **Inspect the repo** for an existing `environmentRecipes` entry, `scripts/manta-vm/`, a state file, or setup
...
2. **Interview the user up front** — gather these choices and confirm them back before scaffolding
   ...
   - **Connection mode:** how Manta attaches to the environment — an **Manta server** (the VM runs
     `manta serve` and Manta pairs over its pairing URL; worked example §7f) or **SSH** (Manta connects to
   ...
   - **Provider:** Vercel Sandbox, Fly, Modal, an existing SSH host, … For non-obvious providers, also
     ...
     (host/port/user/key or proxy command) or only a provider-mediated interactive shell; Manta SSH mode
     needs the former.
...
7. **Wire the recipe** so `manta.yaml` points create/suspend/resume/destroy at the scripts (§8). The
   workspace composer reads `environmentRecipes` from the project's primary checkout of `manta.yaml`, **not** from
   a feature branch or worktree. So a recipe added only on a branch won't appear as a "Run on" option
   until that `manta.yaml` change is committed and merged to the project's primary branch. Tell the user
...
8. **Dry-run doctor** — `manta vm recipe doctor <recipe-id> --repo-path <repo> --json` (free, static; §9).
   Fix every failure before going live.
9. **[CHECKPOINT] Live self-test** — get the user's OK once, then run
   `manta vm recipe doctor <recipe-id> --provision --json` as a loop: it runs create → validates →
...
- **Connection mode** (Manta server vs SSH) confirmed with the user — see §1 step 2; it shapes the recipe.
...
- **Never snapshot a machine on which the Manta runtime has already run.** The first `manta serve` creates
  the runtime's user-data dir, and everything in it gets baked into the image and shared by every VM
  booted from it: the pairing keypair and device-token registry (`manta-devices.json`,
  `manta-e2ee-keypair.json`), `agent-session-authority.key`, and the build box's logs, terminal history
  ...
  directory first: `manta_user_data_path="${MANTA_USER_DATA_PATH:-${XDG_CONFIG_HOME:-$HOME/.config}/manta}"; rm -rf -- "$manta_user_data_path"`.
  This matches Manta's Linux precedence for custom and default paths; deleting a named file list will
  drift as Manta adds state.
...
This layer inherits §3's rule: if you started `manta serve` on the base or auth sandbox to smoke-test it,
delete the runtime's user-data dir (`~/.config/manta` on Linux) before re-snapshotting, or every workspace
...
A repo-local JSON file (e.g. `scripts/manta-vm/<provider>-state.json`) threads non-secret values between
...
Scaffold under `scripts/manta-vm/`. These are **shapes** — fill in the provider's real commands. All
...
bash`, `set -euo pipefail`, quoted paths. **Windows:** a bare `.sh` won't run — scaffold `.ps1`/`.cmd`
  or require WSL/Git-Bash and point `manta.yaml` at the right launcher.
...
Worked Vercel commands for this phase are in §7f. You run this script by hand (not via `manta.yaml`),
...
# name = manta-${MANTA_RECIPE_ID}-${MANTA_VM_INSTANCE_ID} (sanitized, length-capped)
...
# 3. remote exec: start manta serve in the background and read the recipe JSON it writes (see below)
...
**The exact `manta serve` invocation and its output (verified — do not improvise the flags).** Inside the
VM, run:
...
**Binary name:** in a VM built from source (the Phase-2 flow), run it as `pnpm exec manta-dev serve …`
from the repo root — `manta-dev` is the in-repo entrypoint and is what the §7f example uses. Plain
`manta serve …` is the same command when the built CLI is installed on the VM's PATH. The flags/output
...
#!/usr/bin/env bash
set -euo pipefail
payload="$(cat)"                       # Manta passes lifecycle JSON on stdin
...
# destroy: provider remove "$resource_id"   (or set destroy: none in manta.yaml)
```
...
vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 20m \
  --env "GH_TOKEN=$gh_token" --env "MANTA_PROJECT_ROOT=$project_root" \
  --env "MANTA_REPO_URL=$repo_url" --env "MANTA_REPO_REF=$repo_ref" \
  -- bash -lc 'set -euo pipefail; cd "$MANTA_PROJECT_ROOT"; \
    ...
    git fetch origin "$MANTA_REPO_REF"; \
    git checkout -B "$MANTA_REPO_REF" FETCH_HEAD; \
    rm -f /tmp/askpass.sh; \
    c="$(git rev-parse HEAD)"; [ -f .manta-built ] && [ "$(cat .manta-built)" = "$c" ] || { \
      ...
      printf "%s" "$c" > .manta-built; }' >&2

# 3. (remote) start manta serve in the background, writing recipe JSON to a file; poll until it parses
recipe_json="$(vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 60s \
  --env "MANTA_PORT=$port" --env "MANTA_PROJECT_ROOT=$project_root" --env "MANTA_PAIRING_ADDRESS=$pairing_ws" \
  -- bash -lc 'set -euo pipefail; cd "$MANTA_PROJECT_ROOT"; rm -f /tmp/manta-recipe.json /tmp/manta-serve.log; \
    nohup pnpm exec manta-dev serve --port "$MANTA_PORT" --project-root "$MANTA_PROJECT_ROOT" \
      --pairing-address "$MANTA_PAIRING_ADDRESS" --recipe-json >/tmp/manta-recipe.json 2>/tmp/manta-serve.log </dev/null & \
    pid=$!; for _ in $(seq 1 80); do \
      node -e "JSON.parse(require(\"node:fs\").readFileSync(\"/tmp/manta-recipe.json\",\"utf8\"))" >/dev/null 2>&1 && { cat /tmp/manta-recipe.json; exit 0; }; \
      kill -0 "$pid" 2>/dev/null || { cat /tmp/manta-serve.log >&2; exit 1; }; sleep 0.25; \
    done; cat /tmp/manta-serve.log >&2; echo "serve recipe JSON timed out" >&2; exit 1')"
...
`userData.resourceId` from stdin (§7d). This is the **Manta-server** connection mode (the recipe emits a
...
- **`create` does NOT run `manta serve` and does NOT emit a `pairingCode`.** Manta itself connects to the
  host over its SSH relay, brings up the git + filesystem providers, and imports the repo. The script's
  only job is to make the host ready and **print SSH connection details** Manta will dial.
- The result uses a `connection` block with `type: "ssh"` and a `target`, **not** the flat
  `pairingCode`/`projectRoot` shape. Exact shape (Manta rejects anything else):
...
`MANTA_RECIPE_RESULT_SCHEMA_VERSION`, `MANTA_REPO_URL`, `MANTA_REPO_REF`, `MANTA_REPO_REF_HEAD`, and
`MANTA_REPO_BRANCH`. Use `MANTA_REPO_REF` to fetch the selected source, but create
`MANTA_REPO_BRANCH` at the exact `MANTA_REPO_REF_HEAD` commit; resolving the symbolic ref again can race
with an upstream update. `MANTA_REPO_URL` and `MANTA_REPO_REF` are a matched fetch pair, including when
the desktop source uses multiple remotes. Return that primary checkout at `projectRoot` and emit the
same SSH result with:

```bash
[ -n "${MANTA_REPO_REF_HEAD:-}" ] || { echo "missing pinned source commit" >&2; exit 1; }
git fetch origin "$MANTA_REPO_REF"
git cat-file -e "${MANTA_REPO_REF_HEAD}^{commit}"
git checkout -B "$MANTA_REPO_BRANCH" "$MANTA_REPO_REF_HEAD"
```
...
- `relayGracePeriodSeconds` (optional): how long Manta keeps the SSH relay alive after the workspace
...
# 1. ensure the repo is present and at the right commit on the host (NO manta serve here)
...
# 2. print the SSH connection block (NO pairingCode, NO manta serve). host/port/username tell Manta's
...
`destroy: none` and omit suspend/resume. (Manta still disconnects/reconnects its own SSH relay on
...
`connection.type:"ssh"` block above instead of starting `manta serve`.
...
- If committing from an interactive shell, force the runtime entrypoint back to `sshd`:
  `docker commit --change='ENTRYPOINT ["/usr/local/bin/manta-docker-ssh-entrypoint"]' …`.
...
docker run -d --name "$name" -p 127.0.0.1::22 -e "MANTA_SSH_PUBLIC_KEY=$pubkey" "$auth_image"
...
require WSL/Git-Bash (and point `manta.yaml` at e.g. `bash ./scripts/manta-vm/<name>.sh` via a `.cmd`
...
# Manta-server mode: @{ schemaVersion=1; pairingCode=$pairingCode; projectRoot=$projectRoot; userData=@{...} }
...
environmentRecipes:
  - id: cloud-sandbox
    name: Cloud Sandbox
    create: ./scripts/manta-vm/cloud-sandbox-create.sh
    suspend: ./scripts/manta-vm/cloud-sandbox-suspend.sh
    resume: ./scripts/manta-vm/cloud-sandbox-resume.sh
    destroy: ./scripts/manta-vm/cloud-sandbox-destroy.sh
```
...
**Manta-server mode** — boot the env, start `manta serve` in it, and print serve's result:

```json
{
  "schemaVersion": 1,
  "pairingCode": "manta-pairing-code-or-url",
  "projectRoot": "/absolute/path/to/repo/on/remote",
  "userData": { "provider": "example", "resourceId": "provider-resource-id" }
}
```

Here `pairingCode` (from `manta serve --recipe-json`) and `projectRoot` are required; `schemaVersion` (`1`)
and `userData` are optional.

**SSH mode** — do **not** run `manta serve`; print the `connection.type:"ssh"` block instead (full shape +
...
`checkoutMode: provisioned-root` to the recipe, require `MANTA_RECIPE_RESULT_SCHEMA_VERSION=2`, create
the requested `MANTA_REPO_BRANCH` at the pinned `MANTA_REPO_REF_HEAD` commit (use `MANTA_REPO_REF` only
...
Start Manta remotely with `manta serve --port "$PORT" --project-root "$ABS_ROOT" --pairing-address
...
`manta vm recipe doctor <recipe-id> --repo-path <repo> --json` validates **static wiring only** — it does
...
`manta vm recipe doctor <recipe-id> --repo-path <repo> --provision --json` actually runs the recipe end
...
- Don't make Manta own provider lifecycle beyond invoking the configured scripts.
- Don't commit or create a Manta workspace unless asked.
