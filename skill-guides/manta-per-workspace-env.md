...
**Result:** a repo-owned `environmentRecipes` entry in `manta.yaml`, the provider lifecycle
scripts under `scripts/manta-vm/` it points at, and an authenticated base snapshot recorded in a
state file.

**Next consumer:** the Manta workspace composer. It reads `environmentRecipes` from the project's
registered checkout, offers the recipe as a "Run on" target, and runs
`create`/`suspend`/`resume`/`destroy` against it.

**Done:** `MANTA vm recipe doctor <recipe-id> --repo-path <repo> --provision --json` returns
...
`MANTA` in every example is the executable you used to run `skills get`. Substitute it before
running; do not make a shell variable or run `MANTA` literally. Inside the lifecycle scripts the
placeholder does not apply: `manta serve` written there runs on the remote machine's own binary.

## Autonomy envelope

Without asking again you may read the repo and its `manta.yaml`, detect provider CLIs and their
login state, scaffold and edit files under `scripts/manta-vm/`, and run `MANTA vm recipe doctor`
...
done. Never create a Manta workspace except for the step-10 test the user asked for. Never
...
In **Manta-server** mode `create` runs `manta serve` in the environment and emits a `pairingCode`. In
**SSH** mode `create` runs no server and emits a `connection.type:"ssh"` block Manta dials into.
...
let Manta create a linked worktree. Use `checkoutMode: provisioned-root` only when the user
...
1. **Inspect the repo** for an existing `environmentRecipes` entry, `scripts/manta-vm/`, a state
...
2. **Interview the user up front.** Gather these choices and confirm them back before scaffolding
   ...
   - **Connection mode:** a Manta server or SSH, as above. Settle it first.
   ...
   - **Provider:** Vercel Sandbox, Fly, Modal, an existing SSH host, and so on. For a non-obvious
     ...
     Manta's SSH mode needs the former.
...
7. **Wire the recipe** so `manta.yaml` points create, suspend, resume, and destroy at the scripts.
   ...
   any branch; the picker needs `manta.yaml` on the primary branch.
...
- Trap errors and remove the half-built environment, so a crash does not leave a paid resource
  running.
- **Never snapshot a machine on which the Manta runtime has already run.** The first `manta serve`
  ...
  (`manta-devices.json`, `manta-e2ee-keypair.json`), `agent-session-authority.key`, and the build
  ...
  `manta_user_data_path="${MANTA_USER_DATA_PATH:-${XDG_CONFIG_HOME:-$HOME/.config}/manta}"; rm -rf -- "$manta_user_data_path"`.
  That matches Manta's Linux precedence for custom and default paths; deleting a named file list
  drifts as Manta adds state.
...
Section 3's rule still applies: if you ran `manta serve` on this machine to smoke-test it, delete
...
A repo-local JSON file such as `scripts/manta-vm/<provider>-state.json` threads non-secret values
...
Scaffold under `scripts/manta-vm/`. These are shapes; fill in the provider's real commands. **Every
...
You run this by hand, not via `manta.yaml`, after exporting the first-run inputs state does not have
...
# 3. Manta-server mode only: remote exec starting manta serve and reading the recipe JSON it writes
...
The base result, which is what Manta-server mode prints:
...
- **`manta serve --recipe-json` output** is this same object without `userData`. Merge your own
...
- **SSH mode** replaces `pairingCode` and `projectRoot` with a `connection` block whose `type` is
  `"ssh"`, and does not run `manta serve`. The exact target shape is in `references/ssh-host.md`.
- **Provisioned root** applies only to direct SSH and only when the user explicitly asked for it. Add
  `checkoutMode: provisioned-root` to the recipe, require `MANTA_RECIPE_RESULT_SCHEMA_VERSION=2`, and
...
### The `manta serve` invocation

Inside the environment, in Manta-server mode, run exactly this. These flags are verified; do not
improvise them.
...
In an environment built from source, run it as `pnpm exec manta-dev serve …` from the repo root;
`manta-dev` is the in-repo entrypoint. Plain `manta serve …` is the same command when the built CLI is
...
`MANTA vm recipe doctor <recipe-id> --repo-path <repo> --json` validates static wiring only; it boots
...
| The recipe connects over SSH instead of starting `manta serve`, including provisioned root | `references/ssh-host.md` |
