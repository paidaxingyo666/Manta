...
This is the Manta-server connection mode: the recipe emits a pairing URL. If the user chose SSH in
...
in both scripts and arm the trap before creating their temporary sandbox.
`${vercel_args[@]+"${vercel_args[@]}"}` rather than the plain form: macOS still ships bash
3.2, where `set -u` treats an empty array expansion as an unbound variable and the script
dies before it can report anything. Keep it armed through
...
max_recipe_id_length=$((128 - ${#instance_id} - 7))  # 'manta-' plus the joining '-'.
