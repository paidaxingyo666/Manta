...
driven from the CLI while the live stream stays visible in Manta's emulator pane.
...
`MANTA emulator exec --command "<adb shell command>"`, which runs
...
- Android Studio or the Android SDK installed, with `ANDROID_HOME` or `ANDROID_SDK_ROOT`
  set. Manta also checks the per-OS default location (`%LOCALAPPDATA%\Android\Sdk`,
...
- A booted, adb-visible device before any input or capability command. A shutdown AVD is
  listed with `state: shutdown` and must be started first, by `MANTA emulator attach`,
...
- `--device <serial>` such as `emulator-5554`, from `MANTA emulator devices`. An AVD name
...
- `MANTA emulator devices` is global and lists every backend; the other verbs route to the
  backend that owns the resolved device.

## Constraints

- All coordinates are normalized 0..1 with a top-left origin, never pixels. Manta scales them
...
MANTA emulator attach emulator-5554 --json
MANTA emulator tap 0.5 0.85 --json
MANTA emulator type "hello world" --json
MANTA emulator button recents --json
MANTA emulator install ./app-debug.apk --reinstall --json
MANTA emulator launch com.acme.app --json
MANTA emulator permissions grant com.acme.app android.permission.CAMERA --json
MANTA emulator ax --json
MANTA emulator logcat --lines 100 --json
MANTA emulator kill --json
```

## Next action

Run `MANTA emulator devices --json` to find a booted device, attach it, then drive it while
