...
whenever `MANTA_E2E_HEADLESS=1`, `MANTA_E2E_HEADFUL=1`, or `MANTA_BACKGROUND_LAUNCH=1`:
...
- Launch through `tests/e2e/helpers/manta-app.ts` (or `manta-restart.ts`) — they already set the env.
- A raw `electron.launch()` outside those helpers must pass `MANTA_BACKGROUND_LAUNCH: '1'`.
...
- `MANTA_E2E_FOREGROUND=1` is the only opt-out, for runs whose subject _is_ native focus (IME and
  other OS-level key injection). Clear `MANTA_BACKGROUND_LAUNCH` for that isolated run and add a
