 ...
 * parses `MANTA_STARTUP_DIAGNOSTICS=1` milestone lines from stderr, and
 ...
 *     [--exe <path-to-packaged-Manta>] [--timeout-ms 240000]
 *
 ...
 * tiny. Layout mirrors Chromium cache dirs plus a few Manta-owned dirs.
 */
  ...
  for (let i = 0; i < githubRepos; i++) {
    ...
    const remote = spawnSync(
      ...
      [
        ...
        'add',
        'origin',
        `https://github.com/manta-bench/bench-gh-repo-${i}.git`
      ],
      { stdio: 'ignore' }
    )
...
function writePersistedStateFixture(fixtureDir, { stateProfile, sessionTabs, githubRepos }) {
  const dataPath = join(fixtureDir, 'manta-data.json')
