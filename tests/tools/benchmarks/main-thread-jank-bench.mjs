#!/usr/bin/env node
/**
 * Manta main-thread jank / subprocess-churn benchmark (issue #7576).
 *
 ...
 * launched with MANTA_MAIN_THREAD_DIAGNOSTICS=1 so the main process emits one
 ...
 *     [--exe <path-to-packaged-Manta>] [--headless] [--no-log-stream]
 *
 ...
 * the shape Manta worktrees commonly have, which forces the effective-upstream
...
function ensureFixture(fixtureDir) {
  ...
  writeFileSync(join(fixtureDir, 'manta-data.json'), JSON.stringify(state, null, 2))
  return repoPath
}
...
async function main() {
  ...
  const env = {
    ...
    MANTA_MAIN_THREAD_DIAGNOSTICS: '1',
    MANTA_E2E_USER_DATA_DIR: fixtureDir,
    HOME: isolatedHome,
