...
terminal activity. The launcher sets `MANTA_BACKGROUND_LAUNCH=1`; tests never need to
...
`MANTA_BACKGROUND_LAUNCH=1` in the environment:

```sh
MANTA_TYPING_BENCH=1 MANTA_BACKGROUND_LAUNCH=1 pnpm exec playwright test tests/e2e/terminal-typing-measurement-control.spec.ts --config tests/playwright.config.ts --project electron-headless --workers=1
