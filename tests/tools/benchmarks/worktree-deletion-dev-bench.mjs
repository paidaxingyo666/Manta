#!/usr/bin/env node
/**
 * A/B benchmark for worktree deletion against real Manta dev instances.
 *
 * Usage:
...
function createFixture(instanceLabel) {
  const root = mkdtempSync(path.join(os.tmpdir(), `manta-delete-bench-${instanceLabel}-`))
  ...
  run('git', ['config', 'user.email', 'worktree-delete-bench@manta.invalid'], repoPath)
  run('git', ['config', 'user.name', 'Manta Worktree Delete Bench'], repoPath)
  writeFileSync(path.join(repoPath, 'README.md'), '# Manta worktree deletion benchmark\n')
...
function launchDevInstance({ label, repoRoot }, fixture, port) {
  ...
  const env = {
    ...process.env,
    MANTA_DEV_USER_DATA_PATH: fixture.userDataPath,
...
async function connectToManta(instance) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      try {
        ...
        // findMantaPage timeout drops a live browser handle and leaks a connection per retry.
        const page = await findMantaPage(browser)
        return { browser, page }
      } catch (error) {
...
async function findMantaPage(browser) {
  ...
  throw new Error('Manta renderer with window.__store was not found')
}
...
async function verifyRestart(instanceConfig, fixture, port, repoId) {
  ...
  let browser = null
  try {
    const connection = await connectToManta(relaunched)
...
async function benchmarkInstance(instanceConfig, index, options) {
  ...
  try {
    ...
    const { browser: connectedBrowser, page } = await connectToManta(instance)
