...
function makeSyntheticInventory() {
  ...
  return {
    appDir: 'C:\\App',
    unpackedRoot,
    hostExe: { name: 'Manta.exe', ...f('C:\\App\\Manta.exe') },
    runtimeData: [
...
function run() {
  ...
  for (const tier of TIERS) {
    ...
    check(`${tier}: has exe`, dests.includes('Manta.exe'))
  ...
  // ── Module-path filter ──────────────────────────────────────────────
  const appDir = 'C:\\Users\\me\\AppData\\Local\\Programs\\manta'
  const modules = [
    'C:\\Users\\me\\AppData\\Local\\Programs\\manta\\Manta.exe',
    'C:\\Windows\\System32\\kernel32.dll',
    'C:\\Users\\me\\AppData\\Local\\manta-daemon-host\\Manta.exe'
  ]
  ...
  check(
    ...
    findAppDirResidentModules(['C:\\a\\manta-daemon-host\\x.dll'], 'C:\\a\\manta').length === 0
  )
  // Forward/back-slash + case normalization.
  check(
    'slash + case normalized',
    findAppDirResidentModules(['c:/a/MANTA/x.dll'], 'C:\\A\\manta').length === 1
  )
  check(
    'relocated host has zero app-dir modules',
    findAppDirResidentModules(
      ['C:\\work\\daemon-host\\Manta.exe', 'C:\\Windows\\System32\\ntdll.dll'],
      appDir
    ).length === 0
  )
