...
async function callManta(cli, args, options = {}) {
...
function buildLongSpec(sizeKb, marker) {
  ...
  const header = [
    `MANTA_LONG_PROMPT_REPRO_START ${marker}`,
  ...
  let index = 0
  while (
    Buffer.byteLength(`${lines.join('\n')}\nMANTA_LONG_PROMPT_REPRO_END ${marker}`, 'utf8') <
    targetBytes
  ) {
    lines.push(
  ...
  lines.push(`MANTA_LONG_PROMPT_REPRO_END ${marker}`)
  return lines.join('\n')
}
...
async function parentMain() {
  ...
  const tempDir = path.join(tmpdir(), `manta-orchestration-long-prompt-${process.pid}-${Date.now()}`)
  await mkdir(tempDir, { recursive: true })
  const workerReportPath = path.join(tempDir, 'worker-report.json')

  console.log(`runtime: ${cli} status`)
  await callManta(cli, ['status'], { cwd })

  const coordinator = await callManta(
    cli,
    [
      'terminal',
      'create',
  ...
  try {
    ...
    const dispatch = await callManta(
      cli,
      [
        'orchestration',
        'dispatch',
...
async function fakeWorkerMain() {
  ...
  const finish = async (reason) => {
    ...
    process.stdout.write(
      `\nMANTA_REPRO_REPORT ${report.contractOk ? 'ok' : 'failed'} ${reportPath}\n`
    )
...
The parent mode requires a running Manta runtime and creates temporary Manta
terminals. The fake worker records whether orchestration dispatch delivered a
long prompt in a safe agent-input contract.

Options:
  --cli <path>         Manta CLI command (default: MANTA_REPRO_CLI or manta)
