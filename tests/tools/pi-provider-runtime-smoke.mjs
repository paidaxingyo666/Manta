...
try {
  ...
  await writeFile(
    ...
    `export default function(pi){pi.registerProvider('manta-proof',{name:'Proof',baseUrl:'http://127.0.0.1:${server.address().port}/v1',apiKey:'fixture-only',api:'openai-completions',models:[{id:'local',name:'Proof',reasoning:false,input:['text'],cost:{input:0,output:0,cacheRead:0,cacheWrite:0},contextWindow:8192,maxTokens:256}]})}`
  )
  await writeFile(
    join(dir, 'settings.json'),
    JSON.stringify({ defaultProvider: 'manta-proof', defaultModel: 'local' })
  )
  const planned = planCommitMessageGeneration(
    { agentId: 'pi', model: 'manta-proof/local' },
