...
1. Launch an isolated Manta development instance with CDP using the Electron skill.
2. Open one terminal in a git worktree or folder workspace. Start Pi with Manta's
   generated status extension and this additional extension:

   ```sh
   pi --offline --no-session -e /absolute/path/to/manta/tests/tools/pi-ui-prompt-extension.mjs
   ```

   If launching Pi directly through `node` or disabling extension discovery, explicitly
   load Manta's generated `manta-agent-status.ts` with another `-e` argument.

3. Leave Pi at its input editor, then run from the Manta repository:
