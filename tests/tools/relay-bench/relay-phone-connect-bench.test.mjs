...
describe('decodeOffer', () => {
  it('decodes a well-formed pairing link', () => {
    ...
    expect(decodeOffer(`manta://pair?code=${encode(offer)}`)).toEqual(offer)
  })

  it('ignores parameters after the code', () => {
    const offer = { deviceToken: 'token' }
    expect(decodeOffer(`manta://pair?code=${encode(offer)}&v=2`)).toEqual(offer)
  })

  it.each([
    [undefined, /manta:\/\/pair/],
    ['', /manta:\/\/pair/],
    ['https://example.com/?code=abc', /manta:\/\/pair/],
    ['manta://pair', /no code= parameter/],
    ['manta://pair?code=', /not base64url/],
    ['manta://pair?code=not base64', /not base64url/],
    [`manta://pair?code=${Buffer.from('not json').toString('base64url')}`, /did not decode to JSON/],
    [`manta://pair?code=${Buffer.from('[1,2]').toString('base64url')}`, /offer object/],
    [`manta://pair?code=${Buffer.from('null').toString('base64url')}`, /offer object/]
