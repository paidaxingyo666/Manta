...
The `manta://pair?code=...` value in that output is the pairing link.
...
umask 077 && printf '%s' '<manta://pair?code=...>' > /tmp/relay-bench/pair.txt
...
`MANTA_DEV_USER_DATA_PATH`, removing that directory drops the pairing with it.
