// Drive the installed, packaged Manta app with Playwright's Electron driver.
//
...
/**
 * Launch the installed Manta.exe. Pointing userDataDir at a harness-owned temp
 * dir isolates this run's daemon (its socket/token path becomes unique), so
 * daemon lookups never collide with other Manta installs/daemons on the box.
 * Pass `seedProfile` (a buildFreshProfile object) to write manta-data.json
...
export async function launchInstalledApp({
  ...
  const {
    ...
    MANTA_CODEX_HOME: _mantaCodexHome,
    ...cleanEnv
  } = process.env
  void _drop
  void _codexHome
  void _mantaCodexHome
  ...
  const app = await electron.launch({
    ...
    env: {
      ...cleanEnv,
      // Packaged main honors MANTA_E2E_USER_DATA_DIR to relocate userData
...
const CLI_FEATURE_TIP_TITLE = 'Let agents drive Manta with the Manta CLI'
