...
/**
 * Read-only discovery of the machine's current Manta install registry state.
...
export function discoverInstallRegistryState() {
  const ps = [
    ...
    `  if ($dn -eq 'Manta' -or ($il -and $il -match '\\\\Programs\\\\manta')) {`,
    ...
    `  if ($il -and ($sn -eq 'Manta' -or $il -match 'manta')) {`,
...
/** The two directories NSIS writes Manta shortcuts into: Start Menu + Desktop. */
...
/** Read-only: list Manta *.lnk shortcuts under the given dirs (recursive). */
export function discoverMantaShortcuts(dirs = shortcutDirs()) {
  const found = []
  for (const dir of dirs) {
    collectMantaLnks(dir, found)
  }
  return found
}

function collectMantaLnks(dir, out) {
  if (!existsSync(dir)) {
    return
  }
  let entries = []
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      collectMantaLnks(full, out)
    } else if (entry.isFile() && /\.lnk$/i.test(entry.name) && /manta/i.test(entry.name)) {
      out.push(full)
    }
  }
}
...
/**
 * Snapshot the shared install registry keys + Manta shortcuts before an isolated
...
export function backupInstallState(runDir) {
  ...
  let i = 0
  for (const lnk of discoverMantaShortcuts(dirs)) {
 ...
 * Pre-existing shortcuts are copied back; test-created Manta shortcuts are removed.
...
function restoreShortcuts(manifest, result) {
  ...
  for (const lnk of discoverMantaShortcuts(manifest.shortcutDirs)) {
...
function printMismatchWarning(manifest, expected, actual) {
  ...
  console.error('!! Your REAL Manta install pointer may be hijacked to the test directory.')
  console.error('!! The next real Manta update could install into the test location.')
...
if (process.argv[1] && path.resolve(process.argv[1]) === import.meta.filename) {
  ...
  console.log('[registry-shortcut-backup] Manta shortcuts that would be backed up:')
  console.log(JSON.stringify(discoverMantaShortcuts(), null, 2))
}
