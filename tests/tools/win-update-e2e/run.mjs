...
//
// Given two Manta Windows installers (version N and N+1), proves what happens to
...
async function main() {
  ...
  const canary = `MANTA-E2E-CANARY-${runId}`
  const runDir = mkdtempSync(path.join(tmpdir(), `manta-win-update-e2e-${runId}-`))
...
function resolveScopedDaemon(userDataDir) {
  ...
  return {
    ...
    // install-dir Manta.exe — the key survival signal.
...
async function teardown({
  ...
  if (hadPreexistingInstall) {
    console.log(
      '\n*** NOTE: a Manta install existed before this run. It was OVERWRITTEN and the\n' +
