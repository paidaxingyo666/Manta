// Persistence seed for a fresh packaged Manta profile: dismisses onboarding and
...
// manta-data.json` BEFORE launch dismisses it. The app derives the Projects list
...
//
// IMPORTANT: seed only BEFORE the first launch. The app rewrites manta-data.json
 ...
 * for it. A real checkout (init + one commit) is required — Manta treats a
...
export function createSeededRepo(dir) {
  ...
  git('init', '-b', 'main')
  git('config', 'user.email', 'win-update-e2e@manta.test')
...
export function seedFreshProfile(userDataDir, profile) {
  ...
  writeFileSync(path.join(userDataDir, 'manta-data.json'), `${JSON.stringify(profile, null, 2)}\n`)
}
