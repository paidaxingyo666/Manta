...
export default function (pi) {
  pi.registerCommand('orca-modal', {
    description: 'Verify Manta status: select, confirm, input, editor, or custom',
    handler: async (args, ctx) => {
      const kind = args.trim() || 'select'
      const title = `Manta verification: ${kind}`
      let answer
      switch (kind) {
        case 'select':
      ...
      ctx.ui.notify(`Manta verification: ${kind} ${answer === undefined ? 'cancelled' : 'answered'}`)
    }
  })
}
