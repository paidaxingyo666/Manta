...
//
// GitHub #7742: on Windows, when Manta's main/renderer process crashed, open
...
// manta-terminal-daemon.exe that survives main death (src/main/daemon/
...
const SENTINEL_ENV = 'MANTA_CRASH_SENTINEL'
...
async function main() {
  ...
  // clear platform message, not a confusing "no Manta.exe found" default-resolution
  // failure.
  ...
  const canary = `MANTA-CRASH-SENTINEL-${runId}`
  const runDir = mkdtempSync(path.join(tmpdir(), `manta-win-crash-e2e-${runId}-`))
  ...
  const diagDir = process.env.MANTA_E2E_DIAG_DIR || path.join(runDir, 'diag')
  let passed = false
  try {
 ...
 * daemons on the machine (including the developer's live Manta) are ignored.
 ...
 * touches any other Manta on the box (a live user instance uses a different
