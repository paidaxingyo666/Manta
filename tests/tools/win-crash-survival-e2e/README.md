...
On Windows, when Manta's main/renderer process crashed, open terminal PTYs were
...
1. **Launch** the installed `Manta.exe` under an isolated `userData` dir
   (`MANTA_E2E_USER_DATA_DIR`), seeded with a fresh profile (onboarding dismissed
...
2. **Stamp the interactive shell** — typing DIRECTLY into it (not a nested
   `powershell`), set a per-shell env sentinel `MANTA_CRASH_SENTINEL=<canary>` and
...
6. **Relaunch** (same `userData`, no reseed) and assert the daemon PID is
   **unchanged** (the new main **adopts** the surviving daemon instead of forking
   ...
   `MANTA_CRASH_SENTINEL` and the shell's `$PID`, which a freshly re-spawned shell
   would not carry.
...
8. **Teardown** — close the relaunched app, then kill this run's scoped daemon
   **tree** (re-discovered fresh via `findDaemonProcesses(userData)`, which the
   ...
   process), never installs/uninstalls, and never touches any other Manta on the box.
...
node tests/tools/win-crash-survival-e2e/run.mjs --expect survival --exe-path "C:\Users\<you>\AppData\Local\Programs\manta\Manta.exe"
```
...
| `--exe-path <path>`  | Installed `Manta.exe` to drive (default: per-user install under `%LOCALAPPDATA%\Programs\Manta`) |
...
- The crash kills **only** the real Electron main of the instance this harness
  ...
  name or a scanned pid, so a developer's live Manta (a different `userData`, out of
  scope) is untouched.
