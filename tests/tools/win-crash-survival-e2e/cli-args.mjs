...
// installed, packaged Manta.exe. So its only source is `--exe-path` (defaulting
...
Proves that force-killing ONLY Manta's main process (a real crash, no tree-kill)
...
Usage:
  node tests/tools/win-crash-survival-e2e/run.mjs --expect <profile> [--exe-path <Manta.exe>] [options]
...
Options:
  --exe-path <path>        Installed Manta.exe to drive (default: the per-user
                           install under %LOCALAPPDATA%\\Programs\\Manta). The
...
export function parseArgs(argv) {
  ...
  const opts = {
    ...
    // "no Manta.exe found" default-resolution error.
    exePath:
...
function validate(opts, exePathFlagPresent, argv) {
  ...
  if (exePathFlagPresent && takeValue(argv, '--exe-path') === undefined) {
    ...
    errors.push(
      'No installed Manta.exe found under %LOCALAPPDATA%\\Programs — pass --exe-path <Manta.exe>'
    )
