...
and calls Manta's production `shutdownLocalPty` and `killAllLocalPtys` functions,
...
```sh
MANTA_BACKGROUND_LAUNCH=1 ORCA_OMP_PROBE_BINARY=/absolute/path/to/omp \
...
At Manta base `93c370246388`, macOS arm64, installed `omp/18.1.18`:
...
On macOS, installed Manta `1.4.202-hourly.202609132311` and OMP `18.1.18`, an actual
...
Do not close #9530 from this probe. The original report did not identify Manta/OMP
