...
function subscribeLikeManta(dir) {
  ...
  return client
    .subscribeViaWatcherProcess(
      ...
      (err, events) => {
        if (err) {
          stats.watchErrors++
          // Manta's error path: unsubscribe from inside the error callback.
          if (subRef.current) {
            stats.unsubscribes++
...
async function deleteRootLane(baseDir, durationMs, lane) {
  ...
  while (Date.now() < end) {
    ...
    let subRef
    try {
      subRef = await subscribeLikeManta(dir)
    } catch {
      rmrf(dir)
      round++
      continue
...
async function main() {
  ...
  const baseDir = path.join(os.tmpdir(), 'manta-7547-harness-fixed', `run-${process.pid}`)
