...
// watches our exit code). Each scenario mimics a real Manta usage pattern:
//
      ...
      sub = await watcher.subscribe(
        ...
        (err, events) => {
          if (err) {
            recordError(err)
            errored = true
            // Manta's createWatcher error path: unsubscribe from inside the
            // error callback.
            if (sub) {
              stats.unsubscribes++
...
async function main() {
  ...
  // Outside the repo worktree: a live Manta instance may be watching the repo,
  // and this churn must not feed the production watcher.
  const baseDir = path.join(os.tmpdir(), 'manta-7547-harness', `run-${process.pid}`)
