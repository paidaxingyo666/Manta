...
test.describe('SSH host add/edit modal', () => {
  ...
  test.afterEach(async ({ mantaPage }) => {
    const ids = await listTargetIdsByLabelPrefix(mantaPage, HOST_PREFIX)
    if (ids.length > 0) {
      await removeSshTargets(mantaPage, ids)
    }
  })
  ...
  test('opens add/edit form in a viewport-stable dialog over a long host list', async ({
    ...
    const seeded = await seedSshTargets(mantaPage, 10)
    await openSshHostSettings(mantaPage)

    const sshSection = mantaPage.locator('[data-settings-section="ssh"]')
    ...
    const addDialog = mantaPage.getByRole('dialog', { name: 'Add SSH host' })
    ...
    const editDialog = mantaPage.getByRole('dialog', { name: 'Edit SSH host' })
    ...
    await mantaPage.locator('[data-slot="dialog-overlay"]').click({ position: { x: 8, y: 8 } })
    await expect(editDialog).toBeVisible()
    ...
    await createdCard.getByRole('button', { name: 'Edit target' }).click()
    const reopened = mantaPage.getByRole('dialog', { name: 'Edit SSH host' })
  ...
  test('add-ssh-host settings intent opens the same modal dialog', async ({ mantaPage }) => {
    ...
    await expect(mantaPage.getByPlaceholder('Search settings')).toBeVisible({ timeout: 10_000 })
    await dismissTransientAnnouncement(mantaPage)

    const dialog = mantaPage.getByRole('dialog', { name: 'Add SSH host' })
