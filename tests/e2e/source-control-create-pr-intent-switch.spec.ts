...
test.describe('Source Control Create PR intent worktree switching', () => {
  ...
  test('keeps Create PR intent running after switching worktrees', async ({
        ...
        store.setState((current) => ({
          ...
          createHostedReview: async (repoPath, input) => {
            ...
            return {
              ok: true as const,
              number: 74,
              url: 'https://github.com/acme/manta/pull/74'
            }
          },
          gitStatusByWorktree: {
    ...
    const createPr = mantaPage.getByRole('button', { name: 'Create PR' }).first()
    ...
    const completedWhileSwitchedEvidence = await mantaPage.evaluate(() => {
  ...
  test('carries unavailable dirty intent through push to the final create preflight', async ({
    ...
    await waitForSessionReady(mantaPage)
    await waitForActiveWorktree(mantaPage)
    const { prWorktreeId, prWorktreePath } = await seedCreatePrComposer(mantaPage)
    const remoteRoot = mkdtempSync(path.join(os.tmpdir(), 'manta-e2e-create-pr-remote-'))
          ...
          repos: current.repos.map((repo) =>
            repo.id === worktree.repoId
              ? {
                  ...repo,
                  gitRemoteIdentity: {
                    canonicalKey: 'github.com/acme/manta',
                    remoteName: 'origin',
                    remoteUrl: 'https://github.com/acme/manta.git'
                  }
                }
              : repo
          ),
    ...
    await expect
      ...
      .toBe(true)
    await expect(mantaPage.getByText(finalCreateError)).toBeVisible({ timeout: 10_000 })
  })
})
