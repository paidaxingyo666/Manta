...
async function createGitRepo(): Promise<string> {
  const root = realpathSync.native(await mkdtemp(path.join(os.tmpdir(), 'manta-e2e-golden-fresh-')))
...
test('fresh profile opens a live project terminal @golden', async ({
  ...
  await expect(mantaPage.locator('#root')).toBeVisible()
  await expect(mantaPage.getByRole('heading', { name: /Pick your default agent/i })).toBeVisible()

  await selectCodexAndSkipToProject(mantaPage)
  ...
  await expect(mantaPage.getByText(path.basename(repoPath), { exact: true }).first()).toBeVisible({
    timeout: 30_000
  })
  ...
  await expect.poll(() => mantaPage.evaluate((id) => window.api.pty.hasPty(id), ptyId)).toBe(true)

  const marker = `manta-e2e-fresh-${Date.now()}`
  ...
  await focusActiveTerminalInput(mantaPage)
  await mantaPage.keyboard.type('git rev-parse --show-toplevel')
  await mantaPage.keyboard.press('Enter')
  await expect
    .poll(async () => (await getTerminalContent(mantaPage)).replaceAll('\\', '/'), {
