...
function largePastePayload(runId: string): string {
  return `MANTA_LARGE_PASTE_${runId}_0123456789abcdef`.repeat(4096)
}
...
test.describe('large terminal paste responsiveness', () => {
  test('chunked keyboard paste keeps the renderer responsive while PTY writes are pending', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.manta-large-paste-${runId}.cjs`)
    ...
    try {
      ...
      scriptStarted = true
      await waitForTerminalOutput(mantaPage, `LARGE_PASTE_READY_${runId}`, 10_000)
      ...
      await installRendererHeartbeat(mantaPage)
      await focusActiveTerminalInput(mantaPage)

      const pasteKey = mantaPage.keyboard.press(keyboardPasteChord())
      await expect
        .poll(
          async () =>
      ...
      const heartbeatBefore = await readRendererHeartbeat(mantaPage)
      await mantaPage.waitForTimeout(150)
      const heartbeatAfter = await readRendererHeartbeat(mantaPage)
      expect(heartbeatAfter).toBeGreaterThan(heartbeatBefore)

      await pasteKey
      await waitForTerminalOutput(mantaPage, doneLine, 20_000, 12_000)
      ...
      await stopRendererHeartbeat(mantaPage).catch(() => undefined)
      if (scriptStarted) {
