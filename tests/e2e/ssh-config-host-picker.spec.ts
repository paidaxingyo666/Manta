...
test.describe('SSH config host picker', () => {
  ...
  test.afterEach(async ({ mantaPage }) => {
    await closeOpenDialogs(mantaPage).catch(() => undefined)
    await removeSshTargetsByPrefix(mantaPage, HOST_PREFIX).catch(() => undefined)
  })

  // ── P1 ─────────────────────────────────────────────────────────────
  test('P1: empty config shows empty state; Back returns to blank form', async ({ mantaPage }) => {
    ...
    await expect(picker.getByRole('button', { name: 'Add all to Manta' })).toBeDisabled()

    await picker.getByRole('button', { name: 'Back' }).click()
    const form = mantaPage.getByRole('dialog', { name: 'Add SSH host' })
  ...
  test('P3: select host prefills form; Save persists; N3 identity hint', async ({
    ...
    await expect(
      mantaPage.getByText(new RegExp(`Filled from ${escapeRegExp(prod.alias)}`, 'i'))
  ...
  // ── P4 ─────────────────────────────────────────────────────────────
  test('P4: filter narrows host list', async ({ electronApp, mantaPage }) => {
    const hosts = await seedPairConfig(electronApp, HOST_PREFIX)
    const picker = await openSshConfigHostPicker(mantaPage)
  ...
  // ── P8 ─────────────────────────────────────────────────────────────
  test('P8: Back without select leaves form empty', async ({ electronApp, mantaPage }) => {
    const hosts = await seedPairConfig(electronApp, HOST_PREFIX)
    const picker = await openSshConfigHostPicker(mantaPage)
    await expect(configHostRow(picker, hosts.alpha)).toBeVisible()
    ...
    await expect(fields.username).toHaveValue('')
    await expect(fields.label).toHaveValue('')
    await expect(mantaPage.getByText(/Filled from /i)).toHaveCount(0)
  })
})
