...
test.describe('SSH Agent Session History', () => {
  ...
  test('shows remote session history only for the SSH host and resumes Codex on that worktree', async ({
    ...
    try {
      ...
      const scan = await mantaPage.evaluate(
      ...
      expect(scan.remoteCommands.join('\n')).toContain(
        "CODEX_HOME='/root/.local/share/manta/codex-runtime-home/home'"
      )

      const defaultSessionTitle = mantaPage.getByText(defaultTitle, { exact: true })
      const runtimeSessionTitle = mantaPage.getByText(runtimeTitle, { exact: true })

      await openAiVaultSidebar(mantaPage)
      await expect(defaultSessionTitle.first()).toBeVisible({ timeout: 30_000 })

      const hostButton = mantaPage.getByRole('button', { name: /Session History host:/ })
      await hostButton.click()
      await mantaPage.getByRole('menuitemradio', { name: /Local/ }).click()
      await expect(defaultSessionTitle).toHaveCount(0, { timeout: 30_000 })

      await hostButton.click()
      await mantaPage.getByRole('menuitemradio', { name: 'All hosts' }).click()
      ...
      await installStartupQueueProbe(mantaPage)
      await defaultSessionTitle.first().click()
      await mantaPage.getByText('Resume in Worktree', { exact: true }).click()

      await expect
        .poll(() => readLastQueuedStartupCommand(mantaPage), { timeout: 30_000 })
        .toContain(`CODEX_HOME='/root/.codex' codex resume '${defaultSessionId}'`)
      const queuedWorktreeId = await readLastQueuedStartupWorktreeId(mantaPage)
...
function seedRemoteAiVaultHistory(
  ...
  dockerExec(
    ...
    [
      ...
      'mkdir -p /root/.local/share/manta/codex-runtime-home/home/sessions/2026/07/04',
      'mkdir -p /root/.claude/projects/manta'
    ].join(' && ')
  )
  dockerWriteFile(
    target,
  ...
  dockerWriteFile(
    target,
    `/root/.local/share/manta/codex-runtime-home/home/sessions/2026/07/04/${args.runtimeSessionId}.jsonl`,
    codexTranscript({
  ...
  dockerWriteFile(
    target,
    `/root/.claude/projects/manta/${args.claudeSessionId}.jsonl`,
    claudeTranscript({
