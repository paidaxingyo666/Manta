...
test('killing an agent PTY terminates its detached-pgid descendants', async ({ mantaPage }) => {
  test.skip(process.platform === 'win32', 'descendant tree-kill is POSIX-only for now')

  const stage = mkdtempSync(join(tmpdir(), 'manta-agent-descendant-'))
  ...
  try {
    ...
    const ptyId = await mantaPage.evaluate(
    ...
    await mantaPage.evaluate((id) => window.api.pty.kill(id), ptyId)
