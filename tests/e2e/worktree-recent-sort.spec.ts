...
test.describe('Worktree Recent Sort', () => {
  ...
  function createFolderFixture(): string {
    const dir = mkdtempSync(path.join(os.tmpdir(), 'manta-e2e-folder-'))
  ...
  test.afterEach(() => {
    ...
    // matches the mkdtempSync/rmSync pairing used in helpers/manta-app.ts
    // and helpers/manta-restart.ts.
  ...
  test('stamps lastActivityAt on a newly-added folder repo so it sorts to the top of Recent', async ({
    ...
    const repoId = await addFolderRepo(mantaPage, folderPath)
    const lastActivityAt = await readFolderWorktreeLastActivity(mantaPage, repoId)
  ...
  test('leaves lastActivityAt stable across repeated list refreshes', async ({ mantaPage }) => {
    ...
    const first = await readFolderWorktreeLastActivity(mantaPage, repoId)
    ...
    const second = await readFolderWorktreeLastActivity(mantaPage, repoId)
