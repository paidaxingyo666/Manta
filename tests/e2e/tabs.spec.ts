...
test.describe('Tabs', () => {
   ...
   */
  test('clicking "+" then "New Terminal" creates a new terminal tab', async ({ mantaPage }) => {
    ...
    const storeActiveId = await getActiveTabId(mantaPage)
    expect(storeActiveId).not.toBeNull()
    await expect(tabLocator(mantaPage, storeActiveId!)).toBeVisible()
    await expect.poll(() => getDomActiveTabId(mantaPage), { timeout: 3_000 }).toBe(storeActiveId)
    await expect
      .poll(() => getFocusedTerminalTabId(mantaPage), {
        timeout: 5_000,
  ...
  test('clicking "+" then "New Markdown" focuses the editor', async ({
    ...
    const preExistingFileIds = await mantaPage.evaluate(
    ...
    const createdFileHandle = await mantaPage.waitForFunction(
      (knownFileIds) => {
    ...
    await mantaPage.evaluate((fileId) => {
   ...
   * - New tab works
   */
  test('Cmd/Ctrl+T creates a new terminal tab', async ({ mantaPage }) => {
    ...
    await mantaPage.keyboard.press(`${mod}+t`)
   ...
   * right tab element.
   */
  test('Cmd/Ctrl+Shift+] and Cmd/Ctrl+Shift+[ switch between tabs', async ({ mantaPage }) => {
    ...
    if ((await countRenderedTabs(mantaPage)) < 2) {
      await mantaPage.getByRole('button', { name: 'New tab' }).click()
      await mantaPage
        .getByRole('menuitem', { name: /New Terminal/i })
        .first()
        .click()
      await expect
        .poll(() => countRenderedTabs(mantaPage), { timeout: 5_000 })
        .toBeGreaterThanOrEqual(2)
    }

    const firstTabId = await getActiveTabId(mantaPage)
    const orderedTabs = await getWorktreeTabs(mantaPage, worktreeId)
    ...
    await expect.poll(() => getDomActiveTabId(mantaPage), { timeout: 3_000 }).toBe(secondTabId)

    // Switch back.
    await mantaPage.evaluate((tabId) => {
      window.__store?.getState().setActiveTab(tabId)
    }, firstTabId)
    await expect.poll(() => getDomActiveTabId(mantaPage), { timeout: 3_000 }).toBe(firstTabId)
  })
   ...
   */
  test('dragging a tab to a new position reorders it', async ({ mantaPage }) => {
    const worktreeId = (await getActiveWorktreeId(mantaPage))!
    ...
    const domOrderBefore = await mantaPage.$$eval(SORTABLE_TAB, (nodes) =>
      nodes.map((n) => (n as HTMLElement).dataset.tabId ?? '')
    )
    expect(domOrderBefore.length).toBeGreaterThanOrEqual(2)

    await mantaPage.evaluate((targetWorktreeId) => {
    ...
    await expect
      .poll(
        async () =>
          mantaPage.$$eval(SORTABLE_TAB, (nodes) =>
            nodes.map((n) => (n as HTMLElement).dataset.tabId ?? '')
          ),
  ...
  test('clicking tabs still switches after dragging a terminal tab to reorder', async ({
    ...
    await tabLocator(mantaPage, firstTabId).click({ force: true })
    await expect.poll(() => getDomActiveTabId(mantaPage), { timeout: 3_000 }).toBe(firstTabId)

    const firstTabBox = await tabLocator(mantaPage, firstTabId).boundingBox()
    const secondTabBox = await tabLocator(mantaPage, secondTabId).boundingBox()
    ...
    await mantaPage.mouse.move(startX, startY)
    ...
    await mantaPage.mouse.move(endX, endY, { steps: 8 })
    ...
    await tabLocator(mantaPage, firstTabId).click({ force: true })
    await expect.poll(() => getDomActiveTabId(mantaPage), { timeout: 3_000 }).toBe(firstTabId)
    await tabLocator(mantaPage, secondTabId).click({ force: true })
    await expect
      .poll(() => getDomActiveTabId(mantaPage), {
        timeout: 5_000,
   ...
   */
  test('Cmd/Ctrl+Shift+[ walks tabs in drag-reordered order', async ({ mantaPage }) => {
    const isMac = process.platform === 'darwin'
    const mod = isMac ? 'Meta' : 'Control'
    const worktreeId = (await getActiveWorktreeId(mantaPage))!
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(mantaPage, worktreeId)).length, {
        timeout: 5_000
      })
      .toBeGreaterThanOrEqual(3)

    const initialOrder = await getTabBarOrder(mantaPage, worktreeId)
    ...
    await expect
      .poll(async () => getTabBarOrder(mantaPage, worktreeId), {
        timeout: 3_000
      })
      .toEqual([b, c, a])
    ...
    await expect.poll(() => getDomActiveTabId(mantaPage), { timeout: 3_000 }).toBe(a)

    await mantaPage.keyboard.press(`${mod}+Shift+BracketLeft`)
    await expect.poll(() => getDomActiveTabId(mantaPage), { timeout: 3_000 }).toBe(c)
    await expect(tabLocator(mantaPage, c)).toHaveAttribute('data-active', 'true')

    await mantaPage.keyboard.press(`${mod}+Shift+BracketLeft`)
    await expect.poll(() => getDomActiveTabId(mantaPage), { timeout: 3_000 }).toBe(b)
    await expect(tabLocator(mantaPage, b)).toHaveAttribute('data-active', 'true')
  })
   ...
   * re-render.
   */
  test('closing a tab removes it from the tab bar', async ({ mantaPage }) => {
    const worktreeId = (await getActiveWorktreeId(mantaPage))!
   ...
   */
  test('closing the active tab activates a neighbor tab', async ({ mantaPage }) => {
    const worktreeId = (await getActiveWorktreeId(mantaPage))!
    ...
    await expect.poll(() => getDomActiveTabId(mantaPage), { timeout: 5_000 }).not.toBeNull()
  })
})
