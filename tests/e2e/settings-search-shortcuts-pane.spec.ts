...
test.describe('Settings sidebar search on the Shortcuts pane', () => {
  test('pane-title-only query keeps rows visible and local search usable', async ({ mantaPage }) => {
    ...
    const searchInput = mantaPage.getByPlaceholder('Search settings')
    ...
    await expect(mantaPage.getByRole('heading', { name: 'Shortcuts', exact: true })).toBeVisible()

    // Regression: a pane-title-only query used to blank the whole list (0/112).
    await expect(mantaPage.getByText('Go to File', { exact: true })).toBeVisible()
    await expect(mantaPage.getByText('No shortcuts match those filters.')).not.toBeVisible()
    ...
    const localSearch = mantaPage.getByPlaceholder('Search command or keys')
    await localSearch.fill('go to')
    await expect(mantaPage.getByText('Go to File', { exact: true })).toBeVisible()
    await expect(mantaPage.getByText('Force Reload', { exact: true })).not.toBeVisible()

    // A row-matching global query still narrows the list as before.
    await localSearch.clear()
    await searchInput.fill('worktree')
    await expect(mantaPage.getByText('Create worktree', { exact: true })).toBeVisible()
    await expect(mantaPage.getByText('Go to File', { exact: true })).not.toBeVisible()
  })
})
