...
const TOGGLE_EVENT = 'manta-toggle-floating-terminal'
...
test.describe('floating workspace reopen WebGL recovery @headful', () => {
  test('reopening the floating workspace recovers a corrupted glyph atlas', async ({
    ...
    const shots = await setUpCorruptedFloatingTerminal(mantaPage, 'REOPEN')
    ...
    expect(await instrumentRecoveryCounters(mantaPage)).toBe(true)

    await toggleFloatingPanel(mantaPage, false)
    ...
    const webglAttachedWhileClosed = await mantaPage.evaluate((worktreeId) => {
      const state = window.__store?.getState()
      const tab = (state?.tabsByWorktree?.[worktreeId] ?? [])[0]
      const manager = tab ? window.__paneManagers?.get(tab.id) : null
      const diagnostics = manager?.getRenderingDiagnostics?.() ?? []
    ...
    await toggleFloatingPanel(mantaPage, true)
    await settleRecoveryWindows(mantaPage)

    const counters = await readRecoveryCounters(mantaPage)
    const afterReopen = await screenshotFloatingTerminal(mantaPage)
  ...
  test('system resume recovers the corrupted atlas (harness control)', async ({
    ...
    const shots = await setUpCorruptedFloatingTerminal(mantaPage, 'CONTROL')
    test.skip(!shots, 'WebGL was not active or atlas corruption could not be injected')
    const { baseline, corrupted } = shots!
    expect(corrupted.equals(baseline)).toBe(false)
    ...
    await electronApp.evaluate(({ BrowserWindow }) => {
      ...
      if (!mainWindow) {
        throw new Error('Manta window unavailable for system resume')
      }
      mainWindow.webContents.send('system:resumed')
    })
    await settleRecoveryWindows(mantaPage)

    const afterResume = await screenshotFloatingTerminal(mantaPage)
