...
test.describe('Wedged terminal write pipeline recovery', () => {
  test('pane recovers rendering and input after its write pipeline wedges', async ({
    ...
    await mantaPage.keyboard.type(`echo ${beforeMarker}`, { delay: 20 })
    await mantaPage.keyboard.press('Enter')
    await expect
      .poll(async () => (await getTerminalContent(mantaPage)).includes(beforeMarker), {
        timeout: 15_000,
        message: 'Baseline echo did not render — pane unhealthy before wedge'
      })
      .toBe(true)

    await wedgeActivePaneWritePipeline(mantaPage)
    ...
    await focusActiveTerminalInput(mantaPage)
    await mantaPage.keyboard.type(`echo ${afterMarker}`, { delay: 20 })
    await mantaPage.keyboard.press('Enter')

    await expect
      .poll(async () => (await getTerminalContent(mantaPage)).includes(afterMarker), {
  ...
  test('pane recovers after its xterm is disposed under live bindings (zombie pane)', async ({
    ...
    await sendToTerminal(mantaPage, ptyId, `echo ${outputMarker}\r`)

    await expect
      .poll(async () => (await getTerminalContent(mantaPage)).includes(outputMarker), {
        timeout: 45_000,
        message:
    ...
    await mantaPage.keyboard.type(`echo ${typedMarker}`, { delay: 20 })
    await mantaPage.keyboard.press('Enter')
    await expect
      .poll(async () => (await getTerminalContent(mantaPage)).includes(typedMarker), {
        timeout: 15_000,
