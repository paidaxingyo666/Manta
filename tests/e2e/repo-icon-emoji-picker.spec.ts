...
async function openRepoSettings(page: Page, repoId: string): Promise<void> {
  // Why: the host OS locale (e.g. ko-KR) drives Manta's default UI language.
...
test.describe('Repository icon emoji picker', () => {
  test('search selects a native emoji and persists it as the repo icon', async ({
    ...
    const repos = await getStoreState<Repo[]>(mantaPage, 'repos')
    expect(repos.length).toBeGreaterThan(0)
    const repo = repos[0]

    await openRepoSettings(mantaPage, repo.id)

    const repoSection = mantaPage.locator(`[data-settings-section="repo-${repo.id}"]`)
    ...
    await attachScreenshot(mantaPage, testInfo, 'repo-icon-emoji-picker-search')
    ...
    await expect
      .poll(
        async () => {
          const current = await getStoreState<Repo[]>(mantaPage, 'repos')
    ...
    await attachScreenshot(mantaPage, testInfo, 'repo-icon-emoji-picker-selected')
  })
})
