...
const fakeCliDir = mkdtempSync(path.join(os.tmpdir(), 'manta-live-mount-cli-'))
...
appendFileSync(process.env.MANTA_E2E_CODEX_SPAWN_LEDGER, JSON.stringify({ args, pid: process.pid }) + '\\n')
...
for (const signal of ['SIGINT', 'SIGHUP', 'SIGTERM']) process.on(signal, () => appendFileSync(process.env.MANTA_E2E_SIGNAL_LEDGER, JSON.stringify({ kind: 'agent', pid: process.pid, signal }) + '\\n'))
  ...
  launchEnv: [
    {
      ...
      MANTA_E2E_CODEX_SPAWN_LEDGER: spawnLedgerPath,
      MANTA_E2E_SETUP_LEDGER: setupLedgerPath,
      MANTA_E2E_CANARY_LEDGER: canaryLedgerPath,
      MANTA_E2E_SIGNAL_LEDGER: signalLedgerPath
    },
    { option: true }
  ]
})
...
function createSourceRepo(): string {
  const repoPath = mkdtempSync(path.join(os.tmpdir(), 'manta-live-mount-repo-'))
  writeFileSync(
    path.join(repoPath, 'setup-live.js'),
    `const { appendFileSync } = require('node:fs')\nappendFileSync(process.env.MANTA_E2E_SETUP_LEDGER, JSON.stringify({ pid: process.pid }) + '\\n')\nconsole.log('SETUP_READY:' + process.pid)\nlet inputBuffer = ''\nprocess.stdin.on('data', chunk => {\n  inputBuffer += chunk.toString()\n  const lines = inputBuffer.split(/[\\r\\n]+/)\n  inputBuffer = lines.pop() || ''\n  for (const line of lines) if (line) console.log('SETUP_INPUT:' + process.pid + ':' + line)\n})\nfor (const signal of ['SIGINT', 'SIGHUP', 'SIGTERM']) process.on(signal, () => appendFileSync(process.env.MANTA_E2E_SIGNAL_LEDGER, JSON.stringify({ kind: 'setup', pid: process.pid, signal }) + '\\n'))\nprocess.stdin.resume()\nsetInterval(() => {}, 60000)\n`
  )
  writeFileSync(
    path.join(repoPath, 'canary-live.js'),
    `const { appendFileSync } = require('node:fs')\nappendFileSync(process.env.MANTA_E2E_CANARY_LEDGER, JSON.stringify({ pid: process.pid }) + '\\n')\nconsole.log('CANARY_READY:' + process.pid)\nlet inputBuffer = ''\nprocess.stdin.on('data', chunk => {\n  inputBuffer += chunk.toString()\n  const lines = inputBuffer.split(/[\\r\\n]+/)\n  inputBuffer = lines.pop() || ''\n  for (const line of lines) if (line) console.log('CANARY_INPUT:' + process.pid + ':' + line)\n})\nfor (const signal of ['SIGINT', 'SIGHUP', 'SIGTERM']) process.on(signal, () => appendFileSync(process.env.MANTA_E2E_SIGNAL_LEDGER, JSON.stringify({ kind: 'canary', pid: process.pid, signal }) + '\\n'))\nprocess.stdin.resume()\nsetInterval(() => {}, 60000)\n`
  )
  writeFileSync(path.join(repoPath, 'manta.yaml'), 'scripts:\n  setup: node setup-live.js\n')
  execFileSync('git', ['init'], { cwd: repoPath })
  execFileSync('git', ['checkout', '-b', 'main'], { cwd: repoPath })
  execFileSync('git', ['add', '.'], { cwd: repoPath })
  execFileSync(
    'git',
    ['-c', 'user.name=Manta E2E', '-c', 'user.email=manta-e2e@example.com', 'commit', '-m', 'seed'],
    { cwd: repoPath }
  )
  return repoPath
}
...
test('adopts runtime-owned agent and Setup PTYs on first mount', async ({
  ...
  await expect
    .poll(() =>
      mantaPage.evaluate(async (repoId) => {
        const state = window.__store?.getState()
  ...
  await assertTargetBindings(mantaPage, worktreeId, allIdentities)
  await seedAgentRecoveryMetadata(mantaPage, worktreeId, terminalIdentity(agent!))

  await faultProjectionAndActivate(mantaPage, worktreeId, [agent!, setup!], agent!.tabId)
  const mountedAgentPtyId = await waitForActivePanePtyId(mantaPage)
  await enableTerminalAccessibility(mantaPage, agent!.tabId)
  await expect
    .poll(
      async () => ({
        ...
        visibleOriginalReady: (
          await terminalAccessibility(mantaPage, agent!.tabId).innerText()
  ...
  await typeIntoTerminal(mantaPage, agent!.tabId, agentMarker)
  await assertExactPtyReceivedMarker(electronApp, agent!.ptyId, agentMarker)
  await expect(terminalAccessibility(mantaPage, agent!.tabId)).toContainText(
    `AGENT_INPUT:${agentPid}:${agentMarker}`
  )
  await expect(terminalAccessibility(mantaPage, agent!.tabId)).not.toContainText(
    'Conversation interrupted'
  )

  await activateTerminal(mantaPage, worktreeId, setup!.tabId)
  const mountedSetupPtyId = await waitForActivePanePtyId(mantaPage)
  await enableTerminalAccessibility(mantaPage, setup!.tabId)
  expect(mountedSetupPtyId).toBe(setup!.ptyId)
  await expect(terminalAccessibility(mantaPage, setup!.tabId)).toContainText(
    `SETUP_READY:${setupPid}`
  )
  const setupMarker = `SETUP_KB_${randomUUID().slice(0, 8)}`
  await clearTerminalPtyWriteLog(electronApp)
  await typeIntoTerminal(mantaPage, setup!.tabId, setupMarker)
  await assertExactPtyReceivedMarker(electronApp, setup!.ptyId, setupMarker)
  await expect(terminalAccessibility(mantaPage, setup!.tabId)).toContainText(
    `SETUP_INPUT:${setupPid}:${setupMarker}`
  )
  await expect(terminalAccessibility(mantaPage, setup!.tabId)).not.toContainText(
    'Conversation interrupted'
  )
  ...
  const beforeReloadDelivery = await mantaPage.evaluate(() =>
  ...
  await expect
    .poll(() => mantaPage.evaluate(() => window.api.pty.getRendererDeliveryDebugSnapshot()))
    .toMatchObject(postReloadDelivery)
  await activateTerminal(mantaPage, worktreeId, agent!.tabId)
  const remountedAgentPtyId = await waitForActivePanePtyId(mantaPage)
  ...
  await expect
    .poll(() => terminalViewportText(mantaPage, agent!.tabId))
    .toContain(remountAgentLiveOutput)
  expect(
    await mantaPage.evaluate(() => window.api.pty.getRendererDeliveryDebugSnapshot())
  ).toMatchObject(postReloadDelivery)
  ...
  await clearTerminalPtyWriteLog(electronApp)
  await typeIntoTerminal(mantaPage, agent!.tabId, remountAgentMarker)
  ...
  await activateTerminal(mantaPage, worktreeId, setup!.tabId)
  const remountedSetupPtyId = await waitForActivePanePtyId(mantaPage)
  ...
  await expect
    .poll(() => terminalViewportText(mantaPage, setup!.tabId))
  ...
  await clearTerminalPtyWriteLog(electronApp)
  await typeIntoTerminal(mantaPage, setup!.tabId, remountSetupMarker)
