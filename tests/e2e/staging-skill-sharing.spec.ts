...
const RUN_STAGING = process.env.MANTA_E2E_SKILL_STAGING === '1'
const AUTH_TOKEN = process.env.MANTA_CLOUD_AUTH_TOKEN?.trim()
const PHYSICAL_HOST_PAIRING_URL = process.env.MANTA_E2E_SKILL_PHYSICAL_PAIRING_URL?.trim()
const RUN_HEADLESS_PAIRED = process.env.MANTA_E2E_SKILL_PAIRED_HEADLESS === '1'
const PHYSICAL_WSL_DISTRO = 'Ubuntu-24.04'
const SKILL_NAME = `manta-staging-${randomUUID().slice(0, 8)}`
const SSH_TARGET = RUN_STAGING ? stagingSkillSshTargetFromEnvironment() : null

if (RUN_STAGING && !AUTH_TOKEN) {
  throw new Error('MANTA_CLOUD_AUTH_TOKEN is required for the noninteractive staging journey.')
}
...
test.use({
  mantaAppExtraEnv: {
    MANTA_ARTIFACTS_API_URL: 'https://cloud-api-staging.manta.sh.cn',
    MANTA_CLOUD_API_URL: 'https://auth-staging.manta.sh.cn',
    MANTA_CLOUD_AUTH_URL: 'https://auth-staging.manta.sh.cn',
    MANTA_CLOUD_CLIENT_ID: 'manta-desktop',
    ...(AUTH_TOKEN ? { MANTA_CLOUD_AUTH_TOKEN: AUTH_TOKEN } : {})
  }
})

test.skip(!RUN_STAGING, 'Set MANTA_E2E_SKILL_STAGING=1 to run the live staging journey.')
...
test('publishes, updates, revokes, and deletes without losing local state', async ({
  ...
  const sourceRoot = mkdtempSync(join(tmpdir(), 'manta-staging-skill-source-'))
  ...
  try {
    if (RUN_HEADLESS_PAIRED) {
      ...
      physicalEnvironmentId = await addPhysicalHost(mantaPage, pairedHost.offer.pairingUrl)
    } else if (PHYSICAL_HOST_PAIRING_URL) {
      physicalEnvironmentId = await addPhysicalHost(mantaPage, PHYSICAL_HOST_PAIRING_URL)
    }
    if (SSH_TARGET) {
      sshTargetId = await connectStagingSkillSshTarget(mantaPage, SSH_TARGET)
    }
    ...
    const firstInstall = await installVersion(mantaPage, first.published, { scope: 'global' })
    ...
    for (const target of externalTargets(physicalEnvironmentId, sshTargetId)) {
      ...
      await expectManagedRemoteVersion(mantaPage, target, second.published.version.versionId)
    }
    const conflict = await installVersion(mantaPage, second.published, { scope: 'global' })
    ...
    for (const target of externalTargets(physicalEnvironmentId, sshTargetId)) {
      ...
      await expectManagedRemoteVersion(mantaPage, target, first.published.version.versionId)
    }
    ...
    const revokedInstall = await mantaPage.evaluate(
    ...
    const removed = await mantaPage.evaluate(
    ...
    expect(existsSync(globalSkill)).toBe(false)
    expect(
      await mantaPage.evaluate((id) => window.api.skills.getPackage(id), packageId)
    ).toMatchObject({ status: 'ok' })
    expect(
      await mantaPage.evaluate((id) => window.api.skills.deletePackage(id), packageId)
    ).toMatchObject({ status: 'ok' })
    packageId = null
  } finally {
    for (const target of externalTargets(physicalEnvironmentId, sshTargetId)) {
    ...
    if (sshTargetId) {
      await removeStagingSkillSshTarget(mantaPage, sshTargetId).catch(() => undefined)
    }
...
function writeSkill(directory: string, version: string): void {
  writeFileSync(
    ...
    `---\nname: ${SKILL_NAME}\ndescription: Manta staging installation journey\n---\n\n# Staging journey\n\nversion: ${version}\n`
  )
}
