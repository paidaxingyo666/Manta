 ...
 * - PR #677 (`return to Manta landing screen after closing last terminal`)
...
test.describe('Tab Close Navigation', () => {
   ...
   */
  test('closing the active editor tab activates its visual neighbor', async ({ mantaPage }) => {
    const worktreeId = await waitForActiveWorktree(mantaPage)

    const fileIds = await openSeededEditorTabs(mantaPage, worktreeId, [
      'package.json',
      'README.md',
      'tsconfig.json'
    ])
    ...
    await setActiveFile(mantaPage, fileIds[1])
    await expect.poll(async () => getActiveFileId(mantaPage), { timeout: 3_000 }).toBe(fileIds[1])

    await closeFile(mantaPage, fileIds[1])
    ...
    await expect
      .poll(async () => getActiveFileId(mantaPage), {
        timeout: 5_000,
   ...
   */
  test('closing the active diff tab activates a still-open neighbor', async ({ mantaPage }) => {
    const worktreeId = await waitForActiveWorktree(mantaPage)

    // Seed two editor tabs + one diff tab in the same worktree.
    const editorIds = await openSeededEditorTabs(mantaPage, worktreeId, [
      'package.json',
      'README.md'
    ])
    expect(editorIds.length).toBe(2)

    const diffId = await mantaPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
        return null
      }
    ...
    await expect.poll(async () => getActiveFileId(mantaPage), { timeout: 3_000 }).toBe(diffId)

    await closeFile(mantaPage, diffId!)
   ...
   */
  test('closing the last visible surface returns the app to Landing', async ({ mantaPage }) => {
    const worktreeId = await waitForActiveWorktree(mantaPage)
    ...
    const editorIds = await openSeededEditorTabs(mantaPage, worktreeId, ['package.json'])
    expect(editorIds.length).toBe(1)

    await setActiveFile(mantaPage, editorIds[0])
    await expect.poll(async () => getActiveFileId(mantaPage), { timeout: 3_000 }).toBe(editorIds[0])
    ...
    await closeFile(mantaPage, editorIds[0])
