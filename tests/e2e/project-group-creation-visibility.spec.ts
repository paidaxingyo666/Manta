...
for (const delayCreateResponse of [false, true]) {
  test(`created groups survive sidebar expansion (${delayCreateResponse ? 'refresh first' : 'ordinary timing'})`, async ({
    ...
    const repoIds = await mantaPage.evaluate(async (paths) => {
    ...
    const creation = mantaPage.evaluate(() =>
    ...
    await mantaPage.screenshot({ path: testInfo.outputPath('before-expansion.png') })
    ...
    await mantaPage.evaluate(() => window.__store!.getState().fetchProjectGroups())
    await expect(group).toHaveCount(1)
    await group.click()
    for (const repo of groupedRepos) {
    ...
    await mantaPage.screenshot({ path: testInfo.outputPath('after-expansion.png') })
  })
}
