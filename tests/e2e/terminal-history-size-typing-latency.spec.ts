...
test.describe('Terminal typing latency vs scrollback history size', () => {
  test('typing stays responsive as terminal history grows', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.manta-history-benchmark-${runId}.mjs`)
    ...
    try {
      ...
      commandSent = true
      await waitForRecentTerminalMarker(mantaPage, `HIST_READY_${runId}`, 10_000)
      await focusActiveTerminalInput(mantaPage)

      const phases: PhaseLatency[] = []
      let seq = 0

      const baseline = await measureTypingLatency(mantaPage, runId, 'empty history', seq)
