...
async function startRemoteMonitor(page: Page, ptyId: string): Promise<void> {
  const marker = `MANTA_SSH_WAKE_READY_${Date.now()}`
...
test.describe('SSH terminal window-wake stale PTY grid repro', () => {
  ...
  test('window focus heals a remote PTY whose applied grid drifted from xterm', async ({
    ...
    try {
      ...
      mantaPage.on('pageerror', (error) => pageErrors.push(error.message))
      installIdleGridMonitor(target)
      await mantaPage.setViewportSize(BASE_VIEWPORT)
      await waitForSessionReady(mantaPage)
      await waitForActiveWorktree(mantaPage)
      const identity = await mantaPage.evaluate(() => window.api.app.getIdentity())
      expect(identity.isDev).toBe(true)
      expect(identity.devWorktreeName).toBe(path.basename(process.cwd()))

      await connectDockerSshRelayTarget(mantaPage, target, { relayGracePeriodSeconds: 300 })
      await ensureTerminalVisible(mantaPage, 60_000)
      await waitForActiveTerminalManager(mantaPage, 60_000)
      const ptyId = await waitForActivePanePtyId(mantaPage, 60_000)
      await startRemoteMonitor(mantaPage, ptyId)
      await expect
        .poll(
          () => {
            try {
      ...
      const drifted = await readRendererGrid(mantaPage, ptyId)
      expect(drifted.xterm).toEqual(baseline.xterm)
      expect(drifted.applied).toEqual(staleGrid)

      await mantaPage.evaluate(() => window.dispatchEvent(new Event('focus')))
      ...
      if (!actualGridMatchesXterm(wakeResult.last.remote, wakeResult.last.renderer)) {
        ...
        await mantaPage.setViewportSize({
      ...
      await mantaPage.screenshot({ path: healedScreenshot, fullPage: true })
