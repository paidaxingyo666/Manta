 ...
 * covered in src/main/runtime/manta-runtime.test.ts. What lives here is every
...
const POINTER_COMMAND = 'manta-dev orchestration check'
...
async function setUpMailFixture(
  ...
  const openAgentPane = async (options?: {
    ...
    await waitForPtyShellEcho(mantaPage, ptyId, 60_000)
    const agent = createMailPaneAgent(options)
    await execInTerminal(mantaPage, ptyId, agent.launchCommand)
    await expect
...
test.describe('orchestration push-on-idle mail delivery', () => {
  test('delivers mail that arrives while the agent is already idle', async ({
    ...
    const { client, userDataDir, openAgentPane } = await setUpMailFixture(mantaPage, electronApp)
  ...
  test('holds mail while the agent is working and releases it on the idle frame', async ({
    ...
    const messageId = await sendMail(client, mailbox, { subject })
    await expectStaysPending(mantaPage, userDataDir, pane, messageId)
  ...
  test('writes and submits the pointer for the active coordinator pane', async ({
    ...
    test.setTimeout(180_000)
    const { client, openAgentPane } = await setUpMailFixture(mantaPage, electronApp)
    const pane = await openAgentPane()
    await driveToLiveIdle(client, pane)
...
test.describe('orchestration delivery to a cold-parked agent', () => {
  ...
  test.use({
    mantaAppExtraEnv: { MANTA_E2E_TERMINAL_PARKING_DELAY_MS: String(parkingDelayMs) }
  })
  ...
  test('keeps one pointer and one idempotent prompt on the same parked PTY', async ({
    ...
    const beforePark = await waitForPaneIdentitySnapshot(mantaPage, 1)
    ...
    const parkDetectedAfterMs = await parkHiddenTabBehindDecoy(mantaPage, worktreeId, tabId, {
      parkDelayMs: parkingDelayMs
    })
    expect(await getActiveTabId(mantaPage)).not.toBe(tabId)
    expect(await mantaPage.locator(`[data-terminal-tab-id=${JSON.stringify(tabId)}]`).count()).toBe(
      0
    )
    ...
    const promptParams = {
      ...
      client: { id: 'manta-e2e', type: 'desktop' as const }
    }
    ...
    const afterReveal = await waitForPaneIdentitySnapshot(mantaPage, 1)
    expect(afterReveal.tabId).toBe(tabId)
    expect(afterReveal.panes[0]?.ptyId).toBe(pane.ptyId)
    await expect(
      mantaPage.locator(`[data-terminal-tab-id=${JSON.stringify(tabId)}] .xterm-screen`).first()
    ).toBeVisible()
