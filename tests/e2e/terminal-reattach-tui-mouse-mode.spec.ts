/**
 * Regression proof for #8291: a real alt-screen TUI survives a Manta quit/relaunch, and after the
...
function createIsolatedProofRepo(): string {
  // Why realpathSync: macOS tmpdir symlinks through /private and Manta canonicalizes repo.path.
  const repoDir = realpathSync(mkdtempSync(path.join(os.tmpdir(), 'manta-mouse-reattach-repo-')))
  ...
  writeFileSync(path.join(repoDir, 'README.md'), '# Manta mouse-mode reattach proof repo\n')
  git('add', '-A')
...
// Why: this suite quits and relaunches Manta against one userDataDir, and the
...
test.describe('terminal reattach mouse mode', () => {
  ...
  {}, testInfo) => {
    ...
    try {
      ...
      // Why a screenshot and not the video fixture: this spec quits and relaunches Manta,
      ...
      const proofShot = process.env.MANTA_E2E_PROOF_SCREENSHOT
      if (proofShot) {
