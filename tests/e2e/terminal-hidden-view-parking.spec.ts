...
test.describe('Terminal hidden view parking', () => {
  test('parks a hidden terminal tab and restores rich TUI output on reveal', async ({
    ...
    const setup = await setUpParkableTabA(mantaPage)
    const { worktreeId, tabAId, tabAPtyId } = setup

    const runId = randomUUID()
    const finalMarker = `PARKED_RESTORE_FINAL_${runId}_${PARKED_FRAME_COUNT - 1}`
    const scriptPath = path.join(testRepoPath, `.manta-parked-rich-tui-${runId}.mjs`)
    writeParkedFrameScript(scriptPath, runId)
    try {
      await sendToTerminal(mantaPage, tabAPtyId, `node ${JSON.stringify(scriptPath)}\r`)
      await expect
        .poll(() => getTerminalContent(mantaPage, 12_000), {
          timeout: 15_000,
          message: 'rich TUI final frame did not render while tab A was visible'
        })
        .toContain(finalMarker)

      const tabBId = await createActiveTerminalTab(mantaPage, worktreeId)
      const parkDetectedAfterMs = await parkHiddenTabBehindDecoy(mantaPage, worktreeId, tabAId, {
        parkDelayMs: PARKING_DELAY_MS
      })
      const wiring = await readParkingWiring(mantaPage)
      ...
      const tabBState = await readTerminalTabViewState(mantaPage, tabBId)
      expect(tabBState.hasManager).toBe(true)
      expect(tabBState.paneCount).toBeGreaterThan(0)

      await activateTerminalTab(mantaPage, tabAId)
      await waitForActiveTerminalManager(mantaPage, 30_000)
      const revealedSnapshot = await waitForPaneIdentitySnapshot(mantaPage, 1)
      ...
      const content = await getTerminalContent(mantaPage, 12_000)
      expect(content).toContain(`Frame ${String(PARKED_FRAME_COUNT - 1).padStart(3, '0')}`)
      expect(content).toContain('╭')
      expect(content).toContain('├')
      expect(content).toContain('█')
      expect(content).not.toContain('Manta skipped hidden terminal output')
      ...
      await sendToTerminal(mantaPage, tabAPtyId, '\x03')
      ...
      await waitForPtyShellEcho(mantaPage, tabAPtyId, 15_000).catch((error: unknown) => {
        throw new Error(
      ...
      await runNodeScriptInTerminal(mantaPage, tabAPtyId, typedProbeScript, {
        prefix: 'manta-parked-typed-probe'
      })
      await expect
        .poll(() => getTerminalContent(mantaPage, 12_000), {
          timeout: 10_000,
  ...
  test('keeps bell and title side effects live while parked', async ({ mantaPage }) => {
    await waitForSessionReady(mantaPage)
    const setup = await setUpParkableTabA(mantaPage)
    const { worktreeId, tabAId, tabAPtyId } = setup

    await createActiveTerminalTab(mantaPage, worktreeId)
    await parkHiddenTabBehindDecoy(mantaPage, worktreeId, tabAId, {
      parkDelayMs: PARKING_DELAY_MS
    })
    ...
    // Why: delivered via a temp file — `node -e` quoting is not PowerShell-safe (#8521).
    await runNodeScriptInTerminal(mantaPage, tabAPtyId, sideEffectScript, {
      prefix: 'manta-parked-side-effect'
    })

    await expect
      .poll(() => getTerminalTabTitle(mantaPage, worktreeId, tabAId), {
        timeout: 10_000,
        message: 'parked OSC 0 title did not update the tab title in the store'
      })
      .toBe(parkedTitle)
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(mantaPage)).includes(tabAId), {
        timeout: 10_000,
        message: 'parked BEL did not mark the terminal tab unread'
      })
      .toBe(true)
    await expect
      .poll(() => isWorktreeUnread(mantaPage, worktreeId), {
        timeout: 10_000,
    ...
    expect((await readTerminalTabViewState(mantaPage, tabAId)).hasManager).toBe(false)
  ...
  test('does not park excluded tabs', async ({ mantaPage }) => {
    await waitForSessionReady(mantaPage)
    const setup = await setUpParkableTabA(mantaPage)
    ...
    const tabCId = await createActiveTerminalTab(mantaPage, worktreeId)
    ...
    expect(await hasPendingStartupCommand(mantaPage, tabCId)).toBe(true)

    // Tab B on top hides both A and C.
    const tabBId = await createActiveTerminalTab(mantaPage, worktreeId)
    await expect
      .poll(() => getActiveTabId(mantaPage), {
        timeout: 5_000,
    ...
    expect(await hasPendingStartupCommand(mantaPage, tabCId)).toBe(true)
    const tabCState = await readTerminalTabViewState(mantaPage, tabCId)
  ...
  test('reproduces a static frame byte-for-byte across 25 park/reveal cycles', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.manta-cycle-reference-${runId}.mjs`)
    ...
    try {
      ...
      const tabBId = await createActiveTerminalTab(mantaPage, worktreeId)
      const decoyTabId = await createActiveTerminalTab(mantaPage, worktreeId)
      ...
      const runOneParkRevealCycle = async (cycle: number): Promise<string[]> => {
        await activateTerminalTab(mantaPage, tabBId)
        // Hide tab B behind the decoy so B (not A) holds the #8262 exemption.
        await activateTerminalTab(mantaPage, decoyTabId)
        await waitForTabParked(mantaPage, tabAId, { parkDelayMs: PARKING_DELAY_MS })
        await activateTerminalTab(mantaPage, tabAId)
        await waitForActiveTerminalManager(mantaPage, 30_000)
        const revealed = await waitForPaneIdentitySnapshot(mantaPage, 1)
        ...
        await expect
          ...
          .toContain(marker)
        const rows = terminalContentRows(await getTerminalContent(mantaPage, 12_000))
        // Garble sentinel: the hidden-skip banner must never appear.
        expect(rows.join('\n')).not.toContain('Manta skipped hidden terminal output')
        return rows
      }
      ...
      for (let cycle = 1; cycle < CYCLES; cycle++) {
        ...
        await mantaPage.waitForTimeout(PARK_VERDICT_BURST_SETTLE_MS)
