...
async function sendProviderHook() {
  ...
  await fetch('http://127.0.0.1:' + process.env.MANTA_AGENT_HOOK_PORT + '${hookPath}', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-Manta-Agent-Hook-Token': process.env.MANTA_AGENT_HOOK_TOKEN
    },
    body: JSON.stringify({
...
test('worker-read uses provider transcripts across supported orchestration agents', async ({
  ...
  for (const provider of PROVIDERS) {
    ...
    const transcriptDir = mkdtempSync(
      path.join(os.tmpdir(), `manta-e2e-${provider.agent}-transcript-`)
    )
