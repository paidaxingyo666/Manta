...
test.describe('R2 Docker SSH bulk-open freeze', () => {
  ...
  test('bulk-open many flooding SSH terminals and measure renderer lag @freeze-repro', async ({
    ...
    try {
      ...
      await execInTerminal(mantaPage, continuousFloodCommand(runId, 0))
      await waitForTerminalOutput(mantaPage, `READY:SSH_BULK_${runId}_0`, 60_000)

      for (let i = 1; i < SESSION_SPLITS; i += 1) {
        await splitActiveTerminalPane(mantaPage)
        await focusLastTerminalPane(mantaPage)
        await waitForActivePanePtyId(mantaPage, 30_000)
        await execInTerminal(mantaPage, continuousFloodCommand(runId, i))
        await waitForTerminalOutput(mantaPage, `READY:SSH_BULK_${runId}_${i}`, 60_000)
      }
