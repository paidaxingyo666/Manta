 ...
 *
 * Run:
 *   MANTA_E2E_SSH_DOCKER=1 pnpm run test:e2e:ssh-docker-bulk-open-freeze
 */
...
test.describe('R2 Docker SSH bulk-open freeze', () => {
  test.skip(!RUN_DOCKER_SSH, 'Set MANTA_E2E_SSH_DOCKER=1 to run Docker SSH freeze repro')
  ...
  test.fixme('bulk-open many flooding SSH terminals and measure renderer lag @freeze-repro', async ({
    ...
    try {
      ...
      const firstPtyId = await waitForActivePanePtyId(mantaPage, 60_000)
      await execInTerminal(mantaPage, firstPtyId, continuousFloodCommand(runId, 0))
      ...
      await waitForTerminalOutput(mantaPage, `BG:SSH_BULK_${runId}_0:`, 60_000, FLOOD_READ_CHARS)

      for (let i = 1; i < SESSION_SPLITS; i += 1) {
        await splitActiveTerminalPane(mantaPage, 'vertical')
        await focusLastTerminalPane(mantaPage)
        const panePtyId = await waitForActivePanePtyId(mantaPage, 30_000)
        await execInTerminal(mantaPage, panePtyId, continuousFloodCommand(runId, i))
      ...
      await mantaPage.evaluate(() => window.__store?.getState().setActiveView('tasks'))
      await mantaPage.waitForTimeout(4_000)

      const hiddenProbe = await startRendererLagProbe(mantaPage)
      ...
      const openProbe = await startRendererLagProbe(mantaPage)
      await mantaPage.evaluate(() => window.__store?.getState().setActiveView('terminal'))
      for (let pass = 0; pass < 3; pass += 1) {
        for (let i = 0; i < SESSION_SPLITS; i += 1) {
          await mantaPage.keyboard.press(process.platform === 'darwin' ? 'Meta+]' : 'Control+]')
      ...
      const interactionProbeMs = await mantaPage.evaluate(async () => {
