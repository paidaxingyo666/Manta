...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'manta-paired-stalled-stream-'))
...
test('restarts one ACK-starved paired terminal stream without replacing its PTY @headful', async ({
  ...
  const worktree = await mantaPage.evaluate(() => {
  ...
  const client = await launchPairedWebClient(electronApp, offer, {
    disableRemoteTerminalStallRecovery:
      process.env.MANTA_E2E_DISABLE_REMOTE_TERMINAL_STALL_RECOVERY === '1'
  })
  ...
  try {
    await expect
      ...
      .toBe(true)
    const observerOffer = await createRuntimeDesktopPairingOffer(mantaPage)
    ...
    await minimizeHeadedHost(electronApp, mantaPage)
    ...
    await restoreHeadedHost(electronApp, mantaPage)
    await mantaPage.evaluate(
      (worktreeId) => window.__store?.getState().setActiveWorktree(worktreeId),
      worktree.id
    )
    const hostTab = mantaPage.locator(
      `[data-testid="sortable-tab"][data-tab-id="${created.tab.parentTabId}"]`
    )
    await expect(hostTab).toBeVisible({ timeout: 30_000 })
    await hostTab.click()
    const hostPane = await findHostPaneWithMarker(mantaPage, `LIVE:${liveMarker}`)
    expect(hostPane.tabId).toBe(created.tab.parentTabId)
    await mantaPage.evaluate(({ paneId, tabId }) => {
      const manager = window.__paneManagers?.get(tabId)
      manager?.setActivePane?.(paneId, { focus: true })
    }, hostPane)
    await expect
      .poll(() => getTerminalContent(mantaPage), { timeout: 30_000 })
      .toContain(`LIVE:${liveMarker}`)
    const restoredTerminalScreenshot = await mantaPage
      .locator(
    ...
    await restoreHeadedHost(electronApp, mantaPage)
    const restoredHostScreenshot = await mantaPage.screenshot({ fullPage: true })
    expect(
