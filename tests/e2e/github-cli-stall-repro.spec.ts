...
const fakeGhDir = mkdtempSync(path.join(os.tmpdir(), 'manta-e2e-fake-gh-'))
...
test('GitHub Tasks drawer recovers when gh stalls on issue details', async ({
  ...
  const { repoId } = await mantaPage.evaluate((repoPath) => {
  ...
  const detailHeading = mantaPage.getByRole('heading', {
  ...
  await expect(mantaPage.getByText('Unable to load details for this GitHub item.')).toBeVisible({
    timeout: 5_000
  })
