...
for (const dpr of [1, 1.25, 2]) {
  for (const gpu of ['on', 'off'] as const) {
    test.describe(`IME preedit grid DPR ${dpr} GPU ${gpu} @headful`, () => {
      test.use({ mantaAppExtraArgs: [`--force-device-scale-factor=${dpr}`] })
      ...
      test('matches committed character advances across font and spacing changes', async ({
        ...
        try {
          ...
          await writeToActiveTerminal(mantaPage, '\x1b[2J\x1b[H\x1b[999G')
          await setImeComposition(arena.session, 'あ'.repeat(8))
          await expect(mantaPage.locator('.xterm-composition-preedit')).toHaveText(
            `‎${'あ'.repeat(8)}‎`
          )
          const edge = await sampleMidlinePreeditOcclusion(mantaPage)
          ...
          await expect
            .poll(async () => (await sampleMidlinePreeditOcclusion(mantaPage)).cellWidth)
...
test('preserves native shaping for mixed text, complex scripts, and emoji', async ({
  ...
  try {
    ...
    for (const text of [
      ...
      await writeToActiveTerminal(mantaPage, '\x1b[2J\x1b[H')
      await setImeComposition(arena.session, text)
      const preedit = mantaPage.locator('.composition-view.active .xterm-composition-preedit')
      await expect(preedit).toHaveText(`‎${text}‎`)
