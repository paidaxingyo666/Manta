...
test.describe('Terminal attention', () => {
  ...
  test('background terminal attention marks a tab unread and clears on focus', async ({
    ...
    const secondTabId = await createTerminalTab(mantaPage)
    await waitForActiveTerminalManager(mantaPage, 30_000)
    ...
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(mantaPage)).includes(secondTabId), {
        timeout: 10_000,
        message: 'Background tab did not become unread after BEL'
      })
      .toBe(true)

    const secondTabBell = mantaPage
      .locator(
  ...
  test('a BEL on the focused tab raises, then clears on click', async ({ mantaPage }) => {
    await waitForSessionReady(mantaPage)
    await waitForActiveWorktree(mantaPage)
    await ensureTerminalVisible(mantaPage)
    await waitForActiveTerminalManager(mantaPage, 30_000)

    const activeTabId = await getActiveTabId(mantaPage)
    if (!activeTabId) {
      throw new Error('Expected an active terminal tab')
    }
    const activePtyId = await waitForActivePanePtyId(mantaPage)
    await installRendererTitleLog(mantaPage)
    ...
    expect((await getUnreadTerminalTabIds(mantaPage)).includes(activeTabId)).toBe(true)
    const activeTabBell = mantaPage
      .locator(
    ...
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(mantaPage)).includes(activeTabId), {
        timeout: 5_000,
  ...
  test('a BEL on the focused tab raises, then clears on plain Escape', async ({ mantaPage }) => {
    await waitForSessionReady(mantaPage)
    await waitForActiveWorktree(mantaPage)
    await ensureTerminalVisible(mantaPage)
    await waitForActiveTerminalManager(mantaPage, 30_000)
    ...
    const activePaneKey = await getActivePaneKey(mantaPage, activeTabId)
    const activePtyId = await waitForActivePanePtyId(mantaPage)
    await installRendererTitleLog(mantaPage)
    ...
    await expect
      .poll(async () => (await getUnreadTerminalPaneKeys(mantaPage)).includes(activePaneKey), {
        timeout: 5_000,
    ...
    await focusActiveXterm(mantaPage, activeTabId)
  ...
  // after restarting Manta with a Claude Code session open, clicking between
  ...
  test('mode bits replayed into xterm do not leak focus escapes to the shell', async ({
    ...
    try {
      ...
      // shell-startup BELs.
      const emittedFromXterm = await mantaPage.evaluate(
        () =>
