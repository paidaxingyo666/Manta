...
test.describe('Docker SSH relay resource accumulation', () => {
  ...
  test('does not accumulate pts devices, relay fds, or relay processes @resource-accumulation', async ({
    ...
    try {
      ...
      await execInTerminal(mantaPage, firstPtyId, `echo PANE_READY_${runId}_0`)
      await waitForTerminalOutput(mantaPage, `PANE_READY_${runId}_0`, 60_000)
      ...
      for (let index = 1; index < TERMINAL_COUNT; index += 1) {
        ...
        await execInTerminal(mantaPage, ptyId, `echo PANE_READY_${runId}_${index}`)
        await waitForTerminalOutput(mantaPage, `PANE_READY_${runId}_${index}`, 60_000)
