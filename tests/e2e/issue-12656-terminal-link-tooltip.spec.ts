...
test.describe('Issue #12656 terminal link tooltip', () => {
  test('clears hover state without permanently shrinking the terminal', async ({
    ...
    const url = `https://example.com/manta-issue-12656-${randomUUID().slice(0, 8)}`
    await sendToTerminal(
      mantaPage,
      ptyId,
      `printf 'issue-12656-output-%02d\\n' $(seq 1 64); printf '${url}\\n'\r`
    )
    await waitForTerminalOutput(mantaPage, url)

    let probe: LinkProbe | null = null
    await expect
      .poll(
        async () => {
          probe = await locateUrl(mantaPage, url)
          return probe
        },
    ...
    const idle = await readTooltipState(mantaPage, probe.tabId)
    expect(Math.abs(idle.paneBottom - idle.terminalBottom)).toBeLessThanOrEqual(1)
    await expect
      .poll(async () => {
        const currentProbe = await locateUrl(mantaPage, url)
        if (!currentProbe) {
          return { display: 'none', text: '' }
        }
        probe = currentProbe
        await moveToLink(mantaPage, currentProbe)
        return readTooltipState(mantaPage, currentProbe.tabId)
      })
      .toMatchObject({ display: '', text: expect.stringContaining(url) })

    const hovered = await readTooltipState(mantaPage, probe.tabId)
    ...
    await captureProof(mantaPage, testInfo, 'issue-12656-fixed-hover.png')

    await mantaPage.evaluate(() => window.dispatchEvent(new Event('blur')))
    await expect
      .poll(() => readTooltipState(mantaPage, probe.tabId))
      .toMatchObject({ display: 'none', cursor: 'text' })
    const cleared = await readTooltipState(mantaPage, probe.tabId)
    expect(Math.abs(cleared.paneBottom - cleared.terminalBottom)).toBeLessThanOrEqual(1)
    await captureProof(mantaPage, testInfo, 'issue-12656-fixed-after-blur.png')

    await expect.poll(() => getTerminalContent(mantaPage)).toContain(url)
  })
})
