...
test.describe('Notification settings', () => {
  ...
  test('can be toggled from settings and disables child controls', async ({ mantaPage }) => {
    await openNotificationSettings(mantaPage)

    const notificationsSection = mantaPage.locator('[data-settings-section="notifications"]')
    ...
    await expect
      .poll(async () => (await getSettings(mantaPage)).notifications.agentTaskComplete, {
        timeout: 5_000,
    ...
    await expect
      .poll(async () => (await getSettings(mantaPage)).notifications.enabled, {
        timeout: 5_000,
    ...
    await expect
      .poll(async () => {
        const settings = await getSettings(mantaPage)
        return {
