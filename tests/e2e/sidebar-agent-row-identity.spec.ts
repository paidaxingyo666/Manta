...
test('sidebar keeps a Cursor pane visible and an OpenCode pane out of Claude Code hands', async ({
  ...
  await useFullAgentActivityRows(mantaPage)

  const openCode = await openAgentTab(mantaPage, worktreeId, 'opencode')
  ...
  await waitForTerminalOutput(mantaPage, PANE_HOLD_MARKER, 15_000)
  ...
  await expect
    .poll(() => paneTitles(mantaPage, openCode.tabId), {
      timeout: 15_000,
      message: 'the OpenCode task title never reached the renderer'
    })
    .toContain(OPENCODE_TASK_OSC_TITLE)

  const cursor = await openAgentTab(mantaPage, worktreeId, 'cursor')
  ...
  const agentList = worktreeRow(mantaPage, worktreeId).locator('[aria-label="Agents"]')
  ...
  expect(await settledSidebarAgentRowIdentities(mantaPage, agentListSelector)).toEqual([
    'Cursor',
    'OpenCode'
  ])
