...
function addressBarInput(page: Page): Locator {
  return page.locator('[data-manta-browser-address-bar="true"]')
}

function addressBarOverlay(page: Page): Locator {
  return page.locator('[data-manta-browser-address-bar-overlay="true"]')
}
...
test.describe('Browser address bar in a narrow toolbar', () => {
  ...
  test('focusing the squeezed address bar expands a typable field that navigates', async ({
    ...
    try {
      ...
      await createBlankBrowserTab(mantaPage, worktreeId)
      await settleToSqueezedRestingState(mantaPage, electronApp)

      const overlay = addressBarOverlay(mantaPage)
      // The bug: the inline field is squeezed away entirely.
      await expect.poll(() => addressBarInputWidth(mantaPage), { timeout: 10_000 }).toBeLessThan(40)

      await mantaPage.locator('form:has(> [data-manta-browser-address-bar="true"])').click()

      await expect(overlay).toBeVisible()
      await expect
        .poll(() => addressBarInputWidth(mantaPage), { timeout: 5_000 })
        .toBeGreaterThan(BROWSER_ADDRESS_BAR_MIN_INLINE_WIDTH / 2)

      await addressBarInput(mantaPage).fill(destination.url)
      await addressBarInput(mantaPage).press('Enter')

      await expect
        .poll(async () => (await getBrowserTabs(mantaPage, worktreeId)).at(-1)?.url ?? null, {
          timeout: 15_000
        })
        .toContain('/typed')
    } finally {
