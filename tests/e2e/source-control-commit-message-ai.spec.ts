...
test.describe('Source Control AI commit messages', () => {
  ...
  for (const { label, linkedIssue, expected } of [
    ...
    test(`${label} the commit-message recipe`, async ({ mantaPage, testRepoPath }) => {
      ...
      try {
        ...
        await openSourceControlForWorktree(mantaPage, testRepoPath, worktreePath)
        ...
        const generate = mantaPage.getByRole('button', { name: 'Generate commit message with AI' })
