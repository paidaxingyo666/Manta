...
test.describe('terminal TUI wheel reports', () => {
  test('notched mouse wheel ticks produce immediate mouse-reporting TUI scroll reports', async ({
    ...
    const samples = await probeSmallMouseWheelReports(mantaPage, 4)
  ...
  test('fullscreen mouse-reporting TUI scroll distance follows wheel magnitude @headful', async ({
    ...
    await expect
      ...
      .toBe(true)
    await mantaPage.waitForTimeout(1200)
    ...
    await execInTerminal(mantaPage, ptyId, `node ${JSON.stringify(VISIBLE_TUI_FIXTURE_PATH)}`)

    await expect
      .poll(() => readVisibleTuiOffset(mantaPage), {
        timeout: 10_000,
        message: 'visible fullscreen TUI did not render numbered rows'
      })
      .toBe(0)

    await dispatchTuiWheel(mantaPage, {
      deltaY: 10,
    ...
    const cellHeight = await mantaPage.evaluate(() => {
      const state = window.__store?.getState()
      const worktreeId = state?.activeWorktreeId
      const tabId =
        state?.activeTabType === 'terminal'
  ...
  test('TUI scroll setting scales notched mouse wheel reports', async ({ mantaPage }) => {
    await waitForSessionReady(mantaPage)
    await waitForActiveWorktree(mantaPage)
    await ensureTerminalVisible(mantaPage)
    await waitForActiveTerminalManager(mantaPage, 30_000)
    ...
    const slow = await probeTimedSmallMouseWheelReports(mantaPage, {
      drainWaitMs: 120,
      intervalMs: 220,
      ticks: 5
    })
    await mantaPage.waitForTimeout(220)
    const paced = await probeTimedSmallMouseWheelReports(mantaPage, {
      drainWaitMs: 220,
      intervalMs: 80,
      ticks: 5
    })
