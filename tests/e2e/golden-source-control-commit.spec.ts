...
test('@golden stages and commits a file through Source Control', async ({
  ...
  await mantaPage.getByRole('textbox', { name: 'Commit message' }).fill('test: golden daily loop')
  await mantaPage.getByRole('button', { name: 'Commit', exact: true }).click()
  ...
  await expect(
    mantaPage.locator('[data-sonner-toast]').filter({ hasText: /node|command not found|cmd\.exe/i })
  ).toHaveCount(0)
})
