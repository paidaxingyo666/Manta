...
test.describe('app menu paste ownership', () => {
  test.beforeEach(async ({ electronApp, mantaPage }) => {
  ...
  test('Edit > Paste sends clipboard text to the focused terminal exactly once', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.manta-app-menu-paste-${runId}.mjs`)
    ...
    try {
      ...
      scriptStarted = true
      await waitForTerminalOutput(mantaPage, `APP_MENU_PASTE_READY_${runId}`, 10_000)

      const payload = `MANTA_E2E_APP_MENU_TERMINAL_${runId}`
  ...
  test('Edit > Paste into a rename textbox does not also write to the active terminal', async ({
    ...
    await tabLocatorByTitle(mantaPage, originalTitle).dblclick()
    ...
    const payload = `MANTA_E2E_APP_MENU_TEXTBOX_${randomUUID()}`
    await mantaPage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
    await clearTerminalPtyWriteLog(electronApp)
