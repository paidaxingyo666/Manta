...
import { test, expect, forwardElectronProcessLogs } from './helpers/manta-app'
import { TEST_REPO_PATH_FILE } from './global-setup'
import { getE2ECompletedOnboardingProfile } from './helpers/e2e-completed-onboarding-profile'
import { getMantaElectronLaunchArgs } from './helpers/electron-launch-args'
...
function createHeadlessLaunchIsolation(userDataDir: string): ElectronHomeIsolation {
  ...
  return createElectronHomeIsolation({
    ...
    launchEnv: {
      ...
      MANTA_E2E_ENFORCE_SINGLE_INSTANCE_LOCK: '1'
    },
    extraEnv: {},
    userDataDir
  })
...
function readPersistedPromotionBinding(
  ...
  try {
    const persisted = JSON.parse(
      readFileSync(
        path.join(userDataDir, 'profiles', DEFAULT_LOCAL_MANTA_PROFILE_ID, 'manta-data.json'),
        'utf8'
      )
    ) as {
      workspaceSession?: {
...
{}, testInfo) => {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'manta-e2e-serve-promotion-'))
  ...
  try {
    serveApp = await electron.launch({
      args: [...getMantaElectronLaunchArgs(mainPath, false), '--serve', '--serve-no-pairing'],
      env
    })
    ...
    const forwardAppLogs = process.env.MANTA_E2E_FORWARD_APP_LOGS === '1'
    activatingProcess = spawn(electronPath, getMantaElectronLaunchArgs(mainPath, false), {
      env,
