...
test.describe('Workspace Space git status checks', () => {
  test('checks every scanned deletable row, including rows after the first 50', async ({
    ...
    // Why: on symlinked tmpdirs (/var→/private/var on macOS, /tmp→… on CI) Manta
    ...
    const worktreeParent = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), 'manta-space-git-status-'))
    )
    ...
    try {
      ...
      const repoId = await mantaPage.evaluate((testRepoPath) => {
      ...
      await loadWorktreesUntilPathsPresent(mantaPage, repoId, registeredWorktreePaths)

      const rowDisplayNames = await mantaPage.evaluate(
      ...
      // anything.
      const rowCheckboxes = mantaPage.getByRole('checkbox', {
        name: new RegExp(
      ...
      await expect(mantaPage.getByText('Keep: git not checked')).toHaveCount(0, { timeout: 30_000 })
    } finally {
      for (const worktreePath of worktreePaths) {
        try {
