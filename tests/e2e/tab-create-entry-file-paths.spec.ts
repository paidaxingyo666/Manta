...
const relativeFilePath =
  'packages/manta/src/renderer/src/components/navigation/worktree/secondary-nav/SecondaryNav.tsx'
...
test('new-tab file results prioritize the filename and reveal the full path on hover', async ({
  ...
  const input = mantaPage.locator(
    'input[role="combobox"][aria-controls="tab-create-entry-results"]'
  )
  await input.fill('secondaryNav')

  const row = mantaPage.locator('[role="option"]').filter({ hasText: 'Open file' }).first()
  ...
  const proofPath = process.env.MANTA_STA3424_PROOF_PATH
  if (proofPath) {
