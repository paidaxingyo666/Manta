...
function codexLikeStartupCommand(marker: string): string {
  ...
  return stageNodeScriptForTerminal(script, { prefix: 'manta-codex-startup-bg' }).command
}
...
test.describe('Codex hidden startup composer background', () => {
  test('restores the input background when a Codex worktree first becomes visible', async ({
    ...
    const hiddenTabId = await mantaPage.evaluate(
    ...
    const hiddenPtyId = await waitForHiddenTabPtyId(mantaPage, hiddenTabId)
    await expect
      .poll(() => mainSnapshotContains(mantaPage, hiddenPtyId, marker), {
        timeout: 20_000,
    ...
    await expect
      .poll(
        async () => {
          try {
            const nextTarget = await readCodexStartupBackgroundTarget(mantaPage, marker)
            target = nextTarget
    ...
    const visibleBackgroundPixels = await countVisibleBackgroundPixels(mantaPage, target)
