...
test.describe('terminal hidden-worktree retention budget', () => {
  ...
  test('force-parks the older hidden un-parkable worktree and spares the newest', async ({
    ...
    try {
      ...
      const older = await connectDockerSshRelayTarget(mantaPage, target)
      await expect
      ...
      const olderPtyId = await waitForActivePanePtyId(mantaPage, 60_000)
      const olderTabId = await getActiveTabId(mantaPage)
      if (!olderTabId) {
      ...
      await sendToTerminal(mantaPage, olderPtyId, `echo "${olderMarker}:"\r`)
      await expect
      ...
      const newerTabId = await getActiveTabId(mantaPage)
      if (!newerTabId) {
      ...
      await waitForTabParked(mantaPage, olderTabId, { parkDelayMs: PARKING_DELAY_MS })
      // …while the newest hidden worktree keeps its mounted panes (last-active exemption).
      const newerStillMounted = await mantaPage.evaluate(
