...
>({
  ...
  await deps.waitForSessionReady(mantaPage)
  await deps.waitForActiveWorktree(mantaPage)
  const panes = await deps.ensureActiveWorktreePaneLoad(mantaPage, backgroundPaneCount + 1)
  const [typingPane, ...loadPanes] = panes
  await deps.focusPane(mantaPage, typingPane.paneKey)

  const runId = randomUUID()
  const scrollRunId = randomUUID()
  const typingScriptPath = path.join(testRepoPath, `.manta-opencode-pressure-typing-${runId}.mjs`)
  const pressureScriptPath = path.join(testRepoPath, `.manta-opencode-pressure-load-${runId}.mjs`)
  await seedActiveTerminalScrollback(mantaPage, typingPane.ptyId, scrollRunId)
  ...
  try {
    ...
    const pressureBeforeTyping = await deps.waitForMainPtyPressureBacklog(mantaPage)
    ...
    const mainPressure = await deps.readMainPtyPressureDebug(mantaPage)
    ...
    deps.annotateTypingMeasurement(
      ...
      panes.length,
      measurement,
      await deps.readTerminalPtyOutputDebug(mantaPage),
      scheduler,
      mainPressure,
      ackGate
    )
...
>({
  ...
  const scrollMeasurement = await measureActiveTerminalWheelScroll(mantaPage)
  const mainPressureAfterScroll = await deps.readMainPtyPressureDebug(mantaPage)
  const ackGateAfterScroll = await deps.readTerminalAckGateDebug(mantaPage)
  annotateScrollMeasurement(
    testInfo,
  ...
  await scrollActiveTerminalToBottom(mantaPage)
}
