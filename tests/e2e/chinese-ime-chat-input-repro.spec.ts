...
function emitState(reason) {
  ...
  process.stdout.write('\\x1b]1337;MantaImeState=' + Buffer.from(JSON.stringify({
    reason,
    model,
    cursor,
    submitted
...
async function installImeEventProbe(page: Page): Promise<void> {
  await page.evaluate(() => {
    const targetWindow = window as unknown as { __mantaImeEventLog?: ImeEventLogEntry[] }
    targetWindow.__mantaImeEventLog = []
    ...
    const record = (event: Event): void => {
      ...
      targetWindow.__mantaImeEventLog!.push({
        type: event.type,
        at: performance.now(),
...
async function readImeEventLog(page: Page): Promise<ImeEventLogEntry[]> {
  return page.evaluate(() => {
    const targetWindow = window as unknown as { __mantaImeEventLog?: ImeEventLogEntry[] }
    return targetWindow.__mantaImeEventLog ?? []
  })
}
...
async function dispatchSogouEmptyCompositionUpdate(page: Page): Promise<void> {
  ...
  // popup is still open (#6765); Manta's tracker must not flip inactive on it.
  await page.evaluate(() => {
    const active = document.activeElement
    if (!(active instanceof HTMLTextAreaElement)) {
      throw new Error('xterm helper textarea is not focused')
...
async function dispatchSogouPostCompositionEnd(page: Page, data: string): Promise<void> {
  ...
  // compositionend; target the terminal element so Manta's tracker sees the end
...
test.describe('Chinese IME terminal chat input repro', () => {
  test('keeps composed Chinese text, cursor movement, and Backspace stable in the agent input surface', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.manta-chinese-ime-harness-${runId}.cjs`)
    ...
    try {
      ...
      await installImeEventProbe(mantaPage)

      await dispatchImeProcessKey(session, 'KeyN')
      await composeAndCommitChineseText(session, mantaPage, ['n', 'ni', '你', '你好'], '你好')
      await waitForLivePrompt(mantaPage, '你好')
      await attachImeEvidence(mantaPage, testInfo, 'after-compose-hello')
      await mantaPage.keyboard.press('Enter')
      await expect
        .poll(async () => (await readPromptState(mantaPage))?.submitted.at(-1) ?? null, {
          timeout: 5_000,
      ...
      await waitForLivePrompt(mantaPage, '一二三四五六七八九十')
      for (let index = 0; index < 5; index += 1) {
        await mantaPage.keyboard.press('ArrowLeft')
      }
      await dispatchImeProcessKey(session, 'KeyZ')
      await composeAndCommitChineseText(session, mantaPage, ['z', 'zh', '中'], '中')
      await waitForLivePrompt(mantaPage, '一二三四五中六七八九十')
      await attachImeEvidence(mantaPage, testInfo, 'after-middle-insert')
      ...
      await mantaPage.keyboard.press('Backspace')
      await waitForLivePrompt(mantaPage, '一二三四五六七八九十')
      await attachImeEvidence(mantaPage, testInfo, 'after-single-backspace')
      ...
      const log = await readImeEventLog(mantaPage)
      expect(
      ...
      await attachImeEvidence(mantaPage, testInfo, 'final-ime-evidence').catch(() => undefined)
  ...
  test('commits the active Pinyin preedit when Shift toggles the Windows IME', async ({
    ...
    await reloadWithWindowsImePolicy(mantaPage)
    await waitForSessionReady(mantaPage)
    await waitForActiveWorktree(mantaPage)
    await ensureTerminalVisible(mantaPage)
    await waitForActiveTerminalManager(mantaPage, 30_000)
    ...
    const ptyId = await waitForActivePanePtyId(mantaPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.manta-windows-shift-ime-${runId}.cjs`)
    const inputLogPath = path.join(testRepoPath, `.manta-windows-shift-ime-${runId}.jsonl`)
    ...
    try {
      ...
      await waitForLivePrompt(mantaPage, 's')
      await expect.poll(() => readActiveCompositionText(mantaPage)).toBe('')
      await expect.poll(async () => (await readPromptState(mantaPage))?.submitted).toEqual([])
      await expect
        .poll(() => readPtyInputs(inputLogPath).slice(inputCountBeforeShift))
        .toEqual(['s'])
    } finally {
      await attachImeEvidence(mantaPage, testInfo, 'windows-shift-ime-evidence').catch(
        () => undefined
      )
      await session.detach().catch(() => undefined)
      await sendToTerminal(mantaPage, ptyId, '\x03').catch(() => undefined)
  ...
  test('keeps Sogou-style candidate selection keys out of the PTY while committing Chinese text', async ({
    ...
    await reloadWithLinuxImePolicy(mantaPage)
    await waitForSessionReady(mantaPage)
    await waitForActiveWorktree(mantaPage)
    await ensureTerminalVisible(mantaPage)
    await waitForActiveTerminalManager(mantaPage, 30_000)
    ...
    const ptyId = await waitForActivePanePtyId(mantaPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.manta-sogou-ime-harness-${runId}.cjs`)
    ...
    try {
      ...
      writeFileSync(scriptPath, terminalImeHarnessScript(runId))
      session = await mantaPage.context().newCDPSession(mantaPage)
      await sendToTerminal(mantaPage, ptyId, `node ${JSON.stringify(scriptPath)}\r`)
      harnessStarted = true
      await waitForTerminalOutput(mantaPage, `IME_HARNESS_READY_${runId}`, 10_000, 20_000)
      await focusActiveTerminalInput(mantaPage)
      ...
      await mantaPage.waitForTimeout(80)
      await dispatchSogouEmptyCompositionUpdate(mantaPage)
      await dispatchCandidateSelectionKey(session, { key: ' ', code: 'Space', keyCode: 32 }, () =>
        commitImeText(session, '你')
      )
      await waitForLivePrompt(mantaPage, '你')
      await attachImeEvidence(mantaPage, testInfo, 'sogou-after-space-commit')
      await mantaPage.keyboard.press('Enter')
      await expect
        .poll(async () => (await readPromptState(mantaPage))?.submitted.at(-1) ?? null, {
          timeout: 5_000,
      ...
      await waitForLivePrompt(mantaPage, '你好')
      await attachImeEvidence(mantaPage, testInfo, 'sogou-after-digit-commit')
      await mantaPage.keyboard.press('Enter')
      await expect
        .poll(async () => (await readPromptState(mantaPage))?.submitted.at(-1) ?? null, {
          timeout: 5_000,
      ...
      const postCompositionLogStart = (await readImeEventLog(mantaPage)).length
      await setImeComposition(session, 'zaijian')
      await mantaPage.waitForTimeout(80)
      await dispatchSogouEmptyCompositionUpdate(mantaPage)
      await dispatchSogouPostCompositionEnd(mantaPage, '再见')
      await dispatchCandidateSelectionKey(session, { key: '3', code: 'Digit3', keyCode: 51 }, () =>
        commitImeText(session, '再见')
      )
      await waitForLivePrompt(mantaPage, '再见')
      const postCompositionLog = await readImeEventLog(mantaPage)
      ...
      await attachImeEvidence(mantaPage, testInfo, 'sogou-after-post-composition-digit-commit')
      await mantaPage.keyboard.press('Enter')
      await expect
        .poll(async () => (await readPromptState(mantaPage))?.submitted.at(-1) ?? null, {
          timeout: 5_000,
      ...
      await expect
        ...
        .toBe('')
      await attachImeEvidence(mantaPage, testInfo, 'sogou-after-orphan-keyup-digit-suppression')

      const promptState = await readPromptState(mantaPage)
      expect(
  ...
  test('keeps composed Chinese text stable in the real Codex TUI input @real-codex-ime', async ({
    ...
    test.skip(
      process.env.MANTA_E2E_REAL_CODEX_IME !== '1',
      'Set MANTA_E2E_REAL_CODEX_IME=1 to exercise the locally installed Codex TUI'
    )
    ...
    try {
      await launchCodexTui(mantaPage, ptyId)
      await installImeEventProbe(mantaPage)

      await dispatchImeProcessKey(session, 'KeyN')
      await composeAndCommitChineseText(session, mantaPage, ['n', 'ni', '你', '你好'], '你好')
      await waitForCleanTerminalText(mantaPage, /你好/, 'Codex input did not show composed Chinese')
      await attachImeEvidence(mantaPage, testInfo, 'codex-after-compose-hello', {
        cleanTerminal: stripTerminalControls(await getTerminalContent(mantaPage, 20_000))
      })
      ...
      await attachImeEvidence(mantaPage, testInfo, 'codex-after-composition-backspace', {
        cleanTerminal: stripTerminalControls(await getTerminalContent(mantaPage, 20_000))
      })

      const cleanTerminal = stripTerminalControls(await getTerminalContent(mantaPage, 20_000))
      expect(
        cleanTerminal,
      ...
      await attachImeEvidence(mantaPage, testInfo, 'codex-final-ime-evidence', {
        cleanTerminal: stripTerminalControls(await getTerminalContent(mantaPage, 20_000))
      }).catch(() => undefined)
      await session.detach().catch(() => undefined)
      await sendToTerminal(mantaPage, ptyId, '\x03/quit\r').catch(() => undefined)
    }
  })
})
