...
const fakeCliDir = mkdtempSync(path.join(os.tmpdir(), 'manta-e2e-created-issue-prefill-'))
...
test('starting a just-created GitHub issue launches Claude with its URL prefilled', async ({
  ...
  const newIssueButton = mantaPage.getByRole('button', { name: 'New GitHub issue' })
  await expect(newIssueButton).toBeEnabled({ timeout: 15_000 })
  await newIssueButton.click()

  const createDialog = mantaPage.getByRole('dialog', { name: 'New GitHub issue' })
  ...
  await expect(mantaPage.getByRole('heading', { name: ISSUE_TITLE })).toBeVisible({
    timeout: 10_000
  })

  await mantaPage.getByRole('button', { name: 'Start workspace from issue' }).click()
  ...
  const composer = mantaPage.getByRole('dialog', { name: /Create (workspace|worktree)/i })
  ...
  await expect
    .poll(
      async () => {
        terminalText = await getTerminalContent(mantaPage, 12_000)
        return terminalText
      },
      {
        timeout: 30_000,
