/**
 * JOURNEY: two desktop clients paired to one Manta server, working in the same workspace.
 *
 * TOPOLOGY: the `mantaPage` app is the host (manta server). Two separate real Manta desktop
 * clients pair to it, exactly as two of the user's machines would. Nothing is faulted — this
 * is the ordinary shape of using Manta from a laptop and a desktop at the same time.
 *
...
test('two paired clients stay in step with the host across an emptied workspace', async ({
  ...
  try {
    ...
    const hostOwnView = await readMirroredTabCount(mantaPage, worktreeId)
    console.error(
