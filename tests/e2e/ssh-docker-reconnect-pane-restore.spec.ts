...
test.describe('SSH reconnect pane restore', () => {
  ...
  test('restores shell scrollback, a full-screen frame, and a usable new tab across a reconnect', async ({
    ...
    try {
      ...
      await execInTerminal(mantaPage, ptyId, 'top -b -n 1 > /dev/null; top')
      await waitForTerminalOutput(mantaPage, 'load average', 30_000, 8000)

      await reconnectDockerSshRelayTarget(mantaPage, remote.targetId)
      await waitForActiveTerminalManager(mantaPage, 60_000)
      await waitForActivePanePtyId(mantaPage, 60_000)

      await waitForTerminalOutput(mantaPage, 'load average', 60_000, 8000)
      const tuiContent = await getTerminalContent(mantaPage, 8000)
      ...
      await openTerminalTabInActiveGroup(mantaPage)
      await waitForActiveTerminalManager(mantaPage, 60_000)
      const freshPtyId = await waitForActivePanePtyId(mantaPage, 60_000)
      ...
      await execInTerminal(mantaPage, freshPtyId, `echo ${freshMarker}`)
      await waitForTerminalOutput(mantaPage, freshMarker, 60_000)

      // And it must be a FRESH shell, not a repaint of the old pane's history.
      const freshContent = await getTerminalContent(mantaPage, 8000)
      ...
      // never printed a prompt for Manta to read one from.
      await expect
        .poll(
          async () =>
            mantaPage.evaluate(() => {
