 ...
 *
 * Topology: headed Manta desktop host (remote server) + a separate paired Manta desktop client —
...
async function runScenario(
  ...
  const client = await launchPairedElectronClient(offer, testInfo, clientName, {
    extraEnv: {
      MANTA_E2E_TERMINAL_PARKING_DELAY_MS: String(PARK_DELAY_MS),
  ...
  try {
    const worktreeIds = await readHostWorktreeIds(mantaPage)
    await expect
      .poll(
        () =>
...
test.describe('host retains the buffer', () => {
  test('a cold-parked remote terminal restores its scrollback on reveal', async ({
    ...
    expect(await runScenario(mantaPage, testInfo, 'host-retains', 'ordinary')).toEqual(
      RESTORED_LOCAL_ONLY
    )
  })
})
...
test.describe('host retains nothing', () => {
  test.use({
    mantaAppExtraEnv: { ORCA_E2E_FORCE_REMOTE_TERMINAL_SNAPSHOT_UNAVAILABLE: '1' }
  })
  ...
  test('a cold-parked remote terminal keeps its scrollback when the host cannot answer', async ({
    ...
    test.setTimeout(600_000)
    expect(await runScenario(mantaPage, testInfo, 'host-empty', 'ordinary')).toEqual(
      RESTORED_LOCAL_ONLY
    )
  })
})
...
test.describe('force-park with a host that cannot answer', () => {
  test.use({
    mantaAppExtraEnv: {
      ...
      MANTA_E2E_DISABLE_PAIRED_TERMINAL_PARKING: '1'
    }
  })
  ...
  test('a force-parked remote terminal keeps its shared-layout capture across an inventory frame', async ({
    ...
    test.setTimeout(600_000)
    const outcome = await runScenario(mantaPage, testInfo, 'force-park', 'force')
    expect({
