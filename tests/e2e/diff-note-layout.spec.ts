...
async function assertCardClearsFollowingLine(page: Page): Promise<void> {
  const card = page.locator('.manta-diff-comment-card').first()
...
test.describe('Diff note layout', () => {
  ...
  test('saved notes reserve their rendered height in both diff layouts', async ({
    ...
    const added = await mantaPage.evaluate(
    ...
    await expect(mantaPage.locator('button:has(svg.lucide-rows-2)')).toBeVisible()
    await assertCardClearsFollowingLine(mantaPage)
    await attachDiffScreenshot(mantaPage, testInfo, 'side-by-side-diff-note-layout')

    await mantaPage.evaluate(() =>
      window.__store?.getState().updateSettings({ diffDefaultView: 'inline' })
    )
    await expect(mantaPage.locator('button:has(svg.lucide-columns-2)')).toBeVisible()
    await assertCardClearsFollowingLine(mantaPage)
    await attachDiffScreenshot(mantaPage, testInfo, 'inline-diff-note-layout')
  })
})
