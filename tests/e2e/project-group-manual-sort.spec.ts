...
async function createProjectHeaderSortFixture(): Promise<string[]> {
  ...
  const root = realpathSync(await mkdtemp(path.join(os.tmpdir(), 'manta-e2e-project-sort-')))
  tempRoots.push(root)
...
test.describe('Project Group manual sorting', () => {
  test('dragging a project header body reorders the visible project headers', async ({
    ...
    const projects = await seedProjectHeaderSortScenario(mantaPage, repoPaths)

    await expect
      .poll(() => getProjectHeaderOrder(mantaPage, projects), {
        timeout: 12_000,
  ...
  test('dragging a duplicate-ranked Project Group header reorders the visible headers', async ({
    ...
    await waitForSessionReady(mantaPage)
    const groups = await seedDuplicateTabOrderProjectGroups(mantaPage)

    await expect
      .poll(() => getProjectGroupHeaderOrder(mantaPage, groups), {
        timeout: 12_000,
