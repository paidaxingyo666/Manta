...
test.describe('Terminal Panes', () => {
  ...
  test('Always-on pane header split button hover stays transparent', async ({ mantaPage }) => {
    const splitButton = mantaPage.getByRole('button', { name: 'Split Terminal Right' })
   ...
   * - resizing terminal panes works
   */
  test('shows a pane divider after splitting', async ({ mantaPage }) => {
    ...
    await expect(mantaPage.locator('.pane-divider.is-vertical').first()).toBeVisible({
      timeout: 3_000
    })
  })
   ...
   *   MANTA_E2E_HEADFUL=1 pnpm run test:e2e
   */
  test('@headful can resize terminal panes by real mouse drag', async ({ mantaPage }) => {
    ...
    const paneWidthsBefore = await mantaPage.evaluate(() => {
    ...
    const divider = mantaPage.locator('.pane-divider.is-vertical').first()
    ...
    await mantaPage.mouse.move(startX + 150, startY, { steps: 20 })
    ...
    await expect
      .poll(
        async () => {
          const widthsAfter = await mantaPage.evaluate(() => {
            const xterms = document.querySelectorAll('.xterm')
            return Array.from(xterms)
              .filter((x) => (x as HTMLElement).offsetParent !== null)
              .map((x) => (x as HTMLElement).getBoundingClientRect().width)
  ...
  test('@headful resizing split panes forwards only the settled PTY size', async ({
    ...
    await expect
      .poll(
        async () =>
          (await readVisiblePaneContents(mantaPage)).every((content) =>
    ...
    await mantaPage.mouse.move(startX, startY)
    await mantaPage.mouse.down()
    await mantaPage.mouse.move(startX - 350, startY, { steps: 40 })
    await mantaPage.mouse.move(startX + 250, startY, { steps: 40 })
    await mantaPage.mouse.up()
    await mantaPage.waitForTimeout(500)

    const paneContents = await readVisiblePaneContents(mantaPage)
   ...
   * - closing panes works
   */
  test('closing a split pane removes it and remaining pane fills space', async ({ mantaPage }) => {
    const panesBefore = await countVisibleTerminalPanes(mantaPage)
    ...
    const panesAfterSplit = await countVisibleTerminalPanes(mantaPage)
    expect(panesAfterSplit).toBeGreaterThanOrEqual(2)

    await closeActiveTerminalPane(mantaPage)
    await waitForPaneCount(mantaPage, panesAfterSplit - 1)

    // The remaining pane should fill the available space
    const paneWidth = await mantaPage.evaluate(() => {
      const xterms = document.querySelectorAll('.xterm')
