...
const BAND = '.manta-diff-comment-range-highlight'
const COMPOSER = '.manta-diff-comment-popover'
const COMPOSER_LABEL = '.manta-diff-comment-popover-label'
const COMPOSER_TEXTAREA = '.manta-diff-comment-popover-textarea'
...
async function revealAddButton(
  ...
  const button = page.locator('.manta-diff-comment-add-btn')
...
test.describe('Diff note line range', () => {
  ...
  test('dragging the gutter selects a range, keeps it lit, and saves one ranged note', async ({
    ...
    await seedDiffFile(mantaPage, worktreeId, 'src/diff-note-range-drag.ts')

    const from = await gutterPoint(mantaPage, 4)
    const to = await gutterPoint(mantaPage, 9)

    await mantaPage.mouse.move(from.x, from.y)
    await mantaPage.mouse.down()
    // Anchor alone is lit before the pointer travels.
    await expect(mantaPage.locator(BAND)).toHaveCount(1)

    await mantaPage.mouse.move(to.x, (from.y + to.y) / 2)
    await mantaPage.mouse.move(to.x, to.y)
    await expect(
      mantaPage.locator(BAND),
      'the band did not follow the pointer while the button was held'
    ).toHaveCount(6)

    await mantaPage.mouse.up()

    await expect(mantaPage.locator(COMPOSER_LABEL)).toHaveText('Lines 4-9')
    await expect(
      mantaPage.locator(BAND),
    ...
    expect(await mantaPage.evaluate(() => window.getSelection()?.toString() ?? '')).toBe('')

    await submitNote(mantaPage, 'Collapse these six lines into a loop.')

    expect(await readNotes(mantaPage, worktreeId)).toEqual([
      { startLine: 4, lineNumber: 9, body: 'Collapse these six lines into a loop.' }
    ])
    const card = mantaPage.locator('.manta-diff-comment-card').first()
    ...
    await expect(mantaPage.locator(BAND)).toHaveCount(0)
  })
  ...
  test('dragging the gutter upward commits the same range as dragging down', async ({
    ...
    const worktreeId = await waitForActiveWorktree(mantaPage)
    await seedDiffFile(mantaPage, worktreeId, 'src/diff-note-range-drag-up.ts')

    const from = await gutterPoint(mantaPage, 9)
    const to = await gutterPoint(mantaPage, 4)
    ...
    await submitNote(mantaPage, 'Dragged bottom to top.')

    expect(await readNotes(mantaPage, worktreeId)).toEqual([
      { startLine: 4, lineNumber: 9, body: 'Dragged bottom to top.' }
    ])
    await expect(mantaPage.locator('.manta-diff-comment-card').first()).toContainText('lines 4-9')
  })
  ...
  test('dragging from the "+" itself selects a range and the button rides the selection', async ({
    ...
    const worktreeId = await waitForActiveWorktree(mantaPage)
    await seedDiffFile(mantaPage, worktreeId, 'src/diff-note-range-button-drag.ts')

    const button = await revealAddButton(mantaPage, 4)
    const to = await gutterPoint(mantaPage, 9)

    await mantaPage.mouse.move(button.x, button.y)
    await mantaPage.mouse.down()
    await mantaPage.mouse.move(button.x, (button.y + to.y) / 2)
    await mantaPage.mouse.move(button.x, to.y)
    ...
    await expect
      .poll(
        async () =>
          mantaPage.locator('.manta-diff-comment-add-btn').evaluate((node) => {
            if (!(node instanceof HTMLElement)) {
              throw new Error('the add button is not an HTMLElement')
            }
            return node.style.top
    ...
    await mantaPage.mouse.up()
    await expect(mantaPage.locator(COMPOSER_LABEL)).toHaveText('Lines 4-9')
    await submitNote(mantaPage, 'Dragged straight off the plus button.')
  ...
  test('a press with no drag still saves the single-line note it always did', async ({
    ...
    const worktreeId = await waitForActiveWorktree(mantaPage)
    await seedDiffFile(mantaPage, worktreeId, 'src/diff-note-range-click.ts')

    const point = await gutterPoint(mantaPage, 6)
    await mantaPage.mouse.move(point.x, point.y)
    await mantaPage.mouse.down()
    await mantaPage.mouse.up()

    await expect(mantaPage.locator(COMPOSER_LABEL)).toHaveText('Line 6')
    await expect(mantaPage.locator(BAND)).toHaveCount(1)

    await submitNote(mantaPage, 'Single line still works.')
    ...
    await expect(mantaPage.locator('.manta-diff-comment-card').first()).toContainText('line 6')
  })

  test('Escape abandons a drag without opening a composer', async ({ mantaPage }) => {
    const worktreeId = await waitForActiveWorktree(mantaPage)
    await seedDiffFile(mantaPage, worktreeId, 'src/diff-note-range-escape.ts')

    const from = await gutterPoint(mantaPage, 3)
    const to = await gutterPoint(mantaPage, 8)
    await mantaPage.mouse.move(from.x, from.y)
    await mantaPage.mouse.down()
    await mantaPage.mouse.move(to.x, to.y)
    await expect(mantaPage.locator(BAND)).toHaveCount(6)

    await mantaPage.keyboard.press('Escape')
    await expect(mantaPage.locator(BAND), 'Escape left the band behind').toHaveCount(0)

    await mantaPage.mouse.up()
    await expect(
      mantaPage.locator(COMPOSER),
  ...
  test('the Add Review Note chord turns an editor selection into a ranged note', async ({
    ...
    const worktreeId = await waitForActiveWorktree(mantaPage)
    await seedDiffFile(mantaPage, worktreeId, 'src/diff-note-range-chord.ts')

    // Click the code column, not the gutter: Monaco still owns that side.
    const line = await gutterPoint(mantaPage, 5)
    await mantaPage.mouse.click(line.x + 220, line.y)
    await mantaPage.keyboard.press('Shift+ArrowDown')
    await mantaPage.keyboard.press('Shift+ArrowDown')

    await pressShortcut(mantaPage, 'KeyA', { shift: true })

    await expect(mantaPage.locator(COMPOSER_LABEL)).toHaveText('Lines 5-7')
    await submitNote(mantaPage, 'Range from the keyboard.')
