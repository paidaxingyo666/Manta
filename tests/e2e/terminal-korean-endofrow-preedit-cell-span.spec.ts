...
test.describe('Terminal end-of-row Korean preedit cell span', () => {
  test('keeps the preedit caret inside the final terminal cell', async ({ mantaPage }, testInfo) => {
    ...
    try {
      ...
      await writeToActiveTerminal(mantaPage, '\x1b[2J\x1b[H\x1b[999Gx')
      await setImeComposition(arena.session, '가')

      const sample = await sampleOpenComposition(mantaPage)
  ...
  test('renders a composing syllable wider than the one cell #12729 measured', async ({
    ...
    try {
      ...
      await writeToActiveTerminal(mantaPage, '\x1b[2J\x1b[H안녕하세요')
      await setImeComposition(arena.session, '가')
