...
test('highlights bash and sh fences in Markdown Source mode', async ({ mantaPage }, testInfo) => {
  ...
  try {
    filePath = await createMarkdownFixture(
      context,
      '.manta-e2e-markdown-source-highlighting',
      'bash-and-sh',
    ...
    await switchToSourceMode(mantaPage)
