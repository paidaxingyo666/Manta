...
test.describe('Native chat message rail', () => {
  test('previews prompts and jumps without following later output', async ({ mantaPage }) => {
    ...
    const rail = mantaPage.locator('[data-native-chat-rail]')
    ...
    await rail.hover()
    const panel = mantaPage.getByRole('dialog', { name: 'Your messages' })
    ...
    const scroller = mantaPage.locator('[data-native-chat-scroll]')
