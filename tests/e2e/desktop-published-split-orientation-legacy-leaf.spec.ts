 ...
 * `mantaPage` fixture launches the app before any test body runs.
 */
...
test('publishes an unmounted split with its real orientation when a legacy leaf lingers in the saved tree', async ({
  ...
  const worktreeId = await mantaPage.evaluate(() => window.__store?.getState().activeWorktreeId)
  if (!worktreeId) {
  ...
  try {
    ...
    const hostTabId = await resolveActiveTabId(mantaPage)
    if (!hostTabId) {
    ...
    const panes = await readPaneIdentitySnapshot(mantaPage)
      ...
      .poll(
        async () =>
          splitDirectionSeparating(
            (await readSavedLayout(mantaPage, hostTabId))?.root,
            firstLeafId,
            secondLeafId
          ),
    ...
    await parkHiddenTabBehindDecoy(mantaPage, worktreeId, hostTabId, {
    ...
    expect(collectLeafIds((await readSavedLayout(mantaPage, hostTabId))?.root)).toContain(
      LEGACY_LEAF_ID
    )
