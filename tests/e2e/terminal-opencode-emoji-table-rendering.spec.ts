...
const EMOJI_TABLE_MARKER = 'MANTA_EMOJI_TABLE_RENDER_DONE'
...
test.describe('OpenCode emoji table terminal rendering', () => {
  test('keeps emoji table output visually sane and restores the cursor', async ({
    ...
    await waitForPtyShellEcho(mantaPage, ptyId, 20_000)
    const runId = randomUUID()
    const marker = `${EMOJI_TABLE_MARKER}_${runId}`
    const scriptPath = path.join(testRepoPath, `.manta-opencode-emoji-table-${runId}.mjs`)
    ...
    try {
      ...
      await forceCursorProbeTheme(mantaPage)
      await mantaPage.waitForTimeout(50)

      const renderState = await readActiveTerminalRenderState(mantaPage)
      const blinkSamples = await sampleCursorBlink(mantaPage)
  ...
  test('local real OpenCode demo keeps table rendering and cursor visible', async ({
    ...
    test.skip(
      process.env.MANTA_E2E_REAL_OPENCODE !== '1',
      'Set MANTA_E2E_REAL_OPENCODE=1 to exercise the locally installed OpenCode TUI'
    )
    ...
    try {
      await waitForTerminalOutput(mantaPage, 'Give me markdown table', 15_000)
      await waitForTerminalOutput(mantaPage, 'Emoji', 60_000)
      await waitForTerminalOutput(mantaPage, 'Alice', 60_000)
      await mantaPage.waitForTimeout(1_500)

      await testInfo.attach('real-opencode-demo-table', {
        body: await mantaPage.screenshot({ fullPage: true }),
