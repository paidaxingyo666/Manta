...
test.describe('SSH transport drop recovery', () => {
  ...
  test('recovers a live pane after the transport dies under it', async ({ mantaPage }, testInfo) => {
    ...
    try {
      ...
      await execInTerminal(mantaPage, ptyId, `printf 'DROP_MARKER_%s\\n' ${markerSuffix}`)
      await waitForTerminalOutput(mantaPage, marker, 30_000)
      ...
      await waitForTerminalOutput(mantaPage, marker, 60_000)
      ...
      await waitForTerminalOutput(mantaPage, afterMarker, 60_000)
    } finally {
      if (target) {
  ...
  test('stays bounded when a disconnected shell floods its pty', async ({
    ...
    try {
      ...
      await ensureTerminalVisible(mantaPage, 45_000)
      await waitForActiveTerminalManager(mantaPage, 240_000)
      const ptyId = await waitForActivePanePtyId(mantaPage, 240_000)
      ...
      await waitForTerminalOutput(mantaPage, 'FLOODED', 120_000, 20_000)
      ...
      await execInTerminal(
        mantaPage,
        await waitForActivePanePtyId(mantaPage, 240_000),
        `printf 'FLOOD_AFTER_%s\\n' ${markerSuffix}`
      )
      await waitForTerminalOutput(mantaPage, marker, 60_000, 20_000)
    } finally {
      if (target) {
        clearDockerSshRelayFaults(target)
        cleanupDockerSshRelayTarget(target)
  ...
  test('replaces the pane only when the host proves the session is gone', async ({
    ...
    try {
      ...
      await execInTerminal(mantaPage, ptyId, `printf 'KILL_MARKER_%s\\n' ${markerSuffix}`)
      await waitForTerminalOutput(mantaPage, marker, 30_000)
      ...
      await expect
        .poll(() => waitForActivePanePtyId(mantaPage, 60_000).catch(() => ptyId), {
          timeout: 120_000,
  ...
  test('keeps one reattachable lease per pane across repeated relay restarts', async ({
    ...
    try {
      ...
      for (let generation = 1; generation <= 5; generation++) {
        const previousPtyId = await waitForActivePanePtyId(mantaPage, 60_000)
        await recoverDockerSshRelayAfterFault(mantaPage, remote.targetId, () => {
          expect(
   ...
   */
  test('keeps the session while a frozen host goes silent', async ({ mantaPage }, testInfo) => {
    test.slow()
    let target: DockerSshRelayTarget | null = null
    try {
      target = startDockerSshRelayTarget(testInfo)
      ...
      await waitForSessionReady(mantaPage)
      await waitForActiveWorktree(mantaPage)
      await connectDockerSshRelayTarget(mantaPage, target, { relayGracePeriodSeconds: 0 })
      await ensureTerminalVisible(mantaPage, 45_000)
      await waitForActiveTerminalManager(mantaPage, 60_000)
      const ptyId = await waitForActivePanePtyId(mantaPage, 60_000)
      ...
      await execInTerminal(mantaPage, ptyId, `printf 'STALL_MARKER_%s\\n' ${markerSuffix}`)
      await waitForTerminalOutput(mantaPage, marker, 30_000)
      ...
      await withStalledDockerSshRelayTarget(target, async () => {
        await mantaPage.waitForTimeout(30_000)
      })
  ...
  test('accepts input again after a frozen host resumes', async ({ mantaPage }, testInfo) => {
    test.slow()
    let target: DockerSshRelayTarget | null = null
    ...
    try {
      ...
      await execInTerminal(mantaPage, ptyId, `printf 'STALL_BEFORE_%s\\n' ${beforeSuffix}`)
      await waitForTerminalOutput(mantaPage, `STALL_BEFORE_${beforeSuffix}`, 60_000)
      ...
      await recoverDockerSshRelayAfterFault(mantaPage, remote.targetId, async () => {
        await withStalledDockerSshRelayTarget(target!, async () => {
          await mantaPage.waitForTimeout(30_000)
        })
      })
      ...
      await execInTerminal(mantaPage, ptyId, `printf 'STALL_AFTER_%s\\n' ${afterSuffix}`)
      await attachSshRecoveryInputObservation(
        mantaPage,
        testInfo,
        remote.targetId,
