 ...
 */
async function readSshStatus(mantaPage: Page, targetId: string) {
...
test.describe('SSH transport drop recovery', () => {
  ...
  test('recovers a live pane after the transport dies under it', async ({ mantaPage }, testInfo) => {
    ...
    try {
      ...
      await expect
        .poll(() => readSshStatus(mantaPage, remote.targetId), {
          timeout: 120_000,
  ...
  test.fixme('stays bounded when a disconnected shell floods its pty', async ({
    ...
    try {
      ...
      await execInTerminal(
        mantaPage,
        ptyId,
        `yes "$(printf 'MANTA_%s' FLOOD_LINE)" | head -c 48000000; echo FLOODED`
      )
  ...
  test('keeps one reattachable lease per pane across repeated relay restarts', async ({
    ...
    try {
      ...
      for (let generation = 1; generation <= 5; generation++) {
        ...
        await execInTerminal(mantaPage, ptyId, `printf '%s\\n' ${marker}`)
        await waitForTerminalOutput(mantaPage, marker, 60_000)
   ...
   */
  test.fixme('accepts input again after a frozen host resumes', async ({ mantaPage }, testInfo) => {
    test.slow()
    let target: DockerSshRelayTarget | null = null
    try {
      target = startDockerSshRelayTarget(testInfo)
