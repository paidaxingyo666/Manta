/**
 * Regression: a worktree created via the CLI (`manta worktree
...
test.describe('worktree visibility with a remote runtime active', () => {
  test('a CLI-created worktree appears in the sidebar while a remote runtime is active', async ({
    ...
    const repoId = await mantaPage.evaluate(() => {
    ...
    // dir — exactly what `manta worktree create` does from a terminal.
    ...
    const worktreeRow = (worktreeId: string) =>
      mantaPage.locator(`[data-worktree-id=${JSON.stringify(worktreeId)}]`).first()
