...
test.describe('Terminal sleep wake restore', () => {
  test('restores slept terminal output and accepts fresh input after wake', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.manta-sleep-wake-restore-${runId}.mjs`)
    ...
    try {
      ...
      await waitForTerminalOutput(mantaPage, restoreMarker, 10_000, 20_000)
      const beforeSleepDebug = await readSleepWakeTerminalDebug(mantaPage, secondWorktreeId)
      for (const marker of expectedMarkers) {
        expect(await mainSnapshotContains(mantaPage, ptyId, marker)).toBe(true)
      }

      await switchToWorktree(mantaPage, firstWorktreeId)
      await sleepWorktreeTerminals(mantaPage, secondWorktreeId)
      const afterSleepDebug = await readSleepWakeTerminalDebug(mantaPage, secondWorktreeId)
      await expect
        .poll(() => readLivePtyCountForWorktree(mantaPage, secondWorktreeId), {
          timeout: 10_000,
          message: 'sleep did not release live PTYs for the background worktree'
        })
        .toBe(0)
      await expect
        .poll(() => readRemoteSleepOracle(mantaPage, secondWorktreeId), {
          timeout: 10_000,
      ...
      await ensureTerminalVisible(mantaPage)
      await waitForActiveTerminalManager(mantaPage, 30_000)
      const awakePtyId = await waitForActivePanePtyId(mantaPage)
      const afterWakeDebug = await readSleepWakeTerminalDebug(mantaPage, secondWorktreeId)
      const awakeTerminalContent = await getTerminalContent(mantaPage, 20_000)
      for (const marker of expectedMarkers) {
        expect
      ...
      await waitForTerminalOutput(mantaPage, restoreMarker, 15_000, 20_000)
      await sendToTerminal(mantaPage, awakePtyId, `printf '\\n${freshMarker}\\n'\r`)
      await waitForTerminalOutput(mantaPage, freshMarker, 10_000, 20_000)
    } finally {
