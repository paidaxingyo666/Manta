...
test.describe('Native chat transcript anchoring', () => {
  test('keeps the visible transcript row at the same viewport offset', async ({ mantaPage }) => {
    ...
    try {
      ...
      const scroll = mantaPage.locator('[data-native-chat-scroll]')
      const transcriptWindow = mantaPage.locator('[data-native-chat-window]')
      const loadEarlier = mantaPage.getByRole('button', { name: 'Load earlier messages' })
      ...
      const anchoredMarker = mantaPage.getByText(anchor.marker, { exact: true })
  ...
  test('keeps a detached transcript in place across a hidden update', async ({ mantaPage }) => {
    await waitForSessionReady(mantaPage)
    await waitForActiveWorktree(mantaPage)
    await ensureTerminalVisible(mantaPage)
    await waitForActiveTerminalManager(mantaPage, 30_000)
    ...
    try {
      ...
      const root = mantaPage.locator('[data-native-chat-root="true"]')
      const scroll = mantaPage.locator('[data-native-chat-scroll]')
      const jump = mantaPage.getByRole('button', { name: 'Jump to latest' })
      await expect(root).toBeVisible({ timeout: 15_000 })
      await expect(mantaPage.getByText('E2E transcript row 0649', { exact: true })).toBeAttached({
        timeout: 30_000
      })
      await scroll.hover()
      await mantaPage.mouse.wheel(0, -2_000)
      await expect
        .poll(async () =>
          scroll.evaluate(
      ...
      await activateNewTerminalTab(mantaPage, descriptor.worktreeId)
      await expect(root).toBeHidden()
      await publishHiddenLaunchMessage(mantaPage, {
        tabId,
      ...
      await expect(root).toBeVisible({ timeout: 15_000 })
      await expect(
        mantaPage.getByText('E2E update received while the transcript is hidden', { exact: true })
      ).toBeAttached()
      await expect
        .poll(async () =>
