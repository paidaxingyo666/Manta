...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'manta-paired-truncated-tail-'))
...
test.use({
  mantaAppExtraEnv: {
    MANTA_E2E_TERMINAL_PARKING_DELAY_MS: String(RETENTION_PARK_DELAY_MS),
...
test('legacy paired hosts retain the lossy hidden-manager budget fallback @headful', async ({
  ...
  test.setTimeout(120_000)
  const worktreeIds = await mantaPage.evaluate(() =>
    window.__store
      ?.getState()
      .allWorktrees()
      .slice(0, 2)
