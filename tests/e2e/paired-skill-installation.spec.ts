...
test('installs on a headed desktop runtime without a local fallback', async ({
  ...
  const folderRoot = mkdtempSync(join(tmpdir(), 'manta-paired-skill-folder-'))
  let client: PairedElectronClient | null = null
  try {
    const hostHome = await electronApp.evaluate(({ app }) => app.getPath('home'))
    const worktreeId = await activeWorktreeId(mantaPage)
    const folderWorkspaceId = await createHostFolderWorkspace(mantaPage, folderRoot)
...
async function connectCloud(page: Page): Promise<void> {
  const auth = await page.evaluate(() => window.api.mantaProfiles.connectCurrent())
