...
test.describe('Agent Session History live log', () => {
  test.use({
    mantaAppExtraArgs: ['--js-flags=--expose-gc', '--enable-precise-memory-info']
  })
  ...
  test('keeps a long View Log viewport stable while the transcript grows', async ({
    ...
    await mantaPage.setViewportSize({ width: 520, height: 720 })
    await openSessionHistory(mantaPage)
    const title = mantaPage.getByText(fixture.title, { exact: true }).first()
    await expect(title).toBeVisible({ timeout: 30_000 })
    await title.click()
    await mantaPage.getByText('View Log', { exact: true }).click()
    await expect
      .poll(() => readProbeOrNull(mantaPage), { timeout: 120_000 })
      .toMatchObject({ filePath: fixture.filePath, valueLength: fixture.initialLength })
    await mantaPage.setViewportSize({ width: 900, height: 720 })
    ...
    let baseline = await restoreAnchorState(mantaPage, true)
    ...
    for (let batch = 0; batch < 3; batch++) {
      ...
      replacementMemory.push(await measureLegacyControl(electronApp, mantaPage, baseline, batch))
      baseline = await restoreAnchorState(mantaPage, baseline.find.open, equivalentState.scrollTop)
      ...
      await forceGcAndSettle(mantaPage)
      const before = await readMemory(electronApp, mantaPage)
      appendFileSync(fixture.filePath, suffix)
      fixture.initialLength += suffix.length
      await mantaPage.waitForTimeout(APPEND_CADENCE_MS)
      await expect
        .poll(() => readProbe(mantaPage), { timeout: 30_000 })
        .toMatchObject({ valueLength: fixture.initialLength })
      const after = await readMemory(electronApp, mantaPage)
      await forceGcAndSettle(mantaPage)
      const settled = await readMemory(electronApp, mantaPage)
      suffixMemory.push({ before, after, settled })
      const current = await readProbe(mantaPage)
      ...
      expect(await mantaPage.evaluate(() => 2 + 2)).toBe(4)
      expect((await readMainProcessCrashProbe(electronApp)).processGone).toBeNull()
      baseline = current
      if (batch === 0) {
        await mantaPage.keyboard.press('Escape')
        await expect(mantaPage.locator('.monaco-editor .find-widget')).toHaveAttribute(
          'aria-hidden',
          'true'
        )
        baseline = await readProbe(mantaPage)
