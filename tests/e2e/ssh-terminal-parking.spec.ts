...
test.describe('SSH terminal hidden view parking', () => {
  ...
  test('parks a hidden SSH tab and restores its scrollback on reveal', async ({
    ...
    try {
      ...
      const sshPtyId = await waitForActivePanePtyId(mantaPage, 60_000)
      const sshTabId = await getActiveTabId(mantaPage)
      if (!sshTabId) {
      ...
      await expect
        .poll(() => terminalTailContains(mantaPage, `${marker}_200:`), {
          timeout: 30_000,
      ...
      await expect
        .poll(() => terminalTailContains(mantaPage, `${marker}_PAD_DONE:`), {
          timeout: 60_000,
      ...
      await parkHiddenTabBehindDecoy(mantaPage, remote.worktreeId, sshTabId, {
      ...
      await expect
        .poll(() => getTerminalContent(mantaPage, 2_000_000), {
          timeout: 15_000,
