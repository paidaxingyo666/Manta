...
test.describe('local HTTPS certificate trust', () => {
  ...
  test('approves one exact local certificate endpoint without trusting sibling tabs or ports', async ({
    ...
    try {
      ...
      const firstTab = await createBrowserTab(mantaPage, worktreeId, firstServer.schemeLessUrl)
      const firstSlot = browserSlot(mantaPage, firstTab.id)
      ...
      await expect
        .poll(() => readBrowserHeading(mantaPage, firstTab.id), { timeout: 10_000 })
        .toBe('Local HTTPS request 1')
      await expect
        .poll(() => readBrowserState(mantaPage, firstTab.id, '__localTlsState'))
      ...
      const secondTab = await createBrowserTab(mantaPage, worktreeId, firstServer.secureUrl)
      const secondSlot = browserSlot(mantaPage, secondTab.id)
      await expect(
      ...
      const probeTab = await createBrowserTab(mantaPage, worktreeId, siblingProbeServer.url)
      await expect
        .poll(() => readBrowserState(mantaPage, probeTab.id, '__siblingTlsProbe'))
        .toEqual({ asset: 'blocked', webSocket: 'blocked' })
      expect(firstServer.assetRequestCount()).toBe(1)
      expect(firstServer.webSocketConnectionCount()).toBe(1)

      await switchToBrowserTab(mantaPage, worktreeId, firstTab.id)
      await reloadBrowserGuest(mantaPage, firstTab.id)
      ...
      const firstAddressBar = firstSlot.locator('[data-manta-browser-address-bar="true"]')
