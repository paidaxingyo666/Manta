...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'manta-headless-stalled-stream-'))
...
test('recovers an ACK-starved stream from an isolated headless Manta host @headful', async ({
  testRepoPath
}) => {
