...
test.describe('PTY input write queue over SSH', () => {
  ...
  test('keeps fish query replies out of the next child stdin on an upstream relay pty', async ({
    ...
    try {
      ...
      const runId = String(Date.now())
      const home = `/tmp/manta-fish-${runId}`
      const prompt = `MANTA_SSH_FISH_${runId}> `
      ...
      const relayExports = execDockerSshRelayTargetCommand(
        target,
        'module=$(find /root/.manta-remote -type d -path \'*/node_modules/node-pty\' | head -n 1); node -e "const p=require(process.argv[1]); console.log(Object.keys(p.native || {}).join(\',\'))" "$module"'
      )
      ...
      await waitForTerminalOutput(mantaPage, prompt, 30_000, 80_000)

      const blocker = `node -e ${shellQuote(`console.log('BLOCKER_STARTED_${runId}'); setTimeout(() => process.exit(0), 5000)`)}`
      await execInTerminal(mantaPage, ptyId, blocker)
      await waitForTerminalOutput(mantaPage, `BLOCKER_STARTED_${runId}`, 30_000, 80_000)
      await execInTerminal(mantaPage, ptyId, `node ${shellQuote(childScript)}`)
      await waitForTerminalOutput(mantaPage, childReady, 30_000, 80_000)
      await sendToTerminal(mantaPage, ptyId, 'hello\r')
      await waitForTerminalOutput(mantaPage, `${childRead}:"hello\\n"`, 30_000, 80_000)
