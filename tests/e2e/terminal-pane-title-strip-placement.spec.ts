...
test.describe('Terminal Panes', () => {
  ...
  test('Set Title strip activates its pane and accepts file-path drops', async ({ mantaPage }) => {
    ...
    const initialSnapshot = await waitForPaneIdentitySnapshot(mantaPage, 1)
    ...
    const splitSnapshot = await waitForPaneIdentitySnapshot(mantaPage, 2)
    ...
    await expect
      .poll(async () => (await readPaneIdentitySnapshot(mantaPage))?.activeLeafId ?? null)
      .toBe(otherPane.leafId)

    const titleBar = mantaPage.locator('.pane-title-bar', { hasText: title }).first()
    ...
    await titleBar.evaluate((element, path) => {
      ...
      dataTransfer.setData('text/x-manta-file-path', path)
    ...
    await expect
      ...
      .toBe(titledLeafId)
    await expect
      .poll(async () => (await getTerminalContent(mantaPage)).includes(droppedPath), {
        timeout: 5_000,
  ...
  test('Set Title overlay follows its pane after same-count pane move', async ({ mantaPage }) => {
    ...
    await splitActiveTerminalPane(mantaPage, 'vertical')
    await waitForPaneCount(mantaPage, 2)
    const beforeMove = await waitForPaneIdentitySnapshot(mantaPage, 2)
    ...
    const beforeOrder = await readTerminalPaneDomLeafOrder(mantaPage)

    await expectPaneTitleAttachedToLeaf(mantaPage, title, titledLeafId)
    await moveTerminalPaneByLeafId(mantaPage, titledLeafId, target.leafId, 'right')

    await expect
      .poll(async () => readTerminalPaneDomLeafOrder(mantaPage), {
        timeout: 10_000,
  ...
  test('Set Title keeps the pane drag handle available over the title strip', async ({
    ...
    const titleTopHit = await mantaPage.evaluate(
      ({ title, titledLeafId }) => {
    ...
    await mantaPage.locator('.pane-title-bar', { hasText: title }).click({
      position: { x: 20, y: 18 }
    })
    await expect(mantaPage.locator('.pane-title-input')).toBeVisible()
  })

  test('@headful Set Title pane can be dragged from the title strip', async ({ mantaPage }) => {
    ...
    await splitActiveTerminalPane(mantaPage, 'vertical')
    await waitForPaneCount(mantaPage, 2)
    const beforeDrag = await waitForPaneIdentitySnapshot(mantaPage, 2)
    ...
    const titleDragHandle = mantaPage
    ...
    const targetBox = await mantaPage.locator(`.pane[data-leaf-id="${target.leafId}"]`).boundingBox()
    ...
    await mantaPage.mouse.move(sourceBox!.x + sourceBox!.width / 2, sourceBox!.y + 4)
    await mantaPage.mouse.down()
    await mantaPage.mouse.move(targetDropX, targetBox!.y + targetBox!.height / 2, {
      steps: 20
    })
    ...
    await expect
      ...
      .not.toEqual(beforeOrder)
    const afterDrag = await waitForPaneIdentitySnapshot(mantaPage, 2)
