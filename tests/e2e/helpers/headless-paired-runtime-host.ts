...
const PAIRING_URL_PATTERN = /manta:\/\/[^\s"\\]+/g
...
export function parseHeadlessPairedRuntimePairingOffer(
  ...
  if (
    readiness.type !== 'manta_server_ready' ||
...
function redactPairingMaterial(value: string): string {
  return value
    .replace(PAIRING_URL_PATTERN, 'manta://[redacted]')
...
export async function launchHeadlessPairedRuntimeHost(): Promise<HeadlessPairedRuntimeHost> {
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'manta-e2e-headless-paired-'))
  ...
  try {
    ...
    app = await electron.launch({
      args: [
        ...getMantaElectronLaunchArgs(mainPath, false),
        '--serve',
        '--serve-json',
        '--serve-port',
        '0',
