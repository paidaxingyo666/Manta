...
export async function launchHeadlessPairedRuntimeHost(
  ...
  const userDataDir = mkdtempSync(
    path.join(options.userDataParent ?? os.tmpdir(), 'manta-e2e-headless-paired-')
  )
  ...
  try {
    agentBrowserSocketDir = options.agentBrowserSocketParent
      ? mkdtempSync(path.join(options.agentBrowserSocketParent, 'manta-ab-'))
      : null
    writeFileSync(
    ...
    const launchServeProcess = (): Promise<ElectronApplication> =>
      electron.launch({
        ...
        args: [
          ...(options.executablePath ? [] : getMantaElectronLaunchArgs(mainPath, false)),
          '--serve',
          '--serve-json',
          '--serve-port',
          String(servePort),
