...
export async function waitForRestoredTerminalInputReady(
  ...
  while (Date.now() < deadline) {
    const marker = `MANTA_RESTORED_INPUT_READY_${randomUUID().replaceAll('-', '')}`
        ...
        ({ expectedPtyId, input, pendingAttempts }) => {
          for (const manager of window.__paneManagers?.values() ?? []) {
            ...
            const container = pane.container as HTMLElement & {
              __mantaE2eTerminalInputReadinessInstanceId?: string
            }
            container.__mantaE2eTerminalInputReadinessInstanceId ??= crypto.randomUUID()
            const paneInstanceId = container.__mantaE2eTerminalInputReadinessInstanceId
