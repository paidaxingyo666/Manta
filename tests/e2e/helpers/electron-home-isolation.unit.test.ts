...
function createUserDataDir(): string {
  const tempDir = mkdtempSync(path.join(os.tmpdir(), 'manta-home-isolation-test-'))
  ...
  it('strips ambient home and Codex state before forcing a disposable home', () => {
    ...
    const isolation = createElectronHomeIsolation({
      inheritedEnv: {
        ...
        MANTA_CODEX_HOME: '/real/manta-codex',
    ...
    expect(isolation.env.MANTA_CODEX_HOME).toBeUndefined()
  ...
  it('rejects generic fixture overlays that could escape the boundary', () => {
    ...
    expect(() =>
      createElectronHomeIsolation({
        inheritedEnv: {},
        launchEnv: {},
        extraEnv: { MANTA_E2E_USER_DATA_DIR: '/unsafe' },
        userDataDir: createUserDataDir(),
        realHome: '/real/home'
      })
    ).toThrow(/mantaAppExtraEnv\.MANTA_E2E_USER_DATA_DIR/)
  })
