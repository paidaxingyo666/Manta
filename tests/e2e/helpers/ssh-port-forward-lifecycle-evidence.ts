...
export function startRemoteHttpListener(
  ...
  execDockerSshRelayTargetCommand(
    target,
    [
      `nohup node -e ${shellQuote(script)} >/tmp/manta-http-${port}.log 2>&1 < /dev/null &`,
      `echo $! >/tmp/manta-http-${port}.pid`
    ].join(' ')
  )
  return Number(execDockerSshRelayTargetCommand(target, `cat /tmp/manta-http-${port}.pid`))
}
...
export function readRemoteListenerIdentity(
  ...
  const pid = Number(execDockerSshRelayTargetCommand(target, `cat /tmp/manta-http-${port}.pid`))
  return {
    pid,
