...
const TEMP_PREFIX = 'manta-9749-dg-'
...
function knownMantaUserDataDirs(): string[] {
  if (process.platform === 'darwin') {
    const appSupport = path.join(homedir(), 'Library', 'Application Support')
    return [path.join(appSupport, 'manta'), path.join(appSupport, 'manta-dev')]
  }
  if (process.platform === 'win32') {
    const roaming = process.env.APPDATA ?? path.join(homedir(), 'AppData', 'Roaming')
    return [path.join(roaming, 'manta'), path.join(roaming, 'manta-dev')]
  }
  const config = process.env.XDG_CONFIG_HOME ?? path.join(homedir(), '.config')
  return [path.join(config, 'manta'), path.join(config, 'manta-dev')]
}
...
function assertDisposableRoot(rootDir: string): void {
  ...
  for (const userDataDir of knownMantaUserDataDirs()) {
    if (isEqualToOrInside(rootDir, userDataDir)) {
      throw new Error('Refusing daemon-generation fixture inside real Manta user data')
    }
  }
}
