 ...
 *
 * Why: the default `mantaPage` fixture creates a fresh `userDataDir` per test
...
type RestartSession = {
  ...
  launch: (options?: LaunchOptions) => Promise<LaunchedManta>
...
export function createRestartSession(
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'manta-e2e-restart-'))
  ...
  const launch = async (options?: LaunchOptions): Promise<LaunchedManta> => {
    ...
    const app = await electron.launch({
      ...
      env: {
        ...homeIsolation.env,
        ...options?.extraEnv,
        MANTA_E2E_RUNTIME_WS_PORT: String(runtimeWsPort)
      }
    })
  ...
  const dispose = async (): Promise<void> => {
    ...
    if (process.env.MANTA_E2E_PRESERVE_RESTART_PROFILE === '1') {
      ...
      () =>
        readRestartRendererState(() =>
          page.evaluate(async (repoId) => {
            ...
            // worktree is external to Manta's workspace root after the visibility rollout.
