...
const DIRECT_SSH_PROBE_CANARY_TARGET_ID = '__manta_e2e_direct_ssh_probe_canary__'
...
export async function launchPairedElectronClient(
  ...
  const userDataDir =
    options.reuseUserDataDir ?? mkdtempSync(path.join(os.tmpdir(), 'manta-e2e-paired-desktop-'))
  ...
  const app = await electron.launch({
    args: getMantaElectronLaunchArgs(mainPath, false),
    env: {
      ...homeIsolation.env,
      NODE_ENV: 'development',
      MANTA_E2E_HEADLESS: '1',
      MANTA_E2E_FORBID_LOCAL_SSH_CONNECT_PROBE: directSshProbePath
    }
  })
