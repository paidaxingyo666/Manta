...
export type PairedQuickOpenLargeTreeFixture = {
  ...
  mantaIgnoredTargetPath: string
  root: string
}

export function createPairedQuickOpenLargeTreeFixture(): PairedQuickOpenLargeTreeFixture {
  const root = mkdtempSync(path.join(os.tmpdir(), 'manta-paired-quick-open-large-tree-'))
  try {
    execFileSync('git', ['init'], { cwd: root, stdio: 'pipe' })
    writeFileSync(path.join(root, '.gitignore'), 'data/\n')
    writeFileSync(path.join(root, '.mantaignore'), 'manta-ignored/\n')
    ...
    const mantaIgnoredTargetPath = 'manta-ignored/sta-4354-mantaignore-target.ts'
    mkdirSync(path.join(root, 'manta-ignored'))
    closeSync(openSync(path.join(root, ...mantaIgnoredTargetPath.split('/')), 'w'))
    truncateSync(
      path.join(root, 'data', 'chunk-000000', `${LONG_STEM}000000.bin`),
      LARGE_TREE_NOMINAL_IGNORED_BYTES
    )

    execFileSync('git', ['add', '.gitignore', '.mantaignore'], { cwd: root, stdio: 'pipe' })
    execFileSync(
      'git',
      [
        '-c',
        'user.name=Manta E2E',
        '-c',
        'user.email=manta-e2e@example.invalid',
        'commit',
        '-m',
