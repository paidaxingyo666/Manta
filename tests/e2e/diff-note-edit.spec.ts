...
test.describe('Diff note edit', () => {
  ...
  test('editing a saved inline note updates the open diff card', async ({ mantaPage }) => {
    ...
    const addResult = await mantaPage.evaluate(
      async ({ wId, rel, body }) => {
        const store = window.__store
        if (!store) {
    ...
    await expect(card.locator('.manta-diff-comment-body')).toHaveText(seededBody)

    await card.getByTitle('Edit note').click()

    const textarea = card.locator('.manta-diff-comment-popover-textarea')
    await expect(textarea).toBeVisible()
    await expect(textarea).toHaveValue(seededBody)

    const saveButton = card
      .locator('.manta-diff-comment-popover-footer button')
      .filter({ hasText: 'Save' })
    ...
    const updatedCard = mantaPage
      .locator('.manta-diff-comment-card')
      .filter({ has: mantaPage.locator('.manta-diff-comment-body', { hasText: editedBody }) })
      .first()
    await expect(updatedCard, 'inline card did not update in the open diff').toBeVisible()
    await expect(updatedCard.locator('.manta-diff-comment-body')).toHaveText(editedBody)
    await expect(updatedCard.locator('.manta-diff-comment-body')).not.toHaveText(seededBody)
