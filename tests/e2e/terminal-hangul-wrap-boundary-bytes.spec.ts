 ...
 * Manta's to guarantee, and the half that had no coverage.
 */
  ...
  for (const scenario of SCENARIOS) {
    for (const policy of scenario.policies) {
      test(`sends ${scenario.name} unaltered to the pty (${policy})`, async ({
        ...
        await expectImePlatformPolicy(mantaPage, policy)
        await narrowPaneBySplitting(mantaPage, 2)
        ...
        try {
          ...
          const observedCols = await waitForSettledCols(mantaPage, arena.ptyId)
          ...
          const startRow = (await readPaneGrid(mantaPage, arena.ptyId)).cursorRow
          if (scenario.offsetByOneCell) {
            await dispatchImeRewrittenPrintableKey(arena.session, OFFSET_KEY)
          }
          await scenario.drive(arena.session, mantaPage, run)
          ...
          await expect
            .poll(async () => (await readPaneGrid(mantaPage, arena.ptyId)).cursorRow, {
              timeout: 10_000,
          ...
          const received = await waitForTerminalImeBytes(mantaPage, reader, 20_000)
