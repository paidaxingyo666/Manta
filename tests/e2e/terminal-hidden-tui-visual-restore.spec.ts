...
async function writeHiddenSideEffectBurst(
  ...
  await runNodeScriptInTerminal(page, ptyId, script, { prefix: 'manta-hidden-side-effect' })
}
...
test.describe('Hidden terminal TUI visual restore', () => {
  test('restores hidden full-screen TUI output without visible corruption', async ({
    ...
    const hiddenSnapshot = await waitForPaneIdentitySnapshot(mantaPage, 1)
    ...
    await expect
      .poll(() => getActiveWorktreeId(mantaPage), {
        timeout: 10_000,
    ...
    const scriptPath = path.join(testRepoPath, `.manta-hidden-tui-visual-${runId}.mjs`)
    writeHiddenFrameScript(scriptPath, runId)
    await resetHiddenDebug(mantaPage)
    try {
      await writeHiddenFrames(mantaPage, hiddenPane.ptyId, scriptPath)
      await resetHiddenDebug(mantaPage)
      ...
      await expect
        .poll(() => readMainHiddenDeliveryDroppedChars(mantaPage), {
          timeout: 10_000,
          message: 'visually rich hidden TUI output was not withheld from the renderer'
        })
        .toBeGreaterThan(1024)
      await expect
        .poll(() => readMainSnapshotSource(mantaPage, hiddenPane.ptyId!), {
          timeout: 10_000,
      ...
      await expect
        .poll(() => readTuiCursorState(mantaPage), {
          timeout: 5_000,
      ...
      await sendToTerminal(mantaPage, hiddenPane.ptyId, '\x03').catch(() => undefined)
  ...
  test('keeps newer live output correct after plain hidden output restores', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.manta-low-risk-hidden-${runId}.mjs`)
    writeLowRiskFrameScript(scriptPath, hiddenFrame)
    await resetHiddenDebug(mantaPage)
    await sendToTerminal(mantaPage, hiddenPane.ptyId, `node ${JSON.stringify(scriptPath)}\r`)
    await resetHiddenDebug(mantaPage)
    ...
    await ensureTerminalVisible(mantaPage)
    await waitForActiveTerminalManager(mantaPage, 30_000)
    await injectPaneData(mantaPage, paneKey, liveFrame, {
  ...
  test('restores rich synchronized TUI output from the headless model', async ({
    ...
    const runId = randomUUID()
    const finalMarker = `VISUAL_RESTORE_FINAL_${runId}_24`
    const scriptPath = path.join(testRepoPath, `.manta-hidden-rich-model-${runId}.mjs`)
    writeHiddenFrameScript(scriptPath, runId)
    await resetHiddenDebug(mantaPage)
    try {
      await writeHiddenFrames(mantaPage, hiddenPane.ptyId, scriptPath)
  ...
  test('keeps hidden terminal side effects live while hidden output may restore', async ({
    ...
    await resetHiddenDebug(mantaPage)
    await writeHiddenSideEffectBurst(mantaPage, hiddenPane.ptyId, hiddenTitle, marker)

    await expect
      .poll(() => getRuntimePaneTitle(mantaPage, hiddenSnapshot.tabId, hiddenPane.numericPaneId), {
        timeout: 10_000,
        message: 'hidden OSC title did not update renderer-visible model state'
      })
      .toBe(hiddenTitle)
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(mantaPage)).includes(hiddenSnapshot.tabId), {
        timeout: 10_000,
