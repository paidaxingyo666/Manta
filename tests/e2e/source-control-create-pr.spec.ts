...
async function seedCreatePREligibleBranch(
  ...
  return page.evaluate(async ({ createResult }) => {
    ...
    const pr = {
      ...
      state: 'open' as const,
      url: 'https://github.com/acme/manta/pull/73',
...
test.describe('Source Control create pull request', () => {
  ...
  test('creates the pull request from the Source Control primary action', async ({ mantaPage }) => {
    const { branch, worktreeId } = await seedCreatePREligibleBranch(mantaPage)
    await openSourceControl(mantaPage, worktreeId)
    await forceCreatePREligibleStatus(mantaPage, worktreeId, branch)

    const titleInput = mantaPage.getByRole('textbox', { name: 'Pull request title' })
    const descriptionInput = mantaPage.getByRole('textbox', {
      name: 'Pull request description'
    })
    const createButton = getCreatePRComposerSubmitButton(mantaPage)
    await expect(createButton).toBeVisible({ timeout: 10_000 })
    await expect(createButton).toBeEnabled()
    await expect(titleInput).toHaveValue('E2e secondary')
    await expect(mantaPage.getByRole('combobox', { name: 'Pull request base branch' })).toHaveValue(
      'main'
    )
    ...
    const payloads = await mantaPage.evaluate(
      () => (window as unknown as { __createPRPayloads: CreatePRPayload[] }).__createPRPayloads
    )
  ...
  test('surfaces create failures without clearing the pull request composer', async ({
    ...
    await expect(mantaPage.getByText(failureMessage)).toBeVisible()
