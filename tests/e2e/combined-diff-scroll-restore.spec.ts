...
function createCombinedDiffScrollRepo(): CombinedDiffScrollRepo {
  const repoPath = realpathSync(mkdtempSync(path.join(os.tmpdir(), 'manta-combined-diff-scroll-')))
...
test.describe('Combined diff scroll restore', () => {
  ...
  test('keeps the visible section anchored after switching tabs', async ({ mantaPage }) => {
    await waitForSessionReady(mantaPage)
    const fixture = createCombinedDiffScrollRepo()

    try {
      const worktreeId = await addAndActivateRepo(mantaPage, fixture.repoPath)
      const diffTabId = await openCombinedDiff(mantaPage, worktreeId, fixture.repoPath)
      await expect(mantaPage.locator('.combined-diff-scroll-container')).toBeVisible()
      await expect(mantaPage.getByText(`${FILE_COUNT} changed files`)).toBeVisible()

      await scrollCombinedDiffDeep(mantaPage)
      await waitForStableViewportAnchor(mantaPage)
      const activeScrollSamples = await wheelCombinedDiffDown(mantaPage)
      ...
      const beforeSwitch = await waitForStableViewportAnchor(mantaPage)
      ...
      await expect(mantaPage.locator('.combined-diff-scroll-container')).toHaveCount(0)

      await mantaPage.locator(`[data-tab-id="${diffTabId}"]`).click({ force: true })
      await expect(mantaPage.locator('.combined-diff-scroll-container')).toBeVisible()
      const afterSwitch = await waitForRestoredViewportAnchor(mantaPage, beforeSwitch)

      expect(afterSwitch.key).toBe(beforeSwitch.key)
      expect(Math.abs(afterSwitch.top - beforeSwitch.top)).toBeLessThan(80)

      await clickVisibleDiffLine(mantaPage)
      const afterLineClick = await waitForStableViewportAnchor(mantaPage)
