...
test('panel consent enables real local transcript search; clearing restores history', async ({
  ...
  await mantaPage.getByRole('button', { name: 'Agents', exact: true }).click()
  await mantaPage.getByRole('radio', { name: 'All', exact: true }).click()
  const input = mantaPage.getByRole('textbox', { name: 'Search sessions', exact: true })
  await input.fill('nebulariver')
  await expect(mantaPage.getByText('Enable full-text search?', { exact: false })).toBeVisible()
  ...
  await expect(mantaPage.getByText('Enable full-text search?', { exact: false })).toHaveCount(0)
  expect(
    await mantaPage.evaluate(async () => (await window.api.aiVault.searchStatus('local')).enabled)
  ).toBe(false)
  await input.fill('nebulariver')
  await mantaPage.getByRole('button', { name: 'Enable', exact: true }).click()
  ...
  await expect
    ...
    .toBeGreaterThan(0)
  await mantaPage.getByRole('button', { name: 'Refresh Session History', exact: true }).click()
  await expect(mantaPage.locator('mark').filter({ hasText: 'nebulariver' })).toBeVisible()
  await expect(mantaPage.getByText('Synthetic panel transcript', { exact: true })).toBeVisible()
  await screenshot('results.png')
  await mantaPage.getByTitle('Drag to resume in a new tab', { exact: true }).click()
  await expect(mantaPage.locator('mark').filter({ hasText: 'nebulariver' })).toBeVisible()
  await mantaPage.getByTitle('Drag to resume in a new tab', { exact: true }).click()
  const title = mantaPage.getByText('Synthetic panel transcript', { exact: true })
  ...
  const drag = await title.evaluate((element) => {
    ...
    const payload = dataTransfer.getData('application/x-manta-ai-vault-session')
  ...
  await expect(
    mantaPage.getByRole('menuitem', { name: 'Copy Session ID', exact: true })
  ).toBeVisible()
  await mantaPage.keyboard.press('Escape')
  await expect(mantaPage.locator('[role="menu"]')).toHaveCount(0)
  ...
  await expect(mantaPage.locator('html')).toHaveClass(/dark/)
  await screenshot('results-dark-narrow.png')
  await title.click({ button: 'right' })
  await mantaPage.getByRole('menuitem', { name: 'Delete', exact: true }).click()
  await mantaPage.getByRole('button', { name: 'Delete', exact: true }).click()
  await expect(title).toHaveCount(0)
  await expect(mantaPage.locator('mark')).toHaveCount(0)
  await input.fill('nothingmatchesprseven')
  await expect(
    mantaPage.getByText('No matching sessions in the indexed history.', { exact: false })
  ).toBeVisible()
  await screenshot('empty.png')
  await input.press('Escape')
  await expect(input).toHaveValue('')
  await expect(mantaPage.getByText('Indexed history · best matches', { exact: false })).toHaveCount(
    0
  )
  await cdp.detach()
})
...
test('panel renders transport failure and unavailable reasons without a local fallback', async ({
  ...
  await expect(
    mantaPage.getByText('Could not search this computer.', { exact: false })
  ).toBeVisible()
  await mantaPage.screenshot({ path: testInfo.outputPath('failure.png') })
  for (const reason of ['disabled', 'not-ready', 'no-service'] as const) {
    ...
    await mantaPage.getByRole('button', { name: 'Try again', exact: true }).click()
    const copy =
      reason === 'disabled'
    ...
    await expect(mantaPage.getByText(copy, { exact: false })).toBeVisible()
  }
})
