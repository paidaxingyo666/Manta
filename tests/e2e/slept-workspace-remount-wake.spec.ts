...
test('remounting a slept hidden pane does not respawn its PTY', async ({ mantaPage }) => {
  await waitForSessionReady(mantaPage)
  const [slept, other] = await getAllWorktreeIds(mantaPage)
  expect(other, 'seeded repo must expose two worktrees').toBeTruthy()
  await giveWorkspaceALivePty(mantaPage, slept)
  await giveWorkspaceALivePty(mantaPage, other)
  await activateWorkspaceByClick(mantaPage, slept)
  expect((await readWorkspaceSample(mantaPage, slept)).livePtyCount).toBeGreaterThan(0)

  await sleepWorkspaceViaSidebar(mantaPage, slept)
  await expect
    .poll(async () => (await readWorkspaceSample(mantaPage, slept)).livePtyCount, {
      timeout: 20_000,
      message: 'sleep did not release the workspace PTYs'
    })
    .toBe(0)
  await activateWorkspaceByClick(mantaPage, other)

  const sample = await readWorkspaceSample(mantaPage, slept)
  ...
  const remounted = await mantaPage.evaluate(
  ...
  await assertStaysCold(mantaPage, slept)
  ...
  expect((await readWorkspaceSample(mantaPage, slept)).livePtyCount).toBe(1)
  expect(await readHostLiveTerminalCount(mantaPage, slept)).toBe(1)
})
