...
test('dispatches coordinate pointer input fast enough for real gestures', async ({ mantaPage }) => {
  ...
  try {
    ...
    const pageId = await createBrowserTab(mantaPage, worktreeId!, server.url)

    await expect
      .poll(() => evaluateInPage(mantaPage, pageId, 'document.title'), { timeout: 20_000 })
    ...
    const latency = await mantaPage.evaluate(async (targetPage) => {
    ...
    await evaluateInPage(mantaPage, pageId, 'window.__events = []; true')
    const clickPairStarted = Date.now()
    await driveRpcSequence(mantaPage, pageId, [...clickSteps(120, 60), ...clickSteps(120, 60)])
    ...
    await evaluateInPage(mantaPage, pageId, 'window.__events = []; true')
    await driveRpcSequence(mantaPage, pageId, [
    ...
    await expect
      .poll(() =>
        evaluateInPage(mantaPage, pageId, 'window.__events.some((e) => e.type === "contextmenu")')
      )
      .toBe('true')

    // ── Drag selection ──

    await evaluateInPage(mantaPage, pageId, 'window.getSelection().removeAllRanges(); true')
    await driveRpcSequence(mantaPage, pageId, [
    ...
    await expect
      .poll(() => evaluateInPage(mantaPage, pageId, 'String(window.getSelection())'))
      .toContain('alpha')
    ...
    await expect
      .poll(
        async () =>
          Number(
            await evaluateInPage(mantaPage, pageId, 'document.querySelector("#scroller").scrollTop')
          ),
        { timeout: 10_000 }
      )
      .toBeGreaterThan(0)
