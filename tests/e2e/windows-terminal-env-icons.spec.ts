...
test.describe('Windows terminal env and shell identity', () => {
  ...
  test('native Windows tab icons stay pinned to the effective shell at tab creation', async ({
    ...
    const tabSnapshot = await mantaPage.evaluate(({ fallbackTabId, cmdTabId }) => {
    ...
    const fallbackTab = mantaPage.locator(
  ...
  test('WSL project tab icons retain runtime ownership across global shell changes', async ({
    ...
    const tabIds = await mantaPage.evaluate(async () => {
      const store = window.__store!
    ...
    const shells = await mantaPage.evaluate((ids) => {
    ...
    for (const id of tabIds) {
      const tab = mantaPage.locator(`[data-testid="sortable-tab"][data-tab-id="${id}"]`)
