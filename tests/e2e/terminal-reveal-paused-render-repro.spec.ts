...
test.describe('terminal reveal paused-render recovery', () => {
  test("reveal repaint forces a render through xterm's paused gate", async ({ mantaPage }) => {
    // Why: __store / __paneManagers live on the main Manta renderer window
    // (mantaPage), not Playwright's default first page.
    const page = mantaPage
  ...
  test('@headful atlas recovery presents a synchronized-output WebGL frame', async ({
    ...
    await forceWebglOn(mantaPage, tabId)
    const webglAttached = await mantaPage
      .waitForFunction(
        (tabId) =>
    ...
    const installed = await installSynchronizedRevealProbe(mantaPage, tabId)
    expect(installed, 'WebGL renderer internals are available').toBe(true)

    await callSynchronizedRevealProbe(mantaPage, 'paintFrame', {
    ...
    const held = await readSynchronizedRevealProbe(mantaPage)
    ...
    await expect
      .poll(async () => (await readSynchronizedRevealProbe(mantaPage)).atlasClears)
      .toBeGreaterThan(0)
    const afterReveal = await captureStableTabScreenshot(mantaPage, tabId)

    await callSynchronizedRevealProbe(mantaPage, 'forceRendererPresent')
    const afterForcedPresent = await captureStableTabScreenshot(mantaPage, tabId)
  ...
  test('@headful reveal preserves the coherent frame until synchronized output releases', async ({
    ...
    expect(await installSynchronizedRevealProbe(mantaPage, tabId)).toBe(true)
    ...
    const coherent = await captureStableTabScreenshot(mantaPage, tabId)
    const beforeHide = await readSynchronizedRevealProbe(mantaPage)
    const siblingTabId = await mantaPage.evaluate((worktreeId) => {
      const state = window.__store?.getState()
      if (!state) {
    ...
    await expect(mantaPage.locator(`[data-terminal-tab-id="${tabId}"]`)).toBeHidden()
    ...
    await mantaPage.evaluate((tabId) => window.__store?.getState().setActiveTab(tabId), tabId)
    await expect.poll(() => getActiveTabId(mantaPage)).toBe(tabId)
    const held = await captureFirstRevealedFrame(mantaPage, tabId)
    const heldState = await readSynchronizedRevealProbe(mantaPage)
    ...
    const released = await captureStableTabScreenshot(mantaPage, tabId)
    const releaseState = await readSynchronizedRevealProbe(mantaPage)
    await callSynchronizedRevealProbe(mantaPage, 'forceRendererPresent')
    const direct = await captureStableTabScreenshot(mantaPage, tabId)
    ...
    const beforeWatchdog = await readSynchronizedRevealProbe(mantaPage)
    await mantaPage.evaluate((tabId) => {
      window.__paneManagers?.get(tabId)?.scheduleRevealPresent?.()
    }, tabId)
    await expect
      .poll(async () => (await readSynchronizedRevealProbe(mantaPage)).synchronizedOutput, {
        timeout: 2_500
      })
      .toBe(false)
    const watchdog = await captureStableTabScreenshot(mantaPage, tabId)
    const watchdogState = await readSynchronizedRevealProbe(mantaPage)
    await callSynchronizedRevealProbe(mantaPage, 'forceRendererPresent')
    const watchdogDirect = await captureStableTabScreenshot(mantaPage, tabId)
