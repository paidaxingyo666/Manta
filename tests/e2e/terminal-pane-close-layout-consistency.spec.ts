...
const PARKING_DELAY_MS = Number(process.env.MANTA_E2E_TERMINAL_PARKING_DELAY_MS) || 2_000
...
test.describe('terminal pane close vs hidden/park lifecycle keeps layout consistent', () => {
  test('control: close while visible', async ({ mantaPage }) => {
    const { tabId } = await setUpSplitTab(mantaPage)
    await closeLastPaneOnTab(mantaPage, tabId)
    await expectLayoutConsistent(mantaPage, tabId, 1, 'close-visible')
  })

  test('close and hide the tab in the same tick', async ({ mantaPage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(mantaPage)
    ...
    await waitForTabRemounted(mantaPage, tabId)
    await expectLayoutConsistent(mantaPage, tabId, 1, 'close-then-hide-same-tick')
  })

  test('close while hidden but still mounted (hot-retain window)', async ({ mantaPage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(mantaPage)
    ...
    await activateTerminalTab(mantaPage, tabId)
    await waitForTabRemounted(mantaPage, tabId)
    await expectLayoutConsistent(mantaPage, tabId, 1, 'close-while-hidden-mounted')
  })

  test('close immediately after reveal remount, before panes settle', async ({ mantaPage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(mantaPage)
    await createActiveTerminalTab(mantaPage, worktreeId)
    await parkHiddenTabBehindDecoy(mantaPage, worktreeId, tabId, {
      parkDelayMs: PARKING_DELAY_MS
    ...
    await expectLayoutConsistent(mantaPage, tabId, 1, 'close-mid-reveal')
  })

  test('clean visible close survives a later park/reveal cycle', async ({ mantaPage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(mantaPage)
    await closeLastPaneOnTab(mantaPage, tabId)
    await expectLayoutConsistent(mantaPage, tabId, 1, 'pre-park close')
    await createActiveTerminalTab(mantaPage, worktreeId)
    await parkHiddenTabBehindDecoy(mantaPage, worktreeId, tabId, {
      parkDelayMs: PARKING_DELAY_MS
    })
    await activateTerminalTab(mantaPage, tabId)
    await waitForTabRemounted(mantaPage, tabId)
    await expectLayoutConsistent(mantaPage, tabId, 1, 'post-park-reveal')
  })

  test('split pane shell exits while hidden but still mounted', async ({ mantaPage }) => {
    const { worktreeId, tabId, splitPtyId } = await setUpSplitTab(mantaPage)
    await createActiveTerminalTab(mantaPage, worktreeId)
    // The setup-script analog: the split's shell ends on its own while the
    // tab is hidden-but-mounted.
    await sendToTerminal(mantaPage, splitPtyId, 'exit\r')
    await mantaPage.waitForTimeout(PARKING_DELAY_MS / 2)
    await activateTerminalTab(mantaPage, tabId)
    await waitForTabRemounted(mantaPage, tabId)
    await expectLayoutConsistent(mantaPage, tabId, 1, 'shell-exit-while-hidden-mounted', splitPtyId)
  })

  test('split pane shell exits while the tab is parked', async ({ mantaPage }) => {
    const { worktreeId, tabId, splitPtyId } = await setUpSplitTab(mantaPage)
    await createActiveTerminalTab(mantaPage, worktreeId)
    await parkHiddenTabBehindDecoy(mantaPage, worktreeId, tabId, {
      parkDelayMs: PARKING_DELAY_MS
    ...
    await sendToTerminal(mantaPage, splitPtyId, 'exit\r')
    await mantaPage.waitForTimeout(PARKING_DELAY_MS)
    await activateTerminalTab(mantaPage, tabId)
    await waitForTabRemounted(mantaPage, tabId)
    ...
    await expectLayoutConsistent(mantaPage, tabId, 1, 'shell-exit-while-parked', splitPtyId)
  })
})
