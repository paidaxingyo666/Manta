...
test('floating Mobile Emulator tab renders content and closes from the tab strip', async ({
  ...
  const tab = await seedFloatingSimulatorTab(mantaPage)
  await openFloatingPanelIfNeeded(mantaPage)

  const openPanel = mantaPage.locator(OPEN_PANEL_SELECTOR).first()
  ...
  await attachPanelScreenshot(mantaPage, testInfo)
