...
test('keeps conventional commit-message lines intact in the history tooltip', async ({
  ...
  await openSourceControlForWorktree(mantaPage, testRepoPath, fixture.worktreePath)

  const commitsToggle = mantaPage.getByRole('button', { name: /Commits/ })
  await expect(commitsToggle).toBeVisible()
  await commitsToggle.click()

  const row = mantaPage.getByTestId('git-history-row').filter({ hasText: subject })
