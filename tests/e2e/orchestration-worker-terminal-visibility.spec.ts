...
const fakeCliDir = mkdtempSync(path.join(os.tmpdir(), 'manta-e2e-orchestration-worker-'))
...
appendLedger('MANTA_E2E_SPAWN_LEDGER', { event: 'spawn', startedAt: Date.now() })
...
test('worker-start preserves one live inactive worker across workspace re-entry', async ({
  ...
  const coordinatorTabId = await getActiveTabId(mantaPage)
  ...
  const workerTab = mantaPage.locator(
  ...
  await expect(
    mantaPage.locator(`[data-testid="sortable-tab"][data-tab-id="${coordinatorTabId}"]`)
  ...
  const otherWorktreeId = await switchToOtherWorktree(mantaPage, worktreeId)
  ...
  await expect(workerTab).toBeVisible()
  await expect(
    mantaPage.locator(`[data-testid="sortable-tab"][data-tab-id="${workerTerminal!.tabId}"]`)
  ).toHaveCount(1)
  await expect(
    mantaPage.locator(`[data-testid="sortable-tab"][data-tab-title="${workerTabTitle}"]`)
  ).toHaveCount(1)
  ...
  await expect(mantaPage.locator('body')).not.toContainText('Conversation interrupted')
})
