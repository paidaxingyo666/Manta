...
test('keeps long repository names inside the cross-project confirmation dialog', async ({
  ...
  const tempRoot = mkdtempSync(path.join(os.tmpdir(), 'manta-e2e-cross-project-dialog-'))
  const secondRepositoryPath = path.join(tempRoot, LONG_REPOSITORY_NAME)
  createGitRepository(secondRepositoryPath)

  try {
    const secondRepositoryId = await mantaPage.evaluate(async (repositoryPath) => {
    ...
    const currentRepositoryId = await mantaPage.evaluate(() => {
      const store = window.__store
    ...
    await mantaPage.setViewportSize({ width: 640, height: 720 })
    ...
    const confirmation = mantaPage.getByRole('dialog', { name: 'Switch project?' })
    await expect(confirmation).toBeVisible()
    await expect(confirmation).toContainText(LONG_REPOSITORY_NAME)

    if (process.env.MANTA_VISUAL_PROOF === '1') {
