...
test.describe('Option-composed ASCII in a kitty-keyboard pane', () => {
  ...
  test('types the composed character instead of reporting the physical Alt chord', async ({
    ...
    const { joinedWrites } = await setUpPane(mantaPage, electronApp)
    await setMacOptionAsAlt(mantaPage, 'false')
    await clearPtyWriteLog(electronApp)

    // Turkish Q: the physical `q` key composes `@`.
    const dispatch = await pressOptionComposedKey(mantaPage, { key: '@', code: 'KeyQ' })
  ...
  test('types a composed character that also needs Shift', async ({ mantaPage, electronApp }) => {
    const { joinedWrites } = await setUpPane(mantaPage, electronApp)
    await setMacOptionAsAlt(mantaPage, 'false')
    await clearPtyWriteLog(electronApp)
    ...
    const dispatch = await pressOptionComposedKey(mantaPage, {
      key: '\\',
      code: 'Digit7',
      shiftKey: true
    })
  ...
  test('still reports the Alt chord when Option is configured as Alt', async ({
    ...
    const { joinedWrites } = await setUpPane(mantaPage, electronApp)
    await setMacOptionAsAlt(mantaPage, 'true')
    await clearPtyWriteLog(electronApp)
  ...
  test('keeps non-ASCII Option glyphs as TUI hotkeys', async ({ mantaPage, electronApp }) => {
    const { joinedWrites } = await setUpPane(mantaPage, electronApp)
    await setMacOptionAsAlt(mantaPage, 'false')
    await clearPtyWriteLog(electronApp)

    // #8031: OMP-class TUIs bind Option+P, which composes the non-ASCII `π`.
    const dispatch = await pressOptionComposedKey(mantaPage, { key: 'π', code: 'KeyP' })
    expect(dispatch.keydownDefaultPrevented).toBe(true)
