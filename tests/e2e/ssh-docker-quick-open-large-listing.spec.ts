/**
 * #12547 acceptance: open a repository the size of Manta's own checkout over SSH and list its files.
 *
...
test.skip(!RUN_DOCKER_SSH, 'Set MANTA_E2E_SSH_DOCKER=1 to run the Docker SSH relay lane')
...
test('lists a monorepo-sized remote workspace, with and without a client page size (#12547)', async ({
  ...
  try {
    ...
    const connected = await connectDockerSshRelayTarget(mantaPage, target, {
