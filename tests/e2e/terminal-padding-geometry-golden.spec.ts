...
test.describe('terminal padding geometry', () => {
  ...
  test('keeps configured padding on all four edges with a forced cell remainder @terminal-rendering-golden', async ({
    ...
    const baseline = await readTerminalGeometry(mantaPage)
    const firstSize = {
    ...
    await applyPaddingAndPaneSize(mantaPage, FIRST_PADDING, firstSize)
    const first = await readTerminalGeometry(mantaPage)
    ...
    await applyPaddingAndPaneSize(mantaPage, SECOND_PADDING, secondSize)
    const second = await readTerminalGeometry(mantaPage)
