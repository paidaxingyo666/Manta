 ...
 * Manta's file-path provider resolves link candidates asynchronously.
 */
...
test.describe('Terminal link hover after worktree return', () => {
  ...
  test('re-establishes a URL link on hover after the pointer leaves the terminal', async ({
    ...
    const url = `https://example.com/manta-link-${randomUUID()}`
    await sendToTerminal(mantaPage, ptyId, `echo ${url}\r`)
    await expect
      .poll(() => getTerminalContent(mantaPage, 4000), {
        timeout: 10_000,
    ...
    const probe = await locateHoverProbe(mantaPage, url)
    await expect
      .poll(() => hoverAndReadActiveLinkText(mantaPage, probe), {
        timeout: 5_000,
        message: 'baseline hover never established the URL link'
      })
      .toContain(url)

    await dispatchScreenMouseLeave(mantaPage, probe.tabId)
    await expect.poll(() => readActiveLinkText(mantaPage, probe.tabId)).toBeNull()
    await expect.poll(() => readTerminalCursor(mantaPage, probe.tabId)).not.toBe('pointer')
    ...
    await expect
      ...
      .toContain(url)
    await expect.poll(() => readTerminalCursor(mantaPage, probe.tabId)).toBe('pointer')
  })
  ...
  test('re-establishes a file-path link on hover after switching worktrees and back', async ({
    ...
    const worktreePath = await activeWorktreePath(mantaPage)
    const fileName = `manta-linkfile-${randomUUID().slice(0, 8)}.txt`
    const filePath = path.join(worktreePath, fileName)
    writeFileSync(filePath, 'manta file link target\n')
    const needle = `./${fileName}`

    try {
      await sendToTerminal(mantaPage, ptyId, `echo ${needle}\r`)
      await expect
        .poll(() => getTerminalContent(mantaPage, 4000), {
          timeout: 10_000,
      ...
      const probe = await assertLinkRecoversAfterReturn(mantaPage, {
        firstWorktreeId,
        secondWorktreeId,
        needle,
        expectContains: fileName
      })
      await activateHoveredLink(mantaPage, probe)
      ...
      await expect(mantaPage.locator('.editor-header-path').first()).toContainText(fileName, {
        timeout: 20_000
      })
    } finally {
  ...
  test('re-establishes a URL link on hover after switching worktrees and back', async ({
    ...
    await assertLinkRecoversAfterReturn(mantaPage, {
      firstWorktreeId,
      secondWorktreeId,
      needle: url,
      expectContains: url
