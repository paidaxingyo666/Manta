...
const SETUP_SCRIPT_COMMAND = 'echo manta-e2e-setup'
...
/** Repo whose shared manta.yaml carries a real setup script, plus a second worktree. */
function createRepoWithSharedSetupScript(repoPath: string, featureWorktreePath: string): void {
  ...
  writeFileSync(path.join(repoPath, 'README.md'), '# Unreadable manta.yaml E2E\n')
...
/**
 * Makes the main process report the failure the fix now surfaces: manta.yaml could
 ...
 */
async function installUnreadableMantaYamlFault(electronApp: ElectronApplication): Promise<void> {
  await electronApp.evaluate(({ ipcMain }) => {
    type InvokeHandler = (event: unknown, ...args: unknown[]) => unknown
    const faultState = globalThis as typeof globalThis & { __mantaE2eMantaYamlUnreadable?: boolean }
    ...
    faultState.__mantaE2eMantaYamlUnreadable = true
    ipcMain.removeHandler('hooks:check')
    ipcMain.handle('hooks:check', async (event, ...args) => {
      if (faultState.__mantaE2eMantaYamlUnreadable) {
...
/** manta.yaml becomes readable again — every later check runs the production handler. */
async function healMantaYamlRead(electronApp: ElectronApplication): Promise<void> {
  await electronApp.evaluate(() => {
    ;(
      globalThis as typeof globalThis & { __mantaE2eMantaYamlUnreadable?: boolean }
    ).__mantaE2eMantaYamlUnreadable = false
  })
}
...
test.describe('Setup script prompt', () => {
  ...
  test('recovers from an unreadable manta.yaml instead of pinning the failed verdict', async ({
    electronApp,
    mantaPage
  }, testInfo) => {
    const repoPath = testInfo.outputPath('unreadable-manta-yaml-repo')
    const featureWorktreePath = testInfo.outputPath('unreadable-manta-yaml-feature')
    createRepoWithSharedSetupScript(repoPath, featureWorktreePath)

    await installUnreadableMantaYamlFault(electronApp)
    ...
    const promptCard = mantaPage.locator('[data-setup-script-prompt-layer]')
    const inspectionError = promptCard.getByText(INSPECTION_ERROR_TEXT)
    await expect(inspectionError).toBeVisible({ timeout: 20_000 })

    // manta.yaml is readable again; the card is still pinned to the failed verdict.
    await healMantaYamlRead(electronApp)
    const healthyCheck = await mantaPage.evaluate(
    ...
    await mantaPage.waitForTimeout(RECORDING_DWELL_MS)

    // Activating another worktree in the same repo must re-inspect.
    const featureRow = worktreeRow(mantaPage, featureWorktreeId)
    await expect(featureRow).toBeVisible()
    await worktreeRowSurface(mantaPage, featureWorktreeId).click()
    await expect(featureRow).toHaveAttribute('aria-current', 'page')

    // The repo has a valid manta.yaml scripts.setup, so no prompt may remain.
