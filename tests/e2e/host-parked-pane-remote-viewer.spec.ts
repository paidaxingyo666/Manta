 ...
 *
 * Topology: headed Manta desktop host (remote server) + a separate paired Manta
...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'manta-host-park-viewer-'))
...
test.use({ mantaAppExtraEnv: { MANTA_E2E_TERMINAL_PARKING_DELAY_MS: String(PARK_DELAY_MS) } })
...
test('a cold-parked host pane keeps serving its paired remote viewer', async ({
  ...
  const previousParkDelay = process.env.MANTA_E2E_TERMINAL_PARKING_DELAY_MS
  process.env.MANTA_E2E_TERMINAL_PARKING_DELAY_MS = String(PARK_DELAY_MS)
  ...
  try {
    ...
    await expect
      .poll(() => mantaPage.evaluate((id) => window.__paneManagers?.has(id) ?? false, hostTabId), {
        timeout: 60_000,
    ...
    await waitForTabParked(mantaPage, hostTabId, { parkDelayMs: PARK_DELAY_MS })
    ...
    while (Date.now() < timelineDeadline) {
      const hostMounted = await mantaPage.evaluate(
    ...
    if (previousParkDelay === undefined) {
      delete process.env.MANTA_E2E_TERMINAL_PARKING_DELAY_MS
    } else {
      process.env.MANTA_E2E_TERMINAL_PARKING_DELAY_MS = previousParkDelay
    }
