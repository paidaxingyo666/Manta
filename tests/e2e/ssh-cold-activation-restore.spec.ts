...
test.describe('SSH cold activation restore', () => {
  ...
  test('eagerly remounts every restored remote terminal after renderer reload', async ({
    ...
    try {
      ...
      const tabStrip = mantaPage.locator('.terminal-tab-strip').first()
      const firstTab = mantaPage.getByRole('button', { name: /^Terminal 1 Close tab Terminal 1/ })
      await expect
        .poll(
          async () => {
            ...
            await mantaPage.mouse.move(box.x + box.width / 2, box.y + box.height / 2)
            await mantaPage.mouse.down()
            await mantaPage.mouse.up()
            return mantaPage.evaluate(() => window.__store?.getState().activeTabId ?? null)
          },
          {
            timeout: 30_000,
      ...
      await expectTerminalAccessibilityText(mantaPage, firstTabId, marker)
