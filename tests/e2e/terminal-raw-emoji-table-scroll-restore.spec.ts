...
test.describe('Terminal raw emoji table scroll restore repro', () => {
  ...
  test('uses WebGL by default for ordinary terminal output when available @terminal-rendering-golden', async ({
    ...
    const marker = `MANTA_AUTO_WEBGL_SMOKE_${randomUUID()}`

    await sendToTerminal(mantaPage, ptyId, `printf ${JSON.stringify(`${marker}\\n`)}\r`)
    await waitForTerminalOutput(mantaPage, marker, 10_000)

    const expectedWebgl = await expectAutoWebgl(mantaPage)
    ...
    let diagnostics = await readTerminalRenderDiagnostics(mantaPage)
    await expect
      .poll(
        async () => {
          diagnostics = await readTerminalRenderDiagnostics(mantaPage)
  ...
  test('keeps raw emoji box table aligned after restore and scroll @terminal-rendering-golden', async ({
    ...
    await ensureTerminalVisible(mantaPage)
    await waitForActiveTerminalManager(mantaPage, 30_000)
    await setWideRenderedTableViewport(mantaPage)
    await waitForActiveTerminalColumns(mantaPage, RAW_EMOJI_BOX_TABLE_WIDTH)
    const ptyId = await waitForActivePanePtyId(mantaPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.manta-raw-emoji-fixture-table-${runId}.mjs`)
    ...
    try {
      ...
      await waitForTerminalOutput(mantaPage, completionMarker, 20_000, 30_000)
      await expect
      ...
      await scrollActiveTerminalToText(mantaPage, 'Singer')
      await closeFeatureTips(mantaPage)
      const expectedWebgl = await expectAutoWebgl(mantaPage)
      ...
      const singerGeometry = await readVisibleSingerRowGeometry(mantaPage)
