...
test.describe('five SSH panes under simultaneous output', () => {
  test.skip(process.env.MANTA_E2E_SSH_DOCKER !== '1', 'Requires the Docker SSH target')
  ...
  test('each pane acknowledges keyboard input after hiding and reopening the flooding workspace', async ({
    ...
    for (let index = 0; index < 5; index++) {
      ...
      const identity = await readPaneIdentitySnapshot(mantaPage)
      ...
      await execInTerminal(mantaPage, ptyId, floodWithInputAcknowledgements(marker))
      await expect
        .poll(() => getTerminalContent(mantaPage, 80_000), { timeout: 60_000 })
    ...
    for (let round = 0; round < 2; round++) {
      ...
      for (const [index, owner] of owners.entries()) {
        ...
        expect(await waitForActivePanePtyId(mantaPage)).toBe(owner.ptyId)
        await focusActiveTerminalInput(mantaPage)
        const input = `input_${runId}_${round}_${index}`
        const inputTrace = await mantaPage.evaluateHandle((tabId) => {
          const manager = window.__paneManagers!.get(tabId)!
        ...
        try {
          await mantaPage.keyboard.type(input)
          await mantaPage.keyboard.press('Enter')
          await expect
            .poll(() => getTerminalContent(mantaPage, 80_000), { timeout: 30_000 })
            .toMatch(new RegExp(`${owner.marker}:[1-9][0-9]*:ACK=${input}:`))
        } catch (error) {
          const panes = await mantaPage.evaluate((tabId) => {
            const manager = window.__paneManagers!.get(tabId)!
