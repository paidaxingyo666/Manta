...
test.describe('Terminal 2-Set Korean preedit visibility', () => {
  test('shows every assembling jamo at non-zero size and commits the syllable ahead of the newline', async ({
    ...
    try {
      ...
      await expectPreeditHidden(mantaPage, 'before composing')

      for (const frame of HAN_FRAMES) {
        await composeHangulSyllable(arena.session, mantaPage, [frame])
        await expectPreeditRendered(mantaPage, frame.preedit, `composing ${frame.preedit}`)
      }

      await commitImeText(arena.session, '한')
      await expectPreeditHidden(mantaPage, 'after committing 한')

      await dispatchPlainEnter(arena.session)

      const received = await waitForTerminalImeBytes(mantaPage, reader)
  ...
  test('keeps a preedit the IME resumes without a compositionstart visible', async ({
    ...
    try {
      ...
      await dispatchResumedCompositionUpdate(mantaPage, '한')

      const sample = await samplePreeditOverlay(mantaPage)
  ...
  test('renders the preedit at every update of a recorded Windows/WSL Hangul session', async ({
    ...
    await applyImePlatformPolicy(mantaPage, 'windows')
    const arena = await openTerminalImePaneArena(mantaPage)
    let completed = false
    try {
      const replay = await replayRecordedImeDomTrace(mantaPage, RECORDED_TRACE)
  ...
  test('loses and duplicates nothing when back-to-back syllables commit at full speed', async ({
    ...
    try {
      ...
      for (let repetition = 0; repetition < 4; repetition += 1) {
        await composeHangulSyllable(arena.session, mantaPage, HAN_FRAMES, 0)
        await commitImeText(arena.session, '한')
        await composeHangulSyllable(arena.session, mantaPage, GEUL_FRAMES, 0)
