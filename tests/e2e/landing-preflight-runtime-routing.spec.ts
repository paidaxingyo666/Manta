...
const missingGitPath = mkdtempSync(path.join(os.tmpdir(), 'manta-preflight-path-'))
...
test('routes landing preflight across runtime switch and reconnect', async ({
  electronApp,
  mantaPage
}, testInfo) => runRuntimePreflightJourney(electronApp, mantaPage, testInfo, false))

test('routes landing preflight across runtime switch and reconnect @headful', async ({
  electronApp,
  mantaPage
}, testInfo) => runRuntimePreflightJourney(electronApp, mantaPage, testInfo, true))
