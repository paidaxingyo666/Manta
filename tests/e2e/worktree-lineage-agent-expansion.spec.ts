...
// Set MANTA_CAPTURE_EVIDENCE=1 to also write before/after screenshots to
// pr-evidence/. Off by default so CI just runs the behavioral assertions.
const CAPTURE_EVIDENCE = process.env.MANTA_CAPTURE_EVIDENCE === '1'
...
test.describe('Worktree lineage agent-list expansion independence', () => {
  ...
  test('toggling child worktrees does not collapse the expanded agent summary', async ({
    ...
    const parentRow = worktreeRow(mantaPage, parentId)
    const childRow = worktreeRow(mantaPage, childId)

    await parentRow.click()
    await expect(parentRow).toHaveAttribute('aria-current', 'page')

    await seedTwoParentAgents(mantaPage, parentId)

    // Both sections present: the "2 agents" summary and the child-workspaces chip.
    await expect(compactSummary(mantaPage, parentId)).toBeVisible({ timeout: 10_000 })
    await expect(childWorkspacesChip(mantaPage, parentId)).toBeVisible()
    await expect(childRow).toBeVisible()
    await expect(compactSummary(mantaPage, parentId)).toHaveAttribute('aria-expanded', 'false')
    await captureSidebar(mantaPage, '1-before-both-collapsed.png')

    // Expand the agent summary.
    await compactSummary(mantaPage, parentId).click()
    await expect(compactSummary(mantaPage, parentId)).toHaveAttribute('aria-expanded', 'true')
    await captureSidebar(mantaPage, '2-agents-expanded.png')

    // Collapse the child worktrees via the chip. This remounts the parent card.
    await childWorkspacesChip(mantaPage, parentId).click()
    await expect(childRow).toBeHidden()

    // FIXED: the agent summary stays expanded despite the card remount.
    await expect(compactSummary(mantaPage, parentId)).toHaveAttribute('aria-expanded', 'true')
    await captureSidebar(mantaPage, '3-after-children-toggle-agents-still-expanded.png')
  })
})
