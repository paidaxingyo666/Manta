...
import { attachRepoAndOpenTerminal } from './helpers/manta-restart'
import {
...
const DRAFT = 'MANTA_CODEX_PASTE_DRAFT_SHOULD_STAY_UNSENT'
...
test.describe('Windows Codex multiline paste', () => {
  ...
  test('multiline Ctrl+V keeps the existing Codex draft unsent @local-real-codex', async ({
    ...
    await activateTestRepository(mantaPage, testRepoPath)
    ...
    await sendToTerminal(mantaPage, ptyId, 'codex -m manta-e2e-invalid-model\r')
    await expect
      .poll(() => getTerminalContent(mantaPage, 12_000), { timeout: 20_000 })
      .toMatch(/Do[\s\S]*you[\s\S]*trust[\s\S]*contents|OpenAI Codex/i)
    if (CODEX_TRUST_PROMPT_RE.test(await getTerminalContent(mantaPage, 12_000))) {
      await sendToTerminal(mantaPage, ptyId, '\r')
    }
    await waitForTerminalOutput(mantaPage, 'OpenAI Codex', 20_000, 30_000)
    await waitForCodexComposerReady(mantaPage)
    await enableTerminalAccessibilityDom(mantaPage, ptyId)
    await focusActiveTerminalInput(mantaPage)
    await mantaPage.keyboard.type(DRAFT)
    const terminalDom = mantaPage.locator(
      `[data-pty-id=${JSON.stringify(ptyId)}] .xterm-accessibility-tree`
    )
    await expect(terminalDom).toContainText(DRAFT, { timeout: 10_000 })
    await mantaPage.evaluate((text) => window.api.ui.writeClipboardText(text), pastePayload())
  ...
  test('delivers a normalized large paste through native ConPTY', async ({
    ...
    const marker = `MANTA_LARGE_PASTE_${randomUUID().replaceAll('-', '')}`
    ...
    try {
      ...
      await waitForTerminalOutput(mantaPage, `${marker}_READY`, 10_000, 12_000)
      await enableTerminalAccessibilityDom(mantaPage, ptyId)
      await focusActiveTerminalInput(mantaPage)
