...
test.describe('Terminal column desync repro', () => {
  test('PTY columns stay in sync with xterm across a visible resize', async ({ mantaPage }) => {
    test.setTimeout(120_000)
    await waitForSessionReady(mantaPage)
    await waitForActiveWorktree(mantaPage)
    await closeRightSidebarAndFeatureTips(mantaPage)
    await ensureTerminalVisible(mantaPage)
    const ptyId = await settleTerminal(mantaPage)
    ...
    const expectColumnsInSync = async (label: string): Promise<void> => {
      await expect
        .poll(
          async () => {
            const snap = await readColumnSnapshot(mantaPage, ptyId)
    ...
    await mantaPage.setViewportSize({ width: 760, height: 800 })
    await expectColumnsInSync('after shrink')

    await mantaPage.setViewportSize({ width: 1280, height: 800 })
  ...
  test('pty:getSize reports the size the PTY actually applied', async ({ mantaPage }) => {
    test.setTimeout(120_000)
    await waitForSessionReady(mantaPage)
    await waitForActiveWorktree(mantaPage)
    await closeRightSidebarAndFeatureTips(mantaPage)
    ...
    await mantaPage.setViewportSize({ width: 900, height: 800 })
    ...
    await expect
      .poll(
        async () => {
          const ptyCols = await readPtyCols(mantaPage, ptyId)
          const reportedCols = await readReportedPtyCols(mantaPage, ptyId)
  ...
  test('PTY columns re-sync after the terminal is resized while hidden', async ({ mantaPage }) => {
    test.setTimeout(120_000)
    await waitForSessionReady(mantaPage)
    const homeWorktreeId = await waitForActiveWorktree(mantaPage)
    const otherWorktreeId = (await getAllWorktreeIds(mantaPage)).find((id) => id !== homeWorktreeId)
    ...
    const ptyId = await settleTerminal(mantaPage)
    await mantaPage.setViewportSize({ width: 1280, height: 800 })
    await mantaPage.waitForTimeout(400)

    const baseline = await readColumnSnapshot(mantaPage, ptyId)
    ...
    await mantaPage.setViewportSize({ width: 720, height: 800 })
    ...
    const afterReturn = await readColumnSnapshot(mantaPage, ptyId)
    expect(
      afterReturn.ptyCols,
  ...
  test('PTY columns re-sync after repeated background resizes', async ({ mantaPage }) => {
    ...
    for (const [index, width] of widths.entries()) {
      await switchToWorktree(mantaPage, otherWorktreeId)
      await waitForActiveTerminalManager(mantaPage, 30_000)
      await mantaPage.setViewportSize({ width, height: 800 })
      await mantaPage.waitForTimeout(350)
      await switchToWorktree(mantaPage, homeWorktreeId)
      await ensureTerminalVisible(mantaPage)
      await waitForActiveTerminalManager(mantaPage, 30_000)
      await mantaPage.waitForTimeout(500)

      const snapshot = await readColumnSnapshot(mantaPage, ptyId)
      expect(
        snapshot.ptyCols,
  ...
  test('both panes keep PTY columns synced after a vertical split reparent', async ({
    ...
    const firstPtyId = await settleTerminal(mantaPage)

    const baseline = await readColumnSnapshot(mantaPage, firstPtyId)
    expect(baseline.ptyCols).toBe(baseline.xtermCols)
    ...
    for (const pane of snapshot.panes) {
      ...
      const ptyCols = await readPtyCols(mantaPage, ptyId)
      const xtermCols = await readRenderedColsForPty(mantaPage, ptyId)
      expect(
        ptyCols,
  ...
  test('both panes stay PTY-synced when a tab MOUNTS with a split layout present', async ({
    ...
    await closeRightSidebarAndFeatureTips(mantaPage)
    await ensureTerminalVisible(mantaPage)
    await mantaPage.setViewportSize({ width: 1440, height: 900 })
    await mantaPage.waitForTimeout(300)
    await settleTerminal(mantaPage)
    ...
    for (let attempt = 0; attempt < MOUNT_ATTEMPTS; attempt += 1) {
      ...
      await mantaPage.setViewportSize({ width: 1180, height: 800 })
      await mantaPage.waitForTimeout(300)
