...
test.describe('Markdown prose reflow', () => {
  ...
  test('hard-wrapped prose reflows as one document paragraph', async ({ mantaPage }, testInfo) => {
    let filePath: string | null = null

    try {
      filePath = await openHardWrappedFixture(mantaPage, testInfo)
      const metrics = await getGoalParagraphMetrics(mantaPage)
  ...
  test('deleting an inserted empty paragraph keeps hard-wrapped prose reflowing', async ({
    ...
    try {
      filePath = await openHardWrappedFixture(mantaPage, testInfo)
      await placeCaretAtGoalParagraphEnd(mantaPage)
  ...
  test('deleting the block boundary before a heading keeps hard-wrapped prose reflowing', async ({
    ...
    try {
      filePath = await openHardWrappedFixture(mantaPage, testInfo)
      await placeCaretAtHeadingStart(mantaPage)
      await mantaPage.keyboard.press('Backspace')
