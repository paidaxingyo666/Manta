...
test('shows interrupted hidden SSH cleanup as retryable', async ({ electronApp, mantaPage }) => {
  const userDataPath = await electronApp.evaluate(({ app }) => app.getPath('userData'))
  writeFileSync(
    path.join(userDataPath, 'manta-ephemeral-vm-runtimes.json'),
    JSON.stringify({
      version: 1,
      runtimes: [
        {
  ...
  const runtimes = mantaPage.locator('[data-settings-section="temporary-vm-runtimes"]')
...
test('stops long-running cleanup and keeps it retryable', async ({ electronApp, mantaPage }) => {
  const repoPath = mkdtempSync(path.join(tmpdir(), 'manta-cleanup-stop-'))
  ...
  try {
    ...
    const repoId = await mantaPage.evaluate(async (repo) => {
    ...
    await openCloudVmRuntimes(mantaPage)
    const runtimes = mantaPage.locator('[data-settings-section="temporary-vm-runtimes"]')
    ...
    const dialog = mantaPage.getByRole('dialog', { name: 'Stop cleanup?' })
    ...
    await expect(runtimes.getByRole('button', { name: 'Retry cleanup' })).toBeVisible()
    await expect(mantaPage.getByText('Cleanup stopped by user.')).toBeVisible()
