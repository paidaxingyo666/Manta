...
async function installClipboardReadTerminalBlurRepro(app: ElectronApplication): Promise<void> {
  await app.evaluate(({ BrowserWindow, ipcMain }) => {
    ...
    const global = globalThis as unknown as {
      __mantaOriginalClipboardReadTextHandler?: ClipboardReadHandler
    }
    ...
    if (!invokeHandlers || !handler || global.__mantaOriginalClipboardReadTextHandler) {
      return
    }
    global.__mantaOriginalClipboardReadTextHandler = handler
    invokeHandlers.set('clipboard:readText', async (event, ...args) => {
      ...
      return global.__mantaOriginalClipboardReadTextHandler!(event, ...args)
    })
  })
}
...
async function restoreClipboardReadTerminalBlurRepro(app: ElectronApplication): Promise<void> {
  await app.evaluate(({ ipcMain }) => {
    ...
    if (invokeHandlers && global.__mantaOriginalClipboardReadTextHandler) {
      invokeHandlers.set('clipboard:readText', global.__mantaOriginalClipboardReadTextHandler)
      delete global.__mantaOriginalClipboardReadTextHandler
    }
  })
}
...
test.describe('terminal paste ownership', () => {
  test('keyboard paste shortcuts send clipboard text to the focused terminal exactly once', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.manta-paste-ownership-${runId}.mjs`)
    ...
    try {
      ...
      for (const [index, chord] of keyboardPasteChords().entries()) {
        const payload = `MANTA_E2E_PASTE_${runId}_${index}`
        ...
        await waitForTerminalOutput(mantaPage, encodedPayload, 10_000, 12_000)
  ...
  test('keyboard paste survives transient terminal blur during clipboard read', async ({
    ...
    const ptyId = await waitForActivePanePtyId(mantaPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.manta-paste-blur-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false
    ...
    try {
      ...
      const payload = `MANTA_E2E_TRANSIENT_BLUR_PASTE_${runId}`
      const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
      await clearTerminalPtyWriteLog(electronApp)
      await mantaPage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(mantaPage)
      ...
      await mantaPage.keyboard.press(keyboardPasteChords()[0])
      await waitForTerminalOutput(mantaPage, encodedPayload, 10_000, 12_000)
  ...
  test('terminal context-menu Paste sends clipboard text exactly once', async ({
    ...
    const ptyId = await waitForActivePanePtyId(mantaPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.manta-paste-context-menu-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false
    ...
    try {
      ...
      const payload = `MANTA_E2E_CONTEXT_MENU_PASTE_${runId}`
      const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
      await clearTerminalPtyWriteLog(electronApp)
      await mantaPage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(mantaPage)
      ...
      await mantaPage.getByRole('menuitem', { name: /Paste/ }).click()
      await waitForTerminalOutput(mantaPage, encodedPayload, 10_000, 12_000)
  ...
  test('Windows multiline keyboard paste normalizes terminal newlines with one PTY owner', async ({
    ...
    const ptyId = await waitForActivePanePtyId(mantaPage)
    const runId = randomUUID()
    const sentinel = `MANTA_E2E_MULTILINE_DONE_${runId}`
    const payload = [
      `MANTA_E2E_MULTILINE_${runId}`,
    ...
    // Manta's direct Windows bracketed-paste path must produce the same bytes.
    const terminalText = payload.replace(/\r?\n/g, '\r')
    const scriptPath = path.join(testRepoPath, `.manta-paste-multiline-${runId}.mjs`)
    ...
    try {
      ...
      await waitForTerminalOutput(mantaPage, `PASTE_COMPLETE_${runId}:MATCH`, 10_000, 12_000)
  ...
  test('right-click paste sends clipboard text to the focused terminal exactly once', async ({
    ...
    const ptyId = await waitForActivePanePtyId(mantaPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.manta-paste-right-click-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false
    ...
    try {
      ...
      const payload = `MANTA_E2E_RIGHT_CLICK_PASTE_${runId}`
      const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
      await clearTerminalPtyWriteLog(electronApp)
      await mantaPage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(mantaPage)

      await rightClickActiveTerminalSurface(mantaPage)
      await waitForTerminalOutput(mantaPage, encodedPayload, 10_000, 12_000)
