/**
 * E2E tests for the "Create Workspace" flow in Manta.
 *
...
test.describe('Create Workspace', () => {
  ...
  test('creates a worktree through the composer UI and activates it', async ({ mantaPage }) => {
    const worktreeIdBefore = await getActiveWorktreeId(mantaPage)
    ...
    mantaPage.on('console', onConsole)
    ...
    try {
      ...
      await expect
        .poll(async () => worktreeExists(mantaPage, workspaceName), {
          timeout: 10_000,
      ...
      await expect
        .poll(
          async () => {
            const id = await getActiveWorktreeId(mantaPage)
      ...
      mantaPage.off('console', onConsole)
  ...
  test('creates an emoji-named worktree with a safe git branch', async ({ mantaPage }) => {
    const workspaceName = '🚀🧪✨'
    ...
    try {
      ...
      const branch = await mantaPage.evaluate((displayName) => {
  ...
  test('enters the Korean flag with the flag_kr shortcode suggestion', async ({ mantaPage }) => {
    try {
      await mantaPage.getByRole('button', { name: 'New workspace', exact: true }).click()
      ...
      const emojiSuggestions = mantaPage.locator('[data-workspace-emoji-suggestions="true"]')
      const sourceSuggestions = mantaPage.locator('[data-workspace-source-suggestions="true"]')
      ...
      await expect(mantaPage.getByRole('option', { name: /:flag_kr:/i })).toHaveCount(0)
  ...
  test('shows a failed workspace entry when worktree creation fails', async ({ mantaPage }) => {
    ...
    try {
      ...
      const failedWorkspace = mantaPage.getByRole('button', {
        name: new RegExp(`${workspaceName} No base branch found`)
      })
      await expect(failedWorkspace).toBeVisible()
      await expect(mantaPage.getByText('Couldn’t create worktree')).toBeVisible()
      await expect(failedWorkspace).toContainText('No base branch found')
      await expect(mantaPage.getByRole('button', { name: 'Retry' })).toBeVisible()
    } finally {
      await mantaPage
        .evaluate(() => {
          ;(
  ...
  test('reuses a resolved pasted GitHub URL when quick create submits', async ({
    ...
    try {
      ...
      await expect(dialog).toBeHidden({ timeout: 15_000 })
      await expect(mantaPage.getByRole('option', { name: linkedWorkspacePattern })).toBeVisible({
        timeout: 10_000
      })
      await expect(mantaPage.getByRole('option', { name: url })).toHaveCount(0)
      await expect(mantaPage.getByText('Linked PR #2049')).toBeVisible()
  ...
  test('names the workspace after the PR title when the pasted URL suggestion is selected', async ({
    ...
    try {
      ...
      await expect(mantaPage.getByRole('option', { name: /https-github/i })).toHaveCount(0)
      await expect(mantaPage.getByText('Linked PR #2050')).toBeVisible()
    } finally {
      await mantaPage
        .evaluate(() => {
          window.__store?.getState().closeModal()
