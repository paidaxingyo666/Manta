...
async function seedUntrackedFile(page: Page): Promise<SeededUntrackedFile> {
  return page.evaluate(async () => {
    ...
    const fileName = `manta-discard-confirm-${Date.now()}.txt`
...
test.describe('Source Control discard confirmation', () => {
  ...
  test('deletes an untracked file without confirmation', async ({ mantaPage }) => {
    const seededFile = await seedUntrackedFile(mantaPage)
    await openSourceControl(mantaPage)
    ...
    await confirmPendingDelete(mantaPage)

    await expect(
      mantaPage.getByRole('dialog', { name: `Delete "${seededFile.fileName}"?` })
    ).toHaveCount(0)
    await expect(row).toHaveCount(0, { timeout: 10_000 })

    await refreshGitStatus(mantaPage)
    await expect(
      mantaPage.locator('[data-testid="source-control-entry"]').filter({
