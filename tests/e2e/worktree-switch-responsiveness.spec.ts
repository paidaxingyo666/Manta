...
test.describe('Worktree switch responsiveness', () => {
  ...
  test('updates the selected workspace in the same click task when changing back', async ({
    ...
    const [firstWorktreeId, secondWorktreeId] = await prepareSidebarForSwitchTest(mantaPage)
    const firstRow = worktreeRow(mantaPage, firstWorktreeId)
    const secondRow = worktreeRow(mantaPage, secondWorktreeId)

    await expect(firstRow).toBeVisible()
    await expect(secondRow).toBeVisible()
    await expect(firstRow).toHaveAttribute('aria-current', 'page')
    await expect(mantaPage.locator('[data-rendered-active-worktree-id]')).toHaveAttribute(
      'data-rendered-active-worktree-id',
      firstWorktreeId
    )

    const result = await mantaPage.evaluate(
