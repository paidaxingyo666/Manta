...
test.describe('Terminal output scheduler', () => {
  test('background tab output bursts use the shared drain while the active tab renders', async ({
    ...
    const ptyIdsByTabId: Record<string, string> = {
      [firstTabId]: await waitForTabPtyId(mantaPage, firstTabId)
    }

    while (tabIds.length < TAB_COUNT) {
      const tabId = await createTerminalTab(mantaPage)
      await waitForActiveTerminalManager(mantaPage, 30_000)
      tabIds.push(tabId)
      ptyIdsByTabId[tabId] = await waitForTabPtyId(mantaPage, tabId)
    }

    await tabLocator(mantaPage, firstTabId).click()
    await expect
    ...
    await sendPtyCommands(mantaPage, [
      {
    ...
    await expect
      .poll(async () => (await getTerminalContent(mantaPage)).includes(foregroundMarker), {
        timeout: 5_000,
    ...
    await tabLocator(mantaPage, firstBackgroundTabId).click()
    await expect
      .poll(() => getDomActiveTabId(mantaPage), {
        timeout: 5_000,
    ...
    await expect
      .poll(async () => (await getTerminalContent(mantaPage)).includes(firstBackground.marker), {
        timeout: 5_000,
  ...
  test('visible bulk output uses the high-priority drain instead of synchronous xterm writes', async ({
    ...
    const activeTabId = await createTerminalTab(mantaPage)
    if (!activeTabId) {
      throw new Error('Expected a fresh terminal tab')
    }
    const ptyId = await waitForTabPtyId(mantaPage, activeTabId)
    await resetSchedulerDebug(mantaPage)
    ...
    await sendPtyCommands(mantaPage, [{ ptyId, command: floodCommand }])

    await expect
      .poll(async () => (await getTerminalContent(mantaPage, 12_000)).includes(marker), {
        timeout: 30_000,
  ...
  test('hidden overflow restores from main-owned terminal state when the tab becomes visible', async ({
    ...
    const foregroundTabId = await getActiveTabId(mantaPage)
    if (!foregroundTabId) {
      throw new Error('Expected an initial terminal tab')
    }
    const hiddenTabId = await createTerminalTab(mantaPage)
    await waitForActiveTerminalManager(mantaPage, 30_000)
    const hiddenPtyId = await waitForTabPtyId(mantaPage, hiddenTabId)

    await tabLocator(mantaPage, foregroundTabId).click()
    await expect
      .poll(() => getDomActiveTabId(mantaPage), {
        timeout: 5_000,
    ...
    await sendPtyCommands(mantaPage, [{ ptyId: hiddenPtyId, command: floodCommand }])

    await expect
      .poll(async () => mainSnapshotContains(mantaPage, hiddenPtyId, marker), {
        timeout: 30_000,
        message: 'Main-owned terminal snapshot did not capture the hidden flood marker'
      })
      .toBe(true)

    await tabLocator(mantaPage, hiddenTabId).click()
    await expect
      .poll(() => getDomActiveTabId(mantaPage), {
        timeout: 5_000,
