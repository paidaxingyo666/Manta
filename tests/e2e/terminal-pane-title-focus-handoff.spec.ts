...
test.describe('Terminal Panes', () => {
  ...
  test('Set Title input stays open when clicked in a split terminal', async ({ mantaPage }) => {
  ...
  test('Set Title survives an early blur during first focus handoff', async ({ mantaPage }) => {
    await openTerminalContextMenu(mantaPage)
  ...
  test('Set Title survives delayed terminal focus handoffs', async ({ mantaPage }) => {
    await openTerminalContextMenu(mantaPage)
    await installDelayedTerminalFocusSteals(mantaPage, [50, 150, 300])
    await mantaPage.getByText('Set Title…', { exact: true }).click()
  ...
  test('Set Title preserves draft text across terminal focus steals', async ({ mantaPage }) => {
  ...
  test('Set Title does not submit when synthetic focus restore fails', async ({ mantaPage }) => {
    ...
    await expect(titleInput).toBeVisible()
    await expect(titleInput).toHaveValue(draftTitle)
    await expect(mantaPage.locator('.pane-title-text', { hasText: draftTitle })).toHaveCount(0)
  })
  ...
  test('Set Title still commits by blur after synthetic terminal focus steals', async ({
    ...
    await openTerminalContextMenu(mantaPage)
    await installDelayedTerminalFocusSteals(mantaPage, [50, 150])
    await mantaPage.getByText('Set Title…', { exact: true }).click()
