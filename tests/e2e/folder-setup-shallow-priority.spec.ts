...
async function createShallowPriorityTruncationFixture(): Promise<{
  ...
  const parentPath = realpathSync(
    await mkdtemp(path.join(os.tmpdir(), 'manta-e2e-shallow-priority-'))
  )
...
async function createCancellableScanFixture(): Promise<{
  ...
  const parentPath = realpathSync(
    await mkdtemp(path.join(os.tmpdir(), 'manta-e2e-cancellable-scan-'))
  )
  tempRoots.push(parentPath)
...
test('prioritizes shallow sibling repositories in a bounded nested scan', async ({
  ...
  const dialog = mantaPage.getByRole('dialog', { name: /Add a project/i })
  await expect(dialog).toBeVisible()
  await dialog.getByRole('button', { name: /Browse folder/i }).click()

  const importDialog = mantaPage.getByRole('dialog', {
  ...
  await expect(mantaPage.getByText(fixture.groupName)).toBeVisible()
  await expect(mantaPage.getByText('z-web-client').first()).toBeVisible()
})
