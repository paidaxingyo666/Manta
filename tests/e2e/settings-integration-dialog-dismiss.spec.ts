...
test.describe('Settings integrations connect dialogs', () => {
  test.beforeEach(async ({ mantaPage }) => {
    ...
    await openIntegrationsSettings(mantaPage)
  })

  test('Linear API key draft survives a backdrop click but clears on cancel', async ({
    mantaPage
  }) => {
    const card = mantaPage.locator('[data-settings-section="integrations-linear"]')
  ...
  test('Jira site URL draft survives a backdrop click', async ({ mantaPage }) => {
    const card = mantaPage.locator('[data-settings-section="integrations-jira"]')
    // Why: the button label depends on connection state; a fresh profile is disconnected.
    const openButton = card.getByRole('button', { name: /^(Connect Jira|Add Jira site)$/ })
    await expect(openButton).toBeVisible({ timeout: 15_000 })
    await openButton.click()

    const dialog = mantaPage.getByRole('dialog', { name: 'Connect Jira site' })
    await expect(dialog).toBeVisible()
