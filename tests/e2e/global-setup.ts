...
export const E2E_TEST_REPO_PATH_FILE_ENV = 'MANTA_E2E_TEST_REPO_PATH_FILE'
...
export const TEST_REPO_PATH_FILE =
  ...
  path.join(os.tmpdir(), `manta-e2e-test-repo-path-${randomUUID()}.txt`)
...
export default function globalSetup(): void {
  ...
  if (process.env.MANTA_E2E_WEB_CLIENT === '1') {
  ...
  if (
    ...
    (process.env.MANTA_E2E_SKILL_STAGING === '1' && Boolean(process.env.MANTA_E2E_SKILL_SSH_HOST))
  ) {
  ...
  if (process.env.MANTA_E2E_SSH_DOCKER === '1' || process.env.MANTA_E2E_NESTED_RUNTIME_SSH === '1') {
  ...
  const testRepoDir = realpathSync(mkdtempSync(path.join(os.tmpdir(), 'manta-e2e-repo-')))
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: 'manta-e2e-test', version: '0.0.0', private: true }, null, 2)}\n`
  )
  ...
  const worktreeDir = path.join(testRepoDir, '..', `manta-e2e-worktree-${randomUUID()}`)
