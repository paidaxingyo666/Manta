...
const SCRATCH_DIR =
  process.env.MANTA_TABLE_ROW_BACKSPACE_SCREENSHOT_DIR ??
...
test.describe('Markdown table keyboard', () => {
  ...
  test('Tab/Shift-Tab move between cells and empty-row Backspace deletes the row', async ({
    ...
    try {
      ...
      await mantaPage.keyboard.press('Tab')
      await expect
        .poll(async () => selectionCellText(mantaPage), {
          timeout: 5_000,
      ...
      await mantaPage.keyboard.press('Shift+Tab')
      await expect
        .poll(async () => selectionCellText(mantaPage), {
          timeout: 5_000,
      ...
      await expect
        .poll(async () => tableRowCount(mantaPage), {
          timeout: 5_000,
