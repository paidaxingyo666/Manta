 ...
 *   MANTA_LARGE_FILE_COUNT=9500 npx playwright test \
...
async function addAndActivateRepo(mantaPage: Page, repoPath: string): Promise<string> {
  ...
  const sourceControlButton = mantaPage.getByRole('button', { name: /^Source Control/ })
  await expect(sourceControlButton).toBeVisible()
  await expect
    .poll(() => mantaPage.evaluate(() => window.__store?.getState().rightSidebarTab))
    .toBe('source-control')
  await expect(mantaPage.getByRole('button', { name: 'Filter files by name' })).toBeVisible()
...
async function unregisterLargeFileCountRepos(
  ...
  for (const repoPath of repoPaths) {
    await mantaPage.evaluate(async (pathToRepo) => {
      const store = window.__store
...
async function measureSourceControlLoad(
  ...
  return await mantaPage.evaluate(async ({ worktreeId, repoPath, expectedRows, pollCycles }) => {
    const store = window.__store
    if (!store) {
      throw new Error('window.__store is not available')
    }
...
test.describe('Source Control large file count (#8013)', () => {
  ...
  test('a large untracked set under the status cap stays responsive', async ({
    ...
    const untrackedFiles = Number(process.env.MANTA_LARGE_FILE_COUNT ?? '950')
    // Why: MANTA_LARGE_FILE_BYTES gives untracked files realistic sizes so the
    ...
    const untrackedFileBytes = Number(process.env.MANTA_LARGE_FILE_BYTES ?? '0')
    ...
    try {
      ...
      const measurement = await measureSourceControlLoad(mantaPage, {
        worktreeId,
      ...
      await unregisterLargeFileCountRepos(mantaPage, [fixture.repoPath])
    }
  })
  ...
  test('a large modified set under the status cap stays responsive', async ({
    ...
    test.setTimeout(600_000)
    const modifiedFiles = Number(process.env.MANTA_LARGE_FILE_COUNT ?? '750')
  ...
  test('a change set over the status cap degrades to the too-many-changes state', async ({
    ...
    try {
      ...
      const activationMaxLagMs = await mantaPage.evaluate(() => {
      ...
      const hugeState = await mantaPage.evaluate(
      ...
      await expect(tooManyChangesBanner).toBeVisible()
      await mantaPage.getByRole('button', { name: 'Retry' }).click()
  ...
  test('untracked line-stat cache stays effective up to the status cap', async ({
    ...
    try {
      ...
      const warmRescanPerFileMs = async (repoPath: string, files: number): Promise<number> => {
        const worktreeId = await addAndActivateRepo(mantaPage, repoPath)
        const measurement = await measureSourceControlLoad(mantaPage, {
          worktreeId,
          repoPath,
          expectedRows: files,
      ...
      await unregisterLargeFileCountRepos(mantaPage, [
        smallRepo.repoPath,
  ...
  test('a large clean repo (tracked files only) loads instantly', async ({
    ...
    test.setTimeout(600_000)
    const trackedFiles = Number(process.env.MANTA_LARGE_FILE_COUNT ?? '15000')
