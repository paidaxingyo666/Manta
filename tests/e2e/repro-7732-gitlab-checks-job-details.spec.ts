...
const FIXTURE = {
  ...
  webUrl: 'https://gitlab.example.test/acme/manta/-/jobs/987654',
  mrUrl: 'https://gitlab.example.test/acme/manta/-/merge_requests/4242',
...
const SCREENSHOT_DIR =
  process.env.MANTA_GITLAB_CHECKS_JOB_DETAILS_SCREENSHOT_DIR ??
...
test.describe('#7732 GitLab Checks panel job details', () => {
  test('expanding a failed pipeline job shows its log, not "No inline details"', async ({
    ...
    await mantaPage.waitForTimeout(8_000)

    const jobRow = mantaPage.getByText(`${FIXTURE.stage}: ${FIXTURE.jobName}`, { exact: true })
    for (let attempt = 0; attempt < 40 && (await jobRow.count()) === 0; attempt++) {
      await linkGitLabMRToWorktree(mantaPage, worktreeId)
    ...
    await jobRow.click()
    const noDetails = mantaPage.getByText('No inline details are available for this check.')
    const viewFullLogs = mantaPage.getByRole('button', { name: 'View full logs' })
