...
test.describe('Diff note delete', () => {
  ...
  test('clicking the trash button removes the saved note', async ({ mantaPage }) => {
    ...
    const deleteButton = mantaPage.getByTitle('Delete note').first()
    ...
    await expect(mantaPage.getByTitle('Delete note')).toHaveCount(0, {
      timeout: 5_000
    })
  })
})
