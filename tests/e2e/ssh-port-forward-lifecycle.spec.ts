...
const FORCE_SYSTEM_SSH = process.env.MANTA_SSH_FORCE_SYSTEM_TRANSPORT === '1'
...
test.describe('Docker SSH port-forward lifecycle', () => {
  ...
  test('keeps a user-forwarded listener live across scan refresh @headful', async ({
    ...
    const marker = `MANTA_FORWARD_${Date.now()}`
    ...
    try {
      ...
      await openPortsPanel(mantaPage)
      ...
      await expect
        ...
        .toBe(remotePid)
      await expect(mantaPage.getByText(`:${REMOTE_PORT}`, { exact: true })).toBeVisible()
      await expect
        .poll(
          () =>
            mantaPage.evaluate(
        ...
        .toBe(true)
      await expect(mantaPage.getByText(`:${REFRESH_BARRIER_PORT}`, { exact: true })).toBeVisible()
      ...
      await installRendererForwardCapture(mantaPage)
      await openPortsPanel(mantaPage)
      await localPortReservation.release()
      await forwardPortFromPanel(mantaPage, localPort, REMOTE_PORT)
      await expect(mantaPage.getByText('Forwarded', { exact: true })).toBeVisible()
      await expect(
        mantaPage.getByText(`:${localPort} → :${REMOTE_PORT}`, { exact: true })
      ).toBeVisible()
      await expect.poll(() => requestForward(localPort)).toContain(marker)
      await expectForwardEvidence(mantaPage, remote.targetId, [
      ...
      const postHydrationRoundTripForwards = await mantaPage.evaluate(
      ...
      await expect(mantaPage.getByText(`:${REFRESH_BARRIER_PORT}`, { exact: true })).toBeVisible()
      const staleSnapshotEvidence = await readPortForwardEvidence(mantaPage, remote.targetId)
      ...
      await expect
        ...
        .toBe(true)
      await expect(mantaPage.getByText(`:${SCAN_REFRESH_PORT}`, { exact: true })).toBeVisible()

      await unrelatedLocalPortReservation.release()
      const unrelatedForward = await addPortForward(mantaPage, {
      ...
      await forceDockerSshRelayChannelReconnect(mantaPage, target, remote.targetId)
      await expectForwardEvidence(mantaPage, remote.targetId, [
        { localPort, remotePort: REMOTE_PORT },
        { localPort: unrelatedLocalPort, remotePort: REFRESH_BARRIER_PORT }
      ])
      ...
      const authorityBeforeTransportReconnect = await mantaPage.evaluate(
      ...
      await expect
        .poll(
          async () => {
            const state = await mantaPage.evaluate(
              (targetId) => window.__store?.getState().sshConnectionStates.get(targetId),
              remote.targetId
            )
            return (
      ...
      await expect(
        mantaPage.getByText(`:${unrelatedLocalPort} → :${REFRESH_BARRIER_PORT}`, { exact: true })
      ).toBeVisible()
      ...
      try {
        const collisionResult = await mantaPage.evaluate(
      ...
      const evidence = await readPortForwardEvidence(mantaPage, remote.targetId)
      ...
      const relayReconnectStates = await readSshStateCapture(mantaPage)
