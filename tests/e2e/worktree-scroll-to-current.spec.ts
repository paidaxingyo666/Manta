...
test.describe('Reveal active workspace button', () => {
  ...
  test('clears sidebar filters before revealing a hidden current workspace', async ({
    ...
    const filterRepoId = await mantaPage.evaluate(async (repoPath) => {
    ...
    const targetId = await mantaPage.evaluate((repoPath) => {
    ...
    await expect(targetRows.and(mantaPage.getByRole('option'))).toHaveCount(1)
