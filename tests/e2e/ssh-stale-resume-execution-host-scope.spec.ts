/**
 * A provider session id names a transcript in ONE machine's agent state directory. Manta issued one
 ...
 * on the remote machine. Both tests restart the app across a relay kill (the shape of a Manta
 ...
 *                     can still resume by hand. It must still hold Manta's ordinary `--version`
...
const FOREIGN_CONNECTION_ID = 'manta-e2e-some-other-host'
...
async function resumeAcrossRestart(
  ...
  try {
    ...
    firstApp = null
    // The shape of a Manta update: the relay and every PTY under it are gone, so nothing is
    ...
    // Manta's per-launch `claude --version` probe proves the relaunch reached the host at all; the
...
test.describe('SSH sleeping-agent resume execution-host scope', () => {
  ...
  {}, testInfo) => {
    ...
    try {
      ...
      // Why not an empty ledger: Manta legitimately probes `claude --version` on the remote to
