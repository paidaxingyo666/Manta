...
test.describe('Tab Rename (Inline)', () => {
  ...
  test('double-clicking a tab opens an inline rename input and Enter commits', async ({
    ...
    const originalTitle = await getActiveTabTitle(mantaPage, worktreeId)
    expect(originalTitle.length).toBeGreaterThan(0)

    const tabLocator = tabLocatorByTitle(mantaPage, originalTitle)
    ...
    await expect
      .poll(async () => getActiveCustomTitle(mantaPage, worktreeId), { timeout: 3_000 })
      .toBe('My Custom Title')
    await expect(renameInput).toBeHidden()
    await expect(tabLocatorByTitle(mantaPage, 'My Custom Title')).toBeVisible()
  })
  ...
  test('context-menu Change Title opens a focused select-all rename input', async ({
    ...
    await expect(tabLocatorByTitle(mantaPage, 'Context Menu Title')).toBeVisible()
  })

  test('Escape during inline rename discards the edit', async ({ mantaPage }) => {
    const worktreeId = (await getActiveWorktreeId(mantaPage))!
    const originalTitle = await getActiveTabTitle(mantaPage, worktreeId)
    ...
    await expect(tabLocatorByTitle(mantaPage, originalTitle)).toBeVisible()
    await expect
      .poll(async () => getActiveCustomTitle(mantaPage, worktreeId), { timeout: 3_000 })
      .toBe(null)
  })

  test('renaming to an empty string resets the tab to its default title', async ({ mantaPage }) => {
    const worktreeId = (await getActiveWorktreeId(mantaPage))!
    ...
    const defaultTitle = await getActiveTabTitle(mantaPage, worktreeId)
    ...
    const tabLocator = tabLocatorByTitle(mantaPage, 'Seeded Custom')
    await tabLocator.dblclick()
    ...
    await expect(tabLocatorByTitle(mantaPage, defaultTitle)).toBeVisible()
    await expect
      .poll(async () => getActiveCustomTitle(mantaPage, worktreeId), { timeout: 3_000 })
      .toBe(null)
  })

  test('clicking away (blur) commits the rename', async ({ mantaPage }) => {
    const worktreeId = (await getActiveWorktreeId(mantaPage))!
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(mantaPage, worktreeId)).length, { timeout: 3_000 })
      .toBeGreaterThanOrEqual(2)

    const tabs = await getWorktreeTabs(mantaPage, worktreeId)
    const activeId = await getActiveTabId(mantaPage)
    const activeTab = tabs.find((t) => t.id === activeId)!
    const otherTab = tabs.find((t) => t.id !== activeId)!

    const tabLocator = tabLocatorByTitle(mantaPage, activeTab.title!)
    await tabLocator.dblclick()
    ...
    await tabLocatorByTitle(mantaPage, otherTab.title!).click()

    await expect(renameInput).toBeHidden()
    await expect(tabLocatorByTitle(mantaPage, 'Committed By Blur')).toBeVisible()
    expect(
      await mantaPage.evaluate(
  ...
  test('terminal title updates do not resize neighboring tabs', async ({ mantaPage }) => {
    const worktreeId = (await getActiveWorktreeId(mantaPage))!
    const tabIds = await mantaPage.evaluate((targetWorktreeId) => {
    ...
    const tabs = tabIds.map((id) =>
      mantaPage.locator(`[data-testid="sortable-tab"][data-tab-id="${id}"]`)
    )
  ...
  test('rename input stays at a usable width when many tabs are open', async ({ mantaPage }) => {
    const worktreeId = (await getActiveWorktreeId(mantaPage))!
    const targetTabId = await getActiveTabId(mantaPage)
    ...
    const tabLocator = tabLocatorByTitle(mantaPage, targetTitle)
  ...
  test('middle-clicking inside the rename input does not close the tab', async ({ mantaPage }) => {
    const worktreeId = (await getActiveWorktreeId(mantaPage))!
    const tabsBefore = (await getWorktreeTabs(mantaPage, worktreeId)).length
    const originalTitle = await getActiveTabTitle(mantaPage, worktreeId)
    ...
    await expect(renameInput).toBeVisible()
    await expect(tabLocatorByTitle(mantaPage, originalTitle)).toBeVisible()
    expect((await getWorktreeTabs(mantaPage, worktreeId)).length).toBe(tabsBefore)
  })
})
