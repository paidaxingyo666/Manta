...
// scoped to this spec's app launches via mantaAppExtraEnv (same pattern as
...
test.use({
  ...
  mantaAppExtraArgs: ['--enable-precise-memory-info']
})

// Why: 8 hidden tabs is below the 12-tab hot-retain limit, but that limit
// never retains anything here — the MANTA_E2E_TERMINAL_PARKING_DELAY_MS
...
test.describe('Terminal parked memory', () => {
  test('releases renderer terminal memory when hidden tabs park', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.manta-parked-memory-${runId}.mjs`)
    writeScrollbackFillScript(scriptPath, runId)
    try {
      const { worktreeId, scrollbackTabs } = await setUpScrollbackTabs(mantaPage, scriptPath, runId)
      ...
      // the other 7 park.
      const visibleTab = await createActiveTerminalTab(mantaPage, worktreeId)
      ...
      const metrics = await sampleParkedMemoryMetrics(mantaPage)
      ...
      for (const tab of parkableTabs) {
        expect((await readTerminalTabViewState(mantaPage, tab.tabId)).hasManager).toBe(false)
      }
      expect((await readTerminalTabViewState(mantaPage, lastActiveTab.tabId)).hasManager).toBe(true)
      const visibleState = await readTerminalTabViewState(mantaPage, visibleTab.tabId)
  ...
  test('retains terminal views when parking is disabled', async ({
    ...
    await expect
      .poll(
        () =>
          mantaPage.evaluate(() => window.__store?.getState().settings?.terminalHiddenViewParking),
    ...
    try {
      ...
      await mantaPage.waitForTimeout(PARKING_DELAY_MS * 3)
      expect(await countMountedPaneManagers(mantaPage, scrollbackTabIds)).toBe(SCROLLBACK_TAB_COUNT)
      ...
      for (const tab of scrollbackTabs) {
        const state = await readTerminalTabViewState(mantaPage, tab.tabId)
        expect(state.hasManager).toBe(true)
        expect(state.paneCount).toBeGreaterThan(0)
      }
      expect((await readTerminalTabViewState(mantaPage, visibleTab.tabId)).hasManager).toBe(true)
...
test.describe('Terminal hidden worktree retention budget', () => {
  test.use({
    mantaAppExtraEnv: {
      MANTA_E2E_TERMINAL_PARKING_DELAY_MS: String(PARKING_DELAY_MS),
      ...
      MANTA_E2E_TERMINAL_RETENTION_LIMIT: '1'
    },
    mantaAppExtraArgs: ['--enable-precise-memory-info']
  })
  ...
  test('releases un-parkable hidden worktree buffers only once the retention budget engages', async ({
    ...
    await waitForSessionReady(mantaPage)
    const victimWorktreeId = await waitForActiveWorktree(mantaPage)
    await skipUnlessParkingWired(mantaPage)

    const decoyWorktreeId = (await getAllWorktreeIds(mantaPage)).find(
    ...
    await updateTerminalSettings(mantaPage, {
      terminalHiddenWorktreeRetentionBudget: false,
      terminalScrollbackRows: RETENTION_SCROLLBACK_ROWS
    })
    await waitForRetentionBudgetSetting(mantaPage, false)

    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.manta-retention-memory-${runId}.mjs`)
    ...
    try {
      ...
      const baselineSnapshot = await waitForPaneIdentitySnapshot(mantaPage, 1)
      const baselinePtyId = baselineSnapshot.panes[0]?.ptyId
      if (!baselinePtyId) {
      ...
      for (let tabIndex = 0; tabIndex < RETENTION_TAB_COUNT; tabIndex += 1) {
        if (tabIndex > 0) {
          victimTabs.push(await createActiveTerminalTab(mantaPage, victimWorktreeId))
        }
        ...
        await stageUnparkableWorktreeTabs(mantaPage, victimWorktreeId)
      }
      ...
      await switchToWorktree(mantaPage, decoyWorktreeId)
      await expect
      ...
      const decoySnapshot = await waitForPaneIdentitySnapshot(mantaPage, 1)
      const decoyTabIds = [decoySnapshot.tabId]
      expect(await countMountedPaneManagers(mantaPage, decoyTabIds)).toBe(1)
      ...
      await stageUnparkableWorktreeTabs(mantaPage, victimWorktreeId)
      await stageUnparkableWorktreeTabs(mantaPage, decoyWorktreeId)
      await waitForUnparkableWorktreeTabs(mantaPage, victimWorktreeId)
      await waitForUnparkableWorktreeTabs(mantaPage, decoyWorktreeId)
      ...
      await expect
        .poll(
          async () => {
            await stageUnparkableWorktreeTabs(mantaPage, victimWorktreeId)
            await stageUnparkableWorktreeTabs(mantaPage, decoyWorktreeId)
            const victimMounted = await countMountedPaneManagers(mantaPage, victimTabIds)
            const decoyMounted = await countMountedPaneManagers(mantaPage, decoyTabIds)
      ...
      const before = await readRetentionMemorySample(mantaPage)
      expect(before.bufferMb).toBeGreaterThan(MIN_STAGED_BUFFER_MB)

      const flipStartedAt = Date.now()
      await updateTerminalSettings(mantaPage, { terminalHiddenWorktreeRetentionBudget: true })
      await waitForRetentionBudgetSetting(mantaPage, true)
      await expect
        .poll(() => countMountedPaneManagers(mantaPage, victimTabIds), {
          timeout: 30_000,
      ...
      const after = await readRetentionMemorySample(mantaPage)
      testInfo.annotations.push({
      ...
      expect(await countMountedPaneManagers(mantaPage, decoyTabIds)).toBe(decoyTabIds.length)
