...
test('closing a parked terminal tab retires its exact PTY session', async ({ mantaPage }) => {
  ...
  await execInTerminal(mantaPage, ptyId, nodeTerminalCommand(['-e', keepAliveScript]))
  await expect
    .poll(() => getTerminalContent(mantaPage), {
      timeout: 10_000,
      message: 'long-lived terminal child did not print its ready marker'
    })
    .toContain(marker)
  await expect.poll(() => hasPtySession(mantaPage, ptyId), { timeout: 10_000 }).toBe(true)
  ...
  await expect
    .poll(() => hasPtySession(mantaPage, ptyId), {
      timeout: 15_000,
