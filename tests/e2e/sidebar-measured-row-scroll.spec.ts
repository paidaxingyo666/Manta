...
test('keeps a collapsed group visible when an idle card below it grows', async ({
  ...
  const fixture = await mantaPage.evaluate(async (repoPaths): Promise<SidebarFixture> => {
  ...
  const before = await settledGeometry(mantaPage, fixture)
  ...
  await expect
    .poll(async () => (await readGeometry(mantaPage, fixture)).cardHeight)
    .toBeCloseTo(before.cardHeight + 50, 0)
  const after = await settledGeometry(mantaPage, fixture)
