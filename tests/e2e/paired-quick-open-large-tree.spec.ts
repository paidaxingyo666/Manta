...
async function expectQuickOpenAndRuntimeHealthy(
  ...
  for (const targetPath of [fixture.gitIgnoredTargetPath, fixture.mantaIgnoredTargetPath]) {
    ...
    await expect(dialog).not.toContainText('Remote Manta runtime closed the connection')
  }
...
test('finds paths beyond the old prefix on a headed paired runtime @headful', async ({
  ...
  try {
    ...
    await mantaPage.evaluate(async (repoPath) => {
