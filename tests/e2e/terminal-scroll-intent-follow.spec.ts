...
test.describe('terminal scroll intent keeps following output', () => {
  test('a sub-row wheel-up that never moves the viewport must not stop follow-output', async ({
    ...
    const ptyId = await startStreamingFixturePhase1(mantaPage)
    ...
    await dispatchSubRowWheelUp(mantaPage)
    await mantaPage.waitForTimeout(INTENT_SETTLE_WAIT_MS)

    // Any byte releases the fixture's phase-2 stream.
    await sendToTerminal(mantaPage, ptyId, 'g')
    await waitForMarkerAtBottom(mantaPage, 'STREAM_PHASE2_DONE')
  })

  test('a plain Home keypress delivered to the app must not stop follow-output', async ({
    mantaPage
  }) => {
    await startStreamingFixturePhase1(mantaPage)
    ...
    await dispatchPlainHomeKeydown(mantaPage)
    await waitForMarkerAtBottom(mantaPage, 'STREAM_PHASE2_DONE')
  })

  test('a real wheel pin stays fixed while visible output streams', async ({ mantaPage }) => {
    const ptyId = await startStreamingFixturePhase1(mantaPage)

    await dispatchRealWheel(mantaPage, -240)
    await expect
      .poll(async () => {
        const probe = await probeActiveViewport(mantaPage, 'STREAM_PHASE1_DONE')
        return probe ? probe.baseY - probe.viewportY : 0
      })
      .toBeGreaterThan(1)
    const pinned = await probeActiveViewport(mantaPage, 'STREAM_PHASE1_DONE')
    if (!pinned) {
    ...
    await expect
      .poll(
        async () => {
          const probe = await probeActiveViewport(mantaPage, 'STREAM_PHASE2_DONE')
  ...
  test('typing after a pinned write is queued resumes follow-output', async ({ mantaPage }) => {
    await startStreamingFixturePhase1(mantaPage)
    const { paneKey } = await waitForActivePaneHookDescriptor(mantaPage)
    await waitForTerminalPtyDataInjector(mantaPage, paneKey)

    await dispatchRealWheel(mantaPage, -320)
    await expect
      .poll(async () => {
        const probe = await probeActiveViewport(mantaPage, 'STREAM_PHASE1_DONE')
        return probe ? probe.baseY - probe.viewportY : 0
    ...
    await injectQueuedWriteThenType(mantaPage, paneKey)
    await expect
      .poll(
        async () => {
          const probe = await probeActiveViewport(mantaPage, 'STREAM_PHASE1_DONE')
