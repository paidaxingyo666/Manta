...
test.describe('Worktree switch first paint @headful', () => {
  test.skip(
    process.env.MANTA_BACKGROUND_LAUNCH === '1',
  ...
  test('repaints an unmounted worktree within the switch budget', async ({
    ...
    const filler = await addFillerWorktrees(mantaPage, testRepoPath)

    const samples: SwitchSample[] = []
    const lines: string[] = []
    try {
      const targetTabIds = await ensureTabs(mantaPage, targetId, 'WTB')
      ...
      for (let round = 0; round < SWITCH_SAMPLE_COUNT; round += 1) {
        ...
        await switchToWorktree(mantaPage, primaryId)
        await ensureTerminalVisible(mantaPage)
        ...
        await ensureTerminalVisible(mantaPage)
        await mantaPage.waitForTimeout(2_500)
        const unmounted = await waitForUnmountedTabs(mantaPage, targetTabIds)
        expect(unmounted, 'target worktree was already mounted before the switch').toBe(true)

        const sample = await measureSwitch(mantaPage, targetId, targetTabIds)
        samples.push(sample)
        ...
        const warmedTabIds = await waitForMountedTabs(mantaPage, targetTabIds)
