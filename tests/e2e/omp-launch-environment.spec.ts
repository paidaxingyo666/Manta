...
`
    ...
    await writeFile(
      shell,
      `#!/bin/sh
case "$MANTA_E2E_HOME_DIR" in
  */manta-e2e-userdata-*/home) ;;
  *) echo 'Refusing profile proof outside isolated E2E home' >&2; exit 73 ;;
esac
export HOME="$MANTA_E2E_HOME_DIR"
...
test('OMP launched by Manta uses login-profile data and config roots', async ({
  mantaPage,
  electronApp
}, testInfo) => {
...
export default function (api) {
 api.on('session_start', (_event, ctx) => {
  writeFileSync(process.env.ORCA_PATH_PROBE_OUTPUT || ${JSON.stringify(result)}, JSON.stringify({
   ...
   source: process.env.MANTA_OMP_SOURCE_AGENT_DIR,
   status: process.env.MANTA_OMP_STATUS_EXTENSION,
  ...
  await expect(mantaPage.locator('.xterm-screen').first()).toBeVisible()
  await mantaPage.screenshot({ path: testInfo.outputPath('omp-profile-root.png') })
  await expect(async () => {
  ...
  const overridePty = await mantaPage.evaluate(
  ...
  try {
    ...
    await mantaPage.evaluate((id) => window.api.pty.kill(id), overridePty)
  }
  ...
  const baselinePty = await mantaPage.evaluate(
  ...
  try {
    ...
    await mantaPage.evaluate((id) => window.api.pty.kill(id), baselinePty)
  }
})
