...
export function createTerminalImeByteReader(
  ...
  const readyMarker = `MANTA_IME_READER_READY_${runId}`
  const resultPrefix = `MANTA_IME_BYTES_${runId}`
  const scriptPath = path.join(testRepoPath, `.manta-ime-byte-reader-${runId}.cjs`)
  const source = `
