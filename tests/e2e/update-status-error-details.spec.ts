...
for (const theme of ['dark', 'light'] as const) {
  test(`automatic update failure opens details from the status bar (${theme})`, async ({
    ...
    await expect(mantaPage.locator('html')).toHaveClass(theme === 'dark' ? /\bdark\b/ : /\blight\b/)
    await mantaPage.evaluate((message) => {
    ...
    const statusButton = mantaPage.getByRole('button', {
      name: 'Update failed. Click to expand.',
      exact: true
    })
    const card = mantaPage.getByRole('complementary', { name: 'Update error', exact: true })
    ...
    await mantaPage.screenshot({ path: statusClickScreenshot, animations: 'disabled' })
    ...
    await mantaPage.screenshot({ path: detailsScreenshot, animations: 'disabled' })
