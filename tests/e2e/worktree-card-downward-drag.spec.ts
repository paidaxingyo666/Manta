...
test('dragging a virtualized worktree downward keeps rows stable', async ({ mantaPage }) => {
  await waitForSessionReady(mantaPage)
  await mantaPage.setViewportSize({ width: 1_000, height: 620 })
  const { sourceId, nextId, idPrefix } = await seedVirtualizedManualWorktrees(mantaPage)
  const scroller = mantaPage.locator('[data-worktree-sidebar]')
  const source = mantaPage.locator(
  ...
  const nextSource = mantaPage.locator(
  ...
  try {
    ...
    for (let step = 0; step < 12; step++) {
      await mantaPage.mouse.move(edgeX, edgeY, { steps: 2 })
    ...
    const samples = await sampleMountedPreviewOffsets(mantaPage, sourceId)
  ...
  await expect(mantaPage.locator('[data-worktree-sidebar-drag-preview="true"]')).toHaveCount(0)
  await expect(mantaPage.locator('html')).not.toHaveAttribute(
