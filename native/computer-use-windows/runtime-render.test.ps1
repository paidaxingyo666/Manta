...
public sealed class MantaRenderTestCounter {
    public int FindAll;
}

public sealed class MantaRenderTestBounds {
...
public sealed class MantaRenderTestControlType {
    public string ProgrammaticName = "";
}

public sealed class MantaRenderTestCurrent {
    public string AutomationId = "";
    public MantaRenderTestBounds BoundingRectangle = new MantaRenderTestBounds();
    public string ClassName = "fake";
    public MantaRenderTestControlType ControlType = new MantaRenderTestControlType();
...
public sealed class MantaRenderTestPatternCurrent {
    public bool IsSelected;
    public string Value = "";
}

public sealed class MantaRenderTestPattern {
    public MantaRenderTestPatternCurrent Current = new MantaRenderTestPatternCurrent();
}

public sealed class MantaRenderTestCollection : IEnumerable {
    private readonly MantaRenderTestElement[] values;

    public MantaRenderTestCollection(MantaRenderTestElement[] values) {
        this.values = values;
    }
    ...
    public MantaRenderTestElement Item(int index) {
        return values[index];
    }
...
public sealed class MantaRenderTestElement {
    public readonly List<MantaRenderTestElement> Children =
        new List<MantaRenderTestElement>();
    public MantaRenderTestCounter Counter = new MantaRenderTestCounter();
    public readonly MantaRenderTestCurrent Current = new MantaRenderTestCurrent();
    public bool FailFindAll;
    public int RuntimeIdValue;
    public string ValueText = "";

    public MantaRenderTestCollection FindAll(object scope, object condition) {
        Counter.FindAll++;
        if (FailFindAll) {
            throw new System.InvalidOperationException("defunct node");
        }
        return new MantaRenderTestCollection(Children.ToArray());
    }

    public MantaRenderTestPattern GetCurrentPattern(object pattern) {
        MantaRenderTestPattern result = new MantaRenderTestPattern();
...
function New-TestCounter {
    New-Object -TypeName MantaRenderTestCounter
}
...
function New-TestElement {
    ...
    $element = New-Object -TypeName MantaRenderTestElement
...
$operationPath = Join-Path ([IO.Path]::GetTempPath()) ("manta-runtime-render-test-" + [guid]::NewGuid() + ".json")
try {
    ...
    $tree = Render-MantaTree $root $null
