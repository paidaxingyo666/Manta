// Read-only Windows registry value enumeration for Manta.
//
// Vendored from windows-native-registry@3.2.2 (MIT, Eugene Pankov), reduced to the one
// entry point Manta calls. The upstream write surface (setValue/createKey/deleteKey) is
// deliberately absent: Manta only ever reads, and shipping RegDeleteTreeW in the app is
