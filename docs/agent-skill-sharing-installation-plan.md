...
Manta will own a focused Skill Bundle packaging and installation pipeline for private sharing. A
...
not provide the transaction, provenance, or private-package semantics Manta needs.

Manta will:

1. Package one or many local skills into one immutable, content-addressed artifact and durable
   link.
2. Store private artifacts behind unlisted, revocable Manta Cloud share records.
...
The portable archive root follows Agent Plugins 1.0.0 for a skills-only package. Manta does not
...
The first release will support Manta package sources only. Arbitrary Git, npm, and community
...
The Manta implementation is on `skills-share` through `2a0b7b2acf`; no Manta pull request exists.
...
`Manta skill sharing alerts` email channel. No full production plan was applied because it contains
...
unlisted landing route returns only generic install copy plus the Manta deep link with `no-store`,
...
`lstat` misses an existing distro symlink, Manta now verifies the alias through WSL and uses
...
`manta-node-pty-floor-ok` from an ARM64 Ubuntu 20.04 container on native `aarch64` hardware. The
...
Cloud deletion. The encrypted credential artifact, one-time keys, Manta SSH target, remote install,
...
image advance. The development Manta instance
`skill-sharing-manual` now points at `login.manta.sh.cn` and `cloud-api.manta.sh.cn`; sign-in and the
...
`/Users/jinwoo/refs/misc/agent-plugins-example`. Manta independently implements the reviewed
behavior. The specification prose is CC BY 4.0 and its schemas/software are Apache 2.0, so Manta
...
Behavior Manta will not inherit:
...
Manta will not copy upstream source, tests, fixtures, or registry entries. Provider paths will be
implemented independently from official provider documentation and verified installations. Under
that constraint, Manta does not incorporate upstream copyrightable material and needs no upstream
...
- Install on the local computer or any compatible connected Manta runtime.
...
- Remain compatible with independently updated desktop clients and remote Manta servers.
...
- End-to-end encryption from Manta Cloud operators in the initial trust model. Private means
...
and filter changes retain selections; **Select all results** is explicit and Manta never silently
...
After confirmation, Manta validates and packages the exact bytes shown in the preview. Progress is
...
Opening a deep link launches Manta and inspects the bundle without installing it. Preview identifies
...
Package builder -- validates and creates immutable artifact
    |
    v
Manta Cloud -- private blob + version metadata + revocable bearer share
    |
    v
...
- `plugin.json` uses the canonical Agent Plugins 1.0.0 `$schema`, a valid portable plugin name,
  bundle version, description, and optional standard metadata. Manta validates from locally owned
...
- Manta-specific integrity and version data lives in `dev.manta.skill-sharing/manifest.json` rather
...
- Manta constructs a fresh staging root. If imported content already contains
  `dev.manta.skill-sharing`, Manta accepts only an appropriate recognized schema and rejects unknown,
...
commitment. Manta will replace it before launch and migrate or discard staging-only records. Remote
...
| Object storage    | `onorca-cloud-artifacts` is a `US` bucket with uniform bucket-level access, public-access prevention, exact CORS for `https://share.manta.sh.cn`, and seven-day soft delete |
...
Firestore is not required. Skill metadata belongs in PostgreSQL beside the existing Manta Cloud
...
All production changes are declared in the existing Manta Cloud Terraform configuration. Do not
...
| CORS                | Exact approved Manta share/app origins; no wildcard origin                                                                     |
| Database            | `manta_skills` on `orca-cloud-auth-db`                                                                                         |
| Database principal  | Dedicated `manta_skills_app` user with access only to `manta_skills`                                                            |
...
- `application/vnd.manta.skill+tar+gzip` content type.
...
The destination runtime accepts download URLs only from the configured GCS origin for the Manta
...
No long-lived GCP key is shipped in Manta or stored on a remote runtime.
...
```text
MANTA_SKILL_PACKAGE_BUCKET=onorca-cloud-skill-packages
MANTA_SKILL_PACKAGE_MAX_COMPRESSED_BYTES=41943040
MANTA_SKILL_PACKAGE_UPLOAD_TTL_SECONDS=900
MANTA_SKILL_PACKAGE_DOWNLOAD_TTL_SECONDS=300
MANTA_SKILL_PACKAGE_FINALIZE_CONCURRENCY=<small fixed value>
MANTA_SKILLS_DATABASE_URL=<Secret Manager reference>
```
...
`MANTA_SKILLS_DATABASE_URL` secret reference in one no-traffic candidate. Before smoke or promotion,
...
- Route the endpoint family through the existing Manta Cloud API hostname and deployment path.
...
1. Confirm the existing Manta Cloud Terraform worktree and staging environment are current.
...
An SSH target without a Manta runtime cannot execute the runtime RPC directly. Before GA, add a
host-side installation command invoked through the existing SSH execution and file-transfer
providers. The desktop uploads the immutable package through SFTP, invokes the bundled Manta
installer on the SSH host, and receives the same structured result as runtime RPC.

If the required host component is unavailable, Manta reports the limitation and provides a local
package download or command; it never installs into the desktop's home as a fallback.

## Provider destination strategy

Manta owns a small data-only registry for agents Manta supports. Each entry describes:
...
The registry is authored from official agent documentation and Manta's own verified installations.
Upstream may reveal that a compatibility case exists, but its implementation, path table, and
tests are not copied or transformed into Manta code.
...
Provider releases and official documentation are reviewed periodically. Manta's registry changes
...
plugins. Manta therefore installs loose skills by default and may generate provider-native adapters
...
- `src/main/skills/skill-install-destinations.ts`: global, workspace, WSL, and provider path
  resolution.
- `src/main/skills/skill-provider-destinations.ts`: Manta-owned provider registry.
...
Exact files follow the repository that owns Manta Cloud APIs; desktop contracts stay provider
neutral.
...
- For grants, require HTTPS when the configured cloud endpoint is HTTPS and restrict redirects and
  final hosts to approved Manta storage origins.
...
- Matches Manta receipt and remains unmodified: update is allowed.
- Differs from its Manta receipt: return a modified conflict.
- Exists without Manta provenance: return an unowned conflict.
- External or broken link: return a topology conflict.

Provider outcomes:

- Correct alias or matching Manta-owned fallback copy: no-op.
- Missing: create after canonical commit.
- Broken Manta-owned alias: repair.
...
- Installation timestamp and Manta host/runtime identity.
...
5. If bytes were modified, offer:
   - Keep local version.
   - Publish local bytes as a new immutable version when exactly one non-missing Manta-managed
...
Do not invoke `npx skills update` for Manta-owned packages or write entries that cause the community
CLI to claim ownership of them.

## Removal behavior

1. Read Manta provenance and observe every recorded placement.
...
4. Remove the canonical copy only when it is Manta-owned, unmodified, and no retained placement or
...
3. Modify the installed copy, publish an update, and prove Manta refuses silent replacement.
...
- No `npx` or Node installation dependency outside Manta's own runtime.
...
- Provider registry for Manta's primary detected agents.
...
- Reviewed Terraform for the dedicated private GCS bucket, lifecycle, CORS, bucket-scoped IAM,
  service-account signing, `manta_skills` database, database secret, Cloud SQL attachment,
...
### Phase 3: paired Manta runtime installation
...
For one engineer familiar with Manta:
...
- Upstream behavior informs compatibility but does not define Manta's internal architecture.
