...
Manta's code editor is Monaco — the same editor VS Code uses — with a few Manta-specific tweaks.
...
Syntax highlighting ships for the languages Monaco supports out of the box. Manta is intentionally editor-first, not IDE-first — run type-checkers and linters in a terminal pane.
