...
Orchestration is Manta's structured multi-agent layer: a **Run** (namespace + coordinator inbox), **Tasks**, **Dispatches**, supervised **workers**, messages, and decision gates.

Use it when you need ownership, completion tracking, or a DAG. For one-off prompts, use `manta terminal send`. For full ownership handoffs without supervision, use worktree/terminal commands from the `orca-cli` skill.

<Callout title="Experimental">
Enable orchestration under Settings → Experimental before using these commands. The CLI talks to the running Manta runtime, so `manta status --json` should succeed first.
</Callout>

<Callout title="Legacy commands retired">
`manta orchestration run` and `run-stop` (and `coordinator-start` / `coordinator-stop`) perform **no effects**. They return recovery text pointing at `manta skills get orchestration --full`. Use the Run + worker-start flow below.
</Callout>
...
Task IDs printed in terminals, such as `task_...`, are clickable links. Clicking one asks the Manta runtime for the task's current dispatch and focuses the assigned terminal, including when the task lives in a remote or SSH runtime.

## Preferred supervised loop

```bash
manta orchestration run-create --objective "Split checkout QA and summarize blockers" --json
manta orchestration task-create --spec "Audit billing settings for mobile layout" --task-title "Billing audit" --json
manta orchestration worker-start --task <taskId> --worktree current --agent codex --json
# or new worktree:
manta orchestration worker-start --task <taskId> --worktree new-child --name billing-audit --agent codex --setup run --json
# optional per-worker model / effort (Claude, Codex, Cursor only; not with --terminal):
manta orchestration worker-start --task <taskId> --worktree current --agent claude --model <opaque-model-id> --effort high --json
```
...
manta orchestration check --ack <deliveryId> --wait --types worker_done,escalation,question --timeout-ms 900000 --json
```

Worker completion (from the worker pane; include injected IDs):

```bash
manta orchestration send \
  --type worker_done \
...
```bash
manta orchestration worker-show --dispatch <dispatchId> --json
manta orchestration worker-read --dispatch <dispatchId> --limit 50 --json
manta orchestration worker-stop --dispatch <dispatchId> --json
...
manta orchestration worker-release --dispatch <dispatchId> --json
# Keep a settled worker live for debugging when the user asked to retain it:
manta orchestration worker-retain --dispatch <dispatchId> --json
# retry placement is explicit — --retry-of does not inherit --on/worktree:
manta orchestration worker-start --task <taskId> --retry-of <dispatchId> --worktree current --agent codex --json
```
...
```bash
manta orchestration worker-start \
  --task <taskId> \
  --on windows \
  ...
  --setup run \
  --json
manta orchestration send --to dispatch:<dispatchId> --subject "Follow-up" --body "…" --json
```
...
```bash
manta worktree create --name billing-audit --agent codex --json
manta terminal wait --terminal <workerHandle> --for tui-idle --timeout-ms 60000 --json
manta orchestration dispatch --task <taskId> --to <workerHandle> --inject --json
```
...
```bash
manta orchestration send --to @all --subject "Heads up" --body "Pausing dispatches for a review." --json
manta orchestration send --to @idle --subject "Anyone free?" --json
manta orchestration send --to @codex --subject "Codex agents only" --json
```
...
- Use `manta orchestration ask` for blocking questions instead of local TUI prompts.

```bash
manta orchestration ask \
...
```bash
manta orchestration gate-create \
  --task <taskId> \
  --question "Merge the shared button change into the task branch?" \
  --options '["yes","no"]' \
  --json

manta orchestration gate-resolve --id <gateId> --resolution "yes" --json
```

## Recovery

```bash
manta orchestration dispatch-show --task <taskId> --json
manta orchestration dispatch-show --task <taskId> --preamble --json
manta orchestration task-list --json
manta orchestration task-update --id <taskId> --status blocked --result '{"reason":"waiting on credentials"}' --json
```

Reset only when you are intentionally abandoning orchestration state:

```bash
manta orchestration reset --tasks --json
manta orchestration reset --messages --json
manta orchestration reset --all --json
```
...
Use `manta terminal send` for a lightweight prompt to an agent you are watching.

Use `manta orchestration worker-start` (or `dispatch --inject`) when a worker must report `worker_done`, ask questions through the coordinator, and be tracked by task ID.

Use `manta orchestration run-create` + tasks + workers when you want a durable Run namespace and supervised multi-agent loop — not the retired `orchestration run` command.
