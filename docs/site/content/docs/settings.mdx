...
- **Manta CLI** — register the bundled command-line tool for shells and agents.
...
- Auto-Rename Branch From Work — rename Manta-generated creature branches after an agent starts work.
...
- With remote or multi-host setups, commands are grouped by the Manta host that owns them (**Saved on**). Host ownership is separate from Global/Project scope and from where the command runs — see [Terminal → Quick Commands](/docs/terminal#quick-commands).
...
- **Agent status hooks** — show working / waiting / done states in Manta. Toggle takes effect without restarting the app (including the Windows WSL hook relay). CLI: `manta agent hooks on|off|status`.
...
- **Link Routing** — open http(s) links from the terminal, markdown, and editor in Manta's browser or the system browser. Nested **Hold Shift…** inverts that default for one click (`⇧⌘-click` / `Shift+Ctrl+click`). See [Per-worktree browser](/docs/browser/overview#link-routing).
...
- **Manta account** — sign in (same account family as Manta Relay) to publish and manage shared files.
- **Allow publishing public artifact links** — **off by default**. Device-wide gate: when on, you, agents, and the `manta` CLI on this machine can upload HTML/Markdown and mint public view links. Turning it off does not delete existing links.
...
- Share from an open local HTML page or Markdown editor with **Share as artifact**, or via `manta artifacts …`. See [CLI reference → Artifacts](/docs/cli/reference#artifacts).
...
- Bitbucket Cloud — **Connect** with **Email & API token** (default) or an **Access token**. Manta verifies the credential before saving. `MANTA_BITBUCKET_*` environment variables take precedence and hide Connect / Disconnect. Saved credentials stay on this machine — on a [Remote Manta Server](/docs/remote-servers), set the env vars on the server instead. See [Hosted reviews](/docs/review/github).
...
- **Microphone** — pick which input device dictation uses. Defaults to the system mic; choose a specific device (for example a headset) when you do not want the OS default. If the chosen mic is unplugged or missing, Manta falls back to the system default and shows a non-blocking notice.
...
- Pair and connect to remote Manta runtimes.
...
- **Tab navigation defaults (new installs):** next/previous tab **across all types** is `Cmd+Shift+]` / `Cmd+Shift+[` (Ctrl on Linux/Windows). Same-type next/previous is `Cmd+Option+]` / `Cmd+Option+[`. Previous recent tab is `Ctrl+Tab`. Existing installs keep customized overrides under `~/.manta/keybindings.json`.
...
- **Delete workspace** defaults to `Cmd+Shift+Backspace` on macOS and `Ctrl+Shift+Backspace` on Windows / Linux. Hover the sidebar workspace you mean to delete; Manta still asks for confirmation.
...
- **Worktree Shared Paths** — gitignored paths to materialize from the primary checkout into each new worktree (APFS clone-copy on macOS when possible, otherwise symlink). Complements repo-checked-in `worktree.sharedDirectories` in `manta.yaml` and `.worktreeinclude` (see [Worktrees](/docs/model/worktrees)).
...
- Plugin workers always run on this computer; SSH workspace actions still route through Manta.
...
- **Cloud VM** — show setup controls and workspace **Run on** targets for repo-owned, on-demand environments (cloud sandboxes, VMs, or local Docker). Setup guide and recipe install live under this experimental toggle. See [Ways to run Manta](/docs/ways-to-run#4-cloud-vms-per-workspace-environments).
