...
ship alongside Manta's source code.

Open-source product documentation for [Manta](https://www.manta.sh.cn), served at `/docs` (same URL shape as `https://www.manta.sh.cn/docs`).
...
`www.manta.sh.cn`; configure its Next/Vercel proxy with these `beforeFiles`
