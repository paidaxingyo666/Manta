...
AI Vault will remain part of the integrated Manta renderer, but its host-side work will move behind a persistent service-process boundary.

The target has three rules:

1. The desktop and Manta runtime route local Vault scans and title resolution to one lazy, supervised Vault service process per host process.
...
- Treating the service process as a security sandbox. It runs trusted Manta code with the same user identity.
...
| Manta runtime scan and title resolution  | The same worker client used by runtime RPC methods                           | Same-process CPU, memory, and lifecycle coupling with the runtime host                                                       |
...
| Dynamic roots         | Preserve environment overrides, managed/per-account Codex homes, runtime Codex homes, WSL default and Manta-owned homes, and every platform-specific agent root                                                          | Router resolves dynamic homes per request; service discovers within them    |
...
- Manta runtime: one Vault service per runtime host process, shared by all connected clients.
...
```bash
MANTA_AI_VAULT_SERVICE_PROCESS=0 pnpm bench:ai-vault-typing -- --label worker-control
MANTA_AI_VAULT_SERVICE_PROCESS=1 pnpm bench:ai-vault-typing -- --label process-treatment
```
...
- Desktop and runtime list scans, title resolution, subagent reads, and first-prompt reads use a lazy supervised child process by default outside unit tests. `MANTA_AI_VAULT_SERVICE_PROCESS=0` retains the worker fallback.
...
- Route desktop IPC and runtime RPC through the service when `MANTA_AI_VAULT_SERVICE_PROCESS` is enabled.
...
- Keep `MANTA_AI_VAULT_SERVICE_PROCESS=0` as a release kill switch for one stable cycle.
...
- Desktop/runtime: set `MANTA_AI_VAULT_SERVICE_PROCESS=0` to restore the current worker path without changing public APIs or stored data.
