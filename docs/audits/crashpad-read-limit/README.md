...
After a crash, Manta extracts a short diagnostic signature from a potentially large dump.
...
```sh
MANTA_BACKGROUND_LAUNCH=1 node docs/audits/crashpad-read-limit/reproduce.mjs
```
...
```sh
MANTA_BACKGROUND_LAUNCH=1 node docs/audits/crashpad-read-limit/stream-signature-parity.cjs
```
