...
```sh
MANTA_BACKGROUND_LAUNCH=1 node --expose-gc --max-old-space-size=192 docs/audits/osc133-carry-retention/reproduce.cjs
```

Run the same script with the installed Electron executable, setting
`ELECTRON_RUN_AS_NODE=1` and `MANTA_BACKGROUND_LAUNCH=1`, with the same Node flags.
...
`manta-runtime-get-unpersisted-tracked-title-for-pty.ts`; scanner enablement still
