...
if (process.env.MANTA_BACKGROUND_LAUNCH !== '1' || !global.gc) {
  throw new Error('Run with MANTA_BACKGROUND_LAUNCH=1 and node --expose-gc.')
}
const hash = (text) => createHash('sha256').update(text).digest('hex')
const verifierPath = 'src/main/runtime/manta-runtime-stop-structured-session-process.ts'
