...
10. The "Manta Relay" path label only renders once `state === 'connected'`
...
| `manta://` deep links (scheme in `app.json:9`, no linking config) | any `/h/...` | Default filesystem linking, zero coordination; `app/_layout.tsx:52-58` only intercepts pairing codes |
...
- Path label ("Manta Relay") gated on `connected` (`src/components/MobileHostCard.tsx:47`);
...
follow-up). `manta://` deep links can follow later via a route-level guard.
...
show the path being dialed ("Connecting via Manta Relay…") by exposing the pending path, and let
