...
Manta's Linux builds target **stock Ubuntu 20.04 and newer** — glibc 2.31 and
...
Because the Manta main process loads node-pty at startup, that failure crashes the
...
libxcb-dri3, FontConfig/expat). Manta 1.4.199/1.4.200 shipped that runtime and
...
below the floor. Manta itself writes `process.env` during early startup
(`patchPackagedProcessPath`, `configureMantaUserDataPathEnv`,
