...
against Manta for exactly this reason.
...
- **The interactive terminal.** `src/main/daemon/pty-subprocess/native-pty-spawn.ts`
  calls `pty.spawn` directly, so typing `codex` in a Manta terminal is
  completely unaffected.
- **Manta's own hook wrappers** (`codex-hook.cmd` and friends). These are batch
  files Manta writes, matching none of the generator shapes, so they keep the
...
exporting it before launching Manta disables resolution process-wide.
