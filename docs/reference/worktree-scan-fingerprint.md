...
(`MantaRuntimeService.listRepoWorktreesForResolution`). It keeps the existing
...
- `computeResolvedWorktrees` fans out over **every** registered repo
  (`src/main/runtime/manta-runtime.ts`), calling
...
The in-Manta mutation surface is already event-driven: create, remove, rename,
...
worktree changes made **outside** Manta (`git worktree add/remove/move/prune`,
...
| Change | Before | After |
| --- | --- | --- |
| Manta-initiated create/remove/rename/sparse/repo edit | immediate (event) | immediate (event) |
...
changes Manta does not make itself.
...
Git runtime is still probed, because it is not WSL-routed from Manta's point of
