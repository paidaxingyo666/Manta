...
`HostKeyAlias` if set, else the **resolved hostname** — keying on the Manta label would miss every
existing entry.
...
`StrictHostKeyChecking=no` did not help. `MANTA_SSH_FORCE_SYSTEM_TRANSPORT=1` is read from the
...
> **Ours disagreed** — "The host key for `build-01` changed since you last connected from Manta. If you
...
2. **We keyed on the Manta label.** `ssh -G` echoes its own argument back as `hostname` when no Host
...
- **The web user never sees the reason.** `runtime/rpc/methods/ssh.ts` rethrows
  ...
  _desktop_ user viewing a remote-Manta-server-owned host gets the same generic string.
...
**Rollout:** the first release carrying this is the first time Manta can refuse an SSH connection at
