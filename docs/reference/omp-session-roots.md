...
- Manta retains its legacy `OMP_CODING_AGENT_DIR` sessions-root override and prefix
...
```sh
MANTA_BACKGROUND_LAUNCH=1 bun tests/tools/omp-session-root-upstream-smoke.mjs /path/to/oh-my-pi
```

The smoke uses disposable home/data roots and compares Manta's result with OMP's
...
```sh
MANTA_BACKGROUND_LAUNCH=1 bun tests/tools/omp-transcript-root-reader-smoke.mjs /path/to/oh-my-pi
```
...
ID through Manta's native reader. All files use disposable roots; no model runs.
