...
Manta ships `@xterm/xterm` with four source changes it needs and upstream has
...
an image the protocol already acked as `OK`. Manta uses fixed 32 MB storage and
...
`tsgo` walk up into Manta's own `node_modules` and fail with `TS2300: Duplicate
...
A vendored `@xterm/xterm` fork removes the patch entirely, but it moves Manta off
the published package, so every upstream beta becomes a merge rather than a
version bump, and Manta inherits responsibility for building and publishing a
...
hand-edited minified bundles carrying a literal `/* PATCH(manta): ... */` comment
