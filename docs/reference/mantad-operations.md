...
`mantad` is the Manta runtime served from plain Node. This is the contract between it and
...
A deployment is **mantad** plus **the terminal daemon**.

|            | mantad                            | terminal daemon                       |
| ---------- | -------------------------------- | ------------------------------------- |
| Started by | the supervisor                   | mantad, detached                       |
| Owns       | RPC, git, worktrees, persistence | every local PTY                       |
| Lifetime   | one supervised run               | detached from mantad, not its service  |
| Endpoint   | `ws://<bind>:<port>`             | `<data-root>/daemon/daemon-v<N>.sock` |

mantad detaches the daemon and calls `disconnectDaemon()`, never `shutdownDaemon()`. The
built-in remote deployment path stops only the recorded mantad PID, so the daemon and its PTYs
...
Process detachment is not service isolation. A daemon forked by mantad, and every PTY it owns,
...
`manta serve`'s wide default, and a startup where some device has connected before — and an
...
Under the shipping design a client reaches a remote mantad over an SSH local port-forward, so
loopback is the correct default and the pairing credential travels over SSH.

## Data root and the instance lock

The data root is `$MANTA_USER_DATA`, else `$XDG_DATA_HOME/Manta`, else `~/.manta`.

Before the profile index or the store is touched, mantad takes `<data-root>/orcad.lock`.
...
| `orcad_instance_lock_held`             | another live mantad owns this root                             |
...
refused — mantad stores credentials there unsealed (no OS keyring on this host), so the goal
...
Before a cgroup-wide stop, obtain a fresh `manta-ide terminal list --json` result using the same OS
...
`sudo -Hu manta /home/manta/.local/bin/manta-ide terminal list --json`). Replace both `manta` and
`/home/manta` with the service account and home used by the unit; an extracted deployment may use
its absolute `resources/bin/manta-ide` launcher instead. A safe empty census is untruncated, has an explicit `hostScope`, covers every
...
not admit new work after the census. Manta does not yet provide an atomic census-and-stop fence.

### Who supervises mantad

An external supervisor (systemd, launchd, a process manager). mantad conforms to it:

- **Readiness.** One JSON line on stdout (`--json`), `type: "manta_server_ready"`, published
...
- **Shutdown.** `SIGTERM` or `SIGINT` starts a graceful stop. A **second** signal exits
  ...
  the one teardown that skips the daemon handoff. mantad also imposes its own 15s deadline
...
- **Logs.** mantad writes human-readable diagnostics to **stderr** and its readiness contract
...
- **Launch.** Forked detached from `daemon-entry.js` beside `mantad.js`, with its own PID
...
- **Crash-loop containment.** At most **5 launches per 60s rolling window** per mantad run;
  ...
  recovers without restarting mantad. An operator-initiated daemon restart clears it — that
  is the deliberate "try again".
- **No macOS login-session watch.** That watch retires the daemon when the spawning GUI login
  session dies. An mantad daemon must survive its SSH session ending.
- **Shutdown.** mantad never stops the daemon. A daemon that was never adopted retires itself
  after its adoption window; an adopted one stays resident (see Decommissioning).

### Decommissioning

After a PID-scoped stop, an adopted daemon stays resident so the next mantad can reattach.
A combined-unit systemd stop kills it instead. To retire a process-scoped deployment, apply
the census rule above, stop mantad, then stop the daemon named by `health.terminalDaemon.pid`.
...
```
buildHash    sha256 (16 hex) of the running mantad bundle — build identity that a version
             string cannot give, so a rollback that did not replace the file is visible
buildVersion MANTA_VERSION
...
terminalDaemon:
  ...
  buildVersion       the build the LIVE daemon was forked from (may legitimately predate
                     this mantad after an update — reporting mantad's version for both would
                     hide exactly that)
...
processes: mantad drives it, the daemon performs it, the verdict crosses the socket.
...
because those terminals die with mantad. A daemon that answered and then failed its spawn
...
- **A continuous health endpoint.** `health` is published once, in the readiness payload. A
  ...
  `collectMantadHealth()`; that surface does not exist yet.
- **Systemd-isolated daemon supervision.** mantad and its daemon currently share one service
