...
Manta shows "what is this agent doing" in four places: the desktop sidebar, the
`manta worktree ps` command, the mobile app, and the agent dashboard. Before
...
call (`manta-runtime-create-terminal-side-effect-command-code-detector.ts`).
...
- a row carrying `structuredHost` is admitted while the host holds the session, and
  ...
  obligation per entry point: the Electron hosts (desktop and `manta serve`)
  share `main-process-runtime-service.ts`, and `mantad` constructs its own
  runtime in `src/main/mantad/mantad-entry.ts`. A host missing that wiring lists
...
| ------------------------------------------------------------------------------ | --------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| `manta-runtime-create-terminal-side-effect-command-code-detector.ts` `retain()` | second write of the OSC payload already sent to the hook server | deleted; the event now carries the pane's `terminalHandle` and the hook ingest keeps the only copy |
| `...command-code-detector.ts` `clearPty()`                                     | drops rows on pty exit                                          | deleted; pane teardown already clears the hook row                                                 |
| `manta-runtime-get-worktree-ps.ts` `values()`                                   | fed `retainedSnapshots`                                         | deleted; the reader keeps only `hookSnapshots`                                                     |
| `manta-runtime-serialize-agent-prompt-submission.ts` `getFreshExplicit()`       | retained row first, hook rows second                            | `selectFreshExplicitAgentStatus`, hook rows only                                                   |
| `manta-runtime-prune-mobile-session-tab-group-layout.ts` `getFreshForMobile()`  | pane key, then pty id                                           | `selectFreshAgentRowForMobileTab`: pane key, then `terminalHandle`                                 |
...
`mantad` constructed its runtime with no `onTerminalAgentStatus`, so main's OSC
parse never reached the store there and the retained copy was the only carrier.
Deleting it without wiring mantad would have made a headless host list no PTY
agents at all. `mantad-entry.ts` now binds the producer and installs the
...
- **Performance budget:** publication stays event-driven with no new polling or
  ...
  cleanup is bounded by the existing status and handle inventories, and mantad
...
- Unit: the `worktree ps` suites written against the retained store are rewired
  ...
  `manta-runtime-tests/worktree-ps-agent-row-dismissal.spec.ts`, which fails with
