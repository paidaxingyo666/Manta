...
Windows 10, so every OS Manta supports has it. The entry point is resolved with
...
pass to Manta's own subtree is the complementary change, and it belongs with the
...
`@manta/windows-registry` (asserted by
...
addon can produce is unusable for the sizes Manta now sees: `process.cc` stores
...
Start time is a proxy for identity, not identity. For the process trees Manta
itself spawns the durable answer is still an inherited handle: a job object
names the tree Manta created, so no start-time comparison is needed. The
`creationTimeMs` this snapshot now carries is for the trees Manta did **not**
