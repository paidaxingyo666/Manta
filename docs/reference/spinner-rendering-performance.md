...
does not make rendering or the rest of Manta free.
...
The opt-in Playwright benchmark launches a fresh, hidden Manta app for each
...
iteration listener. That avoided work also exists in Manta itself, independently
...
```sh
MANTA_BACKGROUND_LAUNCH=1 pnpm bench:spinners --sample-ms=5000
MANTA_BACKGROUND_LAUNCH=1 pnpm bench:spinners --verify-only --scale-factor=1
MANTA_BACKGROUND_LAUNCH=1 pnpm bench:spinners --verify-only --scale-factor=2
MANTA_BACKGROUND_LAUNCH=1 ORCA_SPINNER_BENCH=1 ORCA_SPINNER_KEYS=64 \
...
is not representative of Manta with visible animations. Neither result justifies
