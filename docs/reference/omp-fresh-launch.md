...
Manta's new-session and draft launch plans apply a one-time `--config` overlay
...
The execution host creates the overlay. Local and WSL terminals use Manta userData
...
```sh
MANTA_BACKGROUND_LAUNCH=1 bun tests/tools/omp-fresh-session-runtime-smoke.mjs /path/to/oh-my-pi
```
...
bundle to the host and run it using Node with `MANTA_BACKGROUND_LAUNCH=1`.
