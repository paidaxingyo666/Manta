...
Manta selects a Relay region in the Electron main process before requesting a new assignment. The
director publishes an allowlisted region catalog containing only HTTPS cell subdomains of that
director. Manta discards one warm-up `/health` request per probe origin — a cold request pays TCP and
...
be measured, Manta sends no hint rather than selecting the sole survivor. Sending no hint is not
...
After a control socket registers, Manta probes the cell it actually landed on, once per cell URL per
