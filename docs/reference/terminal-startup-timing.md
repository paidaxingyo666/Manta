...
```js
localStorage.setItem('manta:terminal-startup-timing', '1')
```

Remove the key to disable it. Existing sessions are unaffected. To capture the existing host spawn phases, start the host with `MANTA_PTY_SPAWN_TIMING=1`. Do not restart a host with active work just to enable diagnostics.
