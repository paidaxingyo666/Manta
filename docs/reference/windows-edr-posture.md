...
`%LOCALAPPDATA%\Manta\daemon-host\<version>\` and renames `Manta.exe` to
`manta-terminal-daemon.exe`. The comment on `DAEMON_HOST_EXE_NAME` states the
reason without varnish: _"so the NSIS updater's `taskkill /IM Manta.exe` can't
match it."_
...
`Manta.exe` → `manta-terminal-daemon.exe` → a shell → an agent CLI is what a
