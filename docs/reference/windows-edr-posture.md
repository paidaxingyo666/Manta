...
Manta's Windows process tree is shaped like the thing behavioural EDR is built to
find. An enterprise Windows 11 / Intune tenant opened **six Microsoft Defender
for Endpoint incidents against Manta 1.4.192 in eight days**. All six fired as
...
that Manta is malware. It is a map of which of our behaviours are legible to an
...
| ----------------- | --------- | -------------------------------------------------------------------------------------------------------------------- |
| **Update**        | A, B, C   | `manta-windows-setup.exe` → `old-uninstaller.exe`, `Uninstall Manta.exe` (electron-builder generates these; they are in no repo file) |
| **Spawn**         | all six   | `Manta.exe` → `manta-terminal-daemon.exe` → `powershell.exe` / `pwsh.exe` / `cmd.exe` / `reg.exe` → `claude.exe`, `gh.exe`, `codex.cmd` |
...
In the update cluster the uninstaller is genuinely `NotSigned`, while `Manta.exe`
and `manta-terminal-daemon.exe` report `Valid CN=SignPath Foundation`.
...
`%LOCALAPPDATA%\Manta\daemon-host\<version>\` and forks the terminal daemon from
there.
...
`Manta.exe` rather than `node.exe` so there is no console flash and asar still
resolves; `config/nsis/manta-installer-hooks.nsh` reaps it on a real uninstall
...
`manta-terminal-daemon.exe`, the image name every incident here reports, and
`DAEMON_HOST_EXE_NAME`'s comment stated the reason without varnish: _"so the NSIS
updater's `taskkill /IM Manta.exe` can't match it."_ The rename has since been
...
`Manta.exe` → the relocated daemon host (`manta-terminal-daemon.exe` in the builds
these incidents cover, `Manta.exe` since) → a shell → an agent CLI is what a
...
breadth: every interpreter hop between Manta and the thing the user asked for adds
...
decoded payload was a `Test-Path` and a call to `~/.manta/agent-hooks/claude-hook.cmd`.
...
70% from Claude sessions that were not running under Manta at all and whose hook
exits at its first `MANTA_PANE_KEY` guard.
...
flagged, while Manta's is signed and was flagged six times.** Their conclusion:
...
on install count alone. Manta's signature is a free OV certificate from SignPath
...
- **Shorten the interpreter chain.** Each hop between Manta and the user's actual
...
## Guidance for administrators deploying Manta
...
`%LOCALAPPDATA%\Programs\manta\` to the AV exclusion list and expecting the
...
- **Alert titles** — `A suspicious file was observed` and
  ...
  this list.
- **File paths** — `Manta.exe` and `manta-terminal-daemon.exe` under
  `%LOCALAPPDATA%\Programs\manta\` and `%LOCALAPPDATA%\Manta\daemon-host\`.

Scope it as narrowly as your tenant will tolerate, and review it when Manta
...
`%LOCALAPPDATA%\Manta\daemon-host\` is the normal location rather than a
guaranteed one. Prefer a prefix match on `…\Manta\daemon-host\` over a rule
...
criterion"** enabled. If it is, that alone explains a freshly signed Manta build
...
allowlist the Manta install paths for that rule or expect a hit on each update.
...
Every part of that is an accurate description of what the feature does. Manta's
...
- **Allowlist it**, with a suppression rule covering the computer-use tree
  (`powershell.exe` with `-File …\runtime.ps1`) as well as the base Manta paths,
