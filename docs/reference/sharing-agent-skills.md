...
Manta can put one skill or a bundle of skills behind one unlisted, revocable link. Shared bundles
...
Publishing and link management require a Manta account in the desktop app.
...
Manta publishes an immutable version. Later changes do not silently alter a link's current bytes;
...
Opening a Manta skill link shows a preview before changing any files. You can also open **Skills**,
...
Supported destinations include the local machine, paired Manta runtimes, WSL, and SSH hosts. The
...
Manta keeps one canonical installed copy and places it where supported agents can discover it.
...
**Keep local** is the default when an existing skill differs. Manta replaces modified content only
...
Manta updates only the selected skills that still exist in that version.

An interrupted install is recovered on restart. If Manta reports a conflict or partial result,
review the named skill and retry; completed skills do not need to be installed again.

## Remove an installed skill

Use **Skills → Manage installs → Remove**. Manta removes only copies and provider placements that it
...
executables may run later when a person or agent uses the skill. Manta validates the package and
...
Manta records bounded operational identifiers and outcomes. Normal logs, telemetry, and support
...
links intentionally show the same response so Manta does not disclose private package existence.
