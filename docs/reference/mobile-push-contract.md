...
Base URL: `https://push.manta.sh.cn` (dev override via env). JSON bodies, `Content-Type: application/json`.
...
- `plaintext = "manta-push-host-challenge/v1\0" || u32be(len(transcript)) || transcript || secret(32)`
...
- Transcript is the relay's length-prefixed field encoding (`field(name, value)` =
  ...
  `protocol="manta-push-host-proof/v1"`, `version=0x01`, `gatewayOrigin`, `gatewayEphemeralPublicKey`,
...
- Host opens the box with its secret key, validates every transcript field (same checks as
  ...
  and returns `proof = HMAC-SHA256(secret, "manta-push-host-proof/v1\0ack\0" || transcript)`.
...
summary: title `Manta`, body `<N> agents need attention` (or `<N> updates` when no needs-input), data
...
- body: `{"aps":{"alert":{"title","body"},"sound":"default","thread-id":"<hostFingerprint>"},
  "manta":{ hostFingerprint, worktreeId, notificationId, notificationSeq, notificationEpoch, source,
...
- `{"message":{"token","notification":{"title","body"},"android":{"priority":"HIGH","ttl":"14400s",
  "collapse_key":"<sha256(collapseId) hex 32>","notification":{"channel_id":"manta-desktop","tag":"<collapseId>"}},
  "data":{ all manta fields as strings }}}`
...
Secret Manager names (already exist in `onorca-cloud`): `manta-cloud-push-apns-key`,
`manta-cloud-push-apns-key-id`, `manta-cloud-push-apple-team-id`. Runtime SA:
...
- Push client `src/main/runtime/push/push-gateway-client.ts`: challenge/proof/session with token cache,
  ...
  (`pushGatewayUrl`, default `https://push.manta.sh.cn`, env override `ORCA_PUSH_GATEWAY_URL`).
...
- Settings (`mobile/app/notifications.tsx`): single "Background notifications" switch, default off,
  hint text exactly: "Get alerts while Manta is closed. Alerts show the same text as on your desktop. That
  text, your phone's push token, and opaque host and device ids pass through Manta's push service and Apple
...
- Cloud Run service `manta-cloud-push`, region `us-central1`, project from the environment tfvars, runtime
  SA `manta-cloud-push@<project>.iam.gserviceaccount.com` (exists in prod; declare and import), the three
...
- Hostname `push.manta.sh.cn`. The DNS zone lives in the apps root in `stablyai/orca-cloud`; add the
...
uses Android's `manta-desktop-silent` channel. Missing sound preserves existing audible delivery.
