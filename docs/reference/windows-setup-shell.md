...
On native Windows, Manta writes the `manta.yaml` setup script (and the issue command) to a generated
...
- Two people on the same repo got different interpreters for the same `manta.yaml`, so no project
...
A `#!` line only takes effect when Manta can actually launch bash from the configured terminal — the
