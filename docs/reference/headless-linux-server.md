...
Use this guide when you want to run `manta serve` on a Linux machine without a
desktop session, such as an Ubuntu VPS or a remote build box.

`manta serve` starts the Manta runtime without opening the desktop window. On
Linux, the packaged AppImage still needs the libraries that Electron expects at
startup. Current Manta builds start Xvfb automatically for `manta serve` when no
`DISPLAY` is set, but Xvfb must be installed first. A separate D-Bus session is
not required. When `DISPLAY` is set, Manta uses that display instead of starting
...
whose lock names a dead process is refused rather than replaced, and `manta serve`
exits — unset `DISPLAY` to let Manta start its own Xvfb. A socket published with
...
```bash
LIBGL_ALWAYS_SOFTWARE=1 /opt/manta/manta-linux.AppImage serve --port 6768
```
...
```bash
LIBGL_ALWAYS_SOFTWARE=1 /opt/manta/manta-linux.AppImage serve \
  --port 6768 \
  --pairing-address 100.64.1.20
```

`--pairing-address` is only the address advertised to clients. It does not
change the listener bind address. Manta binds its WebSocket listener, then
...
proxy URL such as `https://manta.example.com/runtime` (`http(s)` is normalized
...
Pairing URL: manta://pair?code=...
```

For supervisors, request the versioned single-line JSON contract:

```bash
/opt/manta/manta-linux.AppImage serve --port 6768 \
...
```ini
# /etc/systemd/system/manta-serve.service
[Unit]
Description=Manta runtime server
...
Type=simple
User=manta
WorkingDirectory=/home/manta
Environment=LIBGL_ALWAYS_SOFTWARE=1
ExecStart=/opt/manta/manta-linux.AppImage serve --port 6768 --pairing-address 100.64.1.20
...
`KillMode=mixed` sends the graceful stop signal only to Manta's main process,
then retains systemd's cgroup-wide `SIGKILL` fallback if shutdown times out.
This lets Manta keep its owned Xvfb alive until Electron disconnects cleanly.
It does **not** preserve the detached terminal daemon: the daemon and its PTYs
remain in `manta-serve.service`'s cgroup and are killed when the stop completes.
...
`sudo systemctl reset-failed manta-serve.service` first to clear it — the
...
20.04, Manta's oldest supported base, ships systemd 245.

Enable the service:

```bash
sudo systemctl daemon-reload
sudo systemctl enable --now manta-serve.service
sudo journalctl -u manta-serve.service -f
```
...
```bash
sudo journalctl -u manta-serve.service -o cat \
  | jq -Rrc 'fromjson? | select(.type == "manta_server_ready" and .schemaVersion == 1)'
```
...
separate service and set `DISPLAY=:99` for Manta.

```ini
# /etc/systemd/system/manta-xvfb.service
[Unit]
Description=Virtual X display for Manta
After=network-online.target
Wants=network-online.target
...
Then add the display dependency to the Manta service:

```ini
# /etc/systemd/system/manta-serve.service
[Unit]
Description=Manta runtime server
After=network-online.target manta-xvfb.service
Wants=network-online.target manta-xvfb.service
StartLimitIntervalSec=300
StartLimitBurst=5
...
```bash
sudo systemctl daemon-reload
sudo systemctl enable --now manta-xvfb.service manta-serve.service
```

## CLI Install Note

The registered Linux CLI command is `manta-ide`, not `orca`, to avoid shadowing
the GNOME Orca screen reader. Desktop-managed terminals receive a
terminal-scoped bare-`manta` shim. A packaged headless `manta serve` also makes a
best-effort dispatcher at `$HOME/.local/bin/manta` for the service user's own
shell, so the Claude Teams launcher can resolve its bare command; it does not
replace another user's `manta`. From an ordinary shell outside that service
user's managed environment, substitute `manta-ide` for `manta` in commands below.

On a headless host, you do not need to open the desktop UI just to run the
server. Invoke the AppImage directly:

```bash
/opt/manta/manta-linux.AppImage serve --help
```
...
```bash
/opt/manta/manta-linux.AppImage --no-sandbox serve --port 6768
```
...
If you later install the desktop CLI from Manta settings, use that CLI for normal
shell workflows. Keep the AppImage path in systemd so service restarts do not
depend on an interactive shell profile.

## Upgrade

`manta serve` never updates itself. In headless mode Manta wires up no auto-updater
...
- **New builds migrate old state on load.** Manta loads older `manta-data.json`
...
`sudo -Hu manta /home/manta/.local/bin/manta-ide terminal list --json`.
Replace both `manta` and `/home/manta` with the service account and home used by
your unit; for an extracted deployment, use its absolute `resources/bin/manta-ide`
...
and the stop; Manta does not yet provide an atomic census-and-stop fence.

Rolling back is the case that needs care — see [Roll back](#roll-back).

### Record the version you deploy

The bundled CLI launcher prints the Manta build with `manta-ide --version`. For an
extracted deployment, that launcher is
`squashfs-root/resources/bin/manta-ide`; deb/rpm installs and CLI registration put
it on `PATH`. Do not use `manta-linux.AppImage --version` for this audit because
...
next to the binary. The steps below keep that record in `/opt/manta/VERSION`.

### Upgrade steps

Never download straight onto `/opt/manta/manta-linux.AppImage`. The AppImage is
...
```bash
sudo chown root:root /opt/manta
sudo chmod 755 /opt/manta
sudo test ! -L /opt/manta/manta-linux.AppImage
sudo chown root:root /opt/manta/manta-linux.AppImage
sudo chmod 755 /opt/manta/manta-linux.AppImage
# Clear predictable staging names left by an older attempt after locking the directory
sudo rm -f /opt/manta/manta-linux.AppImage.new /opt/manta/VERSION.new \
  /opt/manta/manta-linux.AppImage.recovering /opt/manta/VERSION.recovering
sudo du -sh /home/manta/.config
df -h /opt/manta /home/manta
```

`/opt/manta` needs room for the compressed Manta profile archive, the staged
build, and the rollback binary. A rollback extracts the old profile and preserves
the post-upgrade Manta profile directories, so `/home` needs room for both copies.
...
MANTA_VERSION=v1.4.147

# Select the release asset on the server where Manta runs
case "$(uname -m)" in
  x86_64)
    MANTA_ASSET=manta-linux.AppImage
    MANTA_FILE_MACHINE=x86-64
    ;;
  aarch64 | arm64)
    MANTA_ASSET=manta-linux-arm64.AppImage
    MANTA_FILE_MACHINE='ARM aarch64'
    ;;
  *)
...
MANTA_ROLLBACK_NEW=
MANTA_ROLLBACK=
MANTA_SERVICE_STOPPED=0
MANTA_BINARY_PROMOTED=0
recover_failed_upgrade() {
  ...
  if ((exit_status != 0)) && [[ -n "$MANTA_ROLLBACK_NEW" ]] && \
    sudo test -d "$MANTA_ROLLBACK_NEW"; then
    sudo rm -rf -- "$MANTA_ROLLBACK_NEW"
  fi
  if ((exit_status != 0 && MANTA_SERVICE_STOPPED)); then
    recovery_ok=1
    if ((MANTA_BINARY_PROMOTED)); then
      if ! sudo cp -a "$MANTA_ROLLBACK/manta-linux.AppImage" \
        /opt/manta/manta-linux.AppImage.recovering || \
        ! sudo mv -f /opt/manta/manta-linux.AppImage.recovering \
          /opt/manta/manta-linux.AppImage; then
        recovery_ok=0
      fi
      if sudo test -f "$MANTA_ROLLBACK/VERSION"; then
        if ! sudo cp -a "$MANTA_ROLLBACK/VERSION" /opt/manta/VERSION.recovering || \
          ! sudo mv -f /opt/manta/VERSION.recovering /opt/manta/VERSION; then
          recovery_ok=0
        fi
      elif ! sudo rm -f /opt/manta/VERSION; then
        recovery_ok=0
      fi
    fi
    sudo rm -f /opt/manta/manta-linux.AppImage.recovering \
      /opt/manta/VERSION.recovering
    if ((recovery_ok)); then
      # A tripped StartLimitBurst refuses a plain start
      sudo systemctl reset-failed manta-serve.service || true
      sudo systemctl start manta-serve.service || true
    else
...
MANTA_FILE_INFO=$(LC_ALL=C file /opt/manta/manta-linux.AppImage.new)
grep 'ELF .* executable' <<<"$MANTA_FILE_INFO"
grep -F "$MANTA_FILE_MACHINE" <<<"$MANTA_FILE_INFO"

# 2. Assemble the prior binary and version in a root-only rollback bundle
MANTA_ROLLBACK_BASE=/opt/manta/manta-rollback-$(date +%F-%H%M%S-%N)
MANTA_ROLLBACK_NEW=${MANTA_ROLLBACK_BASE}.new
MANTA_ROLLBACK=${MANTA_ROLLBACK_BASE}.ready
sudo install -d -m 700 "$MANTA_ROLLBACK_NEW"
sudo cp -a /opt/manta/manta-linux.AppImage "$MANTA_ROLLBACK_NEW/manta-linux.AppImage"
if sudo test -f /opt/manta/VERSION; then
  sudo cp -a /opt/manta/VERSION "$MANTA_ROLLBACK_NEW/VERSION"
fi

# Stage the new version record before the stop window
printf '%s\n' "$MANTA_VERSION" | sudo tee /opt/manta/VERSION.new >/dev/null
sudo chown root:root /opt/manta/VERSION.new
sudo chmod 644 /opt/manta/VERSION.new

# 3. Stop the server so the profile backup is consistent
MANTA_SERVICE_STOPPED=1
sudo systemctl stop manta-serve.service

# Add only Manta-owned profile directories, then publish the complete bundle
MANTA_PROFILE_DIRS=()
for profile_dir in manta Manta; do
  if sudo test -L "/home/manta/.config/$profile_dir"; then
    echo "Refusing symlinked Manta profile: /home/manta/.config/$profile_dir" >&2
    exit 1
  fi
  if sudo test -d "/home/manta/.config/$profile_dir"; then
    if [[ "$profile_dir" == Manta ]] && \
      sudo test /home/manta/.config/manta -ef /home/manta/.config/Manta; then
      continue
    fi
    MANTA_PROFILE_DIRS+=("$profile_dir")
  fi
done
if ((${#MANTA_PROFILE_DIRS[@]} == 0)); then
  echo 'No Manta profile directory found under /home/manta/.config' >&2
  exit 1
fi
sudo tar czf "$MANTA_ROLLBACK_NEW/profile.tgz" \
  -C /home/manta/.config "${MANTA_PROFILE_DIRS[@]}"
sudo chmod 600 "$MANTA_ROLLBACK_NEW/profile.tgz"
sudo mv "$MANTA_ROLLBACK_NEW" "$MANTA_ROLLBACK"

# 4. Atomically replace the binary and version record, then start
MANTA_BINARY_PROMOTED=1
sudo mv -f /opt/manta/manta-linux.AppImage.new /opt/manta/manta-linux.AppImage
sudo mv -f /opt/manta/VERSION.new /opt/manta/VERSION
# Clears a start-limit hit left by the version being replaced
sudo systemctl reset-failed manta-serve.service
sudo systemctl start manta-serve.service
MANTA_SERVICE_STOPPED=0
trap - EXIT
```

The profile archive created in step 3 captures both Manta profile directory names
when present without rewinding unrelated tools under `/home/manta/.config`. The
...
`manta-serve.service` needs restarting — leave `manta-xvfb.service` running.
...
A healthy start prints one `Manta server ready` block with the actual bound and
...
```bash
shopt -s nullglob
MANTA_ROLLBACK_SETS=(/opt/manta/manta-rollback-*.ready)
((${#MANTA_ROLLBACK_SETS[@]} > 0))
MANTA_ROLLBACK=${MANTA_ROLLBACK_SETS[${#MANTA_ROLLBACK_SETS[@]} - 1]}
printf 'Removing rollback bundle: %s\n' "$MANTA_ROLLBACK"
sudo test -d "$MANTA_ROLLBACK"
sudo rm -rf -- "$MANTA_ROLLBACK"
```
...
rewrite `manta-data.json` in the current schema. If an older build then writes
that file, it can discard fields it does not recognize. The rolling
`manta-data.json.bak.*` files are corruption-recovery snapshots, not a dedicated
...
((${#MANTA_ROLLBACK_SETS[@]} > 0))
MANTA_ROLLBACK=${MANTA_ROLLBACK_SETS[${#MANTA_ROLLBACK_SETS[@]} - 1]}
sudo test -f "$MANTA_ROLLBACK/manta-linux.AppImage"
sudo tar tzf "$MANTA_ROLLBACK/profile.tgz" >/dev/null

# Extract and validate the old profile while the current server stays online
sudo test ! -L /home
MANTA_HOME_OWNER=$(sudo stat -c %u /home)
MANTA_HOME_MODE=$(sudo stat -c %a /home)
if [[ "$MANTA_HOME_OWNER" != 0 ]] || ((8#$MANTA_HOME_MODE & 0022)) || \
  sudo -u manta test -w /home; then
  echo 'Refusing rollback because /home is not root-controlled' >&2
  exit 1
fi
MANTA_RESTORE=$(sudo mktemp -d /home/.manta-restore.XXXXXX)
MANTA_SERVICE_STOPPED=0
MANTA_MOVED_CURRENT_DIRS=()
MANTA_INSTALLED_RESTORE_DIRS=()
MANTA_CURRENT_BINARY_MOVED=0
MANTA_CURRENT_VERSION_MOVED=0
MANTA_VERSION_REPLACEMENT_STARTED=0
MANTA_POST_UPGRADE=
MANTA_ROLLBACK_BINARY_STAGED=
MANTA_ROLLBACK_VERSION_STAGED=
MANTA_ROLLBACK_HAS_VERSION=0
restart_after_rollback_error() {
  ...
  if ((exit_status != 0 && MANTA_SERVICE_STOPPED)); then
    recovery_ok=1
    if ((${#MANTA_INSTALLED_RESTORE_DIRS[@]})); then
      for profile_dir in "${MANTA_INSTALLED_RESTORE_DIRS[@]}"; do
        if sudo test -d "/home/manta/.config/$profile_dir"; then
          if ! sudo mv "/home/manta/.config/$profile_dir" \
            "$MANTA_RESTORE/$profile_dir.failed"; then
            recovery_ok=0
          fi
        fi
      done
    fi
    if ((${#MANTA_MOVED_CURRENT_DIRS[@]})); then
      for profile_dir in "${MANTA_MOVED_CURRENT_DIRS[@]}"; do
        if sudo test -d "$MANTA_POST_UPGRADE/$profile_dir"; then
          if ! sudo mv "$MANTA_POST_UPGRADE/$profile_dir" /home/manta/.config/; then
            recovery_ok=0
          fi
        elif ! sudo test -d "/home/manta/.config/$profile_dir"; then
          recovery_ok=0
        fi
      done
    fi
    if [[ -n "$MANTA_POST_UPGRADE" ]]; then
      sudo rmdir "$MANTA_POST_UPGRADE" 2>/dev/null || true
    fi
    if ((MANTA_CURRENT_BINARY_MOVED)); then
      if sudo test -f "$MANTA_CURRENT_BINARY"; then
        if ! sudo mv -f "$MANTA_CURRENT_BINARY" /opt/manta/manta-linux.AppImage; then
          recovery_ok=0
        fi
      elif ! sudo test -f /opt/manta/manta-linux.AppImage; then
        recovery_ok=0
      fi
    fi
    if ((MANTA_CURRENT_VERSION_MOVED)); then
      if sudo test -f "$MANTA_CURRENT_VERSION"; then
        if ! sudo mv -f "$MANTA_CURRENT_VERSION" /opt/manta/VERSION; then
          recovery_ok=0
        fi
      elif ! sudo test -f /opt/manta/VERSION; then
        recovery_ok=0
      fi
    elif ((MANTA_VERSION_REPLACEMENT_STARTED)); then
      if ! sudo rm -f /opt/manta/VERSION; then
        recovery_ok=0
      fi
    fi
    if ((recovery_ok)); then
  ...
  if [[ -n "$MANTA_ROLLBACK_BINARY_STAGED" ]]; then
    sudo rm -f -- "$MANTA_ROLLBACK_BINARY_STAGED"
  fi
  if [[ -n "$MANTA_ROLLBACK_VERSION_STAGED" ]]; then
    sudo rm -f -- "$MANTA_ROLLBACK_VERSION_STAGED"
  fi
  sudo rm -rf -- "$MANTA_RESTORE"
  exit "$exit_status"
}
trap restart_after_rollback_error EXIT

if [[ "$(sudo stat -c %d "$MANTA_RESTORE")" != \
  "$(sudo stat -c %d /home/manta/.config)" ]]; then
  echo 'Refusing rollback because staging and the Manta profile are on different filesystems' >&2
  exit 1
fi
sudo tar xzf "$MANTA_ROLLBACK/profile.tgz" -C "$MANTA_RESTORE"
MANTA_RESTORE_DIRS=()
for profile_dir in manta Manta; do
  if sudo test -L "$MANTA_RESTORE/$profile_dir"; then
    echo "Rollback bundle contains a symlinked profile: $profile_dir" >&2
    exit 1
  fi
  if sudo test -d "$MANTA_RESTORE/$profile_dir"; then
    if [[ "$profile_dir" == Manta ]] && \
      sudo test "$MANTA_RESTORE/manta" -ef "$MANTA_RESTORE/Manta"; then
      continue
    fi
    MANTA_RESTORE_DIRS+=("$profile_dir")
  fi
done
if ((${#MANTA_RESTORE_DIRS[@]} == 0)); then
  echo "Rollback bundle has no Manta profile directories: $MANTA_ROLLBACK" >&2
  exit 1
fi
for profile_dir in "${MANTA_RESTORE_DIRS[@]}"; do
  sudo chown -R manta:manta "$MANTA_RESTORE/$profile_dir"
done

MANTA_ROLLBACK_STAMP=$(date +%F-%H%M%S-%N)
MANTA_ROLLBACK_BINARY_STAGED=/opt/manta/manta-linux.AppImage.rollback-staged-$MANTA_ROLLBACK_STAMP
sudo cp -a "$MANTA_ROLLBACK/manta-linux.AppImage" "$MANTA_ROLLBACK_BINARY_STAGED"
if sudo test -f "$MANTA_ROLLBACK/VERSION"; then
  MANTA_ROLLBACK_HAS_VERSION=1
  MANTA_ROLLBACK_VERSION_STAGED=/opt/manta/VERSION.rollback-staged-$MANTA_ROLLBACK_STAMP
  sudo cp -a "$MANTA_ROLLBACK/VERSION" "$MANTA_ROLLBACK_VERSION_STAGED"
fi

MANTA_SERVICE_STOPPED=1
sudo systemctl stop manta-serve.service

# Preserve and replace only Manta-owned profile directories
MANTA_CURRENT_DIRS=()
for profile_dir in manta Manta; do
  if sudo test -L "/home/manta/.config/$profile_dir"; then
    echo "Refusing symlinked Manta profile: /home/manta/.config/$profile_dir" >&2
  ...
  if sudo test -d "/home/manta/.config/$profile_dir"; then
    ...
    MANTA_CURRENT_DIRS+=("$profile_dir")
  fi
done
MANTA_POST_UPGRADE=/home/manta/.config/manta-rollback-$MANTA_ROLLBACK_STAMP
sudo install -d -o manta -g manta -m 700 "$MANTA_POST_UPGRADE"
if ((${#MANTA_CURRENT_DIRS[@]})); then
  for profile_dir in "${MANTA_CURRENT_DIRS[@]}"; do
    MANTA_MOVED_CURRENT_DIRS+=("$profile_dir")
    sudo mv "/home/manta/.config/$profile_dir" "$MANTA_POST_UPGRADE/"
  done
fi
for profile_dir in "${MANTA_RESTORE_DIRS[@]}"; do
  MANTA_INSTALLED_RESTORE_DIRS+=("$profile_dir")
  sudo mv "$MANTA_RESTORE/$profile_dir" /home/manta/.config/
done

MANTA_CURRENT_BINARY=/opt/manta/manta-linux.AppImage.rollback-current-$MANTA_ROLLBACK_STAMP
MANTA_CURRENT_BINARY_MOVED=1
sudo mv /opt/manta/manta-linux.AppImage "$MANTA_CURRENT_BINARY"
sudo mv -f "$MANTA_ROLLBACK_BINARY_STAGED" /opt/manta/manta-linux.AppImage

MANTA_CURRENT_VERSION=/opt/manta/VERSION.rollback-current-$MANTA_ROLLBACK_STAMP
if sudo test -f /opt/manta/VERSION; then
  MANTA_CURRENT_VERSION_MOVED=1
  sudo mv /opt/manta/VERSION "$MANTA_CURRENT_VERSION"
fi
MANTA_VERSION_REPLACEMENT_STARTED=1
if ((MANTA_ROLLBACK_HAS_VERSION)); then
  sudo mv -f "$MANTA_ROLLBACK_VERSION_STAGED" /opt/manta/VERSION
else
  sudo rm -f /opt/manta/VERSION
fi
...
the newer `manta-data.json` in place, where an older build can discard state it
does not understand. Keep the pre-upgrade backup until the new version is proven
on your host. The `manta-rollback-*` directory inside `.config` is also retained
deliberately. The post-upgrade binary and version record are retained in
`/opt/manta` with the same `rollback-current-<timestamp>` suffix. Inspect these
artifacts and remove them according to your retention policy after the rollback
is resolved.

## Installing Agent Skills Without A Desktop

Manta's agent skills (CLI usage, orchestration, computer use, etc.) are normally
installed from Manta Settings, which pre-fills an `npx skills add ... --global`
command in a terminal for you to run. A headless host has no Settings UI, so
use `manta skills install` instead:
...
host; it does not need a running Manta runtime.
...
the choice Manta makes it explicitly: it passes an `--agent` list built from the
...
To refresh already-installed skills, `manta skills update` mirrors the same
...
Both commands install onto the machine that runs them. In a Manta SSH workspace
or the WSL bridge the `manta` shim forwards commands to the Manta host, so they
...
profile` and the unit exits `3`: another process already owns the profile, so
  ...
  owner with `systemctl status manta-serve` and `pgrep -af manta`. Stop it (or
  keep it and leave the unit down), then run
  `sudo systemctl reset-failed manta-serve && sudo systemctl start manta-serve` —
  ...
  AppImage mounts, list them with `findmnt -rn -t fuse.manta-linux.AppImage` and
