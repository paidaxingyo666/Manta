  ...
  const result = parseRegionalRehomeInventory([
    {
      ...
      textPayload: '[manta-relay] regional rehome inventory active=2 awaitingReceipt=1 targetRegistered=1 completedLast24Hours=9 abortedLast24Hours=0 oldestActiveAgeMs=30000'
    },
    {
      timestamp: '2026-08-14T11:50:00Z',
      textPayload: '[manta-relay] regional rehome inventory active=1 awaitingReceipt=0 targetRegistered=1 completedLast24Hours=8 abortedLast24Hours=0 oldestActiveAgeMs=none'
    }
...
test('rejects stale, malformed, and identity-bearing lookalikes', () => {
  assert.throws(() => parseRegionalRehomeInventory([{
    ...
    textPayload: '[manta-relay] regional rehome inventory active=0 awaitingReceipt=0 targetRegistered=0 completedLast24Hours=0 abortedLast24Hours=0 oldestActiveAgeMs=none'
  }], { now, maxAgeMs: 5 * 60_000 }), /stale/)
  assert.throws(() => parseRegionalRehomeInventory([{
    timestamp: '2026-08-14T11:59:00Z',
    textPayload: '[manta-relay] regional rehome inventory active=0 hostId=secret'
  }], { now }), /no aggregate/)
})
