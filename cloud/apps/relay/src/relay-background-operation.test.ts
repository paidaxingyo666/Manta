...
describe('relay background operations', () => {
  it('redacts free-form failure messages that could carry secrets', async () => {
    ...
    expect(warn).toHaveBeenCalledWith('[manta-relay] credential cleanup failed: Error: redacted')
  ...
  it('logs invariant slugs and SQLSTATE codes verbatim', async () => {
    ...
    await runRelayBackgroundOperation(
      ...
      '[manta-relay] assignment cleanup failed',
      warn
    )

    expect(warn).toHaveBeenCalledWith(
      '[manta-relay] assignment cleanup failed: Error: regional_rehome_assignment_mismatch code=55P03'
    )
  })
