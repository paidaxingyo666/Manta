...
import { PUSH_DEFAULTS, PUSH_LIMITS } from '@manta-cloud/push-contract'
...
const MINIMAL = { ORCA_PUSH_PUBLIC_URL: 'https://push.manta.sh.cn' }

describe('push gateway config', () => {
  it('applies the documented defaults', () => {
    expect(loadPushConfig(MINIMAL)).toEqual({
      port: 8080,
      publicUrl: 'https://push.manta.sh.cn',
  ...
  it('requires a canonical HTTPS origin outside loopback', () => {
    expect(() => loadPushConfig({ ORCA_PUSH_PUBLIC_URL: 'https://push.manta.sh.cn/v1' })).toThrow(
      'must be an origin'
    )
    expect(() => loadPushConfig({ ORCA_PUSH_PUBLIC_URL: 'http://push.manta.sh.cn' })).toThrow(
      'must use HTTPS'
    )
