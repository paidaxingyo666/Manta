# Manta mobile push gateway

`manta-cloud-push` is a public Cloud Run service in `onorca-cloud` that turns a desktop
...
| Setting | Value | Where |
| --- | --- | --- |
| Cloud Run service | `manta-cloud-push` | `push_cloud_run_service_name` |
...
| Hostname | `push.manta.sh.cn` | `push_base_url` |
...
| `ORCA_PUSH_DATABASE_URL` | Secret `manta-cloud-push-database-url`, version `latest` |
| `ORCA_PUSH_DATABASE_POOL_MAX` | `push_database_pool_max`, 2 per instance |
| `ORCA_PUSH_APNS_KEY` | Secret `manta-cloud-push-apns-key`, version `latest` |
| `ORCA_PUSH_APNS_KEY_ID` | Secret `manta-cloud-push-apns-key-id`, version `latest` |
| `ORCA_PUSH_APPLE_TEAM_ID` | Secret `manta-cloud-push-apple-team-id`, version `latest` |
...
terraform -chdir=infra/terraform import -var-file=environments/production.tfvars \
  'google_secret_manager_secret.push_provider["manta-cloud-push-apns-key"]' \
  projects/onorca-cloud/secrets/orca-cloud-push-apns-key

terraform -chdir=infra/terraform import -var-file=environments/production.tfvars \
  'google_secret_manager_secret.push_provider["manta-cloud-push-apns-key-id"]' \
  projects/onorca-cloud/secrets/orca-cloud-push-apns-key-id

terraform -chdir=infra/terraform import -var-file=environments/production.tfvars \
  'google_secret_manager_secret.push_provider["manta-cloud-push-apple-team-id"]' \
  projects/onorca-cloud/secrets/orca-cloud-push-apple-team-id

terraform -chdir=infra/terraform import -var-file=environments/production.tfvars \
  'google_secret_manager_secret_iam_member.push_provider_runtime_accessor["manta-cloud-push-apns-key"]' \
...
terraform -chdir=infra/terraform import -var-file=environments/production.tfvars \
  'google_secret_manager_secret_iam_member.push_provider_runtime_accessor["manta-cloud-push-apns-key-id"]' \
...
terraform -chdir=infra/terraform import -var-file=environments/production.tfvars \
  'google_secret_manager_secret_iam_member.push_provider_runtime_accessor["manta-cloud-push-apple-team-id"]' \
...
not only the push bindings: Artifact Registry writer on `manta-cloud`, `roles/run.developer` on
...
1. Builds `apps/push/Dockerfile` with the `cloud/` build context and pushes to the existing
   `manta-cloud` Artifact Registry repository as `push:sha-<commit>`, then resolves the digest.
...
9. Checks `https://push.manta.sh.cn/ready`, up to 30 times at five-second intervals, since the
...
```sh
gcloud run services update-traffic manta-cloud-push \
...
2. Add a version to each changed secret, without printing the value:
   ...
   ```sh
   gcloud secrets versions add manta-cloud-push-apns-key \
     --project onorca-cloud --data-file /path/to/AuthKey_NEW.p8
   printf '%s' '<new key id>' | gcloud secrets versions add manta-cloud-push-apns-key-id \
     --project onorca-cloud --data-file=-
   ```

   The team ID does not change, so `manta-cloud-push-apple-team-id` is untouched.
...
5. Only then revoke the old key in the Apple portal, and disable the superseded secret versions:
   ...
   gcloud secrets versions disable <old-version> \
     --project onorca-cloud --secret manta-cloud-push-apns-key
   ```
...
`manta.sh.cn` zone is not in this root: it is a Cloudflare zone whose Terraform-managed records
...
```text
push.manta.sh.cn.  CNAME  ghs.googlehosted.com.   (DNS only, not proxied)
```
...
Push uses the relay's schema-startup retry implementation through `@manta-cloud/postgres-schema`.
