# Manta mobile push gateway (`cloud/apps/push`).
#
...
resource "google_service_account" "push_runtime" {
  ...
  display_name = "Manta mobile push gateway"
  description  = "Runtime identity for the Manta mobile push gateway; sends through FCM V1."
}
...
# hand-managed Cloudflare CNAME to ghs.googlehosted.com, like relay.manta.sh.cn; this root has no
...
# Artifact Registry writer on `manta-cloud`, `roles/run.developer` on the relay director and the
