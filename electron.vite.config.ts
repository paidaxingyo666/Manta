...
// (MANTA_BUILD_IDENTITY='stable' | 'rc', MANTA_POSTHOG_WRITE_KEY=phc_...);
...
const mantaBuildIdentity = process.env.MANTA_BUILD_IDENTITY
const MANTA_BUILD_IDENTITY_LITERAL =
  mantaBuildIdentity === 'stable' || mantaBuildIdentity === 'rc'
    ? JSON.stringify(mantaBuildIdentity)
    : 'null'
const mantaPostHogWriteKey = process.env.MANTA_POSTHOG_WRITE_KEY
const MANTA_POSTHOG_WRITE_KEY_LITERAL =
  typeof mantaPostHogWriteKey === 'string' && mantaPostHogWriteKey.length > 0
    ? JSON.stringify(mantaPostHogWriteKey)
    : 'null'
const mantaDiagnosticsTokenUrl = process.env.MANTA_DIAGNOSTICS_TOKEN_URL
const MANTA_DIAGNOSTICS_TOKEN_URL_LITERAL =
  typeof mantaDiagnosticsTokenUrl === 'string' && mantaDiagnosticsTokenUrl.length > 0
    ? JSON.stringify(mantaDiagnosticsTokenUrl)
    : 'null'
...
;(() => {
  ...
  const mode = env?.MANTA_STARTUP_DIAGNOSTICS
  ...
  const diagnosticFile = env?.MANTA_STARTUP_DIAGNOSTICS_FILE
  ...
  if (!globalThis.__MANTA_BOOTSTRAP_EXIT_LOG_INSTALLED__) {
    globalThis.__MANTA_BOOTSTRAP_EXIT_LOG_INSTALLED__ = true
  ...
  if (mode === 'trace' && !globalThis.__MANTA_BOOTSTRAP_REQUIRE_TRACE_INSTALLED__) {
    globalThis.__MANTA_BOOTSTRAP_REQUIRE_TRACE_INSTALLED__ = true
    try {
      const Module = require('node:module')
      const originalLoad = Module._load
      const parsedTraceLimit = Number(env?.MANTA_STARTUP_DIAGNOSTICS_TRACE_LIMIT ?? 20000)
...
export const electronViteConfig: UserConfig = {
  main: {
    build: {
      ...
      rollupOptions: {
        ...
        input: {
          ...
          // this path for `manta agent hooks ...`, so it must survive rebuilds.
    ...
    define: {
      MANTA_BUILD_IDENTITY: MANTA_BUILD_IDENTITY_LITERAL,
      MANTA_POSTHOG_WRITE_KEY: MANTA_POSTHOG_WRITE_KEY_LITERAL,
      MANTA_DIAGNOSTICS_TOKEN_URL: MANTA_DIAGNOSTICS_TOKEN_URL_LITERAL
    },
