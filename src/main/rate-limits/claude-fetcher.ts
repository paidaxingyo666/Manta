...
async function readFromKeychain(configDir?: string): Promise<OAuthCredentialReadResult> {
  ...
  if (configDir) {
    ...
    // Why: a real access token beats refresh-only creds (Manta can't refresh), so a stale scoped item can't shadow a working legacy token.
...
function resolveOwnedWslClaudeManagedAuthPath(account: InactiveClaudeAccountInfo): string | null {
  ...
  if (
    !linuxPath.includes('/.local/share/manta/claude-accounts/') ||
    !linuxPath.endsWith(`/${account.id}/auth`)
  ) {
    return null
  }
  try {
    const markerPath = path.join(account.managedAuthPath, '.manta-managed-claude-auth')
    if (
