...
// returned an all-null parse, so every `*.manta.local` host was its own family; tldts applies the
// default single-label rule and stops at `manta.local`, which is what Chromium treats as registrable.
