...
const HIDE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  var g = window.__mantaGrab;
  if (g && g.host) g.host.style.display = 'none';
  document.querySelectorAll('[data-manta-browser-annotation-overlay]').forEach(function(el) {
    el.setAttribute('data-manta-previous-display', el.style.display || '');
...
const RESTORE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  ...
  document.querySelectorAll('[data-manta-browser-annotation-overlay]').forEach(function(el) {
    el.style.display = el.getAttribute('data-manta-previous-display') || '';
    el.removeAttribute('data-manta-previous-display');
  });
})()`
