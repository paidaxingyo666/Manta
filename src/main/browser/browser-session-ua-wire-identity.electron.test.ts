...
// identity that minted it, and an `Manta/x.y.z … Electron/x.y.z` UA is not one any browser sends —
...
// Why: production's UA carries an app token ("Manta/1.4.198") between the engine comment and
...
describe('browser session wire identity under Electron', () => {
  it('strips the Electron and app tokens for ordinary hosts and sends Firefox to Google auth hosts', async () => {
    ...
    expect(wireBrands.some(({ brand }) => /Electron|Manta/i.test(brand))).toBe(false)
