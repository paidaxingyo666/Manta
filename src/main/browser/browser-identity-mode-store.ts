 ...
 * phase used to mirror the *active Manta profile's* setting back into that record, so switching
...
export async function setBrowserIdentityMode(
  ...
  if (current.configuredMode === null) {
    // Why never automatic: the data may belong to a newer Manta, and overwriting it silently
        ...
        error: {
          ...
          message:
            current.state === 'future'
              ? 'Browser identity data was written by a newer Manta; update Manta, or reset it explicitly to overwrite it.'
