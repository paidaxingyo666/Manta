...
function isGuestCancellationPayload(rawPayload: unknown): boolean {
  ...
  if (payload.__mantaCancelled === true) {
    return true
  }
...
export class BrowserGrabSessionController {
  ...
  awaitGrabSelection(
    ...
    return new Promise<BrowserGrabResult>((resolve) => {
      ...
      // __mantaGrabResolve() which is wired by the 'awaitClick' script to
      ...
      const awaitGuestClick = async (): Promise<void> => {
        try {
          ...
          // Why: the guest wraps right-click results in { __mantaContextMenu, payload }
          // so the renderer can show the full action dropdown instead of auto-copying.
          const isContextMenu =
            '__mantaContextMenu' in (rawPayload as Record<string, unknown>) &&
            (rawPayload as Record<string, unknown>).__mantaContextMenu === true
