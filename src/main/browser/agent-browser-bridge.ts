...
function pageUnavailableMessageForSession(sessionName: string): string {
  const prefix = MANTA_TAB_SESSION_PREFIX
...
export class AgentBrowserBridge {
  ...
  async onPageClosed(browserPageId: string): Promise<void> {
    const sessionName = `${MANTA_TAB_SESSION_PREFIX}${browserPageId}`
        ...
        try {
          ...
          if (!this.getWebContents(target.webContentsId)) {
            throw this.createPageUnavailableError(
              `${MANTA_TAB_SESSION_PREFIX}${target.browserPageId}`
            )
          }
  ...
  async setViewport(
    ...
    return this.enqueueTargetedCommand(worktreeId, browserPageId, async (_sessionName, target) => {
      ...
      // Why: agent-browser's `set viewport` has no `mobile` flag, so apply the emulation directly via CDP to honor Manta's --mobile.
  ...
  async exec(command: string, worktreeId?: string, browserPageId?: string): Promise<unknown> {
    return this.enqueueTargetedCommand(worktreeId, browserPageId, async (sessionName) => {
      // Why: strip target/session flags from passthrough so a caller can't override Manta's selected page or CDP proxy.
  ...
  private async enqueueTargetedCommand<T>(
    ...
    const sessionName = `${MANTA_TAB_SESSION_PREFIX}${target.browserPageId}`
  ...
  private async execAgentBrowser(
    ...
    // Why: a restarted named daemon auto-launches Chrome unless every invocation reasserts Manta's CDP owner.
  ...
  private requireTargetWebContents(target: ResolvedBrowserCommandTarget): WebContents {
    const wc = this.getWebContents(target.webContentsId)
    if (!wc || wc.isDestroyed()) {
      throw this.createPageUnavailableError(`${MANTA_TAB_SESSION_PREFIX}${target.browserPageId}`)
    }
    return wc
  }
