...
describe('discoverCommitMessageModelsLocal', () => {
    ...
    listeners.get('stdout:data')?.(
      Buffer.from(
        `${JSON.stringify({
          ...
          response: {
            subtype: 'success',
            request_id: 'manta-model-discovery',
            response: {
              models: [
  ...
  it('falls back to the Claude seed models when the CLI lacks list_models', async () => {
    ...
    listeners.get('stdout:data')?.(
      Buffer.from(
        '{"type":"control_response","response":{"subtype":"error","request_id":"manta-model-discovery","error":"Unsupported control request subtype: list_models"}}\n'
      )
    )
    listeners.get('close')?.(0)
