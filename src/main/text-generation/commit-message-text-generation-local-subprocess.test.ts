...
describe('generateCommitMessageFromContext', () => {
  ...
  it('passes prepared provider environment to local agent subprocesses', async () => {
    ...
    const pending = generateCommitMessageFromContext(
      ...
      },
      {
        agentId: 'custom',
        model: '',
        customAgentCommand: 'manta-test-agent-nope'
      },
      {
        kind: 'local',
        cwd: '/repo',
  ...
  it('routes WSL local commit generation through the selected distro login shell', async () => {
    await withPlatform('win32', async () => {
      process.env.MANTA_HOST_ONLY_SECRET = 'do-not-leak'
      const listeners = new Map<string, (value: unknown) => void>()
      const child = {
        pid: 123,
        kill: vi.fn(),
      ...
      expect(spawnMock).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          // Manta's own cwd, a deletable WSL UNC path when it was launched from a
      ...
      expect(spawnEnv.MANTA_HOST_ONLY_SECRET).toBeUndefined()
  ...
  it('rejects unsafe argv prompts for Windows batch-script agent commands', async () => {
    await withPlatform('win32', async () => {
      ...
      expect(result).toEqual({
        success: false,
        error:
          'C:/tools/agent.cmd cannot be run as a Windows batch command with the prompt in argv. Remove {prompt} so Manta sends the prompt on stdin.'
      })
