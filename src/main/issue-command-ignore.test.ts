...
describe('issue command ignore rules', () => {
  ...
  it.each(['.manta', '.manta/', '/.manta/', '.manta/*', '.manta/issue-command'])(
    ...
    async (pattern) => {
      ...
      expect(readFileSync(join(repo, '.manta', 'issue-command'), 'utf8')).toBe('local command\n')
    }
  )

  it('does not create .gitignore when the repository exclude already ignores .manta', async () => {
    writeFileSync(join(repo, '.git', 'info', 'exclude'), '.manta/\n')
  ...
  it('respects anchored repository rules', async () => {
    writeFileSync(join(repo, '.gitignore'), '/.manta/\n')

    await writeIssueCommand(repo, 'local command')

    expect(readFileSync(join(repo, '.gitignore'), 'utf8')).toBe('/.manta/\n')
  })
  ...
  it('adds the rule once when .manta is not ignored', async () => {
    writeFileSync(join(repo, '.gitignore'), 'node_modules/')
    ...
    expect(readFileSync(join(repo, '.gitignore'), 'utf8')).toBe('node_modules/\n.manta\n')
    expect(readFileSync(join(repo, '.manta', 'issue-command'), 'utf8')).toBe('second command\n')
  })

  it('honors a repository rule that negates a global ignore', async () => {
    writeFileSync(join(root, 'ignore'), '.manta/\n')
    writeFileSync(join(repo, '.gitignore'), '!.manta/\n')
