...
const VERSION = '1.2.3'
const PAYLOAD = 'manta package payload'
...
function capture(overrides: Record<string, unknown> = {}): LinuxPackageArtifact | null {
  const downloadedFile = (overrides.downloadedFile ?? path.join(downloadDir, 'manta.deb')) as string
...
beforeEach(async () => {
  ...
  tempRoot = await fsp.mkdtemp(path.join(os.tmpdir(), 'manta-recovery-'))
  cacheRoot = path.join(tempRoot, 'cache')
  updaterDir = path.join(cacheRoot, 'manta-updater')
...
describe('captureLinuxPackageArtifact', () => {
  it('retains the downloaded package with the digest from the event metadata', () => {
    const artifact = {
      packageType: 'deb',
      version: VERSION,
      path: path.join(downloadDir, 'manta.deb'),
      sha512: SHA512
  ...
  it('requires an absolute downloaded path', () => {
    capture({ downloadedFile: 'manta.deb' })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })
  ...
  it('requires a string downloaded path', () => {
    recovery.captureLinuxPackageArtifact({
      downloadedFile: 123,
      version: VERSION,
      files: [{ url: 'manta.deb', sha512: SHA512 }]
    })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })

  it('requires the package extension to match the installed format', () => {
    capture({ downloadedFile: path.join(downloadDir, 'manta.rpm') })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })
  ...
  it('matches an absolute file URL by basename', () => {
    capture({
      files: [{ url: 'https://downloads.example.com/releases/1.2.3/manta.deb', sha512: SHA512 }]
    })
  ...
  it('matches a percent-encoded relative URL against the decoded basename', () => {
    const downloadedFile = path.join(downloadDir, 'Manta Setup 1.2.3.deb')
    capture({ downloadedFile, files: [{ url: 'Manta%20Setup%201.2.3.deb', sha512: SHA512 }] })
  ...
  it('ignores entries for other files and formats', () => {
    capture({
      files: [
        { url: 'manta.AppImage', sha512: 'other-appimage-digest' },
        { url: 'manta-arm64.deb', sha512: 'other-arch-digest' },
        { url: 'manta.deb', sha512: SHA512 }
      ]
    })
    expect(recovery.getTrackedLinuxPackageArtifact()?.sha512).toBe(SHA512)
  })
  ...
  it('accepts duplicate entries that agree on the digest', () => {
    capture({
      files: [
        { url: 'manta.deb', sha512: SHA512 },
        { url: 'https://downloads.example.com/manta.deb', sha512: SHA512 }
      ]
    })
    expect(recovery.getTrackedLinuxPackageArtifact()?.sha512).toBe(SHA512)
  })
  ...
  it('refuses an ambiguous basename with two different digests', () => {
    capture({
      files: [
        { url: 'manta.deb', sha512: SHA512 },
        { url: 'https://mirror.example.com/manta.deb', sha512: 'conflicting-digest' }
      ]
    })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })
  ...
  it('refuses an entry with a missing or non-string digest', () => {
    ...
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
    capture({ files: [{ url: 'manta.deb', sha512: 42 }] })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })
  ...
  it('refuses a digest that cannot decode to 64 bytes', () => {
    ...
    capture({
      files: [{ url: 'manta.deb', sha512: createHash('sha256').update(PAYLOAD).digest('base64') }]
    })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })
  ...
  it('refuses a digest with characters outside the base64 alphabet', () => {
    ...
    capture({ files: [{ url: 'manta.deb', sha512: `${SHA512.slice(0, 4)}!${SHA512.slice(4)}` }] })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })

  it('refuses a digest that is not base64 at all', () => {
    capture({ files: [{ url: 'manta.deb', sha512: 'not-a-digest' }] })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })

  it('keeps a retained artifact when a later event carries a malformed digest', () => {
    capture()
    expect(capture({ files: [{ url: 'manta.deb', sha512: 'not-a-digest' }] })).toBeNull()
    expect(recovery.getTrackedLinuxPackageArtifact()?.sha512).toBe(SHA512)
  })

  it('refuses a malformed percent-encoded URL', () => {
    capture({ files: [{ url: 'manta%ZZ.deb', sha512: SHA512 }] })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })
  ...
  it('refuses a missing or non-array file list', () => {
    ...
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
    capture({ files: { url: 'manta.deb', sha512: SHA512 } })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })
  ...
  it('replaces the retained artifact with a newer download', () => {
    ...
    capture({
      version: '1.2.4',
      downloadedFile: path.join(downloadDir, 'manta-next.deb'),
      files: [{ url: 'manta-next.deb', sha512: SHA512 }]
    })
    expect(recovery.getTrackedLinuxPackageArtifact()).toMatchObject({
      version: '1.2.4',
      path: path.join(downloadDir, 'manta-next.deb')
    })
  })
...
describePosix('digest validation', () => {
  ...
  it('rejects a well-formed digest that belongs to another file', async () => {
    ...
    capture({
      files: [
        { url: 'manta.deb', sha512: createHash('sha512').update('other payload').digest('base64') }
      ]
    })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: false,
...
describePosix('cache containment', () => {
  it('rejects a path that traverses out of the cache', async () => {
    const filePath = path.join(outsideDir, 'manta.deb')
    await fsp.writeFile(filePath, PAYLOAD)
    capture({ downloadedFile: path.join(downloadDir, '..', '..', '..', 'outside', 'manta.deb') })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: false,
      reason: 'not-regular'
    })
  ...
  it('rejects a directory in place of the package', async () => {
    await fsp.mkdir(path.join(downloadDir, 'manta.deb'))
    capture()
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: false,
      reason: 'not-regular'
  ...
  it('rejects a symlink to a real package inside the cache', async () => {
    ...
    const linkPath = path.join(downloadDir, 'manta.deb')
  ...
  it('rejects a package whose parent symlink escapes the cache', async () => {
    await fsp.writeFile(path.join(outsideDir, 'manta.deb'), PAYLOAD)
    await fsp.symlink(outsideDir, path.join(cacheRoot, 'escape'))
    capture({ downloadedFile: path.join(cacheRoot, 'escape', 'manta.deb') })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: false,
      reason: 'not-regular'
    })
  ...
  it('rejects a subdirectory of the pending directory', async () => {
    ...
    const filePath = path.join(nested, 'manta.deb')
    await fsp.writeFile(filePath, PAYLOAD)
    capture({ downloadedFile: filePath })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: false,
  ...
  it('uses the home cache when XDG_CACHE_HOME is unset', async () => {
    ...
    const homeDownloadDir = path.join(homeCache, '.cache', 'manta-updater', 'pending')
    await fsp.mkdir(homeDownloadDir, { recursive: true })
    const filePath = path.join(homeDownloadDir, 'manta.deb')
    await fsp.writeFile(filePath, PAYLOAD)
...
describePosix('pending-directory anchoring', () => {
  it('rejects a package in an attacker-owned directory under the cache root', async () => {
    ...
    const filePath = path.join(evilDir, 'manta.deb')
    await fsp.writeFile(filePath, PAYLOAD)
    capture({ downloadedFile: filePath })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: false,
  ...
  it('rejects a package in the updater directory itself', async () => {
    const filePath = path.join(updaterDir, 'manta.deb')
    await fsp.writeFile(filePath, PAYLOAD)
    capture({ downloadedFile: filePath })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: false,
  ...
  it('rejects a pending directory buried deeper than one level under the cache root', async () => {
    ...
    const filePath = path.join(deepPending, 'manta.deb')
    await fsp.writeFile(filePath, PAYLOAD)
    capture({ downloadedFile: filePath })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: false,
  ...
  it('rejects a pending directory that is a symlink out of the cache', async () => {
    await fsp.writeFile(path.join(outsideDir, 'manta.deb'), PAYLOAD)
    const fakeUpdaterDir = path.join(cacheRoot, 'manta-updater-2')
    await fsp.mkdir(fakeUpdaterDir)
    await fsp.symlink(outsideDir, path.join(fakeUpdaterDir, 'pending'))
    capture({ downloadedFile: path.join(fakeUpdaterDir, 'pending', 'manta.deb') })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: false,
      reason: 'not-regular'
    })
  ...
  it('accepts any updater directory name that holds the pending directory', async () => {
    ...
    const otherPending = path.join(cacheRoot, 'manta-updater-next', 'pending')
    await fsp.mkdir(otherPending, { recursive: true })
    const filePath = path.join(otherPending, 'manta.deb')
    await fsp.writeFile(filePath, PAYLOAD)
    capture({ downloadedFile: filePath })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: true,
  ...
  it('accepts a pending directory reached through a symlink that stays in the cache', async () => {
    ...
    capture({ downloadedFile: path.join(cacheRoot, 'link-to-updater', 'pending', 'manta.deb') })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: true,
      command: 'installed command',
      packageFileName: 'manta.deb'
...
describePosix('validation coalescing', () => {
  ...
  it('does not reuse an in-flight pass for a different artifact', async () => {
    await writePackage('manta.deb')
    await writePackage('manta-next.deb')
    capture()
