...
beforeEach(async () => {
  scratchDir = await mkdtemp(join(tmpdir(), 'manta-worktree-trash-'))
})
...
describe('sweepStaleWorktreeTrash', () => {
  it('removes leftover entries from both workspace layouts and nothing else', async () => {
    ...
    // Not generated by Manta: a stray directory and file inside the trash root must survive.
...
describe('collectWorktreeTrashSweepRoots', () => {
  const settings = { workspaceDir: '/home/dev/manta/workspaces', nestWorkspaces: true }

  function repo(overrides: Partial<Repo>): Repo {
    return { id: 'repo-1', path: '/home/dev/code/manta', ...overrides } as unknown as Repo
  }
  ...
  it('collects one root per local git repo', () => {
    ...
    ).toEqual(['/home/dev/manta/workspaces'])
  })
  ...
  it('skips WSL repos that cannot create host trash', () => {
    ...
    try {
      expect(
        collectWorktreeTrashSweepRoots(
          [
            repo({ path: '\\\\wsl.localhost\\Ubuntu\\home\\dev\\manta' }),
            repo({
              id: 'repo-2',
              path: 'C:\\code\\manta',
