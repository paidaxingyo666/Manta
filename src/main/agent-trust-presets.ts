 ...
 * first launch.
 *
 * Why: Manta's "drop URL into agent input as a draft" flow injects the URL
...
export function markCodexProjectTrusted(workspacePath: string): void {
  ...
  // Why: Manta-launched Codex runs with a Manta-owned CODEX_HOME, so the trust
  // preset must also update the runtime config Codex will actually read.
  upsertProjectTrustLevel(join(getMantaManagedCodexHomePath(), 'config.toml'), absPath, 'trusted')
}
...
function canonicalize(p: string): string {
  ...
  // (manta caches realpath()'d worktree paths) matches the agent's lookup.
  try {
    if (existsSync(p)) {
