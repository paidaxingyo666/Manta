...
function tempDataFile(): string {
  return join(realpathSync(mkdtempSync(join(tmpdir(), 'orca-alias-projection-'))), 'manta-data.json')
}
...
function buildFixture(): Fixture {
  ...
  // 3. No identity twin at all, and no hostId — the shape of Manta's synthetic pseudo-worktrees.
