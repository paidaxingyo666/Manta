...
function surface(input: {
  ...
  holdFails?: boolean
  clearFails?: boolean
  /** Manta refused to take the message at all. */
   ...
   * is what the real host does: it resolves as soon as Manta owns the message, before the provider
    ...
    restartResume: createStructuredAgentSessionRestartResume(
      ...
      sessions as never,
      {
        ...
        send: async ({ envelope, body }) => {
          ...
          // What the real send answers: Manta owns the message, the provider has not replied yet.
          return {
            ok: true,
            replayed: false,
            fence: 1,
...
describe('reporting what the continuation actually did', () => {
  // THE REGRESSION. A send resolves as soon as Manta owns the message, while its dispatch is still
  ...
  it('reports a send Manta could not hand off as refused and writes no note', async () => {
    const { restartResume, noted } = surface({
