 ...
 *
 * The send layer answers `ok: true` as soon as Manta OWNS the message — a rejected or unverifiable
...
export type StructuredAgentSessionContinuationDeps = {
  ...
  /** Records the host-authored journal note that marks this send as Manta's, not the user's. */
...
export async function continueStructuredAgentSessionAfterRestart(
  ...
  // The send result carries the dispatch as it stood when Manta took the message, which for a normal
  ...
  // Only an accepted dispatch gets the note: it is a durable claim that Manta asked this agent to
