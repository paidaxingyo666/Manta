 ...
 * not open, and `agent_session_ownership_unknown` tells them neither what Manta could not prove nor
...
function latchedMessage(record: AgentSessionRecord): string {
  ...
  if (record.lease.claimStatus === 'conflicted') {
    return owner
      ...
      : 'Two runtimes claimed this session and the record names no process to check. Quit any other Manta or agent process using this workspace, then reopen this chat.'
  }
  return owner
    ? `Manta cannot prove that ${ownerDescription(record)} — the previous owner of this session — has exited, so it will not start a second agent on the same conversation. Quit that process and reopen this chat.`
    : 'Manta cannot tell whether an agent started for this session before the app stopped, so it will not start a second one on the same conversation. Quit any leftover agent process for this workspace and reopen this chat.'
}
...
export function structuredAgentSessionRefusalMessage(
  ...
  if (code === 'execution_owner_reconciling') {
    return 'Manta is still working out who owns this session on this machine. Reopen the chat in a moment.'
  }
  return null
}
