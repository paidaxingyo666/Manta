...
describe('providerHistoryId', () => {
  it('uses the provider handle, never the Manta session id', () => {
...
describe('openAgentSessionJournalWithRecovery', () => {
  ...
  // Manta minted the submission, receipt and lifecycle identities; no provider
  ...
  it('rebuilds from provider history when the epoch row itself is gone', async () => {
    ...
    await journal.appendLifecycleBatch({
      ...
      mutations: [
        {
          kind: 'item',
          identity: { provider: 'manta', clientMessageId: 'approval-1' },
