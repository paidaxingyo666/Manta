...
describe('nativeChatTranscriptIncludesPath', () => {
  it('accepts a path in recent assistant output from the host-bound session', async () => {
    const readTranscript = vi.fn(async () => ({
      messages: [message('assistant', 'Open ~/manta-plans/result.html to review it.')]
    }))
    ...
    await expect(
      nativeChatTranscriptIncludesPath({
        ...
        pathText: '~/manta-plans/result.html',
        absolutePath: '/Users/ada/manta-plans/result.html',
        readTranscript
      })
    ).resolves.toBe(true)
  ...
  it('accepts a path followed by a sentence-final period', async () => {
    const readTranscript = vi.fn(async () => ({
      messages: [message('assistant', 'Open /tmp/manta-pr14166-external.txt.')]
    }))
    ...
    await expect(
      nativeChatTranscriptIncludesPath({
        tabs: [terminalTab()],
        context: { tabId: 'tab-1', sessionId: 'session-1' },
        pathText: '/tmp/manta-pr14166-external.txt',
        absolutePath: '/tmp/manta-pr14166-external.txt',
        readTranscript
      })
    ).resolves.toBe(true)
  })
  ...
  it('does not accept a longer filename sharing the requested path prefix', async () => {
    const readTranscript = vi.fn(async () => ({
      messages: [message('assistant', 'Open /tmp/manta-pr14166-external.txt.backup')]
    }))
