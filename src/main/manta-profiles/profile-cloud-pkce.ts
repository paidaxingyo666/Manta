...
import {
  MANTA_CLOUD_CALLBACK_RESPONSE_HEADERS,
  MANTA_CLOUD_CALLBACK_SUCCESS_PAGE
} from './profile-cloud-callback-page'

export type MantaCloudAuthorizationCode = {
  code: string
  codeVerifier: string
  nonce: string
  redirectUri: string
...
export function beginMantaCloudPkceFlow(
  config: MantaCloudAuthConfig,
  localProfileId: string
): Promise<MantaCloudAuthorizationCode> {
  ...
  return new Promise((resolve, reject) => {
    ...
    function writeInvalidCallback(response: ServerResponse): void {
      ...
      response.end('Invalid Manta sign-in response.')
    }
    ...
    const server = createServer((request, response) => {
      try {
        ...
        response.writeHead(200, MANTA_CLOUD_CALLBACK_RESPONSE_HEADERS)
        response.end(MANTA_CLOUD_CALLBACK_SUCCESS_PAGE)
        resolveFlow(code)
      } catch (error) {
        rejectFlow(error instanceof Error ? error : new Error('manta_cloud_auth_callback_failed'))
      }
    })

    const timeout = setTimeout(() => {
      rejectFlow(new Error('manta_cloud_auth_timeout'))
    }, AUTH_TIMEOUT_MS)
    ...
    server.listen(0, '127.0.0.1', () => {
      ...
      if (!address || typeof address === 'string') {
        rejectFlow(new Error('manta_cloud_auth_loopback_unavailable'))
        return
      }
      ...
      void shell.openExternal(authorizeUrl.toString()).catch((error) => {
        rejectFlow(
          error instanceof Error ? error : new Error('manta_cloud_auth_browser_open_failed')
        )
      })
    })
  })
