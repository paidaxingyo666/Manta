import type {
  MantaOrgMember,
  MantaOrgMembersRoster,
  MantaOrgPendingInvite,
  MantaOrgRole
...
const ORG_ROLES: readonly MantaOrgRole[] = ['owner', 'admin', 'member']

function isOrgRole(value: unknown): value is MantaOrgRole {
  return typeof value === 'string' && (ORG_ROLES as readonly string[]).includes(value)
}

function normalizeRole(value: unknown, fallback: MantaOrgRole): MantaOrgRole {
...
function normalizeMember(value: unknown): MantaOrgMember | null {
...
function normalizePendingInvite(value: unknown): MantaOrgPendingInvite | null {
  if (!value || typeof value !== 'object') {
    return null
  }
  const record = value as Record<string, unknown>
...
function normalizeRoster(value: unknown): MantaOrgMembersRoster {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_manta_org_members_roster')
  }
  const record = value as Record<string, unknown>
  const members = Array.isArray(record.members)
    ? record.members
        .map(normalizeMember)
        .filter((member): member is MantaOrgMember => member !== null)
    : []
  const pendingInvites = Array.isArray(record.pendingInvites)
    ? record.pendingInvites
        .map(normalizePendingInvite)
        .filter((invite): invite is MantaOrgPendingInvite => invite !== null)
    : []
  return {
    members,
    pendingInvites,
...
function orgMembersUrl(config: MantaCloudAuthConfig, orgId: string, path: string): string {
...
async function requestOrgMembers<T>(
  ...
  if (!response.ok) {
    throw new MantaCloudRequestError(response.status, await extractErrorCode(response))
  }
  return parse((await response.json()) as unknown)
}

export async function listMantaCloudOrgMembers(
  config: MantaCloudAuthConfig,
  session: MantaCloudSession,
  orgId: string
): Promise<MantaOrgMembersRoster> {
...
export async function inviteMantaCloudOrgMember(
  config: MantaCloudAuthConfig,
  session: MantaCloudSession,
  args: { orgId: string; email: string; role: MantaOrgRole }
): Promise<void> {
...
export async function revokeMantaCloudOrgInvite(
  config: MantaCloudAuthConfig,
  session: MantaCloudSession,
...
export async function changeMantaCloudOrgMemberRole(
  config: MantaCloudAuthConfig,
  session: MantaCloudSession,
  args: { orgId: string; userId: string; role: MantaOrgRole }
): Promise<void> {
  await requestOrgMembers(
...
export async function removeMantaCloudOrgMember(
  config: MantaCloudAuthConfig,
  session: MantaCloudSession,
