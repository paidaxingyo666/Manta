...
function profile(
  id: string,
  name: string,
  kind: MantaProfileKind = 'local'
): MantaProfileIndex['profiles'][number] {
  return {
    id,
    name,
...
function writeProfileState(profileId: string, repos: Repo[]): void {
  ...
  const dataFile = join(testState.dir, 'profiles', profileId, 'manta-data.json')
...
describe('profile project presence', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'manta-profile-presence-'))
    writeIndex()
  })
  ...
  it('finds matching projects in other profiles while excluding the active profile', async () => {
    writeProfileState('personal', [
      makeRepo({ id: 'personal-repo', path: 'C:\\Code\\Manta', displayName: 'Personal Manta' })
    ])
    writeProfileState('work', [
      makeRepo({ id: 'work-repo', path: 'C:\\Code\\Manta', displayName: 'Work Manta' })
    ])

    const { findMantaProfileProjectsByPath } = await loadPresenceModule()
    const result = findMantaProfileProjectsByPath(
      {
        path: 'c:/code/manta/',
  ...
  it('keeps SSH projects separate from local projects with the same path', async () => {
    writeProfileState('personal', [
      makeRepo({ id: 'local-repo', path: '/srv/manta', displayName: 'Local Manta' })
    ])
    writeProfileState('work', [
      makeRepo({
        id: 'ssh-repo',
        path: '/srv/manta',
        displayName: 'SSH Manta',
