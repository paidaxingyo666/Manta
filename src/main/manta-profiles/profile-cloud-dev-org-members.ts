...
function devSelf(): MantaOrgMember {
  return {
  ...
  return {
    members: [
      devSelf(),
      {
        ...
        email: 'teammate@manta.local',
        displayName: 'Dev Teammate',
        role: 'admin'
      },
      // Why: userId null exercises the "hasn't signed in to Manta yet" disabled row.
      { userId: null, email: 'invited-member@manta.local', displayName: undefined, role: 'member' }
    ],
    pendingInvites: [{ email: 'pending@manta.local', role: 'member', createdAt: Date.now() }]
  }
}
...
export function listDevMantaCloudOrgMembers(orgId: string): MantaOrgMembersRoster {
...
export function inviteDevMantaCloudOrgMember(
...
export function revokeDevMantaCloudOrgInvite(
...
export function changeDevMantaCloudOrgMemberRole(
...
export function removeDevMantaCloudOrgMember(
