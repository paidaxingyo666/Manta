...
import type { MantaProfileCloudSummary } from '../../shared/manta-profiles'
...
import { linkMantaProfileToCloud, unlinkMantaProfileFromCloud } from './profile-cloud-index'
import {
...
describe('profile artifact cloud cleanup', () => {
  it('preserves recovery state when the profile write fails', async () => {
    ...
    expect(() => linkMantaProfileToCloud(profileId, cloud('org-b'), userDataPath)).toThrow(
      'profile write failed'
    )
    expect(getArtifactCreateIntent(profileId, userDataPath, '/report.md', scope)).not.toBeNull()

    linkMantaProfileToCloud(profileId, cloud('org-b'), userDataPath)
  ...
  it('reconciles an interrupted transition before linking again', async () => {
    ...
    expect(() => linkMantaProfileToCloud(profileId, cloud('org-c'), userDataPath)).toThrow(
      'cleanup failed'
    )
    ...
    linkMantaProfileToCloud(profileId, cloud('org-c'), userDataPath)
  ...
  it('reconciles an interrupted transition before unlinking', async () => {
    ...
    expect(() => unlinkMantaProfileFromCloud(profileId, userDataPath)).toThrow('cleanup failed')
    expect(writeProfileIndex).not.toHaveBeenCalled()
    expect(currentCloud(userDataPath)?.activeOrgId).toBe('org-b')
    expect(getArtifactCreateIntent(profileId, userDataPath, '/report.md', scope)).not.toBeNull()

    unlinkMantaProfileFromCloud(profileId, userDataPath)
  ...
  it('preserves an orphaned local marker for an unknown profile', async () => {
    ...
    const scope = shareScope('org-a')
    mkdirSync(getMantaProfileDirectory(orphanProfileId, userDataPath), { recursive: true })
    createIntent(userDataPath, scope, orphanProfileId)
    prepareArtifactCloudCleanup(orphanProfileId, userDataPath, undefined)

    const transitions = [
      () => linkMantaProfileToCloud(orphanProfileId, cloud('org-b'), userDataPath),
      () => unlinkMantaProfileFromCloud(orphanProfileId, userDataPath)
    ]
    for (const transition of transitions) {
      expect(transition).toThrow('unknown_manta_profile')
...
function cloud(activeOrgId: string): MantaProfileCloudSummary {
  return {
...
function shareScope(cloudOrganizationId: string): ArtifactShareScope {
  return {
    ...
    cloudProfileId: 'cloud-profile-a',
    cloudOrganizationId,
    apiOrigin: 'https://share.manta.sh.cn'
  }
}

async function createLinkedProfile(cloudSummary: MantaProfileCloudSummary): Promise<string> {
  const userDataPath = await mkdtemp(join(tmpdir(), 'manta-profile-artifact-cleanup-'))
  createdPaths.push(userDataPath)
  loadOrCreateProfileIndex(userDataPath)
  linkMantaProfileToCloud(profileId, cloudSummary, userDataPath)
  vi.clearAllMocks()
  return userDataPath
}
...
function currentCloud(userDataPath: string): MantaProfileCloudSummary | undefined {
  return readProfileIndex(getMantaProfileIndexPath(userDataPath))?.profiles.find(
