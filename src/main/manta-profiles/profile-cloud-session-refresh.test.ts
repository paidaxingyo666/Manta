...
vi.mock('./profile-cloud-session-store', () => ({
  readMantaCloudSession: readMock,
  saveMantaCloudSessionIfCurrent: saveIfCurrentMock,
  clearMantaCloudSession: clearMock
}))
...
vi.mock('./profile-cloud-client', async (importOriginal) => {
  ...
  return { ...original, refreshMantaCloudSession: refreshMock }
})

vi.mock('./profile-cloud-index', () => ({ linkMantaProfileToCloud: linkMock }))

import { readFreshMantaCloudSession } from './profile-cloud-session-refresh'
...
const config = {} as MantaCloudAuthConfig
const active = {
  profile: {
    id: 'profile-1',
    cloud: {
...
describe('profile cloud session refresh', () => {
  ...
  it('does not publish a refresh whose persistent mutation snapshot became stale', async () => {
    ...
    const refreshing = readFreshMantaCloudSession(config, active, '/data')
  ...
  it('single-flights concurrent rotating refresh-token use per profile and store', async () => {
    ...
    const first = readFreshMantaCloudSession(config, active, '/data')
    const second = readFreshMantaCloudSession(config, active, '/data')
  ...
  it('notifies subscribers when an auth failure clears the stored session', async () => {
    ...
    refreshMock.mockRejectedValue(new MantaCloudRequestError(401))

    await expect(readFreshMantaCloudSession(config, active, '/data')).resolves.toEqual({
...
describe('refresh-token replay after an ambiguous attempt', () => {
  ...
  it('never resends a refresh token whose attempt timed out', async () => {
    ...
    await expect(readFreshMantaCloudSession(config, active, '/data')).rejects.toThrow(
    ...
    await expect(readFreshMantaCloudSession(config, active, '/data')).rejects.toThrow(
      'manta_cloud_refresh_replay_blocked'
    )
    expect(refreshMock).toHaveBeenCalledTimes(1)
    expect(clearMock).not.toHaveBeenCalled()
  })
  ...
  it('retries once after a definitive 5xx, which cannot have rotated the token', async () => {
    refreshMock
      .mockRejectedValueOnce(new MantaCloudRequestError(503))
      .mockResolvedValueOnce(rotatedResponse)

    const result = await readFreshMantaCloudSession(config, active, '/data')
  ...
  it('gives a definitive 5xx exactly one retry', async () => {
    refreshMock.mockRejectedValue(new MantaCloudRequestError(503))

    await expect(readFreshMantaCloudSession(config, active, '/data')).rejects.toThrow(
      'manta_cloud_request_failed_503'
    )
    expect(refreshMock).toHaveBeenCalledTimes(2)
    expect(clearMock).not.toHaveBeenCalled()
  })
  ...
  it('marks a 401 that follows an ambiguous attempt as a possible self-replay', async () => {
    ...
    refreshMock.mockRejectedValueOnce(new MantaCloudRequestError(401))
    ...
    expect(refreshMock).toHaveBeenCalledTimes(2)
    expect(warn.mock.calls.flat().join(' ')).toContain('manta_cloud_refresh_possible_replay')
    expect(clearMock).toHaveBeenCalledTimes(1)
    expect(invalidated).toHaveBeenCalledTimes(1)
    unsubscribe()
  })
  ...
  it('does not mark a 401 that follows no ambiguous attempt', async () => {
    ...
    expect(warn.mock.calls.flat().join(' ')).not.toContain('manta_cloud_refresh_possible_replay')
    expect(clearMock).toHaveBeenCalledTimes(1)
  })
})
