...
export async function selectCloudOrgWithMutationFence(input: {
  ...
  userDataPath: string
  orgId: string
}): Promise<ReturnType<typeof linkMantaProfileToCloud> | null> {
  const cloud = input.active.profile.cloud
  const stored = readMantaCloudSession(input.active.profile.id, input.userDataPath)
  ...
  let workingSession: MantaCloudSession = stored.session
  try {
    let selected
    try {
      selected = await selectMantaCloudOrg(input.config, workingSession, input.orgId)
    } catch (error) {
      if (!(error instanceof MantaCloudRequestError) || error.statusCode !== 401) {
        throw error
      }
      const refreshed = await refreshMantaCloudSession(input.config, workingSession)
      if (
        refreshed.cloud.userId !== cloud.userId ||
        refreshed.cloud.cloudProfileId !== cloud.cloudProfileId
      ) {
        throw new Error('manta_cloud_identity_changed_during_org_selection')
      }
      workingSession = {
    ...
    if (
      ...
      throw new Error('manta_cloud_org_selection_identity_mismatch')
    }
    const nextSession: MantaCloudSession = {
      ...workingSession,
    ...
    const list = linkMantaProfileToCloud(input.active.profile.id, selected.cloud, input.userDataPath)
    return list
  } catch (error) {
