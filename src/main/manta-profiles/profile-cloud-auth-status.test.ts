...
import type { MantaCloudSessionReadResult } from './profile-cloud-session-store'
import { getMantaProfileAuthStatusFromProfile } from './profile-cloud-auth-status'

const { readSession, configuration } = vi.hoisted(() => ({
  readSession: vi.fn<() => MantaCloudSessionReadResult>(),
  configuration: { configured: true }
}))

vi.mock('./profile-cloud-session-store', () => ({ readMantaCloudSession: readSession }))
vi.mock('./profile-cloud-auth-config', () => ({
  getMantaCloudAuthConfig: () => configuration,
  isMantaCloudDevAuthEnabled: () => false
}))

function activeProfile(linked: boolean): ActiveMantaProfileState {
  const profile: ActiveMantaProfileState['profile'] = {
    id: 'profile-1',
    name: 'Personal',
...
const absentSessions: MantaCloudSessionReadResult[] = [
...
describe('unexpected sign-out auth evidence', () => {
  ...
  it.each(absentSessions)('requires a preserved cloud link for $status credentials', (session) => {
    ...
    expect(getMantaProfileAuthStatusFromProfile(linked, '')).toMatchObject({
    ...
    const signedOut = getMantaProfileAuthStatusFromProfile(activeProfile(false), '')
  ...
  it.each(absentSessions)(
    ...
    (session) => {
      ...
      readSession.mockReturnValue(session)
      expect(getMantaProfileAuthStatusFromProfile(activeProfile(true), '').state).toBe(
        'unconfigured'
      )
      expect(getMantaProfileAuthStatusFromProfile(activeProfile(false), '').state).toBe(
        'unconfigured'
      )
    }
  )
  ...
  it('treats a live memory-only session as connected, then reconnects after its loss', () => {
    ...
    expect(getMantaProfileAuthStatusFromProfile(linked, '').state).toBe('reconnect-required')
  })
})
