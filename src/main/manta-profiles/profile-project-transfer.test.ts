...
function profile(id: string, name: string): MantaProfileIndex['profiles'][number] {
  return {
    id,
    name,
...
function profileDataPath(profileId: string): string {
  return join(testState.dir, 'profiles', profileId, 'manta-data.json')
}
...
describe('profile project transfer', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'manta-profile-transfer-'))
    writeIndex()
  })
  ...
  it('copies a project into another profile with a new repo id and re-keyed metadata', async () => {
    const sourceWorktreeId = 'repo-1::/workspace/manta-feature'
    writeProfileState(
      'personal',
      makeState({
        repos: [makeRepo()],
        ...
        retiredWorktreeNamesByNamespace: {
          'ssh:ssh-1:/workspace/manta-nautilus': { exhaustedTiers: 0, names: ['seahorse'] }
        },
        worktreeMeta: {
    ...
    const { transferMantaProfileProject } = await loadTransferModule()
    const result = transferMantaProfileProject(
      {
    ...
    const targetWorktreeId = `${targetRepoId}::/workspace/manta-feature`
    expect(target.repos).toEqual([
      expect.objectContaining({ id: targetRepoId, path: '/workspace/manta' })
    ])
  ...
  it('moves a project, preserving SSH identity and restorable workspace session state', async () => {
    const sourceWorktreeId = 'repo-ssh::/srv/manta-feature'
    ...
    writeProfileState(
      ...
      makeState({
        ...
        retiredWorktreeNamesByNamespace: {
          'ssh:ssh-1:posix:/srv/manta-manta-retirement-probe': {
            exhaustedTiers: 0,
            names: ['seahorse']
          }
        },
            ...
            [sourceWorktreeId]: [
              {
                ...
                sessionPartition: 'persist:manta-profile-personal-deadbeef-browser-default',
    ...
    expect(target.retiredWorktreeNamesByNamespace).toEqual({
      'ssh:builder.example.com|22|dev:posix:/srv/manta-manta-retirement-probe': {
        exhaustedTiers: 0,
        names: ['seahorse']
      }
    })
  ...
  it('rejects a duplicate physical project inside the target profile', async () => {
    writeProfileState(
      'personal',
      makeState({
        repos: [makeRepo({ path: 'C:\\Work\\Manta\\' })]
      })
    )
    writeProfileState(
      'work',
      makeState({
        repos: [makeRepo({ id: 'repo-existing', path: 'c:/work/manta' })]
      })
    )
