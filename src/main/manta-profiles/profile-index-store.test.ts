...
describe('profile index store', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'manta-profile-test-'))
  ...
  it('creates the default local profile and copies legacy state without deleting it', async () => {
    ...
    writeFileSync(join(testState.dir, 'manta-data.json'), JSON.stringify(legacyState), 'utf-8')
    writeFileSync(
      join(testState.dir, 'manta-data.json.bak.0'),
    ...
    const { ensureActiveMantaProfile, getMantaProfileIndexPath } = await loadProfileIndexStore()
    const activeProfile = ensureActiveMantaProfile()

    expect(activeProfile.profile.id).toBe(DEFAULT_LOCAL_MANTA_PROFILE_ID)
    expect(activeProfile.profile.name).toBe(DEFAULT_LOCAL_MANTA_PROFILE_NAME)
    expect(activeProfile.dataFile).toBe(
      join(testState.dir, 'profiles', DEFAULT_LOCAL_MANTA_PROFILE_ID, 'manta-data.json')
    )
    ...
    expect(
      readJson(
        join(testState.dir, 'profiles', DEFAULT_LOCAL_MANTA_PROFILE_ID, 'browser-session-meta.json')
      )
    ).toEqual(legacyBrowserSessionMeta)
    expect(existsSync(join(testState.dir, 'manta-data.json'))).toBe(true)

    expect(readJson(getMantaProfileIndexPath())).toMatchObject({
      schemaVersion: MANTA_PROFILE_INDEX_SCHEMA_VERSION,
      activeProfileId: DEFAULT_LOCAL_MANTA_PROFILE_ID,
      profiles: [expect.objectContaining({ id: DEFAULT_LOCAL_MANTA_PROFILE_ID, kind: 'local' })]
    })
  })
  ...
  it('uses an existing active profile data file without overwriting it from legacy state', async () => {
    ...
    writeFileSync(join(profileDirectory, 'manta-data.json'), JSON.stringify(profileData), 'utf-8')
    writeFileSync(
      join(testState.dir, 'manta-data.json'),
      JSON.stringify({ schemaVersion: 1, repos: [{ id: 'legacy-repo' }] }),
      'utf-8'
    )
    const index: MantaProfileIndex = {
      schemaVersion: MANTA_PROFILE_INDEX_SCHEMA_VERSION,
    ...
    writeFileSync(join(testState.dir, 'manta-profile-index.json'), JSON.stringify(index), 'utf-8')

    const { ensureActiveMantaProfile } = await loadProfileIndexStore()
    const activeProfile = ensureActiveMantaProfile()

    expect(activeProfile.profile.id).toBe(profileId)
    expect(activeProfile.dataFile).toBe(join(profileDirectory, 'manta-data.json'))
  ...
  it('creates an empty local profile without copying legacy state into it', async () => {
    ...
    const { createLocalMantaProfile, getMantaProfileDataFile, getMantaProfileListState } =
      await loadProfileIndexStore()
    const created = createLocalMantaProfile({ name: ' Work ' })

    expect(created.profile.name).toBe('Work')
    expect(created.profile.id).toMatch(/^local-/)
    expect(created.activeProfileId).toBe(DEFAULT_LOCAL_MANTA_PROFILE_ID)
    expect(created.profiles.map((profile) => profile.id)).toContain(created.profile.id)
    expect(existsSync(getMantaProfileDataFile(created.profile.id))).toBe(false)
    expect(getMantaProfileListState().profiles.map((profile) => profile.id)).toContain(
      created.profile.id
    )
  })

  it('switches the active profile and updates last-opened metadata', async () => {
    const { createLocalMantaProfile, setActiveMantaProfile } = await loadProfileIndexStore()
    const created = createLocalMantaProfile({ name: 'Work' })

    const switched = setActiveMantaProfile(created.profile.id)
  ...
  it('rejects switching to an unknown profile', async () => {
    const { setActiveMantaProfile } = await loadProfileIndexStore()

    expect(() => setActiveMantaProfile('missing-profile')).toThrow('unknown_manta_profile')
  })
  ...
  posixIt('writes a fresh profile index when umask removes owner-write permission', async () => {
    ...
    const indexPath = store.getMantaProfileIndexPath()
    const profile = createDefaultLocalMantaProfile(1)
    const index: MantaProfileIndex = {
      schemaVersion: MANTA_PROFILE_INDEX_SCHEMA_VERSION,
  ...
  it('recovers a corrupted profile index from the backup copy', async () => {
    const store = await loadProfileIndexStore()
    store.ensureActiveMantaProfile()
    const created = store.createLocalMantaProfile({ name: 'Work' })
    // Trigger one more write so the backup captures the two-profile index.
    store.setActiveMantaProfile(created.profile.id)

    const indexPath = store.getMantaProfileIndexPath()
    expect(existsSync(`${indexPath}.bak`)).toBe(true)
    writeFileSync(indexPath, '{ not json', 'utf-8')

    const recovered = store.getMantaProfileListState()
  ...
  it('rejects profile ids that are not safe path segments', async () => {
    ...
    const state = store.ensureActiveMantaProfile()
    expect(state.profile.id).toBe(DEFAULT_LOCAL_MANTA_PROFILE_ID)
  })
})
