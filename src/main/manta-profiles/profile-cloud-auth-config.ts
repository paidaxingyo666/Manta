...
const PRODUCTION_PUSH_GATEWAY_URL = 'https://push.manta.sh.cn'
...
export function getMantaCloudAuthConfig(
...
): { configured: true; config: MantaCloudAuthConfig } | { configured: false; setupMessage: string } {
  ...
  const configuredApiBaseUrl = env.MANTA_CLOUD_API_URL?.trim()
  ...
  const apiBaseUrl = configuredApiBaseUrl
    ...
    : packaged
      ...
      : null
  const clientId = env.MANTA_CLOUD_CLIENT_ID?.trim() || (packaged ? PRODUCTION_CLIENT_ID : undefined)
  ...
  const authBaseUrl = cleanEndpointUrl(env.MANTA_CLOUD_AUTH_URL) ?? apiBaseUrl
  return {
    configured: true,
    config: {
      apiBaseUrl,
      ...
      relayDirectorUrl:
        cleanOrigin(env.MANTA_RELAY_URL, allowLoopbackHttp) ?? PRODUCTION_RELAY_DIRECTOR_URL,
      clientId,
