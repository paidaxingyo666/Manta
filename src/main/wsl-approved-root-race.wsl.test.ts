...
const runRealWsl = process.platform === 'win32' && process.env.MANTA_REAL_WSL_DELETE_TEST === '1'
...
async function wsl(command: string, ...args: string[]): Promise<string> {
  const result = await execFileAsync(
    'wsl.exe',
    ['-d', DISTRO, '--exec', 'sh', '-c', command, 'manta-wsl-test', ...args],
...
describe.skipIf(!runRealWsl)('WSL approved-root traversal race', () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await wsl("mktemp -d -p /tmp 'manta-wsl-root-race.XXXXXX'")
    const statHook = String.raw`#!/bin/sh
for argument do last=$argument; done
if [ "$PWD" = "$MANTA_RACE_PARENT" ] && [ "$(/usr/bin/basename "$last")" = vault ]; then
  inspected=$(/usr/bin/stat "$@") || exit $?
  /usr/bin/mv -- "$MANTA_RACE_PARENT/vault" "$MANTA_RACE_PARENT/vault-original" || exit $?
  /usr/bin/ln -s -- "$MANTA_RACE_OUTSIDE" "$MANTA_RACE_PARENT/vault" || exit $?
  printf '%s\n' "$inspected"
  exit 0
fi
exec /usr/bin/stat "$@"
`
  ...
  afterAll(async () => {
    if (/^\/tmp\/manta-wsl-root-race\.[A-Za-z0-9]+$/u.test(fixtureRoot)) {
    ...
    await expect(
      execFileAsync(
        ...
        [
          ...
          `MANTA_RACE_PARENT=${fixtureRoot}/race`,
          `MANTA_RACE_OUTSIDE=${fixtureRoot}/outside/root-target`,
          ...(command ?? [])
        ],
        { encoding: 'utf-8', timeout: 30000 }
      )
    ).rejects.toMatchObject({ stderr: expect.stringContaining('MANTA_WSL_DELETE_REJECT:race') })
