...
const DEB_PATH = '/home/tester/.cache/manta-updater/pending/manta-ide_1.0.61_amd64.deb'
...
function downloadedEvent(overrides?: Record<string, unknown>): Record<string, unknown> {
  return {
    ...
    files: [{ url: 'manta-ide_1.0.61_amd64.deb', sha512: DEB_SHA512 }],
    ...overrides
  }
}
...
describe('registerAutoUpdaterHandlers linux package artifact tracking', () => {
  ...
  it.each(['deb', 'rpm'] as const)(
    ...
    async (packageType) => {
      ...
      const fileName = packageType === 'deb' ? 'manta.deb' : 'manta.rpm'

      emit(
        'update-downloaded',
        downloadedEvent({
          downloadedFile: `/home/tester/.cache/manta-updater/pending/${fileName}`,
  ...
  it.each([
    ['missing', [{ url: 'manta-ide_1.0.61_amd64.deb' }]],
    ['malformed', [{ url: 'manta-ide_1.0.61_amd64.deb', sha512: 'not-a-digest' }]]
  ...
  it('blocks downloaded-state handling when the packaged marker is unusable', async () => {
    ...
    expect(context.sendStatus).toHaveBeenLastCalledWith({
      state: 'error',
      message:
        'Manta could not verify the installed Linux package format, so it will not install this update automatically. Download the update from the official release page and install it manually.',
      version: '1.0.61',
      retryable: false
    })
    expect(getArtifact()).toBeNull()
