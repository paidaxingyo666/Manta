...
const POSIX_HOOK_JSON_STDIN_PYTHON_VAR = 'manta_hook_json_stdin_py'
...
export function buildPosixHookSpoolLines(source: string, eventNameVar?: string): string[] {
  ...
  const spoolRecordLine = "  { printf '\\n{".concat(
    ...
    eventArg,
    ' "$(spool_json_escape "${MANTA_PANE_KEY:-}")" "$(spool_json_escape "${MANTA_TAB_ID:-}")" "$(spool_json_escape "${MANTA_WORKTREE_ID:-}")" "$(spool_json_escape "${MANTA_AGENT_HOOK_ENV:-}")" "$(spool_json_escape "${MANTA_AGENT_HOOK_VERSION:-}")" "$(spool_json_escape "${MANTA_AGENT_LAUNCH_TOKEN:-}")" "$(spool_json_escape "',
    source,
  ...
  return [
    ...
    '  [ -n "${MANTA_AGENT_HOOK_ENDPOINT:-}" ] || return 0',
    // Why: an endpoint can linger in a parent shell after leaving Manta; without a pane key
    // the record is un-attributable and would accumulate as pane-unknown.jsonl.
    '  [ -n "${MANTA_PANE_KEY:-}" ] || return 0',
    // Why: a stale env var must not create a spool tree for a Manta that is not installed here.
    '  [ -r "$MANTA_AGENT_HOOK_ENDPOINT" ] || return 0',
    '  spool_base=${MANTA_AGENT_HOOK_ENDPOINT%/*}',
    '  spool_dir="$spool_base/spool"',
    '  mkdir -p "$spool_dir" 2>/dev/null || return 0',
    '  chmod 700 "$spool_dir" 2>/dev/null || :',
    "  spool_id=$(printf %s \"${MANTA_PANE_KEY:-unknown}\" | tail -c 36 | tr '/:' '__')",
...
export const WINDOWS_HOOK_STDIN_DRAIN_LABEL = 'manta_agent_hook_drain_stdin'
