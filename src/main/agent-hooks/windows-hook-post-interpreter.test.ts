...
describe('Windows managed hook post interpreter', () => {
  ...
  beforeEach(() => {
    ...
    isolatedUserDataDir = mkdtempSync(join(tmpdir(), 'manta-hook-interpreter-user-data-'))
    // Why: Manta-managed Codex hooks resolve through MANTA_USER_DATA_PATH before the mocked
    ...
    home = mkdtempSync(join(tmpdir(), 'manta-hook-interpreter-'))
  ...
  it('posts through curl.exe from every managed batch script, spawning no interpreter', async () => {
    ...
    const posts = (body: string): boolean => body.includes('127.0.0.1:%MANTA_AGENT_HOOK_PORT%')
