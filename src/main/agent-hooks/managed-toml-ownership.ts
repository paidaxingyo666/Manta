// Manta appends marker-delimited blocks to user-owned TOML config files. Two
// independent things can make a byte Manta's: it sits between a matched start and
// end marker, or the provider positively recognizes it as content Manta emitted.
...
// its own stray marker line, and anything Manta actually wrote is reclaimed by
...
export type ManagedTomlBlockRegion = ManagedTomlRegion & {
  ...
  markerOffset: number
  /** End marker found: everything between the markers is Manta's. */
  terminated: boolean
}
...
export function findManagedTomlBlocks(
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    // the stray marker. Its body, if Manta wrote it, is reclaimed by recognition.
    regions.push({
 ...
 * position is irrelevant: content Manta emitted is Manta's to remove even when a
