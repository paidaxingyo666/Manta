...
export abstract class AgentHookServerStatusApplication extends AgentHookServerStatusDisposition {
  ...
  noteSessionContinued(
    ...
    const sessionId = next.sessionId.trim()
    if (!from || !sessionId || from === sessionId) {
      return
    }
    let changed = false
    for (const entry of this.state.lastStatusByPaneKey.values()) {
      // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: Main admits enriched legacy rows; the shared view exposes their base type.
      const previous = entry as EnrichedAgentHookEventPayload
      if (previous.providerSession?.id !== from) {
        continue
      }
      const continued: EnrichedAgentHookEventPayload = {
        ...previous,
        providerSession: {
      ...
      if (!this.writeLegacyStatusRow(continued)) {
        continue
      }
      this.commitStatusRowMutation(previous, continued)
      changed = true
    }
    if (changed) {
