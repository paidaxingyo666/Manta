    ...
    async () => {
      ...
      try {
        ...
        for (const fileName of mainScripts) {
          ...
          // Manta env is missing, so the writer may break — EPIPE, or ECONNRESET when Windows
          // tears the pipe down first. hookEnvironment() strips every MANTA_* var, so this
