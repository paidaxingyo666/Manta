...
// missing-Manta-env path, so their writer may break there.
...
beforeEach(() => {
  ...
  isolatedUserDataDir = mkdtempSync(join(tmpdir(), 'manta-hook-stdin-user-data-'))
  // Why: Manta-managed Codex hooks resolve through MANTA_USER_DATA_PATH before
  // the mocked home; an inherited live path would let this test rewrite them.
  process.env.MANTA_USER_DATA_PATH = isolatedUserDataDir
})
...
function runHookProcess(
  ...
  // Why: `abandon` leaves the pipe open and unwritten — the shape a caller outside a Manta
...
function hookEnvironment(extraEnv: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  const env = Object.fromEntries(
    Object.entries(process.env).filter(([key]) => !key.startsWith('MANTA_'))
  )
  return {
    ...env,
    HOME: REMOTE_HOME,
...
async function generatePosixScripts(): Promise<Map<string, string>> {
  ...
  for (const entry of REMOTE_INSTALLERS) {
    ...
    const generated = [...memory.fs.files.entries()].filter(
      ([path]) => path.includes('/.manta/agent-hooks/') && path.endsWith('.sh')
    )
...
describe('Windows managed hook stdin structure', () => {
  it('exits immediately when Manta env is missing and keeps drain for other failures', async () => {
    const home = mkdtempSync(join(tmpdir(), 'manta-hook-stdin-windows-'))
    ...
    try {
      ...
      const hooksDir = join(home, '.manta', 'agent-hooks')
      ...
      for (const fileName of mainBatchScripts) {
        ...
        expect(script, `${fileName} pane guard`).toContain('if "%MANTA_PANE_KEY%"=="" exit /b 0')
        // Why: pin the rule, not today's three guards — a fourth MANTA_* guard routed to the
        ...
        // MANTA_* var, so it stays exempt.
        expect(script, `${fileName} no MANTA_* guard may route to the more.com drain`).not.toMatch(
          /MANTA_[A-Z_]+.*goto :?manta_agent_hook_drain_stdin/
        )
      ...
      // must sit below the env guards — otherwise a Devin session outside a Manta pane still
      ...
      expect(claude, 'claude devin guard present').toContain(
        'if not "%DEVIN_PROJECT_DIR%"=="" goto :manta_agent_hook_drain_stdin'
      )
      expect(claude.indexOf('if "%MANTA_PANE_KEY%"=="" exit /b 0')).toBeLessThan(
        claude.indexOf('if not "%DEVIN_PROJECT_DIR%"=="" goto :manta_agent_hook_drain_stdin')
      )
      ...
      expect(copilot.indexOf('if (-not $env:MANTA_AGENT_HOOK_PORT')).toBeGreaterThan(-1)
      expect(copilot.indexOf('if (-not $env:MANTA_AGENT_HOOK_PORT')).toBeLessThan(
      ...
      for (const [name, command] of [
        [
          ...
          wrapWindowsHookCommand('C:\\missing\\manta-hook.cmd', {}, { fallbackStdout: '{}' })
        ],
        [
          'wrapRuntimeHomeHookCommand',
          wrapRuntimeHomeHookCommand('missing-manta-hook', { neutralJsonWhenMissing: true })
        ]
      ] as const) {
      ...
      expect(kimi.indexOf('if [ -z "$MANTA_AGENT_HOOK_PORT" ]')).toBeGreaterThan(-1)
      expect(kimi.indexOf('if [ -z "$MANTA_AGENT_HOOK_PORT" ]')).toBeLessThan(
  ...
  it.skipIf(process.platform !== 'win32')(
    'exits 0 for every local script and missing-script launcher, dropping stdin only without Manta env',
    async () => {
      const home = mkdtempSync(join(tmpdir(), 'manta-hook-stdin-windows-live-'))
      homedirMock.mockReturnValue(home)
      seedCmdAutoRunTarget(home)
      try {
        ...
        for (const fileName of mainScripts) {
          ...
          // Manta env is missing, so the writer may break. hookEnvironment() strips every
          // MANTA_* var, so this relaxation only ever covers the missing-env path — a
        ...
        const missingScript = 'C:\\missing\\manta-hook.cmd'
        ...
        const launcherCases = [
          ...
          },
          {
            ...
            executable: gitBash,
            args: ['-lc', wrapRuntimeHomeHookCommand('missing-manta-hook')]
          }
        ]
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    async () => {
      const home = mkdtempSync(join(tmpdir(), 'manta-hook-stdout-json-'))
      homedirMock.mockReturnValue(home)
      ...
      try {
        ...
        const environments = [
          { name: 'no Manta env', env: hookEnvironment({ USERPROFILE: home }) },
          {
            name: 'Manta env with dead listener',
            env: hookEnvironment({
              USERPROFILE: home,
              MANTA_AGENT_HOOK_PORT: '59999',
              MANTA_AGENT_HOOK_TOKEN: 'token',
...
describe.skipIf(process.platform === 'win32')('managed hook stdin lifecycle', () => {
  it('emits neutral JSON when the Claude lifecycle script is missing', async () => {
    const command = getRemoteManagedCommand('/home/dev/.manta/agent-hooks/claude-hook.sh')
  ...
  it('accepts a large payload without Manta environment or a broken writer', async () => {
    const scripts = await generatePosixScripts()
    for (const [agent, script] of scripts) {
      const extraEnv = agent.startsWith('command-code')
        ? {
            MANTA_AGENT_HOOK_PORT: '1',
            MANTA_AGENT_HOOK_TOKEN: 'test-token',
            MANTA_PANE_KEY: 'test-pane'
          }
        : {}
  ...
  it('does not need PATH to capture or drain POSIX hook stdin', async () => {
    ...
    const missing = await runPosixHook(wrapPosixHookCommand('/missing/manta-hook.sh'), { PATH: '' })
  ...
  it.each([
    ...
    const decoyDir = mkdtempSync(join(tmpdir(), 'manta-hook-stdin-decoy-'))
    try {
  ...
  it('drains a large payload when the configured script is missing', async () => {
    const result = await runPosixHook(wrapPosixHookCommand('/missing/manta-hook.sh'))
    expect(result.exitCode).toBe(0)
    expect(result.stdinErrors).toHaveLength(0)
  })
})
