...
export function resolveWslHookRelayBundle(): WslHookRelayBundle | null {
  ...
  if (process.env.MANTA_RELAY_PATH) {
    candidates.push(join(process.env.MANTA_RELAY_PATH, 'wsl'))
  }
...
// Why: the install dir is namespaced by bundle version so concurrent Manta
...
export function buildGuestInstallScript(bundleJs: Buffer, version: string): string {
  ...
  return [
    ...
    'mkdir -p "$d"',
    `base64 -d > "$d/bundle.$$.tmp" << 'MANTA_EOF_BUNDLE'`,
    b64.trimEnd(),
    'MANTA_EOF_BUNDLE',
    `mv "$d/bundle.$$.tmp" "$d/${WSL_HOOK_RELAY_BUNDLE_NAME}"`,
    `cat > "$d/launch.$$.tmp" << 'MANTA_EOF_LAUNCH'`,
...
export function buildWslRelaySpawnEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...process.env,
    WSL_UTF8: '1',
    MANTA_AGENT_HOOK_PORT: coords.MANTA_AGENT_HOOK_PORT,
    MANTA_AGENT_HOOK_TOKEN: coords.MANTA_AGENT_HOOK_TOKEN,
    MANTA_AGENT_HOOK_ENV: coords.MANTA_AGENT_HOOK_ENV,
    MANTA_AGENT_HOOK_VERSION: coords.MANTA_AGENT_HOOK_VERSION,
  ...
  addMantaWslInteropEnv(env as Record<string, string>)
  return env
}
