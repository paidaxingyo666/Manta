...
describe('AgentHookServer Codex subagent transcript polling', () => {
  ...
  it('keeps polling across successive roster changes', async () => {
    ...
    try {
      const env = server.buildPtyEnv()
      await fetch(`http://127.0.0.1:${env.MANTA_AGENT_HOOK_PORT}/hook/codex`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Manta-Agent-Hook-Token': env.MANTA_AGENT_HOOK_TOKEN
  ...
  // Why: a nested non-codex CLI inherits the pane's MANTA_PANE_KEY, so its hook must not tear down the codex poll.
  it('keeps polling when a nested non-codex hook lands on the same pane', async () => {
    ...
    try {
      ...
      const post = (path: string, payload: unknown): Promise<Response> =>
        fetch(`http://127.0.0.1:${env.MANTA_AGENT_HOOK_PORT}${path}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'X-Manta-Agent-Hook-Token': env.MANTA_AGENT_HOOK_TOKEN
