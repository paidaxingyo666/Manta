...
export class AgentHookServer {
  ...
  private port = 0
  private token = ''
  // Why: identifies this Manta instance so the server can detect dev vs. prod cross-talk; set at start() from packaged-build knowledge.
  ...
  private scheduleCodexSubagentPoll(
    ...
    // Why: a nested non-codex CLI inherits MANTA_PANE_KEY, so clearing here would silently end a live codex poll.
    if (source !== 'codex') {
      return
    }
  ...
  ingestTerminalStatus(event: {
    ...
    // That erased it from persisted rows (lost across restart) and from headless `manta serve`, which
  ...
  stop(): void {
    ...
    // Why: don't unlink the endpoint file — a stale file matches fail-open and avoids a TOCTOU race with a concurrent Manta.
   ...
   *  has no live local agent process behind it any more. A PTY that dies while Manta
  ...
  buildPtyEnv(): Record<string, string> {
    ...
    if (this.endpointFileWritten && this.endpointFilePathCache) {
      env.MANTA_AGENT_HOOK_ENDPOINT = this.endpointFilePathCache
    }
    return env
  }
