...
// Manta's script while the presence gate skips install() forever. These tests pin the
...
beforeEach(() => {
  ...
  isolatedUserDataDir = mkdtempSync(join(tmpdir(), 'manta-hook-refresh-user-data-'))
...
const STALE_WINDOWS_HOOK = [
  '@echo off',
  'setlocal',
  'if "%MANTA_AGENT_HOOK_PORT%"=="" goto :manta_agent_hook_drain_stdin',
...
describe('refreshManagedScriptIfPresent', () => {
  it('rewrites an existing script and refuses to create a missing one', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'manta-hook-refresh-unit-'))
    try {
...
describe('managed hook script refresh', () => {
  it('brings a stale leaking script current without touching agent config', async () => {
    const home = mkdtempSync(join(tmpdir(), 'manta-hook-refresh-'))
    homedirMock.mockReturnValue(home)
    try {
      // Why: the bug population has a script (from a past install) but no reachable
      // CLI — and possibly no config dir Manta may create. Seed only the script.
      ...
      expect(refreshed).toContain('if "%MANTA_AGENT_HOOK_PORT%"=="" exit /b 0')
      expect(refreshed).not.toContain('if "%MANTA_AGENT_HOOK_PORT%"=="" goto')
  ...
  it('covers every shared launcher script with a refresher', async () => {
    const home = mkdtempSync(join(tmpdir(), 'manta-hook-refresh-coverage-'))
    homedirMock.mockReturnValue(home)
    ...
    try {
      ...
      for (const file of files) {
        expect(
          ...
          `${file} is written to ~/.manta/agent-hooks but no refresher owns it`
        ).toBe(true)
      }
  ...
  it('creates nothing anywhere when no managed scripts exist', async () => {
    const home = mkdtempSync(join(tmpdir(), 'manta-hook-refresh-empty-'))
    homedirMock.mockReturnValue(home)
    try {
