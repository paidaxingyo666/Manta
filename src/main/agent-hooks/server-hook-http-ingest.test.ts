...
describe('AgentHookServer listener replay', () => {
  it('accepts raw JSON hook bodies with base64 metadata headers', async () => {
    ...
    try {
      const env = server.buildPtyEnv()
      const response = await fetch(`http://127.0.0.1:${env.MANTA_AGENT_HOOK_PORT}/hook/claude`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Manta-Agent-Hook-Token': env.MANTA_AGENT_HOOK_TOKEN,
    ...
    try {
      ...
      const postHook = async (
        ...
        const response = await fetch(
          `http://127.0.0.1:${env.MANTA_AGENT_HOOK_PORT}/hook/${source}`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
  ...
  it('tracks Codex agent statuses from form-encoded managed hook posts', async () => {
    ...
    try {
      ...
      const postCodexHook = async (payload: Record<string, unknown>): Promise<void> => {
        ...
        const response = await fetch(`http://127.0.0.1:${env.MANTA_AGENT_HOOK_PORT}/hook/codex`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
            'X-Manta-Agent-Hook-Token': env.MANTA_AGENT_HOOK_TOKEN
  ...
  it('accepts Hermes plugin hook posts on /hook/hermes', async () => {
    ...
    try {
      const env = server.buildPtyEnv()
      const response = await fetch(`http://127.0.0.1:${env.MANTA_AGENT_HOOK_PORT}/hook/hermes`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Manta-Agent-Hook-Token': env.MANTA_AGENT_HOOK_TOKEN
  ...
  it('accepts Amp plugin hook posts on /hook/amp', async () => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:${env.MANTA_AGENT_HOOK_PORT}/hook/amp`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Manta-Agent-Hook-Token': env.MANTA_AGENT_HOOK_TOKEN
