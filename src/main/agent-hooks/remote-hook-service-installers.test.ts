...
`
    ...
    const grokConfig = JSON.parse(grok.fs.files.get('/home/dev/.grok/hooks/manta-status.json')!) as {
      hooks: Record<string, { matcher?: string; hooks?: { command: string }[] }[]>
    }
    for (const eventName of [
      'SessionStart',
    ...
    expect(devin.fs.files.get('/home/dev/.manta/agent-hooks/devin-hook.sh')).toContain('/hook/devin')
  })
