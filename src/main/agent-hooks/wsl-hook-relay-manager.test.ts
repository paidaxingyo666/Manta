...
describe.skipIf(process.platform === 'win32')(
  ...
  () => {
    ...
    it('runs the unchanged remote managed hook installers against a WSL guest home', async () => {
      ...
      const script = readFileSync(join(home, '.manta', 'agent-hooks', 'claude-hook.sh'), 'utf8')
...
describe('WslHookRelayManager', () => {
  ...
  const codexHome =
    '\\\\wsl.localhost\\Ubuntu\\home\\wsl-test-user\\.local\\share\\manta\\codex-runtime-home\\home'
  const opencodeOverlayDir = `${home}/.manta-relay/opencode-overlays/deadbeefcafe`
  const opencode2OverlayDir = `${home}/.manta-relay/opencode2-overlays/deadbeefcafe`
  ...
  function createManager(overrides: Partial<WslHookRelayManagerDeps>): {
    ...
    const deps: WslHookRelayManagerDeps = {
      ...
      hookCoordsEnv: () => ({
        MANTA_AGENT_HOOK_PORT: '43117',
        MANTA_AGENT_HOOK_TOKEN: 'tok',
      ...
      installCodex: vi.fn(async () => ({
        ...
        configPath: `${home}/.local/share/manta/codex-runtime-home/home/hooks.json`,
  ...
  it('starts one relay per distro, installs hooks, exposes the guest endpoint path, and forwards envelopes', async () => {
    ...
    expect(manager.getGuestEndpointFilePath('Ubuntu')).toBe(
      `${home}/.manta-wsl/agent-hooks/instance-testinstance/endpoint.env`
    )
