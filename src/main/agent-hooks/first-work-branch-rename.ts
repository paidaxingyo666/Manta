...
export type FirstWorkBranchRenameDeps = {
  ...
  /** True only for Manta-created worktrees whose branch Manta is allowed to rename. */
  canRenameMantaCreatedBranch: (worktreeId: string) => boolean
...
async function runAutoRename(
  ...
  if (!deps.canRenameMantaCreatedBranch(worktreeId)) {
    return stop(`worktree is not eligible for auto-rename`, true)
  }
  // Only rename a branch Manta auto-named — never one the user chose.
