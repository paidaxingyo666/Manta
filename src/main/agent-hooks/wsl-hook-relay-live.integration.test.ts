...
describe.skipIf(process.platform === 'win32')(
  ...
  () => {
    ...
    let mantaServer: AgentHookServer | null
    ...
    it('delivers a Claude hook POST from the live relay into ingestRemote and installs guest hooks', async () => {
      ...
      manager = new WslHookRelayManager({
        ...
        hookCoordsEnv: () => ({
          ...
          MANTA_AGENT_HOOK_TOKEN: 'live-token',
      ...
      const claudeScript = readFileSync(
        join(fakeHome, '.manta', 'agent-hooks', 'claude-hook.sh'),
        'utf8'
      )
      ...
      const port = Number(/MANTA_AGENT_HOOK_PORT=['"]?(\d+)/.exec(endpointText)?.[1])
      const token = /MANTA_AGENT_HOOK_TOKEN=['"]?([A-Za-z0-9-]+)/.exec(endpointText)?.[1]
