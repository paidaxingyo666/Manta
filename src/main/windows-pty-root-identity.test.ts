...
const MANTA_PID = 5000

function link(pid: number, ppid: number): { pid: number; ppid: number } {
  return { pid, ppid }
}

/** Windows: services.exe → svchost.exe, a chain that never reaches Manta. */
...
describe('classifyWindowsTreeKillTarget', () => {
  it('accepts a ConPTY shell spawned directly by this process', () => {
    const rows = [...SYSTEM_CHAIN, link(MANTA_PID, 900), link(4242, MANTA_PID)]
    expect(classifyWindowsTreeKillTarget(4242, rows, MANTA_PID)).toBe('own')
  })
  ...
  it('accepts a winpty shell reached through the winpty-agent hop', () => {
    ...
    // parent is winpty-agent.exe rather than Manta itself.
    const rows = [link(MANTA_PID, 900), link(6100, MANTA_PID), link(4242, 6100)]
    expect(classifyWindowsTreeKillTarget(4242, rows, MANTA_PID)).toBe('own')
  })

  it('documents that a recycled PID under another Manta pane still classifies as own', () => {
    ...
    const rows = [link(MANTA_PID, 900), link(7000, MANTA_PID), link(7100, 7000), link(4242, 7100)]
    expect(classifyWindowsTreeKillTarget(4242, rows, MANTA_PID)).toBe('own')
  })

  it('rejects a recycled pid whose ancestry never reaches this process', () => {
    const rows = [...SYSTEM_CHAIN, link(MANTA_PID, 900), link(4242, 900)]
    expect(classifyWindowsTreeKillTarget(4242, rows, MANTA_PID)).toBe('foreign')
  })

  it('reports an exited root as absent rather than sweeping a stale pid', () => {
    const rows = [...SYSTEM_CHAIN, link(MANTA_PID, 900)]
    expect(classifyWindowsTreeKillTarget(4242, rows, MANTA_PID)).toBe('absent')
  })
  ...
  it('rejects a recycled pid whose parent has itself already exited', () => {
    ...
    const rows = [link(MANTA_PID, 900), link(4242, 31337)]
    expect(classifyWindowsTreeKillTarget(4242, rows, MANTA_PID)).toBe('foreign')
  })

  it('rejects a chain longer than the ConPTY/winpty depth even if Manta is above it', () => {
    const rows = [
      link(MANTA_PID, 900),
      link(10, MANTA_PID),
      link(11, 10),
  ...
  it('never force-kills this process tree when the root pid is our own pid', () => {
    const rows = [link(MANTA_PID, 900)]
    expect(classifyWindowsTreeKillTarget(MANTA_PID, rows, MANTA_PID)).toBe('foreign')
  })

  it.each([0, -1, 1.5, Number.NaN])('rejects the invalid root pid %s', (rootPid) => {
    expect(classifyWindowsTreeKillTarget(rootPid, [link(4242, MANTA_PID)], MANTA_PID)).toBe(
      'foreign'
    )
  })

  it('treats duplicate rows for the root as unknown, not as ownership', () => {
    const rows = [link(4242, MANTA_PID), link(4242, 900)]
    expect(classifyWindowsTreeKillTarget(4242, rows, MANTA_PID)).toBe('unknown')
  })
...
describe('verifyWindowsTreeKillTarget', () => {
  it('classifies a live win32 root against the fresh process table', async () => {
    const readRows = vi.fn().mockResolvedValue([link(4242, MANTA_PID)])
    await expect(
      verifyWindowsTreeKillTarget(4242, { readRows, ownerPid: MANTA_PID, platform: 'win32' })
    ).resolves.toBe('own')
  ...
  it('skips the probe off Windows so POSIX teardown keeps its own guards', async () => {
    const readRows = vi.fn()
    await expect(
      verifyWindowsTreeKillTarget(4242, { readRows, ownerPid: MANTA_PID, platform: 'darwin' })
    ).resolves.toBe('unknown')
...
describe('verifyWindowsTreeKillTarget scan volume', () => {
  const ROWS_JSON = JSON.stringify([
    { ProcessId: MANTA_PID, ParentProcessId: 900, Name: 'manta.exe', CommandLine: 'manta.exe' },
    { ProcessId: 4242, ParentProcessId: MANTA_PID, Name: 'pwsh.exe', CommandLine: 'pwsh.exe' }
  ])
  ...
  it('collapses a 32-wide teardown burst into a single process-table scan', async () => {
    const verdicts = await Promise.all(
      Array.from({ length: 32 }, () =>
        verifyWindowsTreeKillTarget(4242, { ownerPid: MANTA_PID, platform: 'win32' })
      )
    )
