...
describe('classifyWindowsTreeKillTarget', () => {
  ...
  it.each([0, -1, 1.5, Number.NaN])('rejects the invalid root pid %s', (rootPid) => {
    expect(classifyWindowsTreeKillTarget(rootPid, [link(4242, MANTA_PID)], MANTA_PID)).toBe('foreign')
  })
...
describe('verifyWindowsTreeKillTarget scan volume', () => {
  const NATIVE_ROWS = [
    { pid: MANTA_PID, ppid: 900, name: 'manta.exe', commandLine: 'manta.exe' },
    { pid: 4242, ppid: MANTA_PID, name: 'pwsh.exe', commandLine: 'pwsh.exe' }
  ]
