...
describe('parseMantaYaml', () => {
  it('parses YAML with setup script only', () => {
    const yaml = `scripts:\n  setup: |\n    echo "setting up"\n    npm install\n`
    const result = parseMantaYaml(yaml)
  ...
  it('returns null when there is no scripts block', () => {
    const yaml = `other:\n  key: value\n`
    expect(parseMantaYaml(yaml)).toBeNull()
  })
  ...
  it('returns null for empty string', () => {
    expect(parseMantaYaml('')).toBeNull()
  })
  ...
  it('parses default terminal tabs from manta.yaml', () => {
    const yaml = [
      'defaultTabs:',
      '  - title: Claude',
      '    color: "#f97316"',
  ...
  it('parses environmentRecipes from manta.yaml', () => {
    const yaml = [
      'environmentRecipes:',
      '  - id: cloud-sandbox',
      '    name: Cloud Sandbox',
      ...
      '    create: ./scripts/manta-vm/start-cloud-sandbox.sh',
      '    suspend: ./scripts/manta-vm/suspend-cloud-sandbox.sh',
      '    resume: ./scripts/manta-vm/resume-cloud-sandbox.sh',
      '    destroy: ./scripts/manta-vm/destroy-cloud-sandbox.sh'
    ].join('\n')
      ...
      environmentRecipeDiagnostics: [
        {
          ...
          field: 'checkoutMode',
          message:
            'Recipe "cloud-sandbox" checkoutMode must be "manta-worktree" or "provisioned-root".'
        }
      ]
    })
  })
  ...
  it('parses legacy environmentRecipes command and cleanup aliases', () => {
    const yaml = [
      ...
      '  - id: manual-sandbox',
      '    name: Manual Sandbox',
      '    command: ./scripts/manta-vm/start-manual-sandbox.sh',
      '    cleanup: none'
    ].join('\n')
      ...
      environmentRecipes: [
        {
          id: 'manual-sandbox',
          name: 'Manual Sandbox',
          create: './scripts/manta-vm/start-manual-sandbox.sh',
  ...
  it('drops invalid and duplicate environmentRecipes', () => {
    const yaml = [
      ...
      '    create: ./scripts/manta-vm/start-duplicate.sh',
      '  - id: missing-create',
      '    name: Missing Create',
      '  - name: Missing Id',
      '    create: ./scripts/manta-vm/start-missing-id.sh',
      '  - id: "Cloud Sandbox"',
      '    name: Unsafe Id',
      '    create: ./scripts/manta-vm/start-unsafe-id.sh',
      '  - 42'
    ].join('\n')
  ...
  it('parses worktree.sharedDirectories from manta.yaml', () => {
    const result = parseMantaYaml(
  ...
  it('returns null when sharedDirectories is the only key and holds nothing usable', () => {
    expect(parseMantaYaml('worktree:\n  sharedDirectories: []\n')).toBeNull()
    expect(parseMantaYaml('worktree:\n  sharedDirectories: node_modules\n')).toBeNull()
  })

  it('keeps sharedDirectories alongside other manta.yaml keys', () => {
    const result = parseMantaYaml(
      [
        'scripts:',
        '  setup: pnpm install',
...
describe('hasUnrecognizedMantaYamlKeys', () => {
  it('returns true when the file contains only keys this version does not handle', async () => {
    ...
    const { hasUnrecognizedMantaYamlKeys } = await import('./hooks')
    expect(hasUnrecognizedMantaYamlKeys('/test/repo')).toBe(true)
  })
  ...
  it('returns false when the file contains only recognised keys', async () => {
    ...
    const { hasUnrecognizedMantaYamlKeys } = await import('./hooks')
    expect(hasUnrecognizedMantaYamlKeys('/test/repo')).toBe(false)
  })
