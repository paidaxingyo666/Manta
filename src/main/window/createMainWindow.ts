...
type CreateMainWindowOptions = {
  ...
  /** Returns true when Manta should reload after renderer loss; update-relaunch/quit tear down children intentionally, so don't fight shutdown. */
...
export function createMainWindow(
  ...
  const mainWindow = new BrowserWindow({
    ...
    title: opts?.title ?? 'Manta',
    show: false,
    // Why: macOS swallows the app-activating click by default, so clicking back into Manta needed a second click (Windows/Linux already deliver it).
  ...
  const dispatchResolvedWindowShortcutAction = (
    ...
    const capturedTerminalActionId =
      ...
      focusedShortcutContext.terminalShortcutPolicy === 'manta-first' &&
  ...
  mainWindow.webContents.on('before-input-event', (event, input) => {
    ...
    if (isMacAppPasteInput(input)) {
      // Why: chat/terminal panes hold focus without native editable controls, so route Cmd+V through Manta's paste ownership.
      event.preventDefault()
