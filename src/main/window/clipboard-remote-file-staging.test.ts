...
const STAGING_ROOT = join(TEMP_ROOT, `manta-clipboard-files${UID_SUFFIX}`)
...
const LEGACY_EXPIRED = `manta-clipboard-file-${EXPIRED_TRANSFER}`
const LEGACY_FRESH = `manta-clipboard-file-${FRESH_TRANSFER}`
...
describe('legacy remote clipboard staging compatibility', () => {
  ...
  it('removes only exact expired legacy children encountered inside the bound', async () => {
    opendirMock.mockResolvedValue(
      openedDirectory([
        directoryEntry('manta-clipboard-file-lookalike'),
