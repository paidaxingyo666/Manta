...
import { getMantaProfileIndexPath, readProfileIndex } from '../manta-profiles/profile-index-store'

/**
 * Worktree ids owned by Manta profiles OTHER than the running one.
 *
...
export function getOtherProfileWorktreeIdsForHistoryGc(userDataPath = getProfileUserDataPath()): {
  ...
  const index = readProfileIndex(getMantaProfileIndexPath(userDataPath))
  if (!index) {
  ...
  for (const profile of index.profiles) {
    ...
    const collected = readProfileWorktreeIds(getMantaProfileDataFile(profile.id, userDataPath))
    if (!collected) {
