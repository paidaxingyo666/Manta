...
describe('createMainWindow', () => {
  ...
  it('enables renderer sandboxing and opens external links safely', () => {
    ...
    const allowBlankPrefs = { partition: 'persist:manta-browser' }
    fire(
      'will-attach-webview',
    ...
    routePartitionAllowedMock.mockImplementation(
      (partition) => partition === 'persist:manta-browser-v1-route-partition'
    )
    const allowRouteEvent = { preventDefault: vi.fn() }
    const allowRoutePrefs = { partition: 'persist:manta-browser-v1-route-partition' }
    fire(
      'will-attach-webview',
    ...
    expect(allowRoutePrefs).toMatchObject({
      partition: 'persist:manta-browser-v1-route-partition',
      sandbox: true
    })
    ...
    fire(
      ...
      { partition: 'persist:manta-browser-v1-route-partition' } as never,
