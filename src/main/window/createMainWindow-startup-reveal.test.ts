...
describe('createMainWindow', () => {
  ...
  it.each(['darwin', 'linux', 'win32'] as const)(
    ...
    (platform) => {
      vi.useFakeTimers()
      vi.stubEnv('MANTA_BACKGROUND_LAUNCH', '1')
  ...
  it('keeps the headless E2E window hidden when the Windows fallback fires', () => {
    vi.useFakeTimers()
    const previousHeadless = process.env.MANTA_E2E_HEADLESS
    process.env.MANTA_E2E_HEADLESS = '1'
    const { browserWindowInstance } = createStartupRevealWindowFixture()
    ...
    try {
      ...
      if (previousHeadless === undefined) {
        delete process.env.MANTA_E2E_HEADLESS
      } else {
        process.env.MANTA_E2E_HEADLESS = previousHeadless
      }
    }
  })
