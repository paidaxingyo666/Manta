...
describe('isBackgroundLaunch', () => {
  it('covers headless and headful E2E plus opted-in dev launches', () => {
    expect(isBackgroundLaunch({ MANTA_E2E_HEADLESS: '1' })).toBe(true)
    expect(isBackgroundLaunch({ MANTA_E2E_HEADFUL: '1' })).toBe(true)
    expect(isBackgroundLaunch({ MANTA_BACKGROUND_LAUNCH: '1' })).toBe(true)
    expect(isBackgroundLaunch({})).toBe(false)
  })

  it('keeps an explicit background request despite inherited foreground flags', () => {
    expect(isBackgroundLaunch({ MANTA_BACKGROUND_LAUNCH: '1', MANTA_E2E_FOREGROUND: '1' })).toBe(true)
  })

  it('lets native-focus specs opt back into the foreground', () => {
    expect(isBackgroundLaunch({ MANTA_E2E_HEADFUL: '1', MANTA_E2E_FOREGROUND: '1' })).toBe(false)
    expect(isWindowlessLaunch({ MANTA_E2E_HEADLESS: '1', MANTA_E2E_FOREGROUND: '1' })).toBe(false)
  })
})

describe('isWindowlessLaunch', () => {
  it('keeps explicit background launches hidden while headful E2E can paint', () => {
    expect(isWindowlessLaunch({ MANTA_E2E_HEADLESS: '1' })).toBe(true)
    expect(isWindowlessLaunch({ MANTA_E2E_HEADLESS: '1', MANTA_E2E_HEADFUL: '1' })).toBe(false)
    expect(isWindowlessLaunch({ MANTA_BACKGROUND_LAUNCH: '1' })).toBe(true)
    expect(
      isWindowlessLaunch({
        MANTA_BACKGROUND_LAUNCH: '1',
        MANTA_E2E_HEADFUL: '1',
        MANTA_E2E_FOREGROUND: '1'
      })
    ).toBe(true)
  })
})
...
describe('showWindowWithoutStealingFocus', () => {
  it('keeps a headless window off screen', () => {
    ...
    showWindowWithoutStealingFocus(window, { MANTA_E2E_HEADLESS: '1' })
  ...
  it('never reveals an explicitly background window', () => {
    const window = makeWindow()
    showWindowWithoutStealingFocus(window, { MANTA_BACKGROUND_LAUNCH: '1' })
    expect(window.showInactive).not.toHaveBeenCalled()
    expect(window.show).not.toHaveBeenCalled()
  })
  ...
  it('still reveals explicitly headful E2E without activation', () => {
    const window = makeWindow()
    showWindowWithoutStealingFocus(window, { MANTA_E2E_HEADFUL: '1' })
...
describe('applyBackgroundActivationPolicy', () => {
  ...
  it.each(['MANTA_E2E_HEADLESS', 'MANTA_BACKGROUND_LAUNCH'])(
  ...
  it('leaves a headful or user launch with its normal Dock presence', () => {
    ...
    applyBackgroundActivationPolicy({
      app: headful,
      env: { MANTA_E2E_HEADLESS: '1', MANTA_E2E_HEADFUL: '1' },
      platform: 'darwin'
    })
