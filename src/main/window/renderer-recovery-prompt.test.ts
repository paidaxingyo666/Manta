...
const POISON: InstallDirAclPoisonDiagnosis = {
  detail: "Windows permissions on Manta's install folder are blocking its own sandboxed processes.",
  commands: ['icacls "C:\\Manta" /grant "*S-1-15-2-2:(OI)(CI)(RX)"', 'icacls "C:\\Manta" /grant b']
}
...
describe('presentRendererRecoveryPrompt', () => {
  ...
  it('interpolates the recovery count', async () => {
    ...
    await run()
    expect(shown[0].detail).toContain('Manta tried to recover 7 times in a row')
