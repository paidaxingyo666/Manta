...
const { getPathMock } = vi.hoisted(() => ({
  getPathMock: vi.fn(() => '/tmp/manta-test-userdata')
}))
...
describe('OpenCodeUsageStore', () => {
  ...
  beforeEach(() => {
    tempUserData = mkdtempSync(join(tmpdir(), 'manta-opencode-usage-store-'))
  ...
  it('adapts OpenCode scans to pretty-printed cache persistence', async () => {
    ...
    const persistedJson = readFileSync(join(tempUserData, 'manta-opencode-usage.json'), 'utf-8')
    expect(scanOpenCodeUsageDatabases).toHaveBeenCalledWith([], [])
    expect(persistedJson).toContain('\n')
  })

  it('reports no data for Manta scope when only non-Manta OpenCode usage exists', async () => {
    const store = createStoreWithState({
      sessions: [
        makeSession({
  ...
  it('uses recorded OpenCode costs and token totals without model pricing inference', async () => {
    ...
    const summary = await store.getSummary('manta', '30d')
    const daily = await store.getDaily('manta', '30d')
  ...
  it('returns recent sessions with OpenCode event and token fields', async () => {
    ...
    const sessions = await store.getRecentSessions('manta', '30d', 5)
