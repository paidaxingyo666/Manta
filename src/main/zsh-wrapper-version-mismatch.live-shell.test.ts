/**
 * Real-zsh proof that a wrapper dir written by two different Manta builds still
 * loads the user's own zsh config.
 *
 * One wrapper dir (`<userData>/shell-ready/zsh`, or `~/.manta-relay/shell-ready/
...
export MANTA_ORIG_ZDOTDIR="$HOME"
...
describe.skipIf(process.platform === 'win32')('zsh wrapper dir written by mixed builds', () => {
  itWithZsh('still sources the user config when the .zshenv is from an older build', () => {
    ...
    const home = mkdtempSync(join(tmpdir(), 'manta-wrapper-mismatch-home-'))
    try {
