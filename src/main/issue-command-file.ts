// Why: `.manta/issue-command` is the per-user override; `manta.yaml` is the tracked project default.
...
export function getIssueCommandFilePath(repoPath: string): string {
  return join(repoPath, MANTA_DIR, ISSUE_COMMAND_FILENAME)
}
...
/**
 * Write the per-user issue command override to `{repoRoot}/.manta/issue-command`.
 * Empty content deletes the override so the shared `manta.yaml` command applies again.
 */
...
export async function isIssueCommandIgnoredByGit(
  ...
  try {
    const issueCommandPath = `${MANTA_DIR}/${ISSUE_COMMAND_FILENAME}`
...
/** Ensure `.manta` is in `.gitignore` so the per-user directory is never committed. */
function ensureMantaDirIgnored(repoPath: string): void {
  const gitignorePath = join(repoPath, '.gitignore')
  try {
    if (existsSync(gitignorePath)) {
      const content = readFileSync(gitignorePath, 'utf-8')
      if (/^\.manta\/?$/m.test(content)) {
        return
      }
      const separator = content.endsWith('\n') ? '' : '\n'
      writeFileSync(gitignorePath, `${content}${separator}.manta\n`, 'utf-8')
    } else {
      writeFileSync(gitignorePath, '.manta\n', 'utf-8')
    }
  } catch {
    console.warn('[hooks] Could not update .gitignore to exclude .manta')
  }
}
