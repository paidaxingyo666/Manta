...
describe('CodexRuntimeHomeService', () => {
  ...
  it('uses the canonical Electron userData for legacy active host migration', async () => {
    const staleUserDataDir = mkdtempSync(join(tmpdir(), 'manta-stale-runtime-home-'))
    const staleRuntimeHomePath = join(staleUserDataDir, 'codex-runtime-home', 'home')
    try {
      mkdirSync(staleRuntimeHomePath, { recursive: true })
      process.env.MANTA_USER_DATA_PATH = staleUserDataDir
      const legacyLaunchHomePath = join(
        testState.userDataDir,
        'codex-runtime-home',
        'launch',
      ...
      const { configureMantaUserDataPathEnv } = await import('../startup/configure-process')
      configureMantaUserDataPathEnv()
      const { CodexRuntimeHomeService } = await import('./runtime-home-service')
      new CodexRuntimeHomeService(store as never)

      expect(process.env.MANTA_USER_DATA_PATH).toBe(testState.userDataDir)
      expect(normalizeLinkTarget(readlinkSync(legacyActiveHomePath))).toBe(
        normalizeLinkTarget(getRuntimeCodexHomePath())
      )
  ...
  it('preserves conflicting legacy session files under deterministic names', async () => {
    ...
    expect(
      readFileSync(join(runtimeSessionsDir, 'session.manta-legacy-account-1.json'), 'utf-8')
    ).toBe('{"turns":[1,2]}')
    expect(
      readFileSync(
        join(runtimeSessionsDir, 'nested', 'session.manta-legacy-account-1.json'),
        'utf-8'
      )
    ).toBe('{"turns":[2,3]}')
