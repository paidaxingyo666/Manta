...
describe('CodexRuntimeHomeService', () => {
  ...
  it('returns the Manta-managed runtime home for Codex launch and rate-limit preparation', async () => {
  ...
  it('routes host system default to the real home', async () => {
    ...
    process.env.MANTA_CODEX_HOME = getRuntimeCodexHomePath()
    try {
      // Background fetchers prefer ambient CODEX_HOME when passed null, so an
      // explicit path proves nested Manta launches cannot poll the managed home.
      expect(service.prepareForRateLimitFetch()).toEqual({
        kind: 'ready',
  ...
  it('resolves only Manta-owned homes used by live retained host shells', async () => {
    ...
    writeFileSync(join(unownedHome, '.manta-managed-home'), 'account-2\n', 'utf-8')
    writePaneRegistry({
