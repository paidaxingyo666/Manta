...
export function runApplyScript(options: DrainApplyInterference = {}): DrainApplyOutcome {
  const root = mkdtempSync(join(tmpdir(), 'manta-drain-apply-'))
...
if (
  ...
  args.at(-1)?.endsWith('.manta-drain-destination')
) {
...
if (
  ...
  args.at(-2)?.includes('.manta-drain-session-stage') &&
  args.at(-1)?.includes('.manta-bridge-')
) {
  process.exit(1)
}
...
`
    ...
    execFileSync(
      ...
      ],
      {
        ...
        env: {
          ...
          SESSION_COMMIT_MARKER: `${markerPath}.manta-drain-session-commit`,
          ...
          REWRITE_SESSION_AUTH:
            options.rewriteTargetAfterSessionLink || options.replaceTargetAfterSessionLink
              ? targetAuthPath
              : options.rewriteQuarantineAfterSessionLink
                ? `${markerPath}.manta-drain-live-source`
                : legacyAuthPath,
          REWRITE_TARGET:
  ...
  if (
    ...
    if (options.rewriteQuarantineBeforeRecovery) {
      const quarantinePath = `${markerPath}.manta-drain-live-source`
  ...
  return {
    ...
    targetSession: existsSync(join(targetHome, ...RETIRED_SESSION_SEGMENTS))
      ...
      : null,
    sourceQuarantineAuth: existsSync(`${markerPath}.manta-drain-live-source`)
      ? readFileSync(`${markerPath}.manta-drain-live-source`, 'utf8')
      : null,
    sourceRecoveryAuth: existsSync(`${markerPath}.manta-drain-source`)
      ? readFileSync(`${markerPath}.manta-drain-source`, 'utf8')
      : null,
    destinationRecoveryAuth: existsSync(`${markerPath}.manta-drain-destination`)
      ? readFileSync(`${markerPath}.manta-drain-destination`, 'utf8')
      : null,
    destinationRecoveryPathExists: existsSync(`${markerPath}.manta-drain-destination-path`),
    sessionCommitMarkerExists: existsSync(`${markerPath}.manta-drain-session-commit`)
  }
}
