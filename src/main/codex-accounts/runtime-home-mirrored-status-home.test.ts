...
beforeEach(() => {
  vi.resetModules()
  testState.userData = mkdtempSync(join(tmpdir(), 'manta-codex-status-home-ud-'))
  testState.home = mkdtempSync(join(tmpdir(), 'manta-codex-status-home-'))
  ...
  for (const key of ['MANTA_USER_DATA_PATH', 'CODEX_HOME', 'MANTA_CODEX_HOME']) {
...
describe('CodexRuntimeHomeService.getMirroredHostHomePathForStatus', () => {
  ...
  it('returns the shared runtime home when a custom CODEX_HOME keeps the mirror lane', async () => {
    ...
    const { CodexRuntimeHomeService } = await import('./runtime-home-service')
    const { getMantaManagedCodexHomePath } = await import('../codex/codex-home-paths')
    const service = new CodexRuntimeHomeService(createStore([], null) as never)
    ...
    expect(service.getMirroredHostHomePathForStatus()).toBe(getMantaManagedCodexHomePath())
  })
})
