...
set -eu
source_recovery_auth="$3.manta-drain-source"
source_quarantine_auth="$3.manta-drain-live-source"
destination_recovery_auth="$3.manta-drain-destination"
destination_recovery_path="$3.manta-drain-destination-path"
session_link_manifest="$3.manta-drain-session-links"
session_commit_marker="$3.manta-drain-session-commit"
session_stage_root="$3.manta-drain-session-stage"
${ROLLBACK_SESSION_LINKS_FUNCTION}
${RESOLVE_LEGACY_HOME_SCRIPT}
...
session_commit_marker="$3.manta-drain-session-commit"
session_stage_root="$3.manta-drain-session-stage"
session_scan_watermark="$3.manta-drain-session-watermark"
${ROLLBACK_SESSION_LINKS_FUNCTION}
if [ -e "$3" ] || [ -L "$3" ]; then
  [ -f "$3" ] && [ ! -L "$3" ] || exit 46
  commit_session_links || exit 46
...
umask 077
temporary_auth="$target_auth.manta-drain-$$"
temporary_credentials="$target_home/.credentials.json.manta-drain-$$"
temporary_previous_auth="$target_auth.manta-drain-previous-$$"
temporary_destination_auth="$target_auth.manta-drain-destination-$$"
temporary_source_auth="$source_auth.manta-drain-source-$$"
temporary_destination_snapshot="$target_auth.manta-drain-snapshot-$$"
temporary_destination_path="$3.manta-drain-destination-path-$$"
temporary_source_snapshot="$3.manta-drain-source-$$"
temporary_marker="$3.manta-drain-$$"
