...
export function runRecoveryScript(options: {
  ...
  const root = mkdtempSync(join(tmpdir(), 'manta-drain-recovery-'))
  ...
  const sourceRecoveryPath = `${markerPath}.manta-drain-source`
  const destinationRecoveryPath = `${markerPath}.manta-drain-destination`
  const destinationRecoveryTargetPath = `${markerPath}.manta-drain-destination-path`
...
export function runAbsentLegacyHomeScript(options: {
  ...
  const root = mkdtempSync(join(tmpdir(), 'manta-drain-absent-home-'))
  const legacyHome = join(root, 'legacy')
