...
function downloadedEvent(packageType: LinuxRootPackageType): Record<string, unknown> {
  const fileName =
    packageType === 'deb' ? 'manta-ide_1.0.61_amd64.deb' : 'manta-ide-1.0.61.x86_64.rpm'
  return {
    version: '1.0.61',
    downloadedFile: join(tmpdir(), 'manta-updater', 'pending', fileName),
...
describe('updater Linux root packages', () => {
  ...
  it('preserves the normal AppImage install path when no root-package marker is present', async () => {
    ...
    await reachDownloaded(
      ...
      {
        version: '1.0.61',
        downloadedFile: join(tmpdir(), 'Manta-1.0.61.AppImage'),
        files: []
      },
      true
    )
  ...
  it('fails closed for an unusable packaged marker', async () => {
    ...
    await reachDownloaded(updater, {
      version: '1.0.61',
      downloadedFile: join(tmpdir(), 'manta-updater', 'pending', 'manta-ide_1.0.61_amd64.deb'),
      files: [{ url: 'manta-ide_1.0.61_amd64.deb', sha512: packageSha512 }]
    })
    expect(lastStatus(send)).toEqual({
      state: 'error',
      message:
