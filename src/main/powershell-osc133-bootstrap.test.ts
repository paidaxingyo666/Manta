...
describe('PowerShell OSC 133 bootstrap', () => {
  it('wraps prompt/readline without bypassing profiles or execution policy', () => {
    ...
    expect(script).toContain('MANTA_OPENCODE_CONFIG_DIR')
    expect(script).toContain('MANTA_MIMOCODE_HOME')
    expect(script).not.toContain('MANTA_PI_CODING_AGENT_DIR')
    expect(script).not.toContain('MANTA_OMP_CODING_AGENT_DIR')
    expect(script).toContain('MANTA_OMP_STATUS_EXTENSION')
    expect(script).toContain('function Global:omp')
    expect(script).toContain('--extension $env:MANTA_OMP_STATUS_EXTENSION')
    expect(script).toContain('MANTA_CODEX_HOME')
    expect(script).toContain('MANTA_CODEX_LAUNCH_PREFLIGHT')
    expect(script).toContain('function Global:codex')
    expect(script).not.toContain('$Global:__MantaCodexExecutable')
    ...
    const codexHomeRestore = script.indexOf('if ($env:MANTA_CODEX_HOME)')
