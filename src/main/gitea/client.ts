...
function getAuthConfig(): GiteaAuthConfig {
  const apiBaseUrl = envValue('MANTA_GITEA_API_BASE_URL')
  return {
    apiBaseUrl: apiBaseUrl ? normalizeGiteaApiBaseUrl(apiBaseUrl) : null,
    token: envValue('MANTA_GITEA_TOKEN')
  }
}
...
/** Invalidate the shared /pulls scan after Manta itself creates a PR so the
