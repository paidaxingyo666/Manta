...
let provider: MantadBrowserProvider | null
...
beforeEach(async () => {
  harnessRoot = await mkdtemp(join(tmpdir(), 'mantad-provider-selection-'))
...
describe('resolveMantadBrowserProvider Electron preference', () => {
  it('uses the installed Electron app even when a Chromium executable is configured', async () => {
    provider = await resolveMantadBrowserProvider({
      userDataPath: join(harnessRoot, 'state'),
      environment: { MANTA_BROWSER_EXECUTABLE: chromiumExecutable },
  ...
  it('falls back to the operator Chromium when the installed Electron app cannot start', async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      '[mantad] Installed Electron browser provider unavailable:',
      expect.anything()
    )
  })
