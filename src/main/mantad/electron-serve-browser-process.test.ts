...
beforeEach(async () => {
  harnessRoot = await mkdtemp(join(tmpdir(), 'mantad-electron-harness-'))
  ...
  spawnProcessMock.mockImplementation((spec: RunProcessModule.ProcessSpec) =>
    actual.spawnProcess({
      ...
      env: {
        ...spec.env,
        MANTA_FAKE_SIDECAR_LOG: logPath,
        MANTA_FAKE_SIDECAR_CONTROL: controlPath,
        ...(sidecarMode ? { MANTA_FAKE_SIDECAR_MODE: sidecarMode } : {})
      }
    })
  )
})
...
describe('ElectronServeBrowserProcess start-up', () => {
  it('launches the installed app in headless serve mode without mantad browser env', async () => {
    for (const key of AGENT_BROWSER_ENVIRONMENT_KEYS) {
      vi.stubEnv(key, `leaked-${key}`)
    }
    vi.stubEnv('MANTA_HARNESS_UNRELATED', 'preserved')
    ...
    expect(spec.env?.MANTA_HARNESS_UNRELATED).toBe('preserved')
  ...
  it('stops the sidecar process and removes its user data directory', async () => {
    ...
    const metadata = JSON.parse(
      await readFile(join(userDataPath, 'manta-runtime.json'), 'utf8')
    ) as { pid: number }
