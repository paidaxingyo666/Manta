/**
 * The path half of mantad's `AppEnvironment`: every `AppPathName` gets a real answer for
...
export function resolveUserDataPath(): string {
  const explicit = env('MANTA_USER_DATA')
  if (explicit) {
    return explicit
  }
...
/**
 * The directory mantad was launched from — the bundle root that `mantad.js` and the
 ...
 */
export function resolveMantadInstallRoot(scriptPath = process.argv[1]): string {
  if (!scriptPath) {
    throw new Error(
      'mantad_install_root_unavailable — process.argv[1] is unset, so this process has no ' +
...
export function resolveMantadPath(name: AppPathName): string {
  switch (name) {
    case 'userData':
