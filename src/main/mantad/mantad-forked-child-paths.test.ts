...
/**
 * The composition, not the pieces: mantad's real host adapters against the real resolver
 * and a real deployment layout on disk. Both halves of this passed their own unit tests
 * while the running server forked a path that has never existed in an mantad deployment.
 */
describe('mantad forked-child paths', () => {
  let deployRoot: string
  let originalArgv: string[]

  beforeEach(() => {
    deployRoot = mkdtempSync(join(tmpdir(), 'mantad-deploy-'))
  ...
  it('ignores a decoy child under the working directory', () => {
    ...
    const elsewhere = mkdtempSync(join(tmpdir(), 'mantad-cwd-'))
