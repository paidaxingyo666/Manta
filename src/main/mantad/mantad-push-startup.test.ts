...
vi.mock('./mantad-app-paths', () => ({
  resolveMantadInstallRoot: () => state.root,
  resolveMantadPath: () => state.root,
  resolveUserDataPath: () => state.root
}))
vi.mock('./mantad-browser-provider', () => ({ resolveMantadBrowserProvider: async () => null }))
...
vi.mock('../manta-profiles/profile-index-store', () => ({
  ...
  ensureActiveMantaProfile: () => ({ dataFile: join(state.root, 'profile.json') })
}))
...
vi.mock('../runtime/manta-runtime', () => ({
  MantaRuntimeService: class {
    getRuntimeId() {
