...
describe('GitHub GraphQL rate-limit guard', () => {
  ...
  it('uses explicit PR repo for merge and title mutations', async () => {
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          title: 'PR',
          state: 'OPEN',
          url: 'https://github.com/stablyai/manta/pull/7',
          statusCheckRollup: [],
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      1,
      ['api', 'repos/stablyai/manta/pulls/7'],
      {
        cwd: '/repo-root',
        host: 'github.com'
      }
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      3,
      ['pr', 'merge', '7', '--squash', '--repo', 'stablyai/manta'],
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      4,
      ['pr', 'edit', '7', '--title', 'New title', '--repo', 'stablyai/manta'],
      { cwd: '/repo-root', host: 'github.com' }
    )
  })
  ...
  it('sets and disables PR auto-merge with explicit PR repos and SSH context', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
      ['pr', 'view', '7', '--json', 'id,headRefOid,baseRefName', '--repo', 'stablyai/manta'],
      { host: 'github.com' }
    )
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      3,
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      4,
      ['pr', 'merge', '7', '--disable-auto', '--repo', 'stablyai/manta'],
      expect.objectContaining({
        env: expect.objectContaining({ GH_PROMPT_DISABLED: '1' }),
        host: 'github.com'
      })
  ...
  it('uses the queue-aware gh merge path when the base branch has a merge queue', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      4,
      ['pr', 'merge', '7', '--auto', '--squash', '--repo', 'stablyai/manta'],
      expect.objectContaining({
        cwd: '/repo-root',
        env: expect.objectContaining({ GH_PROMPT_DISABLED: '1' }),
        host: 'github.com'
  ...
  it('detects merge queues once per base branch and blocks direct merges', async () => {
    ...
    await expect(
      mergePR('/repo-root', 7, 'squash', undefined, { owner: 'stablyai', repo: 'manta' })
    ...
    expect(ghExecFileAsyncMock.mock.calls[2]?.[0]).toEqual(
      expect.arrayContaining(['-f', 'owner=stablyai', '-f', 'repo=manta', '-f', 'branch=true'])
    )
    ...
    ghExecFileAsyncMock.mockImplementation(async (args) => {
      ...
      return {
        stdout: JSON.stringify({
          ...
          title: 'PR',
          state: 'OPEN',
          url: `https://github.com/stablyai/manta/pull/${prViewCount}`,
          statusCheckRollup: [],
          updatedAt: '2026-04-01T00:00:00Z',
          isDraft: false,
          mergeable: 'MERGEABLE',
