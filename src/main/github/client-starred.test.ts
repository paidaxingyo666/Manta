...
import { __resetOrcaStarCheckForTests, checkMantaStarred, starManta } from './client'
...
describe('checkMantaStarred', () => {
  ...
  it('coalesces concurrent checks onto one gh child', async () => {
    ...
    const first = checkMantaStarred()
    const second = checkMantaStarred()
    const third = checkMantaStarred()
...
describe('starManta', () => {
  ...
  it('stars through the bounded gh runner and releases its slot', async () => {
    ...
    await expect(starManta()).resolves.toBe(true)

    expect(execFileAsyncMock).not.toHaveBeenCalled()
    const [args, options] = ghExecFileAsyncMock.mock.calls[0]
    expect(args).toEqual(['api', '-X', 'PUT', 'user/starred/stablyai/manta'])
    expect(options.timeout).toBeGreaterThan(0)
    expect(releaseMock).toHaveBeenCalledTimes(1)
  })
  ...
  it('reports failure and still releases its slot when gh times out', async () => {
    ...
    await expect(starManta()).resolves.toBe(false)
    expect(releaseMock).toHaveBeenCalledTimes(1)
  })
})
