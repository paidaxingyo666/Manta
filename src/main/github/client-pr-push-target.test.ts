...
describe('getPRForBranch', () => {
  ...
  it('resolves fork PR push target using the origin URL protocol', async () => {
    ...
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'stablyai', repo: 'manta' })
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'prateek/fix-sidebar-agents-toggle',
          repo: {
            full_name: 'prateek/manta',
            name: 'manta',
            clone_url: 'https://github.com/prateek/manta.git',
            ssh_url: 'git@github.com:prateek/manta.git',
            owner: { login: 'prateek' }
          }
        }
      })
    ...
    expect(target).toEqual({
      pushTarget: {
        remoteName: 'pr-prateek-manta',
        branchName: 'prateek/fix-sidebar-agents-toggle',
        remoteUrl: 'git@github.com:prateek/manta.git'
      }
    })
  })

  it('pins explicit origin push-target lookup when upstream has the same PR number', async () => {
    getOwnerRepoMock.mockResolvedValue({ owner: 'fork', repo: 'manta' })
    resolvePRRepositoryCandidatesMock.mockResolvedValue({
      candidates: [
        { owner: 'upstream', repo: 'manta' },
        { owner: 'fork', repo: 'manta' }
      ],
      headRepo: { owner: 'fork', repo: 'manta' }
    })
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'contributor/fix',
          repo: {
            full_name: 'contributor/manta',
            name: 'manta',
            clone_url: 'https://github.com/contributor/manta.git',
            ssh_url: 'git@github.com:contributor/manta.git',
            owner: { login: 'contributor' }
          }
        }
      })
    })
    getRemoteUrlForRepoMock.mockResolvedValueOnce('git@github.com:fork/manta.git')

    await getPullRequestPushTarget('/repo-root', 1738, null, {}, 'origin')

    expect(resolvePRRepositoryCandidatesMock).not.toHaveBeenCalled()
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(['api', 'repos/fork/manta/pulls/1738'], {
      cwd: '/repo-root',
      host: 'github.com'
    })
    expect(ghExecFileAsyncMock).not.toHaveBeenCalledWith(
      ['api', 'repos/upstream/manta/pulls/1738'],
      expect.anything()
    )
  })
  ...
  it('keeps getRepoSlug origin-based on a fork checkout (#7331)', async () => {
    ...
    getOwnerRepoMock.mockResolvedValue({ owner: 'fsdwen', repo: 'manta' })
  ...
  it('resolves a distinct upstream remote as the repo upstream', async () => {
    ...
    getOwnerRepoMock.mockResolvedValue({ owner: 'tmchow', repo: 'manta' })
    getOwnerRepoForRemoteMock.mockImplementation(async (_repoPath: string, remoteName: string) =>
      remoteName === 'origin'
        ? { owner: 'tmchow', repo: 'manta' }
        : { owner: 'stablyai', repo: 'manta' }
    )
  ...
  it('does not treat a same-repository upstream remote as a fork', async () => {
    getOwnerRepoMock.mockResolvedValue({ owner: 'StablyAI', repo: 'Manta' })
    getOwnerRepoForRemoteMock.mockImplementation(async (_repoPath: string, remoteName: string) =>
      remoteName === 'origin'
        ? { owner: 'StablyAI', repo: 'Manta' }
        : { owner: 'stablyai', repo: 'manta' }
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ...
      ['repo', 'view', 'github.com/StablyAI/Manta', '--json', 'isFork,parent'],
  ...
  it('falls back to the GitHub parent when no upstream remote is configured', async () => {
    getOwnerRepoForRemoteMock
      .mockResolvedValueOnce({ owner: 'tmchow', repo: 'manta' })
      .mockResolvedValueOnce(null)
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        isFork: true,
        parent: { name: 'manta', owner: { login: 'stablyai' } }
      })
    })
  ...
  it('routes GHES push-target probes through the Enterprise host', async () => {
    const ghes = { owner: 'team', repo: 'manta', host: 'github.acme-corp.com' }
    ...
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'feature',
          repo: {
            full_name: 'team/manta',
            name: 'manta',
            clone_url: 'https://github.acme-corp.com/team/manta.git',
            ssh_url: 'git@github.acme-corp.com:team/manta.git',
            owner: { login: 'team' }
          }
        }
      })
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['api', 'repos/team/manta/pulls/7'],
      expect.objectContaining({ host: 'github.acme-corp.com' })
    )
  })

  it('does not confuse same-slug PR repositories across GitHub hosts', async () => {
    const enterprise = { owner: 'team', repo: 'manta', host: 'github.acme-corp.com' }
    const dotCom = { owner: 'team', repo: 'manta', host: 'github.com' }
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
      candidates: [enterprise, dotCom],
      headRepo: dotCom
    })
    getOwnerRepoForRemoteMock.mockResolvedValueOnce(dotCom)
    getRemoteUrlForRepoMock.mockResolvedValueOnce('git@github.com:team/manta.git')
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'feature',
    ...
    await expect(getPullRequestPushTarget('/repo-root', 7)).resolves.toEqual({
      pushTarget: {
        remoteName: 'pr-team-manta',
        branchName: 'feature',
        remoteUrl: 'git@github.acme-corp.com:team/manta.git'
      }
    })
  })
  ...
  it('probes additional PR repo candidates when the first lookup is not found', async () => {
    ...
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'fork', repo: 'manta' })
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          head: {
            ref: 'feature/test',
            repo: {
              full_name: 'fork/manta',
              name: 'manta',
              clone_url: 'https://github.com/fork/manta.git',
              ssh_url: 'git@github.com:fork/manta.git',
              owner: { login: 'fork' }
            }
          }
        })
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(1, ['api', 'repos/fork/manta/pulls/1849'], {
      cwd: '/repo-root'
    })
  ...
  it('probes the next PR work-item candidate after a permission denial', async () => {
    const upstream = { owner: 'upstream', repo: 'manta', host: 'github.com' }
    const origin = { owner: 'fork', repo: 'manta', host: 'github.com' }
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
    ...
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          title: 'Origin PR',
          state: 'OPEN',
          url: 'https://github.com/fork/manta/pull/42',
          labels: [],
    ...
    expect(ghExecFileAsyncMock.mock.calls[0][0]).toEqual(
      expect.arrayContaining(['pr', 'view', '--repo', 'upstream/manta'])
    )
    expect(ghExecFileAsyncMock.mock.calls[1][0]).toEqual(['api', 'repos/upstream/manta/pulls/42'])
    expect(ghExecFileAsyncMock.mock.calls[2][0]).toEqual(
      expect.arrayContaining(['pr', 'view', '--repo', 'fork/manta'])
    )
  })
