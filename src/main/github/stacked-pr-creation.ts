...
export async function prepareGitHubStackedPullRequest(
  ...
  try {
    ...
    if (currentPullRequests.length > 1) {
      return creationError(`Manta found multiple open pull requests for the current branch ${head}.`)
    }
    ...
    return {
      ...
      error: isStacksUnavailableError(error)
        ...
        : 'Manta could not verify the parent pull request. Retry in a moment.'
    }
  } finally {
    release()
  }
