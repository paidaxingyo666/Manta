...
describe('getPRForBranch', () => {
  ...
  it('uses the tracked upstream remote owner for fork branch lookup', async () => {
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
      candidates: [
        ...
        { owner: 'origin-owner', repo: 'manta' }
      ],
      headRepo: { owner: 'origin-owner', repo: 'manta' }
    })
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'fork-owner', repo: 'manta' })
    ghExecFileAsyncMock
        ...
        stdout: JSON.stringify([
          {
            ...
            state: 'open',
            html_url: 'https://github.com/stablyai/manta/pull/78',
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          state: 'OPEN',
          url: 'https://github.com/stablyai/manta/pull/78',
          statusCheckRollup: [],
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        'api',
        'repos/stablyai/manta/pulls?head=fork-owner%3Acontributor%2Foriginal&state=all&per_page=1'
      ],
      { cwd: '/repo-root' }
    )
    expect(pr).toMatchObject({
      number: 78,
      title: 'Hydrated fork upstream branch PR',
      prRepo: { owner: 'stablyai', repo: 'manta' },
      headRepo: { owner: 'fork-owner', repo: 'manta' }
    })
  })
  ...
  it('uses the tracked upstream remote owner when the fork branch name matches locally', async () => {
    ...
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'brennanb2025', repo: 'manta' })
    ghExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: JSON.stringify([]) })
      .mockResolvedValueOnce({ stdout: JSON.stringify([]) })
      .mockResolvedValueOnce({
        stdout: JSON.stringify([
          {
            ...
            state: 'open',
            html_url: 'https://github.com/stablyai/manta/pull/6433',
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          title: 'Recover Windows worktree deletes from long paths',
          state: 'OPEN',
          url: 'https://github.com/stablyai/manta/pull/6433',
          statusCheckRollup: [],
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        'api',
        'repos/stablyai/manta/pulls?head=brennanb2025%3Abrennanb2025%2Fworktree-remove-fix&state=all&per_page=1'
      ],
      { cwd: '/repo-root' }
    )
    expect(pr).toMatchObject({
      number: 6433,
      prRepo: { owner: 'stablyai', repo: 'manta' },
      headRepo: { owner: 'brennanb2025', repo: 'manta' }
    })
  })
  ...
  it('uses the same-name tracked upstream fork owner through the SSH git provider', async () => {
        ...
        stdout: JSON.stringify([
          {
            ...
            state: 'open',
            html_url: 'https://github.com/stablyai/manta/pull/79',
            updated_at: '2026-03-28T00:00:00Z',
            draft: false,
            mergeable: true,
            base: { ref: 'main', sha: 'base-oid' },
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        'api',
        'repos/stablyai/manta/pulls?head=fork-owner%3Acontributor%2Ffix&state=all&per_page=1'
      ],
      {}
    )
    expect(pr).toMatchObject({
