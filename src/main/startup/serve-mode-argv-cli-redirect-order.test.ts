...
const MOUNTED_APPIMAGE_ENV = { APPIMAGE: '/opt/manta/Manta.AppImage', APPDIR: '/tmp/.mount_ab12' }
...
describe('serve argv rewrite vs AppImage CLI redirect ordering', () => {
  const launchArgv = ['/opt/manta/manta-ide', '--no-sandbox', 'serve', '--port', '7777', '--json']
  ...
  it('leaves non-serve CLI commands redirectable either way', () => {
    const argv = ['/opt/manta/manta-ide', 'status']
