...
describe.runIf(process.platform === 'win32')('Windows shell PATH restoration', () => {
  ...
  beforeEach(() => {
    _resetHydrateShellPathCache()
    fixtureRoot = mkdtempSync(join(tmpdir(), 'manta-shell-path-'))
    ...
    writeFileSync(join(shellADir, 'manta-path-probe.cmd'), '@echo shell-a\r\n')
    writeFileSync(join(shellBDir, 'manta-path-probe.cmd'), '@echo shell-b\r\n')
  })
