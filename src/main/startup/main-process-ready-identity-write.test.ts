...
const mocks = vi.hoisted(() => ({
  ...
  profileDirectory: '',
  state: {
    devInstanceIdentity: { appUserModelId: 'app.id', appName: 'Manta' },
    isServeMode: false,
...
vi.mock('../browser/browser-route-session-runtime', () => ({
  configureRouteSessionsForMantaProfile: vi.fn()
}))
vi.mock('../browser/paired-runtime-browser-client-host-runtime', () => ({
  configurePairedRuntimeBrowserClientHostsForMantaProfile: vi.fn()
}))
      ...
      profiles: [
        {
          ...
          scope: 'isolated',
          partition: getMantaProfileBrowserSessionPartition(ORCA_PROFILE_ID, RETIRED_PROFILE_ID),
          label: 'Existing',
          source: null,
