...
describe('patchPackagedProcessPath', () => {
  ...
  it('prepends agent-CLI install dirs (~/.opencode/bin, ~/.vite-plus/bin) for packaged darwin runs', async () => {
    ...
    // Without them on PATH, GUI-launched Manta reports both as "Not installed"
...
describe('configureDevUserDataPath', () => {
  it('forces Electron home into the disposable E2E profile', async () => {
    ...
    const originalE2EUserDataDir = process.env.MANTA_E2E_USER_DATA_DIR
    const originalE2EHomeDir = process.env.MANTA_E2E_HOME_DIR
    const originalHome = process.env.HOME
    const originalUserProfile = process.env.USERPROFILE
    const tempRoot = mkdtempSync(join(tmpdir(), 'manta-configure-e2e-home-'))
    const e2eRoot = join(tempRoot, 'user-data')
    const e2eHome = join(tempRoot, 'home')
    process.env.MANTA_E2E_USER_DATA_DIR = e2eRoot
    process.env.MANTA_E2E_HOME_DIR = e2eHome
    ...
    try {
      ...
      if (originalE2EUserDataDir === undefined) {
        delete process.env.MANTA_E2E_USER_DATA_DIR
      } else {
        process.env.MANTA_E2E_USER_DATA_DIR = originalE2EUserDataDir
      }
      if (originalE2EHomeDir === undefined) {
        delete process.env.MANTA_E2E_HOME_DIR
      } else {
        process.env.MANTA_E2E_HOME_DIR = originalE2EHomeDir
      }
      if (originalHome === undefined) {
        delete process.env.HOME
      } else {
  ...
  it('rejects an E2E launch whose Node home escaped the disposable profile', async () => {
    ...
    const originalHome = process.env.HOME
    const originalUserProfile = process.env.USERPROFILE
    const e2eRoot = mkdtempSync(join(tmpdir(), 'manta-configure-e2e-escape-'))
    process.env.MANTA_E2E_USER_DATA_DIR = e2eRoot
    process.env.MANTA_E2E_HOME_DIR = join(e2eRoot, 'home')
    ...
    try {
      ...
      restoreEnv('MANTA_E2E_USER_DATA_DIR', originalE2EUserDataDir)
      restoreEnv('MANTA_E2E_HOME_DIR', originalE2EHomeDir)
  ...
  it('uses an explicit dev userData override when provided', async () => {
    const { app } = await import('electron')
    const { configureDevUserDataPath } = await import('./configure-process')
    const originalOverride = process.env.MANTA_DEV_USER_DATA_PATH
    process.env.MANTA_DEV_USER_DATA_PATH = '/tmp/manta-dev-repro'

    try {
      configureDevUserDataPath(true)
    } finally {
      if (originalOverride === undefined) {
        delete process.env.MANTA_DEV_USER_DATA_PATH
      } else {
        process.env.MANTA_DEV_USER_DATA_PATH = originalOverride
      }
    }
  ...
  it('moves dev runs onto a manta-dev userData path', async () => {
    ...
    // Why: production code uses path.join(app.getPath('appData'), 'manta-dev')
    // which produces platform-specific separators.
    expect(app.setPath).toHaveBeenCalledWith('userData', join('/tmp/app-data', 'manta-dev'))
  })
...
describe('configureMantaUserDataPathEnv', () => {
  it('overwrites stale inherited MANTA_USER_DATA_PATH with Electron userData', async () => {
    const { app } = await import('electron')
    const { configureMantaUserDataPathEnv } = await import('./configure-process')
    const originalUserDataPath = process.env.MANTA_USER_DATA_PATH
    process.env.MANTA_USER_DATA_PATH = '/tmp/stale-manta-user-data'
    app.setPath('userData', '/tmp/current-manta-user-data')
    let configuredUserDataPath: string | undefined

    try {
      configureMantaUserDataPathEnv()
      configuredUserDataPath = process.env.MANTA_USER_DATA_PATH
    } finally {
      if (originalUserDataPath === undefined) {
        delete process.env.MANTA_USER_DATA_PATH
      } else {
        process.env.MANTA_USER_DATA_PATH = originalUserDataPath
      }
    }

    expect(configuredUserDataPath).toBe('/tmp/current-manta-user-data')
  })
})
...
describe('configureElectronNetworkCompatibility', () => {
  ...
  const originalEnvValue = process.env.MANTA_DISABLE_HTTP2

  function createUserDataDir(settings: Record<string, unknown>): string {
    const userDataPath = mkdtempSync(join(tmpdir(), 'manta-http1-compat-'))
    tempDirs.push(userDataPath)
    writeFileSync(join(userDataPath, 'manta-data.json'), JSON.stringify({ settings }), 'utf-8')
    return userDataPath
  }
  ...
  afterEach(() => {
    ...
    if (originalEnvValue === undefined) {
      delete process.env.MANTA_DISABLE_HTTP2
    } else {
      process.env.MANTA_DISABLE_HTTP2 = originalEnvValue
    }
  })
...
describe('enableMainProcessGpuFeatures', () => {
  ...
  it('disables the GPU process for Linux E2E runs', async () => {
    ...
    setPlatform('linux')
    process.env.MANTA_E2E_USER_DATA_DIR = '/tmp/manta-e2e'
    vi.mocked(app.disableHardwareAcceleration).mockClear()
    vi.mocked(app.commandLine.appendSwitch).mockClear()
