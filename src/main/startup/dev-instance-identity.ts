...
const BASE_APP_NAME = 'Manta'
const BASE_APP_USER_MODEL_ID = 'cn.sh.manta'
...
export type DevInstanceIdentity = AppIdentity & {
  ...
  // creating a new one per branch and re-prompting. Distinct from prod's 'Manta'.
  appName: string
}
 ...
 * CFBundleName, which downstream forks ship differently ("Manta ALab Edition").
...
export function getDevInstanceIdentity(
  ...
  const repoRoot = cleanEnvValue(env.MANTA_DEV_REPO_ROOT)
  const branch = cleanEnvValue(env.MANTA_DEV_BRANCH)
  const worktreeName =
    cleanEnvValue(env.MANTA_DEV_WORKTREE_NAME) ??
    cleanEnvValue(path.basename(repoRoot ?? process.cwd()))
  const devLabel = cleanEnvValue(env.MANTA_DEV_INSTANCE_LABEL) ?? formatLabel(branch, worktreeName)
  const dockTitle =
    cleanEnvValue(env.MANTA_DEV_DOCK_TITLE) ?? `${BASE_APP_NAME}: ${branch ?? devLabel ?? 'dev'}`

  return {
    name: dockTitle,
    // Why: one stable Keychain key ('Manta Dev Safe Storage') for all dev
