...
const DELIMITER = '__MANTA_SHELL_PATH__'
...
const PROBE_MARKER_ENV_VAR = 'MANTA_SHELL_PATH_PROBE'
...
/** Run a synchronous launcher without passing Manta's seeded PATH to its child. */
...
function spawnShellAndReadPath(shell: string): Promise<HydrationResult> {
  return new Promise((resolve) => {
    ...
    const child = spawn(shell, probe.args, {
      ...
      // and we don't want that in Manta's console.
      env: shellProbeEnv(),
