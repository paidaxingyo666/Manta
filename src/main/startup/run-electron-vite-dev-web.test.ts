...
function stashWebBuild(): () => void {
  ...
  const tempDir = mkdtempSync(join(dirname(outWebPath), '.manta-dev-web-stash-'))
    ...
    try {
      const wrapper = spawn(process.execPath, [wrapperPath, '--remote-debugging-port=9446'], {
        ...
        env: {
          ...
          MANTA_DEV_WRAPPER_TEST_VITE_FILE: viteFile
        },
