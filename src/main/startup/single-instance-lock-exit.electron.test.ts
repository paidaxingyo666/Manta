...
// duplicate headless `manta serve` kept executing the rest of startup, reached Linux Ozone/X11 init
...
const MARKER_ENV = 'MANTA_LOCK_FIXTURE_MARKER'
...
function runLockLossGate(termination: string): FixtureRun {
  const root = mkdtempSync(join(tmpdir(), 'manta-lock-loss-'))
  ...
  writeFileSync(
    ...
    '{ "name": "manta-lock-loss-fixture", "main": "main.js" }'
  )
