...
describe('acquireSingleInstanceLock', () => {
  ...
  it('forwards the second launch argv so the owner can decide whether to activate', () => {
    ...
    registered?.({}, ['/opt/manta/manta-linux.AppImage', '--serve'], '/home/manta')

    expect(onSecondInstance).toHaveBeenCalledTimes(1)
    expect(onSecondInstance).toHaveBeenCalledWith(['/opt/manta/manta-linux.AppImage', '--serve'])
  })
})
...
describe('shouldActivateDesktopForSecondInstance', () => {
  it('ignores a duplicate serve launch but still activates for a desktop launch', () => {
    // Why: a supervisor respawning `manta serve` must not open a window on a display-less host (#11935).
    const serveArgv = ['/opt/manta/manta-linux.AppImage', '--serve']
    expect(shouldActivateDesktopForSecondInstance(serveArgv)).toBe(false)
    expect(shouldActivateDesktopForSecondInstance(['/Applications/Manta.app/manta'])).toBe(true)
  })
  ...
  it('ignores a duplicate CLI-form serve launch the CLI redirect never rewrote', () => {
    ...
    expect(
      shouldActivateDesktopForSecondInstance([
        '/opt/manta/squashfs-root/manta-ide',
        'serve',
        '--port',
        '6768',
        '--pairing-address',
    ...
    expect(
      shouldActivateDesktopForSecondInstance(['/opt/manta/manta-ide', '/home/u/serve-repo'])
    ).toBe(true)
  })
  ...
  it('lets isolated E2E exercise the production single-instance path', () => {
    expect(
      shouldSkipSingleInstanceLock({
        isDev: true,
        isServeMode: false,
        env: { MANTA_E2E_ENFORCE_SINGLE_INSTANCE_LOCK: '1' }
      })
    ).toBe(false)
  })
})
...
describe('shouldBypassSingleInstanceLock', () => {
  it('allows the hidden diagnostic bypass only for packaged macOS app launches', () => {
    expect(
      shouldBypassSingleInstanceLock({
        env: { MANTA_BYPASS_SINGLE_INSTANCE_LOCK: '1' },
        isDev: false,
        isServeMode: false,
        platform: 'darwin'
      })
