import { getOrcaPushGatewayUrl } from '../manta-profiles/profile-cloud-auth-config'
...
export function startDesktopPushService(runtimeRpc: MantaRuntimeRpcServer): void {
  const runtime: MantaRuntimeService | null = state.runtime
  if (!runtime) {
