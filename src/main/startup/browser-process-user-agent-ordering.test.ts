...
const mocks = vi.hoisted(() => {
  const events: string[] = []
  // Why a two-word app token: this file sets the dev app name to "Manta Development", and Electron
  ...
  let userAgent =
    'Mozilla/5.0 (Test) AppleWebKit/537.36 (KHTML, like Gecko) Manta Development/0.0.0 Chrome/150.0.0.0 Electron/43.0.0 Safari/537.36'
  const app = {
    isPackaged: false,
    exit: vi.fn(),
...
vi.mock('./configure-process', () => ({
  ...
  configureMantaUserDataPathEnv: vi.fn(),
...
describe('browser process user-agent startup ordering', () => {
  it('executes after the dev app name and before later preflight work', async () => {
    ...
    const nameIndex = mocks.events.indexOf('set-name:Manta Development')
    ...
    // Both app-name words must be gone, not just the last: a single \S+ would have left "Manta".
    expect(mocks.userAgent()).not.toMatch(/Electron/)
    expect(mocks.userAgent()).not.toMatch(/Manta|Development/)
  })
})
