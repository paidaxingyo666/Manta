...
const HTTP1_COMPATIBILITY_ENV_VAR = 'MANTA_DISABLE_HTTP2'
...
function readPersistedHttp1CompatibilityMode(userDataPath: string): boolean {
  const dataFile = join(userDataPath, 'manta-data.json')
...
export function shouldDisableHttp2ForElectronNetworking(
  ...
  // manta-data.json the Store parses again moments later. The marker is refreshed whenever settings
  ...
  if (process.platform !== 'win32') {
    ...
    if (home) {
      appendPaths.push(
        ...
        // Why: some agent CLIs install into ~/.<name>/bin; GUI-launched Electron's minimal PATH misses them (stablyai/manta#829).
...
export function configureDevUserDataPath(isDev: boolean): void {
  ...
  if (e2eConfig.userDataDir) {
    ...
    const e2eHomeDir = process.env.MANTA_E2E_HOME_DIR ?? join(e2eConfig.userDataDir, 'home')
  ...
  const overrideUserDataPath = process.env.MANTA_DEV_USER_DATA_PATH
  ...
  // Why: without a dev-only path, pnpm dev overwrites the packaged app's runtime pointer under userData and breaks the manta CLI.
  app.setPath('userData', join(app.getPath('appData'), 'manta-dev'))
}
...
export function configureMantaUserDataPathEnv(): void {
  // Why: relaunches can inherit a stale MANTA_USER_DATA_PATH; canonicalize before CLI-shared modules build runtime-home paths.
  process.env.MANTA_USER_DATA_PATH = app.getPath('userData')
}

export function shouldInstallManagedHooks(isDev: boolean): boolean {
  void isDev
  // Why: managed hooks now target Manta-owned Codex homes, not ~/.codex, so keep install on for all agents until each gets its own seam.
  return true
}
...
export function installDevParentWatchdog(isDev: boolean): void {
  ...
  const timer = setInterval(() => {
    ...
    if (parentPidChanged || parentMissing) {
      clearInterval(timer)
      // Why: the dev runner spawns Electron without IPC, so on macOS Ctrl+C leaves Manta open; watch the parent PID to couple shutdown.
      requestDevParentShutdown()
    }
  }, 1000)
