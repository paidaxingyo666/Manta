...
const INSTALL_DIR = 'C:\\Users\\neil\\AppData\\Local\\Programs\\manta'
    ...
    probeWindowsInstallDirAcl({
      ...
      onDone: (data) => {
        startWindowsInstallDirAclRepairIfPoisoned(data, {
          ...
          userDataPath: mkdtempSync(join(tmpdir(), 'manta-acl-recovery-')),
...
describe('describeInstallDirAclPoison', () => {
  ...
  it('offers the copyable commands, and drops them once the repair lands', async () => {
    ...
    expect(repaired?.detail).toContain('Manta repaired the permissions')
...
describe('install-dir ACL repair vs the GPU safe-graphics marker', () => {
  ...
  it('clears the sticky safe-graphics marker once the real cause is repaired', async () => {
    ...
    // GPU child three times, so Manta latched safe graphics for this build.
  ...
  // only retires the latch Manta engaged on its own.
...
// finished tree from one Manta gave up on. Both callers hold outstanding poison evidence —
...
describe('a repair marker recording a completed repair', () => {
  ...
  // as "Manta could not fix this", never as a repair it never made.
