...
export const SINGLE_INSTANCE_LOCK_FAILURE_MESSAGE =
  '[single-instance] Another Manta instance is already running for this userData profile; exiting this launch after requesting the existing window. If no Manta process is running, this may be an Electron/macOS single-instance lock failure.'
export const SINGLE_INSTANCE_LOCK_BYPASS_ENV = 'MANTA_BYPASS_SINGLE_INSTANCE_LOCK'
export const SINGLE_INSTANCE_LOCK_E2E_ENFORCE_ENV = 'MANTA_E2E_ENFORCE_SINGLE_INSTANCE_LOCK'
export const SINGLE_INSTANCE_LOCK_BYPASS_MESSAGE =
  '[single-instance] MANTA_BYPASS_SINGLE_INSTANCE_LOCK=1 is set; bypassing the packaged macOS single-instance lock for diagnostics. Do not use this with another Manta instance running for the same profile.'
...
// Why: a duplicate `manta serve` is a supervisor artifact, not a user asking for a window; fail open when argv is unavailable.
...
/**
 * Why: Manta writes two canonical discovery files into `<userData>/`:
 * `manta-runtime.json` (RPC endpoint + authToken for the bundled CLI) and
 ...
 * instance quits, metadata points at a dead pid and `manta status` reports
 ...
 * way dev (`manta-dev` userData) and packaged (`manta` userData) runs lock in
