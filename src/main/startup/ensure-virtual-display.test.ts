...
describe('ensureVirtualDisplayForHeadlessServe', () => {
  ...
  // leaves a lockless socket on Manta's OWN :99. Adopting it would resurrect the orphan-socket bug.
  ...
  // A root-owned stale :99 socket (crashed system Xvfb, serve running as User=manta) cannot be
  ...
  describe('hasUsableLinuxDisplay', () => {
    ...
    // Manta's own teardown unlinks the lock before the socket, so a lockless :99 is our own
