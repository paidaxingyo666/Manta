...
describe('headless PTY registry hydration ordering', () => {
  ...
  it('captures mantad status identity at ingest for fleet stale-row fencing', () => {
    const source = readFileSync(join(process.cwd(), 'src/main/mantad/mantad-entry.ts'), 'utf8')
    const runtime = source.indexOf('const runtime = new MantaRuntimeService(')
  ...
  it('captures spool-replayed identity after the mantad runtime is ready', () => {
    const source = readFileSync(join(process.cwd(), 'src/main/mantad/mantad-entry.ts'), 'utf8')
