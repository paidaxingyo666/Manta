...
describe('AppImage CLI redirect', () => {
  it('detects direct AppImage CLI commands', () => {
    expect(
      getAppImageCliArgs(
        ['manta-linux.AppImage', 'status', '--json'],
        { APPIMAGE: '/opt/manta' },
        {
          platform: 'linux',
          isPackaged: true,
  ...
  it('allows CLI global flags before the command', () => {
    expect(
      getAppImageCliArgs(
        ['manta-linux.AppImage', '--pairing-code', 'abc123', '--json', 'terminal', 'list'],
        {
          APPIMAGE: '/opt/manta'
        },
        {
  ...
  it('spawns the unpacked CLI entrypoint with Electron node mode', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-appimage-cli-redirect-'))
    ...
    const result = maybeRedirectAppImageCliLaunch({
      argv: ['manta-linux.AppImage', 'status', '--json'],
      env: {
        APPIMAGE: '/opt/manta/manta-linux.AppImage',
    ...
    expect(spawn).toHaveBeenCalledWith('/opt/manta/manta-ide', [cliEntryPath, 'status', '--json'], {
      env: expect.objectContaining({
        APPIMAGE: '/opt/manta/manta-linux.AppImage',
        ELECTRON_RUN_AS_NODE: '1',
        MANTA_NODE_OPTIONS: '--inspect',
        MANTA_NODE_REPL_EXTERNAL_MODULE: '/tmp/repl.js'
      }),
      stdio: 'inherit'
    })
  ...
  it('forwards an explicit no-sandbox choice to the serve child', async () => {
    ...
    maybeRedirectAppImageCliLaunch({
      argv: ['manta-linux.AppImage', '--no-sandbox', 'serve'],
      env: { APPIMAGE: '/opt/manta/manta-linux.AppImage' },
      platform: 'linux',
      isPackaged: true,
      resourcesPath: root,
      execPath: '/opt/manta/manta-ide',
    ...
    expect(spawn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.objectContaining({ MANTA_APPIMAGE_NO_SANDBOX: '1' })
      })
    )
  })
})
