...
describe('run-electron-vite-dev', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), 'manta-dev-wrapper-'))
      ...
      const wrapper = spawn(process.execPath, [wrapperPath], {
        ...
        env: devWrapperTestEnv({
          MANTA_ELECTRON_VITE_CLI: fakeCliPath,
          MANTA_SKIP_DEV_CLI_PREPARE: '1',
          MANTA_SKIP_DEV_ELECTRON_APP_PREPARE: '1',
          MANTA_SKIP_DEV_WEB_PREPARE: '1',
          MANTA_DEV_WRAPPER_TEST_PID_FILE: pidFile
        }),
        stdio: 'ignore'
      })
  ...
  it('forwards dev instance identity to electron-vite', async () => {
    ...
    const wrapper = spawn(process.execPath, [wrapperPath, '--remote-debugging-port=9444'], {
      ...
      env: devWrapperTestEnv({
        ...
        MANTA_SKIP_DEV_WEB_PREPARE: '1',
        MANTA_DEV_WRAPPER_TEST_PID_FILE: pidFile,
        MANTA_DEV_WRAPPER_TEST_ENV_FILE: envFile,
    ...
    expect(envSnapshot.dockTitle).toBe('Manta: feature/billing-shell')
  ...
  it.skipIf(process.platform === 'win32')(
    'prepares userData manta and manta-dev wrappers for dev terminals',
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), 'manta-dev-wrapper-'))
      ...
      const wrapper = spawn(process.execPath, [wrapperPath], {
        cwd: resolve('.'),
        env: devWrapperTestEnv({
          MANTA_DEV_USER_DATA_PATH: userDataPath,
          MANTA_ELECTRON_VITE_CLI: fakeCliPath,
          MANTA_SKIP_DEV_ELECTRON_APP_PREPARE: '1',
          MANTA_SKIP_DEV_WEB_PREPARE: '1',
          MANTA_DEV_WRAPPER_TEST_PID_FILE: pidFile,
      ...
      const trackedPids = trackPidFile(pidFile)
      const devWrapper = readFileSync(join(userDataPath, 'cli', 'bin', 'manta-dev'), 'utf8')
      const publicAliasWrapper = readFileSync(join(userDataPath, 'cli', 'bin', 'manta'), 'utf8')
      expect(publicAliasWrapper).toBe(devWrapper)
      expect(publicAliasWrapper).toContain('MANTA_USER_DATA_PATH')
    ...
    const wrapper = spawn(
      ...
      [wrapperPath, '--stable-name', '--remote-debugging-port=9445'],
      {
        ...
        env: devWrapperTestEnv({
          ...
          MANTA_DEV_WRAPPER_TEST_PID_FILE: pidFile,
          MANTA_DEV_WRAPPER_TEST_ENV_FILE: envFile,
          MANTA_DEV_BRANCH: 'feature/stable-name',
          MANTA_DEV_WORKTREE_NAME: 'stable-ui'
        }),
        stdio: 'ignore'
      }
    )
  ...
  it.skipIf(process.platform !== 'darwin')(
    ...
    async () => {
      ...
      const baseEnv = devWrapperTestEnv({
        ...
        MANTA_SKIP_DEV_CLI_PREPARE: '1',
        MANTA_SKIP_DEV_WEB_PREPARE: '1',
        MANTA_DEV_BRANCH: 'feature/rebuild-electron-app',
        MANTA_DEV_WORKTREE_NAME: 'electron-app-rebuild'
      })
    ...
    async () => {
      ...
      const wrapper = spawn(process.execPath, [wrapperPath, '--remote-debugging-port=9448'], {
        ...
        env: devWrapperTestEnv({
          ...
          MANTA_DEV_WRAPPER_TEST_PID_FILE: pidFile,
          MANTA_DEV_WRAPPER_TEST_ENV_FILE: envFile,
          MANTA_DEV_BRANCH: 'feature/framework-symlinks',
          MANTA_DEV_WORKTREE_NAME: 'symlink-ui'
        }),
        stdio: 'ignore'
      })
