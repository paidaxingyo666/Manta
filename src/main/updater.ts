...
function deferHeadlessServeInstall(phase: 'download' | 'install', version: string): boolean {
  ...
  if (lastInstallDeferralVersion[phase] !== diagnosticVersion) {
    ...
    recordUpdaterLifecycle(
      ...
      {
        level: 'warn',
        message: 'Update install deferred while hosting manta serve'
      }
    )
  }
  sendErrorStatus(
    'This manta serve process was not started by an update-capable supervisor. Keep it running and update Manta through its service manager.',
    true
  )
  return true
}
  ...
  try {
    await withUpdaterSpan({ stage: 'install' }, async (span) => {
      ...
      if (
        ...
        sendErrorStatus(
          'Could not prepare the supervised server restart. Manta remains running.',
          true
        )
...
function scheduleAutomaticUpdateCheck(delayMs: number): void {
  ...
  autoUpdateCheckTimer = setTimeout(() => {
    // Why: Manta runs for days, so keep the next background check scheduled in the main process rather than tying it to relaunches or renderer lifetime.
...
async function checkForPinnedBuild(channel: ReleaseChannel, tag: string): Promise<void> {
  ...
  if (
    ...
    sendStatus({
      state: 'error',
      message: `${RELEASE_CHANNEL_LABELS[channel]} builds are unsigned, and this signed build only installs updates signed by Manta's publisher. Download the installer from the release page and run it once — updates work normally from there, including back to Stable.`,
      userInitiated: true
    })
    return
  }
...
const LINUX_PACKAGE_RECOVERY_MESSAGES: Record<LinuxPackageRecoveryUnavailableReason, string> = {
  ...
  'hash-mismatch':
    'The downloaded package no longer matches the verified release, so Manta will not hand it to a package manager. Download the update again, or get it from the official release page.',
  'read-failed':
    'Manta could not read the downloaded package. Download the update again, or get it from the official release page.',
  'no-sudo':
    'No sudo command was found in the system directories, so Manta cannot build a safe install command. Show the package and install it with your package manager.',
  'no-package-manager':
    'No supported package manager was found in the system directories, so Manta cannot build a safe install command. Show the package and install it with your package manager.',
  // Defensive: capture only ever tracks absolute cache paths, so this reports a bug rather than a machine state.
  'invalid-package-path':
    'The downloaded package is not at a usable path, so Manta cannot build a safe install command. Show the package and install it with your package manager.'
}
