...
function makeBase(): string {
  return mkdtempSync(join(tmpdir(), 'manta-wrapper-address-'))
}

describe('resolveShellWrapperRoot', () => {
  it('gives builds with different wrapper contents different trees', () => {
    const base = makeBase()
    expect(resolveShellWrapperRoot(base, builderFor('MANTA_SHELL_READY_MARKER'))).not.toEqual(
      resolveShellWrapperRoot(base, builderFor('MANTA_SHELL_FEATURES'))
    )
  })
