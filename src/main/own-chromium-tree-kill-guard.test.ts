...
/** Manta's renderer is a direct child of the main process, so the ppid walk says `own`. */
const PROCESS_ROWS = [
...
describe('refusing to tree-kill our own Chromium processes', () => {
  ...
  it.each([
    ...
    'refuses a Manta renderer from a daemon host with %s, because no Chromium descends from it',
    (_case, ownChromiumPids) => {
      // The standalone daemon and mantad install no Chromium-backed AppEnvironment,
  ...
  /**
   * Fail-open is the deliberate choice — see `manta-chromium-process-pids.ts` for
