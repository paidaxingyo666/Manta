...
function expectFinalZdotdirRestoreContext(content: string) {
  ...
  expect(content).toContain('export ZDOTDIR="$_manta_resolved_config_dir"')
}
...
describePosix('daemon shell-ready launch config', () => {
  ...
  it('falls back to HOME for MANTA_ORIG_ZDOTDIR when inherited ZDOTDIR points at a wrapper dir', async () => {
    // Why: an Manta-PTY parent has ZDOTDIR=.../shell-ready/zsh; propagating it makes the wrapper source itself (recursion loop).
  ...
  it('uses inherited MANTA_ORIG_ZDOTDIR when ZDOTDIR is an Manta wrapper dir', async () => {
    ...
    process.env.ZDOTDIR = '/some/other/manta/shell-ready/zsh'
    process.env.MANTA_ORIG_ZDOTDIR = userZdotdir
    process.env.HOME = userDataPath
    try {
      const { getShellReadyLaunchConfig } = await importFreshShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
      expect(config.env.MANTA_ORIG_ZDOTDIR).toBe(userZdotdir)
      expect(config.env.MANTA_ZSHENV_SOURCE_DIR).toBe(userDataPath)
    } finally {
      if (previousZdotdir === undefined) {
        delete process.env.ZDOTDIR
      } else {
  ...
  it('writes zsh wrappers that guard against MANTA_ORIG_ZDOTDIR self-loops', async () => {
    ...
    expect(zshenv).toContain('__manta_resolve_inherited_config_dir "${MANTA_ORIG_ZDOTDIR:-$HOME}"')
    expect(zshenv).toContain('printf "\\033]777;manta-shell-start:%s\\007" "$$"')
    expect(zshenv).toContain('"$_manta_resolved_config_dir" == */shell-ready/zsh ]]; then')
    expect(zshenv).toContain('export MANTA_ORIG_ZDOTDIR="$_manta_resolved_config_dir"')
    ...
    expect(zshrc).toContain('(( ${+functions[__manta_shell_epilogue]} )) && __manta_shell_epilogue')
  })
  ...
  it('owns zle-line-init for the shell-ready marker instead of an azhw hook', async () => {
    ...
    const zshenv = readFileSync(join(getShellReadyWrapperRoot(), 'zsh', '.zshenv'), 'utf8')
    expect(zshenv).toContain('zle -N zle-line-init __manta_prompt_mark')
    expect(zshenv).toContain('__manta_prev_line_init_fn="${widgets[zle-line-init]#user:}"')
    expect(zshenv).toContain('printf "\\033]777;manta-shell-ready\\007"')
    ...
    expect(zshenv).toContain('== "user:__manta_prompt_mark"')
  })
  ...
  it('preserves a real inherited ZDOTDIR as MANTA_ORIG_ZDOTDIR', async () => {
    ...
    try {
      ...
      const config = getShellReadyLaunchConfig('/bin/zsh')
      expect(config.env.MANTA_ORIG_ZDOTDIR).toBe(userZdotdir)
      expect(config.env.MANTA_ZSHENV_SOURCE_DIR).toBe(userZdotdir)
    } finally {
      if (previousZdotdir === undefined) {
        delete process.env.ZDOTDIR
      } else {
  ...
  it('sources user .zshenv at wrapper top level before repinning ZDOTDIR', async () => {
    ...
    expect(zshenv).toContain('__manta_resolve_inherited_config_dir "${MANTA_ZSHENV')
  ...
  it('preserves spawn-env MANTA_ORIG_ZDOTDIR as fallback when discovery yields nothing', async () => {
    ...
    expect(zshenv).toContain('_manta_user_zdotdir="$_manta_resolved_config_dir"')
