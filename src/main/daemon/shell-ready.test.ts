...
describePosix('daemon shell-ready launch config', () => {
  ...
  it('sets no MANTA_ORIG_ZDOTDIR when the inherited ZDOTDIR points at a wrapper dir', async () => {
    ...
    try {
      const { getShellReadyLaunchConfig } = await importFreshShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
      expect(config.env.MANTA_ORIG_ZDOTDIR).toBeUndefined()
    } finally {
  ...
  it('sets no MANTA_ORIG_ZDOTDIR when the inherited one points at a wrapper dir', async () => {
    const previousZdotdir = process.env.ZDOTDIR
    const previousOrigZdotdir = process.env.MANTA_ORIG_ZDOTDIR
    const previousHome = process.env.HOME
    delete process.env.ZDOTDIR
  ...
  it('writes a zsh hook that hands ZDOTDIR back before any user file loads', async () => {
    ...
    expect(zshenv).toContain('builtin export ZDOTDIR="$MANTA_ORIG_ZDOTDIR"')
    expect(zshenv).toContain('builtin unset MANTA_ORIG_ZDOTDIR MANTA_ZSHENV_SOURCE_DIR')
    expect(zshenv).toContain('printf "\\033]777;manta-shell-start:%s\\007" "$$"')
    expect(zshenv.indexOf('builtin export ZDOTDIR=')).toBeLessThan(
      zshenv.indexOf('builtin source -- "$_manta_user_zshenv"')
    )
  ...
  it('sources the user .zshenv at wrapper top level, not inside a function', async () => {
    ...
    expect(zshenv).toContain('builtin source -- "$_manta_user_zshenv"')
    ...
    expect(zshenv.indexOf('__manta_deferred_init() {')).toBeLessThan(
      zshenv.indexOf('builtin source -- "$_manta_user_zshenv"')
    )
  })
})
