...
function getShellReadyWrapperBaseDir(): string {
  ...
  // MANTA_USER_DATA_PATH, and daemon startup must not fail before the parent can
  ...
  return join(userDataPath || tmpdir(), userDataPath ? 'shell-wrappers' : 'manta-shell-wrappers')
}
...
// MANTA_USER_DATA_PATH is ever re-pointed mid-process.
...
export function getShellLaunchConfig(
  ...
  if (shellName === 'zsh') {
    ...
    if (!ensureShellReadyWrappers()) {
      ...
      // zsh skip the user's whole config. Losing Manta's features is recoverable.
      return { args: ['-l'], env: {}, supportsReadyMarker: false }
    }
    return {
      args: ['-l'],
      env: {
        MANTA_ORIG_ZDOTDIR: resolveInheritedZdotdir(process.env),
        MANTA_ZSHENV_SOURCE_DIR: resolveInheritedZshenvSourceDir(process.env),
