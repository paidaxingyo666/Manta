 ...
 * Files\...`, `/Applications/Manta 2.app/...`) cannot be split back into argv unambiguously.
 */
    ...
    try {
      ...
      const child = fork(
        ...
        ],
        {
          ...
          // Why: run the byte-identical relocated Manta.exe so the image path sits outside the updater's kill zone.
...
export async function initDaemonPtyProvider(
  ...
  const e2eInitDelayMs = Number(process.env.MANTA_E2E_DAEMON_INIT_DELAY_MS)
 ...
 * reported mantad's version for both would hide exactly that.
 */
...
// Leave history sessions marked "unclean" so a daemon crash while Manta is closed stays recoverable.
