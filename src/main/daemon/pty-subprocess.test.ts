...
describe('createPtySubprocess', () => {
  ...
  it('derives shell-ready launch config from the resolved fallback shell', async () => {
    ...
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')
    const previousShell = process.env.SHELL
    const previousFeatures = process.env.MANTA_SHELL_FEATURES
    ...
    try {
      ...
      // ZDOTDIR and MANTA_SHELL_FEATURES; /bin/sh must spawn without them.
      expect(spawnOptions.env.ZDOTDIR).toBeUndefined()
      expect(spawnOptions.env.MANTA_SHELL_FEATURES).toBeUndefined()
