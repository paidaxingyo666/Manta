...
describe('createPtySubprocess', () => {
  ...
  it('derives shell-ready launch config from the resolved fallback shell', async () => {
    ...
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')
    const previousShell = process.env.SHELL
    const previousMarker = process.env.MANTA_SHELL_READY_MARKER
    ...
    delete process.env.MANTA_SHELL_READY_MARKER
    ...
    try {
      ...
      // ZDOTDIR and MANTA_SHELL_READY_MARKER; /bin/sh must spawn without them.
      expect(spawnOptions.env.ZDOTDIR).toBeUndefined()
      expect(spawnOptions.env.MANTA_SHELL_READY_MARKER).toBeUndefined()
      ...
      if (previousMarker === undefined) {
        ...
        process.env.MANTA_SHELL_READY_MARKER = previousMarker
      }
    ...
    try {
      await expect(
        createPtySubprocess({
          ...
          cols: 80,
          rows: 24,
          cwd: '/definitely-missing-manta-cwd'
        })
      ).rejects.toThrow(/definitely-missing-manta-cwd/)
    } finally {
      if (platform) {
        Object.defineProperty(process, 'platform', platform)
      }
  ...
  it('combines HOMEDRIVE and HOMEPATH for Windows default cwd', async () => {
    ...
    process.env.HOMEPATH = '\\Users\\manta'
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      expect.any(String),
      expect.any(Array),
      expect.objectContaining({ cwd: 'D:\\Users\\manta' })
    )
  })
})
