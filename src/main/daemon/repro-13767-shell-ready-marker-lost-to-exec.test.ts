...
const COMMAND_OUTPUT = 'MANTA_STARTUP_COMMAND_RAN'
...
const READ_STARTED_FILE = '.manta-read-started'
...
const FIXTURES: ShellFixture[] = [
  {
    ...
    command: `printf 'MANTA_STARTUP_%s:PF=%s\\n' COMMAND_RAN "\${(j:,:)precmd_functions}"\r`,
  ...
  },
  {
    ...
    command: `printf 'MANTA_STARTUP_%s:PC=%s\\n' COMMAND_RAN "$PROMPT_COMMAND"\r`,
...
async function startFixture(
  ...
  const tempHome = mkdtempSync(join(tmpdir(), 'manta-shell-ready-exec-'))
  ...
  try {
    ...
    subprocess = await createPtySubprocess({
      ...
      envToDelete: ['MANTA_EXEC_REPRO_DONE', 'MANTA_ORIG_ZDOTDIR', 'ZDOTDIR']
    })
    session = new Session({
      sessionId: `repro-13767-${fixture.startupFile}`,
      cols: 80,
...
function restoreEnvironment(
  ...
  setEnvironmentValue('MANTA_ORIG_ZDOTDIR', originalZdotdir)
}
...
async function runExecOracle(fixture: ShellFixture): Promise<void> {
  const running = await startFixture(
    fixture,
    `if [[ -z "\${MANTA_EXEC_REPRO_DONE:-}" ]]; then
  export MANTA_EXEC_REPRO_DONE=1
  ${fixture.replacement}
fi
`
  )
  try {
    ...
    expect(running.output()).not.toContain('manta-shell-start')
  } finally {
...
`,
  ...
  const alternateBashProfile = `if [[ -z "\${MANTA_EXEC_REPRO_DONE:-}" ]]; then
  export MANTA_EXEC_REPRO_DONE=1
