...
describe('createPtySubprocess', () => {
  ...
  it.each([
    ...
    ['drops an inherited Manta fish_history', undefined, undefined],
    ['keeps the session this spawn injected', 'manta_c0ffee', 'manta_c0ffee'],
    ['keeps a caller-supplied value', 'mine', 'mine']
  ])('%s', async (_name, requested, expected) => {
    spawnMock.mockReturnValue(mockPtyProcess())
    const saved = process.env.fish_history
    process.env.fish_history = 'manta_abc123'
  ...
  it.each([
    // MANTA_HISTFILE is exported into every pane, so a daemon started from an
    // Manta pane inherits one. Left in place it BOTH re-scopes the pane to
    ...
    ['drops an inherited Manta MANTA_HISTFILE', undefined, undefined],
    [
      'keeps the path this spawn injected',
      '/fake/userData/terminal-history/00112233445566aa/zsh_history',
      '/fake/userData/terminal-history/00112233445566aa/zsh_history'
    ...
    spawnMock.mockReturnValue(mockPtyProcess())
    const saved = process.env.MANTA_HISTFILE
    process.env.MANTA_HISTFILE = '/fake/userData/terminal-history/aabbccddeeff0011/zsh_history'
    ...
    try {
      await createPtySubprocess({
        ...
        ...(requested === undefined ? {} : { env: { MANTA_HISTFILE: requested } })
      })
    } finally {
      if (saved === undefined) {
        delete process.env.MANTA_HISTFILE
      } else {
        process.env.MANTA_HISTFILE = saved
      }
    }

    const env = spawnMock.mock.calls.at(-1)?.[2].env
    expect(env.MANTA_HISTFILE).toBe(expected)
    // The wrapping consequence: no inherited value may point a pane at Manta's
    // ZDOTDIR that the client scoped no history for.
    expect(env.MANTA_SHELL_FEATURES).toBe(expected === undefined ? undefined : 'history')
  })
