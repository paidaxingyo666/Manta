 ...
 * run-as-node Manta.exe copy (not node.exe) so there's no console flash and asar still resolves. Fail-open:
...
// LOCAL appData (not roaming) so OneDrive/roaming never syncs this ~260MB runtime. Shared with NSIS uninstall (config/nsis/manta-installer-hooks.nsh) — keep in sync.
const LOCAL_HOST_ROOT_NAME = 'Manta'

// Copy of Manta.exe renamed to a distinct image name so the NSIS updater's `taskkill /IM Manta.exe` can't match it.
const DAEMON_HOST_EXE_NAME = 'manta-terminal-daemon.exe'

// V8 snapshots + ICU data the Electron bootstrap reads even under ELECTRON_RUN_AS_NODE; siblings of Manta.exe.
