...
export function stubMissingDaemonCwd(): {
  ...
  const missingDaemonCwd = join(tmpdir(), 'manta-daemon-cwd-that-does-not-exist')
...
export function useDaemonPtySubprocessEnv(mocks: PtySubprocessSpawnMocks): {
  ...
  const savedWrapperEnv: Partial<Record<(typeof MANTA_SHELL_WRAPPER_ENV)[number], string>> = {}
  ...
  beforeEach(() => {
    ...
    process.env.MANTA_USER_DATA_PATH = state.userDataPath
    delete process.env[POWERLEVEL10K_WIZARD_DISABLE_ENV]
    for (const key of MANTA_SHELL_WRAPPER_ENV) {
