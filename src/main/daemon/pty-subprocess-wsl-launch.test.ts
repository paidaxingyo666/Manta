...
describe('createPtySubprocess', () => {
    ...
    try {
      await expect(
        createPtySubprocess({
          ...
          cols: 80,
          rows: 24,
          cwd: 'C:\\definitely-missing-manta-wsl-cwd',
          shellOverride: 'wsl.exe'
        })
      ).rejects.toThrow(/Working directory "C:\\definitely-missing-manta-wsl-cwd" does not exist/)
    } finally {
      if (platform) {
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.not.objectContaining({
          ...
          MANTA_CODEX_HOME: expect.anything()
        })
      })
    )
  })
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.objectContaining({
          CODEX_HOME: '/home/jin/.local/share/manta/codex-accounts/a/home',
          MANTA_CODEX_HOME: '/home/jin/.local/share/manta/codex-accounts/a/home',
  ...
  it('marks Manta terminal handles for WSL env import in daemon WSL terminals', async () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')
    ...
    try {
      await createPtySubprocess({
        ...
        env: {
          ...
          MANTA_HERMES_STARTUP_QUERY: 'line one\nline two',
          WSLENV: 'FOO/u'
        }
      })
    } finally {
