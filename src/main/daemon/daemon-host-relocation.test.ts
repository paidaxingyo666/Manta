...
function buildInstallFixture(root: string): void {
  ...
  writeFileSync(join(root, 'Manta.exe'), 'exe-bytes')
...
beforeEach(() => {
  ...
  setProcessProp('execPath', join(installDir, 'Manta.exe'))
...
describe('buildDaemonHostManifest', () => {
  it('mirrors the win-unpacked layout: exe + data blobs + resources tree, no GPU DLLs', () => {
    ...
    expect(byDest.get('Manta.exe')?.kind).toBe('file')
    const exeOp = ops.find((op) => op.sourcePath === 'C:\\app\\Manta.exe')
    expect(exeOp?.destRel).toBe('Manta.exe')
...
describe('materializeRelocatedDaemonHost', () => {
  it('copies the tree, writes the marker, and returns mirrored fork paths', () => {
    ...
    const dest = join(localAppDataDir, 'Manta', 'daemon-host', '9.9.9')
    expect(result?.execPath).toBe(join(dest, 'Manta.exe'))
  ...
  it('copies the exe verbatim: same file name and same bytes as the install-dir exe', () => {
    const result = materializeRelocatedDaemonHost()
    const sourceExe = join(installDir, 'Manta.exe')
  ...
  it('tracks a differently-named app exe rather than pinning an image name of its own', () => {
    ...
    renameSync(join(installDir, 'Manta.exe'), join(installDir, 'Manta Nightly.exe'))
    setProcessProp('execPath', join(installDir, 'Manta Nightly.exe'))
    const result = materializeRelocatedDaemonHost()
    const dest = join(localAppDataDir, 'Manta', 'daemon-host', '9.9.9')
    expect(result?.execPath).toBe(join(dest, 'Manta Nightly.exe'))
    expect(existsSync(join(dest, 'manta-terminal-daemon.exe'))).toBe(false)
    // Re-resolution must agree with materialization or the fork would target a missing exe.
    expect(getRelocatedDaemonHost()?.execPath).toBe(join(dest, 'Manta Nightly.exe'))
  })
  ...
  it('fails open on a missing required input, leaving no dest or staging dir', () => {
    ...
    const hostRoot = join(localAppDataDir, 'Manta', 'daemon-host')
...
describe('getRelocatedDaemonHost', () => {
  it('returns null when the marker version does not match the current version', () => {
    ...
    writeFileSync(join(dest, 'Manta.exe'), 'exe')
