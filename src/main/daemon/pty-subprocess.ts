...
export async function createPtySubprocess(opts: PtySubprocessOptions): Promise<SubprocessHandle> {
  ...
  // launched Manta; only a session this spawn asked for may reach the shell, or
  ...
  // Manta pane hands that worktree's history file to every pane this spawn did
  // not scope itself — including panes of other worktrees.
  if (opts.env?.HISTFILE === undefined) {
    dropInheritedMantaHistFile(env)
  }
  // Why MANTA_HISTFILE too: the desktop drops an inherited one on both of its
  ...
  if (process.platform === 'win32') {
    ...
    // Why delete: MANTA_SHELL_FEATURES is Manta-owned, and only the launch config
