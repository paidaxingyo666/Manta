...
async function getComputerUsePermissionStatusAsync(): Promise<ComputerUsePermissionStatusResult> {
  ...
  if (!helperAppPath) {
    return createUnavailablePermissionStatus('Manta Computer Use.app was not found', null)
  }

  const executablePath = resolveMacOSComputerUseExecutablePath()
  if (!executablePath) {
    return createUnavailablePermissionStatus(
      `${helperAppPath}/Contents/MacOS/manta-computer-use-macos was not found`,
      helperAppPath
    )
  }
...
async function readPermissionStatusFromHelperApp(
  ...
  const tempDir = await mkdtemp(join(tmpdir(), 'manta-computer-use-permissions-'))
