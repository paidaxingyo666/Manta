...
const DEFAULT_COMPUTER_USE_BUNDLE_ID = 'cn.sh.manta.computer-use'
...
async function openComputerUsePermissionsAsync(
  ...
  if (!helperAppPath) {
    throw new RuntimeClientError('accessibility_error', 'Manta Computer Use.app was not found')
  }
...
function closeExistingPermissionHelpers(): void {
  ...
  const setupHelperPatterns = [
    'manta-computer-use-macos[[:space:]]+--permission([[:space:]]|$)',
    'manta-computer-use-macos[[:space:]]+--permissions([[:space:]]|$)'
  ]
...
function nextPermissionStep(
  ...
  return `Grant ${missing.id === 'accessibility' ? 'Accessibility' : 'Screen Recording'} to Manta Computer Use, then retry get-app-state.`
}
