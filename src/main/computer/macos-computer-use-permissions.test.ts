...
const permissionStatusTempDir = '/tmp/manta-computer-use-permissions-test'
const helperAppPath = '/Applications/Manta Computer Use.app'
...
describe('openComputerUsePermissions', () => {
  ...
  it('does not launch the setup helper when all permissions are granted', async () => {
    resolveHelperAppPathMock.mockReturnValue('/Applications/Manta Computer Use.app')

    await expect(openComputerUsePermissions()).resolves.toEqual({
      platform: 'darwin',
      helperAppPath: '/Applications/Manta Computer Use.app',
    ...
    expect(spawn).not.toHaveBeenCalledWith(
      '/usr/bin/open',
      ['-n', '/Applications/Manta Computer Use.app', '--args', '--permissions'],
  ...
  it('launches the helper app in permissions mode', async () => {
    ...
    await expect(openComputerUsePermissions()).resolves.toEqual({
      ...
      nextStep: 'Grant Screen Recording to Manta Computer Use, then retry get-app-state.'
    })
    expect(spawnSync).toHaveBeenCalledWith(
      '/usr/bin/pkill',
      ['-f', 'manta-computer-use-macos[[:space:]]+--permission([[:space:]]|$)'],
      { stdio: 'ignore' }
    )
    expect(spawnSync).toHaveBeenCalledWith(
      '/usr/bin/pkill',
      ['-f', 'manta-computer-use-macos[[:space:]]+--permissions([[:space:]]|$)'],
      { stdio: 'ignore' }
    )
  ...
  it('launches a targeted permission helper flow', async () => {
    ...
    await expect(openComputerUsePermissions('accessibility')).resolves.toEqual({
      ...
      nextStep: 'Grant Accessibility to Manta Computer Use, then retry get-app-state.'
    })
    expect(spawn).toHaveBeenCalledWith(
      '/usr/bin/open',
      ['-n', '/Applications/Manta Computer Use.app', '--args', '--permission', 'accessibility'],
      { detached: true, stdio: 'ignore' }
    )
  })
  ...
  it('throws when the helper app is missing on macOS', async () => {
    ...
    await expect(openComputerUsePermissions()).rejects.toThrow(
      'Manta Computer Use.app was not found'
    )
  })
  ...
  it('throws when the helper executable is missing during setup', async () => {
    ...
    await expect(openComputerUsePermissions('accessibility')).rejects.toThrow(
      '/Applications/Manta Computer Use.app/Contents/MacOS/manta-computer-use-macos was not found'
    )
  })
  ...
  it('resets stale macOS TCC grants for the helper bundle id', async () => {
    ...
    vi.mocked(execFileSync).mockReturnValueOnce('com.example.manta.computer-use\n')
    ...
    await expect(resetComputerUsePermissions()).resolves.toEqual({
      ...
      bundleId: 'com.example.manta.computer-use',
      permissions: [
        { id: 'accessibility', status: 'not-granted' },
        { id: 'screenshots', status: 'not-granted' }
      ]
    ...
    expect(spawnSync).toHaveBeenCalledWith(
      '/usr/bin/tccutil',
      ['reset', 'Accessibility', 'com.example.manta.computer-use'],
      { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] }
    )
    expect(spawnSync).toHaveBeenCalledWith(
      '/usr/bin/tccutil',
      ['reset', 'ScreenCapture', 'com.example.manta.computer-use'],
      { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] }
    )
  })
})
