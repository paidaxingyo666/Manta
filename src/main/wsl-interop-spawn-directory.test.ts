...
// Regression coverage for #16463 ("Removing the worktree Manta was launched from
...
const ENV_KEYS = ['MANTA_USER_DATA_PATH', 'USERPROFILE', 'HOMEDRIVE', 'HOMEPATH'] as const
...
describe('resolveWslInteropSpawnCwd', () => {
  it('names the app-owned directory first, so no worktree can be the answer', () => {
    ...
    process.env.MANTA_USER_DATA_PATH = userData
  ...
  it('skips a candidate that does not resolve instead of naming it', () => {
    process.env.MANTA_USER_DATA_PATH = join(tmpdir(), 'manta-wsl-spawn-cwd-never-created')
