...
beforeEach(() => {
  testState.fakeHomeDir = mkdtempSync(join(tmpdir(), 'manta-trust-presets-'))
  testState.userDataDir = mkdtempSync(join(tmpdir(), 'manta-trust-presets-user-data-'))
  testState.previousUserDataPath = process.env.MANTA_USER_DATA_PATH
  process.env.MANTA_USER_DATA_PATH = testState.userDataDir
})
...
afterEach(() => {
  ...
  if (testState.previousUserDataPath === undefined) {
    ...
    process.env.MANTA_USER_DATA_PATH = testState.previousUserDataPath
  }
...
describe('markCodexProjectTrusted', () => {
  it('trusts the main repository root for a linked worktree without reading commondir', () => {
    const fixtureRoot = mkdtempSync(join(tmpdir(), 'manta-codex-linked-ws-'))
  ...
  it('does not broaden trust through arbitrary or adversarial Git metadata', () => {
    const fixtureRoot = mkdtempSync(join(tmpdir(), 'manta-codex-untrusted-gitdir-'))
