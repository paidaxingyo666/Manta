...
const globallyUpdatableNames = new Set(['computer-use', 'manta-cli', 'orchestration'])
...
describe('skill freshness name-scoped update eligibility', () => {
  ...
  it.each([
    ...
    (status, topology) => {
      ...
      expect(
        eligible([
          placement('manta-cli'),
          placement('manta-cli', { id: `outlier-${status}-${topology}`, status, topology })
        ])
      ).toEqual(['manta-cli'])
    }
  )
  ...
  it.each(['unrecognized', 'inaccessible', 'newer-known'] as const)(
    ...
    (status) => {
      ...
      expect(
        eligible([
          placement('manta-cli', { id: 'blocked-canonical', status }),
          placement('manta-cli', {
            id: 'manta-cli-claude',
            rootId: 'home-claude',
            topology: 'provider-alias',
    ...
    expect(
      eligible([
        ...
        placement('manta-cli', {
          id: 'manta-cli-gemini',
          rootId: 'home-gemini',
          unresolvedPath: '/home/.gemini/skills/manta-cli',
          resolvedPath: '/home/.gemini/skills/manta-cli',
  ...
  it('does not offer an official canonical copy missing from the updater lock (#10791)', () => {
    expect(eligibleSkillUpdateNames([placement('manta-cli')], new Set())).toEqual([])
  })

  it('builds only an explicit, deterministic global command', () => {
    expect(buildTargetedSkillUpdateCommand(['orchestration', 'manta-cli', 'manta-cli'])).toBe(
      'npx skills update manta-cli orchestration --global'
    )
    expect(buildTargetedSkillUpdateCommand([])).toBeNull()
    expect(buildTargetedSkillUpdateCommand(['manta-cli;echo unsafe'])).toBeNull()
  })
})
