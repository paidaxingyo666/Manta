...
describe.runIf(RUN_REAL_WINDOWS)('real Windows skill workspace installation', () => {
  ...
  beforeAll(async () => {
    root = await mkdtemp(join(tmpdir(), 'manta-windows-skill-integration-'))
    ...
    stateDirectory = join(root, 'manta state')
    ...
    await git(['config', 'user.name', 'Manta Test'], repository)
    await git(['config', 'user.email', 'manta@example.invalid'], repository)
