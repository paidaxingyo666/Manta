...
// Why: hashed with real git, not Manta's tree-sha port — the port validating
...
async function gitTreeShaOf(directory: string): Promise<string> {
  const gitDir = await mkdtemp(join(tmpdir(), 'manta-skill-hash-'))
...
async function fixture() {
  const root = await mkdtemp(join(tmpdir(), 'manta-skill-inventory-'))
...
describe('read-only skill freshness inventory', () => {
  ...
  it('reads a plugin-cache copy with untouched official files as current', async () => {
    ...
    // of an edit, so the bytes Manta owns decide alone — here and in every scope, not just
    ...
    await writeFile(join(withSidecarRoot, 'README.md'), 'Neighbouring file Manta never shipped\n')
