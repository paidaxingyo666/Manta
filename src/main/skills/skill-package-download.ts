...
const PROCESS_DOWNLOAD_ROOT_PREFIX = '.manta-skill-download-process-'
      ...
      await Promise.all(
        entries.map(async (entry) => {
          const match = entry.isDirectory()
            ? entry.name.match(/^\.manta-skill-download-process-(\d+)-/)
            : null
...
async function downloadSkillPackageGrantUnobserved(
  ...
  try {
    ...
    const temporaryDirectory = await mkdtemp(join(processRoot, '.manta-skill-download-'))
