...
function userDataPath(): string {
  const path = mkdtempSync(join(tmpdir(), 'manta-skill-cloud-service-'))
...
describe('SkillCloudService bearer links', () => {
  it('resolves and grants downloads without a Manta session', async () => {
  ...
  it('uses the development auth token without opening a profile session', async () => {
    vi.stubEnv('MANTA_CLOUD_AUTH_TOKEN', 'desktop-e2e-token')
    const requests: RequestInit[] = []
    vi.stubGlobal(
      'fetch',
  ...
  it('rejects the development auth token in packaged builds', async () => {
    ...
    await expect(
      new SkillCloudService(userDataPath()).listOwnedShares({ apiUrl: 'https://share.manta.sh.cn' })
