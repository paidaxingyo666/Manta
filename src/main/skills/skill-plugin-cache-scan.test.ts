...
describe('plugin skill candidate scan', () => {
  it('stops at the package candidate budget and marks the scan incomplete', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-plugin-skill-scan-'))
  ...
  it('stops at the entry budget and reports the truncation at the scan root', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-plugin-entry-limit-'))
    temporaryDirectories.push(root)
    await Promise.all(
      ['one', 'two'].map(async (vendor) => {
        await mkdir(join(root, vendor, 'orca-cli'), { recursive: true })
  ...
  it('stops walking the directory whose read crossed the entry budget', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-plugin-entry-limit-stop-'))
    temporaryDirectories.push(root)
  ...
  async function createManifestWithMissingSkillRoots(): Promise<{
    ...
    const root = await mkdtemp(join(tmpdir(), 'manta-plugin-entry-limit-declared-'))
    temporaryDirectories.push(root)
  ...
  it('stops descending once a skill package payload exceeds the nested skill budget', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-plugin-payload-prune-'))
    temporaryDirectories.push(root)
  ...
  it.each([
    ...
    async (intermediateDepth, expectFound) => {
      const root = await mkdtemp(join(tmpdir(), 'manta-plugin-nested-depth-'))
      temporaryDirectories.push(root)
      const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
  ...
  it('does not scan dependency packages for plugin skill entrypoints', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-plugin-dependencies-'))
    temporaryDirectories.push(root)
    await mkdir(
      join(
        root,
  ...
  it('does not traverse plugin payload when the default skills root is missing', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-plugin-missing-default-root-'))
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
    await mkdir(
  ...
  it('does not traverse plugin payload when the manifest declares no skill roots', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-plugin-empty-skill-roots-'))
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
    await mkdir(
  ...
  it('discovers nested skill packages recursively within declared roots', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-plugin-skill-boundary-'))
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
  ...
  it('bounds how many skill roots one manifest can declare', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-plugin-manifest-budget-'))
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
  ...
  it('does not reset the depth budget across nested plugin manifests', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-plugin-nested-manifests-'))
    temporaryDirectories.push(root)
  ...
  it('reports manifests that exceed the bounded read limit', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-plugin-large-manifest-'))
    temporaryDirectories.push(root)
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'manta-plugin-symlink-error-'))
      temporaryDirectories.push(root)
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'manta-plugin-issue-limit-'))
      temporaryDirectories.push(root)
      await Promise.all(
  ...
  it('reports the bound that ended the walk even with the issue budget spent', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-plugin-truncating-issue-'))
    temporaryDirectories.push(root)
    await Promise.all(
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const parent = await mkdtemp(join(tmpdir(), 'manta-plugin-symlink-outside-'))
      temporaryDirectories.push(parent)
      const root = join(parent, 'cache')
      const outside = join(parent, 'outside')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const parent = await mkdtemp(join(tmpdir(), 'manta-plugin-manifest-outside-'))
      temporaryDirectories.push(parent)
      const root = join(parent, 'cache')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'manta-plugin-manifest-fifo-'))
      temporaryDirectories.push(root)
      const manifestPath = join(root, '.codex-plugin', 'plugin.json')
      await mkdir(join(root, '.codex-plugin'), { recursive: true })
