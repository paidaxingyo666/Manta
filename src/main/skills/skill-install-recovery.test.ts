...
async function interruptedUpdate(phase: SkillInstallJournalV1['phase']): Promise<{
  ...
  const root = await mkdtemp(join(tmpdir(), 'manta-skill-recovery-test-'))
  roots.push(root)
  ...
  const extractionPath = join(destinationRoot, '.manta-skill-extract-recovery')
  const stagingPath = join(destinationRoot, '.recovery-skill.manta-staging-recovery')
  const backupPath = join(destinationRoot, '.recovery-skill.manta-backup-recovery')
