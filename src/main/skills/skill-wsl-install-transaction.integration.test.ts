...
const DISTRO = process.env.MANTA_REAL_WSL_SKILL_DISTRO ?? 'Ubuntu-24.04'
const RUN_REAL_WSL = process.platform === 'win32' && process.env.MANTA_REAL_WSL_SKILL_TEST === '1'
...
describe.runIf(RUN_REAL_WSL)('real WSL skill install transactions', () => {
  ...
  beforeAll(async () => {
    localRoot = await mkdtemp(join(tmpdir(), 'manta-wsl-skill-integration-'))
    guestRoot = await runWsl('mktemp', '-d', '/tmp/manta-skill-integration.XXXXXX')
    if (!guestRoot.startsWith('/tmp/manta-skill-integration.')) {
  ...
  function installInput(
    ...
    return {
      ...
      mantaStateDirectory: join(localRoot, `state-${scope}`),
  ...
  it('installs, recovers, updates, conflicts, and removes on the guest filesystem', async () => {
    ...
    const removeInput = {
      ...
      homeDirectory,
      mantaStateDirectory: join(localRoot, 'state-global'),
      detectedProviders: ['codex', 'claude'],
      filesystem
    }
    ...
    expect(
      (
        await removeSharedSkill({
          ...
          homeDirectory,
          workspaceDirectory,
          mantaStateDirectory: join(localRoot, 'state-workspace'),
          detectedProviders: ['codex', 'claude'],
          filesystem
        })
      ).status
  ...
  it('installs and removes within a Windows-backed WSL workspace', async () => {
    ...
    const input = {
      ...
      workspaceDirectory: windowsWorkspace,
      mantaStateDirectory: join(localRoot, 'state-drvfs')
    }
