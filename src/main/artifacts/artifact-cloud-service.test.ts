...
} from '../manta-profiles/profile-cloud-index'
import {
  cloudSessionIdentity,
  recordSuccessfulCloudSessionLogin,
  tombstoneCloudSession
} from '../manta-profiles/profile-cloud-session-mutation'
import { saveMantaCloudSession } from '../manta-profiles/profile-cloud-session-store'
import {
...
const cloudA: MantaProfileCloudSummary = {
  cloudProfileId: 'cloud-a',
  userId: 'user-a',
  email: 'a@example.com',
  linkedAt: 1
}
const cloudB: MantaProfileCloudSummary = {
...
async function setup(sharingEnabled: { value: boolean } = { value: true }): Promise<{
  ...
  const userDataPath = await mkdtemp(join(tmpdir(), 'manta-artifact-service-'))
  createdPaths.push(userDataPath)
  const active = ensureActiveMantaProfile(userDataPath)
  linkMantaProfileToCloud(active.profile.id, cloudA, userDataPath)
...
describe('ArtifactCloudService record authorization', () => {
  ...
  it('creates once and updates on repeated publish', async () => {
    ...
    await expect(service.publish(writeRequest)).resolves.toMatchObject({
      status: 'ok',
      value: { change: 'created', item: { shareUrl: 'https://share.manta.sh.cn/a/artifact-a' } }
    })
    await expect(service.publish(writeRequest)).resolves.toMatchObject({
      status: 'ok',
      value: { change: 'updated', item: { shareUrl: 'https://share.manta.sh.cn/a/artifact-a' } }
    })
  ...
  it('resolves a persisted public link only for its source and cloud scope', async () => {
    ...
    await expect(
      ...
      status: 'ok',
      value: { shareUrl: 'https://share.manta.sh.cn/a/artifact-a' }
    })
    await expect(
  ...
  it('serializes manual publish with CLI share for the same source', async () => {
    ...
    await expect(Promise.all([publish, share])).resolves.toMatchObject([
      { status: 'ok', value: { change: 'created' } },
      { status: 'ok', value: { shareUrl: 'https://share.manta.sh.cn/a/artifact-b' } }
    ])
    expect(fetchMock).toHaveBeenCalledTimes(2)
  })
  ...
  it('recreates an artifact when its stored public link was deleted elsewhere', async () => {
    ...
    await expect(service.publish(writeRequest)).resolves.toMatchObject({
      status: 'ok',
      value: { change: 'created', item: { shareUrl: 'https://share.manta.sh.cn/a/artifact-b' } }
    })
    await expect(service.publish(writeRequest)).resolves.toMatchObject({
      status: 'ok',
      value: { change: 'updated', item: { shareUrl: 'https://share.manta.sh.cn/a/artifact-b' } }
    })
  ...
  it('keeps the idempotency key stable across an auth-refresh retry', async () => {
    ...
    vi.stubEnv('MANTA_CLOUD_API_URL', 'http://localhost:4100')
    vi.stubEnv('MANTA_CLOUD_CLIENT_ID', 'desktop-client')
    saveMantaCloudSession(profileId, userDataPath, {
  ...
  it('refuses account B update and unshare after account A signs out', async () => {
    ...
    linkMantaProfileToCloud(profileId, cloudB, userDataPath)
