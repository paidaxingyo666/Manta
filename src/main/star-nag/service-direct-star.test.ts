...
vi.mock('../github/client', () => ({
  checkMantaStarred: mocks.checkMantaStarredMock,
  starManta: mocks.starMantaMock
}))
...
const { browserWindowMock, getCohortAtEmitMock, starMantaMock, trackMock } = mocks
...
describe('StarNagService', () => {
  ...
  it('emits direct-star attempted and succeeded outcomes plus app_starred_manta', async () => {
    ...
    const ok = await getIpcHandler('star-nag:starManta')()
  ...
  it('uses the source moment for confirmed direct-star success telemetry', async () => {
    ...
    await getIpcHandler('star-nag:starManta')()
  ...
  it('uses fresh cohort context for canonical app_starred_manta success telemetry', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    getCohortAtEmitMock
  ...
  it('records success and completion when direct star resolves after dismissal cleared the visible session', async () => {
    ...
    starMantaMock.mockReturnValue(deferredStar.promise)
    const { service, ui } = createHarness()

    service.registerIpcHandlers()
    getIpcHandler('star-nag:forceShow')()
    const starPromise = getIpcHandler('star-nag:starManta')()
  ...
  it('clears the in-flight direct-star guard after thrown attempts so the user can retry', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    starMantaMock.mockRejectedValueOnce(new Error('gh failed')).mockResolvedValueOnce(true)
    const { service, ui } = createHarness()

    service.registerIpcHandlers()
    getIpcHandler('star-nag:forceShow')()
    const starFromNag = getIpcHandler('star-nag:starManta')

    await expect(starFromNag()).rejects.toThrow('gh failed')
    await expect(starFromNag()).resolves.toBe(true)

    expect(starMantaMock).toHaveBeenCalledTimes(2)
    expect(ui.starNagCompleted).toBe(true)
  })
