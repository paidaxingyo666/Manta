...
import { checkMantaStarred } from '../github/client'
...
export class StarNagService {
  ...
  registerIpcHandlers(): void {
    ...
    ipcMain.handle('star-nag:starManta', () => this.starMantaFromNag())
  ...
  private async maybeShow(
    ...
    try {
      // Why: checkMantaStarred lets us skip users who already starred outside
      ...
      // button.
      const starred = await checkMantaStarred()
  ...
  private async starMantaFromNag(): Promise<boolean> {
    const session = this.promptSession
    if (!session) {
      return false
    }
    ...
    const attempt = this.runStarMantaAttempt(session)
  ...
  private async runStarMantaAttempt(session: StarNagPromptSession): Promise<boolean> {
