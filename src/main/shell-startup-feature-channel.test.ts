...
describePosix('zsh launch config', () => {
  ...
  it('does not treat another terminal’s hijacked ZDOTDIR as the user config dir', async () => {
    ...
    try {
      ...
      expect(getShellLaunchConfig('/bin/zsh', ['history']).env.MANTA_ORIG_ZDOTDIR).toBe(home)
      ...
      expect(stamped.getShellLaunchConfig('/bin/zsh', ['history']).env.MANTA_ORIG_ZDOTDIR).toBe(
        home
      )
      ...
      const output = execFileSync(
        ...
        ],
        {
          ...
          env: {
            ...
            MANTA_ZSHENV_SOURCE_DIR: home
          }
        }
      )

      expect(output).toContain('LINEINIT=user:__manta_prompt_mark')
      expect(output).toContain('PRECMD=__manta_osc133_precmd')
...
describePosix('history-only pane in a real zsh', () => {
  ...
  const OBSERVABLES =
    ...
    'print -r -- "FEATURES=[${MANTA_SHELL_FEATURES:-}]"; ' +
    'print -r -- "MANTA_HISTFILE=[${MANTA_HISTFILE:-}]"'
  ...
  itWithZsh('emits no OSC 133 and matches an unwrapped pane', async () => {
    ...
    const comparable = (output: string): string[] =>
      output
        .split('\n')
        .filter((line) => !line.startsWith('MANTA_HISTFILE=') && !line.startsWith('ZDOTDIR='))
    expect(comparable(wrapped)).toEqual(comparable(unwrapped))
    // The one carried-over difference, unchanged from every pane Manta already
