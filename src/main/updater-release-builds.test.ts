...
const allPlatformAssets = [
  { name: 'latest-mac.yml' },
  { name: 'manta-macos-arm64.dmg' },
  { name: 'latest.yml' },
  { name: 'manta-windows-setup.exe' },
  { name: 'latest-linux.yml' },
  { name: 'manta-linux.AppImage' }
]
...
const release = (tag: string, extra: Record<string, unknown> = {}) => ({
  ...
  html_url: `https://github.com/paidaxingyo666/Manta/releases/tag/${tag}`,
...
describe('listReleaseBuilds', () => {
  ...
  it('lists hourly builds from the dedicated repo, newest first', async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toContain('paidaxingyo666/manta-hourly')
  ...
  it('lists daily builds from the dedicated repo, newest first', async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toContain('paidaxingyo666/manta-daily')
    expect(builds.map((build) => build.version)).toEqual([
    ...
    fetchMock.mockResolvedValue(
      jsonResponse([
        ...
        release('v1.4.163-hourly.202607311933', {
          assets: [{ name: 'latest-mac.yml' }, { name: 'manta-macos-arm64.dmg' }]
        })
      ])
    )
  ...
  it('resolves the platform installer download url', async () => {
    ...
    expect(build.installerUrl).toBe(
      'https://github.com/paidaxingyo666/manta-hourly/releases/download/v1.4.163-hourly.202607312054/manta-windows-setup.exe'
    )
  })
...
describe('resolveTargetBuild', () => {
  it('pins an hourly tag at the hourly repo download path', () => {
    expect(resolveTargetBuild('hourly', 'v1.4.160-hourly.202607281400')).toEqual({
      ...
      feedUrl:
        'https://github.com/paidaxingyo666/manta-hourly/releases/download/v1.4.160-hourly.202607281400'
    })
  })
  ...
  it('pins a daily tag at the daily repo download path', () => {
    expect(resolveTargetBuild('daily', 'v1.4.160-daily.202607281300')).toEqual({
      ...
      feedUrl:
        'https://github.com/paidaxingyo666/manta-daily/releases/download/v1.4.160-daily.202607281300'
    })
  })

  it('pins a stable tag at the main repo download path', () => {
    expect(resolveTargetBuild('stable', 'v1.4.159').feedUrl).toBe(
      'https://github.com/paidaxingyo666/Manta/releases/download/v1.4.159'
    )
  })
