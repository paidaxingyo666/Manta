...
describe('canCleanupUnregisteredMantaLeftoverDirectory', () => {
  const repo = { path: '/repos/main' }
  const ownedMeta = { mantaCreatedAt: 1, mantaCreationSource: 'runtime' as const }
  const baseArgs = {
    meta: ownedMeta,
    worktreePath: '/workspaces/manta-owned',
    runtimeWorktreePath: '/workspaces/manta-owned',
    repo,
  ...
  it('rejects unregistered existing targets that are files or symlinks', async () => {
    ...
    await expect(
      canCleanupUnregisteredMantaLeftoverDirectory({
        ...baseArgs,
        statPath: makeStatPath(['/workspaces/manta-owned']),
        isGitRepository
      })
    ).resolves.toBe(false)
    await expect(
      canCleanupUnregisteredMantaLeftoverDirectory({
        ...baseArgs,
        statPath: async (path) => {
          if (path === '/workspaces/manta-owned') {
            return { type: 'symlink' }
          }
          throw missingPath(path)
        },
  ...
  it('rejects unregistered leftover directories with a .git marker', async () => {
    ...
    await expect(
      canCleanupUnregisteredMantaLeftoverDirectory({
        ...baseArgs,
        statPath: makeStatPath(['/workspaces/manta-owned/.git'], ['/workspaces/manta-owned']),
        isGitRepository
      })
    ).resolves.toBe(false)
  ...
  it('rejects no-marker cleanup when only the Manta path shape matches', async () => {
    const isGitRepository = vi.fn().mockResolvedValue(false)
  ...
  it('rejects unregistered leftover directories that still answer git status', async () => {
    ...
    expect(isGitRepository).toHaveBeenCalledWith('/workspaces/manta-owned')
  })
    ...
    await expect(
      canCleanupUnregisteredMantaLeftoverDirectory({
        ...
        registeredWorktrees: [
          ...
          makeGitWorktree('/workspaces/manta-owned/child')
        ],
        statPath: makeStatPath([], ['/workspaces/manta-owned']),
        isGitRepository: vi.fn().mockResolvedValue(false)
      })
    ).rejects.toThrow(
      'Refusing to delete worktree because it contains another registered worktree: /workspaces/manta-owned/child'
    )
  })
  ...
  it('uses runtime paths for filesystem proof and original paths for nested worktree checks', async () => {
    const statPath = vi.fn(async (path: string) => {
      if (path === '/mnt/c/workspaces/manta-owned') {
        return { type: 'directory' }
      }
      throw missingPath(path)
    })
    ...
    await expect(
      canCleanupUnregisteredMantaLeftoverDirectory({
        ...baseArgs,
        worktreePath: 'C:\\workspaces\\manta-owned',
        runtimeWorktreePath: '/mnt/c/workspaces/manta-owned',
        repo: { path: 'C:\\repos\\main' },
        runtimeRepoPath: '/mnt/c/repos/main',
        registeredWorktrees: [
          makeGitWorktree('C:\\repos\\main', true),
          makeGitWorktree('C:\\workspaces\\manta-owned-sibling')
        ],
        statPath,
        isGitRepository
      })
    ...
    expect(statPath).toHaveBeenCalledWith('/mnt/c/workspaces/manta-owned')
    expect(statPath).toHaveBeenCalledWith('/mnt/c/workspaces/manta-owned/.git')
    expect(statPath).not.toHaveBeenCalledWith('C:\\workspaces\\manta-owned')
    expect(isGitRepository).toHaveBeenCalledWith('/mnt/c/workspaces/manta-owned')
  })
  ...
  it('rejects translated-runtime cleanup when original path contains a registered child', async () => {
    await expect(
      canCleanupUnregisteredMantaLeftoverDirectory({
        ...
        registeredWorktrees: [
          makeGitWorktree('C:\\repos\\main', true),
          makeGitWorktree('C:\\workspaces\\manta-owned\\child')
        ],
        statPath: makeStatPath([], ['/mnt/c/workspaces/manta-owned']),
        isGitRepository: vi.fn().mockResolvedValue(false)
      })
    ).rejects.toThrow(
      'Refusing to delete worktree because it contains another registered worktree: C:\\workspaces\\manta-owned\\child'
    )
  })
})
...
describe('canSafelyRemoveOrphanedWorktreeDirectory on an execution host', () => {
  ...
  it('refuses the leftover-directory cleanup while the host home is unanswered', async () => {
    const leftoverArgs = {
      meta: { mantaCreatedAt: 1, mantaCreationSource: 'ssh' as const },
