 ...
 * the committed `manta.yaml` this gate exists for, and could refuse on a coincidental local one.
 */
export async function gateRemovalWhereArchiveHookCannotRun(args: {
  ...
  if (!args.runHooks) {
    const warning = `manta.yaml archive hook skipped for ${args.worktreePath}; pass --run-hooks to run it.`
