// Why: the SSH relay shim (`~/.manta-relay/bin/manta`) forwards CLI invocations
// to the host app. Instead of re-implementing every command in a hand-rolled
// switch (the cause of "Unsupported SSH Manta CLI command", #7716), the host
// runs the real bundled `manta` CLI entry in Electron node mode — the same
...
export type RemoteMantaCliRequest = {
  argv: string[]
  cwd: string
...
export type RemoteMantaCliResult = {
  stdout: string
  stderr: string
  exitCode: number
  postOutput?: RemoteMantaCliPostOutput
}

export type RemoteMantaCliPostOutput =
  | {
...
// Why: only Manta terminal-context vars may cross from the remote shell into
// the host CLI process. Remote PATH / MANTA_USER_DATA_PATH are paths on the
...
export function buildHostCliEnv(args: {
  ...
  env.MANTA_NODE_OPTIONS = args.hostEnv.NODE_OPTIONS ?? ''
  env.MANTA_NODE_REPL_EXTERNAL_MODULE = args.hostEnv.NODE_REPL_EXTERNAL_MODULE ?? ''
...
export async function runHostMantaCliPassthrough(
  ...
  try {
    ...
    // to (see index.ts MantaRuntimeRpcServer wiring), or the CLI subprocess
    // reports "Manta is not running" against a healthy app.
  ...
  if (!entryExists(cliEntryPath)) {
    throw new HostCliUnavailableError(`Manta CLI entry not found at ${cliEntryPath}`)
  }
  ...
  return await new Promise<RemoteMantaCliResult>((resolve, reject) => {
    let settled = false
    ...
    const killTimer = setTimeout(() => {
      ...
      resolve({
        ...
        stderr: `${stderr.toString()}Manta CLI bridge timed out after ${killTimeoutMs}ms on the host.\n`,
        exitCode: 1
      })
    }, killTimeoutMs)
    killTimer.unref?.()
    ...
    child.on('error', (err) => {
      ...
      reject(
        new HostCliUnavailableError(`Failed to launch the Manta CLI on the host: ${err.message}`)
      )
    })
...
class CappedOutputCollector {
  ...
  toString(): string {
    ...
    return this.truncated ? `${text}\n[manta ssh cli] output truncated\n` : text
  }
}
