 ...
 *
 * The default endpoint is `$HOME/.manta-remote/relay-<fullVersion>/relay-<id>.sock`,
...
export const SHORT_RELAY_SOCKET_DIR_PREFIX = '/tmp/.manta-relay-'
 ...
 * directories in `$HOME/.manta-remote` so one sweep pattern covers both bases.
 *
 ...
 * Manta build would bind the exact path the previous build's relay still holds. The
