...
import { acknowledgeRemoteMantaCliPostOutput } from './ssh-remote-orchestration-post-output'
...
const WORKER_ENV = {
  MANTA_TERMINAL_HANDLE: WORKER_HANDLE,
  MANTA_PANE_KEY: WORKER_PANE,
  MANTA_AGENT_LAUNCH_TOKEN: 'legacy-ssh-token'
}
const COORDINATOR_ENV = {
  MANTA_TERMINAL_HANDLE: COORDINATOR_HANDLE,
  MANTA_PANE_KEY: COORDINATOR_PANE,
  MANTA_AGENT_LAUNCH_TOKEN: 'legacy-ssh-coordinator-token'
}
...
function createLegacyRuntime() {
  ...
  vi.spyOn(runtime, 'verifyOrchestrationCompatibilityCaller').mockImplementation((evidence) => {
    const worker =
      ...
      evidence.launchToken === WORKER_ENV.MANTA_AGENT_LAUNCH_TOKEN
    const coordinator =
      evidence?.terminalHandle === COORDINATOR_HANDLE &&
      evidence.paneKey === COORDINATOR_PANE &&
      evidence.launchToken === COORDINATOR_ENV.MANTA_AGENT_LAUNCH_TOKEN
    if (
      (!worker && !coordinator) ||
    ...
    const identity = worker
      ? {
          ...
          paneKey: WORKER_PANE,
          launchToken: WORKER_ENV.MANTA_AGENT_LAUNCH_TOKEN
        }
      : {
          terminalHandle: COORDINATOR_HANDLE,
          paneKey: COORDINATOR_PANE,
          launchToken: COORDINATOR_ENV.MANTA_AGENT_LAUNCH_TOKEN
        }
    return {
...
describe('legacy SSH orchestration fallback', () => {
  it('acknowledges a consuming check only after remote output', async () => {
    ...
    try {
      const first = await runRemoteMantaCli(runtime, request, LEGACY_FALLBACK_OPTIONS)
      const replay = await runRemoteMantaCli(runtime, request, LEGACY_FALLBACK_OPTIONS)
      ...
      await acknowledgeRemoteMantaCliPostOutput(runtime, {
        postOutput: first.postOutput!,
        env: WORKER_ENV,
        runtimeAuthority: RUNTIME_AUTHORITY
      })
      const afterAck = await runRemoteMantaCli(runtime, request, LEGACY_FALLBACK_OPTIONS)
  ...
  it('keeps peek formatted and non-consuming', async () => {
    ...
    try {
      const peek = await runRemoteMantaCli(
        runtime,
        {
  ...
  it('reads and acknowledges current Run delivery', async () => {
    ...
    try {
      const checked = await runRemoteMantaCli(
        runtime,
        {
          ...baseRequest,
      ...
      const checkedJson = await runRemoteMantaCli(
        runtime,
        {
          ...baseRequest,
      ...
      const acknowledged = await runRemoteMantaCli(
        runtime,
        {
          ...baseRequest,
          argv: [
  ...
  it('resumes and acknowledges a question answer', async () => {
    ...
    try {
      const result = await runRemoteMantaCli(
        runtime,
        {
