...
describe('installNativeDeps staged uploads', () => {
  ...
  it('writes a hardcoded package.json BEFORE running npm install', async () => {
    ...
    expect(parsed.name).toBe('manta-relay')
  ...
  it('names the fix when node-gyp cannot download headers and the host ships none (STA-6674)', async () => {
    ...
    expect((error as Error).message).not.toContain('Manta defect')
  ...
  it('reports a Manta defect when headers were exported but node-gyp downloaded anyway', async () => {
    ...
    expect((error as Error).message).toContain('Manta defect')
