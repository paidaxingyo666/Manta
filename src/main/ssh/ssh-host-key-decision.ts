...
export type HostKeyDecision = {
  ...
  disagreeingSource?: 'manta-store' | 'known-hosts'
...
export function decideHostKey(input: HostKeyDecisionInput): HostKeyDecision {
  ...
  if (
    ...
    return {
      ...
      disagreeingSource: fromKnownHosts ? 'known-hosts' : 'manta-store',
      reason: rejection(
        displayHost,
  ...
  // escape hatch that is an environment variable, unreachable when Manta is launched from the Dock.
