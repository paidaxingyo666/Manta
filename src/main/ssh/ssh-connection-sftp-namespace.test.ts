...
vi.mock('./ssh-system-fallback', () => ({
  getMantaControlSocketPath: vi.fn().mockReturnValue(null),
...
const RELAY_DIR = '.manta-remote/relay-0.1.0+hash'
...
describe('SshConnection SFTP namespace resolution', () => {
  ...
  beforeEach(() => {
    ...
    localDir = realpathSync(mkdtempSync(join(tmpdir(), 'manta-relay-')))
