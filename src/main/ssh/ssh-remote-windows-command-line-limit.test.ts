...
const pool = 'C:\\Users\\manta\\.manta-remote\\.upload-stages'
...
describe('Windows remote command line limit', () => {
  it.each([
    ...
    [
      ...
      promoteOwnedRelayUploadStageCommand(windows, stage, owner, 'C:\\Users\\manta\\.manta-remote')
    ],
    ...
    [
      'publish staged file',
      makeWindowsPublishStagedFileCommand(
        'C:\\Users\\manta\\.manta-remote\\relay.js.orca-partial-0123456789ab',
        'C:\\Users\\manta\\.manta-remote\\relay.js',
        'create'
      )
    ]
  ...
  it('carries an oversized script through gzip without altering it', () => {
    const script = Array.from(
      { length: 200 },
      (_unused, index) => `Write-Output ${index}; $slot = 'C:\\Users\\manta\\stage-${index}'`
    ).join('\n')
  ...
  it('refuses a script no encoding can fit instead of letting cmd.exe reject it', () => {
    ...
    expect(() => powerShellCommand(`Write-Output '${incompressible}'`)).toThrow(
      /Manta budgets 8000 for a line sshd hands to cmd\.exe/u
    )
  })
})
...
describe('Windows file command budget headroom', () => {
  it('absorbs a path far longer than Windows will accept', () => {
    const deep = `C:\\Users\\manta\\${'segment\\'.repeat(30)}relay.js`
