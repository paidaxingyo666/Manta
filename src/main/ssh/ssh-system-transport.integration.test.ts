...
const sentinel = 'MANTA-RELAY v0.1.0 READY\\n';
...
function createRelayTree(root: string, remoteHome: string): void {
  ...
  const remoteDir = join(remoteHome, '.manta-remote', `relay-${RELAY_VERSION}`)
...
describe('system SSH transport integration', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = mkdtempSync(join('/tmp', 'manta-ssh-'))
    oldHome = process.env.HOME
    oldRelayPath = process.env.MANTA_RELAY_PATH
    oldSystemSshPath = process.env.MANTA_SYSTEM_SSH_PATH
    oldForceSystemTransport = process.env.MANTA_SSH_FORCE_SYSTEM_TRANSPORT
    ...
    process.env.MANTA_RELAY_PATH = relayRoot
    process.env.MANTA_SYSTEM_SSH_PATH = writeFakeSsh(tempDir)
    process.env.MANTA_SSH_FORCE_SYSTEM_TRANSPORT = '1'
  })
  ...
  afterEach(() => {
    ...
    if (oldRelayPath === undefined) {
      delete process.env.MANTA_RELAY_PATH
    } else {
      process.env.MANTA_RELAY_PATH = oldRelayPath
    }
    if (oldSystemSshPath === undefined) {
      delete process.env.MANTA_SYSTEM_SSH_PATH
    } else {
      process.env.MANTA_SYSTEM_SSH_PATH = oldSystemSshPath
    }
    if (oldForceSystemTransport === undefined) {
      delete process.env.MANTA_SSH_FORCE_SYSTEM_TRANSPORT
    } else {
      process.env.MANTA_SSH_FORCE_SYSTEM_TRANSPORT = oldForceSystemTransport
    }
