...
describe('deployAndLaunchRelay', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.mocked(execCommand).mockReset().mockResolvedValue('__MANTA_REMOTE_PLATFORM__ Linux x86_64')
  ...
  it('calls exec to detect remote platform', async () => {
    ...
    mockExecCommand.mockResolvedValueOnce('__MANTA_REMOTE_PLATFORM__ Linux x86_64') // tagged POSIX platform probe
    mockExecCommand.mockResolvedValueOnce('/home/user') // echo $HOME
    mockExecCommand.mockResolvedValueOnce('MANTA-NATIVE-DEPS-OK') // native deps probe
    ...
    expect(mockExecCommand).toHaveBeenCalledWith(
      conn,
      "printf '\\n%s ' '__MANTA_REMOTE_PLATFORM__'; uname -sm",
  ...
  it('reports progress via callback', async () => {
    const conn = makeMockConnection()
    const mockExecCommand = vi.mocked(execCommand)
    mockExecCommand.mockResolvedValueOnce('__MANTA_REMOTE_PLATFORM__ Linux x86_64')
  ...
  it('resolves the remote node path once per deploy', async () => {
    ...
    mockExecCommand.mockResolvedValueOnce('__MANTA_REMOTE_PLATFORM__ Linux x86_64')
    mockExecCommand.mockResolvedValueOnce('/home/user')
    mockExecCommand.mockResolvedValueOnce('MANTA-NATIVE-DEPS-OK')
    queueLaunchNamespaceAndDeadSocketProbe()
  ...
  it('uses a content-hashed versioned remote install directory', async () => {
    ...
    const sawVersionedDir = allCmds.some((cmd) =>
      cmd.includes('/.manta-remote/relay-0.1.0+abcdef012345')
    )
    ...
    try {
      ...
      mockExecCommand
        ...
        .mockImplementationOnce(
          () =>
            new Promise<string>((resolve) =>
              setTimeout(() => resolve('MANTA-NATIVE-DEPS-OK'), 899_900)
            )
        )
        .mockResolvedValueOnce('') // launch namespace marker
  ...
  it('uses distinct target-specific relay socket paths', async () => {
    ...
    mockExecCommand.mockImplementation((_conn, command) => {
      if (command.includes('__MANTA_REMOTE_PLATFORM__')) {
        return Promise.resolve('__MANTA_REMOTE_PLATFORM__ Linux x86_64')
      }
      if (command === 'echo $HOME') {
        return Promise.resolve('/home/user')
      }
      if (command.includes('MANTA-NATIVE')) {
        return Promise.resolve('MANTA-NATIVE-DEPS-OK')
      }
  ...
  it('launches Windows remotes via a named pipe endpoint', async () => {
    ...
    mockExecCommand
      ...
      .mockResolvedValueOnce('__MANTA_REMOTE_PLATFORM__ Windows X64') // tagged PowerShell platform probe
      .mockResolvedValueOnce('C:\\Users\\me user') // remote home
      .mockResolvedValueOnce('MANTA-NATIVE-DEPS-OK') // native deps probe
    ...
    expect(result.sockPath).toMatch(/^\\\\\.\\pipe\\manta-relay-[0-9a-f]{20}$/)
    ...
    expect(launchScript).toContain(
      '"C:/Users/me user/.manta-remote/relay-0.1.0+abcdef012345/relay.js"'
    )
    expect(launchScript).toContain(
      '"C:/Users/me user/.manta-remote/relay-0.1.0+abcdef012345/agent-hooks/manta-relay-'
    )
  ...
  it('relaunches Windows remotes on a fallback pipe when reconnecting the occupied pipe fails', async () => {
    ...
    expect(primaryPipe).toMatch(/^\\\\\.\\pipe\\manta-relay-[0-9a-f]{20}$/)
    expect(fallbackPipe).toMatch(/^\\\\\.\\pipe\\manta-relay-[0-9a-f]{20}$/)
  ...
  it('prefers a persisted Windows fallback pipe on later reconnects', async () => {
    const conn = makeMockConnection()
    const mockExecCommand = vi.mocked(execCommand)
    const persistedPipe = '\\\\.\\pipe\\manta-relay-1234567890abcdef1234'
    vi.mocked(resolveRemoteNodePath).mockResolvedValue('C:/Program Files/nodejs/node.exe')
    mockExecCommand
      .mockRejectedValueOnce(new Error('uname not found')) // tagged POSIX platform probe
      .mockResolvedValueOnce('__MANTA_REMOTE_PLATFORM__ Windows X64') // tagged PowerShell platform probe
