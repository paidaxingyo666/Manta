...
describe.runIf(process.platform !== 'win32').each(SHELLS)(
  ...
  (shell) => {
    ...
    const relayDir = (version: string): string => join(home, '.manta-remote', `relay-${version}`)
    ...
    beforeEach(() => {
      ...
      mkdirSync(join(home, '.manta-remote'), { recursive: true })
    })
    ...
    it('reports a symlink no Manta version wrote, so GC can refuse the pass', () => {
      makePrivateInstall('0.1.0+aaa')
      promote('0.1.0+aaa')
      mkdirSync(relayDir('0.1.0+bbb'), { recursive: true })
