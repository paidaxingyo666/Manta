...
vi.mock('./ssh-remote-manta-cli', () => ({
  runRemoteMantaCli: vi.fn().mockResolvedValue({ exitCode: 0, stdout: '', stderr: '' })
}))
...
describe('SshRelaySession reconnect incarnation ordering', () => {
  ...
  it('keeps the winning reconnect incarnation when a stale health check resolves last', async () => {
    ...
    const winningCliHandler = muxInstances[2]?.requestHandlers.get('manta.cli')
    ...
    expect(vi.mocked(runRemoteMantaCli)).toHaveBeenCalledWith(
      runtime,
