...
describe('runRemoteMantaCli Linear commands', () => {
  ...
  it('dispatches Linear discovery and list reads through the remote runtime', async () => {
    ...
    const teamList = await runRemoteMantaCli(runtime, {
      argv: ['linear', 'team', 'list', '--workspace', 'all', '--json'],
      cwd: '/home/alice/remote-repo',
      env: { MANTA_TERMINAL_HANDLE: 'term_ssh' }
    })
    const labels = await runRemoteMantaCli(runtime, {
      argv: ['linear', 'team', 'labels', '--team', 'ENG', '--workspace', 'workspace-1', '--json'],
      cwd: '/home/alice/remote-repo',
      env: { MANTA_TERMINAL_HANDLE: 'term_ssh' }
    })
    const list = await runRemoteMantaCli(runtime, {
      argv: [
        'linear',
        'list',
        '--filter',
    ...
    const projects = await runRemoteMantaCli(runtime, {
      argv: [
        'linear',
        'project',
        'list',
  ...
  it('prints SSH Linear non-json next steps from structured errors', async () => {
    ...
    linearIssueAddComment.mockRejectedValueOnce(
      Object.assign(new Error('Linear may have applied the write.'), {
        ...
        data: { nextSteps: ['Retry once with the pinned command: `manta linear comment add`.'] }
      })
    )
  ...
  it('shows SSH Linear command help without dispatching to the runtime', async () => {
    ...
    expect(result.exitCode).toBe(0)
    expect(result.stdout).toContain('manta linear issue')
    expect(result.stdout).toContain('Usage: manta linear issue')
  ...
  it('shows SSH Linear group help without dispatching to the runtime', async () => {
    ...
    expect(result.exitCode).toBe(0)
    expect(result.stdout).toContain('manta linear')
    expect(result.stdout).toContain('Usage: manta linear <command> [options]')
  ...
  it('shows SSH Linear help through the local help command form', async () => {
    ...
    const group = await runRemoteMantaCli(runtime, {
      argv: ['help', 'linear'],
      cwd: '/home/alice/remote-repo',
      env: { MANTA_TERMINAL_HANDLE: 'term_ssh' }
    })
    const issue = await runRemoteMantaCli(runtime, {
    ...
    expect(group.stdout).toContain('Usage: manta linear <command> [options]')
    expect(issue.exitCode).toBe(0)
    expect(issue.stdout).toContain('Usage: manta linear issue')
    expect(linearIssueContext).not.toHaveBeenCalled()
  })
