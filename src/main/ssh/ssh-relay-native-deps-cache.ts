 ...
 *
 * The tree now lives at `~/.manta-remote/native/<relayPlatform>-<depsHash>/node_modules` and each
...
/** Sibling of `relay-<version>` and `mantad-<version>`; owned by neither model's version GC. */
 ...
 * reading `~/.manta-remote/native/` can tell what an entry is for.
 */
...
/** `~/.manta-remote` — the parent both relay dirs and the cache sit under. */
...
/** `~/.manta-remote/native/<key>/node_modules` — the symlink target, and the reference identity. */
