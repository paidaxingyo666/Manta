...
import {
  ...
  runHostMantaCliPassthrough,
  type HostCliPassthroughOptions,
  type RemoteMantaCliRequest,
  type RemoteMantaCliResult
...
export type { RemoteMantaCliRequest, RemoteMantaCliResult } from './ssh-remote-cli-host-passthrough'
...
const HOST_INTERACTIVE_COMMANDS: Record<string, string> = {
  serve:
    'manta serve starts a foreground headless Manta server and cannot run through the SSH relay bridge. Run it directly on the machine that should host Manta.',
  'claude-teams':
    'manta claude-teams starts an interactive Claude Code session and cannot run through the SSH relay bridge. Run it in a terminal on the Manta host machine.',
  'agent-teams-tmux':
    'manta agent-teams-tmux is a tmux pane shim for the Manta host machine and cannot run through the SSH relay bridge.',
  'account add':
    'manta account add runs an interactive agent login and cannot run through the buffered SSH relay bridge. Run it directly in a terminal on the Manta host machine.'
}

export async function runRemoteMantaCli(
  runtime: MantaRuntimeService,
  request: RemoteMantaCliRequest,
  passthroughOptions?: HostCliPassthroughOptions
): Promise<RemoteMantaCliResult> {
  ...
  if (command === 'orchestration check' || command === 'orchestration ask') {
    ...
    return await runLegacyRemoteMantaCli(
      runtime,
      request,
      parsed,
      json,
  ...
  try {
    return await runHostMantaCliPassthrough(request, passthroughOptions)
  } catch (err) {
  ...
  return await runLegacyRemoteMantaCli(runtime, request, parsed, json, passthroughFailure)
}

async function runLegacyRemoteMantaCli(
  runtime: MantaRuntimeService,
  request: RemoteMantaCliRequest,
...
async function dispatchRemoteCli(
  ...
  runtimeAuthority: RemoteMantaCliRequest['runtimeAuthority']
): Promise<RpcResponse> {
  const command = parsed.commandPath.join(' ')
  ...
  switch (command) {
    case 'status': {
      ...
      const cliStatus: CliStatusResult = {
        ...
        // Why: this answers for the Manta host the caller reached over SSH, not for the caller's
    ...
    case 'orchestration send': {
      ...
      return await call(
        ...
        'orchestration.send',
        {
          ...
          senderPaneKey: env.MANTA_PANE_KEY || undefined
        },
        {
    ...
    default:
      ...
      throw new Error(
        `Unsupported SSH Manta CLI command: ${command} (full Manta CLI bridge unavailable: ${passthroughFailureReason})`
      )
  }
}
