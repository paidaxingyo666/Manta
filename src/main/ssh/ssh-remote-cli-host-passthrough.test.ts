...
describe('buildHostCliEnv', () => {
  it('forwards only Manta terminal-context vars from the remote env', () => {
    const env = buildHostCliEnv({
      hostEnv: { PATH: '/host/bin', NODE_OPTIONS: '--inspect' },
      remoteEnv: {
        MANTA_TERMINAL_HANDLE: 'term_remote',
        MANTA_WORKTREE_ID: 'repo::/home/alice/wt',
        MANTA_PANE_KEY: 'pane-9',
        MANTA_AGENT_LAUNCH_TOKEN: 'launch-secret',
        MANTA_WORKSPACE_ID: 'ws-1',
        ...
        PATH: '/remote/bin',
        MANTA_USER_DATA_PATH: '/remote/user-data'
      },
      userDataPath: '/host/user-data',
    ...
    expect(env.MANTA_TERMINAL_HANDLE).toBe('term_remote')
    expect(env.MANTA_WORKTREE_ID).toBe('repo::/home/alice/wt')
    expect(env.MANTA_PANE_KEY).toBe('pane-9')
    expect(env.MANTA_AGENT_LAUNCH_TOKEN).toBe('launch-secret')
    expect(env.MANTA_WORKSPACE_ID).toBe('ws-1')
...
describe('runHostMantaCliPassthrough', () => {
  it('spawns the bundled CLI entry with the remote argv and returns captured output', async () => {
    ...
    expect(options.env.MANTA_CLI_CWD).toBe('/home/alice/wt')
    expect(options.env.MANTA_TERMINAL_HANDLE).toBe('term_remote')
