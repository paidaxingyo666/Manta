...
const SHELL_RELAY_DIR = `/var/services/homes/alice/.manta-remote/relay-${VERSION}`

describe('relayRemoteDirSegments', () => {
  it('builds the two segments every relay path shares', () => {
    expect(relayRemoteDirSegments(VERSION, 'posix')).toEqual(['.manta-remote', `relay-${VERSION}`])
  })
...
describe('computeRemoteRelayDir agreement', () => {
  ...
  it('applies Windows segment rules on the windows flavor', () => {
    expect(computeRemoteRelayDir('C:\\Users\\u', VERSION, 'windows')).toBe(
      `C:/Users/u/.manta-remote/relay-${VERSION}`
    )
...
describe('createRelayInstallNamespace', () => {
  it('mints an unguessable 128-bit marker name per install', () => {
    ...
    expect(first.homeRelativeRelayDir).toBe(`.manta-remote/relay-${VERSION}`)
  })
})
...
describe('relaySftpNamespaceMapping', () => {
  ...
  it('maps the bundle directory itself when no file name is given', () => {
    ...
    expect(mapping.homeRelativePath).toBe(`.manta-remote/relay-${VERSION}`)
    expect(mapping.homeRelativeNamespaceRoot).toBe(`.manta-remote/relay-${VERSION}`)
    expect(mapping.homeRelativeProbePath).toBe(
      `.manta-remote/relay-${VERSION}/.install-lock/${namespace.markerFileName}`
    )
  ...
  it('maps a file inside the bundle directory', () => {
    ...
    expect(mapping.homeRelativePath).toBe(`.manta-remote/relay-${VERSION}/package.json`)
  })
...
describe('relay upload stage namespace', () => {
  const stageSuffix = `.manta-remote/relay-${VERSION}.upload-123e4567-e89b-12d3-a456-426614174000`
