...
// user restarting Manta. Long enough that a hardened host costs one failed probe per half hour.
...
/**
 * Keyed by the endpoint that executes, not by target id: two Manta targets pointing at one host
 * describe the same sshd, and a target re-created under a new id has not changed what that host
 * supports. A config alias is its own key because ssh_config, not Manta, resolves where it lands.
 */
