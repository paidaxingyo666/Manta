 ...
 *
 * Every relay build installs to `~/.manta-remote/relay-<fullVersion>/` and binds its socket
...
export function supersededRelayEndpointListCommand(options: {
  ...
  return [
    ...
    // under `/tmp/.manta-relay-<uid>/relay-<versionHash>/` instead (relay-socket-path-limit.ts).
...
export function removeStaleRelayEndpointCommand(sockPath: string): string {
  ...
  // `gcOldRelayVersions` only walks `$HOME/.manta-remote`, so nothing else would ever
