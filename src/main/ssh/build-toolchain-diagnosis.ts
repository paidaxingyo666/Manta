 ...
 * `ECONNREFUSED`, and it reads as a broken host or a broken Manta. Which of two things it is
 ...
 * a wrapper scrubbing the env). That is a Manta defect, not a host problem, and must not be
  ...
  const lines = localHeadersDir
    ? [
        `The remote host could not download the Node.js headers needed to compile node-pty, even ` +
          ...
          `Manta pointed node-gyp at them. node-gyp ignored that setting; this is a Manta defect, ` +
