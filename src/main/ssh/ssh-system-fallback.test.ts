...
function expectNoMantaControlMasterArgs(args: string[]): void {
...
function expectMantaControlMasterArgs(args: string[]): void {
  if (process.platform === 'win32') {
    expectNoMantaControlMasterArgs(args)
    return
  }
...
describe('spawnSystemSsh', () => {
  ...
  it('passes explicit options for manual targets with implicit configHost', () => {
    const args = buildSshArgs(
      createTarget({
        ...
        host: '127.0.0.1',
        port: 2222,
        identityFile: '/tmp/manta-docker-key',
        identitiesOnly: true
      })
    )

    expect(args).toEqual(expect.arrayContaining(['-p', '2222', '-i', '/tmp/manta-docker-key']))
    expect(args).toContain('IdentitiesOnly=yes')
  ...
  it('does not inject Manta ControlMaster flags when ssh config already owns muxing', () => {
  ...
  it('injects Manta ControlMaster flags when ssh config only sets ControlPersist', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }), {
      resolvedConfig: createResolvedConfig({
        controlMaster: 'no',
        controlPersist: '10m'
    ...
    expectMantaControlMasterArgs(args)
    expect(args).not.toContain('-S')
  })

  it('injects Manta ControlMaster flags when ssh config only sets ControlPath', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }), {
      resolvedConfig: createResolvedConfig({
        controlMaster: 'no',
        controlPath: '/Users/me/.ssh/cm/%r@%h:%p'
  ...
  it('injects Manta ControlMaster flags when ssh config omits ControlPath', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }), {
      resolvedConfig: createResolvedConfig({
        controlMaster: 'auto'
      })
  ...
  it('does not inject Manta ControlMaster flags for unresolved ssh-config targets', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }))
  ...
  it('does not inject Manta ControlMaster flags for unresolved legacy config aliases', () => {
  ...
  it('can inject Manta ControlMaster flags for ssh-config targets with resolved config', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }), {
      resolvedConfig: createResolvedConfig()
    })
  ...
  it('adds keepalive options to Manta-owned ControlMaster connections', () => {
  ...
  it('streams a local file through one POSIX system SSH command', async () => {
    ...
    spawnMock.mockReturnValue(proc)
    const dir = mkdtempSync(join(tmpdir(), 'manta-system-ssh-upload-'))
  ...
  it('downloads files from POSIX system SSH targets', async () => {
    const proc = createEventedProcess()
    spawnMock.mockReturnValue(proc)
    const dir = mkdtempSync(join(tmpdir(), 'manta-system-ssh-download-'))
  ...
  it('writes files to Windows system SSH targets with PowerShell stdin bytes', async () => {
    ...
    const promise = writeFileViaSystemSsh(
      createTarget(),
      'C:/Users/me/.manta-remote/relay/.version',
      '0.1.0',
      { hostPlatform }
    )
    proc.emit('close', 0, null)
