...
export type KnownHostsEvidence = {
   ...
   * state (ssh creates known_hosts on its own first connect, and most Manta profiles start without
 ...
 * ssh2 dials. Never the Manta label — bastions tunnelled through `localhost:port` depend on the
...
export function resolveKnownHostsLookupHost(
  ...
  // no Host block matches, which for a manual target is the Manta label. Keying on that consults a
