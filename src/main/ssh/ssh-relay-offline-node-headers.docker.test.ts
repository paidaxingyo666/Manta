...
// (default node:24.12.0-bookworm, the user's version); MANTA_REVIEW_SSH_TARGET_HOST overrides
...
const TARGET_HOST = process.env.MANTA_REVIEW_SSH_TARGET_HOST ?? '127.0.0.1'
...
async function startTarget(): Promise<TargetFixture> {
  const image = `manta-review-offline-headers:${NODE_IMAGE.replace(/[^A-Za-z0-9_.-]/g, '-')}`
  run(
    'docker',
    ['build', '-q', '-t', image, '-'],
    600_000,
...
describe.skipIf(!RUN_REVIEW_ORACLE)(
  ...
  () => {
    ...
    it('names the missing-local-headers cause, not a Manta defect, when the host ships no headers', async () => {
      ...
      dockerExec(
        activeFixture,
        'rm -rf /usr/local/include/node /root/.manta-remote /root/.cache/node-gyp'
      )
      const connection = createConnection(activeFixture)
      await connection.connect()
      try {
        ...
        expect(message).not.toContain('Manta defect')
