...
describe('resolveRemoteNodePath', () => {
  ...
  it('skips an incomplete system Node and selects a complete NVM toolchain', async () => {
    execCommandMock
      ...
      .mockResolvedValueOnce('__MANTA_NODE_VERSION__\nv22.22.0\n__MANTA_NPM_VERSION__\n11.13.0\n')
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      const root = mkdtempSync(path.join(os.tmpdir(), 'manta-split-node-npm-'))
      try {
  ...
  it('expands tilde NVM_DIR assignments from shell dotfiles', async () => {
    ...
    const callScript = execCommandMock.mock.calls[0]![1] as string
    const home = mkdtempSync(path.join(os.tmpdir(), 'manta-nvm-probe-'))
    try {
