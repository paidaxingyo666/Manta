...
export function searchSshConfigHosts(
  ...
  for (const entry of hosts) {
    ...
    const alreadyInManta = existingAliases.has(normalizedAlias)
    // Why: tombstones only block passive bulk import — the picker still lists the
    // Host so deleting one Manta target never looks like "~/.ssh/config is empty".
    const previouslyRemoved = !alreadyInManta && suppressedAliasSet.has(normalizedAlias)
    totalHostCount += 1
    // Why: "Add all" must match importFromSshConfig without reAdopt (tombstones stay).
    newHostCount += alreadyInManta || previouslyRemoved ? 0 : 1
    if (!matchesQuery(entry, normalizedQuery)) {
      continue
    }
    matchCount += 1
    if (summaries.length < SSH_CONFIG_HOST_RESULT_LIMIT) {
      summaries.push(toSummary(entry, alreadyInManta, previouslyRemoved))
    }
  }
