...
describe('detectRemoteHostPlatform', () => {
  it('uses uname when the remote is POSIX', async () => {
    vi.mocked(execCommand).mockResolvedValueOnce('__MANTA_REMOTE_PLATFORM__ Darwin arm64')
  ...
  it('falls back to PowerShell when uname is unavailable on Windows', async () => {
    ...
    expect(script).toContain('Write-Output ("`n__MANTA_REMOTE_PLATFORM__ Windows " + $arch)')
  })
})
