...
// then the freshly built manta.exe -> node.exe, twice - and hosted Windows
...
describe('SSH remote Manta CLI launcher', () => {
  function windowsInstallPlan(): ReturnType<typeof createRemoteCliInstallPlan> {
    return createRemoteCliInstallPlan({
      binDir: 'C:/Users/me user/.manta-relay/bin',
      relayDir: 'C:/Users/me user/.manta-remote/relay-v1',
      nodePath: 'C:/Program Files/nodejs/node.exe',
      sockPath: '\\\\.\\pipe\\manta-relay-123',
      credentialFile: 'C:/Users/me user/.manta-remote/relay-v1/relay.sock.credential',
  ...
  it('compiles a native Windows launcher without a cmd.exe argument bridge', () => {
    ...
    expect(plan.launcherPath).toBe('C:/Users/me user/.manta-relay/bin/manta.exe')
    expect(plan.files).toHaveLength(1)
    expect(plan.files[0]?.path).toBe('C:/Users/me user/.manta-relay/bin/manta-launcher.cs')
    expect(plan.files[0]?.contents).toContain('ProcessStartInfo')
    expect(plan.files[0]?.contents).toContain('"--manta-cli"')
    ...
    expect(compileScript).toContain(
      "Set-Location -ErrorAction Stop -LiteralPath 'C:/Users/me user/.manta-relay/bin'"
    )
    expect(compileScript).toContain('/out:manta.exe')
    expect(compileScript).toContain('C:/Users/me user/.manta-relay/bin/manta-launcher.cs')
    expect(compileScript).toContain('C:/Users/me user/.manta-relay/bin/manta.cmd')
  })

  it('removes the legacy manta.cmd only after every compile guard has passed', () => {
    const script = decodePowerShellCommand(windowsInstallPlan().postWriteCommands[0] ?? '')
    const legacyShimRemoval =
      "Remove-Item -LiteralPath 'C:/Users/me user/.manta-relay/bin/manta.cmd' -Force -ErrorAction SilentlyContinue"
    ...
    const guards = [
      "if (-not $compiler) { Write-Error 'Unable to find the .NET Framework C# compiler required for the Manta SSH CLI launcher.'; exit 1 }",
      'if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }',
      "if (-not (Test-Path -LiteralPath 'C:/Users/me user/.manta-relay/bin/manta.exe' -PathType Leaf))"
    ]
  ...
  itWindows('preserves a multiline argument through the compiled remote launcher', () => {
    const root = mkdtempSync(join(tmpdir(), 'manta remote cli '))
    try {
      const binDir = join(root, 'bin').replaceAll('\\', '/')
      const relayDir = join(root, 'relay').replaceAll('\\', '/')
      const sockPath = '\\\\.\\pipe\\manta-relay-test'
      ...
      const launched = spawnSync(
        ...
        ['orchestration', 'send', '--body', body, '--json'],
        {
          ...
          env: {
            ...process.env,
            MANTA_RELAY_NODE_PATH: process.execPath,
            MANTA_RELAY_DIR: relayDir,
            MANTA_RELAY_SOCKET_PATH: sockPath,
            MANTA_RELAY_CREDENTIAL_FILE: credentialFile
          }
        }
      )
  ...
  itWindows('preserves the existing manta.cmd when the compiler is missing', () => {
    const root = mkdtempSync(join(tmpdir(), 'manta remote cli '))
    try {
      const binDir = join(root, 'bin').replaceAll('\\', '/')
      mkdirSync(binDir, { recursive: true })
      const legacyShimPath = join(binDir, 'manta.cmd')
      writeFileSync(legacyShimPath, '@echo legacy manta cli\r\n', 'utf8')

      const plan = createRemoteCliInstallPlan({
        binDir,
        relayDir: join(root, 'relay').replaceAll('\\', '/'),
        nodePath: process.execPath,
        sockPath: '\\\\.\\pipe\\manta-relay-test',
      ...
      expect(existsSync(legacyShimPath), 'existing manta.cmd must survive a failed install').toBe(
        true
      )
    } finally {
      rmSync(root, { recursive: true, force: true })
  ...
  it('keeps the POSIX launcher as an argv-preserving shell exec', () => {
    const plan = createRemoteCliInstallPlan({
      ...
      relayDir: '/home/me/.manta-remote/relay-v1',
      nodePath: '/usr/bin/node',
      sockPath: '/home/me/.manta-remote/relay-v1/relay.sock',
      hostPlatform: getRemoteHostPlatform('linux-x64')
    })

    expect(plan.launcherPath).toBe('/home/me/.manta-relay/bin/manta')
    expect(plan.files).toEqual([
      expect.objectContaining({
        path: '/home/me/.manta-relay/bin/manta',
        contents: expect.stringContaining('--manta-cli "$@"')
      })
    ])
  })
})
