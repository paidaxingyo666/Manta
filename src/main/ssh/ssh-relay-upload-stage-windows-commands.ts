...
export function reserveWindowsRelayUploadStageCommand(poolDir: string, owner: string): string {
  return powerShellCommand(
    [
      ...
      '} catch { continue }',
      '}',
      `throw ${powerShellLiteral(`Manta relay upload staging quota is full; reconnect after 40 minutes or inspect .manta-remote/${RELAY_UPLOAD_STAGE_POOL_NAME}`)}`
    ].join('\n')
  )
}
...
function windowsFileIdentityScript(): string[] {
  return [
    ...
    "if ($null -eq ('MantaRelayUploadFileIdentity' -as [type])) {",
    "Add-Type -TypeDefinition @'",
    'using System;',
    'using System.ComponentModel;',
    'using System.Runtime.InteropServices;',
    'using Microsoft.Win32.SafeHandles;',
    'public static class MantaRelayUploadFileIdentity {',
    ...
    "'@",
    '}',
    '$getFileIdentity = { param($path) [MantaRelayUploadFileIdentity]::Read($path) }',
    '} else {',
