...
import {
  ...
  MANTA_FEATURE_REMOTE_AGENT_HOOKS_ENV,
...
function createFakeRelay(): FakeRelay {
  ...
  dispatcher.onRequest('session.resolveHome', async (params) => ({
    resolvedPath: params.path === '~' ? '/home/manta' : params.path
  }))
  ...
  dispatcher.onRequest(AGENT_HOOK_REQUEST_REPLAY_METHOD, async () => {
    // Why: relay replay must arrive after Manta wires its listener and before
...
describe('SshRelaySession agent hooks over a fake relay transport', () => {
  ...
  beforeEach(() => {
    ...
    previousRemoteHooksFlag = process.env[MANTA_FEATURE_REMOTE_AGENT_HOOKS_ENV]
    process.env[MANTA_FEATURE_REMOTE_AGENT_HOOKS_ENV] = '1'
  ...
  afterEach(() => {
    ...
    if (previousRemoteHooksFlag === undefined) {
      delete process.env[MANTA_FEATURE_REMOTE_AGENT_HOOKS_ENV]
    } else {
      process.env[MANTA_FEATURE_REMOTE_AGENT_HOOKS_ENV] = previousRemoteHooksFlag
    }
  })
  ...
  it('establishes through a fake relay, spawns a remote PTY, and forwards agent status', async () => {
    ...
    const spawn = await provider!.spawn({
      cols: 120,
      rows: 40,
      cwd: '/home/manta/project',
      env: {
        MANTA_PANE_KEY: `tab-ssh:${SSH_LEAF_ID}`,
        MANTA_TAB_ID: 'tab-ssh',
        MANTA_WORKTREE_ID: 'wt-ssh'
      }
    })
  ...
  it('drops malformed remote hook notifications at Manta main before caching', async () => {
    relay = createFakeRelay()
    vi.mocked(deployAndLaunchRelay).mockResolvedValue({
      transport: relay.transport,
      serverBuildId: 'test-relay-build',
