  ...
  if (!localRelayDir) {
    throw new Error(
      `Relay package for ${platform} not found locally. ` +
        `This may be a packaging issue — try reinstalling Manta.`
    )
  }
...
/**
 * A marker is only meaningful where a split namespace can occur and where Manta
...
const NATIVE_DEPS_MISSING_PREFIX = 'MANTA-NATIVE-DEPS-MISSING:'
 ...
 * Same verdict discipline as `src/main/mantad/node-pty-precondition.ts` and
...
async function probeRequiredNativeDeps(
  ...
  const probeJs = nativeDepsProbeJs('MANTA-NATIVE-DEPS-OK')
  let probeStderr = ''
  try {
    const command = isWindowsRemoteHost(hostPlatform)
      ...
      : // Why: no `2>/dev/null` — it discarded the only line that says why node never reached the
        ...
        // src/main/mantad/node-pty-precondition.ts.
        commandWithNodePath(
          hostPlatform,
          nodePath,
          remoteDir,
    ...
    if (probe.includes('MANTA-NATIVE-DEPS-OK')) {
...
async function probeInstalledNativeDeps(
  ...
  const PROBE_OK = 'MANTA-NPTY-PROBE-OK'
...
export function getLocalRelayCandidates(platform: RelayPlatform): string[] {
  ...
  if (process.env.MANTA_RELAY_PATH) {
    candidates.push(join(process.env.MANTA_RELAY_PATH, platform))
  }
