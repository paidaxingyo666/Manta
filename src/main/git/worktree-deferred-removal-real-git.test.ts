...
// accepts `worktree remove --force` on a path Manta just renamed away.
...
beforeEach(async () => {
  // realpath: macOS hands out /var/... temp paths while Git reports /private/var/..., and Manta
  // matches the worktree it is removing against Git's own list.
  scratchDir = await realpath(await mkdtemp(join(tmpdir(), 'manta-deferred-worktree-removal-')))
