...
describe('isGitRepo', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(path.join(tmpdir(), 'manta-repo-detect-'))
  })
  ...
  it('rejects a .git file that points at a missing gitdir when git cannot be run', () => {
    ...
    mkdirSync(fakeRepo)
    writeFileSync(path.join(fakeRepo, '.git'), 'gitdir: /missing/manta/gitdir')
...
describe('getLinkedWorktreeMainRepoRoot', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(path.join(tmpdir(), 'manta-linked-worktree-'))
  })
  ...
  function initRepoWithCommit(repoRoot: string): void {
    ...
    git(repoRoot, ['config', 'user.email', 'test@manta.test'])
    git(repoRoot, ['config', 'user.name', 'Manta Test'])
