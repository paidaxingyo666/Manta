...
// duplicates disk; `manta.yaml` names the ones every worktree should share instead.
...
/** Every path Manta may have symlinked into a worktree: the per-user Worktree
 *  Shared Paths setting plus the repo's `manta.yaml` shared directories.
 *
...
/** Resolve `worktree.sharedDirectories` from the repo-root `manta.yaml` to
