...
const originalAdmissionDisabled = process.env.MANTA_GIT_ADMISSION_DISABLED

afterEach(async () => {
  if (originalAdmissionDisabled === undefined) {
    delete process.env.MANTA_GIT_ADMISSION_DISABLED
  } else {
    process.env.MANTA_GIT_ADMISSION_DISABLED = originalAdmissionDisabled
  }
  _resetGitAdmissionForTests()
...
async function createStubGit(root: string): Promise<string> {
  ...
  await writeFile(
    ...
    `#!/bin/sh
set -eu
live="$MANTA_STUB_STATE_DIR/$MANTA_STUB_ID.live"
: > "$live"
trap 'rm -f "$live"' EXIT HUP INT TERM
sleep "$(awk "BEGIN { print $MANTA_STUB_SLEEP_MS / 1000 }")"
printf 'stub:%s\\n' "$*"
`
  )
...
function setAdmissionMode(mode: StormMeasurement['mode']): void {
  if (mode === 'disabled') {
    process.env.MANTA_GIT_ADMISSION_DISABLED = '1'
  } else {
    delete process.env.MANTA_GIT_ADMISSION_DISABLED
  }
}
...
async function measureStorm(mode: StormMeasurement['mode']): Promise<StormMeasurement> {
  ...
  const root = await mkdtemp(path.join(tmpdir(), `manta-git-storm-${mode}-`))
  tempRoots.push(root)
  ...
  const background = Array.from({ length: 60 }, (_, index) =>
    gitExecFileAsync(['status', '--porcelain=v2', `storm-${index}`], {
      ...
      env: {
        ...baseEnv,
        MANTA_STUB_ID: `background-${index}`,
        MANTA_STUB_SLEEP_MS: index % 10 === 0 ? '5000' : '200',
        MANTA_STUB_STATE_DIR: stateDir
      },
            ...
            () => {
              void (async () => {
                ...
                await gitExecFileAsync(['rev-parse', commandLabel], {
                  ...
                  env: {
                    ...baseEnv,
                    MANTA_STUB_ID: `interactive-${index}`,
                    MANTA_STUB_SLEEP_MS: String(Math.max(10, concurrentAtInjection * 12)),
                    MANTA_STUB_STATE_DIR: stateDir
                  },
