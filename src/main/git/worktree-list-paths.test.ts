...
async function createRepoWithNewlineWorktree(): Promise<{
  ...
  const root = await mkdtemp(path.join(tmpdir(), 'manta-worktree-paths-'))
  tempRoots.push(root)
...
async function createRepoWithLockedDeletedWorktree(): Promise<{
  ...
  const root = await mkdtemp(path.join(tmpdir(), 'manta-worktree-locked-delete-'))
  tempRoots.push(root)
  const repoPath = path.join(root, 'repo')
  ...
  git(repoPath, ['worktree', 'lock', '--reason', 'manta locked stale repro', requestedWorktreePath])
...
async function createRepoWithPrunableWorktree(): Promise<{
  ...
  const root = await mkdtemp(path.join(tmpdir(), 'manta-worktree-prunable-'))
  tempRoots.push(root)
  const repoPath = path.join(root, 'repo')
