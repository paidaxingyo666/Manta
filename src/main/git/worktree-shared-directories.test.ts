...
describe('resolveWorktreeSharedDirectories', () => {
  ...
  const writeMantaYaml = (body: string): void => {
    writeFileSync(join(repo, 'manta.yaml'), body)
  }

  beforeEach(() => {
    clearConfiguredWorktreeSharedDirectoriesCacheForTests()
    repo = mkdtempSync(join(tmpdir(), 'manta-shared-dirs-'))
    git(['init', '-q'], repo)
  ...
  it('returns gitignored directories listed under worktree.sharedDirectories', async () => {
    ...
    writeMantaYaml('worktree:\n  sharedDirectories:\n    - node_modules\n    - .cache\n')

    expect(await resolveWorktreeSharedDirectories(repo)).toEqual(['.cache', 'node_modules'])
  })

  it('returns [] when manta.yaml is absent', async () => {
    mkdirSync(join(repo, 'node_modules'))

    expect(await resolveWorktreeSharedDirectories(repo)).toEqual([])
  })

  it('returns [] when manta.yaml has no worktree key', async () => {
    mkdirSync(join(repo, 'node_modules'))
    writeMantaYaml('scripts:\n  setup: pnpm install\n')
...
describe('getConfiguredWorktreeSharedDirectories', () => {
  ...
  beforeEach(() => {
    clearConfiguredWorktreeSharedDirectoriesCacheForTests()
    repo = mkdtempSync(join(tmpdir(), 'manta-shared-dirs-config-'))
  })
  ...
  it('combines live per-user paths with cached repo configuration', () => {
    writeFileSync(join(repo, 'manta.yaml'), 'worktree:\n  sharedDirectories:\n    - node_modules\n')
  ...
  it('returns [] when manta.yaml is absent or has no worktree key', () => {
    expect(getConfiguredWorktreeSharedDirectories(repo)).toEqual([])

    writeFileSync(join(repo, 'manta.yaml'), 'scripts:\n  setup: pnpm install\n')
    clearConfiguredWorktreeSharedDirectoriesCacheForTests()
    expect(getConfiguredWorktreeSharedDirectories(repo)).toEqual([])
  })
...
describe('shared directories and worktree removal', () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'manta-shared-dirs-removal-'))
