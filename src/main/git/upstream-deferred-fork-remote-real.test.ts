...
describe('getUpstreamStatus with a deferred (not-yet-materialized) fork remote', () => {
  ...
  it('reports the graceful "publish" state instead of 0 ahead/0 behind', async () => {
    ...
    git('init', '--quiet')
    git('config', 'user.name', 'Manta Test')
    git('config', 'user.email', 'manta@example.test')
    ...
    // metadata is persisted, but `pr-contributor-manta` was never added as a
    ...
    expect(status).toEqual({
      hasUpstream: false,
      upstreamName: 'pr-contributor-manta/contributor/fix',
      ahead: 0,
      behind: 0,
