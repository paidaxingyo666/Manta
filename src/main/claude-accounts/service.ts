...
export class ClaudeAccountService {
   ...
   * `manta account add` CLI to run `claude login` in the user's own terminal on a
  ...
  private async persistCapturedClaudeAccount(
    ...
    if (!captured.identity.email) {
      throw new Error('Claude login completed, but Manta could not resolve the account email.')
    }
  ...
  private async createTemporaryClaudeConfigDir(location: ManagedClaudeAuthLocation): Promise<{
    ...
    if (location.managedAuthRuntime !== 'wsl') {
      return {
        windowsPath: mkdtempSync(join(tmpdir(), 'manta-claude-login-')),
        linuxPath: null,
        wslDistro: null
      }
    }
    ...
    const created = await runWslProcess({
      ...
      lane: 'probe',
      shell: 'bash',
      script: 'mktemp -d "${TMPDIR:-/tmp}/manta-claude-login.XXXXXX"',
      timeoutMs: 5000,
  ...
  private async createManagedAuthDir(
    ...
    writeFileSync(join(managedAuthPath, '.manta-managed-claude-auth'), `${accountId}\n`, 'utf-8')
    return {
  ...
  private async tryCreateWslManagedAuthDir(
    ...
    const wslLinuxAuthPath = `${home.replace(/\/$/, '')}/.local/share/manta/claude-accounts/${accountId}/auth`
    const created = await runWslProcess({
      distro,
      lane: 'probe',
      shell: 'bash',
      script: 'mkdir -p "$1" && printf \'%s\\n\' "$2" > "$1/.manta-managed-claude-auth"',
  ...
  private async assertManagedAuthPath(
    ...
    if (wslInfo) {
      if (
        !wslInfo.linuxPath.includes('/.local/share/manta/claude-accounts/') ||
        !wslInfo.linuxPath.endsWith('/auth')
      ) {
        throw new Error('Managed WSL Claude auth storage is outside Manta account storage.')
      }
        ...
        try {
          const owned = await runWslProcess({
            ...
            script: [
              ...
              'managed_root="${HOME%/}/.local/share/manta/claude-accounts"',
              'candidate_real=$(readlink -f -- "$candidate")',
              'managed_root_real=$(readlink -f -- "$managed_root")',
              'test -f "$candidate_real/.manta-managed-claude-auth"',
              expectedAccountId
                ? `test "$(cat "$candidate_real/.manta-managed-claude-auth")" = ${shellQuote(expectedAccountId)}`
                : 'test -n "$(cat "$candidate_real/.manta-managed-claude-auth")"',
            ...
            // would fail every managed read/write on a distro Manta could not probe.
            allowDegradedEnvironment: true
          })
      ...
      if (
        ...
        !existsSync(join(candidatePath, '.manta-managed-claude-auth'))
      ) {
        throw new Error('Managed Claude auth storage is not owned by Manta.')
      }
