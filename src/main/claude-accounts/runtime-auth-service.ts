...
const OWNERSHIP_PROBE_TIMEOUT = 'manta-wsl-ownership-probe-timeout'
...
export class ClaudeRuntimeAuthService {
  ...
  // Why: creds Manta last wrote to the shared file; a mismatch on managed→default transition means an external login overwrote it, so adopt it as the new default.
  ...
  private async doSyncForCurrentSelection(target?: ClaudeAccountSelectionTarget): Promise<void> {
    ...
    if (activeAccount.managedAuthRuntime === 'wsl') {
      if (!(await this.getOwnedManagedAuthPath(activeAccount))) {
        console.warn(
          '[claude-runtime-auth] Active WSL managed account is not owned by Manta, restoring system default'
        )
        const nextSelection = setSelectedClaudeAccountIdForTarget(
          normalizeClaudeRuntimeSelection(settings),
          null,
    ...
    if (!(await this.getOwnedManagedAuthPath(activeAccount))) {
      console.warn(
        '[claude-runtime-auth] Active managed account is not owned by Manta, restoring system default'
      )
      if (this.lastSyncedAccountId !== null) {
        if (
          previousAccount &&
        ...
        try {
          const owned = await runWslProcess({
            ...
            script: [
              ...
              `test "$(cat "$candidate_real/.manta-managed-claude-auth")" = ${shellQuote(account.id)}`,
          ...
          // Why rethrow a timeout: null means "not owned by Manta", and the
  ...
  private getOwnedRuntimeOauthBaseline(
    ...
    // Why: managed metadata hints identity but isn't proof Manta wrote .claude.json; use only after a credential surface proves ownership.
