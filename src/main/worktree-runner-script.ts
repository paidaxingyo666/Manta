...
function createWorktreeRunnerScript(args: {
  ...
  // `#!` line opts a script into bash, so the same manta.yaml runs identically for every Windows
  ...
  const gitRelPath = `manta/${runnerBaseName}.${runnerExtension}`
  ...
  if (wslWorktree) {
    ...
    // and PWD the same way, so leaving MANTA_* in C:\ form would make them the lone exception.
...
export function resolveSetupRunnerShell(
  ...
  // Why: existing Windows setup scripts were authored for Manta's cmd runner;
