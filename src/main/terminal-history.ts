...
export function injectHistoryEnv(
  ...
  delete spawnEnv.MANTA_HISTFILE
  ...
  dropInheritedMantaFishHistory(spawnEnv)
  ...
  // below would honour it for every pane, in every worktree. Only a path Manta
  // minted is dropped; a user's own HISTFILE still wins.
  dropInheritedMantaHistFile(spawnEnv)
  ...
  // (`HISTFILE=${ZDOTDIR:-$HOME}/.zsh_history`) and runs before Manta's wrapper
  ...
  spawnEnv.MANTA_HISTFILE = spawnEnv.HISTFILE
