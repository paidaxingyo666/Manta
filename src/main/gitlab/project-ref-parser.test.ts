...
describe('gitlab project ref parsing', () => {
  it('parses HTTPS and SSH GitLab.com remotes', () => {
    ...
    expect(parseGitLabProjectRef('git@gitlab.com:stablyai/manta.git')).toEqual({
      host: 'gitlab.com',
      path: 'stablyai/manta'
    })
  })
  ...
  it('returns null for non-GitLab hosts when host not in knownHosts', () => {
    expect(parseGitLabProjectRef('git@github.com:stablyai/manta.git')).toBeNull()
...
describe('gitlab remote project ref candidates', () => {
  it('extracts self-hosted candidates before the host is trusted', () => {
    expect(parseRemoteProjectRefCandidate('git@gitlab.internal:team/manta.git')).toEqual({
