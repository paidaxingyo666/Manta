 ...
 * Manta's history tree. That is why deletion needs its own path here instead of
 ...
 */
const SESSION_PREFIX = 'manta_'
const RELAY_SESSION_PREFIX = 'manta_relay_'
const SAFE_SESSION_NAME = /^manta_(?:relay_)?[0-9a-f]{1,64}$/
/** Deliberately does NOT match the relay prefix. A relay host keyed by its
 *  CLIENT's worktree ids shares the one fish data dir with any desktop Manta on
...
const MANTA_HISTORY_FILE = /^manta_([0-9a-f]{1,64})_history$/
...
export function dropInheritedMantaFishHistory(env: Record<string, string | undefined>): void {
 ...
 *  Resolve this from the PTY's SPAWN env: Manta can be launched with a different
 ...
 *  the two disagree whenever Manta's environment differs from the PTY's. */
...
/**
 * Delete every Manta-minted fish history file whose worktree is gone.
 *
 ...
 * scan of the data dir can find those. Matching on Manta's own `manta_<hex>_`
...
export function sweepOrphanedFishHistoryFiles(
  ...
  for (const dir of new Set(dirs)) {
    ...
    for (const entry of entries) {
      const hash = MANTA_HISTORY_FILE.exec(entry)?.[1]
