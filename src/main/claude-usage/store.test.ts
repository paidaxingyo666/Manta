...
describe('ClaudeUsageStore', () => {
  ...
  beforeEach(() => {
    tempUserData = mkdtempSync(join(tmpdir(), 'manta-claude-usage-store-'))
  ...
  it('defaults a null legacy opt-in while invalidating the cache', () => {
    writeFileSync(
      join(tempUserData, 'manta-claude-usage.json'),
  ...
  it('reports no data for Manta scope when only non-Manta usage exists', async () => {
  ...
  it('filters sessions by local calendar day instead of raw UTC date prefixes', async () => {
    ...
    const recentSessions = await store.getRecentSessions('manta', '7d', 10)
  ...
  it('adapts Claude scans to pretty-printed cache persistence', async () => {
    ...
    expect(readFileSync(join(tempUserData, 'manta-claude-usage.json'), 'utf-8')).toContain('\n')
  })
})
