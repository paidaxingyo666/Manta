...
const projectRoot = '/workspace/manta'

describe('adoptProvisionedRootSshCheckout', () => {
  let userDataPath: string

  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'manta-provisioned-root-'))
    resetSshProviderAuthorities()
  })
  ...
  it('rejects a recipe checkout on a branch Manta did not request', async () => {
    seedRuntime(userDataPath, projectRoot)
    registerSshGitProvider(connectionId, {
      listWorktrees: vi
        .fn()
  ...
  it('compares Windows checkout roots using runtime path semantics', async () => {
    const windowsRoot = 'C:\\Workspace\\Manta'
    seedRuntime(userDataPath, windowsRoot)
    registerSshGitProvider(connectionId, {
      listWorktrees: vi.fn().mockResolvedValue([gitWorktree('c:/workspace/manta/')]),
      exec: sparseCheckoutProbe(false)
    } as never)
    const { store } = makeStore()
    ...
    const result = await adoptProvisionedRootSshCheckout({
      userDataPath,
      request: request('C:/WORKSPACE/MANTA'),
      repo: repo('c:\\workspace\\manta'),
      store,
      isRepoCurrent: () => true
    })

    expect(result.worktree.path).toBe('c:/workspace/manta/')
  })
...
function seedRuntime(userDataPath: string, root: string): void {
  upsertEphemeralVmRuntime(userDataPath, {
    ...
    recipeResult: {
      ...
      connection: {
        type: 'ssh',
        target: { label: 'Sandbox', host: '127.0.0.1', port: 22, username: 'manta' },
        projectRoot: root
      }
    }
  })
...
function makeStore(): {
  ...
  return {
    store: {
      getSettings: () => ({ nestWorkspaces: false, workspaceDir: '.manta/worktrees' }),
      setWorktreeMeta
    } as unknown as Store,
    setWorktreeMeta
  }
