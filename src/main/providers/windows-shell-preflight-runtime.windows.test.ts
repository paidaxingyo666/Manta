...
function makeTempDir(): string {
  const root = mkdtempSync(join(tmpdir(), 'manta shell preflight '))
  tempDirs.push(root)
  return root
}
...
function writeFailingPreflight(root: string): string {
  const executable = join(root, 'manta preflight.exe')
  ...
  writeFileSync(
    ...
    [
      ...
      "writeFileSync(process.env.MANTA_PREFLIGHT_MARKER, 'ran')",
      'process.exit(7)'
    ].join('\n')
  )
  return executable
...
describeWindows('Windows Codex shell preflight runtime', () => {
  it('runs a spaced executable through cmd.exe and continues after failure', async () => {
    ...
    await runPty({
      ...
      env: {
        ...process.env,
        MANTA_CODEX_LAUNCH_PREFLIGHT: preflight,
        [MANTA_CODEX_LAUNCH_PREFLIGHT_CMD_QUOTE_ENV]: '"',
        MANTA_PREFLIGHT_MARKER: preflightMarker
      }
    })
  ...
  it('runs the typed-Codex wrapper through Git Bash without MSYS switch rewriting', async () => {
    ...
    process.env.MANTA_USER_DATA_PATH = join(root, 'user data')
    ...
    try {
      ...
      await runPty({
        ...
        env: {
          ...
          MANTA_CODEX_LAUNCH_PREFLIGHT: preflight,
          MANTA_PREFLIGHT_MARKER: preflightMarker,
          MANTA_CODEX_MARKER: codexMarker,
          TERM: 'xterm-256color'
        },
        input:
          "codex -e \"require('node:fs').writeFileSync(process.env.MANTA_CODEX_MARKER,'ran')\"\nexit\n",
