...
function readDurableReviewHeadLocalRef(
  ...
  if (result && typeof result === 'object' && 'localRef' in result) {
    ...
    if (typeof localRef === 'string') {
      ...
      if (trimmed.startsWith('refs/manta/')) {
        return trimmed
      }
    }
  }
  ...
  async getSubmoduleStatus(
    ...
    try {
      ...
      if (isJsonRpcMethodNotFoundError(error)) {
        throw new Error(
          'SSH submodule diff support is unavailable on this relay. Reconnect the SSH target to update Manta on the host, then try again.'
        )
      }
      throw error
    }
  ...
  async fetchGitHubPullRequestHead(
    ...
    try {
      ...
      if (isJsonRpcMethodNotFoundError(error)) {
        ...
        // reconnect prompt instead of a raw JSON-RPC method-not-found error.
        throw new Error(
          'This SSH host is running an older Manta relay that cannot fetch pull request heads. Reconnect to deploy the latest relay, then try again.'
        )
      }
      throw error
    }
  ...
  async forceDeletePreservedBranch(
    ...
    try {
      ...
      if (isJsonRpcMethodNotFoundError(error)) {
        ...
        throw new Error(
          'This SSH host is running an older Manta relay that cannot delete preserved branches. Reconnect to deploy the latest relay, then try again.'
        )
      }
      throw error
    }
  ...
  async clone(
    ...
    return this.runWithGitReadInvalidation(async () => {
      ...
      try {
        ...
        if (isJsonRpcMethodNotFoundError(error)) {
          throw new Error(
            'SSH clone support is unavailable on this relay. Reconnect the SSH target to update Manta on the host, then try again.'
          )
        }
        throw error
      } finally {
