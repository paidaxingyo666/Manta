...
describe('wrapShellSpawnForMacosTccAttribution', () => {
  ...
  beforeEach(() => {
    ...
    origDisable = process.env.MANTA_DISABLE_MACOS_LOGIN_SHELL
  ...
  afterEach(() => {
    ...
    if (origDisable === undefined) {
      ...
      process.env.MANTA_DISABLE_MACOS_LOGIN_SHELL = origDisable
    }
    vi.restoreAllMocks()
    vi.clearAllMocks()
  })
  ...
  it('uses the exact positional login-shell trampoline argv on macOS', async () => {
    ...
    expect(execFileMock).toHaveBeenCalledWith(
      '/usr/bin/login',
      ['-flpq', 'ada', '/usr/bin/printf', 'MANTA_LOGIN_PREFLIGHT_OK'],
      {
        cwd: '/Users/ada',
        encoding: 'utf8',
        killSignal: 'SIGKILL',
  ...
  it('preserves the non-login Bash shell-ready rcfile launch', async () => {
    ...
    await prepareMacosTccLoginShell()
    expect(
      wrapShellSpawnForMacosTccAttribution('/bin/bash', ['--rcfile', '/manta/bash/rcfile'])
    ).toEqual({
      file: '/usr/bin/login',
      args: [
        '-flpq',
...
describe('probeMacosLoginSessionAlive', () => {
  ...
  it('reuses an in-flight startup warmup instead of spawning a duplicate probe', async () => {
    ...
    finishPreflight(null, 'MANTA_LOGIN_PREFLIGHT_OK', '')
  ...
  it('does not let a spawn-path probe overwrite a newer death verdict', async () => {
    ...
    callbacks[1](null, 'MANTA_LOGIN_PREFLIGHT_OK', '')
