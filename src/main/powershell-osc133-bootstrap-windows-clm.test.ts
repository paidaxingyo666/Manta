...
const MANAGED_CODEX_HOME = 'C:\\Manta Managed\\codex-runtime-home'
...
for (const shell of WINDOWS_POWERSHELLS) {
  describe.runIf(isAvailable(shell))(`${shell} managed home bootstrap`, () => {
    it.each(['FullLanguage', 'ConstrainedLanguage'] as const)(
      ...
      (languageMode) => {
        const cwd = mkdtempSync(join(tmpdir(), 'manta-powershell-clm-'))
        try {
          expect(runBootstrap(shell, languageMode, cwd)).toContain(
            `mode=${languageMode};codexHome=${MANAGED_CODEX_HOME};mantaHome=${MANAGED_CODEX_HOME};startupCount=2;cwd=${cwd}`
          )
        } finally {
...
function runBootstrap(
  ...
  const launch = resolveWindowsShellLaunchArgs(
    ...
    undefined,
    '$env:MANTA_TEST_STARTUP_COUNT = 1 + [int]$env:MANTA_TEST_STARTUP_COUNT'
  )
  ...
  return execFileSync(
    ...
    ['-NoLogo', '-NoProfile', '-NonInteractive', '-EncodedCommand', harness],
    {
      ...
      env: {
        ...
        MANTA_CODEX_HOME: MANAGED_CODEX_HOME,
        MANTA_TEST_BOOTSTRAP: encodedCommand,
        MANTA_TEST_LANGUAGE_MODE: languageMode
      },
      windowsHide: true
    }
  )
...
$initialState.LanguageMode = $env:MANTA_TEST_LANGUAGE_MODE
...
$bootstrap = [Text.Encoding]::Unicode.GetString(
  [Convert]::FromBase64String($env:MANTA_TEST_BOOTSTRAP)
)
...
$runner.Commands.Clear()
$runner.AddScript(
  '"mode=$($ExecutionContext.SessionState.LanguageMode);codexHome=$env:CODEX_HOME;mantaHome=$env:MANTA_CODEX_HOME;startupCount=$env:MANTA_TEST_STARTUP_COUNT;cwd=$($PWD.Path)"'
).Invoke()
$runner.Dispose()
$runspace.Dispose()
`)
