...
const MANTA_LOCALHOST_SUFFIX = '.manta.localhost'
...
export class LocalhostWorktreeLabelProxy {
  ...
  private buildLabeledUrl(label: string, target: URL): string {
    ...
    url.hostname = `${label}${MANTA_LOCALHOST_SUFFIX}`
  ...
  private async handleRequest(request: IncomingMessage, response: ServerResponse): Promise<void> {
    ...
    if (!route) {
      ...
      response.end('Unknown Manta localhost label.')
      return
    }
  ...
  private routeForRequest(request: IncomingMessage): RegisteredRoute | null {
    const host =
      String(request.headers.host ?? '')
        .split(':')[0]
        ?.toLowerCase() ?? ''
    if (!host.endsWith(MANTA_LOCALHOST_SUFFIX)) {
      return null
    }
    const label = host.slice(0, -MANTA_LOCALHOST_SUFFIX.length)
