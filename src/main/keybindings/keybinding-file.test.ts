...
describe('keybinding-file', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'manta-keybindings-'))
  ...
  it('resolves the user-facing keybindings path under ~/.manta', () => {
    expect(getUserKeybindingsPath('/home/test')).toBe(
      join('/home/test', '.manta', 'keybindings.json')
    )
  })
  ...
  it('preserves valid plugin overrides while rejecting malformed plugin action IDs', () => {
    writeFileSync(
      ...
      JSON.stringify({
        keybindings: {
          'plugin:manta-samples.tasks/open': 'Mod+Shift+T',
    ...
    writeKeybindingOverride(filePath, 'linux', 'plugin:manta-samples.tasks/open', [])
    expect(readKeybindingFile(filePath, 'linux').overrides).toEqual({
      'plugin:manta-samples.tasks/open': []
    })
  })
