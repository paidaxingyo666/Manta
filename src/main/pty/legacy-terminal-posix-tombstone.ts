...
const POSIX_TOMBSTONE = String.raw`#!__MANTA_INTERPRETER__
set -u

command_name="__MANTA_COMMAND__"
...
legacy_wrapper_dir="${SHELL_DOLLAR}{MANTA_ATTRIBUTION_SHIM_DIR:-}"
...
filter_path() {
  ...
  while true; do
    ...
    if [[ "$entry" != /* ]]; then
      ...
      # directory, so a relative MANTA_ATTRIBUTION_SHIM_DIR let the cwd decide which PATH entry
...
filter_path
unset MANTA_ENABLE_GIT_ATTRIBUTION MANTA_GIT_COMMIT_TRAILER MANTA_GH_PR_FOOTER
unset MANTA_GH_ISSUE_FOOTER MANTA_ATTRIBUTION_SHIM_DIR MANTA_REAL_GIT MANTA_REAL_GH MANTA_ATTRIBUTION_BYPASS
...
if [[ -z "$real_command" ]]; then
  printf 'Manta compatibility wrapper could not locate %s on PATH.\n' "$command_name" >&2
  exit 127
fi
...
export function renderLegacyTerminalPosixTombstone(
  ...
  return POSIX_TOMBSTONE.replaceAll('__MANTA_INTERPRETER__', interpreter).replaceAll(
    '__MANTA_COMMAND__',
    command
  )
}
