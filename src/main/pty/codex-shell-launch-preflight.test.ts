  ...
  return execFileSync(
    ...
    ],
    {
      ...
      env: {
        ...
        MANTA_CODEX_LAUNCH_PREFLIGHT: join(bin, 'manta-test')
      }
    }
  ).trim()
}

/** Launches a startup file that aliases the very name Manta wraps, then asserts the
...
function expectNamedAliasSurvives(shell: string, enableAliases: string): void {
  const root = mkdtempSync(join(tmpdir(), 'manta-codex-named-alias-'))
  roots.push(root)
  const bin = join(root, 'bin')
  mkdirSync(bin)
...
describe.skipIf(process.platform === 'win32')('Codex shell launch preflight', () => {
  it('repairs trust invalidated after shell creation before an alias launches Codex', () => {
    const beforeRoot = mkdtempSync(join(tmpdir(), 'manta-codex-shell-before-'))
    const afterRoot = mkdtempSync(join(tmpdir(), 'manta-codex-shell-after-'))
  ...
  it.skipIf(!existsSync('/bin/zsh'))('repairs a zsh cx alias before Codex starts', () => {
    const root = mkdtempSync(join(tmpdir(), 'manta-codex-zsh-alias-'))
    roots.push(root)
  ...
  it('still launches Codex when the best-effort preflight fails', () => {
    const root = mkdtempSync(join(tmpdir(), 'manta-codex-preflight-failure-'))
    roots.push(root)
  ...
  for (const [shell, strict] of [
    ...
    it.skipIf(!existsSync(shell))(
      ...
      () => {
        const root = mkdtempSync(join(tmpdir(), 'manta-codex-strict-startup-'))
        roots.push(root)
        ...
        const output = execFileSync(
          ...
          {
            encoding: 'utf-8',
            env: { ...process.env, PATH: root, MANTA_CODEX_LAUNCH_PREFLIGHT: 'manta-test' }
          }
        )
  ...
  it.skipIf(!fishAvailable)('preserves a user-defined fish function', () => {
    const output = execFileSync(
      ...
      [
        ...
        '-c',
        [
          'set -gx MANTA_CODEX_LAUNCH_PREFLIGHT missing-preflight',
...
describe('PowerShell Codex shell launch preflight', () => {
  ...
  it.skipIf(!pwshAvailable)('fails open when native errors are promoted', () => {
    const root = mkdtempSync(join(tmpdir(), 'manta-codex-pwsh-failure-'))
    const bin = join(root, 'bin')
    roots.push(root)
    mkdirSync(bin)
    const executableSuffix = process.platform === 'win32' ? '.cmd' : ''
    writeExecutable(
      join(bin, `manta-test${executableSuffix}`),
    ...
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          PATH: `${bin}${delimiter}${process.env.PATH ?? ''}`,
          MANTA_CODEX_LAUNCH_PREFLIGHT: join(bin, `manta-test${executableSuffix}`)
        }
      }
    )
...
describe.skipIf(process.platform === 'win32')('Codex preflight paths containing spaces', () => {
  function writeSpacedPreflight(root: string): { preflightPath: string; markerPath: string } {
    const dir = join(root, 'Manta Dev.app', 'Contents', 'Resources', 'bin')
    mkdirSync(dir, { recursive: true })
    const markerPath = join(root, 'preflight-ran')
    const preflightPath = join(dir, 'manta')
    writeExecutable(
      preflightPath,
  ...
  it('invokes a POSIX preflight whose absolute path contains spaces', () => {
    const root = mkdtempSync(join(tmpdir(), 'manta-codex-spaced-posix-'))
    roots.push(root)
    const bin = join(root, 'bin')
    mkdirSync(bin)
    writeExecutable(join(bin, 'codex'), '#!/bin/sh\nexit 0\n')
    ...
    execFileSync(
      ...
      ['--noprofile', '--norc', '-c', `${getPosixCodexShellLaunchPreflight()}\ncodex`],
      {
        env: {
          ...process.env,
          PATH: `${bin}${delimiter}${process.env.PATH ?? ''}`,
          MANTA_CODEX_LAUNCH_PREFLIGHT: preflightPath
        }
      }
    )
  ...
  it.skipIf(!fishAvailable)('invokes a fish preflight whose absolute path contains spaces', () => {
    const root = mkdtempSync(join(tmpdir(), 'manta-codex-spaced-fish-'))
    roots.push(root)
    const bin = join(root, 'bin')
    mkdirSync(bin)
    writeExecutable(join(bin, 'codex'), '#!/bin/sh\nexit 0\n')
  ...
  it.skipIf(!pwshAvailable)(
    ...
    () => {
      const root = mkdtempSync(join(tmpdir(), 'manta-codex-spaced-pwsh-'))
      roots.push(root)
      const bin = join(root, 'bin')
      mkdirSync(bin)
      writeExecutable(join(bin, 'codex'), '#!/bin/sh\nprintf launched\n')
