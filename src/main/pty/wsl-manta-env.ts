...
function worktreeSetupWslenvEntries(env: Record<string, string | undefined>): string[] {
  return [
    ...
    ...['MANTA_ROOT_PATH', 'MANTA_WORKTREE_PATH', 'CONDUCTOR_ROOT_PATH', 'GHOSTX_ROOT_PATH'].map(
      (name) => `${name}/${env[name]?.startsWith('/') ? 'u' : 'p'}`
    ),
    // A display name, never a path.
    'MANTA_WORKSPACE_NAME/u'
  ]
}
...
export function addMantaWslInteropEnv(env: Record<string, string>): void {
  ...
  const endpointFlag = env.MANTA_AGENT_HOOK_ENDPOINT?.startsWith('/') ? 'u' : 'p'
  ...
  const opencodeOverlayEntries = (['OPENCODE_CONFIG_DIR', 'MANTA_OPENCODE_CONFIG_DIR'] as const)
  ...
  const passthroughEntries = [
    ...
    'MANTA_ORCHESTRATION_COMPATIBILITY_HOST_ID/u',
    'MANTA_ORCHESTRATION_COMPATIBILITY_HOST_INCARNATION/u',
