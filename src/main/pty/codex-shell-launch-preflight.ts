...
/** Absolute path of the Manta CLI the preflight must execute, or null to skip it.
 *
 *  Why absolute: the value rides in MANTA_CODEX_LAUNCH_PREFLIGHT and is invoked
 ...
 *  would be resolved against a PATH Manta neither controls nor can predict —
 *  handing Manta's managed Codex environment to an unidentified program. When no
...
export function getFishCodexShellLaunchPreflight(): string {
  return `if test -n "$MANTA_CODEX_LAUNCH_PREFLIGHT"; and test (type -t codex 2>/dev/null) = file
  function codex
...
if ($env:MANTA_CODEX_LAUNCH_PREFLIGHT -and $mantaCodexCommand -and
    ...
    function Global:codex {
        ...
        & $mantaCodexExecutable.Source @args
