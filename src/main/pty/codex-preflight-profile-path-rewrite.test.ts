...
function buildFixture(options: { aliasCodex?: boolean } = {}): Fixture {
  const root = mkdtempSync(join(tmpdir(), 'manta-codex-profile-path-'))
  roots.push(root)
  ...
  // The CLI Manta ships, at the absolute path Manta controls.
  writeStub(getBundledLauncherPath(process.platform, resourcesPath) as string, intendedMarker)
  // The impostor a user's own bin directory could hold under every CLI name Manta uses.
  for (const name of ['manta', 'manta-ide', 'manta-dev']) {
...
function launchCodexThroughRcfile(fixture: Fixture, preflightValue: string): void {
  ...
  // Why the second command: the rcfile defines __manta_osc133_preexec *below* the
  ...
  const command = `codex --version; declare -F __manta_osc133_preexec >/dev/null && printf ran > ${JSON.stringify(fixture.postSnippetMarker)}; :`
...
describe.skipIf(!bashAvailable)('Codex preflight under a profile-rewritten PATH', () => {
  it('sources the user profile before emitting the codex wrapper', () => {
    ...
    expect(rcfile.indexOf('source "$HOME/.bash_profile"')).toBeLessThan(
      rcfile.indexOf('MANTA_CODEX_LAUNCH_PREFLIGHT')
    )
  })
  ...
  it('would run the impostor if the preflight carried an unqualified command name', () => {
    ...
    launchCodexThroughRcfile(fixture, 'manta')
