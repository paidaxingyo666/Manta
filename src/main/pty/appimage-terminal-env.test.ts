...
describe('removeAppImageRuntimeEnv', () => {
  ...
  it('removes AppImage runtime identity and mount paths from Linux terminal env', () => {
    ...
    const env = {
      APPIMAGE: '/data/apps/manta.appimage',
      APPDIR: '/tmp/.mount_manta123',
      ARGV0: '/data/apps/manta.appimage',
      OWD: '/home/user',
      APPIMAGE_LIBRARY_PATH: '/tmp/.mount_manta123/usr/lib',
      PATH: ['/tmp/.mount_manta123', '/tmp/.mount_manta123/usr/sbin', '/usr/bin'].join(delimiter),
      LD_LIBRARY_PATH: ['/tmp/.mount_manta123/usr/lib', '/opt/audio/lib'].join(delimiter),
      HOME: '/home/user'
    }
  ...
  it('leaves AppImage-looking env untouched outside Linux', () => {
    ...
    const env = {
      ...
      APPDIR: '/tmp/.mount_manta123',
      ARGV0: '/data/apps/manta.appimage',
      PATH: ['/tmp/.mount_manta123/usr/bin', '/usr/bin'].join(delimiter)
    }
