...
// Why: Manta launches bash as a login shell (see local-pty-shell-ready.ts
...
// .bashrc. Scanning .bashrc would mirror values the live Manta bash never sees.
...
function shellStartupFiles(
  ...
  if (!shell) {
    // Why: Manta's POSIX default shell is /bin/zsh when $SHELL is unset.
  ...
  // Manta's zsh/bash/fish shell-ready startup files, so scanning those files
 ...
 *
 * Why: GUI-launched Manta does not inherit interactive shell exports, but the
