...
describe('legacy terminal Windows tombstone text', () => {
  ...
  const makeUserDataDir = (): string => {
    const userData = mkdtempSync(join(tmpdir(), 'manta-legacy-shim-win-'))
  ...
  it('rejects stale Windows real-command paths inside the wrapper directory', () => {
    ...
    expect(cmd).toContain('if defined manta_real if not exist "%manta_real%" set "manta_real="')
    expectOrdered(cmd, 'if not exist "%manta_real%"', ':manta_try_candidate')
  ...
  it('resolves Windows fallbacks against PATH only, never the current directory', () => {
    ...
    for (const command of ['git', 'gh'] as const) {
      ...
      expect(cmd).toContain('for %%P in ("%manta_clean_path:;=" "%") do (')
      expect(cmd).toContain(`if exist "%manta_candidate_dir%\\${command}.exe"`)
      ...
      expect(cmd).toContain('set "manta_sep=\\"')
      ...
      // Why: a captured MANTA_REAL_* may be relative, and both the existence test and the
      // invocation resolve it against the cwd.
      expect(cmd).toContain('if defined manta_real call :manta_check_rooted')
      expect(cmd).toContain('if defined manta_real if not defined manta_rooted set "manta_real="')
      ...
      // Why the exact block, not an ordering pin: the first ':manta_append_path' in the file is
      // the top-level CALL, and the first 'if not defined manta_rooted' belongs to a different
      // subroutine, so an ordering pin stayed green with this subroutine's guard deleted.
      expect(cmd).toContain(':manta_append_path\r\nif not defined manta_entry exit /b\r\n')
      expect(cmd).toContain(
      ...
      expect(cmd).not.toContain('call :manta_check_rooted "')
      expect(cmd).not.toContain('call :manta_try_candidate "')
      expect(cmd).not.toContain('call :manta_append_path "')
      expect(cmd).not.toContain('%~1')
      expect(cmd).toContain('set "manta_probe=%manta_entry%"\r\ncall :manta_check_rooted\r\n')
      // Why: manta_sep is compared against before the legacy dir is normalized; defining it later
      // made that strip silently no-op and left the legacy dir on PATH.
      expectOrdered(cmd, 'set "manta_sep=', 'if "%manta_legacy_norm:~-1%."')
      // Why: %manta_sep% expands before the line is parsed, so `=="%manta_sep%"` becomes `=="\"` —
      // the literal trap the variable exists to avoid. Both sides must carry the trailing dot.
      expect(cmd).not.toContain('=="%manta_sep%"')
      ...
      expectOrdered(
        cmd,
        'call :manta_check_rooted\r\nif not defined manta_rooted exit /b',
        'for %%G in ("%manta_entry%") do set "manta_candidate_dir='
      )
      expect(cmd).toContain('if not defined manta_rooted exit /b')
      expect(cmd).toContain('if "%manta_candidate_dir:~-1%."=="%manta_sep%."')
      expect(cmd).not.toContain(':\\#=#%')
      ...
      expect(cmd).toContain('if /I "%manta_candidate_dir%\\"=="%manta_wrapper_dir%" exit /b')
      ...
      expect(cmd).toContain('if "%manta_probe:~1,2%"==":\\" set "manta_rooted=1"')
      expect(cmd).toContain('if "%manta_probe:~0,2%"=="\\\\" set "manta_rooted=1"')
      ...
      expect(cmd).toContain('if not defined manta_entry exit /b')
      ...
      expect(cmd).toContain('if "%manta_legacy_norm:~-1%."=="%manta_sep%."')
      ...
      expect(cmd).toContain('if defined manta_legacy_wrapper_dir call :manta_normalize_legacy_dir')
      expect(cmd).not.toContain('if defined manta_legacy_wrapper_dir for ')
  ...
  it('guards both cmd PATH walks so an empty variable cannot break parsing', () => {
    ...
    for (const command of ['git', 'gh'] as const) {
      const cmd = readFileSync(join(win32Dir, `${command}.cmd`), 'utf8')
      expect(cmd).toContain('if not defined PATH goto :manta_path_walked')
      expect(cmd).toContain('if not defined manta_clean_path goto :manta_candidates_walked')
      for (const [guard, loop] of [
        ['if not defined PATH goto :manta_path_walked', 'for %%P in ("%PATH:;='],
        [
          'if not defined manta_clean_path goto :manta_candidates_walked',
          'for %%P in ("%manta_clean_path:;='
        ]
      ] as const) {
