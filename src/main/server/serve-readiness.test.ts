...
const ready: ServeReadiness = {
  ...
  advertisedEndpoint: 'wss://manta.example.test/runtime',
  managedWslCliReconciliation: 'settled',
  pairing: {
    available: true,
    url: 'manta://pair?code=secret',
    endpoint: 'wss://manta.example.test/runtime',
    deviceId: 'device-1',
    webClientUrl: 'https://manta.example.test/runtime/web-index.html#pairing=secret',
    scope: 'runtime',
    qr: null
  }
}
...
describe('ServeReadinessPublisher', () => {
  it('writes one complete human-readable ready block', async () => {
    ...
    expect(write).toHaveBeenCalledWith(
      expect.stringContaining(
        'Manta server ready\nBound endpoint: ws://0.0.0.0:6768\nAdvertised endpoint: wss://manta.example.test/runtime'
      )
    )
    expect(write).toHaveBeenCalledWith(
      expect.stringContaining('Pairing URL: manta://pair?code=secret\n')
    )
  })
  ...
  it('preserves the recipe JSON contract', () => {
    expect(renderServeReadiness(ready, { mode: 'recipe-json', projectRoot: '/workspace' })).toBe(
      '{"schemaVersion":1,"pairingCode":"manta://pair?code=secret","projectRoot":"/workspace"}'
    )
  })
