...
beforeEach(async () => {
  ...
  setPlatform('linux')
  resourcesDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'manta-package-type-'))
...
describe('getLinuxRootPackageType', () => {
  ...
  it.each([
    ['executable', '/tmp/.mount_manta-shadow/manta', '/tmp/.mount_manta/resources'],
    ['resources', '/tmp/.mount_manta/manta', '/tmp/.mount_manta-shadow/resources']
  ])(
