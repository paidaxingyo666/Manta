...
describe('parseTccPromptEvent', () => {
  it('parses a real captured AUTHREQ_PROMPTING line', () => {
    expect(parseTccPromptEvent(REAL_PROMPT_LINE)).toEqual({
      ...
      accessingIdentifier: 'com.manta.tccprobe.shapecapture',
      responsibleIdentifier: 'com.manta.tccprobe.shapecapture',
  ...
  it('separates the accessing binary from the responsible app', () => {
    const event = parseTccPromptEvent(MANTA_APPDATA_LINE)
    // The whole point of #9756: the dialog says Manta, but node did the access.
    expect(event?.responsibleIdentifier).toBe('cn.sh.manta')
...
describe('isMantaAttributedPrompt', () => {
  it('accepts the app and detached terminal helper across Manta build identities', () => {
    for (const id of [
      'cn.sh.manta',
      'cn.sh.manta.helper',
      'cn.sh.manta.dev',
      'cn.sh.manta.dev.helper',
      'cn.sh.manta.local',
      'cn.sh.manta.local.helper'
    ]) {
      expect(
        isMantaAttributedPrompt({
  ...
  it('rejects unrelated services even when Manta is responsible', () => {
    expect(
      isMantaAttributedPrompt({
        service: 'kTCCServiceMicrophone',
        accessingIdentifier: 'manta',
        responsibleIdentifier: 'cn.sh.manta'
      })
    ).toBe(false)
  })
})
...
describe('MacosTccPromptWatch', () => {
  ...
  it('reports only Manta-attributed dialogs from a live stream', async () => {
    ...
    stdout.write(`${MANTA_APPDATA_LINE}\n`)
