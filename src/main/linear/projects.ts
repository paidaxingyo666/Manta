...
const MANTA_PROJECT_DETAIL_FIELDS = `
...
const CUSTOM_VIEW_QUERY = `
  query MantaLinearCustomView($id: String!) {
    customView(id: $id) {
      id
      name
      description
...
const CUSTOM_VIEW_ISSUES_QUERY = `
  query MantaLinearCustomViewIssues(
    $id: String!,
    $first: Int,
    $after: String,
    $orderBy: PaginationOrderBy
...
const CUSTOM_VIEW_PROJECTS_QUERY = `
  query MantaLinearCustomViewProjects($id: String!, $first: Int, $orderBy: PaginationOrderBy) {
    customView(id: $id) {
      id
      modelName
...
async function readIssueConnectionPages(
  ...
  while (items.length < limit) {
    ...
    // Manta reads must follow cursors to show more than one backend page.
