...
describe('OpenCode status plugin module contract', () => {
  ...
  // env. Pin every input here so the run does not depend on the developer's Manta session
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'manta-opencode-plugin-contract-'))
  ...
  async function loadPluginModule(): Promise<PluginModule> {
    ...
    const pluginPath = join(
      tempDir,
      `manta-opencode-status-${Math.random().toString(36).slice(2)}.mjs`
    )
  ...
  it('exposes a default export carrying a string id and a callable server()', async () => {
    ...
    expect(module.default?.id).toBe('manta-opencode-status')
  ...
  it('keeps the named factory export so the factory-based loader still resolves', async () => {
    ...
    expect(module.MantaOpenCodeStatusPlugin).toBeTypeOf('function')
  })
  ...
  it('returns an event handler from the default export server(), like the named factory', async () => {
    ...
    const fromNamed = await module.MantaOpenCodeStatusPlugin?.({})
