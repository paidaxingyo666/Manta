 ...
 * lives inside OpenCode's process, not in Manta's TypeScript runtime.
 */
...
describe('OpenCode plugin lifecycle delivery', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'manta-opencode-lifecycle-plugin-'))
    posts = []
    savedEnv = {}
  ...
  async function loadHooksWithSession(session: object): Promise<PluginHooks> {
    ...
    const module = (await import(pathToFileURL(pluginPath).href)) as {
      MantaOpenCodeStatusPlugin: (ctx: unknown) => Promise<PluginHooks>
    }
    return module.MantaOpenCodeStatusPlugin({ client: { session } })
  }
  ...
  it('reasserts Busy through the refreshed endpoint after a live text delta', async () => {
    ...
    globalThis.fetch = vi.fn(async (url: RequestInfo | URL, init?: RequestInit) => {
      ...
      deliveries.push({
        url: String(url),
        token: new Headers(init?.headers).get('X-Manta-Agent-Hook-Token')
      })
      return new Response(null, { status: 204 })
    }) as typeof globalThis.fetch
    const handler = await loadHandler()
    ...
    await handler({ event: status('busy') })
    process.env.MANTA_AGENT_HOOK_PORT = '56789'
    process.env.MANTA_AGENT_HOOK_TOKEN = 'refreshed-token'
