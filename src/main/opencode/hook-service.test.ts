...
describe('OpenCode hook plugin source', () => {
  ...
  it('keeps family routing and session-start policy separate', () => {
    ...
    expect(familySource).toContain('export const MantaOpenCodeStatusPlugin')
  })
  ...
  it('keeps generated plugin bytes stable across the module split', () => {
    ...
    expect(digest(getOpenCodePluginSource())).toBe(
      '43ff47a39daaa7c8cbd8dc9dba4151c861bd1704e7cc53ea754b507e62041549'
    )
    expect(
      digest(getOpenCodeFamilyPluginSource('/hook/mimo-code', { emitSessionStart: false }))
    ).toBe('a9227b540c0f67ad53ec9603b9742f478a81c56f1aabc27e00492ed16e75df6d')
  })
  ...
  it('still accepts an optional opaque plugin context instead of destructuring', () => {
    ...
    expect(source).toContain('export const MantaOpenCodeStatusPlugin = async (_ctx) => {')
  ...
  it('resolves hook coords from the endpoint file before falling back to process.env', () => {
    // Why: a forked session freezes the prior Manta's PORT/TOKEN in env; prefer the on-disk endpoint file or it posts to a dead port after restart.
    const source = _internals.getOpenCodePluginSource()

    expect(source).toContain('function readEndpointFile()')
    expect(source).toContain('process.env.MANTA_AGENT_HOOK_ENDPOINT')
    ...
    expect(source).toContain('"X-Manta-Agent-Hook-Token": coords.token')
  })
...
describe('OpenCodeHookService buildPtyEnv / clearPty round-trip', () => {
  ...
  beforeAll(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'manta-opencode-hooks-'))
  ...
  it('writes a shared OPENCODE_CONFIG_DIR and installs the plugin file', () => {
    ...
    const pluginPath = join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'manta-opencode-status.js')
    expect(existsSync(pluginPath)).toBe(true)
    // Sanity-check the file has plugin source, not a stray write.
    const pluginSource = readFileSync(pluginPath, 'utf8')
    expect(pluginSource).toContain('MantaOpenCodeStatusPlugin')
  ...
  it('buildPtyEnv preserves a user-set OPENCODE_CONFIG_DIR when the id is unusable', () => {
    ...
    const service = new OpenCodeHookService()
    const userDir = mkdtempSync(join(tmpdir(), 'manta-opencode-userdir-'))
    try {
...
describe('OpenCodeHookService overlay mode (user OPENCODE_CONFIG_DIR set)', () => {
  ...
  beforeAll(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'manta-opencode-overlay-userdata-'))
    getPathMock.mockImplementation((name: string) => {
      if (name === 'userData') {
        return userDataDir
      }
  ...
  beforeEach(() => {
    userConfigDir = mkdtempSync(join(tmpdir(), 'manta-opencode-overlay-userconfig-'))
  ...
  it('builds an overlay under userData and exposes user config + Manta plugin together', () => {
    const service = new OpenCodeHookService()
    ...
    // Manta's status plugin is a sibling, not a replacement.
    const mantaPluginPath = join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'manta-opencode-status.js')
    expect(existsSync(mantaPluginPath)).toBe(true)
    expect(readFileSync(mantaPluginPath, 'utf8')).toContain('MantaOpenCodeStatusPlugin')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      // Why: only plugins/ needs per-entry mirroring (Manta drops a sibling); other entries are single symlinks so user edits propagate live.
      const service = new OpenCodeHookService()
      const env = service.buildPtyEnv(ptyId, userConfigDir)
      ...
      // plugins/ must be a real dir in the overlay so Manta can drop its sibling plugin.
  ...
  it("does not overwrite a user plugin file with the same filename as Manta's plugin", () => {
    // Why: a user plugin named manta-opencode-status.js must not be symlinked into the overlay, or writeFileSync would clobber it.
    const userMantaSentinel = 'USER OWNED MANTA-NAMED PLUGIN — DO NOT CLOBBER'
    writeFileSync(join(userConfigDir, 'plugins', 'manta-opencode-status.js'), userMantaSentinel)

    const service = new OpenCodeHookService()
    const env = service.buildPtyEnv(ptyId, userConfigDir)

    // User's source file must be untouched.
    expect(readFileSync(join(userConfigDir, 'plugins', 'manta-opencode-status.js'), 'utf8')).toBe(
      userMantaSentinel
    )

    // Overlay copy is Manta's real plugin source, not the user's file.
    const overlayPlugin = readFileSync(
      join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'manta-opencode-status.js'),
      'utf8'
    )
    expect(overlayPlugin).toContain('MantaOpenCodeStatusPlugin')
    expect(overlayPlugin).not.toBe(userMantaSentinel)
    expectUserConfigIntact()
  })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      // Why: writing Manta's plugin through a symlinked plugins/ would leak into the user's fs (docs/opencode-config-dir-collision.md).
      const realPluginsDir = mkdtempSync(join(tmpdir(), 'manta-real-plugins-'))
      try {
        ...
        // The user's real filesystem must NOT receive Manta's status plugin.
        expect(existsSync(join(realPluginsDir, 'manta-opencode-status.js'))).toBe(false)
        ...
        // Manta's status plugin lands in the overlay only.
        expect(
          existsSync(join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'manta-opencode-status.js'))
        ).toBe(true)
  ...
  it("preserves the user's OPENCODE_CONFIG_DIR when the path does not exist", () => {
    // Why: leave a nonexistent user path alone so OpenCode surfaces the typo instead of Manta silently hiding it.
    const service = new OpenCodeHookService()
    const missingPath = join(tmpdir(), `manta-opencode-nope-${Date.now()}`)
  ...
  it('rebuilding the overlay for the same ptyId does not corrupt the user dir', () => {
    ...
    expect(
      readFileSync(join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'manta-opencode-status.js'), 'utf8')
    ).toContain('MantaOpenCodeStatusPlugin')
    expectUserConfigIntact()
  })
