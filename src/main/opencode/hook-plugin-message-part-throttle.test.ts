 ...
 * coalesced and capped before POSTing to Manta's agent-hook server. The
 * un-throttled plugin re-posted the full accumulated reply per streamed
 * append — O(n²) bytes per turn — which saturated Manta's main + renderer
...
const ENV_KEYS = ['MANTA_PANE_KEY', 'MANTA_AGENT_HOOK_PORT', 'MANTA_AGENT_HOOK_TOKEN'] as const
...
describe('OpenCode plugin MessagePart throttling', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'manta-opencode-plugin-test-'))
    posts = []
    savedEnv = {}
    ...
    process.env.MANTA_AGENT_HOOK_PORT = '45678'
  ...
  async function loadPluginEventHandler(): Promise<PluginEventHandler> {
    const pluginPath = join(tempDir, 'manta-opencode-status.mjs')
    writeFileSync(pluginPath, _internals.getOpenCodePluginSource())
    const module = (await import(pathToFileURL(pluginPath).href)) as {
      MantaOpenCodeStatusPlugin: (ctx: unknown) => Promise<{ event: PluginEventHandler }>
    }
    const client = {
      session: {
    ...
    const hooks = await module.MantaOpenCodeStatusPlugin({ client })
    return hooks.event
  }
