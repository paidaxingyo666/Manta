...
import { readOrcaChromiumProcessPids } from './manta-chromium-process-pids'
 ...
 * Manta descendant also reads `own`. It bounds the blast radius of a bad
 ...
 * usually belongs to an unrelated process whose chain never passes through Manta,
 ...
 * `own`. That is not remote during teardown, when Manta is itself the process
...
export function classifyWindowsTreeKillTarget(
  ...
  // `taskkill /T /F` on ourselves would take Manta and every pane down with it.
  ...
  // hands teardown a licence to taskkill /T /F Manta's own UI (#10680).
