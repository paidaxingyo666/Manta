...
// Why this file exists: the addon behind `@manta/windows-registry` is vendored source rather
// than a published package, so nothing upstream proves it still decodes the registry the way
// Manta's PATH readers expect. These cases check it against `reg.exe`, which is the only
