...
import { RuntimeEmulatorCommands } from '../runtime/manta-runtime-emulator'
...
describe('EmulatorBridge helper ownership', () => {
  ...
  it('stops the previous Manta-managed helper when a worktree switches devices', async () => {
  ...
  it('shuts down the previous Manta-managed device when requested', async () => {
    const bridge = new EmulatorBridge()
    bridge.registerActiveEmulator('wt-1', session('device-old'), { managed: true })
  ...
  it('only kills Manta-managed helpers during app shutdown cleanup', async () => {
    const bridge = new EmulatorBridge()
