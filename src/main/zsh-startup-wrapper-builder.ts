 ...
 * .zlogin) Manta writes for every transport: local PTY, daemon/SSH, and relay.
 *
 ...
 *
 * All order-sensitive Manta work lives in ONE `__manta_shell_epilogue` defined in
...
export type ZshStartupWrapperSpec = {
  /** First line of every generated file, e.g. `# Manta zsh shell-ready wrapper`. */
  headerLabel: string
   ...
   * the inherited ZDOTDIR and republishes it as MANTA_USER_ZDOTDIR.
   */
...
const ZSH_OSC133_COMMAND_MARKER_BLOCK = `__manta_osc133_precmd() {
  local exit_code=$?
...
/** One epilogue feature: `if __manta_has_feature <name>; then ... fi`. */
...
/**
 * Every Manta-owned, order-sensitive step, in one function called once.
 *
...
${joinBlocks([
  ...
  // Why ungated: this repairs damage Manta's own ZDOTDIR caused, so it must also
...
/**
 * Why guarded: one wrapper dir can be shared by two concurrently installed Manta
 ...
 */
const EPILOGUE_CALL = '(( ${+functions[__manta_shell_epilogue]} )) && __manta_shell_epilogue'
...
export function buildZshStartupWrapperFiles(spec: ZshStartupWrapperSpec): ZshStartupWrapperFiles {
  return {
    ...
    zprofile: `${joinBlocks([
      ...
      getZshEmulationDegradeBlock({
        userZdotdirExpression: '"$_manta_home"',
        sourcedUserFileTest: '-f "$_manta_home/.zprofile"'
      })
    ])}\n`,
    zshrc: `${joinBlocks([
      `# ${spec.headerLabel}`,
