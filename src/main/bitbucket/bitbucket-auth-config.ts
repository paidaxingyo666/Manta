...
export function getEnvAuthConfig(): BitbucketAuthConfig {
  return {
    baseUrl: envValue('MANTA_BITBUCKET_API_BASE_URL') ?? DEFAULT_API_BASE_URL,
    accessToken: envValue('MANTA_BITBUCKET_ACCESS_TOKEN'),
    email: envValue('MANTA_BITBUCKET_EMAIL'),
    apiToken: envValue('MANTA_BITBUCKET_API_TOKEN')
  }
}
