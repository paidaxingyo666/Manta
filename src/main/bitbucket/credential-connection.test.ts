...
beforeEach(() => {
  process.env = { ...OLD_ENV }
  for (const key of [
    'MANTA_BITBUCKET_ACCESS_TOKEN',
    'MANTA_BITBUCKET_EMAIL',
    'MANTA_BITBUCKET_API_TOKEN',
    'MANTA_BITBUCKET_API_BASE_URL'
  ]) {
    delete process.env[key]
  }
  tempHome = mkdtempSync(join(tmpdir(), 'manta-bitbucket-conn-'))
})
...
describe('Bitbucket credential connection', () => {
  ...
  it('reports environment variables as the source and takes precedence over stored creds', async () => {
    ...
    process.env.MANTA_BITBUCKET_ACCESS_TOKEN = 'env-token'
  ...
  it('lets an env API base URL override the stored one without env credentials', async () => {
    ...
    process.env.MANTA_BITBUCKET_API_BASE_URL = 'https://env.example.com/2.0'
