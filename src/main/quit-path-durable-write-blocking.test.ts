...
const dataFile = (dir: string): string => join(dir, 'manta-data.json')
const statsFile = (dir: string): string => join(dir, 'manta-stats.json')
...
describe('quit-path durable writes never park the main thread', () => {
  ...
  function makeDir(): string {
    const dir = mkdtempSync(join(tmpdir(), 'manta-quit-path-'))
    dirs.push(dir)
    return dir
  }
  ...
  it('store.flushAsync() also moves the active-view and github-cache sidecars off the thread', async () => {
    const dir = makeDir()
    const staleCacheTemp = join(dir, 'manta-github-cache.json.999999.1.orphan.tmp')
    ...
    expect(existsSync(join(dir, 'manta-github-cache.json'))).toBe(true)
  ...
  it('serializes github-cache snapshots and keeps the newest generation', async () => {
    ...
    fsCalls.waitAsync = (fn, target) => {
      if (held || fn !== 'writeFile' || !target.includes('manta-github-cache.json')) {
        return null
      }
      held = true
      signalWrite()
    ...
    expect(JSON.parse(readFileSync(join(dir, 'manta-github-cache.json'), 'utf-8'))).toEqual({
      version: 2
    })
  })
