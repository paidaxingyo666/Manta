...
describe('withAppImageRegistrationLock', () => {
  it('bounds the wait with a wall-clock deadline, not just an attempt count', async () => {
    ...
    await withAppImageRegistrationLock('/cache/manta/appimage', async () => 'done')
  ...
  it('reports a wedged holder with a remedy instead of hanging', async () => {
    ...
    await expect(
      withAppImageRegistrationLock('/cache/manta/appimage', async () => 'done')
    ).rejects.toThrow(/Timed out waiting for another Manta process[\s\S]*remove .*\.lock/)
  })
  ...
  it('releases the lock when the operation throws', async () => {
    ...
    await expect(
      withAppImageRegistrationLock('/cache/manta/appimage', async () => {
