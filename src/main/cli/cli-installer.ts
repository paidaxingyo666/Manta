...
export class CliInstaller extends CliPathRegistration {
  ...
  async getStatus(): Promise<CliInstallStatus> {
    ...
    if (!launcherPath) {
      const detail = this.hasUnverifiedAppImageRuntime
        ? 'Manta could not verify the inherited AppImage runtime identity, so CLI registration is unavailable.'
    ...
    const baseStatus =
      ...
      !isAppImageInstalledLauncherCurrent(extractionOptions)
        ? {
            ...
            detail: `${spec.commandPath} does not point to the current Manta AppImage payload.`
          }
        : inspectedStatus
  ...
  private async installUnlocked(): Promise<CliInstallStatus> {
    ...
    if (initialStatus.state === 'conflict') {
      throw new Error(
        `Refusing to replace non-Manta command at ${initialStatus.commandPath}. Remove it and register again if it is no longer needed.`
      )
    }
    ...
    if (status.state === 'conflict') {
      throw new Error(
        `Refusing to replace non-Manta command at ${status.commandPath}. Remove it and register again if it is no longer needed.`
      )
    }
