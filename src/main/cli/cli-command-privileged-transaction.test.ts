...
async function createPrivilegedFixture() {
  ...
  const commandPath = join(protectedDirectory, 'manta')
  const userDataPath = join(root, 'user-data')
  const appPath = join(root, 'app')
  await mkdir(protectedDirectory)
  await mkdir(join(appPath, 'out', 'cli'), { recursive: true })
  await writeFile(join(appPath, 'out', 'cli', 'index.js'), 'console.log("manta")\n')
...
describe.skipIf(process.platform !== 'darwin' || process.getuid?.() === 0)(
  ...
  () => {
    ...
    it('restores a managed file changed in place after privileged inspection', async () => {
      ...
      await writeFile(
        fixture.commandPath,
        buildUnixDevLauncher('/Applications/Old.app/Contents/MacOS/Manta', oldCliPath, 'user-data')
      )
