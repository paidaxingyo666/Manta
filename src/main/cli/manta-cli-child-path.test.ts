...
vi.mock('./linux-terminal-manta-cli-shim', () => shim)

import { prependOrcaCliDirToChildPath } from './manta-cli-child-path'
...
describe('prependOrcaCliDirToChildPath', () => {
  it('leads packaged Linux PATH with the bare-manta shim dir', () => {
    // Why this matters at all: the Linux CLI installs as `manta-ide` so it never claims GNOME
    // Manta's /usr/bin/orca screen reader, so bare `manta` only works through this shim.
    ...
    expect(shim.ensureLinuxTerminalMantaCliShimDir).toHaveBeenCalledWith({
      userDataPath: USER_DATA
    })
  })
  ...
  it('leaves packaged Linux PATH untouched when no shim could be written', () => {
    shim.ensureLinuxTerminalMantaCliShimDir.mockReturnValue(null)
