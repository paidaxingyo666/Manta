...
export type LinuxBareMantaDispatcherResult = {
  ...
  /** The bundled `manta-ide` launcher the dispatcher execs. */
  target: string | null
}
...
export async function installLinuxBareMantaDispatcher(
  ...
  const installed = await publishDispatcher(
    dispatcherPath,
    insertDispatcherMarker(buildBareMantaCliScript(launcher))
  )
  return installed
...
/** Bare-`manta` script that execs the one Linux CLI launcher. */
export function buildBareMantaCliScript(launcherPath: string): string {
...
async function publishDispatcher(dispatcherPath: string, content: string): Promise<boolean> {
  ...
  const temporaryPath = join(directoryPath, `.manta-dispatcher-${process.pid}-${randomUUID()}`)
