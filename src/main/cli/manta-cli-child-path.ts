/**
 * The PATH entry through which a Manta-launched child reaches THIS app's own CLI.
 *
 * Extracted from `buildPtyHostEnv` so the structured-session lane can apply the identical
 * treatment. A structured worker has no PTY, but its provider child runs `manta orchestration ...`
 * exactly like a PTY worker's agent does, and it was inheriting the ambient PATH instead. On
 * packaged Linux that made bare `manta` resolve to GNOME's /usr/bin/orca screen reader, because
 * Manta's Linux CLI installs as `manta-ide` to avoid claiming that name (stablyai/orca#7904); on
