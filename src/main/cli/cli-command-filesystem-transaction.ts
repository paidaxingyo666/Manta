...
export async function quarantineCommandPath(commandPath: string): Promise<CommandQuarantine> {
  ...
  const directoryPath = join(commandDirectory, `.manta-cli-${process.pid}-${randomUUID()}`)
...
export function buildMacPrivilegedSymlinkTransaction(
  ...
  const transactionDirectory = join(commandDirectory, `.manta-cli-${process.pid}-${randomUUID()}`)
