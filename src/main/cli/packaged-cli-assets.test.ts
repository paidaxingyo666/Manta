...
const linuxLauncherAsset = new URL('../../../resources/linux/bin/manta-ide', import.meta.url)
const darwinLauncherAsset = new URL('../../../resources/darwin/bin/manta', import.meta.url)
const unixLauncherFixtures = [
  {
    name: 'Linux',
    asset: linuxLauncherAsset,
    appDir: ['Manta'],
    launcher: ['resources', 'bin', 'manta-ide'],
    executable: ['manta-ide'],
  ...
  },
  {
    ...
    appDir: ['Manta.app'],
    launcher: ['Contents', 'Resources', 'bin', 'manta'],
    executable: ['Contents', 'MacOS', 'Manta'],
...
describe('packaged CLI assets', () => {
  ...
  it('retries an incomplete listener-state write', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-listener-state-'))
  ...
  itRunsUnixShell.each(unixLauncherFixtures)(
    ...
    async (launcherFixture) => {
      for (const signal of unixTerminationSignals) {
        const root = await mkdtemp(join(tmpdir(), 'manta-unix-cli-signal-'))
...
server.listen(0, '127.0.0.1', () => {
  fs.writeFileSync(process.env.MANTA_TEST_LISTENER_STATE, JSON.stringify({
    pid: process.pid,
...
`,
          ...
          launcher = spawn(launcherPath, [], {
            env: { ...process.env, MANTA_TEST_LISTENER_STATE: statePath },
            stdio: 'ignore'
          })
  ...
  itRunsUnixShell(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'manta-linux-cli-'))
      try {
        const appDir = join(root, 'Manta')
        const resourcesDir = join(appDir, 'resources')
        const launcherDir = join(resourcesDir, 'bin')
        const cliDir = join(resourcesDir, 'app.asar.unpacked', 'out', 'cli')
        const launcherPath = join(launcherDir, 'manta-ide')
        const electronPath = join(appDir, 'manta-ide')
...
`,
        ...
        const commandPath = join(commandDir, 'manta-ide')
        await mkdir(commandDir, { recursive: true })
        await mkdir(join(homeDir, 'manta'), { recursive: true })
  ...
  itRunsUnixShell('runs the AppImage CLI wrapper through APPDIR at runtime', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-appimage-cli-'))
    try {
      const appDir = join(root, 'Manta.AppDir')
      const cliDir = join(appDir, 'resources', 'app.asar.unpacked', 'out', 'cli')
      const cliPath = join(cliDir, 'index.js')
      const appImagePath = join(root, "Manta's AppImage.AppImage")
      const commandPath = join(root, 'manta-ide')
      await mkdir(cliDir, { recursive: true })
      await writeFile(
        cliPath,
        `exports.main = (argv) => {
  console.log(JSON.stringify({
    ...
    mantaNodeOptions: process.env.MANTA_NODE_OPTIONS ?? null,
    nodeReplExternalModule: process.env.NODE_REPL_EXTERNAL_MODULE ?? null,
    mantaNodeReplExternalModule: process.env.MANTA_NODE_REPL_EXTERNAL_MODULE ?? null
  }))
}
`,
        'utf8'
...
`,
      ...
      const payload = JSON.parse(result.stdout) as {
        ...
        mantaNodeOptions: string | null
        nodeReplExternalModule: string | null
        mantaNodeReplExternalModule: string | null
      }
      ...
      expect(payload.mantaNodeOptions).toBe('--trace-warnings')
      expect(payload.nodeReplExternalModule).toBeNull()
      expect(payload.mantaNodeReplExternalModule).toBe('external-loader')
    } finally {
      await rm(root, { recursive: true, force: true })
    }
  })
