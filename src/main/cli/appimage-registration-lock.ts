...
export async function withAppImageRegistrationLock<T>(
  ...
  try {
    ...
    throw new Error(
      `Timed out waiting for another Manta process to finish CLI registration ` +
        `(waited ${Math.round(LOCK_ACQUIRE_DEADLINE_MS / 1000)}s). ` +
        `If no other Manta is running, remove ${join(cacheRootPath, LOCK_TARGET_NAME)}.lock and retry.`,
      { cause: error }
    )
  }
  try {
