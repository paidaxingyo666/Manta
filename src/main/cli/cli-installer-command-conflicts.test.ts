...
describe('CliInstaller', () => {
  ...
  // Why: this test creates a Unix symlink to /tmp/not-manta, which only applies on macOS/Linux.
  it.skipIf(process.platform === 'win32')(
    'refuses to replace an unknown symlink at the command path',
    async () => {
      const fixture = await makeFixture()
      const installPath = join(fixture.root, 'bin', 'manta')
      const existingTarget = '/tmp/not-manta'
  ...
  // Why: packaged app moves can leave a symlink to an older Manta-owned launcher;
  // those are safe to refresh, unlike arbitrary user symlinks.
  it.skipIf(process.platform === 'win32')(
    'replaces stale packaged Manta launcher symlinks',
    async () => {
      const fixture = await makeFixture()
      const commandDir = join(fixture.root, 'bin')
      const installPath = join(commandDir, 'manta')
  ...
  // Why: old dev/package experiments wrote a generated Manta launcher file
  // directly into /usr/local/bin/manta. That broke profiling because Settings
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      await writeFile(
        installPath,
        '#!/usr/bin/env bash\nELECTRON_RUN_AS_NODE=1 /tmp/not-manta "$@"\n',
        'utf8'
      )
      ...
      await expect(installer.install()).rejects.toThrow('Refusing to replace non-Manta command')
      await expect(readFile(installPath, 'utf8')).resolves.toContain('/tmp/not-manta')
    }
  )

  // Why: a dev build can temporarily own the public command on developer
  // machines; packaged Manta should treat that as stale, not a hard conflict.
  it.skipIf(process.platform === 'win32')(
    'replaces stale sibling dev launcher symlinks from packaged installs',
    async () => {
      const fixture = await makeFixture()
      for (const devLauncherName of ['manta', 'manta-dev']) {
        const caseRoot = join(fixture.root, devLauncherName)
        const commandDir = join(caseRoot, 'bin')
        const installPath = join(commandDir, 'manta')
        const userDataPath = join(caseRoot, 'manta')
