...
export class CliCommandInstallation extends CliCommandInspection {
  protected async installSymlink(status: CliInstallStatus): Promise<void> {
    ...
    if (inspected.status.state === 'conflict') {
      throw new Error(
        `Refusing to replace non-Manta command at ${commandPath}. Remove it and register again if it is no longer needed.`
      )
    }
  ...
  protected async removeSymlink(commandPath: string): Promise<void> {
    ...
    if (!launcherPath) {
      throw new Error('The Manta CLI launcher is no longer available.')
    }
    const inspected = await this.inspectStableSymlink(commandPath, launcherPath)
    ...
    if (inspected.status.state === 'conflict') {
      throw new Error(`Refusing to remove non-Manta command at ${commandPath}.`)
    }
  ...
  protected async ensureLinuxAppImagePayload(): Promise<AppImageExtractedRoot | null> {
    ...
    if (!extractedRoot) {
      throw new Error(
        `Could not extract the Manta AppImage at ${this.appImagePath}. Check that it is executable and that ${this.appImageCacheRootPath} has free space.`
      )
    }
  ...
  > {
    ...
    throw new Error(`The command at ${commandPath} changed while Manta inspected it.`)
  }
  ...
  private async installSymlinkWithPrivileges(
    ...
    if (installed.status.state !== 'installed') {
      throw new Error(`Could not register the Manta command at ${commandPath}.`)
    }
  }
