...
import { installLinuxBareMantaDispatcher } from './linux-bare-manta-dispatcher'
...
async function makeFixture(): Promise<{ homePath: string; resourcesPath: string }> {
  const root = await mkdtemp(join(tmpdir(), 'manta-bare-dispatcher-'))
  created.push(root)
  const resourcesPath = join(root, 'resources')
  // The bundled manta-ide launcher must exist for the dispatcher to be written.
  await mkdir(join(resourcesPath, 'bin'), { recursive: true })
  await writeFile(join(resourcesPath, 'bin', 'manta-ide'), '#!/usr/bin/env bash\n', 'utf8')
...
describe('installLinuxBareMantaDispatcher', () => {
  ...
  it('writes an executable bare-manta dispatcher that execs the bundled manta-ide launcher', async () => {
    ...
    const result = await installLinuxBareMantaDispatcher({
      resourcesPath,
      homePath,
      appImagePath: null
    })

    const expectedTarget = join(resourcesPath, 'bin', 'manta-ide')
    expect(result.state).toBe('installed')
    expect(result.target).toBe(expectedTarget)
    expect(result.dispatcherPath).toBe(join(homePath, '.local', 'bin', 'manta'))
  ...
  it('is idempotent — a second install rewrites its own dispatcher without throwing', async () => {
    ...
    const first = await installLinuxBareMantaDispatcher({
      resourcesPath,
      homePath,
      appImagePath: null
    })
    const second = await installLinuxBareMantaDispatcher({
      resourcesPath,
      homePath,
      appImagePath: null
    })
  ...
  it('publishes when the home filesystem does not support hard links', async () => {
    ...
    await expect(readFile(result.dispatcherPath, 'utf8')).resolves.toContain(
      '# manta-serve-bare-manta-dispatcher'
    )
  })

  it('restores a displaced foreign dispatcher when hard links are unsupported', async () => {
    const { homePath, resourcesPath } = await makeFixture()
    await installLinuxBareMantaDispatcher({ resourcesPath, homePath, appImagePath: null })
  ...
  it('quotes a resources path containing spaces so the exec line cannot be split', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-bare-dispatcher-space-'))
    created.push(root)
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const { homePath, resourcesPath } = await makeFixture()
      const appImagePath = join(homePath, 'Manta.AppImage')
      ...
      const result = await installLinuxBareMantaDispatcher({
        ...
        appImageExtractRunner: async (_appImagePath, cwd) => {
          ...
          await writeFile(join(launcherDir, 'manta-ide'), '', { encoding: 'utf8', mode: 0o755 })
        }
      })
  ...
  it('skips (does not clobber) a user-owned manta already at ~/.local/bin', async () => {
    const { homePath, resourcesPath } = await makeFixture()
    const dispatcherPath = join(homePath, '.local', 'bin', 'manta')
    const appImagePath = join(homePath, 'Manta.AppImage')
  ...
  it('preserves a foreign dispatcher created while AppImage extraction is in flight', async () => {
    ...
    const installation = installLinuxBareMantaDispatcher({
      resourcesPath,
      homePath,
      appImagePath,
      appImageCacheRootPath: cacheRootPath,
  ...
  it('prunes old owner generations without touching a sibling namespace', async () => {
    ...
    await installLinuxBareMantaDispatcher(options)
    ...
    const installation = installLinuxBareMantaDispatcher(options)
  ...
  it('skips when the bundled manta-ide launcher is missing from the build', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-bare-dispatcher-nolauncher-'))
    created.push(root)
