...
const SNAPSHOT_FILE = 'manta-workspace-space-analysis.json'
...
describe('workspace space analysis snapshot', () => {
  beforeEach(async () => {
    ...
    userDataDirHolder.dir = await mkdtemp(join(tmpdir(), 'manta-space-snapshot-'))
  })
  ...
  it('keeps profile snapshots isolated', async () => {
    const otherProfile = await mkdtemp(join(tmpdir(), 'manta-space-snapshot-other-'))
    try {
      await persistWorkspaceSpaceAnalysisSnapshot(
        userDataDirHolder.dir,
