...
const PROMPT_MARK = 'MANTAHIST> '
const WORKTREE_HASH = 'deadbeefdeadbeef'
const MARKER = 'echo manta-worktree-scoped-history'
...
describe('fish keeps per-worktree history under the session Manta names', () => {
  ...
  itWithFish(
    ...
    async () => {
      ...
      home = mkdtempSync(path.join(tmpdir(), 'manta-fish-history-'))
      ...
      term.write(`${MARKER}\r`)
      expect(await waitUntil(() => rendered.includes('manta-worktree-scoped-history'), 5_000)).toBe(
        true
      )
