 ...
 * that the rest of Manta's startup features arrive with it.
 *
 * macOS `/etc/zshrc` assigns `HISTFILE=${ZDOTDIR:-$HOME}/.zsh_history` with no
 * check-before-set. Manta used to fight that by keeping its own ZDOTDIR in place
 ...
 * Manta would not wrap cannot pass here by construction.
 */
...
const systemZshrcClobbersHistfile = (() => {
  ...
  const home = mkdtempSync(join(tmpdir(), 'manta-histfile-probe-'))
  try {
...
/** The launch config Manta produces for a pane with exactly these features. */
function launchPane(
  home: string,
  ...
  const env: Record<string, string> = {
    HOME: home,
    ...(scopedHistfile ? { HISTFILE: scopedHistfile, MANTA_HISTFILE: scopedHistfile } : {})
  }
  ...
  return {
    features,
    launch,
    // Why MANTA_ORIG_ZDOTDIR overridden: the launch config resolves the user's
    // config dir from the real process env, and these runs must resolve against
    // the sandbox home instead.
    env: { PATH: '/usr/bin:/bin', ...env, ...launch.env, MANTA_ORIG_ZDOTDIR: home }
  }
}

const USER_FILES = {
  '.zshenv': 'export MANTA_TEST_USER_ZSHENV=1\n',
  '.zprofile': 'export MANTA_TEST_USER_ZPROFILE=1\n',
  '.zshrc': 'export MANTA_TEST_USER_ZSHRC=1\n'
}
...
describe.skipIf(process.platform === 'win32')(
  ...
  () => {
    itWithZsh(
      'wraps a plain pane once Manta injected a worktree HISTFILE',
      withHome(USER_FILES, async (home) => {
        ...
        expect(launch.env.MANTA_SHELL_FEATURES).toBe('history')
      })
    )
    ...
    itWithClobber(
      ...
      withHome(USER_FILES, async (home) => {
        const scoped = join(home, 'manta-history', 'zsh_history')
    ...
    itWithZsh(
      ...
      withHome(USER_FILES, async (home) => {
        const { env, launch } = launchPane(home, join(home, 'manta-history', 'zsh_history'))

        const { values } = await runZshPty({
          env,
          report: [
            'ZDOTDIR',
            'MANTA_TEST_USER_ZSHENV',
            'MANTA_TEST_USER_ZPROFILE',
            'MANTA_TEST_USER_ZSHRC'
          ]
        })

        // Each user file is read from the user's own dir, exactly as unwrapped.
        expect(values.MANTA_TEST_USER_ZSHENV).toBe('1')
        expect(values.MANTA_TEST_USER_ZPROFILE).toBe('1')
        expect(values.MANTA_TEST_USER_ZSHRC).toBe('1')
    ...
    itWithZsh(
      'never leaves history inside Manta’s own wrapper directory',
      withHome(USER_FILES, async (home) => {
        const { env, launch } = launchPane(home, join(home, 'manta-history', 'zsh_history'))
    ...
    itWithZsh(
      'consumes MANTA_HISTFILE so nothing the shell spawns can inherit it',
      withHome(USER_FILES, async (home) => {
        const scoped = join(home, 'manta-history', 'zsh_history')
        const { env } = launchPane(home, scoped)
        ...
        const { values } = await runZshPty({
          env,
          report: ['MANTA_HISTFILE', 'MANTA_SHELL_FEATURES', 'HISTFILE']
        })

        // Root-cause fix for #11146: the variables no longer exist after use.
        expect(values.MANTA_HISTFILE).toBe('UNSET')
    ...
    itWithZsh(
      'leaves HISTFILE exactly as an unwrapped zsh would when Manta injects nothing',
      withHome(USER_FILES, async (home) => {
        ...
        // The contract is that Manta's wrapper does not change it either way.
        const overlayEnv = { MANTA_CODEX_HOME: join(home, 'codex') }
        const features = selectShellStartupFeatures({
          shellPath: ZSH_PATH,
...
describe.skipIf(process.platform === 'win32')('the deferred hook delivers every feature', () => {
  ...
  itWithZsh(
    'restores Manta’s overlay values after the user’s config overwrites them',
    withHome(
      {
        ...USER_FILES,
      ...
      async (home) => {
        const overlayEnv = {
          MANTA_CODEX_HOME: '/manta/codex',
          MANTA_AGENT_TEAMS_SHIM_DIR: '/manta/shim'
        }
        const features = selectShellStartupFeatures({
          shellPath: ZSH_PATH,
          env: { HOME: home, ...overlayEnv },
        ...
        // the spawn env did, and Manta's values still win.
        expect(values.CODEX_HOME).toBe('/manta/codex')
        expect(values.PATH.startsWith('/manta/shim:')).toBe(true)
      }
    )
  )
})
...
describe.skipIf(process.platform === 'win32')(
  ...
  () => {
     ...
     * `.zshenv` or `.zprofile` cost the pane all of Manta's features. Only one
    ...
    it.each([
      ...
      try {
        ...
        const { values } = await runZshPty({ env, report: ['HISTFILE', 'MANTA_HISTFILE'] })
    ...
    itWithZsh(
      ...
      withHome({ ...USER_FILES, '.zshrc': 'precmd_functions=()\n' }, async (home) => {
        ...
        const report = ['HISTFILE', 'MANTA_HISTFILE', 'ZDOTDIR']
        ...
        // host's call, not Manta's. macOS /etc/zshrc overwrites HISTFILE, so it
        ...
        expect(values.HISTFILE).not.toContain(launch.env.ZDOTDIR)
        // MANTA_HISTFILE was consumed in .zshenv precisely so a dropped hook
    ...
    itWithZsh(
      ...
      withHome(
        ...
        async (home) => {
          const { env } = launchPane(home, join(home, 'manta-history', 'zsh_history'))

          const { values } = await runZshPty({
            env,
            report: ['ZDOTDIR', 'MANTA_TEST_USER_ZSHRC']
          })
...
describe.skipIf(process.platform === 'win32')('the relay variant of the hook', () => {
  itWithZsh(
    ...
    withHome(USER_FILES, async (home) => {
      const relayRoot = mkdtempSync(join(tmpdir(), 'manta-relay-wrapper-'))
      try {
        ...
        const { output, values } = await runZshPty({
          env: {
            ...
            MANTA_ORIG_ZDOTDIR: home,
            MANTA_SHELL_FEATURES: 'overlay,history,ready',
            MANTA_HISTFILE: scoped,
            MANTA_REMOTE_CLI_BIN_DIR: '/manta/remote-bin'
          },
          commands: ['true'],
          report: ['HISTFILE', 'ZDOTDIR', 'PATH', 'MANTA_HISTFILE']
        })

        expect(values.HISTFILE).toBe(scoped)
        expect(values.ZDOTDIR).toBe(home)
        expect(values.MANTA_HISTFILE).toBe('UNSET')
        expect(values.PATH.startsWith('/manta/remote-bin:')).toBe(true)
        expect(output).toContain(MARKERS.ready)
