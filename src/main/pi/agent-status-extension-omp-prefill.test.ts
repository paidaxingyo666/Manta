...
describe('OMP prefill through the explicitly loaded status extension', () => {
  ...
  it('preserves the draft until an editor is available', async () => {
    const h = createAgentStatusExtensionHarness({ kind: 'omp', env: { MANTA_OMP_PREFILL: 'Draft' } })
    await h.callHook('session_start', {}, {})
    expect(h.processEnv.MANTA_OMP_PREFILL).toBe('Draft')
  ...
  it('never consumes a Pi draft in an OMP process', async () => {
    const h = createAgentStatusExtensionHarness({
      kind: 'omp',
      env: { MANTA_PI_PREFILL: 'Pi only' }
    })
    const setEditorText = vi.fn()
    await h.callHook('session_start', {}, { ui: { setEditorText } })
    expect(setEditorText).not.toHaveBeenCalled()
    expect(h.processEnv.MANTA_PI_PREFILL).toBe('Pi only')
  })

  it('leaves the draft alone outside a Manta pane', async () => {
    const h = createAgentStatusExtensionHarness({
      kind: 'omp',
      env: { MANTA_PANE_KEY: undefined, MANTA_OMP_PREFILL: 'Draft' }
    })
    const setEditorText = vi.fn()
    await h.callHook('session_start', {}, { ui: { setEditorText } })
    expect(setEditorText).not.toHaveBeenCalled()
  ...
  it('does not let a nested process consume the owner draft', () => {
    const h = createAgentStatusExtensionHarness({
      kind: 'omp',
      env: { MANTA_PI_STATUS_OWNED: '999', MANTA_OMP_PREFILL: 'Owner draft' }
    })
    expect(h.handlers.session_start).toBeUndefined()
    expect(h.processEnv.MANTA_OMP_PREFILL).toBe('Owner draft')
  })
})
