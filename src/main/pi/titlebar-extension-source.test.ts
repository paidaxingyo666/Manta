...
const PROMPT_TITLE = `π ! ${SESSION} - manta-app`
...
function createHarness(
  ...
  const context = {
    ...
    process: {
      env: { MANTA_PANE_KEY: options.paneKey ?? 'pane-1', ...options.env },
...
describe('getPiTitlebarExtensionSource', () => {
  ...
  it('re-asserts the marker when pi repaints the title under a dialog', async () => {
    ...
    harness.titles.push('π - other - manta-app')
    await vi.advanceTimersByTimeAsync(80)
    expect(harness.lastTitle()).toBe(PROMPT_TITLE)
  })
  ...
  it('leaves the needs-input marker to the process that owns the pane', async () => {
    // Why: child agents inherit MANTA_PANE_KEY, and a second process asserting the marker
