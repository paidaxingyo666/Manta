export function getPiAgentStatusWslCurlSourceLines(): string[] {
  return [
    ...
    '  const windowsPort = process.env.MANTA_AGENT_HOOK_PORT',
    '  const windowsToken = process.env.MANTA_AGENT_HOOK_TOKEN',
    "  if (!curlPath || !windowsPort || !windowsToken) throw new Error('Manta WSL hook bridge unavailable')",
    ...
    "        '-H', `X-Manta-Agent-Hook-Token: ${windowsToken}`,",
    "        '--data-binary', '@-',",
    '        windowsUrl',
    '      ],',
    ...
    "      reject(new Error('Manta WSL hook delivery timed out'))",
    '    }, 11000)',
    ...
    '      clearTimeout(timer)',
    "      if (code === 0) resolve(); else reject(new Error('Manta WSL hook exit ' + code))",
    '    })',
