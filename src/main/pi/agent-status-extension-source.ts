 ...
 *  dir: it POSTs lifecycle, tool, and (under OMP) model events to Manta's hook
...
export function getPiAgentStatusExtensionSource(kind: PiAgentKind = 'pi'): string {
  ...
  const modelMetadataSourceLines = [
    ...
    "    if (provider && id) modelMetadata = { model: provider + '/' + id, ...(ompModelSwitchSupported ? { model_switch_command: 'manta-model' } : {}) }",
    '  } catch {',
  ...
  return [
    ...
    '      timeoutPromise,',
    '    ])',
    "    if (!response.ok) throw new Error('Manta hook HTTP ' + response.status)",
    '  } catch (error) {',
