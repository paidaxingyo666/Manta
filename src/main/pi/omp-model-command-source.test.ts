...
describe('OMP model command', () => {
  it('switches through OMP API and reports the actual selected model', async () => {
    ...
    await harness.commands['manta-model'].handler('openai/test-model', commandContext())
    ...
    expect(JSON.parse(String(harness.fetchMock.mock.calls[0][1]?.body)).payload).toEqual({
      ...
      model_switch_command: 'manta-model'
    })
  })
  ...
  it('does not claim a switch for an unavailable model or rejected API call', async () => {
    ...
    await harness.commands['manta-model'].handler('missing/model', context)
    ...
    await harness.commands['manta-model'].handler('openai/test-model', context)
  ...
  it('does not register for Pi or an inherited child owner', () => {
    ...
    expect(
      createAgentStatusExtensionHarness({ kind: 'omp', env: { MANTA_PI_STATUS_OWNED: '123' } })
        .commands
    ).toEqual({})
  })
})
