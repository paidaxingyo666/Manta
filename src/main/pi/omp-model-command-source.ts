export function getOmpModelCommandSourceLines(): string[] {
  return [
    ...
    "    pi.registerCommand('manta-model', {",
    "      description: 'Switch the model selected in Manta',",
    '      handler: async (selector, ctx) => {',
    '        const models = ctx.modelRegistry.getAvailable()',
    "        const model = models.find((candidate) => candidate.provider + '/' + candidate.id === selector.trim())",
    '        if (!model) {',
    "          ctx.ui.notify('Model is no longer available. Refresh the Manta model picker.', 'error')",
    '          return',
    '        }',
