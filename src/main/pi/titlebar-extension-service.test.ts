...
const userDataDir = mkdtempSync(join(tmpdir(), 'manta-pi-test-userdata-'))
...
describe('PiTitlebarExtensionService', () => {
  ...
  beforeEach(() => {
    piHome = mkdtempSync(join(tmpdir(), 'manta-pi-test-pihome-'))
  ...
  it.each([
    ['pi', '.pi', 'MANTA_PI_SOURCE_AGENT_DIR'],
    ['omp', '.omp', 'MANTA_OMP_SOURCE_AGENT_DIR'],
    ['prime-agent', '.prime', 'MANTA_PRIME_AGENT_SOURCE_AGENT_DIR']
  ...
  it('materializes OMP extensions under the launch config root without overriding data routing', () => {
    ...
    try {
      ...
      expect(env.MANTA_OMP_SOURCE_AGENT_DIR).toBe(agentDir)
      expect(existsSync(join(agentDir, 'extensions', 'manta-agent-status.ts'))).toBe(true)
      expect(env.PI_CODING_AGENT_DIR).toBeUndefined()
  ...
  it('does not use the daemon ambient PI_CONFIG_DIR when launch root is omitted', () => {
    ...
    try {
      ...
      expect(env.MANTA_OMP_SOURCE_AGENT_DIR).toBe(join(fakeHome, '.omp', 'agent'))
  ...
  it('buildPtyEnv installs Manta extensions into the user agent dir without redirecting the home', () => {
    const svc = new PiTitlebarExtensionService()
    const env = svc.buildPtyEnv('pty-1', piHome, 'pi')

    expect(env.PI_CODING_AGENT_DIR).toBeUndefined()
    expect(env.MANTA_PI_SOURCE_AGENT_DIR).toBe(piHome)
    const extensions = readdirSync(join(piHome, 'extensions')).sort()
    expect(extensions).toEqual([
      'manta-agent-status.ts',
      'manta-prefill.ts',
      'manta-titlebar-spinner.ts',
      'user-ext'
    ])
    const statusExtensionSource = readFileSync(
      join(piHome, 'extensions', 'manta-agent-status.ts'),
      'utf-8'
    )
    const titlebarExtensionSource = readFileSync(
      join(piHome, 'extensions', 'manta-titlebar-spinner.ts'),
      'utf-8'
    )
    const prefillExtensionSource = readFileSync(
      join(piHome, 'extensions', 'manta-prefill.ts'),
      'utf-8'
    )
    expect(statusExtensionSource).toContain('@manta-managed-pi-extension')
    ...
    expect(titlebarExtensionSource).toContain('@manta-managed-pi-extension')
    expect(titlebarExtensionSource).toContain('process.env.MANTA_PANE_KEY')
    expect(prefillExtensionSource).toContain('@manta-managed-pi-extension')
    expect(prefillExtensionSource).toContain('process.env.MANTA_PANE_KEY')
    expectPiHomeIntact()
  })
  ...
  it('clearPty leaves the real Pi dir and managed extensions intact', () => {
    ...
    expect(existsSync(join(piHome, 'extensions', 'manta-agent-status.ts'))).toBe(true)
    expectPiHomeIntact()
  })
  ...
  it('installs only Prime status into the selected Prime agent dir', () => {
    ...
    expect(env).toEqual({ MANTA_PRIME_AGENT_SOURCE_AGENT_DIR: piHome })
    expect(readdirSync(join(piHome, 'extensions')).sort()).toEqual([
      'manta-agent-status.ts',
      'user-ext'
    ])
    const source = readFileSync(join(piHome, 'extensions', 'manta-agent-status.ts'), 'utf-8')
    expect(source).toContain('/hook/prime-agent')
    expect(source).not.toContain("return '/hook/omp'")
    expect(existsSync(join(piHome, 'extensions', 'manta-titlebar-spinner.ts'))).toBe(false)
    expect(existsSync(join(piHome, 'extensions', 'manta-prefill.ts'))).toBe(false)
    expectPiHomeIntact()
  })
  ...
  it('uses the same source dir for multiple PTYs with the same Pi dir', () => {
    ...
    expect(secondEnv.MANTA_PI_SOURCE_AGENT_DIR).toBe(firstEnv.MANTA_PI_SOURCE_AGENT_DIR)
    expect(readFileSync(join(piHome, 'extensions', 'user-ext', 'ext.ts'), 'utf-8')).toBe(
      'user extension'
    )
    expectPiHomeIntact()
  ...
  it('leaves OMP SQLite files in the real home instead of redirecting to an overlay', () => {
    ...
    expect(env.MANTA_OMP_SOURCE_AGENT_DIR).toBe(piHome)
    expect(env.MANTA_OMP_STATUS_EXTENSION).toBe(join(piHome, 'extensions', 'manta-agent-status.ts'))
  ...
  it('migrates missing OMP state from the old source overlay without overwriting source files', () => {
    ...
    writeFileSync(
      join(overlayDir, 'extensions', 'legacy-package', 'manta-prefill.ts'),
      'user package file'
    )
    ...
    expect(
      readFileSync(join(piHome, 'extensions', 'legacy-package', 'manta-prefill.ts'), 'utf-8')
    ).toBe('user package file')
    expect(readFileSync(join(piHome, 'extensions', 'manta-agent-status.ts'), 'utf-8')).toContain(
      '/hook/omp'
    )
    expect(readFileSync(join(overlayDir, '.manta-omp-overlay-migration-complete'), 'utf-8')).toBe(
      'complete\n'
    )
  })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      try {
        ...
        expect(existsSync(join(piHome, 'agent.db-wal'))).toBe(false)
        expect(existsSync(join(overlayDir, '.manta-omp-overlay-migration-complete'))).toBe(false)
        ...
        expect(readFileSync(join(piHome, 'agent.db-wal'), 'utf-8')).toBe('legacy sqlite wal')
        expect(
          readFileSync(join(overlayDir, '.manta-omp-overlay-migration-complete'), 'utf-8')
        ).toBe('complete\n')
      } finally {
        chmodSync(walPath, 0o600)
      }
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      mkdirSync(join(overlayDir, 'sessions'), { recursive: true })
      writeFileSync(join(overlayDir, 'sessions', 'legacy-session.jsonl'), 'legacy transcript')
      const outsideDir = mkdtempSync(join(tmpdir(), 'manta-omp-target-junction-'))
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      writeFileSync(join(overlayDir, 'agent.db'), 'legacy sqlite credentials')
      writeFileSync(join(overlayDir, 'sessions', 'legacy-session.jsonl'), 'legacy transcript')
      const outsideDir = mkdtempSync(join(tmpdir(), 'manta-omp-dangling-target-'))
  ...
  it('refreshes a managed spinner in an explicitly selected senpi home', () => {
    ...
    const extensionPath = join(agentDir, 'extensions', 'manta-titlebar-spinner.ts')
    mkdirSync(join(agentDir, 'extensions'), { recursive: true })
    writeFileSync(extensionPath, '// @manta-managed-pi-extension\nstale spinner')
    const svc = new PiTitlebarExtensionService()
    svc.buildPtyEnv('pty-senpi', agentDir, 'pi')
    expect(readFileSync(extensionPath, 'utf8')).toContain(getPiTitlebarExtensionSource())
  })

  it('rebuilding updates Manta-owned extensions while preserving user files', () => {
    const svc = new PiTitlebarExtensionService()
    svc.buildPtyEnv('pty-refresh-1', piHome, 'pi')
    writeFileSync(
      join(piHome, 'extensions', 'manta-agent-status.ts'),
      '// @manta-managed-pi-extension\nstale'
    )
  ...
  it("does not overwrite a user's same-named Manta extension file", () => {
    const userStatusExtension = 'user-owned status extension'
    writeFileSync(join(piHome, 'extensions', 'manta-agent-status.ts'), userStatusExtension, 'utf-8')

    const svc = new PiTitlebarExtensionService()
    const env = svc.buildPtyEnv('pty-same-name-extension', piHome, 'pi')

    expect(env.PI_CODING_AGENT_DIR).toBeUndefined()
    expect(readFileSync(join(piHome, 'extensions', 'manta-agent-status.ts'), 'utf-8')).toBe(
      userStatusExtension
    )
    expectPiHomeIntact()
  })

  it('uses a Manta-owned OMP status extension when a same-named user file exists', () => {
    const userStatusExtension = 'user-owned status extension'
    const userStatusPath = join(piHome, 'extensions', 'manta-agent-status.ts')
    ...
    expect(env.MANTA_OMP_STATUS_EXTENSION).toBe(fallbackStatusPath)
    expect(readFileSync(fallbackStatusPath, 'utf-8')).toContain('@manta-managed-pi-extension')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const realExtensionsDir = mkdtempSync(join(tmpdir(), 'manta-real-pi-extensions-'))
      try {
        ...
        expect(env.PI_CODING_AGENT_DIR).toBeUndefined()
        expect(existsSync(join(realExtensionsDir, 'manta-agent-status.ts'))).toBe(true)
        expect(existsSync(join(realExtensionsDir, 'manta-prefill.ts'))).toBe(true)
        expect(existsSync(join(realExtensionsDir, 'manta-titlebar-spinner.ts'))).toBe(true)
        expect(readFileSync(join(realExtensionsDir, 'manta-agent-status.ts'), 'utf-8')).toContain(
          '/hook/pi'
        )
      } finally {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      // Why: simulate an overlay that was left behind by a prior Manta session,
  ...
  // Why: per-agent source dir. Manta's user picks Pi or OMP per
  ...
  describe('per-agent default source dir (no cross-agent fallback)', () => {
    ...
    it('launching pi with both ~/.pi/agent and ~/.omp/agent present installs into ~/.pi/agent', () => {
      const fakeHome = mkdtempSync(join(tmpdir(), 'manta-pi-both-'))
      ...
      try {
        ...
        expect(env.PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.MANTA_PI_SOURCE_AGENT_DIR).toBe(join(fakeHome, '.pi', 'agent'))
        expect(
          existsSync(join(fakeHome, '.pi', 'agent', 'extensions', 'manta-agent-status.ts'))
        ).toBe(true)
        expect(
          existsSync(join(fakeHome, '.omp', 'agent', 'extensions', 'manta-agent-status.ts'))
        ).toBe(false)
      } finally {
        homedirOverride.current = ''
        rmSync(fakeHome, { recursive: true, force: true })
    ...
    it('launching omp with both ~/.pi/agent and ~/.omp/agent present installs into ~/.omp/agent', () => {
      const fakeHome = mkdtempSync(join(tmpdir(), 'manta-omp-both-'))
      seedAgentDir(fakeHome, '.pi', 'pi')
      seedAgentDir(fakeHome, '.omp', 'omp')
      ...
      try {
        ...
        expect(env.PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.MANTA_OMP_SOURCE_AGENT_DIR).toBe(join(fakeHome, '.omp', 'agent'))
        expect(env.MANTA_OMP_STATUS_EXTENSION).toBe(
          join(fakeHome, '.omp', 'agent', 'extensions', 'manta-agent-status.ts')
        )
        expect(
          readFileSync(
            join(fakeHome, '.omp', 'agent', 'extensions', 'manta-agent-status.ts'),
    ...
    it('launching omp when only ~/.pi/agent exists does NOT mirror Pi state', () => {
      ...
      // overlay from empty (Manta extensions only) — never cross-pollinate
      // from the other agent's dir.
      const fakeHome = mkdtempSync(join(tmpdir(), 'manta-omp-only-pi-'))
      seedAgentDir(fakeHome, '.pi', 'pi')
      expect(existsSync(join(fakeHome, '.omp'))).toBe(false)
      ...
      try {
        ...
        expect(env.PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.MANTA_OMP_SOURCE_AGENT_DIR).toBe(ompAgentDir)
    ...
    it('bare-shell prep does not create missing ~/.pi or ~/.omp homes (#10196)', () => {
      const fakeHome = mkdtempSync(join(tmpdir(), 'manta-no-eager-agent-home-'))
      ...
      try {
        ...
        expect(existsSync(join(fakeHome, '.pi'))).toBe(false)
        expect(existsSync(join(fakeHome, '.omp'))).toBe(false)
        expect(ompEnv.MANTA_OMP_SOURCE_AGENT_DIR).toBeUndefined()
        expect(ompEnv.MANTA_OMP_STATUS_EXTENSION).toEqual(
          expect.stringContaining('omp-managed-status-extension')
        )
        expect(existsSync(ompEnv.MANTA_OMP_STATUS_EXTENSION!)).toBe(true)
      } finally {
        homedirOverride.current = ''
        rmSync(fakeHome, { recursive: true, force: true })
      }
