...
const BASE_ENV = {
  ...
  MANTA_AGENT_LAUNCH_TOKEN: 'launch-1',
  MANTA_TAB_ID: 'tab-1',
  MANTA_WORKTREE_ID: 'tree-1',
  MANTA_AGENT_HOOK_PORT: '4321',
  MANTA_AGENT_HOOK_TOKEN: 'token-1',
  MANTA_AGENT_HOOK_ENV: 'env-1',
  MANTA_AGENT_HOOK_VERSION: '1.2.3'
...
describe('getPiAgentStatusExtensionSource', () => {
  it('registers Prime hooks only in the event-emitting daemon worker', () => {
    ...
    const worker = createHarness({
      kind: 'prime-agent',
      env: { MANTA_PI_STATUS_OWNED: String(SELF_PID - 1) }
    })

    expect(frontend.handlers).toEqual({})
    expect(frontend.processEnv.MANTA_PI_STATUS_OWNED).toBeUndefined()
    expect(worker.handlers.agent_start).toBeTypeOf('function')
    expect(worker.processEnv.MANTA_PRIME_AGENT_STATUS_OWNED).toBe(String(SELF_PID))
  })
  ...
  it('reports agent_end for a top-level run (including non-interactive) and claims the pane by pid', async () => {
    ...
    expect(harness.processEnv.MANTA_PI_STATUS_OWNED).toBe(String(SELF_PID))
  })
  ...
  it('falls back to Windows curl from WSL when fetch fails', async () => {
    ...
    expect(args).toEqual([
      ...
      '-H',
      'X-Manta-Agent-Hook-Token: token-1',
      '--data-binary',
      '@-',
  ...
  it('uses current Windows coordinates when a same-token guest endpoint is stale', async () => {
    const endpointPath = '/home/u/.manta-wsl/agent-hooks/instance-test/endpoint.env'
    const harness = createHarness({
      kind: 'prime-agent',
      env: { WSL_DISTRO_NAME: 'Ubuntu', MANTA_AGENT_HOOK_ENDPOINT: endpointPath },
      existsSync: (path) => path === '/mnt/c/Windows/System32/curl.exe',
      statSync: () => ({ mtimeMs: 1, size: 80, ino: 1 }),
      readFileSync: (path) => {
        if (path === endpointPath) {
          return 'MANTA_AGENT_HOOK_PORT=9999\nMANTA_AGENT_HOOK_TOKEN=token-1\n'
        }
        throw Object.assign(new Error(`ENOENT: ${path}`), { code: 'ENOENT' })
      },
      fetchImpl: vi.fn(async () => {
  ...
  it('does not hold Pi event dispatch open while hook delivery is pending', async () => {
    ...
    // remain on the agent's critical path when Manta is stalled or restarting.
    expect(harness.fetchMock).toHaveBeenCalledTimes(1)
