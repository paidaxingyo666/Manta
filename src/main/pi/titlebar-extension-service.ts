...
import {
  MANTA_PI_AGENT_STATUS_EXTENSION_FILE,
  getPiAgentStatusExtensionSource
} from './agent-status-extension-source'
import {
  MANTA_PI_PREFILL_EXTENSION_FILE,
  getPiPrefillExtensionSource
} from './prefill-extension-source'
import { MANTA_PI_EXTENSION_FILE, getPiTitlebarExtensionSource } from './titlebar-extension-source'
import {
...
// Why: old Manta versions used per-kind overlay roots. Keep the names so
...
// shadowed one in Manta's per-launch agent picker.
...
function withMantaManagedExtensionMarker(source: string): string {
...
export class PiTitlebarExtensionService {
  ...
  private getPtyOverlayDir(ptyId: string, kind: LegacyOverlayAgentKind): string {
    // Why: old Manta versions used PTY-scoped hashed overlays. Keep resolving
  ...
  private writeOmpFallbackStatusExtension(source: string): string | undefined {
    ...
    const fallbackPath = join(fallbackDir, MANTA_PI_AGENT_STATUS_EXTENSION_FILE)
  ...
  private installManagedExtensions(
    ...
    if (kind !== 'prime-agent') {
      ...
      this.writeManagedExtension(
        ...
        withMantaManagedExtensionMarker(getPiPrefillExtensionSource(kind))
      )
    }
    const statusExtensionPath = join(extensionsDir, MANTA_PI_AGENT_STATUS_EXTENSION_FILE)
    const statusSource = withMantaManagedExtensionMarker(getPiAgentStatusExtensionSource(kind))
  ...
  buildPtyEnv(
    ...
    if (!existsSync(sourceAgentDir) && !materializeDefaultHome) {
      if (kind === 'omp') {
        ...
        return statusExtensionPath ? { MANTA_OMP_STATUS_EXTENSION: statusExtensionPath } : {}
      }
      return {}
    }
    ...
    if (kind === 'omp') {
      env.MANTA_OMP_SOURCE_AGENT_DIR = installed.sourceAgentDir
      if (installed.statusExtensionPath) {
        env.MANTA_OMP_STATUS_EXTENSION = installed.statusExtensionPath
      }
    } else if (kind === 'prime-agent') {
      env.MANTA_PRIME_AGENT_SOURCE_AGENT_DIR = installed.sourceAgentDir
    } else {
      env.MANTA_PI_SOURCE_AGENT_DIR = installed.sourceAgentDir
    }
    return env
  }
