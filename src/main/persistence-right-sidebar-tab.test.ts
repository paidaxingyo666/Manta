...
vi.mock('electron', () => ({
  app: {
    getPath: () => '/tmp/manta-persistence-right-sidebar-tab-test'
  },
  safeStorage: {
...
describe('normalizeRightSidebarTab', () => {
  ...
  it('preserves well-formed plugin panel tabs', () => {
    expect(normalizeRightSidebarTab('plugin:manta-samples.my-plugin/dashboard')).toBe(
      'plugin:manta-samples.my-plugin/dashboard'
    )
  })

  it('normalizes malformed plugin tabs to the default tab', () => {
    expect(normalizeRightSidebarTab('plugin:manta-samples.my-plugin')).toBe('explorer')
    expect(normalizeRightSidebarTab('plugin:manta-samples.my-plugin/panel/extra')).toBe('explorer')
