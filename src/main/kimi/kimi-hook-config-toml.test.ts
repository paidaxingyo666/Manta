...
const COMMAND =
  "if [ -x '/home/u/.manta/agent-hooks/kimi-hook.sh' ]; then /bin/sh '/home/u/.manta/agent-hooks/kimi-hook.sh'; fi"
const isManaged = (command: string | undefined): boolean =>
  typeof command === 'string' && command.includes('agent-hooks/kimi-hook.sh')

const END_MARKER_LINE = '# <<< manta-managed-kimi-hooks <<<'
const START_MARKER = '# >>> manta-managed-kimi-hooks (managed by Manta; do not edit) >>>'

/** Drops only the `# <<< ... <<<` line, the hand-edit that orphans the block. */
function deleteEndMarker(text: string): string {
  return text.replace(/\r?\n# <<< manta-managed-kimi-hooks <<<(?=\r?\n|$)/, '')
}
...
describe('kimi managed hooks TOML block', () => {
  ...
  it('is idempotent — reinstalling does not duplicate the block', () => {
    ...
    const markerCount = (twice.match(/manta-managed-kimi-hooks \(/g) ?? []).length
  ...
  it('recovers when a hand-edit deletes only the trailing end marker', () => {
    ...
    expect((reinstalled.match(/manta-managed-kimi-hooks \(/g) ?? []).length).toBe(1)
  })
...
describe('orphaned managed block ownership (#18861)', () => {
  ...
  it('does not treat a user [[hooks]] table as Manta-owned content', () => {
    const orphan = [
      START_MARKER,
      '[[hooks]]',
      'event = "Stop"',
  ...
  // A user adding keys has customised Manta's hook, not authored their own: the
  // command path is what makes it fire. Leaving it would keep sending Manta their
  ...
  // pullfrog on #20148: ownership keys on `command`, so an `event` Manta cannot
