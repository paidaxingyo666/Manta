...
// homedir()/.manta, and getKimiHome() honors KIMI_CODE_HOME. Point both at a
// temp dir so the local install/remove cycle never touches the real ~/.manta or
...
beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), 'manta-kimi-hook-'))
...
const scriptPath = (): string => join(home, '.manta', 'agent-hooks', 'kimi-hook.sh')
