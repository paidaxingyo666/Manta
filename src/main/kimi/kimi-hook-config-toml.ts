...
// so Manta manages only its own marker-delimited block: install rewrites the
// block, remove strips it, and user config is left untouched apart from hook
// tables Manta itself emitted. Appending table headers is always valid TOML, so
...
// Why: mirror the Claude-compatible events Manta normalizes for status. Kimi uses
...
const MARKERS: ManagedTomlMarkers = {
  startMarker: '# >>> manta-managed-kimi-hooks (managed by Manta; do not edit) >>>',
  endMarker: '# <<< manta-managed-kimi-hooks <<<'
}
const HOOK_TABLE_HEADER = '[[hooks]]'

export type ManagedCommandMatcher = (command: string | undefined) => boolean

// A `[[hooks]]` table that invokes Manta's managed script is Manta's hook: that
// command path is the only reason it fires, and it is there because Manta put it
// there. Extra keys are a user customising our hook, not authoring their own, so
// uninstall still owns it — leaving it would keep feeding Manta their events
...
// Ownership keys on the command, so an event Manta cannot parse must still
...
// Manta owns two things here: whatever sits inside a matched marker pair, and
...
// Manta generates; escape both defensively anyway.
...
// would tell the user a hook is uninstalled while Manta keeps receiving events.
