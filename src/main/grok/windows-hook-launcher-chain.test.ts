...
function readInstalledConfig(home: string): InstalledConfig {
  return JSON.parse(
    readFileSync(join(home, '.grok', 'hooks', 'manta-status.json'), 'utf8')
  ) as InstalledConfig
}
...
describe('Windows Grok managed hook launch shape', () => {
  ...
  describe('registered command, from a synthetic Windows path', () => {
    const scriptPath = 'C:\\Users\\dev\\.manta\\agent-hooks\\grok-hook.cmd'
    ...
    it('still wraps a path cmd.exe would split or expand (#6078)', () => {
      const spaced = 'C:\\Users\\Jane Doe\\.manta\\agent-hooks\\grok-hook.cmd'
  ...
  describe('installed config', () => {
    ...
    it.skipIf(process.platform !== 'win32')(
      ...
      () => {
        const scriptPath = join(home, '.manta', 'agent-hooks', 'grok-hook.cmd')
    ...
    it.skipIf(process.platform !== 'win32')(
      ...
      () => {
        ...
        const configPath = join(home, '.grok', 'hooks', 'manta-status.json')
        ...
        // Why: sweeping stale Manta entries must not touch hooks the user wrote.
    ...
    it('falls back to the encoded launcher when the profile path is not cmd-safe (#6078)', () => {
      const spaceHome = mkdtempSync(join(tmpdir(), 'manta grok spaced '))
