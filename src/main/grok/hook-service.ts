...
import { isMantaOwnedRemnant, removeManagedGrokHookEntries } from './grok-hook-config-cleanup'
...
function getConfigPath(): string {
  ...
  // home Grok and transcript lookup use; keep Manta entries in a dedicated file
  // so user-authored hook files stay untouched.
  return join(resolveGrokHomeDir(), 'hooks', 'manta-status.json')
}
...
export class GrokHookService {
  ...
  install(options?: { userInitiated?: boolean }): AgentHookInstallStatus {
    ...
    // marker written by Manta's own prior cleanup.
  ...
  remove(): AgentHookInstallStatus {
    ...
    if (isMantaOwnedRemnant(cleanup.config) && !isSymbolicLinkSync(configPath)) {
  ...
  async removeAsync(): Promise<AgentHookInstallStatus> {
    ...
    const unlinkable = isMantaOwnedRemnant(cleanup.config) && !configIsSymlink
