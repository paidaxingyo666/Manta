...
function buildInstalledConfig(
  ...
  // Why: Manta owns only grok-hook.* entries. Sweep stale managed commands out
...
export class GrokHookService {
  ...
  async installRemote(
    ...
    const remoteConfigPath = `${getRemoteGrokHome(home, remoteGrokHome)}/hooks/manta-status.json`
    const remoteScriptPath = `${home}/.manta/agent-hooks/grok-hook.sh`
    try {
