...
describe('worktree create preparation registry', () => {
  ...
  it('namespaces native Windows preparation directories for long paths', async () => {
    ...
    try {
      ...
      expect(mocks.mkdir).toHaveBeenCalledWith(
        expect.stringMatching(/^\\\\\?\\C:\\workspace\\\.manta-preparing/),
        { recursive: true }
      )
    } finally {
  ...
  it('prepares while stale removal is stalled, shares its scan, and settles removal on reset', async () => {
    const stalePath = '/workspace/.manta-preparing/999999999-11111111-1111-4111-8111-111111111111'
    ...
    mocks.listWorktreeGraph.mockResolvedValueOnce([
      {
        path: stalePath,
        branch: undefined,
        lockReason: 'manta-create-preparation:v1:999999999:stale',
        head: 'deadbeef',
        isBare: false,
  ...
  it('does not classify a user branch worktree under the preparation directory as stale', async () => {
    mocks.listWorktreeGraph.mockResolvedValueOnce([
      {
        path: '/workspace/.manta-preparing/999999999-user-worktree',
    ...
    mocks.listWorktreeGraph.mockResolvedValueOnce([
      {
        ...
        branch: undefined,
        lockReason: 'manta-create-preparation:v1:999999999:spoofed',
        head: 'deadbeef',
        isBare: false,
        isMainWorktree: false
      }
