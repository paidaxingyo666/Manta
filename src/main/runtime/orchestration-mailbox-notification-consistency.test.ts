...
describe('orchestration notification mailbox consistency', () => {
  ...
  it('never points direct Run A mail after the pane is rebound to Run B', async () => {
    vi.useFakeTimers()
    const db = createDatabase('manta-mailbox-consistency-')
  ...
  it('does not point unbound direct mail that a later Run binding would hide', async () => {
    vi.useFakeTimers()
    const db = createDatabase('manta-mailbox-unbound-bind-race-')
  ...
  it('reconciles the persisted production mismatch after runtime restart', async () => {
    vi.useFakeTimers()
    const directory = mkdtempSync(join(tmpdir(), 'manta-mailbox-upgrade-restart-'))
  ...
  it('pages a large persisted mismatch and coalesces each mailbox wake', async () => {
    vi.useFakeTimers()
    const db = createDatabase('manta-mailbox-paged-reconciliation-')
    const harness = createRuntime(db)
  ...
  it('uses composite indexes for direct ownership reconciliation', () => {
    const db = createDatabase('manta-mailbox-routing-indexes-')
  ...
  it('finds owned mail without scanning an older unowned direct backlog', () => {
    const db = createDatabase('manta-mailbox-owned-routing-plan-')
  ...
  it('does not submit or replay a pointer after ownership changes during Enter delay', async () => {
    vi.useFakeTimers()
    const directory = mkdtempSync(join(tmpdir(), 'manta-mailbox-restart-'))
    temporaryDirectories.push(directory)
    const dbPath = join(directory, 'orchestration.db')
  ...
  it('does not replay a staged same-Run pointer when the runtime restarts before Enter', async () => {
    vi.useFakeTimers()
    const directory = mkdtempSync(join(tmpdir(), 'manta-mailbox-staged-restart-'))
    temporaryDirectories.push(directory)
    const dbPath = join(directory, 'orchestration.db')
    const firstDb = new OrchestrationDb(dbPath)
    const first = createRuntime(firstDb)
  ...
  it('fences the pointed mailbox instead of checking a rebound empty Run', async () => {
    vi.useFakeTimers()
    const db = createDatabase('manta-mailbox-post-submit-rebind-')
    const harness = createRuntime(db)
    const runA = createBoundRun(db, 'Run A')
  ...
  it('releases a staged pointer when its Run moves to another live pane', async () => {
    vi.useFakeTimers()
    const db = createDatabase('manta-mailbox-live-rebind-')
    const harness = createRuntime(db)
    const runA = createBoundRun(db, 'Run A')
    ...
    expect(
      harness.write.mock.calls.filter(
        ([ptyId, payload]) =>
          ptyId === SECOND_PTY_ID && String(payload).includes('manta orchestration check')
      )
    ).toHaveLength(1)
    expect(
  ...
  it('serializes equivalent panes while newer mail arrives during Enter delay', async () => {
    vi.useFakeTimers()
    const db = createDatabase('manta-mailbox-equivalent-pane-flight-')
    const harness = createRuntime(db)
    ...
    expect(
      harness.write.mock.calls.filter(
        ([ptyId, payload]) =>
          ptyId === PTY_ID && String(payload).includes('manta orchestration check')
      )
    ).toHaveLength(2)
    await vi.advanceTimersByTimeAsync(500)
  ...
  it.each([
    ...
    async (_provider, workingTitle, idleTitle) => {
      vi.useFakeTimers()
      const db = createDatabase('manta-mailbox-working-before-enter-')
      const harness = createRuntime(db)
  ...
  it.each([
    ...
    async (_provider, permissionTitle) => {
      vi.useFakeTimers()
      const db = createDatabase('manta-mailbox-permission-before-enter-')
      const harness = createRuntime(db)
  ...
  it('releases staged pointer state when an explicit check owns the batch', async () => {
    vi.useFakeTimers()
    const db = createDatabase('manta-mailbox-explicit-check-')
    const harness = createRuntime(db)
  ...
  it('routes same-Run direct mail through the bound check and acknowledgment path', async () => {
    vi.useFakeTimers()
    const db = createDatabase('manta-mailbox-current-run-')
    const harness = createRuntime(db)
  ...
  it('does not replay a successfully submitted actionable pointer after restart', async () => {
    vi.useFakeTimers()
    const directory = mkdtempSync(join(tmpdir(), 'manta-mailbox-actionable-restart-'))
    temporaryDirectories.push(directory)
    const dbPath = join(directory, 'orchestration.db')
    const firstDb = new OrchestrationDb(dbPath)
    const first = createRuntime(firstDb)
  ...
  it('does not replay a visible pointer when the provider rejects Enter', async () => {
    vi.useFakeTimers()
    const db = createDatabase('manta-mailbox-provider-refusal-')
    const first = createRuntime(db)
  ...
  it('does not point newer Run mail behind an outstanding Delivery', async () => {
    vi.useFakeTimers()
    const db = createDatabase('manta-mailbox-outstanding-')
    const harness = createRuntime(db)
  ...
  it('drains persisted Run pages before installing a filtered waiter', async () => {
    const db = createDatabase('manta-mailbox-filtered-run-backlog-')
    const harness = createRuntime(db)
  ...
  it('wakes a filtered waiter when reconciliation moves its type on a later page', async () => {
    const db = createDatabase('manta-mailbox-filtered-reconciliation-wake-')
    const harness = createRuntime(db)
  ...
  it('drains persisted Dispatch pages before installing a filtered waiter', async () => {
    const db = createDatabase('manta-mailbox-filtered-dispatch-backlog-')
    const harness = createRuntime(db)
