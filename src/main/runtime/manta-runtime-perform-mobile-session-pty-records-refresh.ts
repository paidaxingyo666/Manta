...
import { MantaRuntimeWithBuildHeadlessMobileSessionBrowserTabs } from './manta-runtime-build-headless-mobile-session-browser-tabs'
...
export class MantaRuntimeWithPerformMobileSessionPtyRecordsRefresh extends MantaRuntimeWithBuildHeadlessMobileSessionBrowserTabs {
    ...
    if (tab.type === 'terminal') {
      ...
      if (shouldMaterializePendingTerminal) {
        ...
        let agentStartup: Awaited<
          ReturnType<MantaRuntimeService['resolveMobileSessionTerminalCommand']>
        > = {}
        if (tab.launchAgent) {
          try {
