...
// reachable from the LAN; it widens to all interfaces only on explicit pairing (or `manta serve`).
...
type MantaRuntimeRpcServerOptions = {
  ...
  wsPort?: number
  // Why: true when the caller pinned a port (`manta serve --port`) so bind order prefers it over a stale STA-1511 fallback (#8535).
  ...
  // Only `manta serve` (explicit remote opt-in) and E2E set this; the desktop app widens lazily on pairing.
   ...
   * later startup. An unattended host (mantad) whose operator asked for loopback must
...
const DEVICE_REGISTRY_UNAVAILABLE_GUIDANCE =
  'The pairing registry is unavailable. Verify that the Manta data directory is writable.'
const E2EE_KEY_UNAVAILABLE_GUIDANCE =
  'The E2EE identity is unavailable. Verify that the Manta data directory is writable.'
...
export class MantaRuntimeRpcServer {
  private readonly runtime: MantaRuntimeService
  ...
  private async createMobilePairingOfferSerial(
    ...
    const refuseAutomaticWithoutRelay = (
      ...
      return {
        ...
        guidance:
          'Manta Relay could not create a pairing invite. Use LAN (Tailscale or same Wi‑Fi) or retry Relay.',
        relayFailure
      }
    }
  ...
  async start(): Promise<void> {
    ...
    this.metadataOwnershipWatch = watchRuntimeMetadataOwnership({
      ...
      onReclaim: (previous) => {
        console.warn(
          `[runtime] Reclaimed manta-runtime.json from a dead runtime (pid ${previous?.pid ?? 'none'}); republished pid ${this.pid}.`
        )
      }
    })
  }
  ...
  // explicitly pairs; `manta serve`/E2E (exposeNetworkByDefault) and a reconnecting paired device bind wide.
...
export function createRuntimeTransportMetadata(
  ...
  if (platform === 'win32') {
    return {
      ...
      endpoint: `\\\\.\\pipe\\manta-${pid}-${endpointSuffix}`
    }
  }
  return {
    kind: 'unix',
