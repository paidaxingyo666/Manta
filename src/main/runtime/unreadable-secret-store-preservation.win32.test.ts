 ...
 * the data root came from somewhere else — a relocated `MANTA_USER_DATA_PATH`, a share, a roaming
...
describeOnWindows('a secure store that exists but cannot be read', () => {
   ...
   * session used to fall past the `status === 'found'` guard into `clearMantaCloudSession`.
   */
  it('does not delete the account session it could not read', async () => {
    ...
    const { readMantaCloudSession, getMantaCloudSessionPath } =
      await import('./../manta-profiles/profile-cloud-session-store')
    const dir = join(root, 'profiles')
    mkdirSync(dir, { recursive: true })
    const filePath = getMantaCloudSessionPath('profile-1', dir)
    mkdirSync(join(filePath, '..'), { recursive: true })
    ...
    expect(readMantaCloudSession('profile-1', dir).status).toBe('unreadable')
