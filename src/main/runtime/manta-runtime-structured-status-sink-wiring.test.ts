...
type MantaRuntimeDeps = NonNullable<ConstructorParameters<typeof MantaRuntimeService>[2]>
...
] as const satisfies readonly (keyof MantaRuntimeDeps)[]

const MAIN_ROOT = join(import.meta.dirname, '..')

/** The text of the `new MantaRuntimeService(...)` call in one entry point. */
function runtimeConstruction(relativePath: string): string {
  const source = readFileSync(join(MAIN_ROOT, relativePath), 'utf8')
  const start = source.indexOf('new MantaRuntimeService(')
  ...
  throw new Error(`unbalanced MantaRuntimeService construction in ${relativePath}`)
}
 ...
 *  constructs a runtime without these lists no agents at all — and `mantad` serves `worktree.ps`
 *  and `agentSession.*` exactly like the desktop does. */
describe('every host that constructs a runtime wires the agent-status store', () => {
  it.each([['mantad/mantad-entry.ts'], ['startup/main-process-runtime-service.ts']])(
...
describe('structured status sink wiring', () => {
  it('hands the host the sink the runtime was constructed with', async () => {
    ...
    const runtime = new MantaRuntimeService(null, undefined, { structuredAgentStatusSink: sink })
