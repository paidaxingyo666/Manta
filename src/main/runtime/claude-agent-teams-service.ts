...
export class ClaudeAgentTeamsService {
  ...
  createLaunchEnv(args: {
    ...
    const tmuxValue = `/tmp/manta-claude-agent-teams/${teamId},0,1`
    const env: Record<string, string> = {
      ...
      MANTA_AGENT_TEAMS_TEAM_ID: teamId,
      MANTA_AGENT_TEAMS_TOKEN: token,
      MANTA_AGENT_TEAMS_LEADER_PANE: leaderPane,
      MANTA_AGENT_TEAMS_SHIM_DIR: args.shimDir
    }
    if (args.shimBin) {
      env.MANTA_AGENT_TEAMS_SHIM_BIN = args.shimBin
    }
    if (args.baseEnv.MANTA_PAIRING_CODE) {
      env.MANTA_PAIRING_CODE = args.baseEnv.MANTA_PAIRING_CODE
    }
    if (args.baseEnv.MANTA_ENVIRONMENT) {
      env.MANTA_ENVIRONMENT = args.baseEnv.MANTA_ENVIRONMENT
    }
