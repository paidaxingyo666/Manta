...
// app.setName('Manta') changing how app.getPath('userData') resolves (e.g. from
// lowercase 'manta' to uppercase 'Manta' on a case-sensitive filesystem) — the
...
describe('mobile pairing userData path stability', () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'manta-pairing-path-'))
  ...
  it('keeps returning the path captured before app.setName changes resolution', async () => {
    ...
    // app.setName('Manta') happens later in startup, changing late resolution.
  ...
  it('writes DeviceRegistry + E2EE keypair under the canonical path, not the late one', async () => {
    ...
    appState.userData = lateDir // app.setName('Manta') has run by the time the runtime starts

    const { DeviceRegistry } = await import('./device-registry')
    const { loadOrCreateE2EEKeypair } = await import('./e2ee-keypair')

    // Mirrors MantaRuntimeRpcServer.start(): both read from the same userDataPath.
    ...
    // Pairing credentials land beside manta-data.json so they survive restarts/updates.
