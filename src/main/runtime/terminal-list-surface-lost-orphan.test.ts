...
function makeRuntime(session?: WorkspaceSessionState): MantaRuntimeService {
  ...
  const runtime = new MantaRuntimeService(makeStore(session) as never)
...
function dropOnePane(runtime: MantaRuntimeService): void {
  runtime.syncWindowGraph(1, {
...
describe('terminal inventory after a pane is dropped', () => {
  ...
  it('does not call a surface recorded since the last graph statement orphaned', async () => {
    ...
    // manta-runtime-record-pty-worktree.ts is `@ts-nocheck`, so a missing stamp is silent there
