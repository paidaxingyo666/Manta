...
function syncSshSplit(runtime: MantaRuntimeService, snapshot: RuntimeMobileSessionTabsSnapshot) {
...
describe('MantaRuntimeService terminal retirement host partitioning (STA-3463)', () => {
  it('retires an exited SSH pane from the SSH partition and leaves the local partition untouched', async () => {
    ...
    const runtime = new MantaRuntimeService(harness.store)
