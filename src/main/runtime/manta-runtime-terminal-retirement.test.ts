...
function syncSplit(runtime: MantaRuntimeService, snapshot = makeSplitSnapshot()): void {
...
describe('MantaRuntimeService terminal surface retirement', () => {
  ...
  it('rejects one stale shared-PTY surface without removing its live sibling', () => {
    ...
    const runtime = new MantaRuntimeService(runtimeStore({ getWorkspaceSession: () => session }))
    runtime.attachWindow(1)
