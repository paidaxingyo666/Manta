...
/** Antigravity's binary, as Manta launches and probes it (`tui-agent-config.ts` detectCmd). */
  ...
  for (const transcript of TRANSCRIPTS) {
    ...
    it.skipIf(!captured)(
      ...
      async () => {
        ...
        // A silent dialog carries no blocked-signal wording, so the assertion is only that Manta
