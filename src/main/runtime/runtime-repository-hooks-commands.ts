...
export class RuntimeRepositoryHooksCommands {
  ...
  async getRepoHooks(repoSelector: string) {
    ...
    if (repo.connectionId) {
      ...
      try {
        ...
        const hooks = result.isBinary ? null : parseMantaYaml(result.content)
        return {
          hasHooksFile: Boolean(hooks),
          hooks,
          setupRunPolicy: getEffectiveSetupRunPolicy(repo),
          source: hooks ? ('manta.yaml' as const) : null,
    ...
    return {
      ...
      hooks,
      setupRunPolicy: getEffectiveSetupRunPolicy(repo),
      source: hasFile ? ('manta.yaml' as const) : hooks ? ('legacy' as const) : null,
  ...
  async checkRepoHooks(repoSelector: string) {
    ...
    if (repo.connectionId) {
      ...
      try {
        ...
        return {
          status: 'ok' as const,
          hasHooks: true,
          hooks: parseMantaYaml(result.content),
          mayNeedUpdate: false
        }
      } catch (error) {
        return {
    ...
    return {
      ...
      hasHooks: has,
      hooks,
      mayNeedUpdate: has && !hooks && hasUnrecognizedMantaYamlKeys(repo.path)
    }
  }
