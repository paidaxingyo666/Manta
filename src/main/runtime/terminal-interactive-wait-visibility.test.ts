...
async function createPane(
  ...
  // Compose the same central hook-store wiring as desktop and mantad so OSC rows exercise the
...
// cursor-agent renders a braille spinner in its OSC title while it works, and Manta reads
...
describe('terminal interactive-wait visibility (STA-4513, STA-3714)', () => {
  ...
  describe('prompts the runtime already matched but never surfaced', () => {
    it('surfaces a startup trust screen on the pane, not only on terminal wait', async () => {
      // A pane on its trust screen still wears Manta's tab title; the agent has set none.
      const { runtime, handle } = await createPane({
