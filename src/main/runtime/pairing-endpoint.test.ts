...
describe('resolveAdvertisedPairingEndpoint', () => {
  ...
  it.each([
    ...
    ['http://proxy.example.test/manta', 'ws://proxy.example.test/manta'],
    ['https://proxy.example.test/manta', 'wss://proxy.example.test/manta'],
    [
      'wss://proxy.example.test:8443/manta?route=runtime',
      'wss://proxy.example.test:8443/manta?route=runtime'
    ]
...
describe('resolveAdvertisedPairingHostname', () => {
  ...
  it.each([
    ...
    ['desktop', 'desktop'],
    ['wss://proxy.example.test:8443/manta', 'proxy.example.test']
