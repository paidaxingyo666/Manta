...
import { InMemoryOrchestrationMessages } from './manta-runtime-test-orchestration-messages.spec'
...
function expectStablePaneKeyEnv(env: Record<string, string>): string {
  expect(env.MANTA_TAB_ID).toMatch(UUID_RE)
  const leafId = env.MANTA_PANE_KEY?.slice(`${env.MANTA_TAB_ID}:`.length)
  expect(leafId).toMatch(UUID_RE)
  expect(env.MANTA_PANE_KEY).toBe(`${env.MANTA_TAB_ID}:${leafId}`)
  return env.MANTA_PANE_KEY
}

function createRuntime(): RuntimeService {
  return new MantaRuntimeService(store)
}
