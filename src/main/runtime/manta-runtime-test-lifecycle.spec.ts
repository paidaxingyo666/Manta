...
const { mergeGitLabMRMock, muxRequestMock, parseMantaYaml, prepareLocalWorktreeRootForRepoMock } =
  mocks
...
function resetRuntimeTestMocks(): void {
  ...
  vi.mocked(parseMantaYaml).mockReset()
  ...
  vi.mocked(parseMantaYaml).mockReturnValue(null)
