...
import { createRuntimeTransportMetadata, MantaRuntimeRpcServer } from './runtime-rpc'
...
describe('MantaRuntimeRpcServer', () => {
  ...
  it('reclaims runtime metadata clobbered by a second instance that has since died', async () => {
    ...
    // manta-runtime.json with its own pid, so the CLI reports stale_bootstrap
  ...
  it('stops reclaiming runtime metadata after the server is stopped', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'manta-runtime-rpc-'))
    const server = new MantaRuntimeRpcServer({ runtime: new MantaRuntimeService(), userDataPath })
    await server.start()
  ...
  it('drops a republish from an ownership read that lands after the server stopped', async () => {
    ...
    const userDataPath = mkdtempSync(join(tmpdir(), 'manta-runtime-rpc-'))
    const runtime = new MantaRuntimeService()
    const server = new MantaRuntimeRpcServer({ runtime, userDataPath, pid: 1001 })
    await server.start()
  ...
  it('flushes a lastSeen refresh scheduled while transports stop', async () => {
    const server = new MantaRuntimeRpcServer({
      runtime: new MantaRuntimeService(),
      userDataPath: mkdtempSync(join(tmpdir(), 'manta-runtime-rpc-')),
