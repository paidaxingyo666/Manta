import './manta-runtime-test-lifecycle.spec'
import type { RuntimeStore } from './runtime-store-contract'
import { describe, expect, it, vi } from 'vitest'
import { createMobileCreateTestNotifier } from './manta-runtime-test-scenario-builders.spec'
...
describe('terminal rename before renderer graph hydration', () => {
  it.each(['Media Engine Orch', null])(
    ...
    async (title) => {
      ...
      const runtime = new MantaRuntimeService(checkedStore)
      ...
      const restored = new MantaRuntimeService(checkedStore)
  ...
  it('does not recreate a closed persisted tab from a surviving PTY record', async () => {
    ...
    const runtime = new MantaRuntimeService(runtimeStore as RuntimeStore)
