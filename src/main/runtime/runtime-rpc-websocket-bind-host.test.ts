...
describe('MantaRuntimeRpcServer WebSocket bind host (STA-2370)', () => {
  ...
  it('binds all interfaces at startup when exposeNetworkByDefault is set (manta serve)', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'manta-runtime-rpc-'))
    const server = new MantaRuntimeRpcServer({
      runtime: new MantaRuntimeService(),
      userDataPath,
  ...
  it('upgrades a reused pending grant to network reach so its link survives a relaunch', async () => {
    ...
    const relaunched = new MantaRuntimeRpcServer({
      runtime: new MantaRuntimeService(),
      userDataPath,
      enableWebSocket: true,
      wsPort: 0
