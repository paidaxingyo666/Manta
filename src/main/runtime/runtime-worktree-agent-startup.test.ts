...
/** The launched CLI name is the whole decision: `manta` is the relay shim, `manta-ide` is local. */
...
describe('buildWorktreeStartupForAgent host resolution', () => {
  ...
  it('drops the Linux-only rename for both spellings of SSH ownership on two hosts', () => {
    expect(launchCliNameFor(makeRepo({ connectionId: 'm4air' }))).toBe('manta')
    expect(launchCliNameFor(makeRepo({ executionHostId: 'ssh:openclaw' }))).toBe('manta')
  })
  ...
  it('keeps the rename for a runtime host with no nested SSH target', () => {
    expect(launchCliNameFor(makeRepo({ executionHostId: 'runtime:vm-1' }))).toBe('manta-ide')
  })
})
