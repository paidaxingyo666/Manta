...
// Counts blocking fs calls against manta-runtime.json so the poll tick's I/O stays off the main thread.
const metadataSyncCalls = vi.hoisted(() => {
  const state = { recording: false, calls: [] as string[] }
  return {
    state,
    record(fn: string, target: unknown): void {
      if (state.recording && typeof target === 'string' && target.endsWith('manta-runtime.json')) {
        state.calls.push(fn)
      }
    }
  }
...
vi.mock('node:fs/promises', async () => {
  ...
  return {
    ...
    readFile: (async (target: unknown, options: never) => {
      ...
      if (typeof target !== 'string' || !target.endsWith('manta-runtime.json')) {
        return call()
      }
...
function record(overrides: Partial<RuntimeMetadata> = {}): RuntimeMetadata {
  return {
    ...
    pid: OWNED_PID,
    transports: [{ kind: 'unix', endpoint: '/tmp/manta-owner.sock' }],
    authToken: 'secret',
    startedAt: 100,
    ...overrides
  }
...
describe('watchRuntimeMetadataOwnership', () => {
  ...
  function makeUserDataPath(): string {
    const userDataPath = mkdtempSync(join(tmpdir(), 'manta-runtime-ownership-'))
