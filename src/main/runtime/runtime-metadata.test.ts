...
describe('runtime metadata', () => {
  ...
  it('writes and reads runtime metadata atomically', () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'manta-runtime-metadata-'))
  ...
  it('clears the runtime metadata file', () => {
    ...
    expect(getRuntimeMetadataPath(userDataPath)).toContain('manta-runtime.json')
  })
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    () => {
      const userDataPath = mkdtempSync(join(tmpdir(), 'manta-runtime-secure-files-'))
      tempDirs.push(userDataPath)
      ...
      for (const path of [
        join(userDataPath, 'manta-devices.json'),
        join(userDataPath, 'manta-e2ee-keypair.json'),
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    () => {
      const userDataPath = mkdtempSync(join(tmpdir(), 'manta-runtime-existing-secure-files-'))
      tempDirs.push(userDataPath)
      ...
      const devicesPath = join(userDataPath, 'manta-devices.json')
      const keypairPath = join(userDataPath, 'manta-e2ee-keypair.json')
  ...
  it('replaces oversized E2EE keypair files instead of reading them as metadata', () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'manta-runtime-large-keypair-'))
    tempDirs.push(userDataPath)
    const keypairPath = join(userDataPath, 'manta-e2ee-keypair.json')
