// #11761: on a headless `manta serve` host the HTTP agent hook is the only carrier of
...
async function projectAgentStatus(
  rows: AgentStatusIpcPayload[],
  preparePane?: (runtime: MantaRuntimeService) => void
...
function observePaneTitle(runtime: MantaRuntimeService, title: string): void {
  runtime.onPtyData(PTY_ID, `\x1b]0;${title}\x07`, Date.now())
}

function lastOscTitleEpochMs(runtime: MantaRuntimeService): number {
  const pty = (
