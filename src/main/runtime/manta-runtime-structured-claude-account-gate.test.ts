...
function runtimeWithAccounts(claude: ClaudeManagedAccountGateSettings | null): MantaRuntimeService {
  // No store at all is the unreadable-settings case the gate must fail closed on.
  const runtime = claude
    ? new MantaRuntimeService({ getSettings: () => claude } as never)
    : new MantaRuntimeService()
