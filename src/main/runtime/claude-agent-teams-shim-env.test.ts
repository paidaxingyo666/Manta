...
describe('claude agent teams shim env', () => {
  it('writes a private tmux shim that calls the Manta shim command', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-agent-teams-shim-'))
    roots.push(root)
  ...
  it('builds native shim env only for direct Claude commands', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-agent-teams-cli-'))
    roots.push(root)
    const cliName = process.platform === 'win32' ? 'manta-dev.cmd' : 'manta-dev'
  ...
  it('refuses to resolve a CLI through relative PATH entries or a bare override', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-agent-teams-cli-'))
    roots.push(root)
    for (const name of ['manta', 'manta-ide', 'manta.cmd']) {
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'manta-agent-teams-cli-'))
      roots.push(root)
      const cliPath = join(root, 'manta.cmd')
  ...
  it.skipIf(process.platform === 'win32')(
    'never runs a cwd-resolved manta when the shim bin is unqualified',
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'manta-agent-teams-shim-'))
      roots.push(root)
      await ensureClaudeAgentTeamsShimDir(root)
      const cwd = await mkdtemp(join(tmpdir(), 'manta-agent-teams-cwd-'))
      roots.push(cwd)
      const marker = join(cwd, 'hijacked')
      for (const name of ['manta', 'manta-ide']) {
  ...
  it('writes a Windows shim that rejects an unqualified shim bin', () => {
    ...
    expect(script).not.toMatch(/^set "MANTA_AGENT_TEAMS_SHIM_BIN=manta/m)
    expect(script).toContain('if "%MANTA_SHIM_BIN:~1,1%"==":" goto :run')
