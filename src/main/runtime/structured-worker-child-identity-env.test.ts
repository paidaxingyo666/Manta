...
const shim = vi.hoisted(() => ({ ensureLinuxTerminalMantaCliShimDir: vi.fn() }))
vi.mock('../cli/linux-terminal-manta-cli-shim', () => shim)
...
const USER_DATA = '/data/manta'
const RESOURCES = '/app/Resources'
const SHIM_DIR = join(USER_DATA, 'linux-manta-cli-shim')
...
beforeEach(() => {
  shim.ensureLinuxTerminalMantaCliShimDir.mockReset()
  shim.ensureLinuxTerminalMantaCliShimDir.mockReturnValue(SHIM_DIR)
...
describe('structuredWorkerChildIdentityEnv', () => {
  it('marks an ordinary chat session as having NO identity, and grants it nothing', () => {
    ...
    expect(env.MANTA_TERMINAL_HANDLE).toBeUndefined()
    ...
    expect(shim.ensureLinuxTerminalMantaCliShimDir).not.toHaveBeenCalled()
  })

  it('gives a packaged-Linux worker the bare-manta shim its MANTA_CLI_COMMAND assumes', () => {
    // Without this the child's first `orca orchestration check` execs GNOME Orca — the CLI
    // installs as `manta-ide` on Linux (stablyai/orca#7904) — and the dispatch hangs to timeout.
    pinPlatform('linux')
    installFakeAppEnvironment({ isPackaged: () => true, getPath: () => USER_DATA })
    const handle = registerWorker()
    const env = structuredWorkerChildIdentityEnv(SESSION_ID, { PATH: '/usr/bin:/bin' })
    expect(env.MANTA_TERMINAL_HANDLE).toBe(handle)
  ...
  it('never names the WSL-scoped launcher, because a structured worker cannot run in WSL', () => {
    // `manta-ide` is the literal the PTY lane exports for WSL only. A structured session that
    // resolves to a WSL distro is refused a host scope, so it never becomes a worker at all —
    // which is why the bare-`manta` shim, not the literal, is the right fix on Linux.
    expect(
      structuredWorkerHostScope({
    ...
    registerWorker()
    expect(
      structuredWorkerChildIdentityEnv(SESSION_ID, { PATH: '/usr/bin' }).MANTA_CLI_COMMAND
    ).not.toBe('manta-ide')
  })
})
