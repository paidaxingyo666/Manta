...
async function createRuntime(rows: AgentStatusIpcPayload[] = []): Promise<MantaRuntimeService> {
...
function publishRendererWorkingPane(runtime: MantaRuntimeService, paneTitle: string): void {
...
function publishRendererReleasedPane(runtime: MantaRuntimeService, title: string): void {
  runtime.attachWindow(1)
  runtime.syncWindowGraph(1, {
    tabs: [
      {
