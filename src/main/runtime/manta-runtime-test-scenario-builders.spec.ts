import * as mocks from './manta-runtime-test-mocks.spec'
import type { Mock } from 'vitest'

const { HEADLESS_RUNTIME_WINDOW_ID, MantaRuntimeService, electronMocks } = mocks
const { getBrowserHostLeaseRegistry, getDefaultWorkspaceSession, getRuntimeBrowserPageRegistry } =
  mocks
const { gitRunner, vi } = mocks

import * as fixtures from './manta-runtime-test-fixtures.spec'
...
// Why (#10333): `manta serve` publishes a ready graph under
...
function createWorktreeRemovalRuntime(runtimeStore: unknown = store): RuntimeService {
  ...
  return new MantaRuntimeService(runtimeStore as never, undefined, {
