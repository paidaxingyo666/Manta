...
import { getMantaCliCommandNameForPlatform } from '../../shared/manta-cli-command-name'
...
export async function buildClaudeAgentTeamsLaunchPlan(args: {
  ...
  if (!shimBin) {
    // Why: without an absolute CLI path the shim would resolve a bare `manta` against the pane cwd, so degrade instead.
    return {
      command: addClaudeTeammateModeInProcess(args.command),
      env: { CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS: '1' }
    }
...
/** Absolute path to the Manta CLI that backs the tmux shim, or null when none can be qualified. */
export function resolveClaudeAgentTeamsShimBin(
  ...
  const override = env.MANTA_AGENT_TEAMS_SHIM_BIN
  if (override) {
  ...
  return (
    findExecutableOnPath(process.platform === 'win32' ? 'manta-dev.cmd' : 'manta-dev', pathValue) ??
    findExecutableOnPath(getMantaCliCommandNameForPlatform(process.platform), pathValue)
  )
}

function defaultShimRoot(): string {
  return join(homedir(), '.manta', 'claude-agent-teams-bin')
}
...
function bundledLauncherPath(): string | null {
  ...
  if (process.platform === 'darwin') {
    return join(process.resourcesPath, 'bin', 'manta')
  }
  if (process.platform === 'linux') {
    return join(process.resourcesPath, 'bin', 'manta-ide')
  }
  if (process.platform === 'win32') {
    return join(process.resourcesPath, 'bin', 'manta.exe')
  }
  return null
}
...
// PATH entries on POSIX), so a stray `manta` next to the agent's files would run with the team token. Demand a
// fully-qualified binary instead of guessing one.
function unixShimScript(): string {
  return [
    '#!/usr/bin/env sh',
    'set -eu',
    'manta_bin=${MANTA_AGENT_TEAMS_SHIM_BIN:-}',
    'case $manta_bin in',
    '  /*|[A-Za-z]:[\\\\/]*) ;;',
    '  *)',
    '    echo "manta agent-teams tmux shim: MANTA_AGENT_TEAMS_SHIM_BIN must be an absolute path" >&2',
    '    exit 127',
    '    ;;',
    'esac',
    'exec "$manta_bin" agent-teams-tmux "$@"',
    ''
  ].join('\n')
}
...
export function windowsClaudeAgentTeamsShimScript(): string {
  return [
    '@echo off',
    'setlocal',
    'set "MANTA_SHIM_BIN=%MANTA_AGENT_TEAMS_SHIM_BIN%"',
    'if not defined MANTA_SHIM_BIN goto :unqualified',
    'if "%MANTA_SHIM_BIN:~1,1%"==":" goto :run',
    'if "%MANTA_SHIM_BIN:~0,2%"=="\\\\" goto :run',
    'goto :unqualified',
    ':run',
    // Why: no `call` — its extra percent-expansion pass would rewrite tmux pane args such as `%2` into batch parameters.
    '"%MANTA_SHIM_BIN%" agent-teams-tmux %*',
    'exit /b %ERRORLEVEL%',
    ':unqualified',
    'echo manta agent-teams tmux shim: MANTA_AGENT_TEAMS_SHIM_BIN must be an absolute path 1>&2',
    'exit /b 127',
    ''
  ].join('\r\n')
}
