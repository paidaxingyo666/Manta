...
function stubStructuredHostInstall(runtime: MantaRuntimeService): {
...
function createRuntime(location: TestLocation): MantaRuntimeService {
  const runtime = new MantaRuntimeService({ getSettings: () => ({}) } as never)
