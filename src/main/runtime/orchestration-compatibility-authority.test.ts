...
describe('orchestration compatibility runtime authority', () => {
  ...
  it('uses the hydrated hook commitment for a restored exact PTY', () => {
    ...
    const uncommitted = new MantaRuntimeService()
  ...
  it('attests current coordinators immediately without upgrading restored legacy callers', () => {
    ...
    const createIntegratedRuntime = (launchTokenHash: string | null): MantaRuntimeService => {
      const runtime = new MantaRuntimeService(null, undefined, {
    ...
    const evidenceFor = (runtime: MantaRuntimeService, launchToken = TOKEN) => ({
      terminalHandle: 'term-1',
      paneKey: PANE_KEY,
      launchToken,
