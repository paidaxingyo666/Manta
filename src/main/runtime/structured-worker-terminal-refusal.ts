...
export function structuredWorkerTerminalRefusal(
  ...
  const error = new Error(
    `${handle} is an agent session, not a terminal, so terminal commands cannot address it. ` +
      'Read its output with `manta terminal read` or `manta orchestration worker-read --source transcript`, ' +
      'send it work with `manta orchestration send`, and open it from its chat tab.'
  )
