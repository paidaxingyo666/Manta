          ...
          .map((message) =>
            formatMessageBanner(message, {
              ...
              supportedActionHints: readOnly
                ? []
                : supportedLegacyHints(message, principal, params.compatibilityCliCommand ?? 'manta')
            })
          )
          .join('\n\n')
      : undefined
  const currentDelivery =
    principal.role === 'coordinator' && db.hasPendingCurrentDelivery(principal.run_id)
      ? {
          ...
          checkCommand: `${params.compatibilityCliCommand ?? 'manta'} orchestration check --run ${principal.run_id}`,
          ackCommand: `${params.compatibilityCliCommand ?? 'manta'} orchestration check --run ${principal.run_id} --ack <delivery-id>`
        }
      : undefined
  return {
    runId: principal.run_id,
