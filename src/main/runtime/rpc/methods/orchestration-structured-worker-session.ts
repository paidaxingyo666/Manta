...
export async function createStructuredWorkerSession(args: {
  ...
  // ships a worker with no MANTA_TERMINAL_HANDLE, whose bare `manta orchestration check` then
...
export async function discardStructuredWorkerSession(
  sessionId: string,
  runtime: Pick<MantaRuntimeService, 'retireStructuredAgentSessionTabFromSnapshot'>
): Promise<void> {
