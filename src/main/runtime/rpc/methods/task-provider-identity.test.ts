...
const identities = [
  { provider: 'github', owner: 'Acme', repo: 'Manta', host: 'github.example.com' },
  {
    provider: 'gitlab',
    projectId: '123',
    namespace: 'acme/team',
    project: 'manta',
    webUrl: 'https://gitlab.example.com/acme/team/manta'
  },
  {
    provider: 'linear',
...
describe('task provider identity RPC validation', () => {
  ...
  it('preserves unknown fields and never infers GitHub from owner/repo', () => {
    const identity = { provider: 'gitlab', owner: 'acme', repo: 'manta', futureField: 'value' }
    expect(TaskProviderIdentity.parse(identity)).toEqual(identity)
  })
...
describe('github identity blank fields', () => {
  ...
  it.each(['', '   ', '\t'])('rejects a blank owner %j', (owner) => {
    expect(
      TaskProviderIdentity.safeParse({ provider: 'github', owner, repo: 'manta' }).success
    ).toBe(false)
  })
  ...
  it('still accepts a populated identity', () => {
    expect(
      TaskProviderIdentity.safeParse({ provider: 'github', owner: 'stablyai', repo: 'manta' })
        .success
    ).toBe(true)
  })
