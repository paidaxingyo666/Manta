...
import { RuntimeGitCommands, type ResolvedRuntimeGitWorktree } from '../../manta-runtime-git'
...
/** Stands in for manta-runtime-git.ts, which enforces the budget it is handed as its last argument. */
function stubRuntime(runtimeMethod: string): MantaRuntimeService {
  return {
...
function budgetArgument(runtime: MantaRuntimeService, runtimeMethod: string): unknown {
