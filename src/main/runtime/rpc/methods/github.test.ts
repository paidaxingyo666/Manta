...
describe('github RPC methods', () => {
  it('resolves the repo slug on the runtime server', async () => {
    const runtime = {
      ...
      getRepoSlug: vi.fn().mockResolvedValue({ owner: 'acme', repo: 'manta' })
  ...
  it('lists slug-addressed issue metadata on the runtime server', async () => {
    ...
    const labels = await dispatcher.dispatch(
      makeRequest('github.project.listLabelsBySlug', { owner: 'acme', repo: 'manta' })
    )
    const users = await dispatcher.dispatch(
    ...
    const issueTypes = await dispatcher.dispatch(
      makeRequest('github.project.listIssueTypesBySlug', { owner: 'acme', repo: 'manta' })
    )

    expect(runtime.listGitHubLabelsBySlug).toHaveBeenCalledWith({ owner: 'acme', repo: 'manta' })
    expect(runtime.listGitHubAssignableUsersBySlug).toHaveBeenCalledWith({
      owner: 'acme',
      repo: 'manta',
      seedLogins: ['octo']
    })
    expect(runtime.listGitHubIssueTypesBySlug).toHaveBeenCalledWith({ owner: 'acme', repo: 'manta' })
