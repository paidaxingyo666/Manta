...
describe('orchestration federated worker output', () => {
  ...
  let homeRuntime: MantaRuntimeService
  let workerRuntime: MantaRuntimeService
  ...
  beforeEach(() => {
    ...
    workerRuntime = new MantaRuntimeService()
    ...
    homeRuntime = new MantaRuntimeService(null, undefined, {
  ...
  function configureWorkerRuntime(runtime: MantaRuntimeService): void {
  ...
  it('reads the exact transcript on the worker server without leaking its path home', async () => {
    const dispatchId = await startRemoteWorker()
    const directory = await mkdtemp(join(tmpdir(), 'manta-federated-worker-output-'))
