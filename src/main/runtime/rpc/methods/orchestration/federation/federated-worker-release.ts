...
export async function releaseFederatedWorker(args: {
  ...
  try {
    ...
    return {
      ...receipt,
      lastError: `The execution host acknowledged ${remote.state}, but Manta could not apply the confirmed release to the home projection: ${detail}`,
...
function confirmedReleaseProjectionRecovery(dispatchId: string): string {
  return `Inspect with: manta orchestration worker-show --dispatch ${dispatchId} --json — then retry worker-release with a fresh request ID (omit --retry-request to let the CLI generate one). Reusing the prior request ID only replays the confirmed remote receipt without reapplying the home projection. Never substitute a broad terminal close.`
}
