...
export function failWorkerStartWithReceipt(args: {
  ...
  return {
    ...
    ...(adopted
      ? {
          recovery: `This start created a terminal that never ran the Task. Close it with: manta orchestration worker-release --dispatch ${args.dispatchId}`
        }
      : {}),
    ...(unknown
      ? {
          nextCommands: [
            `manta orchestration worker-show --dispatch ${args.dispatchId} --json`,
            `manta orchestration worker-abandon --dispatch ${args.dispatchId} --json`
          ]
        }
      : {})
  }
