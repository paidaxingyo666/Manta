...
export function requireWorkerAuthority(runtime: MantaRuntimeService, terminalHandle: string) {
...
export async function createWorkerWorktree(args: {
  ...
  coordinatorWorktree: Awaited<ReturnType<MantaRuntimeService['showManagedWorktree']>>
  params: {
    repo?: string
    name?: string
    baseBranch?: string
  ...
  worktree: Awaited<ReturnType<MantaRuntimeService['showManagedWorktree']>>
  ...
  return {
    worktree: created.worktree as Awaited<ReturnType<MantaRuntimeService['showManagedWorktree']>>,
    terminalHandle,
    setupReceipt
  }
}
