...
function precommitRefusal(error: unknown): AgentSessionWireRefusal {
  ...
  if (code) {
    return { code, message: 'Manta cannot open a structured agent chat for this workspace.' }
  }
  ...
  return {
    ...
    message: `Manta could not prepare a structured agent chat for this workspace: ${message}`
  }
}
