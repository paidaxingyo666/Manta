...
function stubStaleHandleRuntime(overrides: Partial<MantaRuntimeService> = {}): MantaRuntimeService {
  return {
...
async function expectStale(method: string, params: unknown, mutators: string[]): Promise<void> {
  ...
  const runtime = stubStaleHandleRuntime(spies as Partial<MantaRuntimeService>)
