 ...
 * to speak for itself.
 *
 * Without `MANTA_TERMINAL_HANDLE` the worker's Bash tool has nothing to pass as `--from`, and
 ...
 *
 * `MANTA_CLI_COMMAND: 'manta'` is honest ONLY because of the PATH prepend below. Manta's Linux CLI
 ...
 *
 * Deliberately NOT `MANTA_PANE_KEY`. Claude structured sessions run hooks, and a pane key in their
 ...
 * — which is why it is a different decision from withholding `MANTA_PANE_KEY`, not a reversal of it.
...
export function structuredWorkerChildIdentityEnv(
  ...
  const env: Record<string, string> = {
    ...childEnv,
    MANTA_TERMINAL_HANDLE: identity.handle,
    MANTA_CLI_COMMAND: 'manta'
  }
