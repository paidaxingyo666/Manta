...
import { MantaRuntimeWithCloseStructuredAgentSessionTab } from './manta-runtime-close-structured-agent-session-tab'
import { MantaRuntimeWithReconcileHeadlessMobileSessionBrowserTabs } from './manta-runtime-reconcile-headless-mobile-session-browser-tabs'
...
function reconcile(
  ...
  const runtime = MantaRuntimeWithReconcileHeadlessMobileSessionBrowserTabs.prototype as unknown as {
...
it('keeps the renderer publication epoch when selecting a client-hosted browser tab', () => {
  const storeMobileSessionSnapshot = vi.fn()
  const runtime = MantaRuntimeWithCloseStructuredAgentSessionTab.prototype as unknown as {
