...
import { MantaRuntimeWithRunCreateMobileSessionTerminal } from './manta-runtime-run-create-mobile-session-terminal'
...
export class MantaRuntimeWithResolveMobileSessionTerminalCommand extends MantaRuntimeWithRunCreateMobileSessionTerminal {
  protected async resolveMobileSessionTerminalCommand(
    ...
    // Why: SSH runs the CLI through the relay shim (plain `manta`), so the Linux-only `manta-ide` rename must not apply.
