...
import { enoent, resolveAuthorizedPathMock, statMock } from './manta-runtime-files-mock-registry'
...
} from './manta-runtime-files-terminal-artifact-fixtures'
...
describe('RuntimeFileCommands', () => {
  ...
  describe('resolveTerminalPath', () => {
    ...
    it('does not mint an absolute artifact grant from a relative path observed under stale cwd', async () => {
      const dir = await mkdtemp(join(tmpdir(), 'manta-terminal-artifact-'))
      tempDirs.push(dir)
