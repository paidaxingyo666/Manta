...
describe('remote runtime request connection integration', () => {
  it(
    ...
    async () => {
      const userDataPath = mkdtempSync(join(tmpdir(), 'manta-runtime-close-intent-'))
  ...
  it(
    ...
    { timeout: REMOTE_RUNTIME_TEST_TIMEOUT_MS },
    async () => {
      const userDataPath = mkdtempSync(join(tmpdir(), 'manta-runtime-request-'))
  ...
  it(
    ...
    { timeout: REMOTE_RUNTIME_TEST_TIMEOUT_MS },
    async () => {
      const userDataPath = mkdtempSync(join(tmpdir(), 'manta-runtime-request-events-'))
      const repoPath = join(userDataPath, 'repo')
      const repo: Repo = {
        id: 'repo-1',
        path: repoPath,
  ...
  it(
    ...
    { timeout: REMOTE_RUNTIME_TEST_TIMEOUT_MS },
    async () => {
      const userDataPath = mkdtempSync(join(tmpdir(), 'manta-runtime-remote-sleep-'))
      const clientEventListeners = new Set<(event: RuntimeClientEvent) => void>()
      const subscriptionCleanups = new Map<string, () => void>()
  ...
  it(
    ...
    { timeout: REMOTE_RUNTIME_TEST_TIMEOUT_MS },
    async () => {
      const userDataPath = mkdtempSync(join(tmpdir(), 'manta-runtime-shared-control-'))
      const repoPath = join(userDataPath, 'repo')
      const repo: Repo = {
        id: 'repo-1',
        path: repoPath,
