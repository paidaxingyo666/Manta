...
// STA repro (silent-send incident): `manta terminal send` to a leaf whose ptyId
...
function publishLeafGraph(runtime: MantaRuntimeService, leafPtyId: string): void {
...
describe('push-on-idle orchestration delivery absence gate', () => {
  ...
  it('waits a macrotask before delivering so a resolved check consumes its rows first', async () => {
    ...
    const pointers = payloads.filter((data) => data.includes('manta orchestration check'))
    ...
    try {
      ...
      const pointerWrites = () =>
        write.mock.calls.filter(
          ([, data]) => typeof data === 'string' && data.includes('manta orchestration check')
        )
