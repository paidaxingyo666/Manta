...
describe('terminal listing is deliberately left alone', () => {
  it('only worktree ps consumes the structured status summaries', async () => {
    ...
    // manta-runtime-subscribe-to-terminal-resize.ts owns listTerminals.
    const listing = await readFile(
      new URL('./manta-runtime-subscribe-to-terminal-resize.ts', import.meta.url),
      'utf8'
    )
    ...
    const worktreePs = await readFile(
      new URL('./manta-runtime-get-worktree-ps.ts', import.meta.url),
      'utf8'
    )
