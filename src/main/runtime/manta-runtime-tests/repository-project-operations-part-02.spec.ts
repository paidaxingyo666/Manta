...
describe('MantaRuntimeService', () => {
  it('adopts public clone repos into host-qualified project setup', async () => {
    ...
    try {
      const cloned = await runtime.cloneRepo('https://example.com/manta.git', destination)
  ...
  it('defaults runtime createRepo badgeColor to DEFAULT_REPO_BADGE_COLOR', async () => {
    ...
    const parentDir = await mkdtemp('/tmp/manta-runtime-create-')
    try {
  ...
  it('creates a missing runtime parent before creating the project directory', async () => {
    ...
    const runtime = new MantaRuntimeService(createStore as never)
    const tempRoot = await mkdtemp(join(tmpdir(), 'manta-runtime-create-parent-'))
    const parentDir = join(tempRoot, 'manta', 'projects')
    try {
  ...
  it('prepares the runtime worktree root when creating a repo', async () => {
    ...
    const runtime = new MantaRuntimeService(runtimeStore as never)
    const parentDir = await mkdtemp(join(tmpdir(), 'manta-runtime-create-root-prep-'))
    try {
  ...
  it('drops a same-path negative submodule cache before runtime cloneRepo', async () => {
    const spawnSpy = vi.spyOn(gitRunner, 'gitSpawnAfterWindowsEnvironmentReady')
    const destination = await mkdtemp(join(tmpdir(), 'manta-runtime-reclone-'))
    ...
    const runtime = new MantaRuntimeService(cloneStore as never)
  ...
  it('rejects runtime cloneRepo dot-segment URLs before spawning git', async () => {
    ...
    const tempRoot = await mkdtemp(join(tmpdir(), 'manta-runtime-clone-'))
  ...
  it('removes an owned runtime clone target when git exits unsuccessfully', async () => {
    ...
    const runtime = createRuntime()
    const destination = await mkdtemp(join(tmpdir(), 'manta-runtime-clone-'))
