...
import { TEST_WORKTREE_ID, TEST_WORKTREE_PATH, store } from '../manta-runtime-test-fixtures.spec'
import { wireHeadlessServeRuntime } from '../manta-runtime-test-scenario-builders.spec'
...
describe('MantaRuntimeService', () => {
  it('restores captured native Claude Agent Teams mode with fresh service env', async () => {
    ...
    await runtime.createTerminal(`path:${TEST_WORKTREE_PATH}`, {
      ...
      env: {
        ...
        MANTA_AGENT_TEAMS_SHIM_BIN: '/opt/manta/bin/manta-ide',
        MANTA_AGENT_TEAMS_TEAM_ID: 'stale-team',
        MANTA_AGENT_TEAMS_TOKEN: 'stale-token',
        TMUX: '/tmp/manta-claude-agent-teams/stale-team,0,1'
      },
      launchAgent: 'claude',
      launchConfig: {
        agentCommand: 'claude',
    ...
    expect(spawnCall?.env?.MANTA_AGENT_TEAMS_TEAM_ID).toMatch(/^team-/)
    expect(spawnCall?.env?.MANTA_AGENT_TEAMS_TEAM_ID).not.toBe('stale-team')
    expect(spawnCall?.env?.MANTA_AGENT_TEAMS_TOKEN).not.toBe('stale-token')
    expect(spawnCall?.env?.TMUX).not.toBe('/tmp/manta-claude-agent-teams/stale-team,0,1')
    ...
    expect(revealedLaunchConfig?.agentEnv.MANTA_AGENT_TEAMS_TEAM_ID).not.toBe('stale-team')
    expect(revealedLaunchConfig?.agentEnv.MANTA_AGENT_TEAMS_TOKEN).not.toBe('stale-token')
  })
  ...
  it('adopts renderer pane identity for remote runtime terminal creates', async () => {
    ...
    expect(spawnedEnv.MANTA_TAB_ID).toBe(tabId)
    expect(spawnedEnv.MANTA_PANE_KEY).toBe(`${tabId}:${leafId}`)
  })
  ...
  it('does not adopt web mirror ids as host terminal ids', async () => {
    ...
    const spawnedEnv =
      (spawn.mock.calls[0]?.[0] as { env?: Record<string, string> } | undefined)?.env ?? {}
    expect(spawnedEnv.MANTA_TAB_ID).not.toBe(tabId)
    expect(spawnedEnv.MANTA_TAB_ID).not.toMatch(/^web-terminal-/)
    expect(spawnedEnv.MANTA_PANE_KEY).toMatch(`${spawnedEnv.MANTA_TAB_ID}:`)
  })
  ...
  it('spawns focus-requested CLI terminal creates in background on headless serve', async () => {
    ...
    // `manta terminal create --worktree <wt> --command "echo test" --focus`
    await expect(
      runtime.createTerminal(`path:${TEST_WORKTREE_PATH}`, {
        command: 'echo test',
        focus: true
