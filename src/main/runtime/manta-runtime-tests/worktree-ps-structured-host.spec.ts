 ...
 * in the suite while `manta worktree ps` and mobile's poll would throw for every user.
 */
...
function installHost(feed: StructuredAgentSessionStatusFeed) {
  ...
  // caller cannot, because `manta-runtime-get-worktree-ps.ts` is `@ts-nocheck`.
...
describe('worktree ps reads the installed structured host', () => {
  ...
  it('reports the held session and asks the host for its live summaries', async () => {
    ...
    const { worktrees } = await new MantaRuntimeService(store).getWorktreePs()
