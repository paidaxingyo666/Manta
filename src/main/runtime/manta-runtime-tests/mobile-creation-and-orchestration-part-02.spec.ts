...
describe('MantaRuntimeService', () => {
  ...
  it('points a Run mailbox at its live-idle coordinator without replaying pending rows', async () => {
    ...
    try {
      ...
      expect(write).toHaveBeenCalledWith(
        'pty-1',
        '\nYou have 1 orchestration message. Run `manta orchestration check --run run_mailbox`.\n'
      )
  ...
  it('points already-idle Run mail after Codex replaces its completion title', async () => {
    ...
    await vi.waitFor(() => {
      expect(write).toHaveBeenCalledWith(
        'pty-1',
        '\nYou have 1 orchestration message. Run `manta orchestration check --run run_codex_native_title`.\n'
      )
    })
    db.close()
  })
