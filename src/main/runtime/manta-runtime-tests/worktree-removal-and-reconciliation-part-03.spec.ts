...
describe('MantaRuntimeService', () => {
  ...
  it('treats forced runtime deletion of an already-missing unregistered worktree as cleanup', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'manta-runtime-remove-'))
  ...
  it('routes already-missing SSH runtime history cleanup through the PTY owner', async () => {
    ...
    const metaById: Record<string, WorktreeMeta> = {
      [worktreeId]: makeWorktreeMeta({ hostId: 'ssh:ssh-1', mantaCreationSource: 'ssh' })
    }
    ...
    const runtime = new MantaRuntimeService(runtimeStore as never, undefined, {
  ...
  it('force-removes a legacy Manta-created runtime orphaned worktree directory after Git tracking is gone', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'manta-runtime-orphan-'))
