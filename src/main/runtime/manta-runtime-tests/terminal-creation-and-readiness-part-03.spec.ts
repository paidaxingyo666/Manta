...
describe('MantaRuntimeService', () => {
  ...
  it('matches canonical bare agent commands when a command override is configured', async () => {
    ...
    const runtime = new MantaRuntimeService(runtimeStore)
    runtime.setPtyController({
      spawn,
      write: () => true,
      kill: () => true,
  ...
  it('keeps non-bare agent command terminal creates unchanged', async () => {
    ...
    expect(spawnCall?.env?.MANTA_AGENT_LAUNCH_TOKEN).toBeUndefined()
  })
  ...
  it('injects runtime hook receiver env into terminal sessions', async () => {
    ...
    const runtime = new MantaRuntimeService(store, undefined, {
      buildAgentHookPtyEnv: () => ({
        MANTA_AGENT_HOOK_PORT: '5678',
        MANTA_AGENT_HOOK_TOKEN: 'agent-token',
        MANTA_AGENT_HOOK_ENV: 'remote',
    ...
    await runtime.createTerminal(`path:${TEST_WORKTREE_PATH}`, {
      ...
      env: {
        ...
        MANTA_AGENT_HOOK_TRANSPORT: 'stale-transport',
        MANTA_AGENT_HOOK_ENDPOINT: '/tmp/stale-endpoint.env'
      },
      envToDelete: ['CODEX_HOME', 'MANTA_CODEX_HOME']
    })
    ...
    expect(spawnCall?.env).toEqual(
      expect.objectContaining({
        ...
        MANTA_AGENT_HOOK_ENV: 'remote',
        MANTA_AGENT_HOOK_VERSION: '1',
        MANTA_PANE_KEY: expect.any(String),
        MANTA_TAB_ID: expect.any(String),
        MANTA_WORKTREE_ID: TEST_WORKTREE_ID
      })
    )
    expect(spawnCall?.env?.MANTA_AGENT_HOOK_ENDPOINT).toBeUndefined()
    expect(spawnCall?.env?.MANTA_AGENT_HOOK_TRANSPORT).toBeUndefined()
    expect(spawnCall?.envToDelete).toEqual(['CODEX_HOME', 'MANTA_CODEX_HOME'])
  })
  ...
  it.each([
    ...
    const folderPath = await mkdtemp(join(tmpdir(), 'manta-runtime-folder-workspace-'))
    ...
    expect(spawnedEnv.MANTA_WORKSPACE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
    expect(spawnedEnv.MANTA_PROJECT_GROUP_ID).toBe(TEST_FOLDER_PROJECT_GROUP_ID)
    expect(spawnedEnv.MANTA_WORKSPACE_ROOT).toBe(folderPath)
    expect(spawnedEnv.MANTA_WORKTREE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
  })
  ...
  it.each([
    ...
    expect(spawnCall?.env?.MANTA_WORKTREE_ID).toBe(FLOATING_TERMINAL_WORKTREE_ID)
    expect(spawnCall?.env?.MANTA_WORKSPACE_ID).toBeUndefined()
    expect(spawnCall?.env?.MANTA_PROJECT_GROUP_ID).toBeUndefined()
    expect(spawnCall?.env?.MANTA_WORKSPACE_ROOT).toBeUndefined()
  })

  it('rejects folder workspace terminal creation when the backing path is missing', async () => {
    const missingPath = join(tmpdir(), `manta-missing-folder-workspace-${randomUUID()}`)
    const spawn = vi.fn().mockResolvedValue({ id: 'pty-folder' })
  ...
  it('rejects folder workspace folderPath updates when the new path is missing', async () => {
    const missingPath = join(tmpdir(), `manta-missing-folder-update-${randomUUID()}`)
  ...
  it('reveals Claude Agent Teams launches with the rewritten launch config', async () => {
    ...
    expect(revealTerminalSession).toHaveBeenCalledWith(TEST_WORKTREE_ID, {
      ...
      launchToken: spawnedEnv.MANTA_AGENT_LAUNCH_TOKEN,
      launchAgent: 'claude',
      activate: false,
      tabId: spawnedEnv.MANTA_TAB_ID,
