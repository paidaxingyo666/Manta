...
describe('MantaRuntimeService', () => {
  ...
  it('sweeps an orphaned SSH worktree through its host provider', async () => {
    ...
    const runtime = new MantaRuntimeService(orphanStore as never, undefined, {
  ...
  it('recovers forced Windows runtime long-path removal and keeps skipped-hook warnings', async () => {
    ...
    try {
      ...
      expect(result).toEqual({
        ...
        warning: `manta.yaml archive hook skipped for ${TEST_WORKTREE_PATH}; pass --run-hooks to run it.`
      })
