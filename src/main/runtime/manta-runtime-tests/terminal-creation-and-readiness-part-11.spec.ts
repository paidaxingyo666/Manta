...
import { store, syncSinglePty } from '../manta-runtime-test-fixtures.spec'
...
describe('MantaRuntimeService', () => {
  ...
  it('strips retained terminal path line and hash locators before matching', () => {
    const colonPath = '/tmp/manta report/result.json'
  ...
  it('keeps tmpdir artifact paths in bounded PTY path candidates', () => {
    const artifactPath = join(tmpdir(), 'manta-runtime-retained-result.json')
    const candidates = appendRecentPtyPathCandidates(undefined, `wrote ${artifactPath}\n`)
  ...
  it('matches terminal artifact paths only when they appear in recent terminal output', () => {
    expect(
      recentTerminalOutputIncludesPath(
        'wrote /tmp/manta report/result.json:12:3',
        '/tmp/manta report/result.json',
        '/tmp/manta report/result.json'
      )
    ).toBe(true)
    expect(
      recentTerminalOutputIncludesPath(
        '\x1b]8;;file:///tmp/manta%20report/result.json\x1b\\result\x1b]8;;\x1b\\',
        '/tmp/manta report/result.json',
        '/tmp/manta report/result.json'
      )
    ).toBe(true)
    ...
    expect(
      recentTerminalOutputIncludesPath(
        'wrote /tmp/manta report/other.json',
        '/tmp/manta report/result.json',
        '/tmp/manta report/result.json'
      )
    ).toBe(false)
