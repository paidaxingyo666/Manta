...
import { MantaRuntimeService, join, listWorktrees, tmpdir } from '../manta-runtime-test-mocks.spec'
import type { WorktreeLineage, WorktreeMeta } from '../manta-runtime-test-mocks.spec'
import {
  TEST_REPO_ID,
  TEST_WORKTREE_ID,
  makeWorktreeInfo,
...
describe('MantaRuntimeService', () => {
  it('strips Manta provenance fields from runtime metadata updates', async () => {
    ...
    await runtime.updateManagedWorktreeMeta(`id:${TEST_WORKTREE_ID}`, {
      ...
      mantaCreationWorkspaceLayout: { path: '/tmp', nestWorkspaces: false }
    })
