...
import type { WorktreeMeta } from '../manta-runtime-test-mocks.spec'
import { TEST_REPO_ID, makeWorktreeMeta, store } from '../manta-runtime-test-fixtures.spec'
...
describe('MantaRuntimeService', () => {
  ...
  it('honors split setup placement for opted-in local startup-draft worktrees', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        ...
        MANTA_WORKTREE_PATH: '/tmp/workspaces/runtime-startup-setup-split'
      },
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        cwd: '/tmp/workspaces/runtime-startup-setup-split',
        command: expect.stringContaining('bash /tmp/repo/.git/manta/setup-runner.sh'),
        env: expect.objectContaining({
          MANTA_ROOT_PATH: '/tmp/repo',
          MANTA_WORKTREE_PATH: '/tmp/workspaces/runtime-startup-setup-split',
    ...
    const markerPath = `/tmp/repo/.git/manta/setup-runner.sh.${nonceMatch![1]}.done`
    ...
    expect(mainEnv.MANTA_TAB_ID).toBeDefined()
    expect(mainEnv.MANTA_PANE_KEY).toBeDefined()
  ...
  it('passes the wrapped setup command to activation when startup spawned but setup did not', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        MANTA_ROOT_PATH: '/tmp/repo',
        MANTA_WORKTREE_PATH: '/tmp/workspaces/runtime-startup-setup-retry'
      },
      waitForAgentStartup: true
    })
    vi.mocked(listWorktrees).mockResolvedValue([
    ...
    expect(activateWorktree).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        runnerScriptPath: 'C:\\tmp\\repo\\.git\\manta\\setup-runner.sh',
        command: expect.stringContaining('bash /mnt/c/tmp/repo/.git/manta/setup-runner.sh')
      }),
      undefined,
      undefined
    )
