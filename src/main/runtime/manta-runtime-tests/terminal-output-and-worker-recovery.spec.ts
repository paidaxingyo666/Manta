...
import type { RuntimeMobileSessionTabsResult } from '../manta-runtime-test-mocks.spec'
import {
  HEADLESS_LEAF_ID,
...
describe('MantaRuntimeService', () => {
  ...
  it('does not replay an already-delivered message on a later idle transition', async () => {
    ...
    try {
      ...
      const firstInjections = write.mock.calls.filter(
        (c) => typeof c[1] === 'string' && c[1].includes('manta orchestration check')
      ).length
  ...
  it('adopts preallocated MANTA_TERMINAL_HANDLE as a valid runtime handle', async () => {
    const runtime = new MantaRuntimeService(store)
