...
import { MantaRuntimeService, listWorktrees } from '../manta-runtime-test-mocks.spec'
import {
  HEADLESS_LEAF_ID,
...
describe('MantaRuntimeService', () => {
  ...
  it('attaches inline agent rows from the latest OSC 9999 status', async () => {
    const runtime = new MantaRuntimeService(store, undefined, makeAgentStatusStoreWiring().deps)
