...
describe('MantaRuntimeService', () => {
  ...
  it('applies Settings agent defaults to bare agent command terminal creates', async () => {
    ...
    expect(spawnCall?.env?.MANTA_AGENT_LAUNCH_TOKEN).toMatch(UUID_RE)
  ...
  // the CLI Manta can host (issue #11926).
  ...
  it.each([
    { platform: 'win32' as const, expected: 'manta.cmd claude-teams' },
    { platform: 'linux' as const, expected: 'manta-ide claude-teams' },
    { platform: 'darwin' as const, expected: 'manta claude-teams' }
  ])(
  ...
  it('resolves a startupAgent in a repo-less folder workspace', async () => {
    const folderPath = await mkdtemp(join(tmpdir(), 'manta-runtime-folder-startup-agent-'))
    const spawn = vi.fn().mockResolvedValue({ id: 'pty-bg' })
