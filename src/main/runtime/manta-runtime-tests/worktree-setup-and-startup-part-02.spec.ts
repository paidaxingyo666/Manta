...
import { expectStablePaneKeyEnv, store } from '../manta-runtime-test-fixtures.spec'
...
describe('MantaRuntimeService', () => {
  it('sequences setup before startup for opted-in local headless worktree creates', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      runnerScriptPath: 'C:\\tmp\\repo\\.git\\manta\\setup-runner.sh',
      shell: { family: 'posix', executable: 'wsl.exe' },
      envVars: {
        MANTA_ROOT_PATH: '/tmp/repo',
        MANTA_WORKTREE_PATH: '/tmp/workspaces/runtime-headless-startup-setup'
      },
    ...
    expect(startupScript).toContain('/mnt/c/tmp/repo/.git/manta/setup-runner.sh')
    expect(setupCommand).toContain('bash /mnt/c/tmp/repo/.git/manta/setup-runner.sh')
  ...
  it('starts setup and startup side by side by default for local headless worktree creates', async () => {
    ...
    vi.mocked(shouldRunSetupForCreate).mockReturnValue(true)
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      runnerScriptPath: '/tmp/repo/.git/manta/setup-runner.sh',
      envVars: {
        MANTA_ROOT_PATH: '/tmp/repo',
        MANTA_WORKTREE_PATH: '/tmp/workspaces/runtime-headless-parallel'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        command: expect.stringContaining('__MANTA_SETUP_COMPLETE__:')
      })
    )
  ...
  it('observes setup completion through the launch shell the runner was written for', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        MANTA_ROOT_PATH: '/tmp/repo',
        MANTA_WORKTREE_PATH: '/tmp/workspaces/runtime-observed-wsl-shell'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    const setupCommand = (spawn.mock.calls[1]![0] as { command: string }).command
    expect(setupCommand).toContain('bash /mnt/c/tmp/repo/.git/manta/setup-runner.sh')
    expect(setupCommand).toContain('__MANTA_SETUP_COMPLETE__:')
  })
  ...
  it('creates the first terminal for CLI-created worktrees without activating them', async () => {
    ...
    const initialLeafId = initialSpawnEnv.MANTA_PANE_KEY.slice(
      `${initialSpawnEnv.MANTA_TAB_ID}:`.length
    )
    ...
    expect(revealTerminalSession).toHaveBeenCalledWith(result.worktree.id, {
      ...
      activate: false,
      surfaceOwner: false,
      tabId: initialSpawnEnv.MANTA_TAB_ID,
  ...
  it('does not surface the new workspace when a background CLI create launches its agent', async () => {
    ...
    // `manta worktree create --agent claude` without --activate: the agent tab is
  ...
  it('honors split setup placement for CLI-created worktrees without startup agents', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        MANTA_ROOT_PATH: '/tmp/repo',
        MANTA_WORKTREE_PATH: '/tmp/workspaces/runtime-cli-setup-split'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(setupEnv.MANTA_TAB_ID).toBe(mainEnv.MANTA_TAB_ID)
    const mainLeafId = mainEnv.MANTA_PANE_KEY!.slice(`${mainEnv.MANTA_TAB_ID!}:`.length)
    expect(revealTerminalSession).toHaveBeenLastCalledWith(
      result.worktree.id,
      expect.objectContaining({
        ptyId: 'pty-cli-setup-setup',
        tabId: mainEnv.MANTA_TAB_ID,
        activate: false,
