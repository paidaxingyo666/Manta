...
describe('MantaRuntimeService', () => {
  it('keeps current-generation tab and leaf identity across a host restart', async () => {
    ...
    const makeRuntime = (): MantaRuntimeService => {
  ...
  it('rejects connection mismatch and reused handles while allowing a WSL-owned orphan', async () => {
    const makeRuntime = (): MantaRuntimeService => {
      ...
      return new MantaRuntimeService({ ...runtimeStore, flushOrThrow: vi.fn() } as never)
    }
    ...
    await withPlatform('win32', async () => {
      const makeWslRuntime = (reportedWslDistro?: string | null): MantaRuntimeService => {
        const { runtimeStore } = makeRuntimeStoreWithWorkspaceSession({
          ...getDefaultWorkspaceSession(),
          tabsByWorktree: { [TEST_WORKTREE_ID]: [] }
        })
        const wsl = new MantaRuntimeService({
          ...runtimeStore,
          flushOrThrow: vi.fn(),
          getProjects: () => [
            {
