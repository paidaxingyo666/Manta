...
import type { WorkspaceSessionState } from '../manta-runtime-test-mocks.spec'
import {
  HEADLESS_LEAF_ID,
...
import { publishLegacyWorkerReveal } from '../manta-runtime-test-scenario-builders.spec'

describe('MantaRuntimeService', () => {
  it('recovers exported MANTA_TERMINAL_HANDLE from discovered live PTY sessions', async () => {
  ...
  it('fences provider resume and reveals one exact live legacy worker without stealing focus', async () => {
    ...
    const runtime = new MantaRuntimeService({ ...runtimeStore, flushOrThrow } as never, undefined, {
