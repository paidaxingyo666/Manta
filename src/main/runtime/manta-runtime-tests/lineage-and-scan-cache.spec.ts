...
describe('MantaRuntimeService', () => {
  ...
  it('rejects SSH lineage updates when Manta worktree identity is missing', async () => {
    const remoteRepo = {
      id: 'remote-repo',
      path: '/home/user/repo',
      displayName: 'remote',
  ...
  it('rejects local lineage updates when Manta worktree identity is missing', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'manta-runtime-lineage-'))
    ...
    try {
      ...
      await expect(lstat(join(childPath, '.manta'))).rejects.toThrow()
      await expect(lstat(join(parentPath, '.manta'))).rejects.toThrow()
      expect(setWorktreeLineage).not.toHaveBeenCalled()
    } finally {
