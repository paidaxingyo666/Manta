...
// real in-process runtime via MANTA_USER_DATA_PATH, and asserts:
...
async function runBuiltCli(
  ...
  const child = spawn(process.execPath, [CLI_PATH, ...args], {
    env: {
      ...process.env,
      MANTA_USER_DATA_PATH: userDataPath,
      MANTA_TERMINAL_HANDLE: 'term_cli',
      ...extraEnv
    },
...
describeIfBuilt('manta orchestration check --wait subprocess (§3.4)', () => {
  it('emits newline-flushed JSON keepalives to stderr while waiting', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'manta-cli-sub-'))
    const runtime = new MantaRuntimeService()
    const db = new OrchestrationDb(':memory:')
    runtime.setOrchestrationDb(db)
    const server = new MantaRuntimeRpcServer({ runtime, userDataPath })
    await server.start()

    try {
      // Why: use the MANTA_KEEPALIVE_INTERVAL_MS escape hatch to shrink the
      ...
      const child = spawn(
        ...
        ],
        {
          env: {
            ...process.env,
            MANTA_USER_DATA_PATH: userDataPath,
            MANTA_TERMINAL_HANDLE: 'term_nobody',
            MANTA_KEEPALIVE_INTERVAL_MS: String(keepaliveMs)
          },
...
describeIfBuilt('manta orchestration reset subprocess', () => {
  it('validates reset scopes against an isolated runtime through the built CLI', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'manta-cli-reset-'))
    const runtime = new MantaRuntimeService()
    const db = new OrchestrationDb(':memory:')
    runtime.setOrchestrationDb(db)
...
describeIfBuilt('manta orchestration task readiness subprocess', () => {
  it('creates a late dependent as ready through the built CLI and runtime', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'manta-cli-task-readiness-'))
