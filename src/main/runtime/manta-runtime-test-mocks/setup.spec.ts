...
import type { MantaRuntimeService as MantaRuntimeServiceConstructor } from '../manta-runtime'
...
export function acknowledgeAgentPromptSubmit(
  runtime: InstanceType<typeof MantaRuntimeServiceConstructor>,
  ptyId: string,
  data: string
): void {
  if (data === '\r') {
...
vi.mock('../../hooks', () => ({
  ...
  parseMantaYaml: vi.fn().mockReturnValue(null)
}))
