...
export function createLegacyStorageCutoverFixture(): {
  ...
  const tempDir = mkdtempSync(join(tmpdir(), 'manta-legacy-storage-'))
  ...
  const rejection = first.insertMessage({
    ...
    type: 'heartbeat',
    payload: JSON.stringify({ _mantaLifecycleRejection: { code: 'migration', reason: 'cutover' } })
  })
  const lookalike = first.insertMessage({
    ...
    payload: JSON.stringify({
      userData: { _mantaLifecycleRejection: { code: 'not-a-top-level-audit-marker' } }
    })
  })
  const malformedRejections = [
    first.insertMessage({
      ...
      payload: '{"_mantaLifecycleRejection":'
    }),
    first.insertMessage({
      from: 'term_legacy_worker',
      to: 'term_legacy_coord',
      subject: 'Array marker',
      payload: JSON.stringify({ _mantaLifecycleRejection: [] })
    }),
    first.insertMessage({
      from: 'term_legacy_worker',
      to: 'term_legacy_coord',
      subject: 'String marker',
      payload: JSON.stringify({ _mantaLifecycleRejection: 'migration' })
    }),
    first.insertMessage({
      from: 'term_legacy_worker',
      to: 'term_legacy_coord',
      subject: 'Incomplete marker',
      payload: JSON.stringify({ _mantaLifecycleRejection: { code: 'migration' } })
    }),
    first.insertMessage({
      from: 'term_legacy_worker',
      to: 'term_legacy_coord',
      subject: 'Non-string marker fields',
      payload: JSON.stringify({ _mantaLifecycleRejection: { code: 19, reason: false } })
    }),
    first.insertMessage({
      from: 'term_legacy_worker',
      to: 'term_legacy_coord',
      subject: 'Array root',
      payload: JSON.stringify([
        { _mantaLifecycleRejection: { code: 'migration', reason: 'nested' } }
      ])
    }),
    first.insertMessage({
      from: 'term_legacy_worker',
