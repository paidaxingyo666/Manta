...
export type PreambleParams = {
  ...
  workerHandle: string
  devMode?: boolean
  // Why: packaged WSL panes install the scoped launcher as `manta-ide`;
  // other execution hosts keep their existing bare `manta` bridge.
  ...
  // shells have no agent prompt for Manta to reuse.
...
// Why: the dispatch preamble teaches agents about Manta's CLI commands for
...
export function buildDispatchPreamble(params: PreambleParams): string {
  // Why: in dev mode, agents must use manta-dev to connect to the dev runtime's
  // socket. Without this, agents inside the dev Electron app would call the
  // production CLI and talk to the wrong Manta instance (Section 6.4).
  const cli = params.devMode ? 'manta-dev' : (params.cliCommand ?? 'manta')
  ...
  const header = `You are working inside Manta, a multi-agent IDE. You are a dispatched worker.
...
prompt for Manta to reuse; if the coordinator has more for you it will
