...
export async function dispatchTaskToWorker(params: {
  ...
  // Why: dispatched agents use manta-dev in dev mode to reach the dev runtime's socket, not production (Section 6.4).
  const preamble = buildDispatchPreamble({
    ...
    devMode: process.env.MANTA_USER_DATA_PATH?.includes('manta-dev'),
