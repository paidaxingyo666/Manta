...
describe('resolveGroupAddress', () => {
  ...
  describe('agent name groups', () => {
    ...
    // Why: these are the titles cursor-agent natively emits and the ones Manta synthesizes
    // from Cursor hooks, so each must resolve.
    it('matches native and Manta-synthesized Cursor titles', () => {
      const terminals = [
        makeSummary('coordinator', { title: 'Coordinator' }),
    ...
    it('does not match another agent whose task title mentions a text cursor', () => {
      const terminals = [
        ...
        makeSummary('term_shell', { title: 'Terminal Cursor and Manta slows down' })
      ]
