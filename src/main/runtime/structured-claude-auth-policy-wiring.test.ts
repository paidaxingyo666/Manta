 ...
 * lives in `manta-runtime-get-worktree-ps.ts` — a `@ts-nocheck` file, so neither the
...
describe('structured Claude auth policy wiring', () => {
  ...
  // MantaRuntime chain (measured), so the wiring itself is pinned by source and the
  ...
  it('passes a settings-derived Claude auth policy to the host installer', () => {
    const source = readFileSync(join(__dirname, 'manta-runtime-get-worktree-ps.ts'), 'utf8')
