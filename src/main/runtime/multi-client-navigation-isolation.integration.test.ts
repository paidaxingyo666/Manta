...
describe('paired runtime navigation isolation', () => {
  ...
  /** `headless: true` models `manta serve` — no renderer notifier and no attached window. */
  ...
  it('still reveals to every client when a paired caller asks for all-surface navigation', async () => {
    ...
    // `manta worktree create --activate` against a remote runtime sends navigation 'all'.
    const harness = await startHarness()
    await subscribeBothClientEventStreams(harness)
  ...
  it('keeps create activation caller-scoped on a headless manta serve host', async () => {
