...
//   MANTA_TELEMETRY_DISABLED=1 → identical to DO_NOT_TRACK for this lane.
//   MANTA_DIAGNOSTICS_DISABLED=1 → ALSO disable local file writes. The escape
...
export type ObservabilityConsent = {
  ...
  readonly disabledReason?:
    | 'do_not_track'
    | 'manta_telemetry_disabled'
    | 'manta_diagnostics_disabled'
    | 'ci'
}
...
export function resolveObservabilityConsent(): ObservabilityConsent {
  ...
  const mantaDisabled = envOn('MANTA_TELEMETRY_DISABLED')
  const diagnosticsDisabled = envOn('MANTA_DIAGNOSTICS_DISABLED')
  const ci = inCI()
  ...
  if (diagnosticsDisabled) {
    return {
      localFileEnabled: false,
      bundleEnabled: false,
      disabledReason: 'manta_diagnostics_disabled'
    }
  }
  if (dnt || mantaDisabled) {
    // Local file remains active — DNT is a *network* signal, and the local
    // file never leaves the machine.
    return {
      localFileEnabled: true,
      bundleEnabled: false,
      disabledReason: dnt ? 'do_not_track' : 'manta_telemetry_disabled'
    }
  }
...
export function initObservability(): ObservabilityConsent {
  ...
  if (!c.localFileEnabled) {
    // Disabled at the CI / MANTA_DIAGNOSTICS_DISABLED level — leave the
 ...
 *  `platform` / `arch` / `osRelease` / `mantaChannel` inputs come from main
...
export function collectDiagnosticBundle(
  meta: Pick<
    ...
    'appVersion' | 'platform' | 'arch' | 'osRelease' | 'mantaChannel' | 'lookbackMinutes'
  >
): CollectedBundle {
