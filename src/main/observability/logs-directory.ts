...
// macOS convention is `~/Library/Application Support/Manta/logs/`; Windows and
...
// and that path has to resolve under plain Node (mantad) as well as the desktop.
...
function getUserDataDir(): string {
  ...
  const home = homedir()
  if (platform() === 'darwin') {
    return join(home, 'Library', 'Application Support', 'Manta')
  }
  if (platform() === 'win32') {
    return join(process.env.APPDATA ?? home, 'Manta')
  }
  return join(home, '.config', 'Manta')
}
