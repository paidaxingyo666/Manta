/**
 * Byte-for-byte snapshots of every shell wrapper file Manta generates, for all
...
/**
 * Every shell name the wrapper is allowed to write that is not Manta-namespaced.
 *
 * Each is a deliberate contract with the shell or with Manta's own features, not
...
function foreignGlobalsWritten(content: string): string[] {
  ...
  for (const line of content.split('\n')) {
    ...
    for (const name of [
      ...
      if (name && !/^_{0,2}manta_/i.test(name) && !CONTRACT_GLOBALS.has(name)) {
        names.add(name)
      }
    }
  }
...
describePosix('generated shell wrapper files', () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'manta-wrapper-snapshot-'))
  ...
  ])('%s wrappers write no shell global outside Manta’s namespace', (_transport, generate, dir) => {
    generate()
