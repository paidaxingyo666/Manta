...
beforeEach(() => {
  previousDisableTrustRpc = process.env.MANTA_DISABLE_CODEX_TRUST_RPC
  delete process.env.MANTA_DISABLE_CODEX_TRUST_RPC
...
afterEach(() => {
  ...
  if (previousDisableTrustRpc === undefined) {
    ...
    process.env.MANTA_DISABLE_CODEX_TRUST_RPC = previousDisableTrustRpc
  }
...
// separated (the shape the real 0.144.x binary writes). Manta's
...
describe('CodexHookService app-server trust grant lane', () => {
  it('treats Codex hashes as authoritative and records the verified grant', async () => {
    ...
    const selfComputed = computeTrustedHash({
      ...
      groupIndex: 0,
      handlerIndex: 0,
      command: wrapPosixHookCommand(join(tmpHome, '.manta', 'agent-hooks', 'codex-hook.sh')),
      timeoutSec: 10
    })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const material = getCodexManagedHookInstallMaterial()
      const userHook = { type: 'command' as const, command: 'after-manta.sh' }
      writeFileSync(
        targetPath,
        `${JSON.stringify(
          {
