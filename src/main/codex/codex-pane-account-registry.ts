...
import { getMantaUserDataPath } from './codex-home-paths'
import type {
 ...
 * across app restarts, so without an on-disk record Manta forgets a pane is on
...
// Why: bounds a file that only shrinks when Manta observes a PTY exit; a crash
...
function getRegistryPath(): string {
  return join(getMantaUserDataPath(), 'codex-pane-accounts.json')
}
