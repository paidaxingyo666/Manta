 ...
 * - 'removed': hooks are opted out; Manta entries are swept from the real home.
 */
...
/** Manta-side state dir; nothing extra is ever written into the user's ~/.codex. */
...
function assertHooksJsonGeneration(
  ...
  if (currentRaw !== expectedRaw || resolveHooksJsonWritePath(hooksJsonPath) !== hooksWritePath) {
    ...
    throw new Error('Codex hooks.json changed while Manta prepared its trust repair')
  }
}

/**
 * Ensures the real-home hook state matches the settings: installs and trusts
 * the Manta status hook when enabled, sweeps it when opted out. Idempotent and
...
function installRealHomeCodexHook(userDataPath: string): RealHomeCodexHookLane {
  ...
  // Why: sweep stale Manta entries out of events the managed lane no longer
  ...
  // Why: never leave an untrusted Manta entry in the user's real home — it
...
function reconcileManagedHookDefinition(
  ...
  if (!hasManagedDirectCommand && nestedLocations.length === 1) {
    ...
    if (definition.matcher === undefined && !hasDirectCommand) {
      ...
      // Why: users can append groups or handlers after Manta's first install.
...
function sweepRealHomeCodexHook(): RealHomeCodexHookLane {
  ...
  if (removedAny) {
    ...
    // Why: dead [hooks.state] blocks for a removed hook are Manta-owned records;
    ...
    // groups must never make Manta delete a user's trust record at the same key.
    try {
      ...
      console.warn('[codex-real-home-hooks] failed to drop Manta trust entries:', error)
    }
  }
  return 'removed'
}

/** One-time pristine copy of the user's file, kept under Manta's userData. */
function backupRealHomeHooksJsonOnce(userDataPath: string, previousRaw: string | null): void {
  ...
  const backupPath = join(backupDir, 'hooks.json.pre-manta')
