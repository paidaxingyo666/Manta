...
function resolveAccountManagedHome(candidate: string, userDataPath: string): string | null {
  ...
  try {
    return readFileSync(join(candidate, '.manta-managed-home'), 'utf-8').trim() === accountId
      ? expectedHome
      : null
  } catch {
    return null
...
export function resolveManagedCodexShellPreflightHome(
  ...
  if (!codexHome || !mantaCodexHome || !pathsEqual(codexHome, mantaCodexHome)) {
    return null
  }
...
export async function prepareManagedCodexHomeBeforeShellLaunch(args: {
  ...
  const env = args.env ?? {
    ...
    MANTA_CODEX_HOME: process.env.MANTA_CODEX_HOME
  }
