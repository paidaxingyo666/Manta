...
describe('CodexHookService', () => {
  it('removes legacy Manta-managed hooks from system ~/.codex during install', () => {
  ...
  it('removes very large legacy Manta-managed hook lists from system ~/.codex', () => {
    const systemCodexHome = join(homes.tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
    const legacyCommand = legacyManagedHookCommand()
    mkdirSync(systemCodexHome, { recursive: true })
  ...
  it('removes the legacy Manta Codex profile file when it only contains managed hooks', () => {
    const systemCodexHome = join(homes.tmpHome, '.codex')
  ...
  it('removes only the legacy Manta block from a user-edited Codex profile file', () => {
    const systemCodexHome = join(homes.tmpHome, '.codex')
    const profilePath = join(systemCodexHome, 'manta-agent-status.config.toml')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(
