...
describe('Codex Windows hook command', () => {
  it.each(['测试用户', '홍길동', '日本語', 'rené', '测试 用户', "测试 O'Brien"])(
    ...
    (profile) => {
      ...
      const path = `C:\\Users\\${profile}\\.manta\\agent-hooks\\codex-hook.cmd`
  ...
  it('preserves the existing ASCII command and POSIX launcher', () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('win32')
    const path = 'C:\\Users\\alice\\.manta\\agent-hooks\\codex-hook.cmd'
    expect(getManagedCommand(path)).toBe(path)
    vi.spyOn(process, 'platform', 'get').mockReturnValue('linux')
    expect(getManagedCommand('/home/测试/.manta/agent-hooks/codex-hook.sh')).toContain(
      "[ -x '/home/测试/.manta/agent-hooks/codex-hook.sh' ]"
    )
  })
})
  ...
  it.each([windowsPowerShell, ...(windowsPwsh ? [windowsPwsh] : [])])(
    ...
    async (shell) => {
      ...
      const server = createServer((req, res) => {
        ...
        req.on('end', () => {
          tokens.push(req.headers['x-manta-agent-hook-token'])
      ...
      const env = {
        ...
        MANTA_AGENT_HOOK_TOKEN: 'unicode-test-token',
        MANTA_PANE_KEY: 'unicode-tab:unicode-leaf',
        MANTA_WORKTREE_ID: 'C:\\folder workspace\\测试 & repo'
      }
      ...
      try {
        ...
        for (const post of posts) {
          expect(post.get('paneKey')).toBe(env.MANTA_PANE_KEY)
          expect(post.get('worktreeId')).toBe(env.MANTA_WORKTREE_ID)
        }
