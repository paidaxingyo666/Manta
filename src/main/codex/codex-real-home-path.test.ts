...
describe('hasCustomCodexHomeOverride', () => {
  ...
  it('preserves a genuinely custom CODEX_HOME', () => {
    expect(
      hasCustomCodexHomeOverride({
        ...
        MANTA_CODEX_HOME: `${process.cwd()}${sep}codex-runtime-home${sep}home`
      })
    ).toBe(true)
  })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const paneHome = mkdtempSync(join(tmpdir(), 'manta-codex-pane-home-'))
  ...
  // a Dock-launched Manta, so the launch env is the only place it appears. Reading the
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const paneHome = mkdtempSync(join(tmpdir(), 'manta-codex-fish-home-'))
      temporaryHomes.push(paneHome)
