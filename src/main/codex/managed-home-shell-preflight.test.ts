...
function makeRoot(): string {
  const root = mkdtempSync(join(tmpdir(), 'manta-codex-shell-preflight-'))
  roots.push(root)
  return root
}
...
describe('managed Codex shell preflight', () => {
  it('accepts the Manta shared runtime home', () => {
    ...
    expect(
      resolveManagedCodexShellPreflightHome(
        { CODEX_HOME: home, MANTA_CODEX_HOME: home },
        userDataPath
      )
    ).toBe(home)
  })
  ...
  it('accepts a marker-proven account home and installs only while hooks are enabled', async () => {
    ...
    mkdirSync(home, { recursive: true })
    writeFileSync(join(home, '.manta-managed-home'), 'account-1\n')
    const install = vi.fn(() => ({
    ...
    const env = { CODEX_HOME: home, MANTA_CODEX_HOME: home }
  ...
  it('rejects user-owned homes and mismatched shell routing markers', () => {
    ...
    expect(
      resolveManagedCodexShellPreflightHome(
        { CODEX_HOME: userHome, MANTA_CODEX_HOME: userHome },
        userDataPath
      )
    ).toBeNull()
    expect(
      resolveManagedCodexShellPreflightHome(
        { CODEX_HOME: userHome, MANTA_CODEX_HOME: managedHome },
        userDataPath
      )
    ).toBeNull()
    expect(
      resolveManagedCodexShellPreflightHome(
        { CODEX_HOME: managedHome, MANTA_CODEX_HOME: undefined },
        userDataPath
      )
    ).toBeNull()
  })
  ...
  it.skipIf(process.platform === 'win32')('rejects a shared-home symlink escape', () => {
    ...
    expect(
      resolveManagedCodexShellPreflightHome(
        { CODEX_HOME: candidate, MANTA_CODEX_HOME: candidate },
        userDataPath
      )
    ).toBeNull()
  })
  ...
  it.skipIf(process.platform === 'win32')('rejects an account-home symlink escape', () => {
    ...
    writeFileSync(join(outside, '.manta-managed-home'), 'account-1\n')
  ...
  it('rejects a parent traversal account marker', () => {
    ...
    mkdirSync(candidate)
    writeFileSync(join(candidate, '.manta-managed-home'), '..\n')
  ...
  it.skipIf(process.platform === 'win32')('rejects a symlinked accounts root', () => {
    ...
    writeFileSync(join(outside, 'account-1', 'home', '.manta-managed-home'), 'account-1\n')
