...
describe('codex hook trust write-back promotion', () => {
  ...
  it('keeps an in-Manta approval of a user hook across launches and promotes it to ~/.codex', () => {
    writeSystemUserHook()
  ...
  it('never promotes trust for the Manta-managed status hook into ~/.codex', () => {
    writeSystemUserHook()
    const service = new CodexHookService()
    service.install()
  ...
  it('promotes an in-Manta disable of a mirrored user hook back to the system config', () => {
    writeSystemUserHook()
    const systemTomlPath = join(systemCodexDir(), 'config.toml')
    writeFileSync(systemTomlPath, upsertHookTrustEntriesInContent('', [systemUserStopEntry()]))
