...
function getProvenancePath(runtimeHomePath: string): string {
  return join(runtimeHomePath, '.manta-hook-trust-provenance.json')
}
...
/**
 * Records the runtime config.toml trust state Manta leaves behind after an
 * install/refresh, so the next launch can tell "entry Manta wrote" apart from
 ...
 * `config.toml` trust entries Manta wrote apart from which the user approved
 * inside Codex. Overwriting it from the current config state after a failed
 * read stamps the user's approval as Manta-written, and promotion then skips it
...
/**
 * Promotes hook approvals the user made inside Manta-launched Codex (written
...
function promoteCodexRuntimeHookApprovalsToSystemUnsafe(runtimeHomePath: string): void {
  ...
  // Why: without a snapshot of what Manta last wrote (first launch after
  ...
  // in-Manta approval. Promoting would resurrect trust the user revoked in
  ...
  for (const [key, state] of runtimeTrust) {
    ...
    if (
      ...
      // Manta wrote this entry and nothing touched it since — not an approval.
      continue
    }
    ...
    const hook = Array.isArray(definition?.hooks)
      ...
      : undefined
    // Why: never write trust for Manta's managed status hook into the user's
    // real config — mutating ~/.codex for Manta's own hooks is exactly what the
