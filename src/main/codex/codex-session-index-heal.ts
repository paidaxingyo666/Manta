...
// stamped `complete` on first app-server startup), so rollouts that Manta's
...
// Codex's DB-driven surfaces. Manta never writes Codex's sqlite schema itself.
...
function buildNativeHealInvocation(
  ...
  return {
    ...
    args: spawnArgs,
    cliPath: command,
    // Why: pin the real home explicitly — nested Manta launches can inherit a
