...
export function syncSystemConfigIntoManagedCodexHome(
  ...
  // user made inside Manta-launched Codex (/model, /approvals) must be written
  ...
  // unpromoted runtime change as Manta-written would strand it forever.
  snapshotCodexRuntimeSettingsBaseline(
    homes.runtimeHomePath,
    new Map(
...
function mergeSystemCodexConfigIntoRuntime(runtimeConfig: string, systemConfig: string): string {
  ...
  // trust and project trust are written under Manta's managed CODEX_HOME and
