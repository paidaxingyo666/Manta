...
describe('CodexHookService', () => {
  // Why (#16441): install promotes in-Manta approvals into ~/.codex/config.toml
  ...
  it('installs PermissionRequest with trust so Codex approval prompts reach Manta', async () => {
    const systemCodexHome = join(homes.tmpHome, '.codex')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(
      join(systemCodexHome, 'config.toml'),
        ...
        for (const eventName of localManagedCodexEvents()) {
          ...
          expect(command).toBe(
            buildWindowsHookPowerShellCommand(
              join(homedir(), '.manta', 'agent-hooks', 'codex-hook.cmd')
            )
          )
        }
      } finally {
  ...
  it('keeps hooks isolated by Manta userData instead of mutating system ~/.codex', async () => {
    const systemCodexHome = join(homes.tmpHome, '.codex')
