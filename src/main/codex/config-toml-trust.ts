...
// Why: Codex and Manta can disagree on quote style, separators, and casing for the same Windows project.
...
// Why: separator/casing drift between Codex-written and Manta-built keys must not stop findTrustBlockRanges from matching.
export function normalizeHookTrustKeyForLookup(key: string): string {
  ...
  // Windows remotes need the same folding when Manta runs on macOS or Linux.
