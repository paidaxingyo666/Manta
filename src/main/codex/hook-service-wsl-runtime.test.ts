...
function createTestPlan(): CodexWslRuntimeHookInstallPlan {
  const root = mkdtempSync(join(tmpdir(), 'manta-codex-wsl-hooks-'))
  tempRoots.push(root)
  const linuxHome = '/home/alice/.local/share/manta/codex-runtime-home/home'
  return {
    configPath: join(root, 'hooks.json'),
    tomlPath: join(root, 'config.toml'),
    scriptPath: join(root, '.manta', 'agent-hooks', 'codex-hook.sh'),
    commandScriptPath: `${linuxHome}/.manta/agent-hooks/codex-hook.sh`,
...
describe('Codex WSL runtime hook install', () => {
  ...
  it('coalesces aliases of one runtime home without blocking independent homes', async () => {
    ...
    const firstHome =
      '\\\\wsl$\\Ubuntu\\home\\Alice\\.local\\share\\manta\\codex-runtime-home\\home'
    const alias = firstHome.replace('\\\\wsl$', '\\\\wsl.localhost')
    const independent = firstHome.replace('\\Alice\\', '\\Bob\\')
  ...
  it('plans WSL hook files with Linux command and trust paths', () => {
    ...
    expect(
      ...
      scriptPath: pathWin32.join(runtimeHome, '.manta', 'agent-hooks', 'codex-hook.sh'),
      commandScriptPath:
        '/home/alice/.local/share/manta/codex-runtime-home/home/.manta/agent-hooks/codex-hook.sh',
      trustConfigPath: '/home/alice/.local/share/manta/codex-runtime-home/home/hooks.json',
      wslDistro: 'Ubuntu',
      linuxRuntimeHome: '/home/alice/.local/share/manta/codex-runtime-home/home'
    })
  })

  it('plans WSL hooks when the distro home is mounted on a Windows drive', async () => {
    const runtimeHome = 'D:\\wsl-home\\.local\\share\\manta\\codex-runtime-home\\home'
    ...
    expect(
      ...
      scriptPath: pathWin32.join(runtimeHome, '.manta', 'agent-hooks', 'codex-hook.sh'),
      commandScriptPath:
        '/mnt/d/wsl-home/.local/share/manta/codex-runtime-home/home/.manta/agent-hooks/codex-hook.sh',
      trustConfigPath: '/mnt/d/wsl-home/.local/share/manta/codex-runtime-home/home/hooks.json',
      wslDistro: 'Ubuntu',
      linuxRuntimeHome: '/mnt/d/wsl-home/.local/share/manta/codex-runtime-home/home'
    })
  })

  it('uses WSL-canonical paths for hook commands and trust keys', async () => {
    const runtimeHome =
      '\\\\wsl.localhost\\Ubuntu\\home\\alias\\.local\\share\\manta\\codex-runtime-home\\home'
    const canonicalHome = '/home/alice/.local/share/manta/codex-runtime-home/home'
    ...
    const plan = createCodexWslRuntimeHookInstallPlan(
      ...
      (distro, linuxPath) => {
        ...
        expect(linuxPath).toBe('/home/alias/.local/share/manta/codex-runtime-home/home')
        return canonicalHome
      }
    )

    expect(plan?.commandScriptPath).toBe(`${canonicalHome}/.manta/agent-hooks/codex-hook.sh`)
  ...
  it('removes managed trust when the WSL canonical path changes', async () => {
    ...
    const oldPlan = {
      ...plan,
      commandScriptPath: '/old/home/.manta/agent-hooks/codex-hook.sh',
    ...
    const newPlan = {
      ...plan,
      commandScriptPath: '/new/home/.manta/agent-hooks/codex-hook.sh',
  ...
  it('generates a POSIX hook that bridges WSL loopback failures through Windows curl', async () => {
    ...
    expect(script).toContain('unset MANTA_AGENT_HOOK_TRANSPORT')
    expect(script).toContain('"set MANTA_AGENT_HOOK_TOKEN="*)')
    ...
    expect(script).toContain('-H "X-Manta-Agent-Hook-Meta-Encoding: base64"')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      writeFileSync(
        endpointPath,
        [
          'set MANTA_AGENT_HOOK_PORT=43210',
          'set MANTA_AGENT_HOOK_TOKEN=fresh-token',
          'set MANTA_AGENT_HOOK_ENV=development',
          'set MANTA_AGENT_HOOK_VERSION=1',
          'set MANTA_AGENT_HOOK_TRANSPORT=raw-json-v1',
          ''
        ].join('\r\n'),
        'utf-8'
      )
      ...
      const result = spawnSync('/bin/sh', [plan.scriptPath], {
        ...
        env: {
          ...
          MANTA_AGENT_HOOK_ENDPOINT: endpointPath,
          MANTA_AGENT_HOOK_PORT: '1',
          MANTA_AGENT_HOOK_TOKEN: 'stale-token',
          MANTA_PANE_KEY: 'pane-1',
          MANTA_TEST_CAPTURE: capturePath
        }
      })
      ...
      expect(posted).toContain('X-Manta-Agent-Hook-Token: fresh-token')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const posted = readFileSync(capturePath, 'utf-8')
      expect(posted).toContain('http://127.0.0.1:43210/hook/codex')
      expect(posted).toContain('X-Manta-Agent-Hook-Token: token')
    }
  )

  it('installs trusted WSL hooks and removes only Manta entries when disabled', async () => {
    const plan = createTestPlan()
              ...
              hooks: [
                {
                  type: 'command',
                  command:
                    "if [ -x '/old/.manta/agent-hooks/codex-hook.sh' ]; then /bin/sh '/old/.manta/agent-hooks/codex-hook.sh'; fi"
                }
              ]
            }
          ]
...
describe('Codex WSL runtime hook install app-server grant lane', () => {
  ...
  beforeEach(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'manta-wsl-grant-userdata-'))
