...
export function syncSystemCodexSessionsIntoManagedHome(sourceCodexHomePath?: string): void {
  ...
  const managedSessionsRoot = join(getMantaManagedCodexHomePath(), 'sessions')
...
function replaceSymlinkSessionBridgeWithHardlink(
  ...
  try {
    ...
    replacementPath = `${targetPath}.manta-link-${process.pid}-${Date.now()}`
...
function getLegacySessionCopyMarkerPath(relativePath: string): string {
  return join(getMantaManagedCodexHomePath(), '.manta-session-copies', `${relativePath}.json`)
}
