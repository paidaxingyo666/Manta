...
beforeEach(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'manta-codex-settings-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'manta-codex-settings-user-data-'))
...
function baselinePath(): string {
  return join(runtimeHomeDir(), '.manta-config-settings-baseline.json')
}
...
describe('codex settings write-back promotion', () => {
  ...
  it('keeps runtime-only settings when promotion has to create ~/.codex/config.toml', () => {
    // Why: `codex mcp add` inside a Manta-launched Codex writes into the runtime
    ...
    expect(readRuntimeConfig()).toContain('[features]')
    expect(readRuntimeConfig()).toContain('model = "o4"')
    // Trust stays runtime-owned; Manta must not write it into the real ~/.codex.
...
describe('codex [tui] settings write-back promotion', () => {
  it('promotes a runtime [tui] block (codex 0.144.6 shape) into ~/.codex and survives the remirror', () => {
    ...
    // The user customizes the status line/theme inside Manta-launched Codex.
