...
// Why: Pre/PostToolUse feed the live in-flight-tool readout; PermissionRequest exits with no decision so Codex still shows its approval UI while Manta flips the pane to waiting.
const CODEX_EVENTS = [
  'SessionStart',
  'UserPromptSubmit',
  'PreToolUse',
...
function getConfigPath(runtimeHomePath: string = getMantaManagedCodexHomePath()): string {
...
function writeCodexHooksJson(configPath: string, hooks: Record<string, HookDefinition[]>): void {
  // Why: Codex rejects unknown top-level hooks.json fields, so plugin bookkeeping like `_managed` must not survive Manta's rewrite.
  writeHooksJson(configPath, { hooks })
}

function getCodexConfigTomlPath(runtimeHomePath: string = getMantaManagedCodexHomePath()): string {
...
function getTrustedSystemUserHookSignatures(
  ...
  try {
    ...
    // Why: a hand-broken system config.toml should only disable user-hook trust mirroring, not block Manta's managed runtime hooks.
...
function removeStaleWslRuntimeManagedHookTrustEntries(
  ...
  removeStaleWslCodexManagedHookTrustEntries({
    ...
    buildManagedCommand: (linuxRuntimeHome) =>
      wrapReadablePosixHookCommand(`${linuxRuntimeHome}/.manta/agent-hooks/codex-hook.sh`),
    priorLedgerHomes
  })
}
...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  ...
  return [
    '#!/bin/sh',
    ...buildPosixHookPayloadCapture(),
    // Why: sourcing refreshes PORT/TOKEN/ENV/VERSION from the current Manta so a surviving PTY keeps reporting after a restart (see claude/hook-service.ts).
    'load_hook_endpoint() {',
    '  endpoint_path="$1"',
    '  case "$endpoint_path" in',
    '    *.cmd)',
    // Why: Windows passes endpoint.cmd into WSL via WSLENV; parse only Manta's known assignments since cmd.exe `set` lines aren't shell syntax.
    ...
    '          "set MANTA_AGENT_HOOK_PORT="*) MANTA_AGENT_HOOK_PORT=${endpoint_line#*=} ;;',
    '          "set MANTA_AGENT_HOOK_TOKEN="*) MANTA_AGENT_HOOK_TOKEN=${endpoint_line#*=} ;;',
    '          "set MANTA_AGENT_HOOK_ENV="*) MANTA_AGENT_HOOK_ENV=${endpoint_line#*=} ;;',
    '          "set MANTA_AGENT_HOOK_VERSION="*) MANTA_AGENT_HOOK_VERSION=${endpoint_line#*=} ;;',
    '        esac',
    '      done < "$endpoint_path"',
    '      ;;',
    '    *)',
    ...
    '  load_hook_endpoint "$MANTA_AGENT_HOOK_ENDPOINT"',
    'fi',
    ...
    '  printf \'%s\' "$payload" | "$curl_bin" -sS -X POST "http://127.0.0.1:${MANTA_AGENT_HOOK_PORT}/hook/codex" \\',
...
function installManagedHooksIntoWslRuntime(
  ...
  try {
    ...
    if (grant.lane === 'fallback') {
      ...
      // here; only upsert Manta's entries instead of sweeping the whole source.
...
export class CodexHookService {
  ...
  install(runtimeHomePath: string = getMantaManagedCodexHomePath()): AgentHookInstallStatus {
    const configPath = getConfigPath(runtimeHomePath)
    const scriptPath = getManagedScriptPath()
    // Why: must run before this install rewrites hooks.json/config.toml —
    // approvals the user made inside Manta-launched Codex are keyed to the
    ...
    try {
      ...
      if (grant.lane === 'rpc') {
        ...
        // all old runtime [hooks.state.*] blocks would keep Manta Codex trusted.
  ...
  async installRemote(
    ...
    remoteHome: string,
    options?: {
      /** Explicit CODEX_HOME dir (flat layout). WSL sessions read Manta's managed runtime home, not ~/.codex, so the default location leaves them hookless. */
      codexHomeDir?: string
    ...
    try {
      ...
      // Why: SSH remotes use POSIX `.sh` paths even when Manta runs on Windows; never derive remote script syntax from local OS.
      await writeManagedScriptRemote(sftp, remoteScriptPath, getManagedScript('posix'))
      // Why: SSH edits the user's remote ~/.codex/hooks.json directly, so preserve non-Manta top-level metadata while replacing the hooks tree.
  ...
  refreshRuntimeUserHooks(
    ...
    const configPath = getConfigPath(runtimeHomePath)
    // Why: same as install() — capture in-Manta approvals before this refresh
    ...
    try {
      ...
      // Why: this path is used when Manta status hooks are disabled. The
      // runtime CODEX_HOME should keep user hooks, but not Manta-managed trust.
