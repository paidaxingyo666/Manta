...
describe('ensureRealHomeCodexHookState (install)', () => {
  it('creates hooks.json with the Manta entry in every managed event for a fresh home', () => {
    grantSucceeds()
  ...
  it("keeps later user handler trust positions stable inside Manta's hook group", () => {
    grantSucceeds()
    ensureRealHomeCodexHookState({ hooksEnabled: true, userDataPath: userDataDir })
    const installed = readRealHooksJson()
...
describe('ensureRealHomeCodexHookState (opt-out sweep)', () => {
  ...
  it('rebases trust when a user appended hooks after Manta installed', () => {
    grantSucceeds()
  ...
  it('removes only Manta entries and reports the removed lane', () => {
    grantSucceeds()
    const userStop = {
      matcher: 'deploy-*',
  ...
  it('removes only hash-proven Manta trust from a mixed hook group', () => {
    const material = getCodexManagedHookInstallMaterial()
