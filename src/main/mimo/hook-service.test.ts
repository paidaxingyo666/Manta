...
describe('MimoCodeHookService buildPtyEnv', () => {
  ...
  beforeEach(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'manta-mimocode-userdata-'))
    ...
    mimocodeHome = mkdtempSync(join(tmpdir(), 'manta-mimocode-home-'))
    ...
    writeFileSync(join(configDir, 'plugins', 'manta-mimocode-status.js'), 'USER PLUGIN')
  })
  ...
  it('mirrors user config into shared overlay and installs Manta status plugin', () => {
  ...
  it('reuses the overlay home on a second buildPtyEnv call', () => {
    ...
    expect(
      readFileSync(join(overlayHome, 'config', 'plugins', 'manta-mimocode-status.js'), 'utf8')
