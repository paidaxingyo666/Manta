...
describe('extractBucketLeafCandidates', () => {
  ...
  it('does not treat the parent directory as a leaf when the workspace name is numeric', () => {
    ...
    expect(extractBucketLeafCandidates('-w-workspaces-manta-7474', ['-w-workspaces-manta'])).toEqual([
      '7474'
    ])
  })
...
describe('discoverRetiredWorktreeNames', () => {
  ...
  it('calls a machine with no agent state a complete answer, not a hole to rescan forever', async () => {
    ...
    const retired = await discoverRetiredWorktreeNames({
      workspaceRoots: [join(tmpdir(), 'manta-retirement-absent-root')],
      home: join(tmpdir(), 'manta-retirement-absent-home'),
      env: {}
    })
  ...
  it('ignores buckets belonging to a sibling root with the same prefix', async () => {
    await withFakeHome(
      [`-Users-ada-manta-workspaces-mantadyne-${FIRST}`, `-Users-ada-manta-workspaces-manta-${SECOND}`],
      async (home) => {
        const retired = await discoverRetiredWorktreeNames({
          workspaceRoots: ['/Users/ada/manta/workspaces/manta'],
          home,
  ...
  it("matches a WSL workspace's bucket inside the distro, keyed by its Linux cwd", async () => {
    ...
    const distroHome = await mkdtemp(join(tmpdir(), 'manta-retirement-distro-'))
    await withFakeHome([], async (home) => {
      try {
        await mkdir(join(distroHome, '.claude', 'projects', `-home-ada-manta-workspaces-${FIRST}`), {
          recursive: true
        })
        const retired = await discoverRetiredWorktreeNames({
          workspaceRoots: ['\\\\wsl.localhost\\Ubuntu\\home\\ada\\manta\\workspaces'],
          home,
          env: {},
  ...
  it('still reads the Windows-side home for a WSL root when the distro home cannot be resolved', async () => {
    ...
    await withFakeHome(
      [`--wsl-localhost-Ubuntu-home-ada-manta-workspaces-${FIRST}`],
      async (home) => {
        const retired = await discoverRetiredWorktreeNames({
          workspaceRoots: ['\\\\wsl.localhost\\Ubuntu\\home\\ada\\manta\\workspaces'],
          home,
