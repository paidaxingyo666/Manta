...
export const NO_DISPATCH_HOST = 'No Manta window was available to launch the automation.'

/** A record the tick could not evaluate at all — its schedule no longer resolves (#16303). */
export const UNEVALUABLE_SCHEDULE =
  'Manta could not evaluate this automation and skipped the occurrence.'
...
export function recordMissedRun(input: {
  ...
  input.runs.updateRun({
    ...
    workspaceId: input.automation.workspaceId,
    error: 'This run was past its missed-run grace window when Manta next checked.'
  })
}
