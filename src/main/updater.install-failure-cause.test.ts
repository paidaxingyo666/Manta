...
const DEB_ELEVATION_ERROR =
  'Error: Command failed: /usr/bin/pkexec --disable-internal-agent "/bin/bash" "-c" "dpkg -i \'/home/u/.cache/manta-updater/pending/manta-ide_1.4.163_amd64.deb\'"\npkexec must be setuid root'
...
const WINDOWS_SIGNATURE_MISMATCH_ERROR =
  'New version 1.4.163 is not signed by the application owner: publisherNames: Manta, Inc.'
 ...
 * re-dispatches it through the 'error' event *synchronously* inside `quitAndInstall()`. Manta
...
describe('quitAndInstall failure carries the updater cause', () => {
  ...
  it('redacts the cause the same way the retained-package card does', async () => {
    ...
    expect(message).toContain("<home>/.cache/manta-updater/pending/manta.deb'")
  })
