...
export const MAIN_THREAD_DIAGNOSTICS_ENV = 'MANTA_MAIN_THREAD_DIAGNOSTICS'
...
export function classifySubprocessCommand(command: string, args: readonly string[]): string {
  ...
  const rest = [...args]
  if (binary === 'wsl') {
    // Manta spawns guest commands with `--exec`; `--`/`-e` still appear on
    // wsl.exe processes started outside Manta, so unwrap either separator.
    while (rest.length > 0) {
 ...
 * No-op unless MANTA_MAIN_THREAD_DIAGNOSTICS=1.
 */
