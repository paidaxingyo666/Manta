...
describe('startLocalBuildFeed', () => {
  it('serves only tokenized manifest and validated artifact routes', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'manta-local-feed-'))
    const artifactPath = join(directory, 'manta-macos-arm64.zip')
    ...
    const candidate = {
      ...
      artifacts: new Map([['manta-macos-arm64.zip', { file: artifactFile, size: 3 }]]),
