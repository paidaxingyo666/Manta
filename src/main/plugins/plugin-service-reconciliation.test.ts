...
const pluginKey = 'manta-samples.demo'
...
async function pluginRoot(pluginManifest = manifest()): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'manta-plugin-reconcile-'))
  roots.push(root)
  await writeFile(join(root, 'manta-plugin.json'), JSON.stringify(pluginManifest))
  ...
  const service = new PluginService({
    ...
    getPluginKillListEntry: (key) =>
      killed && key === pluginKey
        ? { pluginKey, reason: 'Security incident', advisoryUrl: 'https://manta.example/advisory' }
        : null,
...
describe('PluginService worker reconciliation', () => {
  it('blocks every runtime surface until a saved override resolves a content conflict', async () => {
    ...
    const service = new PluginService({
      ...
      getDisabledPlugins: () => [],
      getPluginConsents: () => ({
        'manta-samples.first': fingerprintPluginConsent(firstManifest, firstHash.hash),
        'manta-samples.second': fingerprintPluginConsent(secondManifest, secondHash.hash)
      }),
    ...
    expect(service.activationError('manta-samples.first')).toContain('conflicts')
    expect(service.getGrantedCapabilities('manta-samples.first')).toBeNull()
    await expect(service.invokeCommand('manta-samples.first', 'run')).rejects.toThrow('not enabled')
    await expect(service.panels.readEntry('manta-samples.first', 'panel')).resolves.toBeNull()
    ...
    keybindings = { 'plugin:manta-samples.first/run': ['Mod+Shift+T'] }
    await service.reconcileActivationState()

    expect(service.activationError('manta-samples.first')).toBeNull()
    await expect(service.panels.readEntry('manta-samples.first', 'panel')).resolves.toMatchObject({
      html: expect.stringContaining('<h1>Panel</h1>')
    })
    await expect(service.invokeCommand('manta-samples.first', 'run')).resolves.toBeNull()
