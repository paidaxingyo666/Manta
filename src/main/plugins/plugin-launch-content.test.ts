...
describe('Phase 1 launch plugin content', () => {
  ...
  it('publishes every bundled pack only when its release hash matches exact bytes', async () => {
    const userDataPath = await mkdtemp(join(tmpdir(), 'manta-launch-content-'))
  ...
  it('boots release-indexed content from the packaged resources layout', async () => {
    const resourcesPath = await mkdtemp(join(tmpdir(), 'manta-packaged-resources-'))
    const userDataPath = await mkdtemp(join(tmpdir(), 'manta-packaged-user-data-'))
