...
const savedEnvironment = {
  ...
  MANTA_TEST_SSH_REPOSITORIES: process.env.MANTA_TEST_SSH_REPOSITORIES
}
...
describe('private Git marketplace integration', () => {
  it('uses the caller SSH environment for marketplace preview and install', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-private-marketplace-'))
    ...
    const pluginRepository = await createGitRepository(root, 'locale-source', {
      'manta-plugin.json': JSON.stringify({
        manifestVersion: 1,
        id: 'private-locale',
        publisher: 'private',
        name: 'Private Locale',
    ...
    const marketplaceRepository = await createGitRepository(root, 'marketplace-source', {
      'manta-marketplace.json': JSON.stringify({
    ...
    process.env.MANTA_TEST_SSH_REPOSITORIES = JSON.stringify({
