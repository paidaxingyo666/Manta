...
describe('writePluginFileAtomically', () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'manta-plugin-atomic-'))
  })
...
describe('renamePluginFileWithWindowsRetry', () => {
  it('renames when the source exists', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'manta-plugin-rename-'))
    try {
  ...
  describe('on Windows', () => {
    ...
    beforeEach(async () => {
      dir = await mkdtemp(join(tmpdir(), 'manta-plugin-rename-win-'))
  ...
  it('does not retry off Windows', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'manta-plugin-rename-posix-'))
