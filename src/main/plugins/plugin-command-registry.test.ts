...
describe('PluginCommandRegistry', () => {
  ...
  it('preserves the last conflicting spelling for repeated owners and chord groups', () => {
    ...
    registry.reconcile(
      ...
      () => true,
      {
        'plugin:manta-samples.repeat/one': ['Mod+Alt+T', 'Mod+Alt+Y'],
        'plugin:manta-samples.repeat/two': ['Ctrl+Alt+T', 'Ctrl+Alt+Y']
      },
      'linux'
    )
    expect(registry.list()).toEqual([])
  ...
  it('uses saved effective bindings to recover conflicting plugins', () => {
    ...
    registry.reconcile(
      [first, second],
      () => true,
      { 'plugin:manta-samples.first/tasks': ['Mod+Shift+T'] },
      'linux'
    )
  ...
  it('rejects conflicting saved bindings within one plugin', () => {
    ...
    registry.reconcile(
      ...
      () => true,
      {
        'plugin:manta-samples.aliases/tasks': ['Mod+Alt+T'],
        'plugin:manta-samples.aliases/sidebar': ['Mod+Alt+T']
      },
      'linux'
    )
