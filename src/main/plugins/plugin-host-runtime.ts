...
/** API surface handed to a plugin's `activate(manta)` export. Everything is
 *  EXPERIMENTAL until pluginApi v1 freezes. */
export type PluginWorkerMantaApi = {
...
export function createPluginWorkerRuntime(
  ...
  async function handleInit(input: {
    ...
    const manta: PluginWorkerMantaApi = {
      commands: {
