...
describe('quoteForPosixShell', () => {
  it('wraps plain values in single quotes', async () => {
    ...
    expect(quoteForPosixShell('/tmp/manta.deb')).toBe("'/tmp/manta.deb'")
  })

  it('quotes spaces', async () => {
    const { quoteForPosixShell } = await loadCommandModule()
    expect(quoteForPosixShell('/tmp/Manta Setup.deb')).toBe("'/tmp/Manta Setup.deb'")
  })
...
describe('buildLinuxPackageInstallCommand', () => {
  it('refuses a non-absolute package path before looking for anything', async () => {
    ...
    for (const packagePath of ['manta.deb', './manta.deb', '--force-all', '-i']) {
  ...
  it('reports no-sudo when sudo is absent', async () => {
    install('/usr/bin/apt')
    const { buildLinuxPackageInstallCommand } = await loadCommandModule()
    expect(buildLinuxPackageInstallCommand('deb', '/tmp/manta.deb')).toEqual({
      ok: false,
      reason: 'no-sudo'
    })
  })
  ...
  it('builds an apt command', async () => {
    ...
    expect(buildLinuxPackageInstallCommand('deb', '/tmp/manta.deb')).toEqual({
      ok: true,
      command: "/usr/bin/sudo /usr/bin/apt install -- '/tmp/manta.deb'"
    })
  })
  ...
  it('falls back to dpkg', async () => {
    ...
    expect(buildLinuxPackageInstallCommand('deb', '/tmp/manta.deb')).toEqual({
      ok: true,
      command: "/usr/bin/sudo /usr/bin/dpkg -i -- '/tmp/manta.deb'"
    })
  })
  ...
  it('builds a zypper command', async () => {
    ...
    install('/usr/bin/rpm')
    const { buildLinuxPackageInstallCommand } = await loadCommandModule()
    expect(buildLinuxPackageInstallCommand('rpm', '/tmp/manta.rpm')).toEqual({
      ok: true,
      command:
        "/usr/bin/sudo /usr/bin/zypper --no-refresh install --allow-unsigned-rpm -f '/tmp/manta.rpm'"
    })
  })
  ...
  it('prefers dnf over yum and rpm', async () => {
    ...
    expect(buildLinuxPackageInstallCommand('rpm', '/tmp/manta.rpm')).toEqual({
      ok: true,
      command: "/usr/bin/sudo /usr/bin/dnf install --nogpgcheck '/tmp/manta.rpm'"
    })
  })
  ...
  it('falls back to yum', async () => {
    ...
    expect(buildLinuxPackageInstallCommand('rpm', '/tmp/manta.rpm')).toEqual({
      ok: true,
      command: "/usr/bin/sudo /usr/bin/yum install --nogpgcheck '/tmp/manta.rpm'"
    })
  })
  ...
  it('falls back to rpm', async () => {
    ...
    expect(buildLinuxPackageInstallCommand('rpm', '/tmp/manta.rpm')).toEqual({
      ok: true,
      command: "/usr/bin/sudo /sbin/rpm -Uvh '/tmp/manta.rpm'"
    })
  })
  ...
  it('never adds an unattended-confirmation flag', async () => {
    ...
    install('/usr/bin/apt')
    const { buildLinuxPackageInstallCommand } = await loadCommandModule()
    const result = buildLinuxPackageInstallCommand('deb', '/tmp/manta.deb')
