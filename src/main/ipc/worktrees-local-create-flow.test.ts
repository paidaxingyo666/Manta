...
describe('registerWorktreeHandlers', () => {
  ...
  it('uses a repo-specific worktree base path when creating local worktrees', async () => {
    ...
    expect(store.setWorktreeMeta).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        mantaCreationWorkspaceLayout: { path: '../worktrees', nestWorkspaces: false }
      })
    )
  })
  ...
  it('spawns a startup terminal and setup terminal after local worktree registration', async () => {
    ...
    expect(runtimeStub.createTerminal).toHaveBeenNthCalledWith(
      ...
      {
        title: 'Setup',
        command: expect.stringContaining('bash /workspace/repo/.git/manta/setup-runner.sh'),
        env: {
    ...
    expect(setupCommand).toBe('bash /workspace/repo/.git/manta/setup-runner.sh')
  ...
  it('returns the wrapped setup command when startup spawned but setup creation failed', async () => {
    ...
    createSetupRunnerScriptMock.mockReturnValueOnce({
      runnerScriptPath: 'C:\\workspace\\repo\\.git\\manta\\setup-runner.sh',
      shell: { family: 'posix', executable: 'wsl.exe' },
      envVars: {
        MANTA_ROOT_PATH: 'C:\\workspace\\repo',
        MANTA_WORKTREE_PATH: 'C:\\workspace\\improve-dashboard'
      },
    ...
    expect(result.setup).toEqual(
      expect.objectContaining({
        runnerScriptPath: 'C:\\workspace\\repo\\.git\\manta\\setup-runner.sh',
        command: expect.stringContaining('bash /mnt/c/workspace/repo/.git/manta/setup-runner.sh')
      })
    )
