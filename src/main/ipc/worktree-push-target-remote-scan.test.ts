...
const SSH_FORK = 'git@github.com:contributor/manta.git'
const HTTPS_FORK = 'https://github.com/contributor/manta.git'
const GITLAB_FORK = 'https://gitlab.com/contributor/manta.git'
...
const fiftyEightRemotes: RemoteRow[] = [
  ...
  ...Array.from({ length: 56 }, (_, index) => ({
    name: `pr-user${index}-manta`,
    fetchUrl: `https://github.com/user${index}/manta.git`
  })),
  { name: 'pr-contributor-manta', fetchUrl: SSH_FORK }
]
...
const matrix: { name: string; remotes: RemoteRow[]; lookupUrl: string }[] = [
  ...
  },
  {
    ...
    remotes: [{ name: 'gitlab-fork', fetchUrl: GITLAB_FORK }],
    lookupUrl: 'https://bitbucket.org/contributor/manta.git'
  }
]
