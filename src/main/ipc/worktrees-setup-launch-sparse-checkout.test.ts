...
describe('registerWorktreeHandlers', () => {
  ...
  it('returns a setup launch payload when setup should run', async () => {
    ...
    expect(result).toMatchObject({
      ...
      setup: {
        runnerScriptPath: '/workspace/repo/.git/manta/setup-runner.sh',
        envVars: {
  ...
  it('launches setup even when primary and worktree manta.yaml scripts diverge', async () => {
    // Why: benign manta.yaml divergence must not disable setup (regression from #1280 content-equality gate); repo trust already gates execution.
    listWorktreesMock.mockResolvedValue(createdWorktreeList)
