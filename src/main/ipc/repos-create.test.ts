/**
 * Unit tests for repos:create (manta#763).
 *
...
describe('repos:create', () => {
  ...
  const defaultProjectParent = join('/Users/alice', 'manta', 'projects')
  ...
  // ── create-project default parent (manta#14767) ────────────────────
  ...
  it.each([undefined, '', '   '])(
    'falls back to ~/manta/projects for a blank workspace directory: %p',
    async (workspaceDir) => {
  ...
  it('ignores a Windows seeded workspace directory regardless of drive-letter case', async () => {
    ...
    mockStore.getSettings.mockReturnValue({
      workspaceDir: 'c:\\users\\alice\\manta\\workspaces'
    })
    await expect(callDefaultCreateProjectParent()).resolves.toBe(
      join('C:\\Users\\alice', 'manta', 'projects')
    )
  })
