...
describe('detectWslCommandsOnPath', () => {
  ...
  it('parses detected commands from prefixed stdout', async () => {
    runWslProcessMock.mockResolvedValue({
      ...
      code: 0,
      stdout:
        '__MANTA_AGENT_PATH__claude\t/usr/bin/claude\n' +
        '__MANTA_AGENT_PATH__codex\t/home/user/.local/bin/codex\n',
      stderr: '',
      timedOut: false
    })
  ...
  it('ignores commands whose resolved path is not absolute', async () => {
    runWslProcessMock.mockResolvedValue({
      environmentResolved: true,
      code: 0,
      stdout: '__MANTA_AGENT_PATH__claude\tclaude\n' + '__MANTA_AGENT_PATH__codex\tC:\\spoof\n',
      stderr: '',
      timedOut: false
    })
  ...
  it('finds a real command even with rc/motd-shaped noise ahead of it in stdout (regression)', async () => {
    ...
    runWslProcessMock.mockResolvedValue({
      ...
      stdout:
        ...
        '__MANTA_AGENT_PATH__claude\t/home/user/.nvm/versions/node/v20/bin/claude\n',
      stderr: '',
      timedOut: false
    })
