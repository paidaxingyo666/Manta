...
import type { GlobalSettings, MantaWorkspaceLayout } from '../../shared/global-settings-types'
 ...
 * Windows workspace layout inside ~/manta/workspaces on the WSL filesystem
 * (e.g. \\wsl.localhost\Ubuntu\home\user\manta\workspaces\repo\feature).
 */
...
async function computeWorkspaceRootAsync(
  ...
  if (distro && shouldMirrorWorkspaceDirInsideWsl(repoPath, settings.workspaceDir)) {
    ...
    if (wslHome) {
      return win32.join(wslHome, 'manta', 'workspaces')
    }
  }
