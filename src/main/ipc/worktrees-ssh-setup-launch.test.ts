...
describe('registerWorktreeHandlers', () => {
  ...
  it('reads remote manta.yaml and returns a setup launch payload during SSH create', async () => {
    const repo = {
      id: 'repo-ssh',
      path: '/remote/repo',
      displayName: 'ssh',
    ...
    const provider = {
      exec: vi.fn().mockImplementation(async (args: string[]) => {
        ...
        if (args[0] === 'rev-parse' && args[1] === '--git-path') {
          return {
            stdout: '/remote/repo/.git/worktrees/repo-improve-dashboard/manta/setup-runner.sh\n',
            stderr: ''
          }
        }
        if (args[0] === 'rev-parse') {
    ...
    expect(fsProvider.readFile).toHaveBeenCalledWith('/remote/repo-improve-dashboard/manta.yaml')
    expect(provider.exec).toHaveBeenCalledWith(
      ['rev-parse', '--git-path', 'manta/setup-runner.sh'],
      '/remote/repo-improve-dashboard'
    )
    expect(fsProvider.createDir).toHaveBeenCalledWith(
      '/remote/repo/.git/worktrees/repo-improve-dashboard/manta'
    )
    expect(fsProvider.writeFile).toHaveBeenCalledWith(
      '/remote/repo/.git/worktrees/repo-improve-dashboard/manta/setup-runner.sh',
      '#!/usr/bin/env bash\nset -e\npnpm install\n'
    )
  ...
  it('keeps Windows SSH setup runners independent from the local Git Bash setting', async () => {
    ...
    const provider = {
      exec: vi.fn().mockImplementation(async (args: string[]) => {
        ...
        if (args[0] === 'rev-parse' && args[1] === '--git-path') {
          return {
            stdout:
              'C:\\remote\\repo\\.git\\worktrees\\improve-dashboard\\manta\\setup-runner.cmd\n',
            stderr: ''
          }
        }
        if (args[0] === 'rev-parse') {
    ...
    expect(provider.exec).toHaveBeenCalledWith(
      ['rev-parse', '--git-path', 'manta/setup-runner.cmd'],
      ...
      expect.objectContaining({
        setup: {
          runnerScriptPath:
            'C:\\remote\\repo\\.git\\worktrees\\improve-dashboard\\manta\\setup-runner.cmd',
          envVars: expect.objectContaining({
            MANTA_ROOT_PATH: 'C:\\remote\\repo',
            MANTA_WORKTREE_PATH: 'C:\\remote\\improve-dashboard'
          })
        }
      })
    )
