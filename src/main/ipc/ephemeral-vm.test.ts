...
describe('registerEphemeralVmHandlers', () => {
  ...
  beforeEach(() => {
    ...
    connectRuntimeOwnedSshTargetMock.mockResolvedValue({
      targetId: 'runtime-ssh-manta-instance-1',
      target: {
        id: 'runtime-ssh-manta-instance-1',
        label: 'Sandbox',
  ...
  it('lists recipes from local repo manta.yaml', async () => {
  ...
  it('provisions an ssh recipe without creating a runtime environment', async () => {
    ...
    expect(result).toMatchObject({
      ok: true,
      connectionType: 'ssh',
      sshTargetId: 'runtime-ssh-manta-instance-1',
      runtime: {
        repoId: 'repo-1',
        status: 'running',
        connectionMode: 'ssh',
  ...
  it('retains the runtime-owned SSH target when destroy fails', async () => {
    ...
    expect(cleaned.sshTargetId).toBe('runtime-ssh-manta-instance-1')
  })
    ...
    upsertEphemeralVmRuntime(userDataPath, {
      ...
      recipeResult: {
        ...
        connection: {
          type: 'ssh',
          projectRoot: '/workspace/repo',
          target: { label: 'VM', host: 'host', port: 22, username: 'manta' }
        }
      }
    })
    registerEphemeralVmHandlers(makeStore(repoPath) as never)
