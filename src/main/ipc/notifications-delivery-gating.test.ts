...
describe('registerNotificationHandlers', () => {
  ...
  it('opens the current macOS app notification settings entry', async () => {
    ...
    const originalBundleId = process.env.MANTA_DEV_MACOS_BUNDLE_ID
    Object.defineProperty(process, 'platform', { value: 'darwin', configurable: true })
    process.env.MANTA_DEV_MACOS_BUNDLE_ID = 'cn.sh.manta.dev.fb5a47066f08'
    try {
      registerNotificationHandlers({
        getSettings: () => ({
          notifications: {
      ...
      expect(shellOpenExternalMock).toHaveBeenCalledWith(
        'x-apple.systempreferences:com.apple.Notifications-Settings.extension?id=cn.sh.manta.dev.fb5a47066f08'
      )
    } finally {
      Object.defineProperty(process, 'platform', { value: originalPlatform, configurable: true })
      if (originalBundleId === undefined) {
        delete process.env.MANTA_DEV_MACOS_BUNDLE_ID
      } else {
        process.env.MANTA_DEV_MACOS_BUNDLE_ID = originalBundleId
      }
    }
  })
  ...
  it('suppresses active-worktree notifications while Manta is focused', async () => {
