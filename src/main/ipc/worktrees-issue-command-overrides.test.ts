...
describe('registerWorktreeHandlers', () => {
  ...
  it.each(['command', ' '])(
    ...
    async (content) => {
      ...
      mkdirSync(join(root, '.manta'))
      writeFileSync(join(root, '.manta', 'issue-command'), 'old command\n')
      ...
      try {
        ...
        if (content.trim()) {
          expect(readFileSync(join(root, '.manta', 'issue-command'), 'utf8')).toBe('command\n')
          expect(readFileSync(join(root, '.gitignore'), 'utf8')).toBe('.manta\n')
        } else {
          expect(existsSync(join(root, '.manta', 'issue-command'))).toBe(false)
  ...
  it('creates an issue-command runner for an existing repo/worktree pair', async () => {
    ...
    expect(result).toMatchObject({
      runnerScriptPath: '/workspace/repo/.git/manta/issue-command-runner.sh',
      envVars: {
        MANTA_ROOT_PATH: '/workspace/repo',
        MANTA_WORKTREE_PATH: '/workspace/improve-dashboard'
      }
    })
  })
  ...
  it('keeps SSH issue-command local overrides usable when shared read fails', async () => {
    ...
    const fsProvider = {
      readFile: vi.fn(async (filePath: string) => {
        if (filePath.endsWith('/.manta/issue-command')) {
  ...
  it('reads an issue-command override from the requested host when repo ids collide', async () => {
    ...
    expect(fsProvider.readFile).toHaveBeenCalledWith('/remote/repo/.manta/issue-command')
  })
  ...
  it('creates remote .gitignore only when it is missing while writing SSH issue commands', async () => {
    ...
    expect(fsProvider.writeFile).toHaveBeenNthCalledWith(1, '/remote/repo/.gitignore', '.manta\n')
    expect(fsProvider.writeFile).toHaveBeenNthCalledWith(
      2,
      '/remote/repo/.manta/issue-command',
      'manta issue command\n'
    )
  })
  ...
  it('preserves .gitignore when the SSH host already ignores .manta', async () => {
    ...
    const checkIgnoredPaths = vi.fn().mockResolvedValue(['.manta/issue-command'])
    ...
    expect(checkIgnoredPaths).toHaveBeenCalledWith('/remote/repo', ['.manta/issue-command'])
