...
describe('preflight', () => {
  ...
  it('does not report Claude Agent Teams when only the Manta shim is present', async () => {
    execFileAsyncMock.mockImplementation(async (command, args) => {
      ...
      if (String(args[0]) === 'manta') {
        return { stdout: '/Applications/Manta.app/Contents/MacOS/manta\n' }
      }
      throw new Error('not found')
    })
  ...
  it('does not report Claude Agent Teams on native Windows', async () => {
    ...
    execFileAsyncMock.mockImplementation(async (command, args) => {
      ...
      if (String(args[0]) === 'manta') {
        return { stdout: '/mock/windows/programs/manta.cmd\n' }
      }
      throw new Error('not found')
    })
  ...
  it('does not run host shell hydration for WSL agent detection', async () => {
    ...
    execFileAsyncMock.mockImplementation(async (command, args) => {
      ...
      if (script.includes("'claude'")) {
        return { stdout: '__MANTA_AGENT_PATH__claude\t/home/test/.local/bin/claude\n' }
      }
      throw new Error('not found')
    })
  ...
  it('detects agents from the default WSL distro when requested', async () => {
    ...
    execFileAsyncMock.mockImplementation(async (command, args) => {
      ...
      if (script.includes("'codex'")) {
        return { stdout: '__MANTA_AGENT_PATH__codex\t/home/test/.local/bin/codex\n' }
      }
      throw new Error('not found')
    })
