...
describe('preflight', () => {
  ...
  it('does not report Claude Agent Teams when only the Manta shim is present', async () => {
    execFileAsyncMock.mockImplementation(async (command, args) => {
      if (command !== 'which') {
        throw new Error(`unexpected command ${String(command)}`)
      }
      if (String(args[0]) === 'manta') {
        return { environmentResolved: true, code: 0, stdout: '/Applications/Manta.app/Contents/MacOS/manta\n', stderr: '', timedOut: false }
      }
      throw new Error('not found')
    })
  ...
  it('reports Claude Agent Teams when both Manta and Claude are present', async () => {
    execFileAsyncMock.mockImplementation(async (command, args) => {
      if (command !== 'which') {
        throw new Error(`unexpected command ${String(command)}`)
      }
  ...
  it('does not report Claude Agent Teams on native Windows', async () => {
    ...
    execFileAsyncMock.mockImplementation(async (command, args) => {
      ...
      if (String(args[0]) === 'manta') {
        return { environmentResolved: true, code: 0, stdout: '/mock/windows/programs/manta.cmd\n', stderr: '', timedOut: false }
      }
      throw new Error('not found')
    })
  ...
  it('does not run host shell hydration for WSL agent detection', async () => {
    ...
    runWslProcessMock.mockImplementation(async ({ script }: { script: string }) => {
      if (script.includes("'claude'")) {
        return { environmentResolved: true, code: 0, stdout: '__MANTA_AGENT_PATH__claude\t/home/test/.local/bin/claude\n', stderr: '', timedOut: false }
      }
      throw new Error('not found')
    })
  ...
  it('does not report Claude Agent Teams from WSL agent detection', async () => {
    ...
    runWslProcessMock.mockImplementation(async ({ script }: { script: string }) => {
      expect(script).not.toContain("'manta'")
      expect(script).not.toContain("'manta-dev'")
      expect(script).not.toContain("'manta-ide'")
      if (script.includes("'claude'")) {
        return { environmentResolved: true, code: 0, stdout: '__MANTA_AGENT_PATH__claude\t/home/test/.local/bin/claude\n', stderr: '', timedOut: false }
      }
      throw new Error('not found')
    ...
    runWslProcessMock.mockImplementation(async ({ script }: { script: string }) => {
      if (script.includes("'codex'")) {
        return {
          ...
          code: 0,
          stdout: '__MANTA_AGENT_PATH__codex\t/home/test/.local/bin/codex\n',
          stderr: '',
          timedOut: false
        }
      }
