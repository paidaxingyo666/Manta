...
describe('registerPtyHandlers', () => {
  ...
  describe('spawn environment', () => {
    describe('daemon-active provider (parity with LocalPtyProvider)', () => {
      ...
      it('resolves default WSL authority before daemon host env and spawn metadata', async () => {
        await withWin32Platform(async () => {
          ...
          expect(spawnOptions.env).toMatchObject({
            ...
            MANTA_ORCHESTRATION_COMPATIBILITY_HOST_ID: 'compat-host',
      ...
      it('keeps the Agent Teams tmux shim ahead of host PATH shims for runtime-created daemon PTYs', async () => {
        ...
        try {
          await controller.spawn({
            ...
            command: 'claude',
            env: {
              PATH: `/tmp/manta-agent-teams-bin${delimiter}/usr/bin`,
        ...
        expect(spawnedPath[0]).toBe('/tmp/manta-agent-teams-bin')
      ...
      it('injects dev-mode MANTA_USER_DATA_PATH + dev CLI PATH on the daemon path', async () => {
        ...
        try {
          ...
          expect(env.PATH).toContain(join('/tmp/manta-user-data', 'cli', 'bin'))
        } finally {
          mockedApp.isPackaged = prev
        }
      })
      it('preserves the inherited PATH when dev-mode daemon env omits PATH', async () => {
        ...
        try {
          ...
          expect(env.PATH).toContain(
            `${join('/tmp/manta-user-data', 'cli', 'bin')}${delimiter}/system/bin`
          )
        } finally {
          mockedApp.isPackaged = prev
        }
      ...
      it('drops a legacy shim PATH entry inherited from the host process on the daemon path', async () => {
        ...
        try {
          const env = await daemonSpawnAndGetEnv({}, undefined, undefined, {
            PATH: `/tmp/manta-user-data/manta-terminal-attribution/posix${delimiter}/system/bin`
          })
          expect(env.PATH).not.toContain('manta-terminal-attribution')
