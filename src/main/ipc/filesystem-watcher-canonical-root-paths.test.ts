...
describe('local filesystem watcher canonical root paths', () => {
  ...
  it('reports symlink-resolved event paths under the subscribed root', async () => {
    const worktreePath = resolve('/tmp/manta-link')
    const payload = await emitAndCapture(worktreePath, resolve('/private/tmp/manta-real'), [
      { path: resolve('/private/tmp/manta-real/src/agent-edit.ts'), type: 'update' }
    ])
    ...
    expect(payload.events).toEqual([
      {
        kind: 'update',
        absolutePath: resolve('/tmp/manta-link/src/agent-edit.ts'),
        isDirectory: false
      }
    ])
  })
  ...
  it.runIf(process.platform === 'darwin' || process.platform === 'win32')(
    ...
    async () => {
      const worktreePath = resolve('/tmp/manta-case/repo')
  ...
  it('leaves already-matching event paths untouched', async () => {
    const worktreePath = resolve('/tmp/manta-plain')
    const eventPath = resolve('/tmp/manta-plain/src/agent-edit.ts')
    const payload = await emitAndCapture(worktreePath, worktreePath, [
