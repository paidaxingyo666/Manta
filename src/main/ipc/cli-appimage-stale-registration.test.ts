...
function status(
  ...
  launcherPath = '/cache/current/resources/bin/manta-ide'
): CliInstallStatus {
  return {
    platform: 'linux',
    commandName: 'manta-ide',
    commandPath: '/home/me/.local/bin/manta-ide',
    ...
    supported: true,
    state,
    currentTarget: state === 'not_installed' ? null : '/cache/old/resources/bin/manta-ide',
...
beforeEach(() => {
  ...
  mocks.resolveAppImageRuntimeIdentity.mockReset().mockImplementation(() =>
    process.platform === 'linux'
      ? {
          appImagePath: '/opt/Manta.AppImage',
          appDirPath: '/tmp/.mount_Orca123'
        }
      : null
  )
  Object.defineProperty(process, 'platform', { configurable: true, value: 'linux' })
  vi.stubEnv('APPIMAGE', '/opt/Manta.AppImage')
})
...
describe('AppImage CLI registration startup repair', () => {
  ...
  it('repairs a later AppImage generation after an earlier repair succeeds', async () => {
    const firstStale = status('stale', '/cache/launcher/manta-ide')
    const firstInstalled = status('installed', '/cache/launcher/manta-ide')
    const nextStale = status('stale', '/cache/launcher/manta-ide')
    const nextInstalled = status('installed', '/cache/launcher/manta-ide')
    mocks.getStatus
  ...
  it('does not claim a sibling AppImage registration during a status poll', async () => {
    const stale = {
      ...status('stale'),
      currentTarget: '/cache/current/resources/bin/manta-ide'
    }
    mocks.getStatus.mockResolvedValue(stale)
