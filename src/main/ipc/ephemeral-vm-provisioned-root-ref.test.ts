...
beforeEach(() => {
  handlers.clear()
  userDataPath = mkdtempSync(join(tmpdir(), 'manta-vm-ref-user-data-'))
  repoPath = mkdtempSync(join(tmpdir(), 'manta-vm-ref-repo-'))
...
function writeRecipe(path: string, createCountPath: string, createEnvPath?: string): void {
  ...
  writeFileSync(
    ...
    [
      ...
      ...(createEnvPath
        ? [
            `require('node:fs').writeFileSync(${JSON.stringify(createEnvPath)}, JSON.stringify({ref:process.env.MANTA_REPO_REF,refHead:process.env.MANTA_REPO_REF_HEAD,repoUrl:process.env.MANTA_REPO_URL}))`
          ]
        : []),
