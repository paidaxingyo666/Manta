...
describe('registerFilesystemHandlers', () => {
  ...
  it('prepares the Manta-managed Codex home for the default system selection', async () => {
    const context = {
      branch: 'feature/ai',
      stagedSummary: 'M\tREADME.md',
      stagedPatch: '+hello'
    ...
    registerFilesystemHandlers(store as never, {
      prepareForCodexLaunch: () => '/manta-managed/codex-home'
    })
    ...
    expect(generateCommitMessageFromContextMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        kind: 'local',
        cwd: WORKTREE_FEATURE_PATH,
        env: expect.objectContaining({ CODEX_HOME: '/manta-managed/codex-home' })
      })
    )
  })
