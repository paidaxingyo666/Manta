    ...
    describe('daemon-active provider (parity with LocalPtyProvider)', () => {
      ...
      it('overrides an unmarked custom home for an authoritative daemon resume', async () => {
        ...
        await handlers.get('pty:spawn')!(null, {
          ...
          envToDelete: ['CODEX_HOME', 'MANTA_CODEX_HOME', 'REMOVE_ME'],
          launchAgent: 'codex',
        ...
        expect(env.MANTA_CODEX_HOME).toBe(systemHome)
      ...
      it('keeps the authoritative home for runtime-created daemon resumes', async () => {
        ...
        expect(spawnOptions.env.MANTA_CODEX_HOME).toBe(systemHome)
        expect(spawnOptions.env.REMOVE_ME).toBeUndefined()
        expect(spawnOptions.envToDelete ?? []).not.toContain('CODEX_HOME')
        expect(spawnOptions.envToDelete ?? []).not.toContain('MANTA_CODEX_HOME')
      ...
      it('skips host Codex home when a daemon-backed Windows spawn targets a WSL cwd', async () => {
        ...
        try {
          const spawnOptions = await daemonSpawnAndGetOptions(
            {},
            () => 'C:\\Users\\test\\AppData\\Roaming\\Manta\\codex-runtime-home\\home',
            undefined,
            {
              CODEX_HOME: 'C:\\Users\\test\\AppData\\Roaming\\Manta\\codex-runtime-home\\home',
              MANTA_CODEX_HOME: 'C:\\Users\\test\\AppData\\Roaming\\Manta\\codex-runtime-home\\home'
            },
            {
          ...
          expect(spawnOptions.envToDelete).toEqual(
            expect.arrayContaining(['CODEX_HOME', 'MANTA_CODEX_HOME'])
          )
        } finally {
          Object.defineProperty(process, 'platform', {
            configurable: true,
      ...
      it('skips host Codex home when a daemon-backed Windows spawn uses a WSL shell override', async () => {
        ...
        try {
          ...
          expect(spawnOptions.env.MANTA_CODEX_HOME).toBeUndefined()
          expect(spawnOptions.envToDelete).toEqual(
            expect.arrayContaining(['CODEX_HOME', 'MANTA_CODEX_HOME'])
          )
        } finally {
      ...
      it('does not install or inject a Prime extension for an explicit WSL launch', async () => {
        await withWin32Platform(async () => {
          ...
          expect(env.MANTA_PRIME_AGENT_SOURCE_AGENT_DIR).toBeUndefined()
          expect(env.MANTA_PRIME_AGENT_STATUS_EXTENSION).toBeUndefined()
          expect(env.MANTA_WSL_HOOK_INSTANCE).toBeUndefined()
      ...
      it('points OPENCODE_CONFIG_DIR at the guest overlay when the WSL relay reports it', async () => {
        const guestDir = '/home/jin/.manta-relay/opencode-overlays/abc'
        const spy = vi.spyOn(wslHookRelayManager, 'getOpenCodeOverlayDir').mockReturnValue(guestDir)
        try {
          await withWin32Platform(async () => {
            const env = await daemonSpawnAndGetEnv(
              { MANTA_OPENCODE_SOURCE_CONFIG_DIR: '/home/jin/.config/opencode' },
              undefined,
              undefined,
              undefined,
              { shellOverride: 'wsl.exe' }
            ...
            expect(env.MANTA_OPENCODE_CONFIG_DIR).toBe(guestDir)
      ...
      it('strips the daemon-inherited Manta-owned CODEX_HOME for real-home routing', async () => {
        const spawnOptions = await daemonSpawnAndGetOptions(
          {},
          () => null,
          () => ({ codexSystemDefaultRealHomeEnabled: true }) as never,
          { CODEX_HOME: '/managed/home', MANTA_CODEX_HOME: '/managed/home' }
        )
        expect(spawnOptions.env.CODEX_HOME).toBeUndefined()
        expect(spawnOptions.env.MANTA_CODEX_HOME).toBeUndefined()
        expect(spawnOptions.envToDelete).toEqual(expect.arrayContaining(['MANTA_CODEX_HOME']))
      ...
      it('preserves a daemon-inherited user CODEX_HOME for real-home routing', async () => {
        const spawnOptions = await daemonSpawnAndGetOptions(
          ...
          () => null,
          () => ({ codexSystemDefaultRealHomeEnabled: true }) as never,
          { CODEX_HOME: '/home/me/.config/codex', MANTA_CODEX_HOME: undefined }
        )
        expect(spawnOptions.envToDelete).toEqual(expect.arrayContaining(['MANTA_CODEX_HOME']))
      ...
      it('prepends the bare-manta CLI shim dir to PATH for packaged Linux spawns', async () => {
        const originalPlatform = process.platform
        Object.defineProperty(process, 'platform', {
          configurable: true,
          value: 'linux'
        ...
        try {
          ...
          const shimDir = join('/tmp/manta-user-data', 'linux-manta-cli-shim')
          // Why: bare `manta` must resolve to the Manta CLI before /usr/bin/orca (the GNOME screen reader) in Manta terminals (#7904).
          ...
          expect(env.MANTA_CLI_COMMAND).toBeUndefined()
        } finally {
          Object.defineProperty(process, 'platform', {
            configurable: true,
            value: originalPlatform
      ...
      it('prepends the bundled CLI dir to PATH for packaged macOS spawns', async () => {
        ...
        Object.defineProperty(process, 'resourcesPath', {
          configurable: true,
          value: '/tmp/manta-resources'
        })
        try {
          const env = await daemonSpawnAndGetEnv({ PATH: '/usr/bin' })
          expect(env.PATH.split(delimiter)[0]).toBe(join('/tmp/manta-resources', 'bin'))
        } finally {
      ...
      it('injects the agent-hook receiver env on the daemon path', async () => {
        ...
        expect(env.MANTA_AGENT_HOOK_TOKEN).toBe('agent-token')
      })
      it('deletes stale Claude scoped settings env from daemon-hosted PTYs', async () => {
        ...
        expect(spawnOptions.env.MANTA_CLAUDE_AGENT_STATUS_SETTINGS).toBeUndefined()
        expect(spawnOptions.envToDelete).toEqual(
          expect.arrayContaining(['MANTA_CLAUDE_AGENT_STATUS_SETTINGS'])
        )
        expect(spawnOptions.env.MANTA_AGENT_HOOK_PORT).toBe('5678')
        expect(spawnOptions.env.MANTA_AGENT_HOOK_TOKEN).toBe('agent-token')
      })
      it('asks surviving pre-upgrade daemons to delete legacy attribution env', async () => {
        ...
        expect(spawnOptions.envToDelete).not.toContain('MANTA_REAL_GIT')
        expect(spawnOptions.envToDelete).not.toContain('MANTA_REAL_GH')
      })
      it('deletes stale Claude scoped settings env from runtime-created daemon PTYs', async () => {
        ...
        process.env.MANTA_CLAUDE_AGENT_STATUS_SETTINGS =
          '/tmp/manta/agent-hooks/claude-agent-status-settings.json'
        handlers.clear()
      ...
      it('strips inherited Claude child-session stamps from runtime-created PTYs', async () => {
        // Why: the runtime controller is the `manta` CLI / automation spawn path and
