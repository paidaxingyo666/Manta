...
describe('prepareWorktreePushTargetWithExec', () => {
  ...
  it('disambiguates with a numeric suffix when the preferred remote name is taken by a different URL', async () => {
    const exec = makeRepoExec({ 'pr-contributor-manta': 'git@github.com:someone-else/manta.git' })
...
describe('remoteAlreadyMatchesUrl', () => {
  it('matches an exact URL', async () => {
    const exec = makeRepoExec({ 'pr-contributor-manta': FORK_SSH })
    await expect(
      remoteAlreadyMatchesUrl(exec, REPO, 'pr-contributor-manta', FORK_SSH)
    ).resolves.toBe(true)
  })

  it('matches by GitHub owner/repo across URL protocols', async () => {
    const exec = makeRepoExec({ 'pr-contributor-manta': FORK_HTTPS })
    await expect(
      remoteAlreadyMatchesUrl(exec, REPO, 'pr-contributor-manta', FORK_SSH)
    ).resolves.toBe(true)
  })
  ...
  it('returns false when the named remote points elsewhere', async () => {
    const exec = makeRepoExec({
      'pr-contributor-manta': 'git@github.com:someone-else/manta.git'
    })
    await expect(
      remoteAlreadyMatchesUrl(exec, REPO, 'pr-contributor-manta', FORK_SSH)
    ).resolves.toBe(false)
...
describe('configureCreatedWorktreePushTargetWithExec', () => {
  it('points the new branch upstream at the fork remote', async () => {
    ...
    expect(exec).toHaveBeenCalledWith(
      ['branch', '--set-upstream-to', 'pr-contributor-manta/contributor/fix', 'local-branch'],
      '/wt/path'
    )
