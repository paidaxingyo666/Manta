...
import type {
  ...
  RemoteFetchResult,
  RemoteTrackingBase
} from '../runtime/manta-runtime'
import { getProjectHostSetupWorktreeMeta } from '../../shared/project-host-setup-lookup'
import { getEffectiveHooks, loadHooks, parseMantaYaml } from '../hooks'
...
async function getOrStartSshWorktreeCreateFetch(
  ...
  const promise = enqueueSshWorktreeCreateFetch(queueKey, async () => {
    ...
    await fetch()
    // Why: SSH creation has no MantaRuntimeService to share; still reuse recent fetches for repeated creates on the same target.
...
async function adoptExistingForkRemoteForBranch(
  ...
  // Why: a remote another worktree minted is still Manta-owned. Without stamping ownership on
...
export async function cleanupUnusedWorktreePushTargetRemote(
  ...
  // a preserved branch since deleted, a worktree removed outside Manta) -- see
...
export async function prepareWorktreePushTargetSsh(
  ...
  if (target.remoteUrl) {
    ...
    if (existingRemote) {
      ...
      // Why: a reused Manta-created fork remote must inherit ownership so deleting the final user can remove it.
      remoteCreated = store
        ? isPushTargetRemoteCreatedByKnownWorktree(
            store,
            {
...
async function readRemoteEffectiveHooks(
  ...
  return getEffectiveHooksFromConfig(repo, await readRemoteMantaYaml(fsProvider, hooksRootPath))
}

async function readRemoteMantaYaml(
  fsProvider: IFilesystemProvider,
  hooksRootPath: string
): Promise<ReturnType<typeof parseMantaYaml>> {
  try {
    const result = await fsProvider.readFile(joinWorktreeRelativePath(hooksRootPath, 'manta.yaml'))
    return result.isBinary ? null : parseMantaYaml(result.content)
  } catch {
    return null
  }
}
...
async function createRemoteSetupRunnerScript(
  ...
  const runnerRelativePath = useWindowsFormat ? 'manta/setup-runner.cmd' : 'manta/setup-runner.sh'
...
export async function createRemoteWorktree(
  ...
  const metaUpdates: Partial<WorktreeMeta> = {
    ...
    createdAt: now,
    mantaCreatedAt: now,
    mantaCreationSource: 'ssh',
    creatorProvenance: { kind: 'host' },
    mantaCreationWorkspaceLayout: getWorktreeCreationLayout(repo, settings),
  ...
  // Why: shared/symlink paths, `manta.yaml` shared directories, and `.worktreeinclude` copies are local-only; remote (SSH) support needs a new relay method + auth surface, so all are skipped here.

  let setup: CreateWorktreeResult['setup']
  let defaultTabs: CreateWorktreeResult['defaultTabs']
  if (fsProvider) {
    await timing.time('prepare_setup', async () => {
      const yamlHooks = await readRemoteMantaYaml(fsProvider, created.path)
...
export async function createLocalWorktree(
  ...
  // Why: the worktree's base-branch `manta.yaml` is authoritative; we don't re-gate on content parity with the primary checkout since benign divergence silently disabled setup (#1280).
  let setup: CreateWorktreeResult['setup']
  let defaultTabs: CreateWorktreeResult['defaultTabs']
  await timing.time('prepare_setup', async () => {
