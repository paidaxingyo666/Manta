...
describe('GitLab IPC handlers', () => {
  ...
  it('rejects source context for a different host', async () => {
    registerGitLabHandlers(
      storeWithRepos([repo({ id: 'repo-local', path: '/local/manta' })]) as Store
    )
  ...
  it('routes local WSL project GitLab issue, MR, work-item, and todo IPC through project git options', async () => {
    ...
    await ipcHandlers.get('gitlab:projectSlug')?.(null, { repoPath: '/local/manta' })
    await ipcHandlers.get('gitlab:mrForBranch')?.(null, {
      repoPath: '/local/manta',
      branch: 'feature/wsl'
    })
    await ipcHandlers.get('gitlab:mr')?.(null, { repoPath: '/local/manta', iid: 8 })
    await ipcHandlers.get('gitlab:listMRs')?.(null, {
      repoPath: '/local/manta',
      state: 'opened',
      page: 1,
    ...
    await ipcHandlers.get('gitlab:issue')?.(null, { repoPath: '/local/manta', number: 7 })
    ...
    await ipcHandlers.get('gitlab:listLabels')?.(null, { repoPath: '/local/manta' })
    await ipcHandlers.get('gitlab:listAssignableUsers')?.(null, { repoPath: '/local/manta' })
    await ipcHandlers.get('gitlab:todos')?.(null, { repoPath: '/local/manta' })

    const hostedReviewOptions = { localGitExecOptions: localGitOptions }
    expect(getProjectSlugMock).toHaveBeenCalledWith('/local/manta', null, hostedReviewOptions)
    ...
    expect(getMergeRequestMock).toHaveBeenCalledWith('/local/manta', 8, null, hostedReviewOptions)
    expect(listMergeRequestsMock).toHaveBeenCalledWith(
      '/local/manta',
      'opened',
      1,
    ...
    expect(getIssueMock).toHaveBeenCalledWith('/local/manta', 7, null, localGitOptions)
    ...
    expect(listLabelsMock).toHaveBeenCalledWith('/local/manta', undefined, null, localGitOptions)
    ...
    expect(listTodosMock).toHaveBeenCalledWith('/local/manta', null, localGitOptions)
  })
  ...
  it('routes local WSL project GitLab MR details, review, job, and pasted URL IPC through project git options', async () => {
    ...
    await ipcHandlers.get('gitlab:closeMR')?.(null, { repoPath: '/local/manta', iid: 8 })
    await ipcHandlers.get('gitlab:reopenMR')?.(null, { repoPath: '/local/manta', iid: 8 })
    ...
    await ipcHandlers.get('gitlab:jobTrace')?.(null, { repoPath: '/local/manta', jobId: 99 })
    await ipcHandlers.get('gitlab:retryJob')?.(null, { repoPath: '/local/manta', jobId: 99 })
