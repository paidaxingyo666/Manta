...
beforeEach(async () => {
  ...
  await git(['config', 'user.name', 'Manta Test'], repoPath)
  await git(['config', 'user.email', 'manta@example.test'], repoPath)
  ...
  await git(['config', 'user.name', 'Manta Test'], forkPath)
  await git(['config', 'user.email', 'manta@example.test'], forkPath)
...
describe('reconcileOrphanedPrRemotesWithExec against the real Git binary', () => {
  ...
  it('reclaims a remote orphaned by a worktree removed outside Manta (path 3)', async () => {
    ...
    // Simulate a plain `git worktree remove` the user ran outside Manta: Manta's metadata for
