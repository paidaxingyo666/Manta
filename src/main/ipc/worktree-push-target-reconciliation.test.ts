...
describe('isOrcaGeneratedPrRemoteName', () => {
  it('matches Manta-generated names, including disambiguated ones', () => {
    expect(isOrcaGeneratedPrRemoteName('pr-head')).toBe(true)
    expect(isOrcaGeneratedPrRemoteName('pr-contributor-manta')).toBe(true)
...
describe('reconcileOrphanedPrRemotesWithExec', () => {
  it('leaves a remote alone when no worktree metadata ever proves Manta created it (user-created, ambiguous)', async () => {
  ...
  it('leaves a remote alone that is not shaped like a Manta-generated pr-* remote', async () => {
    const exec = makeExec({
  ...
  it('reclaims a remote left behind by a worktree removed outside Manta (path 3)', async () => {
    const exec = makeExec({ remotes: remoteLines([{ name: FORK_REMOTE, url: FORK_URL }]) })
    // Metadata still records the (now-vanished) worktree's Manta-created pushTarget.
    const reclaimed = await reconcileOrphanedPrRemotesWithExec(
      REPO_PATH,
      REPO_ID,
      storeOf({ [worktreeId('/wt/gone')]: forkTarget() }),
