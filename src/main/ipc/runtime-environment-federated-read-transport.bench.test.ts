...
const runLiveBenchmark = isFederatedReadBenchmarkEnabled(process.env.MANTA_FEDERATED_READ_BENCH)
...
describe.runIf(runLiveBenchmark)('federated read RPC transport benchmark', () => {
  it('compares one-shot and shared-control latency on one saved runtime', async () => {
    const userDataPath = process.env.MANTA_RUNTIME_USER_DATA_PATH
    const environmentName = process.env.MANTA_RUNTIME_ENVIRONMENT
    if (!userDataPath || !environmentName) {
      throw new Error('Set MANTA_RUNTIME_USER_DATA_PATH and MANTA_RUNTIME_ENVIRONMENT.')
    }
    const store = RuntimeEnvironmentStoreSchema.parse(
      JSON.parse(readFileSync(join(userDataPath, 'manta-environments.json'), 'utf8'))
    )
