  ...
  describe('spawn environment', () => {
    describe('daemon-active provider (parity with LocalPtyProvider)', () => {
      ...
      it('mirrors a user-provided OPENCODE_CONFIG_DIR into a source-scoped overlay on the daemon path', async () => {
        ...
        expect(env.MANTA_OPENCODE_SOURCE_CONFIG_DIR).toBe('/user/custom/opencode')
      })
      it('uses source OpenCode config env instead of remirroring a parent overlay', async () => {
        const env = await daemonSpawnAndGetEnv({
          ...
          MANTA_OPENCODE_SOURCE_CONFIG_DIR: '/user/custom/opencode'
        })
        expect(openCodeBuildPtyEnvMock).toHaveBeenCalledWith(
          expect.any(String),
          '/user/custom/opencode'
      ...
      it('installs Pi managed extensions without redirecting homes on the daemon path', async () => {
        ...
        expect(env.MANTA_PI_SOURCE_AGENT_DIR).toBe('/user/.pi/agent')
        expect(env.MANTA_OMP_CODING_AGENT_DIR).toBeUndefined()
        expect(env.MANTA_OMP_STATUS_EXTENSION).toBe(expectedOmpStatusExtension)
      })
      ...
      it('threads command: "omp" through to piBuildPtyEnv on the daemon path with OMP status metadata', async () => {
        ...
        expect(env.MANTA_OMP_STATUS_EXTENSION).toBe(
          '/user/.omp/agent/extensions/manta-agent-status.ts'
        )
        expect(env.MANTA_OMP_SOURCE_AGENT_DIR).toBe('/user/.omp/agent')
        expect(env.MANTA_PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.MANTA_PI_SOURCE_AGENT_DIR).toBeUndefined()
      })
