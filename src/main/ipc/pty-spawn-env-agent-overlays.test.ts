...
describe('registerPtyHandlers', () => {
  ...
  describe('spawn environment', () => {
    ...
    it('injects the OpenCode hook env into Manta terminal PTYs', async () => {
      ...
      expect(env.MANTA_OPENCODE_HOOK_PORT).toBe('4567')
      expect(env.MANTA_OPENCODE_HOOK_TOKEN).toBe('opencode-token')
      expect(env.MANTA_OPENCODE_PTY_ID).toBe('test-pty')
    ...
    it('mirrors the original OpenCode source dir when launched from a Manta overlay shell', async () => {
      const env = await spawnAndGetEnv({
        OPENCODE_CONFIG_DIR: '/tmp/parent-manta-opencode-overlay',
        MANTA_OPENCODE_SOURCE_CONFIG_DIR: '/tmp/user-opencode-config'
      })
      expect(openCodeBuildPtyEnvMock).toHaveBeenCalledWith(
        expect.any(String),
        '/tmp/user-opencode-config'
      )
      expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/manta-opencode-overlay')
      expect(env.MANTA_OPENCODE_CONFIG_DIR).toBe('/tmp/manta-opencode-overlay')
      expect(env.MANTA_OPENCODE_SOURCE_CONFIG_DIR).toBe('/tmp/user-opencode-config')
    })
    it('does not treat inherited Manta OpenCode config as user config without a source dir', async () => {
      const env = await spawnAndGetEnv({
        OPENCODE_CONFIG_DIR: '/tmp/parent-manta-opencode-overlay',
        MANTA_OPENCODE_CONFIG_DIR: '/tmp/parent-manta-opencode-overlay'
      })

      expect(openCodeBuildPtyEnvMock).toHaveBeenCalledWith(expect.any(String), undefined)
      expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/manta-opencode-config')
      expect(env.MANTA_OPENCODE_CONFIG_DIR).toBe('/tmp/manta-opencode-config')
      expect(env.MANTA_OPENCODE_SOURCE_CONFIG_DIR).toBeUndefined()
    })
    it('restores user OpenCode config when agent status hooks are disabled in a nested Manta shell', async () => {
      const env = await spawnAndGetEnv(
        {
          OPENCODE_CONFIG_DIR: '/tmp/parent-manta-opencode-overlay',
          MANTA_OPENCODE_CONFIG_DIR: '/tmp/parent-manta-opencode-overlay',
    ...
    it('injects MiMo overlay env only when launch command is mimo', async () => {
      ...
      expect(env.MIMOCODE_HOME).toBe('/tmp/manta-mimocode-shared')
      expect(env.MANTA_MIMOCODE_HOME).toBe('/tmp/manta-mimocode-shared')
      expect(env.MANTA_MIMOCODE_SOURCE_HOME).toBeUndefined()
    })
    ...
    it('restores user MiMo home when agent status hooks are disabled in a nested Manta shell', async () => {
      const env = await spawnAndGetEnv(
        {
          MIMOCODE_HOME: '/tmp/parent-manta-mimocode-overlay',
          MANTA_MIMOCODE_HOME: '/tmp/parent-manta-mimocode-overlay',
          MANTA_MIMOCODE_SOURCE_HOME: '/tmp/user-mimocode-home'
        },
        undefined,
        undefined,
        () => ({ agentStatusHooksEnabled: false }),
      ...
      expect(env.MANTA_MIMOCODE_HOME).toBeUndefined()
      expect(env.MANTA_MIMOCODE_SOURCE_HOME).toBeUndefined()
    })
    posixOnlyIt(
      'reproduces issue #1534: GUI-launched Manta mirrors zshrc-only OpenCode config',
      async () => {
        ...
        const env = await spawnAndGetEnv(undefined, {
          ...
          MANTA_OPENCODE_SOURCE_CONFIG_DIR: undefined
        })
        ...
        expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/manta-opencode-overlay')
        expect(env.MANTA_OPENCODE_CONFIG_DIR).toBe('/tmp/manta-opencode-overlay')
        expect(env.MANTA_OPENCODE_SOURCE_CONFIG_DIR).toBe('/home/pim/company/opencode-config')
        expect(env.OPENCODE_CONFIG_DIR).not.toBe(env.MANTA_OPENCODE_SOURCE_CONFIG_DIR)
      }
    )
    it('installs Pi managed extensions without redirecting Manta terminal PTY homes', async () => {
      ...
      expect(env.MANTA_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(env.MANTA_PI_SOURCE_AGENT_DIR).toBe('/tmp/user-pi-agent')
      expect(env.MANTA_OMP_CODING_AGENT_DIR).toBeUndefined()
      expect(env.MANTA_OMP_STATUS_EXTENSION).toBe(
        '/tmp/manta-user-data/omp-managed-status-extension/manta-agent-status.ts'
      )
      expect(env.MANTA_OMP_SOURCE_AGENT_DIR).toBeUndefined()
    })
    it('does not materialize a missing Pi home when another agent mentions Pi', async () => {
      ...
      expect(env.MANTA_PI_SOURCE_AGENT_DIR).toBeUndefined()
    })
    it('materializes Pi home for an explicit Pi launch through a custom command', async () => {
      ...
      expect(env.MANTA_PI_SOURCE_AGENT_DIR).toBe('/tmp/default-pi-agent')
    })
    it('threads command: "omp" through to piBuildPtyEnv and emits OMP status metadata', async () => {
      // Why: OMP launches emit MANTA_OMP_* shadow vars, not Pi-named ones; only PI_CODING_AGENT_DIR stays (OMP's own binary reads it).
      const env = await spawnAndGetEnv(
        undefined,
      ...
      expect(env.MANTA_OMP_CODING_AGENT_DIR).toBeUndefined()
      expect(env.MANTA_OMP_STATUS_EXTENSION).toBe(
        '/tmp/user-omp-agent/extensions/manta-agent-status.ts'
      )
      expect(env.MANTA_OMP_SOURCE_AGENT_DIR).toBe('/tmp/user-omp-agent')
    ...
    it('installs Prime status into its independent agent dir on explicit launch', async () => {
      ...
      expect(env.PI_CODING_AGENT_DIR).toBe('/tmp/user-pi-agent')
      expect(env.MANTA_PRIME_AGENT_SOURCE_AGENT_DIR).toBe('/tmp/user-prime-agent')
      expect(env.MANTA_PI_SOURCE_AGENT_DIR).toBeUndefined()
      expect(env.MANTA_OMP_SOURCE_AGENT_DIR).toBeUndefined()
    })
    ...
    it('mirrors the original Pi source dir when launched from a Manta overlay shell', async () => {
      const env = await spawnAndGetEnv({
        PI_CODING_AGENT_DIR: '/tmp/parent-manta-pi-overlay',
        MANTA_PI_SOURCE_AGENT_DIR: '/tmp/user-pi-agent'
      })
      expect(piBuildPtyEnvMock).toHaveBeenCalledWith(
        expect.any(String),
        '/tmp/user-pi-agent',
      ...
      expect(env.PI_CODING_AGENT_DIR).toBe('/tmp/parent-manta-pi-overlay')
      expect(env.MANTA_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(env.MANTA_PI_SOURCE_AGENT_DIR).toBe('/tmp/user-pi-agent')
    })
  })
