...
// Only `getEffectiveHooks` is stubbed: the module under test also imports `parseMantaYaml` from
...
describe('getArchiveHooksForRemoval owner resolution', () => {
  ...
  // from the REMOTE manta.yaml. This drives a stubbed provider holding real content and asserts the
  // returned script is the remote one.
  it('returns the hook from the execution host\u2019s manta.yaml, not the local disk', async () => {
    ...
    expect(readFile).toHaveBeenCalledWith('/home/manta/repo/manta.yaml')
  ...
  // non-numeric error code to -32000, so a missing manta.yaml and an unreachable host arrive
