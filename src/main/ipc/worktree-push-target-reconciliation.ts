// Why: `pr-*` remotes Manta adds for fork-PR review are only ever pruned by
...
// removed outside Manta entirely (no removal event ever fires). This sweep
...
// Manta only ever mints `pr-head` or `pr-<owner>-<repo>` (see `sanitizeRemoteName`), optionally
...
async function shouldReclaimPrRemote(
  ...
  // recorded Manta creating. Naming and URL shape are necessary but not sufficient proof.
