...
import { parseMantaYaml } from '../shared/manta-yaml'
...
export { parseMantaYaml }

/**
 * Load hooks from manta.yaml in the given repo root.
 */
export function loadHooks(repoPath: string): MantaHooks | null {
  const yamlPath = join(repoPath, 'manta.yaml')
...
const RECOGNIZED_MANTA_YAML_KEYS = new Set([
  'scripts',
...
/** True when `manta.yaml` has a top-level key this version of Manta does not handle. */
export function hasUnrecognizedMantaYamlKeys(repoPath: string): boolean {
  try {
    const content = readFileSync(join(repoPath, 'manta.yaml'), 'utf-8')
    for (const line of iterateLfScriptLines(content)) {
      ...
      if (m != null && !RECOGNIZED_MANTA_YAML_KEYS.has(m[1])) {
        return true
      }
    }
    return false
...
export function getEffectiveHooks(repo: Repo, worktreePath?: string): MantaHooks | null {
...
export function runHook(
  ...
  if (wslInfo) {
    // Why: hook scripts run inside WSL, so translate the MANTA_* Windows UNC paths to Linux paths.
    ...
    return runWslProcess({
      ...
      loginPath: 'preferred',
      script,
      // Why pinned: these are user-authored manta.yaml scripts and the native
  ...
  return new Promise<HookProcessOutcome>((resolve) => {
    ...
    if (!settled) {
      deadline = setTimeout(() => {
        ...
        // Manta's own tree terminator: POSIX process groups, `taskkill /t /f` on Windows (where a
