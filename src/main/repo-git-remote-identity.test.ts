...
const gitlabRemote = 'origin\tgit@gitlab.example.com:team/manta.git (fetch)\n'
const gitlabIdentity = {
  canonicalKey: 'gitlab.example.com/team/manta',
  remoteName: 'origin',
  remoteUrl: 'git@gitlab.example.com:team/manta.git'
}
...
describe('probeGitRemoteIdentity', () => {
  it('resolves the canonical identity for a non-GitHub remote', async () => {
    ...
    await expect(probeGitRemoteIdentity('/repos/manta', 'local')).resolves.toEqual({
      status: 'resolved',
  ...
  it('routes each SSH host to its own git provider', async () => {
    ...
    await expect(probeGitRemoteIdentity('/repos/manta', 'ssh:m4air')).resolves.toEqual({
      status: 'resolved',
      identity: gitlabIdentity
    })
    await expect(probeGitRemoteIdentity('/repos/manta', 'ssh:openclaw')).resolves.toEqual({
      status: 'resolved',
      identity: {
  ...
  it('reports unavailable when the SSH host has no connected git provider', async () => {
    await expect(probeGitRemoteIdentity('/repos/manta', 'ssh:builder')).resolves.toEqual({
      status: 'unavailable'
    })
    expect(gitExecFileAsync).not.toHaveBeenCalled()
  })
  ...
  it('refuses a runtime host even when its nested SSH target is registered on this client', async () => {
    ...
    await expect(probeGitRemoteIdentity('/repos/manta', 'runtime:env-a')).resolves.toEqual({
      status: 'unavailable'
    })
  ...
  it('bounds the local probe with a deadline and forwards the caller signal', async () => {
    ...
    await probeGitRemoteIdentity('/repos/manta', 'local', { signal: controller.signal })
  ...
  it('bounds the SSH probe under the relay request timeout and forwards the caller signal', async () => {
    ...
    await probeGitRemoteIdentity('/repos/manta', 'ssh:builder', { signal: controller.signal })
  ...
  it('maps an aborted probe to unavailable, never no-remote', async () => {
    ...
    await expect(
      probeGitRemoteIdentity('/repos/manta', 'local', { signal: controller.signal })
