...
describe('probeGitRemoteIdentity', () => {
  it('resolves the canonical identity for a non-GitHub remote', async () => {
    ...
    await expect(probeGitRemoteIdentity('/repos/manta')).resolves.toEqual({
      status: 'resolved',
      identity: {
  ...
  it('settles on no-remote when git answers with nothing usable', async () => {
    ...
    await expect(probeGitRemoteIdentity('/repos/manta')).resolves.toEqual({ status: 'no-remote' })
  })
  ...
  it('reports unavailable when the SSH host has no connected git provider', async () => {
    ...
    await expect(probeGitRemoteIdentity('/repos/manta', 'builder')).resolves.toEqual({
      status: 'unavailable'
    })
  })
  ...
  it('reports unavailable when the local git command fails', async () => {
    ...
    await expect(probeGitRemoteIdentity('/repos/manta')).resolves.toEqual({ status: 'unavailable' })
  })
  ...
  it('bounds the local probe with a deadline and forwards the caller signal', async () => {
    ...
    await probeGitRemoteIdentity('/repos/manta', null, { signal: controller.signal })
  ...
  it('bounds the SSH probe under the relay request timeout and forwards the caller signal', async () => {
    ...
    await probeGitRemoteIdentity('/repos/manta', 'builder', { signal: controller.signal })
  ...
  it('maps an aborted probe to unavailable, never no-remote', async () => {
    ...
    await expect(
      probeGitRemoteIdentity('/repos/manta', null, { signal: controller.signal })
