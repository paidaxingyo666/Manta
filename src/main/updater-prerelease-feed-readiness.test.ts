...
function buildManifest(tag: string): string {
  ...
  return [
    `version: ${version}`,
    'files:',
    `  - url: Manta-${version}-arm64-mac.zip`,
    '    sha512: test',
    `path: Manta-${version}-arm64-mac.zip`
  ].join('\n')
}
...
function buildWindowsManifest(version: string): string {
  return [
    `version: ${version}`,
    'files:',
    '  - url: manta-windows-setup.exe',
    '    sha512: test',
    'path: manta-windows-setup.exe'
  ].join('\n')
}
...
describe('fetchNewerReleaseTagsWithReadiness', () => {
  ...
  it('requires every asset referenced by the manifest files list to be reachable', async () => {
    netFetchMock.mockImplementation((url: string, init?: { method?: string }) => {
          ...
          text: () =>
            Promise.resolve(
              [
                ...
                '  - url: manta-windows-setup.exe',
                '    sha512: test',
                `  - url: Manta-${version}-mac.zip`,
                '    sha512: test',
                `path: Manta-${version}-mac.zip`
              ].join('\n')
            )
        })
      }
      ...
      if (init?.method === 'HEAD') {
        ...
        const unavailable = latest && url.endsWith('/Manta-1.4.28-mac.zip')
        const missing = latest && url.endsWith('/manta-windows-setup.exe')
        return Promise.resolve({
  ...
  it('treats an explicit asset 404 as not-ready when another asset is unavailable', async () => {
    netFetchMock.mockImplementation((url: string, init?: { method?: string }) => {
      ...
      if (init?.method === 'HEAD') {
        const isWindowsAsset = url.endsWith('/manta-windows-setup.exe')
        return Promise.resolve({
          ok: false,
  ...
  it('accepts absolute manifest asset URLs without rewriting them to release asset paths', async () => {
          ...
          text: () =>
            Promise.resolve(
              [
                'version: 1.4.27',
                'files:',
                '  - url: https://downloads.example.com/Manta-1.4.27-arm64-mac.zip',
                '    sha512: test'
              ].join('\n')
            )
        })
    ...
    expect(assetUrls).toEqual(['https://downloads.example.com/Manta-1.4.27-arm64-mac.zip'])
  })
