...
describe('GeminiHookService', () => {
  ...
  beforeAll(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'manta-gemini-home-'))
    userDataDir = mkdtempSync(join(tmpdir(), 'manta-gemini-userdata-'))
  ...
  it('removes stale PreToolUse hooks when reinstalling managed Gemini hooks', () => {
    ...
    const staleManagedHookPath =
      process.platform === 'win32'
        ? `C:\\Users\\ramzi\\.manta\\agent-hooks\\${managedHookFileName}`
        : `/Users/ramzi/.manta/agent-hooks/${managedHookFileName}`
    const staleManagedCommand =
      process.platform === 'win32'
        ? staleManagedHookPath
        : `if [ -x '${staleManagedHookPath}' ]; then /bin/sh '${staleManagedHookPath}'; fi`
    const managedHookPath = join(homeDir, '.manta', 'agent-hooks', managedHookFileName)
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'manta gemini home with spaces')
