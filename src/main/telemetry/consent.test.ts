...
describe('resolveConsent', () => {
  ...
  it('returns manta_disabled when MANTA_TELEMETRY_DISABLED=1', () => {
    process.env.MANTA_TELEMETRY_DISABLED = '1'
    expect(
      resolveConsent(
        settingsWithTelemetry({
          optedIn: true,
      ...
      effective: 'disabled',
      reason: 'manta_disabled'
    })
  })

  it('prefers do_not_track over manta_disabled when both are set', () => {
    process.env.DO_NOT_TRACK = '1'
    process.env.MANTA_TELEMETRY_DISABLED = '1'
    expect(
      resolveConsent(
  ...
  it('env-var override wins over a stored pending_banner state', () => {
    ...
    ).toEqual({ effective: 'disabled', reason: 'manta_disabled' })
  })
})
