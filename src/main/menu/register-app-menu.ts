...
function buildAndApplyMenu(options: RegisterAppMenuOptions): void {
  ...
  const featureTourItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.exploreManta', 'Explore Manta'),
    click: (_menuItem, window) => onOpenFeatureTour(window)
  }

  const setupGuideItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.gettingStarted', 'Getting Started with Manta'),
  ...
  // redundant "Manta" entry with roles that don't apply, so we omit it there
    ...
    submenu: [
      ...
      }),
      {
        ...
        click: () => {
          ...
          // control, so raw Electron paste cannot know which Manta surface owns it.
  ...
  const appearanceSubmenu: Electron.MenuItemConstructorOptions = {
    ...
    submenu: [
      ...
      {
        label: translateMain('menu.showMobileButton', 'Show Manta Mobile Button'),
        type: 'checkbox',
