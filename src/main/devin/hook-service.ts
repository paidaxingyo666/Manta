...
import {
  ...
  readConfigFromMantaOverlapDetail,
...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  ...
  return [
    '#!/bin/sh',
    ...buildPosixHookPayloadCapture(),
    // Why: endpoint file holds the live port/token; PTYs that outlive a Manta restart carry stale env, so source it to reach the new server (else PTY env).
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${MANTA_AGENT_HOOK_PORT}/hook/devin" \\',
...
export class DevinHookService {
  ...
  getStatus(): AgentHookInstallStatus {
    ...
    return {
      ...
      configPath,
      managedHooksPresent,
      detail: mergeHookInstallDetail(detail, readConfigFromMantaOverlapDetail(config))
    }
  }
  ...
  async installRemote(sftp: SFTPWrapper, remoteHome: string): Promise<AgentHookInstallStatus> {
    ...
    const remoteScriptPath = `${remoteHome.replace(/\/$/, '')}/.manta/agent-hooks/${remoteScriptFileName}`
    ...
    try {
      ...
      // Why: SSH remotes use POSIX `.sh` hooks even when Manta runs on Windows; never derive remote script syntax from local OS.
