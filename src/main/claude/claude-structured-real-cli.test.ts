 ...
 *  is not derived from Manta's own path expressions. */
...
describe.skipIf(!realClaudeAvailable)('Claude structured real CLI handshake', () => {
  ...
  // against the root the CLI itself reports. Deriving the expected root from Manta's own
  ...
  // silently degrades to echoing back whatever Manta sent.
  it.skipIf(!realClaudeAuthenticated)(
    ...
    async () => {
      ...
      try {
        ...
        // for in the catalog's resolved shape rather than the id Manta sent.
