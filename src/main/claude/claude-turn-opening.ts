// Whether Manta's own send echo opens a turn.
//
// The provider's own output opens one too — see `ensureTurnOpen` in the
// translator, which the content sites call as they journal. Manta's turn used to
...
// background task reports in and wakes the agent, and nothing Manta sent ever
...
export type ClaudeSendEchoTurnInput = {
  ...
  /** Manta dispatched this send and the provider is replaying it back. */
  startsTurn: boolean
  observedAt: number
