...
describe('Claude structured journal translation', () => {
  ...
  it('journals turn lifecycle and updates one tool row through its result', () => {
    ...
    expect(keyed.get('manta:claude-tool%3Aclaude-session%3Atool-1')).toMatchObject({
      kind: 'tool-call',
      name: 'Bash',
      state: 'completed',
  ...
  it('does not start a lifecycle for a top-level user tool result', () => {
    ...
    expect(state.items.map((item) => agentJournalItemKey(item.identity))).toEqual([
      'manta:claude-tool%3Aclaude-session%3Atool-1'
    ])
  ...
  it('preserves a question group as one addressable prompt and cancels it durably', () => {
    ...
    translator.handle({ type: 'prompt', sessionId: 'manta-session', prompt: approval })
    expect(state.items.at(-1)?.body).toMatchObject({
      kind: 'approval',
      title: 'Allow Bash?',
    ...
    translator.handle({ type: 'prompt', sessionId: 'manta-session', prompt: questions })
    ...
    translator.handle({ type: 'prompt', sessionId: 'manta-session', prompt: multiSelect })
    expect(state.items.at(-1)?.body).toMatchObject({
      kind: 'question',
