 ...
 * emits it — and Manta's send acknowledgement depends on the replay.
 */
...
export type ClaudeStructuredLaunch = {
  /** Always Manta's resolved user CLI: the SDK's bundled binaries are excluded from the install. */
...
export function claudeSessionIdForOrcaSession(sessionId: string): string {
  const bytes = createHash('sha256').update(`manta-claude:${sessionId}`).digest().subarray(0, 16)
...
export function createClaudeStructuredLaunchResolver(
  ...
  return async ({ identity }) => {
    ...
    const env = withCliRuntimeOnPath(
      command,
      // Only a dispatched structured worker gets the orchestration identity and the Manta CLI on
