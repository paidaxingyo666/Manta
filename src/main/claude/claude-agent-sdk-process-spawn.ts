...
export type ClaudeCodeProcessSpawn = {
  ...
  /** The retained child, so Manta keeps its own tree-kill and exit-proof ladder. Null until the SDK spawns. */
...
/**
 * Manta supplies the Claude Code child rather than letting the SDK spawn it.
 *
 * Two independent reasons: the SDK's `SpawnedProcess` has no pid, and Manta's
...
export function createClaudeCodeProcessSpawn(
  ...
  return {
    spawn: (options) => {
      ...
      // Manta's ladder, and close() may never report an exit it did not observe.
