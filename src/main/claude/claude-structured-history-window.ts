...
// conversation Manta is about to resume. Absence here is not an inference about a
// dead child — it is the content of the next turn's context.
//
// The window is anchored on the leaf uuid Manta durably recorded for the session.
...
/**
 * A user record Manta could itself have submitted. Everything the harness injects
...
export function claudeProviderHistoryWindowFromJsonl(input: {
  ...
  /** Manta session id: the fingerprint a submission carries is scoped to it. */
  sessionId: string
