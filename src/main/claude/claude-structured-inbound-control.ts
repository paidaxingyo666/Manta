...
/**
 * The blocking control requests Manta answers, each mapped to the SDK consumer callback that
 ...
 * blocking control Manta must answer means adding its callback here, and the catalog test
...
function denySafeResult(toolUseId: string | undefined): PermissionResult {
  return {
    behavior: 'deny',
    message: 'Manta could not decode this permission request.',
 ...
 * by `answerClaudePrompt`. `onUserDialog` is deny-safe; the CLI only emits dialog kinds Manta
