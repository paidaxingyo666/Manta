...
/** The derived child environment, captured where Manta actually hands it to the OS. */
...
describe('Claude stream-json connection', () => {
  ...
  it('hands the child a derived environment, the resolved CLI path, and keeps the pid', async () => {
    ...
    // itself. Either way the resolved path is what Manta's spawner is asked to execute.
  ...
  it('drives Manta control methods onto the SDK and times out with the init proof message', async () => {
    const scenario = scriptScenario([HOLD_OPEN], {
