...
// to catch a future SDK version drifting under Manta: unknown-frame pass-through,
...
describe('Claude Agent SDK contract pins', () => {
  ...
  it('hands the custom spawner exactly the caller-supplied env, plus the two pinned SDK mutations', async () => {
    ...
    // observable at this boundary, so Manta's auth scrubbing stays assertable.
  ...
  it('inherits process.env into the child when env is omitted — the ambient-auth sharp edge', async () => {
    ...
    // sees everything in process.env. Manta must therefore always pass an
  ...
  it('produces a matching CLI flag for every pre-SDK argv entry', async () => {
    ...
    // binaries reject. Keep the older owned flag until Manta establishes a minimum CLI version.
