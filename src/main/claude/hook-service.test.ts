...
describe('getWindowsManagedLifecycleHook', () => {
  const SAFE_SCRIPT_PATH = 'C:\\Users\\alice\\.manta\\agent-hooks\\claude-hook.cmd'
  const UNSAFE_SCRIPT_PATH = 'C:\\Users\\%name%\\a^b&c\\.manta\\agent-hooks\\claude-hook.cmd'
  ...
  it('registers the script itself, with no interpreter in front of it (#18875)', () => {
    ...
    // event (471ms vs 201ms measured) before the .cmd could reach its MANTA_PANE_KEY guard, and
    ...
    expect(hook.command).toBe('C:/Users/alice/.manta/agent-hooks/claude-hook.cmd || echo {}')
  ...
  it('falls back to the encoded launcher when the profile path is not cmd-safe', () => {
    ...
    expect(decoded).toContain('.manta\\agent-hooks\\claude-hook.cmd')
  })
    ...
    () => {
      ...
      try {
        ...
        for (const eventName of ['UserPromptSubmit', 'Stop', 'StopFailure']) {
          ...
          const decoded = Buffer.from(encoded ?? '', 'base64').toString('utf16le')
          expect(decoded).toContain('$env:USERPROFILE')
          expect(decoded).toContain(`.manta\\agent-hooks\\${CLAUDE_SCRIPT_FILE_NAME}`)
        }
      } finally {
        vi.unstubAllEnvs()
        rmSync(tmpHome, { recursive: true, force: true })
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      ...
      try {
        const settingsPath = join(tmpHome, '.claude', 'settings.json')
        mkdirSync(join(tmpHome, '.claude'), { recursive: true })
        const staleCommand = 'C:/Users/someone-else/.manta/agent-hooks/claude-hook.cmd || echo {}'
...
describe('backgrounded-session pane guard (#9236)', () => {
  ...
  // env of whichever pane started that daemon, so MANTA_PANE_KEY names a pane the session
  ...
  it('guards the statusline too, on both branches', () => {
    ...
    for (const target of ['darwin', 'win32'] as const) {
      const tmpHome = mkdtempSync(join(tmpdir(), `manta-claude-sl-${target}-`))
      ...
      try {
        ...
        expect(script.indexOf('CLAUDE_JOB_DIR')).toBeLessThan(script.indexOf('curl'))
        if (target === 'win32') {
          // Why: a worker is outside a Manta pane, where reading stdin to EOF never returns (#11549).
  ...
  it('exits rather than draining stdin on Windows, where a worker has no Manta pane', () => {
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')!
    const tmpHome = mkdtempSync(join(tmpdir(), 'manta-claude-bg-'))
...
describe('ClaudeHookService.installRemote', () => {
  it('writes Claude settings + managed script under the remote $HOME', async () => {
    ...
    expect(script).toContain('manta_hook_metadata=$(printf')
    expect(script).toContain('unset MANTA_AGENT_HOOK_TRANSPORT')
    expect(script).toContain('-H "X-Manta-Agent-Hook-Meta: ${manta_hook_metadata}"')
...
describe('OpenClaudeHookService-compatible install', () => {
  ...
  it('writes remote OpenClaude settings under .openclaude', async () => {
    ...
    expect(fs.files.get('/home/dev/.manta/agent-hooks/openclaude-hook.sh')).toContain('/hook/claude')
  })
})
