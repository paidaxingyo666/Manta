...
// cannot commit — it is a symlink Manta created.
describe('createHostedReview with shared symlinks', () => {
  ...
  beforeEach(() => {
    worktree = mkdtempSync(join(tmpdir(), 'manta-hosted-shared-'))
  ...
  // Why: only an *untracked* record can be Manta's artifact. A tracked change at a
