...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'manta-copilot-hooks-'))
...
function readConfig(): Record<string, unknown> {
  return JSON.parse(readFileSync(join(copilotHome, 'hooks', 'manta.json'), 'utf-8'))
}
...
describe('CopilotHookService', () => {
  it('installs a user-level Copilot hook file under COPILOT_HOME', () => {
    ...
    expect(status.configPath).toBe(join(copilotHome, 'hooks', 'manta.json'))
    ...
    if (process.platform === 'win32') {
      ...
      expect(decoded).toContain("$env:MANTA_COPILOT_HOOK_EVENT = 'UserPromptSubmit'")
    } else {
      expect(firstPromptHook.bash).toContain('if [ -f ')
      expect(firstPromptHook.bash).toContain('] && [ -x ')
      expect(firstPromptHook.bash).toContain('.manta/agent-hooks/copilot-hook.sh')
      expect(firstPromptHook.bash).toContain("MANTA_COPILOT_HOOK_EVENT='UserPromptSubmit'")
    }
    expect(existsSync(join(tmpDir, '.manta', 'agent-hooks', 'copilot-hook.sh'))).toBe(
  ...
  it('preserves user-authored hooks and sweeps stale managed entries', () => {
    const configPath = join(copilotHome, 'hooks', 'manta.json')
  ...
  it('remove deletes only Manta-managed Copilot hooks', () => {
    const service = new CopilotHookService()
    service.install()
    const configPath = join(copilotHome, 'hooks', 'manta.json')
    const config = readConfig()
  ...
  it('remove does not create an manta.json file when nothing is installed', () => {
    ...
    expect(status.state).toBe('not_installed')
    expect(existsSync(join(copilotHome, 'hooks', 'manta.json'))).toBe(false)
  })
  ...
  it('returns an error status for malformed JSON', () => {
    mkdirSync(join(copilotHome, 'hooks'), { recursive: true })
    writeFileSync(join(copilotHome, 'hooks', 'manta.json'), '{not json')
    ...
    expect(status).toEqual({
      agent: 'copilot',
      state: 'error',
      configPath: join(copilotHome, 'hooks', 'manta.json'),
      managedHooksPresent: false,
      detail: 'Could not parse Copilot hooks/manta.json'
    })
  })
})
