...
// names, which match the event vocabulary already normalized by Manta's hook
...
export const COPILOT_EVENTS = [
  ...
  // Manta can normalize it without depending on a PascalCase payload.
  'subagentStart',
  'SubagentStop',
  'PreCompact',
  'Stop',
...
export function getManagedCommand(scriptPath: string, eventName: string): string {
  if (process.platform !== 'win32') {
    return wrapPosixHookCommand(scriptPath, { MANTA_COPILOT_HOOK_EVENT: eventName })
  }
  return wrapWindowsHookCommand(scriptPath, { MANTA_COPILOT_HOOK_EVENT: eventName })
}
