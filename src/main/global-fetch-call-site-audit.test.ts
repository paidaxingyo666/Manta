...
// socket (nodejs/undici#5360, manta#8695). This applies to every Node process
...
const AUDITED_GLOBAL_FETCH_LINES = new Map<string, number>([
  ...
  ['main/manta-profiles/profile-cloud-account-client.ts', 1],
  ...
  it('keeps every global-fetch line audited with its expected count', () => {
    ...
    expect(
      ...
      'Global fetch (bare, globalThis.fetch, or global.fetch) uses undici, ' +
        'where an unread response body can crash the whole process (manta#8695). ' +
