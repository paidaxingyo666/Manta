...
export function getCodexSessionsDirectory(): string {
  // Why: Manta-launched Codex processes receive a Manta-owned CODEX_HOME, so
  // callers that need the primary runtime path should not consult ambient
  // shell CODEX_HOME.
  return join(getMantaManagedCodexHomePath(), 'sessions')
}
...
function hasLegacyCopiedSessionBridgeMarkers(): boolean {
  return existsSync(join(getMantaManagedCodexHomePath(), '.manta-session-copies'))
}
