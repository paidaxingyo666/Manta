...
describe('CodexUsageStore', () => {
  ...
  it('reports no data for Manta scope when only non-Manta Codex usage exists', async () => {
  ...
  it('uses only Manta-scoped models when projecting mixed-scope sessions', async () => {
    const store = createStoreWithState({
      sessions: [
        {
          sessionId: 'session-1',
    ...
    const breakdown = await store.getBreakdown('manta', '30d', 'model')
    const recentSessions = await store.getRecentSessions('manta', '30d', 10)
