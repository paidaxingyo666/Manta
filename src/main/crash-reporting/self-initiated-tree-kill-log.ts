...
/**
 * Records the force-kills Manta itself issues, so a later `render-process-gone`
 ...
 * (see `manta-chromium-process-pids.ts`), and a group or Job-Object kill can
 * only contain what Manta put in it. Absence is evidence, not proof.
 */
 ...
 * Manta did not put in its target: `taskkill /T /F` walks whatever tree the pid
 * owns at kill time, including a recycled pid that is now our renderer. A
 * process group or Job Object contains only what Manta placed there, so it is
 ...
 * direct proof that Manta was about to kill its own renderer.
 */
...
/**
 * Kills Manta issued near `goneAt`, split by whether the mechanism could have
