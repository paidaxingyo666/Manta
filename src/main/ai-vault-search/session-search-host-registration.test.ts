...
it.each([
  ...
  ['mantad', 'src/main/mantad/mantad-session-search.ts', 'installInProcessSessionSearchService'],
  [
    'the relay daemon',
...
it('mantad resolves no roots while disabled and discovers late roots when enabled', async () => {
  const { installOrcadSessionSearchService } = await import('../mantad/mantad-session-search')
...
// mantad reaches the index through the deps hook the runtime RPC calls; the wiring is
// what no unit of either module can show.
it('wires mantad consent from the runtime hook to the installed service', () => {
  const source = readFileSync(join(ROOT, 'src/main/mantad/mantad-entry.ts'), 'utf8')
