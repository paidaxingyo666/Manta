...
function catalog(overrides: Partial<SessionSearchScopeCatalog> = {}): SessionSearchScopeCatalog {
  return {
    ...
    projectHostSetups: [],
    worktreeMeta: {},
    settings: { workspaceDir: '/home/me/manta/workspaces', nestWorkspaces: true },
    ...overrides
  }
}
...
describe('project scope', () => {
  const projectCatalog = catalog({
    worktreeMeta: {
      'repo-1::/home/me/manta/workspaces/app/one': {},
      'repo-1::/home/me/manta/workspaces/app/two': {},
  ...
  it('folds hundreds of worktrees into the one directory that contains them', () => {
    ...
    for (let index = 0; index < 580; index++) {
      worktreeMeta[`repo-1::/home/me/manta/workspaces/app/wt-${index}`] = {}
    }
    const resolution = resolveSessionSearchScope(
      { kind: 'project', projectKey: 'repo:repo-1' },
      catalog({ worktreeMeta })
    )
    expect(paths(resolution)).toEqual(['/home/me/manta/workspaces/app', '/work/app'])
  })
  ...
  it("uses a repo's own worktree base path instead of the global root", () => {
    ...
    expect(paths(resolution)).toEqual(['/home/me/manta/workspaces/app', '/trees/app', '/work/app'])
  })
  ...
  it('covers a workspace root the user has since moved away from', () => {
    const resolution = resolveSessionSearchScope(
      ...
      catalog({
        settings: {
          workspaceDir: '/home/me/manta/workspaces',
          nestWorkspaces: true,
  ...
  it('claims no managed directory under flat placement, where the root is every project’s', () => {
    const resolution = resolveSessionSearchScope(
      { kind: 'project', projectKey: 'repo:repo-1' },
      catalog({
        settings: { workspaceDir: '/home/me/manta/workspaces', nestWorkspaces: false },
        worktreeMeta: { 'repo-1::/home/me/manta/workspaces/one': {} }
      })
    )
    expect(paths(resolution)).toEqual(['/home/me/manta/workspaces/one', '/work/app'])
  })
  ...
  it('resolves a project id through this host’s setup for it', () => {
    ...
    // of its own, so that is still where Manta would create its worktrees.
    expect(paths(resolution)).toEqual(['/home/me/manta/workspaces/app', '/srv/app', '/srv/trees'])
  })
