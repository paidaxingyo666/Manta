...
// parsed hook envelope to Manta over this process's own stdin/stdout using the
...
async function main(): Promise<void> {
  const windowsPort = Number(process.env.MANTA_AGENT_HOOK_PORT ?? '')
  const token = process.env.MANTA_AGENT_HOOK_TOKEN ?? ''
  if (!Number.isInteger(windowsPort) || windowsPort <= 0 || token.length === 0) {
    process.stderr.write('[wsl-hook-relay] missing MANTA_AGENT_HOOK_PORT/TOKEN in env\n')
    process.exit(1)
  }
