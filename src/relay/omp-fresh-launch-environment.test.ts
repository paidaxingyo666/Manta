...
it('prepares the execution host OMP config and status extension for a guarded launch', async () => {
  ...
  vi.spyOn(PluginOverlayManager.prototype, 'materializePi').mockImplementation(function (
    ...
    this.setSources({ ompExtensionSource: '// Manta managed test status extension' })
  ...
  try {
    ...
    expect(existsSync(environment.MANTA_OMP_STATUS_EXTENSION)).toBe(true)
    expect(environment.MANTA_OMP_SOURCE_AGENT_DIR).toBe(join(state.home, '.omp', 'agent'))
    expect(environment.PI_CODING_AGENT_DIR).toBeUndefined()
    expect(environment.MANTA_PI_SOURCE_AGENT_DIR).toBeUndefined()
    if (process.platform !== 'win32' && existsSync('/bin/bash')) {
      ...
      const result = await runProcess({
        ...
        env: {
          ...
          MANTA_OMP_PREFILL: 'remote draft'
        }
      })
      expect(result.code).toBe(17)
      expect(result.stdout.trim().split('\n')).toEqual([
        '--extension',
        environment.MANTA_OMP_STATUS_EXTENSION,
        '--config',
