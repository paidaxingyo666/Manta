// Wire-level handshake helpers for the Manta relay.
...
export function setupDaemonHandshake(sock: Socket, cb: DaemonHandshakeCallbacks): void {
  ...
  ;(sock as Socket & { __mantaOnHandshake?: typeof onHandshakeData }).__mantaOnHandshake =
    onHandshakeData
}

export function detachHandshakeListener(sock: Socket): void {
  const tagged = sock as Socket & { __mantaOnHandshake?: (chunk: Buffer) => void }
  if (tagged.__mantaOnHandshake) {
    sock.removeListener('data', tagged.__mantaOnHandshake)
    delete tagged.__mantaOnHandshake
  }
}
...
function handleDaemonHandshakeFrame(
  ...
  if (msg.type !== 'manta-relay-handshake') {
  ...
  if (endpointCredential !== undefined && presented !== endpointCredential) {
    ...
    try {
      sock.write(encodeHandshakeFrame({ type: 'manta-relay-handshake-credential-mismatch' }))
    } catch {
  ...
  sock.write(encodeHandshakeFrame({ type: 'manta-relay-handshake-ok', version: launchVersion }))
  return true
}
...
export function runConnectHandshake(
  ...
  const decoder: FrameDecoder = new FrameDecoder(
    (frame: DecodedFrame) => {
      ...
      if (msg.type === 'manta-relay-handshake-ok') {
      ...
      if (msg.type === 'manta-relay-handshake-mismatch') {
      ...
      if (msg.type === 'manta-relay-handshake-credential-mismatch') {
        process.stderr.write(
