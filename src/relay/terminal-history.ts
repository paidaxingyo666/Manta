...
const HISTORY_ROOT = join(homedir(), '.manta-remote', 'terminal-history')
...
export function injectRelayHistoryEnv(
  ...
  // Why first: same reason as the desktop path — an inherited MANTA_HISTFILE
  ...
  try {
    ...
    env.MANTA_HISTFILE = env.HISTFILE
    return HISTORY_ROOT
  } catch {
    return null
  }
