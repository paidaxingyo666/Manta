...
describe('handshake framing', () => {
  ...
  it('rejects payloads with unknown type', () => {
    const bogus = Buffer.from(JSON.stringify({ type: 'manta-something-else', version: 'x' }))
  ...
  it('rejects a version that is not a string on both arms that carry one', () => {
    for (const type of ['manta-relay-handshake', 'manta-relay-handshake-ok']) {
  ...
  it('rejects a mismatch reply whose expected or got is not a string', () => {
    const type = 'manta-relay-handshake-mismatch'
    expect(() =>
  ...
  it('rejects payloads that are not objects', () => {
    for (const payload of ['null', '"manta-relay-handshake"', '42']) {
  ...
  it('rejects a present endpointCredential that is not a string', () => {
    for (const endpointCredential of [{ toString: 1 }, 7, null, ['secret'], true]) {
      const payload = Buffer.from(
        JSON.stringify({ type: 'manta-relay-handshake', version: '0.1.0', endpointCredential })
      )
      expect(
        () => parseHandshakeMessage(payload),
  ...
  it('still accepts a handshake with no endpointCredential, and one with a string', () => {
    const bare = Buffer.from(JSON.stringify({ type: 'manta-relay-handshake', version: '0.1.0' }))
    expect(parseHandshakeMessage(bare)).toEqual({ type: 'manta-relay-handshake', version: '0.1.0' })
    const withCredential = Buffer.from(
      JSON.stringify({ type: 'manta-relay-handshake', version: '0.1.0', endpointCredential: 'sec' })
    )
  ...
  it('still accepts a credential-mismatch reply, which carries no fields', () => {
    const payload = Buffer.from(
      JSON.stringify({ type: 'manta-relay-handshake-credential-mismatch' })
    )
