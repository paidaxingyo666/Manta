...
// Manta Relay — lightweight daemon deployed to remote hosts over SCP and launched via an SSH exec channel.
...
const EMPTY_DETACHED_STARTUP_GRACE_MS = parseNonNegativeIntEnv(
  'MANTA_RELAY_EMPTY_STARTUP_GRACE_MS',
  60_000
)
...
const IDLE_RELAY_GRACE_MS = parseNonNegativeIntEnv('MANTA_RELAY_IDLE_GRACE_MS', 15 * 60_000)
...
async function runMantaCliMode(
  sockPath: string,
  argv: string[],
  ...
  const connectTimeout = setTimeout(() => {
    process.stderr.write(`[manta-cli] Relay connection timed out after ${CONNECT_TIMEOUT_MS}ms\n`)
    sock.destroy()
    process.exit(1)
  }, CONNECT_TIMEOUT_MS)
  ...
  sock.on('error', (err) => {
    clearTimeout(connectTimeout)
    process.stderr.write(`[manta-cli] Relay socket error: ${err.message}\n`)
    process.exit(1)
  })
}
...
async function main(): Promise<void> {
  ...
  dispatcher.onRequest('manta.cli', async (params, context) => {
    return await dispatcher.requestAnyClient('manta.cli', params, {
      excludeClientId: context.clientId,
      timeoutMs: remoteCliRequestTimeoutMs(params)
    })
  })
  dispatcher.onRequest('manta.cli.postOutput', async (params, context) => {
    return await dispatcher.requestAnyClient('manta.cli.postOutput', params, {
      excludeClientId: context.clientId,
      timeoutMs: remoteCliRequestTimeoutMs(params)
    })
  })
  ...
  // ── Agent-hook server ─────────────────────────────────────────────
  // Why: loopback HTTP receiver so remote-PTY agent CLIs post hook events to a local port (they can't reach Manta's host); the relay forwards them as agent.hook notifications.
  ...
  // Why: await the bind before announcing readiness so the first PTY spawn already sees MANTA_AGENT_HOOK_* env; bind failure is soft (log and continue).
  try {
  ...
  // Why: read the augmenter on every spawn so a late (or restarted) hook-server bind still lands in the next PTY's MANTA_AGENT_HOOK_* env.
  ...
  ptyHandler.addEnvAugmenter((ctx) => {
    ...
    if (pluginOverlay.hasPiSource()) {
      // Why: install Manta's guarded extension into the launched agent's (Pi vs OMP) real remote dir without redirecting PI_CODING_AGENT_DIR.
      ...
      if (shouldPrepareOmpShadow) {
        ...
        const sourceDir =
          kind === 'omp'
            ...
            : ctx.env.MANTA_OMP_SOURCE_AGENT_DIR
        ...
        // shells without ~/.omp — still export MANTA_OMP_STATUS_EXTENSION (#10196).
  ...
  // Why: forward cached entries as notifications before returning, so the response trails every replay and Manta can't treat replay as done while frames are still in flight.
  ...
  // Why: plugin sources ship over the wire — the relay is versioned independently of Manta, so bundling them would make every agent-event change a relay redeploy.
  // Why: bound per-source size so a buggy/hostile Manta can't OOM the relay by pushing a giant string.
