...
describe('getRelayShellLaunchConfig', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      // Why no MANTA_USER_ZDOTDIR: the relay used to republish the inherited
      ...
      expect(zshenv).not.toContain('MANTA_USER_ZDOTDIR')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      // bridge sets MANTA_REMOTE_CLI_BIN_DIR, which was already enough to wrap.
      const env = buildSshPtySpawnEnv({
        env: { HOME: homeDir, PATH: '/usr/bin:/bin' },
        remoteCliBridgeEnv: {
          binDir: '/home/remote/.manta-relay/bin',
          relayDir: '/home/remote/.manta-relay',
          nodePath: '/home/remote/.manta-relay/node',
          sockPath: '/home/remote/.manta-relay/relay.sock'
        }
      })
      env.MANTA_HISTFILE = join(homeDir, 'manta-history', 'zsh_history')
      ...
      expect(config.env.MANTA_SHELL_FEATURES).toBe('overlay,history,markers')
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    'keeps a remote zsh with nothing Manta-owned on the plain login path',
    () => {
      const env = buildSshPtySpawnEnv({ env: { HOME: homeDir, PATH: '/usr/bin:/bin' } })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env).toEqual({ MANTA_SHELL_FEATURES: '' })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env.MANTA_SHELL_FEATURES).toContain('ready')
      ...
      expect(zshenv).toContain('builtin unset MANTA_SHELL_FEATURES')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(bashRc).toContain('builtin unset MANTA_SHELL_FEATURES')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env.ZDOTDIR).toBe(join(homeDir, '.manta-relay', 'shell-ready', 'zsh'))
      expect(config.env.MANTA_SHELL_FEATURES).toContain('identity')
      ...
      expect(config.env.MANTA_SHELL_FEATURES).not.toContain('ready')
