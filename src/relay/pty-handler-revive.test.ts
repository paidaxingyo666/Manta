...
describe('PtyHandler', () => {
  ...
  it('revive restores pane identity env alongside hook-server coordinates', async () => {
    ...
    const oldShellFeatures = process.env.MANTA_SHELL_FEATURES
    process.env.MANTA_SHELL_FEATURES = 'ready,identity,markers,overlay'
    try {
      ...
      if (oldShellFeatures === undefined) {
        ...
        process.env.MANTA_SHELL_FEATURES = oldShellFeatures
      }
      killSpy.mockRestore()
    }
    ...
    expect(callArgs.env.MANTA_SHELL_FEATURES).not.toContain('ready')
    expect(callArgs.env.MANTA_SHELL_FEATURES).not.toContain('identity')
  })
