...
describe('RelayAgentHookServer', () => {
  ...
  it('keeps named-pipe endpoint files on a real filesystem path', () => {
    const endpointDir = endpointDirForRelaySocket('\\\\.\\pipe\\manta-relay-abc123')

    expect(endpointDir).toBe(join(homedir(), '.manta-relay', 'agent-hooks', 'manta-relay-abc123'))
    ...
    try {
      ...
      const res = await fetch(`http://127.0.0.1:${port}/hook/claude`, {
        ...
        headers: {
          ...
          'X-Manta-Agent-Hook-Meta': Buffer.from(
      ...
      // Why: the relay forwards body env/version so Manta's warn-once
  ...
  it('rejects requests with the wrong bearer token (403)', async () => {
    ...
    try {
      ...
      const res = await fetch(`http://127.0.0.1:${port}/hook/claude`, {
        ...
        headers: {
          'Content-Type': 'application/json',
          'X-Manta-Agent-Hook-Token': 'wrong'
        },
        body: '{}'
      })
  ...
  // the wire, even though Manta main re-validates at the SSH trust boundary.
  ...
  it('exposes MANTA_AGENT_HOOK_* env vars after start', async () => {
    const forward = vi.fn()
    const server = new RelayAgentHookServer({ endpointDir: dir, forward })
    await server.start()
    try {
      const env = server.buildPtyEnv()
      expect(env.MANTA_AGENT_HOOK_PORT).toMatch(/^\d+$/)
      expect(env.MANTA_AGENT_HOOK_TOKEN).toBeTruthy()
      expect(env.MANTA_AGENT_HOOK_ENV).toBe('remote')
      expect(env.MANTA_AGENT_HOOK_VERSION).toBe('1')
      expect(env.MANTA_AGENT_HOOK_TRANSPORT).toBe('raw-json-v1')
      expect(env.MANTA_AGENT_HOOK_ENDPOINT).toBeTruthy()
    } finally {
      server.stop()
    }
  })
  ...
  it('can defer endpoint file publication until relay socket ownership is proven', async () => {
    ...
    try {
      ...
      expect(server.buildPtyEnv().MANTA_AGENT_HOOK_ENDPOINT).toBeTruthy()
    } finally {
      server.stop()
    }
  })
