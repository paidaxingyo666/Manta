...
describe.skipIf(process.platform === 'win32')('createInstallPluginsHandler (guest side)', () => {
  ...
  it('writes manta-opencode-status.js into the overlay and returns that dir', () => {
    withHome((home) => {
      const install = createInstallPluginsHandler(new PluginOverlayManager({ homeDir: home }), {
        HOME: home,
        MANTA_WSL_HOOK_INSTANCE: 'inst1'
      })
      const source = '// manta opencode status plugin\nexport const Plugin = () => ({})\n'
      ...
      const pluginPath = join(dir as string, 'plugins', 'manta-opencode-status.js')
  ...
  it('writes the OpenCode 2 plugin to its separate overlay', () => {
    withHome((home) => {
      const install = createInstallPluginsHandler(new PluginOverlayManager({ homeDir: home }), {
        HOME: home,
        MANTA_WSL_HOOK_INSTANCE: 'inst-v2'
      })
  ...
  it('rebuilds if the resolved source dir ever changes (defensive)', () => {
    withHome((home) => {
      ...
      const env: NodeJS.ProcessEnv = { HOME: home, MANTA_WSL_HOOK_INSTANCE: 'inst1' }
      ...
      env.MANTA_OPENCODE_SOURCE_CONFIG_DIR = userConfig
  ...
  it('rebuilds when the cached overlay lost its plugin file', () => {
    withHome((home) => {
      ...
      rmSync(join(dir, 'plugins', 'manta-opencode-status.js'))
  ...
  it('re-materializes when the shipped source changes', () => {
    withHome((home) => {
      ...
      // Why: a mid-session Manta upgrade ships new plugin source; future spawns must see it.
      // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: Runtime validation or the local test fixture establishes the asserted shape.
      const dir = install({ opencodePluginSource: '// v2\n' }).overlayDirs.opencode as string
      expect(readFileSync(join(dir, 'plugins', 'manta-opencode-status.js'), 'utf8')).toBe('// v2\n')
    })
  })
  ...
  it('mirrors an explicitly-set config root so overriding the var does not drop it', () => {
    withHome((home) => {
      ...
      const install = createInstallPluginsHandler(new PluginOverlayManager({ homeDir: home }), {
        HOME: home,
        MANTA_OPENCODE_SOURCE_CONFIG_DIR: userConfig,
        MANTA_WSL_HOOK_INSTANCE: 'inst1'
      })
      // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: Runtime validation or the local test fixture establishes the asserted shape.
