...
const ENVIRONMENTS_FILE = 'manta-environments.json'
...
export function addEnvironmentFromPairingCode(
  ...
  if (!offer) {
    throw new RuntimeEnvironmentStoreError(
      'invalid_argument',
      'Invalid pairing code. Expected an manta://pair?... URL or bare pairing payload.'
    )
  }
...
function readEnvironmentStore(userDataPath: string): RuntimeEnvironmentStore {
  ...
  try {
    ...
    throw new RuntimeEnvironmentStoreError(
      'runtime_error',
      `Could not read Manta environments at ${path}; the file is invalid.`
    )
  }
}
...
function writeEnvironmentStore(userDataPath: string, store: RuntimeEnvironmentStore): void {
  ...
  try {
    ...
    if (error instanceof JsonStringifyByteLimitError) {
      throw new RuntimeEnvironmentStoreError(
        'runtime_error',
        `Could not write Manta environments at ${path}; the store exceeds its durable capacity.`
      )
    }
    throw error
  }
