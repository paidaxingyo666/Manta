...
describe('classifyDaemonSpawnerPath', () => {
  ...
  it('classifies the installed app, the ShipIt staging area, and everything else', () => {
    expect(
      classifyDaemonSpawnerPath('/Applications/Manta.app/Contents/MacOS/Manta', alwaysExists)
    ).toBe('applications')
    expect(
      classifyDaemonSpawnerPath('/private/Applications/Manta.app/Contents/MacOS/Manta', alwaysExists)
    ).toBe('applications')
    expect(
      classifyDaemonSpawnerPath(
        '/Users/a/Library/Caches/cn.sh.manta.ShipIt/update.abc/Manta.app/Contents/MacOS/Manta',
        alwaysExists
      )
    ).toBe('updater-cache')
    expect(
      classifyDaemonSpawnerPath('/Users/a/Applications/Manta.app/Contents/MacOS/Manta', alwaysExists)
    ).toBe('other')
    expect(classifyDaemonSpawnerPath('/tmp/MantaA.app/Contents/MacOS/Manta', alwaysExists)).toBe(
      'other'
    )
  })
  ...
  it('reports a deleted spawner as missing and an unrecorded one as unknown', () => {
    expect(
      classifyDaemonSpawnerPath('/Applications/Manta.app/Contents/MacOS/Manta', () => false)
    ).toBe('missing')
...
describe('daemon_adopted / daemon_pty_cwd_denied schemas', () => {
  ...
  it('rejects leaked paths, versions, counts, and unknown enum values', () => {
    for (const leak of [
      { spawner_exec_path: '/Users/alice/Library/Caches/ShipIt/Manta.app' },
      { app_version: '1.4.187' },
