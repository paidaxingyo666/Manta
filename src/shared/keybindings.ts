...
export type TerminalShortcutPolicy = 'manta-first' | 'terminal-first'
...
export const KEYBINDING_DEFINITIONS: readonly KeybindingDefinition[] = [
  ...
  },
  {
    ...
    // Why: macOS uses Shift+Space as an input-source shortcut; Manta otherwise rejects Shift-only bindings to avoid stealing typed text.
...
export function normalizeTerminalShortcutPolicy(
  ...
  return policy === 'terminal-first' ? 'terminal-first' : 'manta-first'
}
...
export function keybindingIsActiveInContext(
  ...
  // Why: Manta-first keeps app shortcuts inside terminals; terminal-first is the escape hatch for shells and TUIs.
  if (normalizeTerminalShortcutPolicy(options.terminalShortcutPolicy) === 'manta-first') {
    return true
  }
