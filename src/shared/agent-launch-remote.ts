...
/**
 * Why: a repo reached over SSH runs the Manta CLI through the relay shim, which is always deployed
 * as plain `manta` (Unix) / `manta.cmd` (Windows). The Linux-only `manta-ide` rename — which exists
 * solely to avoid shadowing the GNOME Orca screen reader on a local desktop — must not be applied
 * to those remotes, or `manta-ide claude-teams` lands on a PATH where it does not exist.
 *
 ...
 *
 *   - a row carrying only `executionHostId: 'ssh:<target>'` reads as local and gets the `manta-ide`
 ...
 * shim), while a runtime host with no nested target is a full Manta install and stays false — as do
