...
it.skipIf(process.platform !== 'win32').each(['cmd', 'powershell'] as const)(
  ...
  async (shell) => {
    ...
    try {
      ...
      await writeFile(
        join(root, 'omp.cmd'),
        '@echo off\r\necho %MANTA_OMP_PREFILL%>>"%CAPTURE%"\r\nexit /b 17\r\n'
      )
          ...
          shell === 'cmd'
            ...
            : await runProcess({
                ...
                args: [
                  ...
                  '-NonInteractive',
                  '-Command',
                  `function omp { Add-Content -LiteralPath $env:CAPTURE -Value $env:MANTA_OMP_PREFILL; $global:LASTEXITCODE = 17 }; ${plan.launchCommand}; $result = $LASTEXITCODE; if (Test-Path Env:MANTA_OMP_PREFILL) { exit 91 }; exit $result`
                ],
                cwd: root,
                env
              })
