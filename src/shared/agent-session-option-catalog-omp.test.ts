...
describe('omp session option catalog', () => {
  ...
  it('switches mid-session with /manta-model <selector>, which omp resolves exactly', () => {
    ...
    expect(command).toBe('/manta-model minimax-cn/MiniMax-M3')
    expect(parseBuiltSessionOptionCommand(midSession.build, command)).toBe('minimax-cn/MiniMax-M3')
    expect(parseBuiltSessionOptionCommand(midSession.build, '/manta-model ')).toBeNull()
  })
})
