...
const QUERY_PLACEHOLDER = '__MANTA_HERMES_STARTUP_QUERY__'
const QUERY_ARG_PLACEHOLDER = `--query=${QUERY_PLACEHOLDER}`
const POSIX_QUERY_VARIABLE = '__manta_hermes_startup_query'
const POWERSHELL_QUERY_VARIABLE = 'mantaHermesStartupQuery'
const POWERSHELL_NATIVE_QUERY_VARIABLE = 'mantaHermesNativeQuery'

export const MANTA_HERMES_STARTUP_QUERY_ENV = 'MANTA_HERMES_STARTUP_QUERY'
...
function stripMantaOwnedHermesArgs(args: readonly string[]): string[] {
...
function normalizeHermesArgv(
  ...
  return [
    ...
    ...stripMantaOwnedHermesArgs(baseArgsWithoutChat),
    ...stripMantaOwnedHermesArgs(configuredArgsWithoutChat),
    '--tui'
  ]
}
...
function buildQueryCommand(argv: string[], shell: AgentStartupShell): string {
  if (!isPosixStartupShell(shell)) {
    ...
    const script = `$${POWERSHELL_QUERY_VARIABLE} = $env:${MANTA_HERMES_STARTUP_QUERY_ENV}; $${POWERSHELL_NATIVE_QUERY_VARIABLE} = $${POWERSHELL_QUERY_VARIABLE} -replace '(\\\\*)"', '$1$1\\"'; Remove-Item Env:${MANTA_HERMES_STARTUP_QUERY_ENV} -ErrorAction SilentlyContinue; ${invocation}`
  ...
  const script = `${POSIX_QUERY_VARIABLE}="\${${MANTA_HERMES_STARTUP_QUERY_ENV}}"; unset ${MANTA_HERMES_STARTUP_QUERY_ENV}; eval "$(printf %b "${encodedInvocation}")"`
...
export function planHermesStartupQuery(args: {
  ...
  const env = { ...args.agentEnv, [MANTA_HERMES_STARTUP_QUERY_ENV]: args.prompt }
