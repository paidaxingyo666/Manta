...
import type { PersistedTrustedMantaHooks } from './manta-yaml-hook-types'
...
export type PersistedUIState = {
  ...
  /** Hide workspaces created through `manta worktree create`. */
  ...
  /** Whether Manta already tried triggering the macOS notification permission dialog; prevents re-firing every launch. */
  ...
  /** Why: Windows-only. Set once on first hide to tray so the "Manta is still running" notice shows only once. */
  ...
  /** Once the user has starred Manta (any entry point), permanently suppress the nag. */
