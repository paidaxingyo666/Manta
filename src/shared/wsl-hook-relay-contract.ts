...
export const WSL_HOOK_RELAY_DIR = '.manta-wsl/hook-relay'
 ...
 *  namespaces the guest install dir, so concurrent Manta instances with
 *  different bundle versions (dev + prod) never reinstall over each other. */
export const WSL_HOOK_RELAY_VERSION_ENV = 'MANTA_WSL_HOOK_RELAY_VERSION'
 ...
 *  after a Manta restart — the exact re-coordination this exists to serve. */
export const WSL_HOOK_RELAY_INSTANCE_ENV = 'MANTA_WSL_HOOK_INSTANCE'
...
export function wslHookRelayEndpointDir(guestHome: string, instanceKey: string): string {
  ...
  return `${home}/.manta-wsl/agent-hooks/instance-${instanceKey}`
}
