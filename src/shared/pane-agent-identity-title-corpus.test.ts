...
function mantaAppSupportCandidates(): string[] {
  ...
  return [
    join(process.env.XDG_CONFIG_HOME ?? join(homedir(), '.config'), 'Manta'),
    join(process.env.XDG_DATA_HOME ?? join(homedir(), '.local', 'share'), 'Manta')
  ]
}
...
function loadRecordedTitleCorpus(): { checkpointCount: number; titles: string[] } | null {
  const root = mantaAppSupportCandidates()
