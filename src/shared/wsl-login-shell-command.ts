...
export function buildWslCapturedLoginShellCommand(
  ...
  const begin = `__MANTA_WSL_CAPTURE_BEGIN_${nonce}__`
  const end = `__MANTA_WSL_CAPTURE_END_${nonce}__`
  return {
    beginMarker: begin,
    endMarker: end,
    command: buildWslLoginShellCommand(
      [
        ...
        command,
        '_manta_capture_status=$?',
        `printf %s ${quotePosixShell(end)}`,
        'exit $_manta_capture_status'
      ].join('\n')
    ),
    readStdout: (stdout) => {
...
export function buildWslInteractiveLoginShellCommand(): string {
  return [
    ...
    // resolved root and WSLENV /p-translates it. The MANTA_USER_DATA_PATH branch
    // stays as the fallback for an older host that exports only that.
    'if [ -n "${MANTA_SHELL_READY_ROOT:-}" ]; then',
    '  _manta_shell_ready_root="${MANTA_SHELL_READY_ROOT%/}"',
    'elif [ -n "${MANTA_USER_DATA_PATH:-}" ]; then',
