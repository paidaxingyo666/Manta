 ...
 * MDE incident against Manta. These shims are generated files whose entire body
...
function readParsedShim(program: string): ParsedWindowsCmdShim | null {
  ...
  // `.cmd` path Manta ever spawns; reaching it means a re-read, not a wrong
  // answer.
...
function resolveShimNode(directory: string, env: NodeJS.ProcessEnv): string | null {
  ...
  // per-process and one entry per distinct shim directory Manta ever spawns from.
