 ...
 * ONE emitted string is correct in sh, bash, zsh, dash and fish alike, so Manta
...
/** Every Unix shell on this machine a queued Manta command line could land in. */
...
const SANDBOX_HOME = mkdtempSync(path.join(tmpdir(), 'manta-shell-portability-'))

/** Env with no user shell config reachable, so only Manta's own text is exercised. */
  ...
  () => {
    ...
    describe.each(SHELLS)('$name', (shell) => {
      ...
      it('clears an exported variable with no wrapper installed', () => {
        const clear = clearEnvCommand('MANTA_PI_PREFILL', 'posix')
        const probe =
          shell.name === 'fish'
            ? `set -gx MANTA_PI_PREFILL draft; ${clear}; set -q MANTA_PI_PREFILL; and echo STILL; or echo CLEARED`
            : `MANTA_PI_PREFILL=draft; export MANTA_PI_PREFILL; ${clear}; echo "\${MANTA_PI_PREFILL:+STILL}\${MANTA_PI_PREFILL:-CLEARED}"`
        expect(runInShell(shell, probe).trim()).toBe('CLEARED')
      })

      it('clears several variables in one statement', () => {
        const clear = clearEnvCommand(['MANTA_A', 'MANTA_B'], 'posix')
        const probe =
          shell.name === 'fish'
            ? `set -gx MANTA_A 1; set -gx MANTA_B 2; ${clear}; set -q MANTA_A; or set -q MANTA_B; and echo STILL; or echo CLEARED`
            : `MANTA_A=1 MANTA_B=2; export MANTA_A MANTA_B; ${clear}; echo "\${MANTA_A:+STILL}\${MANTA_B:+STILL}\${MANTA_A:-CLEARED}"`
        expect(runInShell(shell, probe).trim()).toBe('CLEARED')
      })
      ...
      it.each([
        ...
        const setUp = preset
          ? shell.name === 'fish'
            ? 'set -gx MANTA_PI_PREFILL draft; '
            : 'MANTA_PI_PREFILL=draft; export MANTA_PI_PREFILL; '
          : ''
      ...
      // interactive shell, which is the mode Manta types into — a `-c` run cannot
      ...
      it.runIf(['bash', 'zsh'].includes(shell.name))(
        ...
        () => {
          ...
          const script = [
            ...
            'MANTA_PI_PREFILL=draft; export MANTA_PI_PREFILL',
            clear,
            'echo "RESULT=${MANTA_PI_PREFILL:+STILL}${MANTA_PI_PREFILL:-CLEARED}"'
          ].join('\n')
      ...
      // observed. Interactively — the mode Manta types into — they survive without
      ...
      it.runIf(shell.name === 'fish')('never deletes a lone universal variable', () => {
        const clear = clearEnvCommand('MANTA_PI_PREFILL', 'posix')
        const probe = `set -Ux MANTA_PI_PREFILL persisted; ${clear}; set -q MANTA_PI_PREFILL; and echo "RESULT=$MANTA_PI_PREFILL"; or echo RESULT=DESTROYED; set -Ue MANTA_PI_PREFILL`
      ...
      it.runIf(shell.name === 'fish')('reveals a shadowed universal instead of deleting it', () => {
        const clear = clearEnvCommand('MANTA_PI_PREFILL', 'posix')
        const probe = `set -Ux MANTA_PI_PREFILL mine; set -gx MANTA_PI_PREFILL injected; ${clear}; echo "RESULT=$MANTA_PI_PREFILL"; set -Ue MANTA_PI_PREFILL`
