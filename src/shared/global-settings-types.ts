...
export type GlobalSettings = {
  ...
  workspaceDirHistory?: MantaWorkspaceLayout[]
  ...
  /** When enabled, Manta renames a workspace's auto-generated creature branch to
  ...
  /** macOS: toggles the additive menu-bar entry (Manta survives last-window close); doesn't change Dock behavior. */
  ...
  /** Whether to show the Manta app name in the titlebar. */
  ...
  /** Only toggles the sidebar shortcut; Manta Mobile stays reachable from Settings. */
  ...
  /** Manta-first keeps app shortcuts from TUIs; terminal-first is opt-in to let shell/TUI bindings win. */
  ...
  /** Legacy keyboard-shortcut overrides; new writes go to ~/.manta/keybindings.json, migrated once when present. */
  ...
  /** Why: durable main-owned pref so Manta can prepare shared ~/.codex before the renderer hydrates. */
   ...
   *  History-only: does not change which account/config/hooks Manta uses. */
  ...
  /** When true, Manta requests local awake assertions while hook-reported agents are working. */
