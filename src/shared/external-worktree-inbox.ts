...
function isUserFacingExternalWorktree(worktree: DetectedWorktree): boolean {
  ...
  return (
    ...
    worktree.ownership !== 'manta-managed' &&
...
function isImportableExternalWorktree(worktree: DetectedWorktree): boolean {
  return !worktree.selectedCheckout && worktree.ownership !== 'manta-managed'
}
...
export function getVisibleNonMantaWorktrees(
  detected: DetectedWorktreeListResult | undefined
): DetectedWorktree[] {
  if (detected?.authoritative !== true) {
    return []
  ...
  return detected.worktrees.filter(
    (worktree) =>
      worktree.visible && !worktree.selectedCheckout && worktree.ownership !== 'manta-managed'
  )
}
