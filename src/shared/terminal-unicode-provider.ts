...
class MantaUnicodeProvider implements IUnicodeVersionProvider {
  public readonly version = MANTA_UNICODE_VERSION
...
export function activateMantaTerminalUnicodeProvider(terminal: XtermTerminalWithUnicodeCore): void {
  const { unicode } = terminal
  if (unicode.activeVersion === MANTA_UNICODE_VERSION) {
    return
  }
  ...
  if (!unicode.versions.includes(MANTA_UNICODE_VERSION)) {
    unicode.register(new MantaUnicodeProvider(baseProvider))
  }
  unicode.activeVersion = MANTA_UNICODE_VERSION
}
