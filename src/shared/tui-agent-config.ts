...
import { getMantaCliCommandNameForPlatform } from './manta-cli-command-name'
...
const TUI_AGENT_CONFIG_SOURCE: Record<TuiAgent, TuiAgentConfigSource> = {
  ...
  'claude-agent-teams': {
    // Why: a Manta-provided launch mode, not a separate binary; detection follows the Manta CLI.
    detectCmd: 'manta',
    detectCmdAliases: ['manta-dev', 'manta-ide'],
    // Why: require Claude too so fresh installs (Manta shim always present) don't report Agent Teams without an agent CLI.
    detectRequiredCommands: ['claude'],
    // Why: Windows/WSL use Claude's in-process Agent Teams fallback, not this Manta native-pane/tmux-shim wrapper.
    detectUnsupportedRuntimes: ['win32', 'wsl'],
    launchCmd: 'manta claude-teams',
    launchCmdByPlatform: {
      linux: `${getMantaCliCommandNameForPlatform('linux')} claude-teams`,
  ...
  pi: {
    detectCmd: 'pi',
    promptInjectionMode: 'argv',
    // Why: pi has no `--prefill` and paste-after-ready races its long startup; the manta-prefill extension seeds this env var instead.
    draftPromptEnvVar: 'MANTA_PI_PREFILL',
  ...
  omp: {
    detectCmd: 'omp',
    promptInjectionMode: 'argv',
    draftPromptEnvVar: 'MANTA_OMP_PREFILL',
  ...
  hermes: {
    detectCmd: 'hermes',
    // Why: bare `hermes` opens the classic REPL; `--tui` starts the full-screen agent UI Manta hosts.
