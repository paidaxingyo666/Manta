...
export type AgentStateHistoryEntry = {
   ...
   *  or Manta's guarded fallback). Always falsy for non-`done` states so retention logic can preserve it. */
  interrupted?: boolean
}
...
export type AgentStatusEntry = {
   ...
   *  (agent-reported or Manta's guarded fallback). Undefined otherwise. */
  interrupted?: boolean
   ...
   *  Used only for exact CLI resume; Manta terminal ids are not agent-session ids. */
...
function normalizeAgentStatusObject(parsed: unknown): ParsedAgentStatusPayload | null {
  ...
  return {
    ...
    agentType: normalizeOptionalField(obj.agentType, AGENT_TYPE_MAX_LENGTH),
    model: normalizeOptionalField(obj.model, AGENT_MODEL_MAX_LENGTH),
    ...(obj.modelSwitchCommand === 'manta-model'
      ? { modelSwitchCommand: 'manta-model' as const }
      : {}),
