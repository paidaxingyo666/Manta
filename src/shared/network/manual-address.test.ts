...
describe('parseManualNetworkAddress', () => {
  ...
  describe('full URL', () => {
    ...
    it('accepts reverse-proxy paths and query strings', () => {
      expect(parseManualNetworkAddress('wss://example.com/manta?route=runtime').ok).toBe(true)
    })

    it('accepts HTTP URLs that the main process normalizes to WebSocket URLs', () => {
      expect(parseManualNetworkAddress('http://example.com/manta').ok).toBe(true)
      expect(parseManualNetworkAddress('https://example.com/manta?route=runtime').ok).toBe(true)
    })
