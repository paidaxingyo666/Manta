...
import { activateMantaTerminalUnicodeProvider } from './terminal-unicode-provider'
 ...
 *  (pane-dom-creation.ts) and the Manta ZWJ provider (pane-lifecycle.ts).
...
export function createRendererParityTerminal(dims: { cols: number; rows: number }): ParityTerminal {
  ...
  activateMantaTerminalUnicodeProvider(terminal)
 ...
 *    while Manta's WebGL glyph renderer skips decorations on null cells.
 ...
 *  MantaRuntime.serializeHeadlessTerminalBuffer (normal buffer separated from
