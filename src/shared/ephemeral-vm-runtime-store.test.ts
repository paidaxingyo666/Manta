...
describe('ephemeral VM runtime store', () => {
  ...
  function makeUserDataPath(): string {
    const userDataPath = mkdtempSync(join(tmpdir(), 'manta-ephemeral-vm-store-'))
  ...
  it('updates lifecycle and cleanup state', () => {
    ...
    const failed = updateEphemeralVmRuntimeStatus(userDataPath, 'manta-instance-1', {
    ...
    const recovered = updateEphemeralVmRuntimeStatus(userDataPath, 'manta-instance-1', {
      status: 'cleaned',
