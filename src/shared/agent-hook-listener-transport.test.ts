...
describe('shared agent-hook-listener', () => {
  ...
  it('normalizes a raw JSON hook body with metadata headers', async () => {
    const req = createReadableRequest({
      ...
      'x-manta-pane-key': paneKey,
      'x-manta-worktree-id': 'repo::/tmp/work',
      'x-manta-agent-hook-env': 'production',
      'x-manta-agent-hook-version': '1'
    })
    const body = readRequestBody(req as unknown as IncomingMessage)
  ...
  it('decodes base64 metadata headers without corrupting path text', () => {
    ...
    const merged = mergeAgentHookRequestHeaders(
      { hook_event_name: 'UserPromptSubmit', prompt: 'hello' },
      {
        ...
        'x-manta-agent-hook-meta': packedMetadata(
          paneKey,
          'tab-1',
          '',
          worktreeId,
  ...
  describe('writeEndpointFile', () => {
    ...
    it('writes the endpoint file atomically with the right contents and mode', () => {
      ...
      expect(text).toContain('MANTA_AGENT_HOOK_PORT=12345')
      expect(text).toContain('MANTA_AGENT_HOOK_TOKEN=abcdef-0123')
      expect(text).toContain('MANTA_AGENT_HOOK_VERSION=1')
      expect(text).toContain('MANTA_AGENT_HOOK_TRANSPORT=raw-json-v1')
