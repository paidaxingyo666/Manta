...
function issue(overrides: Partial<JiraIssue> = {}): JiraIssue {
  return {
    ...
    project: { id: '10', key: 'MANTA', name: 'Manta' },
...
describe('parseJiraIssueUrl', () => {
  it.each([
    [
      'https://company.atlassian.net/browse/manta-123?focusedCommentId=1#comment',
      { issueKey: 'MANTA-123', origin: 'https://company.atlassian.net', sitePath: '' }
    ],
    [
  ...
  it.each([
    'MANTA-123',
    '/browse/MANTA-123',
    'ftp://jira.example.com/browse/MANTA-123',
    'https://user:secret@jira.example.com/browse/MANTA-123',
    'https://jira.example.com/browse/123',
    'https://jira.example.com/browse/-123',
    'https://jira.example.com/browse/MANTA_123',
    'https://jira.example.com/browse/MANTA-X',
    'https://jira.example.com/browse/MANTA-123/extra',
    'https://jira.example.com/browse/MANTA-123/'
  ])('rejects %s', (value) => {
...
describe('Jira site and issue matching', () => {
  it('matches the complete origin and base path while retaining duplicate accounts', () => {
    const parsed = parseJiraIssueUrl('https://jira.company.com:8443/jira/browse/MANTA-123')!
  ...
  it('requires the key, site id, and canonical URL site to agree', () => {
    const parsed = parseJiraIssueUrl('https://company.atlassian.net/browse/MANTA-123')!
    ...
    expect(isResolvedJiraIssueMatch(parsed, connectedSite, issue({ key: 'MANTA-124' }))).toBe(false)
    expect(isResolvedJiraIssueMatch(parsed, connectedSite, issue({ siteId: 'other' }))).toBe(false)
    expect(
      isResolvedJiraIssueMatch(
        parsed,
        connectedSite,
        issue({ url: 'https://other.atlassian.net/browse/MANTA-123' })
      )
    ).toBe(false)
  })
})
