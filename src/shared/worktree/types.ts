...
export type Worktree = {
  ...
  /** Set once when Manta creates the worktree. Absent for worktrees discovered
  ...
  createdAt?: number
  /** Agent selected when Manta originally created the worktree. Used only to
  ...
  baseRef?: string
  /** Remote/branch Manta should publish review commits to when it created this worktree. */
...
/** Provenance for workspaces created through `manta worktree create`. Absent on
...
export type CliWorkspaceProvenance = {
  kind: 'created-by-cli'
  createdAt: number
  /** Manta terminal the CLI ran inside, when the caller had one — distinguishes
...
export type GitPushTarget = {
  ...
  branchName: string
  remoteUrl?: string
  /** True when Manta added this remote while preparing a fork-PR worktree. */
...
export type WorktreeOwnership = 'manta-managed' | 'external' | 'unknown-legacy' | 'agent-scratch'
