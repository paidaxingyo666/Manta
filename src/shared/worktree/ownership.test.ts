...
describe('worktree ownership classification', () => {
  it('treats explicit Manta metadata as managed even outside the workspace root', () => {
    const repo = makeRepo()
    ...
    expect(
      classifyWorktreeOwnership({
        ...
        meta: makeMeta({ mantaCreatedAt: 1 }),
        knownMantaLayouts: buildKnownMantaWorkspaceLayouts(settings, repo)
      })
    ).toBe('manta-managed')
  })

  it('treats nested Manta workspace paths without metadata as external', () => {
    const repo = makeRepo()
    const settings = makeSettings()
    ...
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({ path: '/manta/workspaces/app/feature' }),
    ...
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({ path: '/manta/workspaces/other/feature' }),
        knownMantaLayouts: layouts
      })
    ).toBe('external')
  })

  it('treats explicit Manta creation layout metadata as managed', () => {
    const repo = makeRepo()
    const settings = makeSettings()
    expect(
      classifyWorktreeOwnership({
  ...
  it('does not treat metadata-free nested workspace paths as Manta-managed for new repos', () => {
    ...
    const detected = toDetectedWorktree({
      ...
      worktree: makeWorktree({
        path: '/manta/workspaces/app/manual-git-worktree',
  ...
  it('does not treat generic discovery metadata on nested workspace paths as Manta-managed', () => {
    const repo = makeRepo({ externalWorktreeVisibility: 'hide' })
    const settings = makeSettings()
    const detected = toDetectedWorktree({
      repo,
  ...
  it('treats flat workspace-root descendants as unknown legacy without strong metadata', () => {
    ...
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({ path: '/manta/workspaces/feature' }),
        knownMantaLayouts: buildKnownMantaWorkspaceLayouts(settings, repo)
      })
    ).toBe('unknown-legacy')
  })
  ...
  it('keeps flat-layout history weak after switching the same root to nested mode', () => {
    ...
    const settings = makeSettings({
      nestWorkspaces: true,
      workspaceDirHistory: [{ path: '/manta/workspaces', nestWorkspaces: false }]
    })
    expect(
      classifyWorktreeOwnership({
        repo,
  ...
  it('handles Windows drive casing and separators', () => {
    ...
    const settings = makeSettings({ workspaceDir: 'C:\\Manta\\Workspaces' })
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({
          id: 'repo-1::C:\\MANTA\\WORKSPACES\\App\\Feature',
          path: 'C:\\MANTA\\WORKSPACES\\App\\Feature',
          isMainWorktree: false
        }),
        knownMantaLayouts: buildKnownMantaWorkspaceLayouts(settings, repo)
      })
...
describe('agent scratch worktrees', () => {
  ...
  it('classifies scratch worktrees created inside another linked checkout', () => {
    const repo = makeRepo()
    const settings = makeSettings()
    const linkedCheckoutPath = '/manta/workspaces/app/feature-x'
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
  ...
  it('keeps strong Manta metadata authoritative over the scratch path match', () => {
    const repo = makeRepo()
    const settings = makeSettings()
    expect(
      classifyWorktreeOwnership({
