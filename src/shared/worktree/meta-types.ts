...
import type { EphemeralVmCheckoutMode } from '../manta-yaml-hook-types'
import type {
...
import type { MantaWorkspaceLayout } from '../global-settings-types'
...
export type WorktreeMeta = {
  ...
  /** See {@link Worktree.createdAt}. Persisted to manta-data.json. */
  createdAt?: number
  /** See {@link Worktree.createdWithAgent}. Persisted to manta-data.json. */
  ...
  baseRef?: string
  /** True when Manta checked out a pre-existing local branch that delete must not prune. */
  preserveBranchOnDelete?: boolean
  /** See {@link Worktree.pushTarget}. Persisted so refreshed worktree lists keep the target. */
  pushTarget?: GitPushTarget
  /** Explicit marker stamped when Manta creates the worktree. */
  mantaCreatedAt?: number
  mantaCreationSource?: 'desktop' | 'runtime' | 'cli' | 'ssh'
  /** Workspace layout active when Manta created the worktree. */
  mantaCreationWorkspaceLayout?: MantaWorkspaceLayout
  ...
  /** System-owned provenance for workspaces created via `manta worktree create`. */
