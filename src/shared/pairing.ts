...
export function encodePairingOffer(offer: PairingOffer): string {
  ...
  return `manta://pair?code=${base64url}`
}
...
export function decodePairingOffer(url: string): PairingOffer {
  ...
  if (!code) {
    throw new Error('Invalid pairing URL: must start with manta://pair and include a pairing code')
  }
...
function extractPairingCodeFromUrl(url: string): string | null {
  ...
  // Why: prefix checks accepted routes like `manta://pairing?...`; only the
  // pairing deep-link host may carry runtime auth material.
  if (parsed.protocol !== 'manta:' || parsed.hostname !== 'pair') {
    return null
  }
...
// Why: accept either an `manta://pair?...` URL or the bare base64
...
export function parsePairingCode(input: string): PairingOffer | null {
  ...
  try {
    if (trimmed.toLowerCase().startsWith('manta://')) {
