...
export function verifyRemotePairingRuntimeStatus(
  ...
  if (status.deviceScope === 'mobile') {
    return {
      ...
      message: 'This link grants mobile-only access. Generate a link for another Manta client.'
    }
  }
  const versionFields = [
    ...
    return {
      ...
      message:
        compatibility.reason === 'client-too-old'
          ? 'Update this Manta client before adding the remote host.'
          : 'Update Manta on the remote host before adding it.'
    }
  }
