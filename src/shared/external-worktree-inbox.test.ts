...
describe('external worktree inbox', () => {
  ...
  it('returns only hidden external worktrees outside the inbox baseline', () => {
    ...
    const detected = detectedResult([
      ...
      detectedWorktree({ id: 'manta-managed', ownership: 'manta-managed' })
    ])
  ...
  it('excludes explicitly visible agent scratch from visibility-control counts', () => {
    ...
    expect(getVisibleNonMantaWorktrees(detectedResult([visible, scratch]))).toEqual([
      visible,
      scratch
    ])
  })
  ...
  it('offers metadata-free nested Manta workspace worktrees through the inbox', () => {
    const settings = makeSettings()
    const manual = toDetectedWorktree({
      repo,
      settings,
      worktree: makeGitWorktree({
        path: '/manta/workspaces/manta/manual-from-git',
