...
it.each(
  ...
  async ({ shell, nounset }) => {
    ...
    try {
      ...
      const define =
        shell === 'fish'
          ? 'function omp; printf "%s\\n" "$MANTA_OMP_PREFILL" >> "$CAPTURE"; printf agent-stderr >&2; return 17; end; '
          : 'omp() { printf "%s\\n" "$MANTA_OMP_PREFILL" >> "$CAPTURE"; printf agent-stderr >&2; return 17; }; '
      const observe =
        shell === 'fish'
          ? '; set -l result $status; set -q MANTA_OMP_PREFILL; and exit 91; exit $result'
          : '; result=$?; test -z "${MANTA_OMP_PREFILL+x}" || exit 91; exit "$result"'
