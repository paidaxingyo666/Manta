...
/**
 * The community `skills` CLI's own `--agent` key for each agent Manta detects.
 *
 ...
 * rather than guessed. Manta ids and skills keys agree less often than they look
 * (`claude` is `claude-code`, `rovo` is `rovodev`, `aug` is `augment`), and some
 * near-matches are different products — Manta's `aider` CLI is not the CLI's
 * `aider-desk`, and Manta's `openclaude` is its `openclaw` in name only, so it
 * follows Manta's own rule that OpenClaude reads Claude-owned roots.
 */
export const SKILLS_CLI_AGENT_KEY_BY_TUI_AGENT = {
  claude: 'claude-code',
  'claude-agent-teams': 'claude-code',
  // Why: Manta states OpenClaude reads Claude-owned roots (native-chat-agent-profiles).
 ...
 * so agents Manta cannot map still receive the skill.
 */
...
/** Map detected Manta agents onto `skills --agent` keys, plus the universal target. */
