...
for (const [agent, config] of Object.entries(TUI_AGENT_CONFIG) as [
  ...
  for (const candidate of [
    ...
    if (normalized) {
      // Why: claude-agent-teams is a Manta wrapper whose child process is the
...
function tokenLooksExecutable(token: string, index: number, firstNormalized: string): boolean {
  ...
  // agents ("compare opencode vs manta"), and treating every argv token as an
...
export function recognizeAgentProcessFromCommandLine(
  ...
  // Why: the generic Manta CLI is not an agent; only this subcommand launches its TUI mode.
