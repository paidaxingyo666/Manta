...
const FUNCTION_NAME = '__manta_omp_draft'
...
export function withOmpDraftCleanup(command: string, shell: AgentStartupShell): string {
  if (shell !== 'posix') {
    ...
    return `${launch}${commandSeparator(shell)}${clearEnvCommand('MANTA_OMP_PREFILL', shell)}`
  }
  const fish = `function ${FUNCTION_NAME}; ${command}; set -l __manta_status $status; set -e -g MANTA_OMP_PREFILL; return $__manta_status; end`
  const posix = `${FUNCTION_NAME}() { ${command}; set -- "$?"; unset MANTA_OMP_PREFILL; return "$1"; }`
