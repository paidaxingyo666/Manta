...
import type { RepoHookSettings } from './manta-yaml-hook-types'
...
export type Repo = {
   ...
   * At spawn, Manta resolves a short-lived token via `gh auth token --user`
   * and injects it into that child env only.
   */
  ghAccount?: GhAccountBinding
  /** Controls Manta's fork-default-branch sync offer for repos with upstream metadata. */
  forkSyncMode?: ForkSyncMode
  /** Canonical identity for the repo remote Manta should use for provider-level grouping. */
  gitRemoteIdentity?: GitRemoteIdentity | null
  /** Controls whether worktrees Manta did not create appear in the sidebar. */
