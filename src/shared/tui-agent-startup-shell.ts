...
/**
 * `'posix'` covers every Unix shell Manta can type into, fish included — not
 ...
 * is deliberately no fish member: a dialect Manta has to detect is a dialect it
...
/**
 * Quotes one argument so the SAME text is literal in every Unix shell Manta can
 ...
 * of them, and Manta never has to know which shell will read the line.
 *
 ...
 *
 * Why not a helper function defined by Manta's shell wrappers: Manta only wraps
 ...
 * shells Manta never spawned — in all of those a helper would be `command not
 ...
 * clipboard command, which a user may run with no Manta-injected value at all.
