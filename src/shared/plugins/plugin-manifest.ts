...
/**
 * Plugin manifest v1 (`manta-plugin.json` at the plugin root). The
 * `contributes` key names deliberately mirror common Electron-ecosystem
 * manifest conventions so future adapters stay cheap.
 *
 * Lives in `shared` so the desktop app, the headless `manta serve` runtime,
...
const mantaEngineRangeSchema = z
  .string()
  .max(64)
...
export const pluginManifestSchema = z
  .object({
    ...
    engines: z.object({ manta: mantaEngineRangeSchema }),
...
export const PLUGIN_MANIFEST_FILENAME = 'manta-plugin.json'
...
export function satisfiesMantaEngineRange(hostVersion: string, range: string): boolean {
