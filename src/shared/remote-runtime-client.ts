...
function formatRemoteRuntimeCloseMessage(code: number, reason: Buffer): string {
  ...
  return suffixParts.length > 0
    ? `Remote Manta runtime closed the connection (${suffixParts.join(': ')}).`
...
async function sendRemoteRuntimeRequestOnSocket<TResult>(
  ...
  return await new Promise<RuntimeRpcResponse<TResult>>((resolve, reject) => {
    ...
    const cleanupSocketListeners = (): void => {
      ...
      // Why: the settled one-shot no longer needs Manta callbacks, but a ws
    ...
    function onTimeout(): void {
      finish({
        ...
        error: new RemoteRuntimeClientError(
          'runtime_timeout',
          'Timed out waiting for the remote Manta runtime to respond.',
    ...
    function handleAuthenticatedFrame(plaintext: string): void {
      ...
      if (type !== 'e2ee_authenticated') {
        ...
        finish({
          ...
          error: new RemoteRuntimeClientError(
            code,
            'Remote Manta runtime rejected the pairing token.',
    ...
    function sendRequestedRpc(): void {
      ...
      if (request === null) {
        finish({
          ...
          error: new RemoteRuntimeClientError(
            'remote_runtime_unavailable',
            'Remote Manta runtime request was released before it could be sent.'
          )
        })
        return
      }
...
export async function subscribeRemoteRuntimeRequest<TResult>(
  ...
  return await new Promise((resolve, reject) => {
    ...
    const cleanupSocketListeners = (): WebSocket | null => {
      ...
      ws = null
      // Why: startup failures detach Manta callbacks before closing the ws,
      ...
      if (type !== 'e2ee_authenticated') {
        const code =
          ...
          (authenticated as { error?: { code?: unknown } }).error?.code === 'unauthorized'
            ? 'unauthorized'
            : 'invalid_runtime_response'
        fail(new RemoteRuntimeClientError(code, 'Remote Manta runtime rejected the pairing token.'))
        return
      }
      state = 'ready'
      ws?.send(cipher.sealText(serializedRequest))
