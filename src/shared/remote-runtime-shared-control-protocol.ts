...
export function formatSharedControlCloseMessage(code: number, reason: Buffer): string {
  ...
  if (code !== 1005 && code !== 1006 && reasonText) {
    return `Remote Manta runtime closed the connection (${code}: ${reasonText}).`
  }
  if (code !== 1005 && code !== 1006) {
    return `Remote Manta runtime closed the connection (${code}).`
  }
  return 'Remote Manta runtime closed the connection.'
}
