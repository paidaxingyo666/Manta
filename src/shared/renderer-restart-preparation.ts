...
import { MANTA_RENDERER_SHUTDOWN_CHECKPOINT_FAILED_EVENT } from './renderer-shutdown-events'
...
function requestEditorHotExitBackup(eventTarget: EventTarget): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    ...
    eventTarget.dispatchEvent(
      new CustomEvent<EditorPrepareHotExitDetail>(MANTA_EDITOR_PREPARE_HOT_EXIT_EVENT, {
        detail: {
          claim: () => {
            claimed = true
          },
...
export async function prepareRendererForAppRestart(
  ...
  try {
    ...
    eventTarget.addEventListener(
      MANTA_RENDERER_SHUTDOWN_CHECKPOINT_FAILED_EVENT,
