 ...
 * transport (`createPtyOutputProcessor`) and main's per-PTY tracker (`MantaRuntimeService.onPtyData`).
...
export function createTerminalTitleTracker(
  ...
  function applyObservedTitle(rawTitle: string): void {
    // Why: cursor-agent re-emits its bare native title mid-turn; passing it through would stomp Manta's synthesized spinner state.
