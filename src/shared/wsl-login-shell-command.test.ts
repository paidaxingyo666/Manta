...
describe('wsl login shell command helpers', () => {
  ...
  it('runs commands through the distro user login shell', () => {
    ...
    expect(command).toContain('exec "$_manta_wsl_shell" -ilc')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const root = mkdtempSync(join(tmpdir(), 'manta-wsl-login-codex-'))
      ...
      writeFileSync(
        join(tools, 'getent'),
        `#!/bin/sh\nprintf '%s\\n' "user:x:1000:1000::/home/user:$MANTA_TEST_LOGIN_SHELL"\n`
      )
      writeFileSync(
        loginShell,
        '#!/bin/sh\nexport PATH="$MANTA_TEST_CODEX_BIN:/usr/bin:/bin"\nexec /bin/sh -c "$2"\n'
      )
      ...
      const run = (codexBin: string): string =>
        execFileSync('/bin/sh', ['-c', command], {
          ...
          env: {
            ...
            MANTA_TEST_LOGIN_SHELL: loginShell,
            MANTA_TEST_CODEX_BIN: codexBin
          }
        })
  ...
  it('keeps command-scoped environment variables in the quoted payload', () => {
    const command = buildWslLoginShellCommand('HISTFILE=/tmp/manta-history printf "$HISTFILE"')

    expect(command).toContain('\'HISTFILE=/tmp/manta-history printf "$HISTFILE"\'')
  ...
  it('preserves user command variables across the Windows-to-WSL argv boundary', () => {
    ...
    const captured = buildWslCapturedLoginShellCommand('manta_value=ok; printf "<%s>" "$manta_value"')
  ...
  describe('captured login shell', () => {
      ...
      expect(
        captured.readStdout(
          'To run a command as administrator (user "root"), use "sudo <command>".\n\n' +
            '__MANTA_WSL_CAPTURE_BEGIN_nonce1__directory__MANTA_WSL_CAPTURE_END_nonce1__'
        )
      ).toBe('directory')
    })
    ...
    it('emits the status plumbing the payload needs', () => {
      ...
      expect(captured.command).toContain('_manta_capture_status=$?')
      expect(captured.command).toContain('exit $_manta_capture_status')
    })
    ...
    it('keeps payload bytes that themselves contain a fence', () => {
      ...
      const payload = 'see __MANTA_WSL_CAPTURE_BEGIN_nonce1__ and __MANTA_WSL_CAPTURE_END_nonce1__\n'

      expect(
        captured.readStdout(
          `banner\n__MANTA_WSL_CAPTURE_BEGIN_nonce2__${payload}__MANTA_WSL_CAPTURE_END_nonce2__`
        )
      ).toBe(payload)
    })
    ...
    it('returns the tail when a payload exits before the closing fence', () => {
      ...
      expect(captured.readStdout('banner\n__MANTA_WSL_CAPTURE_BEGIN_nonce1__partial')).toBe(
        'partial'
      )
    })
  ...
  it('starts an interactive login shell without assuming bash', () => {
    ...
    expect(command).toContain('getent passwd')
    expect(command).toContain('if [ -z "$_manta_wsl_shell" ] || [ ! -x "$_manta_wsl_shell" ]; then')
    expect(command).toContain('_manta_shell_ready_root=""')
    expect(command).toContain('if [ -n "${MANTA_USER_DATA_PATH:-}" ]; then')
    expect(command).toContain('_manta_wsl_shell_name=$(basename "$_manta_wsl_shell"')
    expect(command).toContain('bash)')
    expect(command).toContain('--rcfile "${_manta_shell_ready_root}/bash/rcfile"')
    expect(command).toContain('zsh)')
    expect(command).toContain('export ZDOTDIR="${_manta_shell_ready_root}/zsh"')
    expect(command).toContain('exec "$_manta_wsl_shell" -l')
    expectValidShSyntax(command)
  })
})
...
describe('in-guest wrapper root resolution', () => {
  ...
  // rebuilt the root as `${MANTA_USER_DATA_PATH}/shell-ready`, which stopped
  ...
  it('prefers the host-published root over the legacy user-data guess', () => {
    ...
    expect(script).toContain('if [ -n "${MANTA_SHELL_READY_ROOT:-}" ]; then')
    expect(script).toContain('_manta_shell_ready_root="${MANTA_SHELL_READY_ROOT%/}"')
    // The legacy branch must remain reachable only as a fallback, so an older
    // host that exports just MANTA_USER_DATA_PATH still wraps its shells.
    expect(script).toContain('elif [ -n "${MANTA_USER_DATA_PATH:-}" ]; then')
  })
  ...
  it('resolves the published root ahead of the legacy path under a real shell', () => {
    ...
    const prologue = script.split('_manta_wsl_shell_name=')[0] as string
    const probe = [
      'MANTA_SHELL_READY_ROOT=/mnt/c/ud/shell-wrappers/deadbeefdeadbeef/shell-ready',
      'MANTA_USER_DATA_PATH=/mnt/c/ud',
      'export MANTA_SHELL_READY_ROOT MANTA_USER_DATA_PATH',
      prologue,
      'printf "%s" "$_manta_shell_ready_root"'
    ].join('\n')
