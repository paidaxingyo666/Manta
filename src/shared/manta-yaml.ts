...
import type {
  ...
  MantaHooks,
  MantaVmRecipe,
  MantaVmRecipeDiagnostic
} from './manta-yaml-hook-types'
import {
  isMantaYamlFieldWithinLimit,
  isMantaYamlTextWithinLimit,
...
function asTrimmedString(value: unknown): string | undefined {
  if (typeof value !== 'string' || !isMantaYamlFieldWithinLimit(value)) {
    return undefined
  }
...
export const MANTA_VM_RECIPE_ID_PATTERN = /^[a-z0-9][a-z0-9._-]{0,63}$/
export const MANTA_VM_RECIPE_ID_RULE =
...
function normalizeDefaultTabs(value: unknown): MantaDefaultTabTemplate[] {
  if (!Array.isArray(value) || value.length > MAX_MANTA_YAML_COLLECTION_ENTRIES) {
    return []
  }
  ...
  return value
    ...
    .filter((entry): entry is MantaDefaultTabTemplate => entry !== null)
}

type VmRecipeParseResult = {
  recipes: MantaVmRecipe[]
  diagnostics: MantaVmRecipeDiagnostic[]
}

function normalizeVmRecipes(value: unknown): VmRecipeParseResult {
  const diagnostics: MantaVmRecipeDiagnostic[] = []
  if (!Array.isArray(value)) {
    return { recipes: [], diagnostics }
  }
  if (value.length > MAX_MANTA_YAML_COLLECTION_ENTRIES) {
    return {
      recipes: [],
      diagnostics: [
        {
          index: MAX_MANTA_YAML_COLLECTION_ENTRIES,
          message: `At most ${MAX_MANTA_YAML_COLLECTION_ENTRIES} environment recipes are supported.`
        }
      ]
    }
  }
  ...
  const recipes = value
    .map((entry, index) => {
      ...
      if (!MANTA_VM_RECIPE_ID_PATTERN.test(id)) {
        diagnostics.push({
          index,
          field: 'id',
          message: `Invalid recipe id "${id}". ${MANTA_VM_RECIPE_ID_RULE}`
        })
        return null
      }
      if (seenIds.has(id)) {
      ...
      if (
        ...
        diagnostics.push({
          index,
          field: 'checkoutMode',
          message: `Recipe "${id}" checkoutMode must be "manta-worktree" or "provisioned-root".`
        })
        return null
      }
    ...
    .filter((entry): entry is MantaVmRecipe => entry !== null)
  return { recipes, diagnostics }
}

/**
 * Parse the supported project defaults from `manta.yaml`.
 */
export function parseMantaYaml(content: string): MantaHooks | null {
  if (!isMantaYamlTextWithinLimit(content)) {
    return null
  }
