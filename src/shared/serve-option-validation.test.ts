...
describe('getServeFlagTypoError', () => {
  ...
  it.each(['--no-pair', '--no-pairng', '--no-paring', '--mobile-pairng'])(
    ...
    (flag) => {
      expect(getServeFlagTypoError(['/opt/manta/manta-ide', '--serve', flag])).toMatch(
  ...
  it('does not reinterpret tokens after --', () => {
    expect(getServeFlagTypoError(['/opt/manta/manta-ide', '--serve', '--', '--no-pairng'])).toBeNull()
  })

  it('does not inspect an equals-form value as a flag', () => {
    expect(
      getServeFlagTypoError(['/opt/manta/manta-ide', '--serve-pairing-address=--no-pairng'])
    ).toBeNull()
  })
