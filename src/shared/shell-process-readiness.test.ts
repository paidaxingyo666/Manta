...
describe('shell process readiness', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'manta-shell-path-'))
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'manta-relative-shell-'))
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'manta-shadowed-shell-'))
