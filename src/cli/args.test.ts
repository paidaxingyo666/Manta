      ...
      [
        {
          ...
          summary: 'Permissions',
          usage: 'manta emulator permissions <op> <package> [permission]',
  ...
  const specs: CommandSpec[] = [
    {
      ...
      usage: 'manta worktree rm --worktree <selector>',
      allowedFlags: ['worktree', 'force']
    },
    {
      path: ['repo', 'show'],
      summary: 'Show a repo',
      usage: 'manta repo show --repo <selector>',
      allowedFlags: ['repo'],
      positionalArgs: ['repo']
    }
  ]
...
describe('validateCommandAndFlags', () => {
  ...
  it('attaches did-you-mean suggestions to unknown-command errors', () => {
    ...
    try {
      ...
      expect(data?.nextSteps[0]).toContain('manta worktree rm')
    }
  })
})
