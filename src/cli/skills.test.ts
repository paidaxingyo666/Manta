...
vi.mock('./runtime-client', async () => {
  ...
  return {
    ...
    serveMantaApp: vi.fn(),
    getDefaultUserDataPath: vi.fn(() => '/tmp/manta-user-data')
  }
})
...
describe('manta skills CLI', () => {
  ...
  it('shows leaf, group, and root help for skills', async () => {
    ...
    expect(String(logSpy.mock.calls[1]?.[0])).toContain(
      'install            Install bundled Manta skills'
    )
    expect(String(logSpy.mock.calls[1]?.[0])).toContain(
      'update             Update already-installed Manta skills'
    )
  ...
  it('lists installable skills when no --skill/--all is given', async () => {
    ...
    expect(stdoutText(stdoutSpy)).toBe(
      [
        ...
        '  zeta',
        '',
        'Usage: manta skills install --skill <name> [--skill <name> ...]',
        '   or: manta skills install --all',
        ''
      ].join('\n')
    )
      ...
      JSON.stringify(
        {
          ...
          error: {
            ...
            message:
              "manta skills install --json only supports --dry-run. Real installs stream npx's " +
  ...
  it('lists updatable skills when no --skill/--all is given', async () => {
    ...
    expect(stdoutText(stdoutSpy)).toBe(
      [
        ...
        '  zeta',
        '',
        'Usage: manta skills update --skill <name> [--skill <name> ...]',
        '   or: manta skills update --all',
        ''
      ].join('\n')
    )
    expect(spawnMock).not.toHaveBeenCalled()
  ...
  it('refuses a real run when the shell forwards manta to the Manta host', async () => {
    vi.stubEnv('MANTA_CLI_CWD', '/home/alice/wt')
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {})

    await main(['skills', 'install', '--skill', 'alpha'], '/tmp/repo')

    // Why: the SSH relay and WSL bridge run argv on the Manta host, so a real
  ...
  it('refuses to install when Manta detects no agent, instead of targeting them all', async () => {
  ...
  it('maps detected agents onto the skills CLI namespace, not Manta ids', async () => {
    ...
    // Manta's `claude` is `claude-code` and its `rovo` is `rovodev`.
      ...
      JSON.stringify(
        {
          ...
          error: {
            code: 'invalid_argument',
            message:
              "manta skills update --json only supports --dry-run. Real updates stream npx's " +
              "own output, which isn't JSON."
          },
          _meta: { runtimeId: null }
        },
