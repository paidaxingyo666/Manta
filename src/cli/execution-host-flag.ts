...
export type HostFlagRoutingSelection = {
  // Why: SSH targets live in the running Manta host, not on disk, so enumerating them needs a
  ...
  // Why: an ambient MANTA_ENVIRONMENT counts as a selection too, so carry the label to
...
export async function resolveHostFlagEnvironmentId(
  ...
  if (!environment) {
    ...
    throw new RuntimeClientError(
      'invalid_argument',
      `Unknown Manta server in --host ${host.id}: no paired Manta server is named or has id ${host.environmentId}.`,
      {
        ...
        nextSteps: [
          ...
          'Run `manta environment list` to see paired Manta servers.',
  ...
  if (selection.pairingCode) {
    throw new RuntimeClientError(
      'invalid_argument',
      `--host ${host.id} already selects a paired Manta server; use either --host runtime:<id> or --pairing-code, not both.`
    )
  }
  if (selection.environmentSelector) {
    ...
    if (selected.id !== environment.id) {
      throw new RuntimeClientError(
        'invalid_argument',
        `--host ${host.id} and ${selection.environmentSelector.label} ${selection.environmentSelector.value} name different Manta servers.`
      )
    }
  }
...
function assertEnvironmentNameUnambiguous(
  ...
  throw new RuntimeClientError(
    'invalid_argument',
    `Ambiguous Manta server in ${flag}: ${ambiguous.length} paired servers are named ${name}. Use the environment id.`,
    {
...
export async function assertEnvironmentSelectorResolvable(
  ...
  throw new RuntimeClientError(
    'invalid_argument',
    `Unknown Manta server in --environment ${selector}: no paired Manta server is named or has id ${selector}.`,
    {
      knownEnvironments: environments,
      knownSshTargets: sshTargets,
      nextSteps: [
        ...crossKindNextSteps(selector, { environments, sshTargets }, 'environment'),
        'Run `manta host list` to see every machine you can target and the flag for each.'
      ]
    }
  )
}
