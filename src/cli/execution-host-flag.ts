...
export async function resolveHostFlagEnvironmentId(
  ...
  if (!environment) {
    ...
    throw new RuntimeClientError(
      'invalid_argument',
      `Unknown Manta server in --host ${host.id}: no paired environment has id ${host.environmentId}. --host runtime:<id> matches environment ids only, never names. Run \`manta environment list\` to see paired servers, or pass --host local.`,
      {
