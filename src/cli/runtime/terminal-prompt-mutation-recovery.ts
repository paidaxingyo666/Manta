...
export function attachLegacyTerminalPromptRecovery(error: unknown): unknown {
  ...
  return attachUnknownTerminalPromptRecovery(
    ...
    [
      INSPECT_STEP,
      'Update Manta on the execution host before future prompt sends that need durable retry.'
    ]
  )
}
...
export function attachUnverifiedTerminalPromptRecovery(error: unknown): RuntimeClientError {
  ...
  return attachUnknownTerminalPromptRecovery(
    normalized,
    'Manta cannot prove whether the prompt was delivered by the prompt-delivery-capable runtime from the preflight',
    [
      INSPECT_STEP,
      'A different Manta runtime answered than the one whose prompt-delivery support was verified; confirm which runtime serves this host before sending again.'
    ]
  )
}
