...
describe('CLI remote WebSocket transport', () => {
  ...
  it('accepts a bare pairing payload as well as the manta URL wrapper', async () => {
  ...
  it('does not launch a local desktop app for remote-paired open', async () => {
    ...
    const status = await client.openManta()
  ...
  it('connects through a saved environment selector', async () => {
    ...
    servers.push(runtime)
    const userDataPath = mkdtempSync(join(tmpdir(), 'manta-cli-env-'))
