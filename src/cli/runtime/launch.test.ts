...
import { launchMantaApp, serveMantaApp } from './launch'
...
const SERVE_INSTALL_STATUS = '[serve] manta CLI install: installed'
...
describe('serveMantaApp', () => {
  ...
  afterEach(() => {
    ...
    delete process.env.MANTA_APPIMAGE_NO_SANDBOX
  ...
  it.runIf(process.platform === 'darwin')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'manta-serve-update-'))
      temporaryDirectories.push(root)
      const appPath = join(root, 'Manta.app')
      const executable = join(appPath, 'Contents', 'MacOS', 'Manta')
      ...
      process.env.MANTA_APP_EXECUTABLE = executable
      ...
      const supervisor = serveMantaApp({ json: true }).then((code) => {
        supervisorExited = true
        return code
      })
      const childEnv = spawnMock.mock.calls[0]?.[2]?.env as NodeJS.ProcessEnv | undefined
      const handoffPath = childEnv?.MANTA_SERVE_UPDATE_HANDOFF_PATH
  ...
  it.runIf(process.platform === 'darwin')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'manta-serve-update-mismatch-'))
      temporaryDirectories.push(root)
      const appPath = join(root, 'Manta.app')
      const executable = join(appPath, 'Contents', 'MacOS', 'Manta')
      const userDataPath = join(root, 'user-data')
      ...
      const supervisor = serveMantaApp({ json: true })
  ...
  it.runIf(process.platform === 'darwin')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'manta-serve-update-spawn-failure-'))
      temporaryDirectories.push(root)
      const appPath = join(root, 'Manta.app')
      const executable = join(appPath, 'Contents', 'MacOS', 'Manta')
      const userDataPath = join(root, 'user-data')
  ...
  it.runIf(process.platform === 'darwin')(
    ...
    async () => {
      vi.useFakeTimers()
      const root = await mkdtemp(join(tmpdir(), 'manta-serve-update-no-readiness-'))
      temporaryDirectories.push(root)
      const appPath = join(root, 'Manta.app')
      const executable = join(appPath, 'Contents', 'MacOS', 'Manta')
      const userDataPath = join(root, 'user-data')
  ...
  it('preserves an AppImage no-sandbox launch for the server child', async () => {
    process.env.MANTA_APPIMAGE_NO_SANDBOX = '1'
    const child = {
      kill: vi.fn(),
      once: vi.fn(
        (event: string, handler: (code: number | null, signal: string | null) => void) => {
    ...
    expect(spawnOptions.env).not.toHaveProperty('MANTA_APPIMAGE_NO_SANDBOX')
  })

  it('passes the app root before serve flags for dev Electron executables', async () => {
    process.env.MANTA_APP_EXECUTABLE = '/repo/node_modules/.bin/electron'
    process.env.MANTA_APP_EXECUTABLE_NEEDS_APP_ROOT = '1'
    const child = {
      kill: vi.fn(),
      once: vi.fn(
        (event: string, handler: (code: number | null, signal: string | null) => void) => {
    ...
    await expect(serveMantaApp({ json: true, port: '6768' })).resolves.toBe(0)
  ...
  it('rejects when the server exits without valid recipe JSON', async () => {
    ...
    const untrustedLines = [
      'MANTA://pair?code=UPPER-SECRET',
      'manta://pair/?code=SLASH-SECRET',
      'manta://pair#LEGACY-SECRET',
    ...
    await expect(result).rejects.toMatchObject({
      ...
      message: 'Manta serve exited before printing valid recipe JSON with code 0.'
    })
  ...
  it('uses a shell when a Windows npm command shim is the Electron executable', async () => {
    ...
    process.env.MANTA_APP_EXECUTABLE = 'C:\\repo\\node_modules\\.bin\\electron.cmd'
    const child = {
      kill: vi.fn(),
      once: vi.fn(
        (event: string, handler: (code: number | null, signal: string | null) => void) => {
...
describe('launchMantaApp', () => {
  beforeEach(() => {
    spawnMock.mockReset()
  })

  afterEach(() => {
    delete process.env.MANTA_OPEN_COMMAND
    delete process.env.MANTA_APP_EXECUTABLE
    delete process.env.MANTA_APP_EXECUTABLE_NEEDS_APP_ROOT
  })

  it('handles asynchronous detached spawn errors without throwing', async () => {
    process.env.MANTA_APP_EXECUTABLE = '/missing/Manta'
    const child = new FakeChildProcess()
    spawnMock.mockReturnValue(child)
