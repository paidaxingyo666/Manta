...
describe.skipIf(process.platform === 'win32')('CLI runtime status', () => {
  it('uses the legacy singular runtime transport when reporting status', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'manta-runtime-status-'))
...
// Why: `kill(pid, 0)` answers EPERM when the pid exists under another uid — a Manta the
// CLI was pointed at with MANTA_USER_DATA_PATH, or one started with sudo. Reading that
...
describe('projectRemoteAppStatus', () => {
  ...
  // Why: the SSH host-passthrough answers for the Manta host the caller reached, and used to
