 ...
 * GUI flow drives this via a browser Manta can't reach on a headless host.
 */
...
async function addClaudeAccount({ client, json }: HandlerContext): Promise<void> {
  const configDir = mkdtempSync(join(tmpdir(), 'manta-account-add-claude-'))
...
async function addCodexAccount({ client, json }: HandlerContext): Promise<void> {
  const codexHome = mkdtempSync(join(tmpdir(), 'manta-account-add-codex-'))
  const session: InteractiveLoginSession = {
    child: null,
    registering: false,
    terminationPromise: null
...
async function assertAccountImportSupported({ client }: HandlerContext): Promise<void> {
  ...
  if (!status.result.capabilities?.includes(ACCOUNT_IMPORT_RUNTIME_CAPABILITY)) {
    throw new RuntimeClientError(
      ...
      'The running Manta runtime is too old to add accounts from the CLI. Update or restart Manta and try again.'
    )
  }
}

/** CLI handlers for `manta account add [--agent claude|codex]` and `manta account list`. */
export const ACCOUNT_HANDLERS: Record<string, CommandHandler> = {
  'account add': async (ctx) => {
    ...
    rejectAccountRemoteSelectionFlags(ctx, 'manta account add')
  ...
  'account list': async (ctx) => {
    rejectAccountRemoteSelectionFlags(ctx, 'manta account list')
