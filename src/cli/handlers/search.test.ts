...
describe('manta search over the runtime RPC', () => {
  ...
  it('propagates a transport failure instead of calling it unavailable', async () => {
    await expect(
      runSearch([['query', 'q']], {
        error: new RuntimeClientError('runtime_unavailable', 'Manta is not running.')
      })
    ).rejects.toThrow('Manta is not running.')
  })
