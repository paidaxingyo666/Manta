...
function readDataFile(userDataPath: string): PersistedState {
  return JSON.parse(readFileSync(join(userDataPath, 'manta-data.json'), 'utf-8')) as PersistedState
}

function writeDataFile(userDataPath: string, state: PersistedState): void {
  mkdirSync(userDataPath, { recursive: true })
  writeFileSync(join(userDataPath, 'manta-data.json'), JSON.stringify(state, null, 2), 'utf-8')
}
...
describe('agent hooks CLI handler', () => {
  ...
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'manta-agent-hooks-cli-'))
  ...
  it('forwards WSL pane routing to the runtime exactly once without using the host installer', async () => {
    ...
    vi.stubEnv('MANTA_CODEX_HOME', home)
    ...
    expect(callMock).toHaveBeenCalledExactlyOnceWith(
      ...
      { codexHome: home, mantaCodexHome: home, wslDistro: 'Ubuntu-24.04' },
      { timeoutMs: 50_000 }
    )
  ...
  it('fails open when WSL runtime preparation is unavailable', async () => {
    vi.stubEnv('CODEX_HOME', '/home/jin/.local/share/manta/codex-runtime-home/home')
    vi.stubEnv('MANTA_CODEX_HOME', '/home/jin/.local/share/manta/codex-runtime-home/home')
  ...
  it('uses the active profile settings instead of stale legacy settings', async () => {
    ...
    writeFileSync(
      join(userDataPath, 'manta-profile-index.json'),
      JSON.stringify({
