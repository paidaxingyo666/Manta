  ...
  'terminal read': async ({ flags, client, cwd, json }) => {
    ...
    if (screen && result.result.terminal.source === undefined) {
      throw new RuntimeClientError(
        ...
        'This Manta host does not support --screen reads, so it answered with accumulated output instead of the rendered screen. Update Manta on the host, or drop --screen to read accumulated output deliberately.'
      )
    }
  ...
  'terminal create': async ({ flags, client, cwd, json }) => {
    ...
    if (shell !== undefined) {
      ...
      if (!status.result.runtime.reachable) {
        throw new RuntimeClientError(
          'runtime_unavailable',
          'Manta could not verify --shell support on the execution host, so no terminal was created. Wait for the execution host to become reachable and retry.'
        )
      }
      if (
        ...
        throw new RuntimeClientError(
          'incompatible_runtime',
          'This Manta host does not support --shell, and would silently create a terminal running its default shell instead. No terminal was created; update Manta on the execution host.'
        )
      }
    }
