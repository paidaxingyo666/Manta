...
// and serveMantaApp binding are referenced by the module under test.
...
describe('manta claude-teams CLI handler', () => {
  ...
  beforeEach(() => {
    ...
    previousPaneKey = process.env.MANTA_PANE_KEY
    previousExitCode = process.exitCode
    // The `manta` launcher runs Manta's Electron binary as Node, so the CLI process
  ...
  afterEach(() => {
    ...
    if (previousPaneKey === undefined) {
      ...
      process.env.MANTA_PANE_KEY = previousPaneKey
    }
  ...
  it.skipIf(isWindows)(
    ...
    async () => {
      const previousMarker = process.env.MANTA_TEST_MARKER
      process.env.MANTA_TEST_MARKER = 'keep-me'
      try {
        await runClaudeTeams()
      } finally {
        if (previousMarker === undefined) {
          delete process.env.MANTA_TEST_MARKER
        } else {
          process.env.MANTA_TEST_MARKER = previousMarker
        }
      }

      const spawnEnv = spawnMock.mock.calls.at(-1)?.[2].env as SpawnEnv
      expect(spawnEnv.MANTA_TEST_MARKER).toBe('keep-me')
