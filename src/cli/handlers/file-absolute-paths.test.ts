...
describe('absolute file CLI paths', () => {
  ...
  describe('WSL UNC worktree roots', () => {
    const uncRoot = '//wsl.localhost/Ubuntu/root/manta/workspaces/xxx'
    ...
    it('relativizes a Linux path typed inside the distro against the UNC root', async () => {
      ...
      await openPath('/root/manta/workspaces/xxx/xxx/xxx.ts')
    ...
    it('relativizes against a legacy backslash wsl$ root', async () => {
      vi.stubEnv('WSL_DISTRO_NAME', 'Ubuntu')
      mockWorktreeShow('\\\\wsl$\\Ubuntu\\root\\manta\\workspaces\\xxx')
    ...
    it('leaves a Linux path containing a backslash for the runtime guard', async () => {
      ...
      await openPath('/root/manta/workspaces/xxx/a\\b.ts')

      expectOpenedWith('/root/manta/workspaces/xxx/a\\b.ts')
    })
    ...
    it('leaves a Linux path from another distro for the runtime guard', async () => {
      ...
      expectOpenedWith('/root/manta/workspaces/xxx/xxx/xxx.ts')
    })
    ...
    it('keeps relativizing POSIX roots while WSL_DISTRO_NAME is set', async () => {
      vi.stubEnv('WSL_DISTRO_NAME', 'Ubuntu')
      mockWorktreeShow('/root/manta/workspaces/xxx')
    ...
    it('does not relativize a sibling directory that merely shares the root prefix', async () => {
      ...
      await openPath('/root/manta/workspaces/xxx-2/xxx.ts')

      expectOpenedWith('/root/manta/workspaces/xxx-2/xxx.ts')
    })
    ...
    it('leaves a Linux path whose own case does not match the root', async () => {
      ...
      await openPath('/root/manta/Workspaces/xxx/xxx.ts')

      expectOpenedWith('/root/manta/Workspaces/xxx/xxx.ts')
    })
    ...
    // always sets MANTA_CLI_CWD, which reaches the handler as the invocation cwd.
    it('falls back to the distro named by a UNC cwd', async () => {
      mockWorktreeShow(uncRoot)

      await openPath(
        '/root/manta/workspaces/xxx/xxx/xxx.ts',
        '\\\\wsl.localhost\\Ubuntu\\root\\manta\\workspaces\\xxx\\xxx'
      )
    ...
    it('rejects the UNC worktree root itself typed as a Linux path', async () => {
      ...
      await openPath('/root/manta/workspaces/xxx')
