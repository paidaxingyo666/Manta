...
describe('manta computer observation CLI handlers', () => {
  beforeEach(() => {
    vi.restoreAllMocks()
    callMock.mockReset()
  ...
  it('prints targeted permission setup command help', async () => {
    ...
    expect(output).toContain(
      'manta computer permissions [--id <accessibility|screenshots>] [--json]'
    )
  ...
  it('prints provider capabilities without resolving a worktree', async () => {
    ...
    expect(vi.mocked(console.log).mock.calls[0][0]).toContain('manta-computer-use-macos')
  })
  ...
  it('opens computer permission setup without resolving a worktree', async () => {
    ...
    const output = vi.mocked(console.log).mock.calls[0][0]
    expect(output).toContain('Opened Manta Computer Use permission setup')
    expect(output).toContain('/Applications/Manta Computer Use.app')
  })
