...
describe('orchestration worker-start CLI contract', () => {
  beforeEach(() => {
    ...
    delete process.env.MANTA_CLI_COMMAND
  })
  ...
  afterEach(() => {
    ...
    if (originalCliCommand === undefined) {
      ...
      process.env.MANTA_CLI_COMMAND = originalCliCommand
    }
  })
    ...
    async (_format, executable, json) => {
      ...
      callMock.mockResolvedValue({
        result: {
          ...
          residualResources: [],
          nextCommands: [
            'manta orchestration worker-show --dispatch ctx_unknown --json',
            'manta orchestration worker-abandon --dispatch ctx_unknown --json'
          ]
        }
      })
  ...
  it('prints the Structured Chat recovery action for a refused worker start', async () => {
    callMock.mockResolvedValue({
      result: {
        ...
        lastError:
          'The target terminal is in Structured Chat. Switch it to Terminal, then retry `manta orchestration worker-start`.',
        effects: [],
        residualResources: []
      }
    })
    ...
    ).toMatch(/Structured Chat.*Switch it to Terminal.*manta orchestration worker-start/s)
  })
