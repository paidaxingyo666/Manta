...
describe('manta emulator CLI handlers', () => {
  ...
  it('uses the folder workspace exported by the current Manta terminal', async () => {
    process.env.MANTA_WORKSPACE_ID = 'folder:folder-1'
    delete process.env.MANTA_WORKTREE_ID
  ...
  it('uses the current git worktree exported by the Manta terminal', async () => {
    process.env.MANTA_WORKSPACE_ID = 'folder:stale-parent'
    process.env.MANTA_WORKTREE_ID = 'repo-1::/repo/project '
    callMock.mockResolvedValue(
      okFixture('req_attach', {
        attached: true,
        info: { deviceUdid: 'device-1', streamUrl: 'scrcpy://device-1' }
