...
function cloudOptions(ctx: HandlerContext): ArtifactCloudOptions {
  const apiUrl = stringFlag(ctx, 'api-url') ?? process.env.MANTA_ARTIFACTS_API_URL?.trim()
  const authToken = process.env.MANTA_CLOUD_AUTH_TOKEN?.trim()
  return {
    ...(apiUrl ? { apiUrl } : {}),
    ...(authToken ? { authToken } : {})
  }
...
async function readStdinWithinLimit(maxBytes: number): Promise<string> {
  ...
  for await (const chunk of process.stdin) {
    ...
    if (bytes > maxBytes) {
      throw new RuntimeClientError(
        'invalid_argument',
        'Artifact is too large for the Manta CLI transport. Use the browser upload page instead.'
      )
    }
    chunks.push(buffer)
  }
