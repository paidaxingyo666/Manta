 ...
 * which then failed every coordinator verb, because the pane remint below needs an `MANTA_PANE_KEY`
...
export function throwNoActiveSenderTerminal(): never {
  ...
  // MANTA_TERMINAL_HANDLE is a different case — that caller HAS an identity, so it keeps the advice
  // to re-run under a live one.
  if (isStructuredSessionWithoutIdentity() && !process.env.MANTA_TERMINAL_HANDLE) {
  ...
  throw new RuntimeClientError(
    ...
    'Could not determine the sender terminal for this orchestration command. ' +
      ...
      'or run the command inside a live Manta terminal with MANTA_TERMINAL_HANDLE set.'
  )
}
