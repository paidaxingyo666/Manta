  ...
  'orchestration worker-list': async ({ flags, client, cwd, json }) => {
    ...
    if (requiresCurrentListSemantics && !result.result.page) {
      throw new RuntimeClientError(
        ...
        'The connected Manta runtime did not prove support for the requested worker-list flags, so no inventory was printed. Update the connected Manta runtime and retry.'
      )
    }
