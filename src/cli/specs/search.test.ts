...
describe('manta search command spec', () => {
  ...
  it('shows both the query and the index report in its usage', () => {
    expect(searchSpec.usage).toContain('manta search <query>')
    expect(searchSpec.usage).toContain('manta search --index-status')
  })
})

describe('manta search discovery surfaces', () => {
  it('is listed in the root help', () => {
    ...
    expect(ROOT_HELP_TEXT_PRIMARY).toContain(
      '  search                    Search the full text of agent sessions on one Manta host'
    )
    expect(ROOT_HELP_TEXT_SECONDARY).toContain('  manta search --index-status [--json]')
  })

  it('prints its own help for `manta search --help`', () => {
    const lines: string[] = []
    ...
    expect(lines.join('\n')).toContain('Usage: manta search <query>')
  })
