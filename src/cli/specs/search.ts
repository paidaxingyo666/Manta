...
// positional, so `manta search status` would be indistinguishable from searching
...
export const SEARCH_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    hidden: true,
    summary: 'Search the full text of agent sessions indexed on the selected Manta host',
    usage:
    ...
    notes: [
      'Searches one host: this machine, or the paired Manta server named by --environment / --pairing-code. There is no all-computers search.',
      'In a Manta SSH terminal, the forwarded CLI searches the controlling Manta runtime by default. Use --environment / --pairing-code to select a paired server; --path only filters results on the selected runtime.',
    ...
    examples: [
      'manta search "strict mode violation getByRole"',
      'manta search resolveTerminalPath --agent claude --sort newest',
      'manta search "kernel panic" --path /Users/me/manta --since 2026-08-01T00:00:00Z --json',
      'manta search "kernel panic" --limit 50 --cursor eyJ2IjoxfQ',
      'manta search --index-status',
      'manta search "flaky test" --environment build-server'
    ]
  }
]
