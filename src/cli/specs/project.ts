...
export const PROJECT_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['project', 'list'],
    summary: 'List durable projects known to Manta',
    usage: 'manta project list [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    examples: ['manta project list', 'manta project list --json']
  },
  {
    path: ['project', 'setups'],
    summary: 'List project host setups',
    usage: 'manta project setups [--project <id>] [--host <host-id>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'project', 'host'],
    notes: [
      'A setup means a project is available on a host at a concrete filesystem path.',
      '--host runtime:<environment-id> runs the command on that paired Manta server instead of filtering this runtime; unknown environment ids are rejected rather than answered with an empty list.',
      'Run `manta environment list` to see the environment ids that runtime:<environment-id> accepts. It matches ids only, never environment names.',
  ...
  },
  {
    ...
    usage: 'manta project setup-delete --setup <setup-id> [--json]',
