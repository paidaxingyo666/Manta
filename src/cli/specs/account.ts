...
export const ACCOUNT_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['account', 'add'],
    summary: 'Add a managed Claude or Codex account by signing in on this Manta host',
    usage: 'manta account add [--agent claude|codex] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'agent'],
    notes: [
      'Runs the agent login (`claude login` / `codex login`) in this terminal, then registers the account with the local Manta runtime.',
      ...
      '--agent defaults to claude. Requires the Manta runtime to be running on this machine.'
    ],
    examples: ['manta account add', 'manta account add --agent codex']
  },
  {
    path: ['account', 'list'],
    summary: 'List managed Claude and Codex accounts on this Manta host',
    usage: 'manta account list [--json]',
