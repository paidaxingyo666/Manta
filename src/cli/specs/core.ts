...
export const CORE_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['open'],
    summary: 'Launch Manta and wait for the runtime to be reachable',
    usage: 'manta open [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    examples: ['manta open', 'manta open --json']
  },
  ...SERVE_COMMAND_SPECS,
  {
    path: ['status'],
  ...
  },
  {
    ...
    summary: 'Start Claude Code Agent Teams in the current Manta terminal',
    usage: 'manta claude-teams [claude args...]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
  ...
  },
  {
    ...
    usage: 'manta worktree list [--repo <selector>] [--limit <n>] [--json]',
  ...
  },
  {
    ...
    usage: 'manta worktree show --worktree <selector> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'worktree']
  },
  {
    path: ['worktree', 'current'],
    summary: 'Show the Manta-managed worktree for the current directory',
    usage: 'manta worktree current [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
      'Resolves the current shell directory to a path: selector so agents can target the enclosing Manta worktree without spelling out $PWD.'
    ],
    examples: ['manta worktree current', 'manta worktree current --json']
  },
  {
    ...
    notes: [
      'This creates a new checkout. For a fresh agent in an existing worktree, use `manta terminal create --worktree active --command "codex"` instead.',
      ...
      'By default this creates the worktree and its first terminal without switching the active Manta view.',
  ...
  },
  {
    ...
    destructive: true,
    summary: 'Remove a worktree from Manta and git',
    usage:
    ...
    notes: [
      'Repo-defined manta.yaml archive hooks are skipped unless --run-hooks is passed.',
      ...
      'For Git worktrees, removal also attempts to delete the checked-out local branch, with or without --force. Manta retains branches it knows predated the worktree and any branch whose changes it cannot prove are already merged.'
    ]
  },
  {
    path: ['worktree', 'ps'],
    summary: 'Show a compact orchestration summary across worktrees',
    usage: 'manta worktree ps [--limit <n>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'limit'],
    notes: [...WORKTREE_LISTING_SCOPE_NOTES]
  },
  {
    path: ['terminal', 'list'],
    summary: 'List live Manta-managed terminals',
    usage:
  ...
  },
  {
    ...
    usage: 'manta terminal show [--terminal <handle>] [--json]',
  ...
  },
  {
    ...
    usage: 'manta terminal stop --worktree <selector> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'worktree'],
    notes: [
  ...
  },
  {
    ...
    notes: [
      ...
      'A host that cannot apply --shell refuses the create rather than quietly spawning its default shell: macOS and Linux execution hosts spawn the login shell, terminals routed over SSH resolve their shell on the SSH host, a --shell that contradicts the project execution runtime (WSL vs Windows host) is refused, and a Manta host older than --shell is refused by the CLI.'
    ],
    examples: [
      'manta terminal create --json',
      'manta terminal create --worktree active --command "codex" --json',
      'manta terminal create --worktree path:/projects/myapp --title "RUNNER" --command "opencode"',
      'manta terminal create --worktree path:/projects/myapp --command "opencode" --focus',
      'manta terminal create --worktree path:C:/src/app --shell cmd.exe --json'
    ]
  },
  {
    ...
    usage: 'manta terminal switch [--terminal <handle>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal'],
    examples: ['manta terminal switch --terminal term_abc123']
  },
  TERMINAL_CLOSE_COMMAND_SPEC,
  {
    path: ['terminal', 'rename'],
    summary: 'Set or clear the title of a terminal tab',
    usage: 'manta terminal rename [--terminal <handle>] [--title <text>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal', 'title'],
    notes: ['Omit --title or pass an empty string to reset to the auto-generated title.'],
    examples: [
      'manta terminal rename --terminal term_abc123 --title "RUNNER"',
      'manta terminal rename --terminal term_abc123 --json'
    ]
  },
  {
    path: ['terminal', 'split'],
    ...
    usage:
      'manta terminal split [--terminal <handle>] [--direction horizontal|vertical] [--command <text>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal', 'direction', 'command'],
    examples: [
      'manta terminal split --terminal term_abc123 --direction horizontal --json',
      'manta terminal split --terminal term_abc123 --command "codex"'
    ]
  }
]
