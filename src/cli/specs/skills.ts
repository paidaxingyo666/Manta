...
export const SKILL_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    usage: 'manta skills installed [--json]',
    ...
    notes: [
      ...
      'Use an exact ID or an unambiguous name with `manta skills share --skill <selector>`.'
    ]
  },
  {
    path: ['skills', 'share'],
    ...
    usage:
      'manta skills share --skill <selector> [--skill <selector> ...] --bundle-name <name> ' +
    ...
    notes: [
      ...
      'The bundle name may be human-readable; Manta converts it to a portable lowercase package name.',
      'Selectors are exact discovery IDs or unambiguous names from `manta skills installed`.',
    ...
    examples: [
      'manta skills share --skill frontend --bundle-name "Frontend Skills"',
      'manta skills share --skill frontend --skill testing --bundle-name "Team Toolkit" --json'
    ]
  },
  {
    path: ['skills', 'list'],
    summary: 'List version-matched skill guides bundled with this Manta CLI',
    usage: 'manta skills list [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
      'Reads bundled guide metadata locally without contacting the Manta runtime.',
      'With --json, prints a topics array of canonical names and one-line descriptions.',
      'Use `manta skills get <name>` for the compact guide, `--full` for its full reference package, or `manta skills install` to install skills.'
    ]
  },
  {
    path: ['skills', 'get'],
    ...
    usage: 'manta skills get <topic> [--full | --reference <name>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'topic', 'full', 'reference', 'references'],
    positionalArgs: ['topic'],
    notes: [
      'Reads bundled guide content locally without contacting the Manta runtime.',
    ...
    examples: [
      ...
      'manta skills get orchestration --references',
      'manta skills get orchestration --reference recovery-and-cleanup'
    ]
  },
  {
    path: ['skills', 'install'],
    summary: 'Install bundled Manta skills via the community skills CLI',
    usage:
      'manta skills install [--skill <name>]... [--all] [--agent <name>[,<name>]] ' +
      '[--local] [--dry-run] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'skill', 'all', 'agent', 'local', 'dry-run'],
    notes: [
      'Reads the bundled skill registry locally without contacting the Manta runtime.',
      'Resolves to the same `npx skills add <repo> --skill <name> ...` command used by ' +
        'Manta Settings, plus the non-interactive flags an unattended host needs ' +
      ...
      'Targets the coding agents Manta detects on this host, plus the shared ' +
      ...
      'Use --agent <name>[,<name>...] to choose targets yourself, or --agent universal ' +
        'for the shared directory alone. Required when Manta detects no agent.',
  ...
  {
    path: ['skills', 'update'],
    summary: 'Update already-installed Manta skills via the community skills CLI',
    usage: 'manta skills update [--skill <name>]... [--all] [--local] [--dry-run] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'skill', 'all', 'local', 'dry-run'],
    notes: [
      'Reads the bundled skill registry locally without contacting the Manta runtime.',
      'Resolves to the same `npx skills update <names...>` command used by Manta Settings, ' +
      ...
      'Only refreshes skills that are already installed; use `manta skills install` first.',
      'Use --dry-run to print the resolved command without running it.',
