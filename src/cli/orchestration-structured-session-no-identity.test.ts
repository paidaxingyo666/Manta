 ...
 *
 * Non-worker structured sessions get no `MANTA_TERMINAL_HANDLE`, so `orchestration check` fell
...
const originalHandle = process.env.MANTA_TERMINAL_HANDLE
...
describe('a structured chat session with no orchestration identity', () => {
  ...
  afterEach(() => {
    ...
    if (originalHandle === undefined) {
      ...
      process.env.MANTA_TERMINAL_HANDLE = originalHandle
    }
  })
