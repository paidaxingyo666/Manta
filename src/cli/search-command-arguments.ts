...
function readQuery(flags: Map<string, string | boolean>): string {
  ...
  if (query === undefined) {
    throw new RuntimeClientError(
      'invalid_argument',
      'Missing a search query. Pass it as `manta search "<query>"` or --query "<query>", or ask for the index report with --index-status.'
    )
  }
  return query
}
...
/** Maps `manta search` flags onto the session-search contract; nothing it does not have. */
