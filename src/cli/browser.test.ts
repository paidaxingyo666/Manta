...
describe('manta cli browser page targeting', () => {
  beforeEach(() => {
    callMock.mockReset()
  })
  ...
  it('passes explicit page ids to snapshot without resolving the current worktree', async () => {
    ...
    await main(['snapshot', '--page', 'page-1', '--json'], '/tmp/not-an-manta-worktree')
  ...
  it('shows tab profile labels in text mode when requested', async () => {
    ...
    await main(['tab', 'list', '--show-profile', '--worktree', 'all'], '/tmp/not-an-manta-worktree')
  ...
  it('shows a single tab by page id', async () => {
    ...
    await main(['tab', 'show', '--page', 'page-1', '--json'], '/tmp/not-an-manta-worktree')
  ...
  it('resolves the current tab in a worktree', async () => {
    ...
    await main(['tab', 'current', '--worktree', 'all', '--json'], '/tmp/not-an-manta-worktree')
...
describe('manta cli browser identity', () => {
  beforeEach(() => {
    callMock.mockReset()
  })
  ...
  it('gets the host identity only after capability negotiation', async () => {
    ...
    await main(['browser', 'identity', 'get', '--json'], '/tmp/not-an-manta-worktree')
  ...
  it('refuses an older runtime instead of guessing at identity support', async () => {
    ...
    await main(['browser', 'identity', 'get'], '/tmp/not-an-manta-worktree')

    expect(callMock).toHaveBeenCalledTimes(1)
    expect(error).toHaveBeenCalledWith(expect.stringContaining('Update or restart Manta'))
  })
})

describe('manta cli browser profile management', () => {
  beforeEach(() => {
    callMock.mockReset()
  })
...
describe('manta cli browser tab profiles', () => {
  beforeEach(() => {
    callMock.mockReset()
  })
  ...
  it('reports an empty browser tab profile list with a friendly message', async () => {
    ...
    await main(['tab', 'profile', 'list'], '/tmp/not-an-manta-worktree')
    ...
    queueFixtures(
      ...
      okFixture('req_profile_create', {
        profile: {
          ...
          scope: 'imported',
          label: 'From Chrome',
          partition: 'persist:manta-browser-session-imp'
        }
      })
    )
    vi.spyOn(console, 'log').mockImplementation(() => {})
  ...
  it('surfaces a runtime error if the registry refuses to create a profile', async () => {
    ...
    await main(['tab', 'profile', 'create', '--label', 'Bogus'], '/tmp/not-an-manta-worktree')
  ...
  it('reports a not-deleted profile in text mode without throwing', async () => {
    ...
    await main(['tab', 'profile', 'delete', '--profile', 'default'], '/tmp/not-an-manta-worktree')
...
describe('manta cli browser cookies', () => {
  beforeEach(() => {
    callMock.mockReset()
...
describe('manta cli browser waits and viewport flags', () => {
  beforeEach(() => {
    callMock.mockReset()
    process.exitCode = undefined
  })
  ...
  it('does not tell users Manta is down for a generic runtime timeout', async () => {
    callMock.mockRejectedValueOnce(
      new RuntimeClientError(
        'runtime_timeout',
        'Timed out waiting for the Manta runtime to respond.'
      )
    )
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {})

    await main(['wait', '--selector', '#ready', '--worktree', 'all'], '/tmp/not-an-manta-worktree')

    expect(errorSpy).toHaveBeenCalledWith('Timed out waiting for the Manta runtime to respond.')
  })
