...
export async function createRuntimeClientModuleMock(mocks: RuntimeClientModuleMocks) {
    ...
    constructor(
      ...
      const effectivePairingCode =
        remotePairingCode === undefined
          ? (process.env.MANTA_PAIRING_CODE ?? process.env.MANTA_REMOTE_PAIRING)
          : remotePairingCode
      const effectiveEnvironment =
        environmentSelector === undefined ? process.env.MANTA_ENVIRONMENT : environmentSelector
  ...
  return {
    ...
    serveMantaApp: mocks.serveMantaAppMock,
...
export function useWorktreeAwarenessEnvironment(mocks: WorktreeAwarenessMocks): void {
  ...
  const originalDevCliInvocation = process.env.MANTA_DEV_CLI_INVOCATION
  const originalPairingCode = process.env.MANTA_PAIRING_CODE
  const originalRemotePairing = process.env.MANTA_REMOTE_PAIRING
  const originalEnvironment = process.env.MANTA_ENVIRONMENT
  const originalWorkspaceId = process.env.MANTA_WORKSPACE_ID
  const originalWorktreeId = process.env.MANTA_WORKTREE_ID

  beforeEach(() => {
    mocks.callMock.mockReset()
    delete process.env.MANTA_TERMINAL_HANDLE
    delete process.env.MANTA_USER_DATA_PATH
    delete process.env.MANTA_DEV_CLI_INVOCATION
    delete process.env.MANTA_WORKSPACE_ID
    ...
    mocks.serveMantaAppMock.mockReset()
  ...
  afterEach(() => {
    ...
    if (originalTerminalHandle === undefined) {
      ...
      process.env.MANTA_TERMINAL_HANDLE = originalTerminalHandle
    }
    ...
    if (originalDevCliInvocation === undefined) {
      ...
      process.env.MANTA_DEV_CLI_INVOCATION = originalDevCliInvocation
    }
    if (originalPairingCode === undefined) {
      delete process.env.MANTA_PAIRING_CODE
    } else {
      process.env.MANTA_PAIRING_CODE = originalPairingCode
    }
    if (originalRemotePairing === undefined) {
      delete process.env.MANTA_REMOTE_PAIRING
    } else {
      process.env.MANTA_REMOTE_PAIRING = originalRemotePairing
    }
    if (originalEnvironment === undefined) {
      delete process.env.MANTA_ENVIRONMENT
    } else {
      process.env.MANTA_ENVIRONMENT = originalEnvironment
    }
    if (originalWorkspaceId === undefined) {
      delete process.env.MANTA_WORKSPACE_ID
    } else {
      process.env.MANTA_WORKSPACE_ID = originalWorkspaceId
    }
    if (originalWorktreeId === undefined) {
      delete process.env.MANTA_WORKTREE_ID
    } else {
      process.env.MANTA_WORKTREE_ID = originalWorktreeId
    }
  })
}
