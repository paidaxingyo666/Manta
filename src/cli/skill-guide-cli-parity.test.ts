...
// path or flag it names must exist in COMMAND_SPECS. `manta emulator camera --webcam` was
 ...
 * `skill-guides/manta-emulator.md` describes serve-sim's own `--detach` in a Notes column beside
 * an `MANTA ...` cell, and that is correct prose a line-scoped check would flag.
 */
...
function parityFailures(invocation: Invocation): string[] {
  ...
  const tokens: string[] = []
  for (const token of masked.slice('MANTA'.length).trim().split(/\s+/u)) {
  ...
  if (command === null) {
    // A prefix reference such as `MANTA emulator ...` or `MANTA linear --help` names no exact
...
describe('skill guides only name commands and flags the CLI defines', () => {
  ...
  it('checks extracted MANTA command paths and flags against COMMAND_SPECS', () => {
  ...
  it('checks flags on a prefix reference against every command under it', () => {
    ...
    expect(at('MANTA emulator ...')).toEqual([])
    expect(at('MANTA linear --help')).toEqual([])
    expect(at('MANTA emulator --webcam')).toEqual([
