...
export type ComposerCardProps = {
  ...
  ephemeralVmRecipes: NonNullable<MantaHooks['environmentRecipes']>
...
export function getInitialAutoManagedWorkspaceName({
  ...
  // Why: a prefilled name counts as user input unless it exactly matches the linked-item seed Manta generated.
...
export function useComposerState(options: UseComposerStateOptions): UseComposerStateResult {
  ...
  // Why: SSH remotes deploy the CLI shim as plain `manta`, so the Linux-only `manta-ide` rename must not apply to remote launch commands.
  ...
  const [yamlHooks, setYamlHooks] = useState<MantaHooks | null>(null)
  ...
  const uploadComposerPaths = useCallback(
    async (
      ...
      const destinationDir = joinPath(targetRepoPath, '.manta/drops')
  ...
  const submit = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      // Why: only a name Manta generated may be retired — the creature pool contains ordinary words
      // ("manta", "runner", "molly") a user can type deliberately and expect to reuse.
  ...
  const submitQuick = useCallback(
    async (requestedAgent: TuiAgent | null): Promise<void> => {
      ...
      try {
        ...
        // Why: only a name Manta generated may be retired — see the full-composer submit path.
        const nameWasGenerated = !name.trim() && workspaceName === fallbackCreatureName
