...
describe('hasInstalledAgentSkill', () => {
  it('matches installed skills by summarized name', () => {
    expect(hasInstalledAgentSkill([skill({ name: 'manta-cli' })], 'manta-cli')).toBe(true)
  })
    ...
    expect(
      hasInstalledAgentSkill(
        [
          skill({
            name: 'Manta CLI',
            directoryPath: 'C:\\Users\\test\\.agents\\skills\\manta-cli'
          })
        ],
        'manta-cli'
      )
  ...
  it('ignores non-installed discovery entries', () => {
    expect(
      hasInstalledAgentSkill([skill({ name: 'manta-cli', installed: false })], 'manta-cli')
    ).toBe(false)
  })
      ...
      hasInstalledAgentSkill(
        [
          skill({
            ...
            directoryPath: '/repo/.agents/skills/manta-cli',
            skillFilePath: '/repo/.agents/skills/manta-cli/SKILL.md'
          }),
          skill({
            id: 'skill-2',
            name: 'manta-cli',
            ...
            directoryPath: '/Users/test/.codex/plugins/cache/vendor/manta-cli',
            skillFilePath: '/Users/test/.codex/plugins/cache/vendor/manta-cli/SKILL.md'
          })
        ],
        'manta-cli',
  ...
  it('counts home skills when matching global installs', () => {
    expect(
      hasInstalledAgentSkill([skill({ name: 'manta-cli' })], 'manta-cli', {
        sourceKinds: GLOBAL_AGENT_SKILL_SOURCE_KINDS
      })
    ).toBe(true)
  })
    ...
    expect(
      hasInstalledAgentSkillNamed(
        [
          skill({
            name: 'Linear Tickets',
            directoryPath: 'C:\\Users\\test\\.agents\\skills\\manta-linear'
          })
        ],
        ['manta-linear', 'linear-tickets']
      )
  ...
  it('keeps aliases opt-in for unrelated single-name checks', () => {
    expect(hasInstalledAgentSkill([skill({ name: 'linear-tickets' })], 'manta-linear')).toBe(false)
  })
})
...
describe('discoverInstalledAgentSkills', () => {
  ...
  it('starts a fresh scan when a forced refresh arrives during a background scan', async () => {
    ...
    const freshResult = discoveryResult([skill({ name: 'manta-cli' })])
  ...
  it('lets completed-scan broadcasts reuse the cached result', async () => {
    const discover = vi
      ...
      .mockResolvedValue(discoveryResult([skill({ name: 'manta-linear' })]))
    vi.stubGlobal('window', {
      api: { skills: { discover } },
      dispatchEvent: vi.fn(),
      CustomEvent: class {}
    ...
    for (const pending of fromSubscribers) {
      await expect(pending).resolves.toMatchObject({
        skills: [expect.objectContaining({ name: 'manta-linear' })]
      })
    }
    expect(discover).toHaveBeenCalledTimes(1)
  })
