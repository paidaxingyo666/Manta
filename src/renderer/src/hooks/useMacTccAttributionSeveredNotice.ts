      ...
      try {
        ...
        toast.warning(
          translate(
            ...
            'macOS permissions may not reach Manta terminals'
          ),
          {
            id: SEVERED_TCC_NOTICE_ID,
            description: translate(
              'auto.hooks.useMacTccAttributionSeveredNotice.description',
              'Running Manta terminals are hosted by a daemon started by a previous Manta installation. macOS may not apply Manta’s Accessibility, Automation, or protected-file permissions to them. Restart the daemon from Manage Sessions to restore access. This will close all running Manta terminals.'
            ),
            duration: Infinity,
            action: {
              label: translate(
