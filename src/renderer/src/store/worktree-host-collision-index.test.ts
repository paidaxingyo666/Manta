...
const SHARED_ID = 'repo-1::/work/manta'
...
const localRow: Worktree = { ...baseWorktree, hostId: 'local', displayName: 'local manta' }
const sshRow: Worktree = { ...baseWorktree, hostId: 'ssh:build-box', displayName: 'ssh manta' }
...
describe('id-keyed worktree projections keep distinct hosts distinct', () => {
  ...
  it('treats an unqualified row as its own bucket rather than inventing a host', () => {
    const unqualified: Worktree = { ...baseWorktree, displayName: 'unqualified manta' }
