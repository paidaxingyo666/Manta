...
async function assertProjectHostSetupRuntimeCapability(
  ...
  await assertRuntimeEnvironmentCapability(
    ...
    PROJECT_HOST_SETUP_RUNTIME_CAPABILITY,
    'The selected Manta server does not support project host setup yet. Update Manta on the server and try again.',
    15_000
  )
}
...
async function assertProjectHostSetupMutationRuntimeCapabilities(
  ...
  await assertRuntimeEnvironmentCapability(
    target.environmentId,
    WORKSPACE_RUN_CONTEXT_RUNTIME_CAPABILITY,
    'The selected Manta server does not support explicit workspace run hosts yet. Update Manta on the server and try again.',
    15_000
  )
}
...
function mergeFetchedProjectCompatibilityForHost({
  ...
  const setupBelongsToFetchedCatalog = (setup: ProjectHostSetup): boolean => {
    ...
    // Why: desktop persistence owns local and direct-SSH setups; runtime setups stay authoritative on their remote Manta server.
...
async function fetchRuntimeAddProjectPathStatus(args: {
  ...
  await assertRuntimeEnvironmentCapability(
    ...
    translate(
      ...
      'Update Manta server to open non-Git folders on this runtime.'
    ),
    15_000
  )
  try {
...
export const createRepoSlice: StateCreator<AppState, [], [], RepoSlice> = (set, get) => ({
    ...
    const validateRepoScopedUi = (): void => {
      set((s) => {
        ...
        return {
          ...
          trustedMantaHooks: filterTrustedMantaHooksToValidRepos(s.trustedMantaHooks, validRepoIds)
        }
      })
    }
  ...
  removeProject: async (projectId, options) => {
    try {
      ...
      get().clearMantaHookTrustForRepo(projectId)
