...
function buildBrowserPage(
  ...
  return {
    ...
    loading: normalizedUrl !== 'about:blank' && normalizedUrl !== MANTA_BROWSER_BLANK_URL,
    faviconUrl: null,
    canGoBack: false,
    canGoForward: false,
    loadError: null,
...
export const createBrowserSlice: StateCreator<AppState, [], [], BrowserSlice> = (set, get) => ({
    ...
    set((s) => {
      ...
      const shouldFocusAddressBar =
        ...
        (options?.focusAddressBar ??
          (page.url === 'about:blank' || page.url === MANTA_BROWSER_BLANK_URL))
  ...
  setBrowserPageUrl: (pageId, url, options) => {
    ...
    if (nextUrl !== 'about:blank' && nextUrl !== MANTA_BROWSER_BLANK_URL) {
  ...
  addBrowserHistoryEntry: (url, title) => {
    ...
    if (safeUrl === MANTA_BROWSER_BLANK_URL || safeUrl === 'about:blank' || !safeUrl) {
      return
    }
