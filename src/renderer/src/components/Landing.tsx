...
function GitHubStarButton({ hasRepos }: { hasRepos: boolean }): React.JSX.Element | null {
  ...
  useEffect(() => {
    let cancelled = false
    void window.api.gh.checkMantaStarred().then((result) => {
      if (cancelled) {
        return
      }
      if (result === null) {
  ...
  const handleClick = async (): Promise<void> => {
    ...
    if (state === 'web-fallback') {
      await window.api.shell.openUrl(MANTA_GITHUB_URL)
      return
    }
    ...
    const ok = await window.api.gh.starManta('landing')
    if (!ok) {
      ...
      <div className="w-full max-w-lg px-6">
        <div className="flex flex-col items-center gap-4 py-8">
          ...
          >
            <img
              src={logo}
              alt={translate('auto.components.Landing.520304a067', 'Manta logo')}
              className="size-12"
            />
          </div>
          <h1 className="text-4xl font-bold text-foreground tracking-tight">
            {translate('auto.components.Landing.6ca6ff404e', 'MANTA')}
          </h1>
