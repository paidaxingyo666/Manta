...
describe('Agent Map workspace context menu', () => {
    ...
    useAppStore.setState({
      detectedWorktreesByRepo: {
        [repo.id]: {
          ...
          source: 'git',
          worktrees: [
            { ...worktree, ownership: 'manta-managed', selectedCheckout: false, visible: true }
          ]
        }
      }
    })
  ...
  it('opens the existing worktree composer from a project ring', async () => {
    ...
    fireEvent.click(await screen.findByText('Create new worktree for Manta', {}, { timeout: 5_000 }))
  ...
  it('clears an ambiguous project request instead of choosing a repo host', async () => {
    ...
    fireEvent.contextMenu(container.querySelector('[data-agent-map-project]')!)
    await act(async () => new Promise((resolve) => window.setTimeout(resolve, 0)))
    expect(screen.queryByText('Create new worktree for Manta')).not.toBeInTheDocument()
