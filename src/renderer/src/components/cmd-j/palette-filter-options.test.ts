...
const repos = [repo('r1', 'Manta'), repo('r2', 'Manta (builder)', 'ssh-1'), repo('r3', 'Solo')]
const projects = [project('p1', 'Manta')]
...
describe('buildPaletteFilterModel', () => {
  it('collapses the repos of one project into a single row', () => {
    ...
    expect(model.projects[0]?.searchText).toBe('manta')
  })
  ...
  it('sorts project rows by workspace count then label', () => {
    ...
    // Manta has 2 workspaces, Solo has 1 — popularity beats alpha.
    expect(model.projects.map((option) => option.label)).toEqual(['Manta', 'Solo'])
  })
