...
import { isMantaCliAvailableOnPath } from '@/lib/agent-skill-cli-prerequisite'
import {
  ...
  const refreshOrchestrationSetupVisibility = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (mountedRef.current) {
        setShowOrchestrationSetup(!isMantaCliAvailableOnPath(status))
      }
    } catch {
      if (mountedRef.current) {
        setShowOrchestrationSetup(true)
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-xs leading-5 text-muted-foreground">
                {translate(
                  ...
                  'Set up the Manta CLI and agent skill so agents can coordinate through Manta.'
                )}
              </p>
            </div>
