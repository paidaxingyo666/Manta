...
describe('PrivacyPane — isEnvBlocked', () => {
  ...
  it('returns true for MANTA_TELEMETRY_DISABLED', () => {
    expect(isEnvBlocked({ effective: 'disabled', reason: 'manta_disabled' })).toBe(true)
  })
...
describe('PrivacyPane — envVarNameForReason', () => {
  ...
  it('maps manta_disabled to MANTA_TELEMETRY_DISABLED', () => {
    expect(envVarNameForReason('manta_disabled')).toBe('MANTA_TELEMETRY_DISABLED')
  })
...
describe('PrivacyPane — computeBlockedReason', () => {
  ...
  it('names MANTA_TELEMETRY_DISABLED as the env reason when set', () => {
    const result = computeBlockedReason({ effective: 'disabled', reason: 'manta_disabled' })
    expect(result).toEqual({ kind: 'env', reason: 'manta_disabled' })
  })
