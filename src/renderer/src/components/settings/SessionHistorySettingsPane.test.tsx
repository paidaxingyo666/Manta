...
it('lists one row per paired Manta server under this computer, and says where SSH stands', async () => {
  mocks.environments = [
  ...
  expect(screen.getByText('Manta remote servers')).toBeInTheDocument()
})
...
it('leaves a lone computer to its own switch, with no roll-up above it', async () => {
  ...
  expect(screen.queryByText('Manta remote servers')).not.toBeInTheDocument()
})
