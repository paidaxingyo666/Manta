    ...
    return (
      <p className="text-xs text-muted-foreground">
        {translate(
          ...
          'Configured via environment variables. Unset the MANTA_BITBUCKET_* variables to manage this credential in Manta.'
        )}
      </p>
    )
  }
      ...
      <p className="text-xs text-muted-foreground">
        {props.storedCredential
          ...
          : translate(
              ...
              'Bitbucket credentials are configured but could not authenticate. Check the token and repository permissions, then restart Manta if environment variables changed.'
            )}
      </p>
    )
  }
    ...
    return (
      <p className="text-xs text-muted-foreground">
        {translate(
          ...
          'Saved in Manta on this machine. MANTA_BITBUCKET_* environment variables take precedence when set.'
        )}
      </p>
    )
  }
  return (
    <p className="text-xs text-muted-foreground">
      {translate(
        ...
        'Connect a Bitbucket Cloud account with an Atlassian API token or an access token. MANTA_BITBUCKET_* environment variables work too and take precedence.'
      )}
    </p>
  )
}
