...
describe('MobilePairingConnectionOptions', () => {
  ...
  it('shows Sign in directly under Manta Relay, above LAN', async () => {
    const user = userEvent.setup()
    const onChange = vi.fn()
    render(<MobilePairingConnectionOptions value="automatic" onChange={onChange} />)

    const relay = screen.getByRole('radio', { name: /Manta Relay/i })
  ...
  it('moves selection with the arrow keys as a radiogroup', async () => {
    ...
    screen.getByRole('radio', { name: /Manta Relay/i }).focus()
  ...
  it('selects a path from the compact list', async () => {
    ...
    await user.click(screen.getByRole('radio', { name: /Manta Relay/i }))
    expect(onChange).toHaveBeenCalledWith('automatic')
  })
  ...
  it('shows relay status when signed in on Manta Relay', async () => {
    mocks.state = {
      mantaProfileAuthStatus: {
        activeProfileId: 'profile-1',
        configured: true,
  ...
  it('re-reads a session revoked since startup and offers Sign in again', async () => {
    ...
    fetchAuthStatus.mockImplementation(async () => {
      const revoked: MantaProfileAuthStatus = {
        activeProfileId: 'profile-1',
        configured: true,
        state: 'reconnect-required',
        persistence: 'encrypted'
      }
      publishStoreState({ ...connectedState, mantaProfileAuthStatus: revoked })
      return revoked
    })
