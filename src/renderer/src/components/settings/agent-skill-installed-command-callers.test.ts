...
const updateCapableCallers = new Map<string, readonly string[]>([
  ...
  [
    ...
    ['MANTA_CLI_SKILL_UPDATE_COMMAND', 'installedCommand={cliSkillUpdateCommand}']
  ],
  [
    'src/renderer/src/components/settings/BrowserUsePane.tsx',
    ['MANTA_CLI_SKILL_UPDATE_COMMAND', 'installedCommand={browserUseUpdateCommand}']
  ],
  [
  ...
  [
    ...
    ['MANTA_CLI_SKILL_UPDATE_COMMAND', 'installedCommand={updateCommand}']
  ],
  [
  ...
  [
    ...
    [
      ...
      'buildSkillCommandForRuntime(MANTA_CLI_SKILL_INSTALL_COMMAND)',
      'buildSkillCommandForRuntime(MANTA_CLI_SKILL_UPDATE_COMMAND)'
    ]
  ]
])
...
describe('AgentSkillSetupPanel installed-command call sites', () => {
  ...
  it('routes the combined feature-tip install through runtime command setup', () => {
    ...
    expect(source).not.toContain('command={MANTA_CLI_ORCHESTRATION_SKILL_INSTALL_COMMAND}')
