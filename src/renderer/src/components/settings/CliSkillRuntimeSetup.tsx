...
import {
  ...
  showMantaCliRegistrationPromptToast
 ...
 * or the setup terminal Manta spawns itself.
 */
type SkillCommandTarget = 'copied-command' | 'manta-setup-terminal'

/**
 * Adapts a copied skill command for Manta's inline setup terminal auto-paste.
...
export async function ensureWslCliAvailableForAgentSkillTerminal(
  ...
  try {
    ...
    if (status.state !== 'installed' || status.pathConfigured === false) {
      await showMantaCliRegistrationPromptToast()
      const next = await window.api.cli.installWsl(args)
      if (!isMantaCliAvailableOnPath(next)) {
        toast.warning(
          translate(
