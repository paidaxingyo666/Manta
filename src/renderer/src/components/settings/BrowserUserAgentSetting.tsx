...
export function BrowserUserAgentSetting({
  ...
  if (!isLocal) {
    control = (
      <span className="text-xs text-muted-foreground">
        {translate(
          ...
          'Manage browser identity on the remote host with the Manta CLI.'
        )}
      </span>
    )
  } else if (!status) {
    ...
    // reset overwrites data that may belong to a newer Manta. Without naming the escape the message
      ...
      <div className="space-y-1 text-right">
        ...
        <div className="text-[11px] text-muted-foreground">
          {translate(
            ...
            'Reset it from the command line: manta browser identity set --mode <mode> --reset'
          )}
        </div>
      </div>
    )
          ...
          options={[
            {
              ...
              tooltip: translate(
                ...
                'Removes Manta and Electron tokens to match imported Chrome sessions.'
              )
            },
            {
              value: 'native',
