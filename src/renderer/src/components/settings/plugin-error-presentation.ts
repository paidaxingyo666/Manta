...
export function pluginInstallErrorMessage(cause: unknown): string {
  ...
  if (detail.includes('manta-plugin.json') && /(missing|unreadable|no )/.test(detail)) {
    return translate(
      'auto.components.settings.pluginError.installManifestMissing',
      "No readable manta-plugin.json was found. Choose the plugin's root folder."
    )
  }
  if (detail.includes('invalid manifest')) {
    return translate(
      'auto.components.settings.pluginError.installManifestInvalid',
      'manta-plugin.json is invalid. Ask the plugin author to fix the manifest.'
    )
  }
  if (detail.includes('requires manta')) {
    return translate(
      'auto.components.settings.pluginError.incompatible',
      'This plugin requires a different Manta version.'
    )
  }
  ...
  if (/(exceeds|too many)/.test(detail)) {
    return translate(
      ...
      "The plugin exceeds Manta's install size or file-count limits."
    )
  }
  if (/(git|repository|fetch|clone|checkout|remote)/.test(detail)) {
    return translate(
      'auto.components.settings.pluginError.installGit',
      'Manta could not fetch the pinned Git revision. Check the URL, #ref, access, and system Git setup.'
    )
  }
  return translate(
...
export function invalidPluginErrorMessage(detailValue: string): string {
  ...
  if (detail.includes('missing manta-plugin.json')) {
    return translate(
      'auto.components.settings.pluginError.invalidManifestMissing',
      'The plugin root is missing manta-plugin.json. Add it, then refresh plugins.'
    )
  }
  if (detail.includes('invalid manifest')) {
    return translate(
      'auto.components.settings.pluginError.invalidManifest',
      'manta-plugin.json is invalid. Fix it, then refresh plugins.'
    )
  }
