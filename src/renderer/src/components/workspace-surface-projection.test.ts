...
const SHARED_WORKTREE_ID = 'repo-shared::/work/manta-feature'
...
const localWorktree: Worktree = {
  ...
  repoId: 'repo-shared',
  path: '/work/manta-feature',
  hostId: 'local',
  head: 'abc123',
  branch: 'feature',
  isBare: false,
  isMainWorktree: false,
  displayName: 'manta-feature',
  comment: '',
  linkedIssue: null,
  linkedPR: null,
...
describe('projectWorkspaceSurfaces', () => {
  it('emits one surface per workspace id when two hosts publish the same worktree', () => {
    ...
    expect(surfaces).toEqual([{ id: SHARED_WORKTREE_ID, path: '/work/manta-feature' }])
  })
  ...
  it('mounts the active folder workspace at its resolved host path, not the first row', () => {
    ...
    expect(surfaces).toEqual([{ id: 'folder:folder-shared', path: '/remote/manta' }])
  })
  ...
  it('keeps first-wins for a colliding folder workspace that is not the active one', () => {
    ...
    expect(surfaces).toEqual([{ id: 'folder:folder-shared', path: '/work/manta-local' }])
    expect(warn).toHaveBeenCalledWith(
      '[workspace-surface] dropping colliding folder path',
      expect.objectContaining({ kept: '/work/manta-local', dropped: '/remote/manta' })
    )
    warn.mockRestore()
  })
  ...
  it('switches the active folder path from first-wins to the host that hydrates', () => {
    ...
    expect(hydrating).toEqual([{ id: 'folder:folder-shared', path: '/work/manta-local' }])
    expect(hydrated).toEqual([{ id: 'folder:folder-shared', path: '/remote/manta' }])
  ...
  it('keeps distinct workspaces on one host', () => {
    const otherWorktree: Worktree = {
      ...localWorktree,
      id: 'repo-shared::/work/manta-other',
      path: '/work/manta-other'
    }

    expect(project({ worktrees: [localWorktree, otherWorktree] })).toEqual([
      { id: SHARED_WORKTREE_ID, path: '/work/manta-feature' },
      { id: 'repo-shared::/work/manta-other', path: '/work/manta-other' }
    ])
  })
})
...
describe('projectWorkspaceSurfaces never under-selects', () => {
  ...
  it('emits every distinct id from a mixed local, SSH, runtime and folder catalog', () => {
    const distinctWorktree: Worktree = {
      ...sshWorktree,
      id: 'repo-shared::/work/manta-ssh-only',
      path: '/work/manta-ssh-only'
    }
  ...
  it('mounts a local-only catalog whose rows never name a host', () => {
    ...
    expect(
      ...
      { id: SHARED_WORKTREE_ID, path: '/work/manta-feature' },
      { id: 'repo-shared::/work/manta-other', path: '/work/manta-other' },
      { id: 'folder:folder-local-only', path: '/work/manta-local' }
    ])
  })
  ...
  it('keeps a git worktree and a folder workspace apart even on the same id text', () => {
    ...
    expect(surfaces).toEqual([
      ...
      { id: 'folder:folder-shared', path: '/work/manta-local' }
    ])
  })
  ...
  it('collapses folder rows across three hosts to one surface without losing the id', () => {
    ...
    expect(surfaces).toEqual([{ id: 'folder:folder-shared', path: '/ssh/manta' }])
  })
  ...
  it('does not let a folder row that names no host win the local tie-break', () => {
    ...
    ).toEqual([{ id: 'folder:folder-shared', path: '/work/manta-local' }])
  })
