...
const MANTA_FEATURE_REQUEST_URL = 'https://github.com/stablyai/manta/issues/new'
...
export default function ProjectViewWrapper({ selectedRepoIds }: Props): React.JSX.Element {
  ...
  // Why: matched-repo rows open `GitHubItemDialog`, unmatched the slug dialog; `repoNotInManta` drives the `repo-not-in-manta` modal.
  ...
  // Why: slug dialog only serves unregistered-repo rows; the parent (not this dialog) owns the repo-not-in-manta "Start work" flow.
  const [slugDialog, setSlugDialog] = useState<{
    origin: GitHubItemDialogProjectOrigin
  } | null>(null)
  const [repoNotInManta, setRepoNotInManta] = useState<{
    owner: string
    repo: string
    host?: string
    url: string | null
  ...
  if (resolvedDialogRepoItem !== dialogRepoItem) {
    // Why: clear the repo-backed dialog when its repo leaves Manta, before the modal tree gets stale repo ids.
  ...
  if (resolvedMissingRepoDialogs.repoNotInManta !== repoNotInManta) {
    setRepoNotInManta(resolvedMissingRepoDialogs.repoNotInManta)
  }
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col">
      ...
      {/* Slug-only dialog for unadded-repo rows; Start-work lives in the parent's `repoNotInManta` modal, not here (avoids a confusing duplicate button). */}
      ...
      {/* repo-not-in-manta prompt: see design doc Interaction States. */}
      <Dialog
        open={resolvedMissingRepoDialogs.repoNotInManta !== null}
        onOpenChange={(open) => !open && setRepoNotInManta(null)}
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {translate(
                ...
                'Repository not in Manta'
              )}
            </DialogTitle>
            <DialogDescription>
              {resolvedMissingRepoDialogs.repoNotInManta
                ? translate(
                    'auto.components.github.project.ProjectViewWrapper.1850fceac8',
                    "{{value0}}/{{value1}} isn't added to Manta. Add it to start work, or open in GitHub.",
                    {
                      value0: resolvedMissingRepoDialogs.repoNotInManta.owner,
                      value1: resolvedMissingRepoDialogs.repoNotInManta.repo
                    }
                  )
                : null}
            </DialogDescription>
          ...
          <DialogFooter className="gap-2 sm:justify-end">
            <Button variant="ghost" onClick={() => setRepoNotInManta(null)}>
              {translate('auto.components.github.project.ProjectViewWrapper.dffa899f36', 'Cancel')}
            </Button>
            {resolvedMissingRepoDialogs.repoNotInManta?.url ? (
              <Button
                variant="outline"
                onClick={() => {
                  if (resolvedMissingRepoDialogs.repoNotInManta?.url) {
                    void window.api.shell.openUrl(resolvedMissingRepoDialogs.repoNotInManta.url)
                  }
                  setRepoNotInManta(null)
                }}
              >
                {translate(
  ...
  return (
    <div className="project-view-tab-strip flex min-h-[41px] min-w-0 flex-none items-end gap-1 overflow-x-auto overflow-y-hidden border-b border-border/50 bg-muted/20 px-3 pt-3">
      {views.map((v) => {
            ...
            title={
              supported
                ...
                : translate(
                    ...
                    { value0: v.name, value1: layoutLabel, value2: MANTA_FEATURE_REQUEST_URL }
                  )
            }
            className={cn(
        ...
        const unsupportedMessage = `Manta doesn't support ${layoutLabel} project views yet.`
        return (
            ...
            <HoverCardTrigger asChild>
              <span
                ...
                aria-label={translate(
                  ...
                  { value0: v.name, value1: unsupportedMessage, value2: MANTA_FEATURE_REQUEST_URL }
                )}
