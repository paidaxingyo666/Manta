...
beforeEach(() => {
  ...
  environmentMocks.resolveFooter.mockResolvedValue(
    ['---', 'Manta: 1.4.178-rc.2', 'OS: darwin 25.0.0 (arm64)', 'Shell: /bin/zsh'].join('\n')
  )
})
...
const LEGACY_HOST_GONE =
  "Error invoking remote method 'pty:spawn': Error: connect ENOENT \\\\?\\pipe\\manta-terminal-host-v30-14cb7f94b511"
...
describe('humanizeTerminalError', () => {
  it('replaces the pane-owner-unverified code with actionable copy', () => {
    ...
    expect(humanized).toContain('Manta left the saved session unchanged')
  ...
  it('humanizes an owner marker without classifying mixed errors as safe warnings', () => {
    ...
    expect(mixed).toContain("Manta couldn't verify this terminal's owner.")
  ...
  it('humanizes a legacy host raw named-pipe error', () => {
    ...
    expect(humanized).not.toContain('manta-terminal-host-v30')
    expect(humanized).toContain('Open a new terminal to continue')
  })
  ...
  it('replaces a raw relay PTY-not-found string and its quoted id', () => {
    const humanized = humanizeTerminalError(
      'Error invoking remote method \'pty:spawn\': Error: PTY "manta:2f1c@@pty-7" not found'
    )
  ...
  it('replaces the identity-mismatch form of PTY-not-found', () => {
    const humanized = humanizeTerminalError('PTY "manta:2f1c@@pty-7" not found (identity mismatch)')
  ...
  it('does not claim the remote shell is still running or dead', () => {
    const humanized = humanizeTerminalError('SSH_SESSION_EXPIRED: manta:2f1c@@pty-7')
  ...
  it.each(['ENOENT', 'ECONNREFUSED'])(
    ...
    (code) => {
      const aggregated =
        `connect ${code} \\\\?\\pipe\\unrelated\n` + 'manta-terminal-host-v30-14cb7f94b511'
...
describe('isExplainedTerminalError', () => {
  ...
  it('suppresses the issue link for a session the host cannot reattach', () => {
    expect(isExplainedTerminalError('SSH_SESSION_EXPIRED: manta:2f1c@@pty-7')).toBe(true)
    expect(
      isExplainedTerminalError(
        'Error invoking remote method \'pty:spawn\': Error: PTY "manta:2f1c@@pty-7" not found'
      )
  ...
  it('keeps the issue link for errors Manta cannot explain', () => {
    ...
    expect(isExplainedTerminalError('open ENOENT \\\\?\\pipe\\manta-terminal-host-v30-dead')).toBe(
      false
    )
    expect(
      isExplainedTerminalError('connect ETIMEDOUT \\\\?\\pipe\\manta-terminal-host-v30-dead')
    ).toBe(false)
...
describe('shouldOfferDaemonRestart', () => {
  it('matches stale daemon node-pty install failures', () => {
    expect(
      shouldOfferDaemonRestart(
        "Daemon's node-pty install is gone (worktree deleted?). Restart Manta. node-pty: posix_spawn failed: ENOENT (errno 2, No such file or directory) - helper='/Applications/Manta.app/Contents/Resources/app.asar.unpacked/node_modules/node-pty/build/Release/spawn-helper'"
      )
    ).toBe(true)
  })
  ...
  it('matches stale daemon cwd failures', () => {
    expect(
      shouldOfferDaemonRestart(
        "Daemon's working directory is gone (worktree deleted?). Restart Manta. node-pty: daemon_cwd failed: ENOENT (errno 2, No such file or directory) - cwd='<unavailable>'"
      )
    ).toBe(true)
  })
...
describe('TerminalErrorToast environment footer', () => {
  it('appends client environment details to local errors', async () => {
    ...
    await waitFor(() => expect(view.container.textContent).toContain('Manta: 1.4.178-rc.2'))
  })
  ...
  it('does not retain a prior async footer when the next error already has one', async () => {
    ...
    expect(view.container.textContent).toContain('Manta: embedded')
    expect(view.container.textContent).not.toContain('Manta: 1.4.178-rc.2')
  })
  ...
  it('renders owner-unverified as a warning without an issue link', () => {
    ...
    expect(toast?.textContent).toContain('Manta left the saved session unchanged')
