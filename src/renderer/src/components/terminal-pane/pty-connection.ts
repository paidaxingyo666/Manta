...
export const STARTUP_CWD_FALLBACK_NOTICE =
  '\r\n[Manta opened this terminal at the workspace root because its saved start folder no longer exists.]\r\n'
...
const HIDDEN_OUTPUT_RESTORE_UNAVAILABLE_WARNING =
  '\r\n[Manta skipped hidden terminal output because main recovery was unavailable.]\r\n'
...
export function connectPanePty(
  ...
  const scheduleReattachIdleAgentCursorReset = (): void => {
    ...
    reattachIdleAgentCursorResetTimer = setTimeout(() => {
      ...
      // fire because the agent was already idle before Manta restarted.
      queueAgentIdleTerminalModeReset()
  ...
  const dropCommandFinishedStatusIfSameTurn = (
    ...
    const state = useAppStore.getState()
    if (!entry) {
      // Why: a Manta-started agent can exit before its first hook status. The
  ...
  const onTerminalKeyDown = (event: KeyboardEvent): void => {
    ...
    // Why: user shell frameworks (bash-preexec/iTerm2) can replace Manta's
  ...
  const applyInitialAgentStatus = (terminalTitle?: string): void => {
    ...
    const statusPayload = {
      ...
      // Why: Manta launched this agent, so this row predates any provider signal for the pane.
  ...
  const onBell = (): void => {
    ...
    // standalone BEL some time after daemon snapshot reattach, even when Manta
  ...
  // Why: inject MANTA_PANE_KEY so global Claude/Codex hooks can attribute their
  // callbacks to the correct Manta pane without resolving worktrees from cwd.
  // The key matches the `${tabId}:${leafId}` composite used for cacheTimerByKey
  // and agentStatusByPaneKey. Treat it as opaque outside Manta.
  const state = useAppStore.getState()
  ...
  const folderWorkspace =
    parsedWorkspaceKey?.type === 'folder'
      ...
      : null
  const workspaceEnv: Record<string, string> = { MANTA_WORKSPACE_ID: deps.worktreeId }
  if (folderWorkspace) {
    workspaceEnv.MANTA_PROJECT_GROUP_ID = folderWorkspace.projectGroupId
    workspaceEnv.MANTA_WORKSPACE_ROOT = folderWorkspace.folderPath
  }
  const paneIdentityEnv = {
    ...workspaceEnv,
    MANTA_PANE_KEY: cacheKey,
    MANTA_TAB_ID: deps.tabId,
    MANTA_WORKTREE_ID: deps.worktreeId,
    ...(launchToken ? { MANTA_AGENT_LAUNCH_TOKEN: launchToken } : {})
  }
  const paneEnv = {
    ...paneStartup?.env,
    ...paneIdentityEnv
  ...
  const runDeferredConnect = (): void => {
    ...
    const buildColdRestoreAgentResumeStartup = (): ColdRestoreAgentResumeStartup | null => {
      ...
      return {
        ...
        env: {
          ...startupPlan.env,
          MANTA_AGENT_LAUNCH_TOKEN: coldRestoreLaunchToken
        },
    ...
    const mergeStartupEnvWithPaneIdentity = (
      ...
      env
        ? {
            ...env,
            ...paneIdentityEnv,
            ...(env.MANTA_AGENT_LAUNCH_TOKEN
              ? { MANTA_AGENT_LAUNCH_TOKEN: env.MANTA_AGENT_LAUNCH_TOKEN }
              : {})
          }
        : undefined
    ...
    const reattachReplayResetSequence = (
      ...
      // Why: an alt-screen pane is a live TUI Manta just does not recognise as an agent, and the
