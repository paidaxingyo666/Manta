...
describe('terminal link action routing', () => {
  ...
  it('offers system browser first and Manta second when system browser is the default', () => {
    const request = vi.fn()
    ...
    expect(
      handleTerminalHttpLink('https://example.com/path', event, {
        ...
        actionDestinations: { primary: 'system', alternate: 'manta' }
      })
    ).toBe(true)
    ...
    expect(request.mock.calls[0][0].alternate.label).toBe('Manta Browser')
  ...
  it('offers Manta first and system browser second when Manta is the default', () => {
    const request = vi.fn()

    handleTerminalHttpLink('https://example.com/path', plainEvent(), {
      worktreeId: 'wt-1',
      linkActionContext: actionContext(request),
      actionDestinations: { primary: 'manta', alternate: 'system' }
    })

    expect(request.mock.calls[0][0].primary.label).toBe('Manta Browser')
  ...
  it('routes an explicit Manta Browser action to the owning runtime', () => {
    const request = vi.fn()
  ...
  it('routes an explicit Manta Browser action through the owning SSH workspace', () => {
    const request = vi.fn()
