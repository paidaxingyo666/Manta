...
function humanizeUnreattachableSession(error: string): string {
  const explanation = translate(
    ...
    "Manta couldn't reattach to this pane's terminal session on the host. Open a new terminal to continue."
  )
  ...
  if (humanized.includes(PANE_OWNER_UNVERIFIED_MARKER)) {
    const explanation = isPaneOwnerUnverifiedError(humanized)
      ? translate(
          ...
          "Manta couldn't safely reconnect this terminal because the host couldn't verify its saved session. Manta left the saved session unchanged. Click Retry to try reconnecting now. If it still cannot reconnect, open a new terminal."
        )
      : translate(
          'auto.components.terminal.pane.TerminalErrorToast.ownerUnknown',
          "Manta couldn't verify this terminal's owner."
        )
        ...
        <span style={{ minWidth: 0 }}>
          ...
          {showDaemonRestart ? (
            ...
            <>
              ...
              <a
                href="https://github.com/stablyai/manta/issues"
