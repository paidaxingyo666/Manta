...
describe('TerminalLinkActionPopover', () => {
  ...
  it('uses distinct icons for system and Manta browser actions', () => {
    vi.stubGlobal('navigator', { userAgent: 'Macintosh' })
    const request: TerminalLinkActionRequest = {
      paneId: 1,
      anchorX: 100,
      ...
      destination: 'https://example.com',
      kind: 'url',
      primary: { external: false, label: 'Manta Browser', run: vi.fn() },
      alternate: { external: true, label: 'System Browser', run: vi.fn() },
      focusTerminal: vi.fn()
    }
    ...
    expect(
      screen.getByText('Manta Browser').closest('button')?.querySelector('.lucide-globe')
    ).toBeTruthy()
    expect(
