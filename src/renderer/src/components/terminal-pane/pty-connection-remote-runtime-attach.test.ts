...
describe('connectPanePty', () => {
  ...
  it('runs an inline setup terminal locally instead of failing its host closed', async () => {
    ...
    const setupWorktreeId =
      'ephemeral-setup-terminal:settings-mobile-emulator-manta-cli-skill-terminal'
    mockStoreState = {
      ...mockStoreState,
  ...
  it('withholds a local spawn while a repo-backed worktree has no hydrated host', async () => {
    ...
    mockStoreState = {
      ...
      worktreesByRepo: {
        repo1: [{ id: 'wt-remote', repoId: 'repo1', path: '/tmp/manta-docker-relay-perf-repo' }]
      },
      repos: []
    } as StoreState
    ...
    const deps = createDeps({
      ...
      cwd: '/tmp/manta-docker-relay-perf-repo'
    })
