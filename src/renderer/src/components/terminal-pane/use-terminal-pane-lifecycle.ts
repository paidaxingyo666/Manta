...
export function useTerminalPaneLifecycle(deps: UseTerminalPaneLifecycleDeps): void {
  ...
  useEffect(() => {
    ...
    window.addEventListener('manta:wake-hibernated-agents-worktree', onWakeHibernatedAgents)
    return () =>
      window.removeEventListener('manta:wake-hibernated-agents-worktree', onWakeHibernatedAgents)
