...
describe('getTerminalUrlOpenHint', () => {
  ...
  // Why: with links already opening in Manta, inverting still lands on the system
  // browser, so the hint must not promise Manta.
  it('keeps the system-browser wording when inverting but links open in Manta', () => {
    stubPlatform(true)
  ...
  it('names Manta when inverting and links open externally', () => {
    stubPlatform(true)
    expect(getTerminalUrlOpenHint({ openLinksInApp: false, modifierInverts: true })).toBe(
      'Click for actions, ⌘+click to open, or ⇧⌘+click to open in Manta'
    )
  })
  ...
  it('uses the Ctrl chord off macOS', () => {
    stubPlatform(false)
    expect(getTerminalUrlOpenHint({ openLinksInApp: false, modifierInverts: true })).toBe(
      'Click for actions, Ctrl+click to open, or Shift+Ctrl+click to open in Manta'
    )
  })
  ...
  it('omits the action-menu gesture when terminal link actions are disabled', () => {
    ...
    ).toBe('Ctrl+click to open, or Shift+Ctrl+click to open in Manta')
  })
})
...
describe('terminalUrlOpenHintOptionsFor', () => {
  ...
  // Why: openHttpLink refuses to route a remote-owned URL into Manta, so promising
  // "open in Manta" there would advertise a click that lands somewhere else.
  ...
  // can still reach Manta while some other pane's runtime is active.
