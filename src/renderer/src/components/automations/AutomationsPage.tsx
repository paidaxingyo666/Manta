...
export default function AutomationsPage(): React.JSX.Element {
  ...
  const [createTarget, setCreateTarget] = useState<AutomationCreateTarget>('manta')
  ...
  const automationHookCheckPromisesRef = useRef<
    Map<string, Promise<{ hooks: MantaHooks | null; ok: boolean }>>
  >(new Map())
  const [automationYamlHooksByRepoKey, setAutomationYamlHooksByRepoKey] = useState<
    Record<string, MantaHooks | null>
  >({})
  ...
  const loadAutomationYamlHooksForRepo = useCallback(
    async (repoId: string, hostId?: ExecutionHostId): Promise<MantaHooks | null> => {
      ...
      const promise = checkRuntimeHooks(settingsForRepo, repoId, hostId)
        .then((result) => ({
          hooks: result.status === 'error' ? null : ((result.hooks as MantaHooks | null) ?? null),
  ...
  const createDestination = useAutomationCreateDestination({
    open: createOpen && editingAutomationId === null && createTarget === 'manta',
  ...
  const saveAutomation = async (now: number): Promise<void> => {
    ...
    try {
      ...
      let setupDecision = resolveAutomationSetupDecisionForSave({
        ...
        projectHostSetups: setupProjectHostSetups,
        yamlHooks:
          createTarget === 'manta' && draft.workspaceMode === 'new_per_run'
  ...
  return (
    <main className="relative flex h-full min-h-0 flex-col bg-background pt-5 text-foreground md:pt-6">
      ...
      <AutomationEditorDialog
        ...
        editDestination={isMantaForm ? editDestinationControl : undefined}
        notice={editorNotice}
