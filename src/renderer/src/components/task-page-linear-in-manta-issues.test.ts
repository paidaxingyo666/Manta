...
} from './task-page-linear-in-manta-issues'
...
describe('filterLinearIssuesForInMantaWorkspace', () => {
  it('keeps issues without workspace metadata when a workspace is selected', () => {
    ...
    expect(
      filterLinearIssuesForInMantaWorkspace(issues, 'ws-a').map((item) => item.identifier)
    ).toEqual(['ENG-1', 'ENG-3'])
  })
})
