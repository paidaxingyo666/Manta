  ...
  if (forceDeleteReason) {
    ...
    if (forceDeleteReason === 'unstopped-pty') {
      return {
        ...
        // Telling a user "could not confirm" about terminals Manta watched stay alive asks
    ...
    if (forceDeleteReason === 'running-agent-session') {
      return {
        ...
        // these sessions, and only the observation AFTER that attempt separates one Manta watched
    ...
    if (forceDeleteReason === 'missing-registration') {
      return {
        ...
        description: translate(
          ...
          'Git already removed this workspace. Use Force Delete to clear it from Manta.'
        ),
        isDestructive: false
      }
    }
    return {
      ...
      // made a normal cleanup step look like a Manta bug, so this common case
