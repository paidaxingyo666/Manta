    ...
    if (
      ...
      toast.error(
        translate(
          ...
          'That SSH host is already in Manta.'
        )
      )
      return 'validation-failed'
    }
  ...
  if (typeof ssh.resolveConfigHost !== 'function') {
    throw new Error(
      translate(
        ...
        'Restart Manta to finish applying the SSH config picker update.'
      )
    )
  }
...
/** Bulk-load ~/.ssh/config hosts into Manta’s host list (sidebar targets). */
export async function addAllSshConfigHostsToManta({
  ssh,
  recordSshRepoReadoptions,
  setSshTargetsMetadata,
  recordFeatureInteraction
    ...
    return {
      ...
      newHostCount: value.filter(
        (host): host is SshConfigHostSummary =>
          typeof host === 'object' && host !== null && host.alreadyInManta === false
      ).length,
