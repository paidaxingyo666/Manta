...
describe('shouldDeferRemoteFileBrowserPasteResolve', () => {
  it('keeps small path and filter pastes on the immediate resolver path', () => {
    expect(shouldDeferRemoteFileBrowserPasteResolve('/workspaces/manta/src')).toBe(false)
...
describe('parsePathInput', () => {
  ...
  it('`Documents/manta` commits `Documents` and filters by `manta`', () => {
    expect(parsePathInput('Documents/manta')).toEqual({
      mode: 'path',
      base: 'cwd',
      committedSegments: ['Documents'],
      trailingFilter: 'manta'
    })
  })
...
describe('resolveSegmentStep', () => {
  const listing: DirEntry[] = [
    ...
    { name: 'manta-internal', isDirectory: true },
  ...
  it('unique prefix descends', () => {
    expect(resolveSegmentStep('manta', '/home/neil', listing)).toEqual({
      type: 'descend',
      name: 'manta-internal'
    })
  })
