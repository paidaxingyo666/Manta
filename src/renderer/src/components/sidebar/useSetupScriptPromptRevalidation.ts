...
/**
 * Re-runs the setup-script prompt inspection when a shared `manta.yaml` setup hook
...
export function useSetupScriptPromptRevalidation(input: {
  ...
  // Why: manta.yaml is edited on disk or the hook runs in a terminal outside React
  // state. Re-inspect on window focus so returning to Manta detects it (mirrors
  ...
  useEffect(() => {
    ...
    // Why: a runtime reconnect can turn an unreadable manta.yaml into a readable one,
