  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from Manta. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from Manta. Git worktrees will also be removed from git and disk; folder workspaces will only remove the Manta workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from Manta. The project folder on disk will not be deleted.'
