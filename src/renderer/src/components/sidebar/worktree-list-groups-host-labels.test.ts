...
describe('buildRows with pinned worktrees', () => {
  it('groups Windows host and WSL setups on the same runtime host', () => {
    ...
    const windowsRepo: Repo = {
      ...repo,
      id: 'repo-windows',
      path: String.raw`C:\Users\alice\git\manta`,
      displayName: 'manta',
    ...
    const wslRepo: Repo = {
      ...repo,
      id: 'repo-wsl',
      path: String.raw`\\wsl.localhost\Ubuntu\home\alice\git\manta`,
      displayName: 'manta',
      executionHostId: runtimeHostId
    }
    const windowsWorktree: Worktree = {
      ...
      path: String.raw`C:\Users\alice\git\manta\feature`
    }
    const wslWorktree: Worktree = {
      ...worktree,
      id: 'wt-wsl',
      repoId: wslRepo.id,
      path: String.raw`\\wsl.localhost\Ubuntu\home\alice\git\manta\feature`
    }
  ...
  it('uses saved host labels for mixed-host sidebar card badges', () => {
    const runtimeRepo: Repo = {
      ...remoteRepo,
      id: 'repo-runtime',
      path: '/Users/alice/runtime-manta',
      connectionId: null,
  ...
  it('shows distinct Manta server names when status grouping mixes runtime hosts', () => {
  ...
  it('keeps same-named repos separate without project setup identity', () => {
    const rows = buildRows(
      ...
      new Map([
        [repo.id, { ...repo, displayName: 'manta' }],
        [remoteRepo.id, { ...remoteRepo, displayName: 'manta' }]
      ]),
      null,
      new Set()
    )
  ...
  it('returns project group keys for worktree reveal when project setup identity exists', () => {
    ...
    ).toBe('project:github:stablyai/manta')
  })
})
