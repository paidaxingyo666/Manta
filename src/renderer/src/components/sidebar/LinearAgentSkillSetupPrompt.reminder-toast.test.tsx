...
const HOST_DISMISS_STORAGE_KEY = 'manta.linearTicketsSkill.setupDismissed.host'
...
vi.mock('@/lib/agent-skill-cli-prerequisite', () => ({
  ...
  ensureMantaCliAvailableForAgentSkillTerminal: mocks.ensureCli,
  isMantaCliAvailableOnPath: (status: CliInstallStatus | null | undefined) =>
...
function cliStatus(overrides: Partial<CliInstallStatus>): CliInstallStatus {
  return {
    ...
    pathConfigured: true,
    launcherPath: '/Applications/Manta.app/Contents/MacOS/Manta',
    installMethod: 'symlink',
    supported: true,
    state: 'installed',
    currentTarget: '/Applications/Manta.app/Contents/MacOS/Manta',
...
describe('LinearAgentSkillSetupPrompt reminder toast', () => {
  ...
  it('shows a warning toast on a later modal-only activation after a casual close', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      'Manta CLI and Linear skill are missing',
      expect.objectContaining({
        id: 'linear-agent-skill-setup-manta.linearTicketsSkill.setupDismissed.host',
        description:
          'Install the Manta CLI and the Linear skill to enable your agents to read and edit Linear tasks.',
        action: {
          label: 'Set up',
  ...
  it('does not repeat the Manta CLI in CLI-only reminder toast copy', async () => {
    mocks.skillState.installed = true
    await snoozeInitialModal({ linked: true, remote: false, surface: 'modal' })
    await renderPrompt({ linked: true, remote: false, surface: 'modal' })

    expect(toast.warning).toHaveBeenCalledWith(
      'Manta CLI is missing',
      expect.objectContaining({
        description: 'Install the Manta CLI to enable your agents to read and edit Linear tasks.'
      })
    )
  })
  ...
  it('keeps remote setup nuance in reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        description:
          'Install the Manta CLI and the Linear skill to enable your agents to read and edit Linear tasks. Remote agent environments may need their own setup.'
      })
    )
  })
  ...
  it('keeps WSL target nuance in reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        description:
          'Install the Manta CLI and the Linear skill to enable your agents to read and edit Linear tasks. This setup runs in the selected WSL agent runtime.'
      })
    )
  })
  ...
  it('opens the setup dialog from the reminder toast action', async () => {
    ...
    expect(toast.dismiss).toHaveBeenCalledWith(
      'linear-agent-skill-setup-manta.linearTicketsSkill.setupDismissed.host'
    )
  })
