...
describe('getDeleteWorktreeToastCopy', () => {
  ...
  it('uses terminal-teardown guidance when a PTY stop could not be proven', () => {
    expect(
      ...
      title: 'Failed to delete workspace feature/foo',
      description:
        'Manta could not confirm every terminal in this workspace has exited, so it stopped before deleting any files. Use Force Delete to remove it anyway.',
      isDestructive: false
    })
  })

  // Why: Force Delete proceeds on a proven-live PTY too, so the copy must not describe
  // that as an unconfirmed exit — the user is killing a terminal Manta watched running.
  it('names the running terminals when verification proved they are still live', () => {
    expect(
      ...
      title: 'Failed to delete workspace feature/foo',
      description:
        'This workspace still has running terminals, so Manta stopped before deleting any files. Force Delete will kill them and discard any uncommitted work they hold.',
      isDestructive: false
    })
  })

  // Why: the structured sweep now CLOSES an attached session on the ordinary delete, so reaching
  // this toast means the close was attempted and did not settle — not that Manta declined to try.
  it('offers force delete when an agent session could not be confirmed closed', () => {
    expect(
      ...
      title: 'Failed to delete workspace feature/foo',
      description:
        'Manta could not confirm every agent session in this workspace has closed, so it stopped before deleting any files. Use Force Delete to remove it anyway.',
      isDestructive: false
    })
  })

  // Why: the same split the PTY pair above draws. Force Delete proceeds either way, and telling a
  // user "could not confirm" about a conversation Manta watched stay attached asks them to waive a
  ...
  it('names the running agent sessions when the close left them attached', () => {
    expect(
      ...
      title: 'Failed to delete workspace feature/foo',
      description:
        'This workspace still has running agent sessions that Manta could not close, so it stopped before deleting any files. Force Delete will discard any work they hold.',
      isDestructive: false
    })
  })
  ...
  it('uses stale-row guidance when Git already removed the worktree directory', () => {
    expect(
      ...
      title: 'Failed to delete workspace feature/foo',
      description: 'Git already removed this workspace. Use Force Delete to clear it from Manta.',
      isDestructive: false
    })
  })
