      ...
      <DialogHeader className="text-left">
        ...
        <DialogDescription>
          {translate(
            ...
            'Pick a host to fill the form, or add every new host to Manta’s host list.'
          )}
        </DialogDescription>
      </DialogHeader>
          ...
          >
            {hosts.map((host) => (
              <li key={host.alias}>
                <button
                  type="button"
                  disabled={picksDisabled || host.alreadyInManta}
                  className={cn(
                ...
                >
                  ...
                  {resolvingAlias === host.alias ? (
                    ...
                    >
                      {translate(
                        'auto.components.sidebar.AddRemoteHostDialog.sshConfigPickerInManta',
                        'In Manta'
                      )}
                    </Badge>
          ...
          {isBulkImporting
            ...
            : newHostCount > 0
              ? translate(
                  ...
                  'Add all {{value0}} to Manta',
                  { value0: newHostCount }
                )
              : // Why: the remainder can be already-in-Manta or merely tombstoned, so the
