...
describe('buildWorktreeMetaUpdates', () => {
  ...
  it('rejects issue URLs in the PR input', () => {
    expect(buildUpdates({ prInput: 'https://github.com/stablyai/manta/issues/6933' })).toEqual({})
  })

  it('accepts PR URLs in the PR input', () => {
    expect(buildUpdates({ prInput: 'https://github.com/stablyai/manta/pull/6934' })).toEqual({
      linkedPR: 6934
    })
  })
  ...
  it('accepts issue URLs in the issue input', () => {
    expect(
      buildUpdates(
        { issueInput: 'https://github.com/stablyai/manta/issues/6933' },
        {},
        { linkedLinearIssue: 'STA-335' }
      )
    ).toEqual({
  ...
  it('rejects PR URLs in the issue input', () => {
    expect(buildUpdates({ issueInput: 'https://github.com/stablyai/manta/pull/6934' })).toEqual({
      linkedPR: null
    })
  })
