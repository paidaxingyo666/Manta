...
describe('WorktreeCard quick actions', () => {
  ...
  it('renders repo identity in the detailed metadata row', () => {
    ...
    expect(markup).not.toContain('aria-label="Project manta"')
    expect(markup).toContain('>manta</span>')
  ...
  it('renders detached HEAD identity in detailed card metadata', () => {
    ...
    const markup = renderToStaticMarkup(
      <WorktreeCard
        worktree={makeWorktree({ displayName: 'manta', branch: '' })}
        repo={makeRepo()}
        isActive={false}
        hideRepoBadge
      />
