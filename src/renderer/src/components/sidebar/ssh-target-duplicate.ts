...
/** True when an existing Manta host already owns this config alias / label. */
export function isDuplicateSshTargetAlias({
  ...
  // Why: the config picker's `alreadyInManta` flag compares lowercased aliases; match it or the
 ...
 *  otherwise an alias it greys out as "In Manta" is still savable as a second target. */
