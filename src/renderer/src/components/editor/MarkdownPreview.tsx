...
const markdownPreviewSanitizeSchema = {
  ...
  attributes: {
    ...
    details: [
      ...
      'open',
      ['className', 'manta-details'],
      ['dataMantaToggle', 'heading-1', 'heading-2', 'heading-3', 'heading-4', 'heading-5']
    ],
      ...
      a: ({ href, children, className, ...props }) => {
        ...
        const handleClick = async (event: React.MouseEvent<HTMLAnchorElement>): Promise<void> => {
          ...
          if (target.protocol === 'http:' || target.protocol === 'https:') {
            // Why: route through openHttpLink (not shell.openUrl) so a plain click honors "open links in Manta"; remote runtimes stay on the system browser.
            openHttpLink(
              target.toString(),
              resolveMarkdownPreviewHttpOpenOptions(
                event,
          ...
          if (!targetWorktree) {
            if (sourceRoutingWorktreeId && worktreeRoot) {
              // Why: floating markdown lives in a synthetic workspace with no repo worktree, though Manta can still open links relative to the source root.
...
function MarkdownSingleNoteSendMenu({
  ...
  return (
    <NotesSendMenu
      ...
      triggerClassName="manta-diff-comment-pill-btn"
...
function MarkdownAnnotationComposer({
  ...
  return (
    ...
    >
      <div className="manta-diff-comment-popover-label">
