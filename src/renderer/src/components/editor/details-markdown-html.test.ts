...
function nestedToggles(depth: number): string {
  let html = '<details class="manta-details" open>\n<summary>leaf</summary>\n\nBody\n\n</details>'
  for (let level = depth - 1; level > 0; level -= 1) {
    html = `<details class="manta-details" open>\n<summary>level ${level}</summary>\n\n${html}\n\n</details>`
  }
  return html
}
...
describe('details markdown html', () => {
  ...
  it('accepts heading-5 toggle variants and rejects unsupported levels', () => {
    ...
    expect(parseDetailsAttributes(' data-manta-toggle="heading-5"')).toMatchObject({
      variant: 'heading-5'
    })
    expect(parseDetailsAttributes(' data-manta-toggle="heading-6"')).toMatchObject({
      variant: null
    })

    const editableHeading5: DetailsHtmlBlock = {
      raw: '',
      openingAttributes: ' data-manta-toggle="heading-5"',
      inner: '<summary>Toggle</summary><p>Body</p>'
    }
    const unsupportedHeading6: DetailsHtmlBlock = {
      raw: '',
      openingAttributes: ' data-manta-toggle="heading-6"',
      inner: '<summary>Toggle</summary><p>Body</p>'
    }
  ...
  it('rejects a toggle whose nested toggle is not itself editable', () => {
    const block: DetailsHtmlBlock = {
      raw: '',
      openingAttributes: ' class="manta-details"',
