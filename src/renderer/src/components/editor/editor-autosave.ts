...
export const MANTA_EDITOR_QUIESCE_FILE_SAVES_EVENT = 'manta:editor-quiesce-file-saves'
export const MANTA_EDITOR_EXTERNAL_FILE_CHANGE_EVENT = 'manta:editor-external-file-change'
export const MANTA_EDITOR_SAVE_FILE_EVENT = 'manta:editor-save-file'
export const MANTA_EDITOR_SAVE_AND_CLOSE_EVENT = 'manta:save-and-close'
export const MANTA_EDITOR_FILE_SAVED_EVENT = 'manta:editor-file-saved'
export const MANTA_EDITOR_REQUEST_CMD_SAVE_EVENT = 'manta:editor-request-cmd-save'
export const MANTA_EDITOR_REQUEST_FILE_CLOSE_EVENT = 'manta:editor-request-file-close'
...
export async function requestEditorSaveQuiesce(target: EditorSaveQuiesceTarget): Promise<void> {
  await new Promise<void>((resolve) => {
    ...
    window.dispatchEvent(
      new CustomEvent<EditorSaveQuiesceDetail>(MANTA_EDITOR_QUIESCE_FILE_SAVES_EVENT, {
        detail: {
          ...target,
          claim: () => {
            claimed = true
...
export async function requestEditorFileSave(target: EditorSaveFileTarget): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    let claimed = false
    window.dispatchEvent(
      new CustomEvent<EditorSaveFileDetail>(MANTA_EDITOR_SAVE_FILE_EVENT, {
        detail: {
          ...target,
          claim: () => {
            claimed = true
...
export function requestEditorFileClose(fileId: string): void {
  window.dispatchEvent(
    new CustomEvent<EditorRequestFileCloseDetail>(MANTA_EDITOR_REQUEST_FILE_CLOSE_EVENT, {
      detail: { fileId }
    })
  )
}
...
export function notifyEditorExternalFileChange(target: EditorPathMutationTarget): void {
  window.dispatchEvent(
    new CustomEvent<EditorPathMutationTarget>(MANTA_EDITOR_EXTERNAL_FILE_CHANGE_EVENT, {
      detail: target
    })
  )
}
