...
// reads the clipboard with navigator.clipboard.readText(). Manta runs the renderer with
...
// (execCommand('copy') from a user gesture) both work. We route the read through Manta's
// trusted clipboard IPC bridge instead, matching how the terminal already reads it.
export const MANTA_CONTEXT_MENU_PASTE_PRIORITY = 10001
export const MANTA_CONTEXT_MENU_PASTE_NAME = 'manta-ipc-paste'
...
export type MantaContextMenuPasteDeps = {
...
export type MantaContextMenuPasteOutcome =
 ...
 * bridge); otherwise performs the paste through Manta's IPC clipboard read and
 * returns a Promise (truthy) so Monaco's blocked default never runs.
 */
export function runMantaContextMenuPaste(
  deps: MantaContextMenuPasteDeps
): false | Promise<MantaContextMenuPasteOutcome> {
  ...
  return performMantaContextMenuPaste(editorInstance, deps)
}

async function performMantaContextMenuPaste(
  editorInstance: PasteCapableEditor,
  deps: MantaContextMenuPasteDeps
): Promise<MantaContextMenuPasteOutcome> {
  let text: string
  try {
