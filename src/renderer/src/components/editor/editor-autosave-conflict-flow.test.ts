...
import { MANTA_EDITOR_EXTERNAL_FILE_CHANGE_EVENT, requestEditorFileSave } from './editor-autosave'
...
function dispatchExternalChange(): void {
  window.dispatchEvent(
    new CustomEvent(MANTA_EDITOR_EXTERNAL_FILE_CHANGE_EVENT, {
...
describe('editor autosave changed-on-disk conflict flow', () => {
  ...
  it('does not backstop-mark a dirty tab for the echo of its own save', () => {
    ...
    // event is Manta's own echo, not an external change.
    recordSelfWrite('/repo/file.ts', 'manta save')
