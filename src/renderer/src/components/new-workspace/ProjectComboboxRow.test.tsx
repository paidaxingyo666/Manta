...
describe('MatchedText', () => {
  it('underlines the matched run', () => {
    act(() => {
      root.render(<MatchedText text="manta" hits={[0, 1, 2, 3, 4]} />)
    })

    expect(markedText()).toBe('manta')
  })
  ...
  it('aligns marks with the query when the name starts with an emoji', () => {
    ...
    const [match] = rankProjectOptions([option], 'manta', [])
  ...
  it('leaves text unmarked when there are no hits', () => {
    act(() => {
      root.render(<MatchedText text="🚀 manta" hits={[]} />)
    })

    expect(container.querySelectorAll('mark')).toHaveLength(0)
    expect(container.textContent).toBe('🚀 manta')
  })
})

describe('ProjectOptionDetail', () => {
  it('keeps Windows path matches highlighted in the preserved tail', () => {
    const detail = 'C:\\Users\\ada\\projects\\manta\\src\\renderer\\app.ts'
