...
describe('deriveAiVaultWorkspaceScopePaths', () => {
  it('returns the active workspace path', () => {
    ...
    expect(deriveAiVaultWorkspaceScopePaths(active, [active])).toEqual(['/repo/manta'])
  })
  ...
  it('drops a prior path another live workspace now owns', () => {
    ...
    const claimant = makeWorktree({ id: 'repo-1::/repo/manta', path: '/repo/manta' })
    const active = makeWorktree({
      id: 'repo-1::/repo/manta-renamed',
      path: '/repo/manta-renamed',
      priorWorktreeIds: ['repo-1::/repo/manta']
  ...
  it('keeps a prior path the active workspace itself still owns', () => {
    const active = makeWorktree({ priorWorktreeIds: ['repo-1::/repo/manta'] })
    expect(deriveAiVaultWorkspaceScopePaths(active, [active])).toEqual(['/repo/manta'])
  })
  ...
  it('drops a claimed prior path regardless of where the claimant sits in the list', () => {
    ...
    const activeAtPriorPath = makeWorktree({ id: active.id, path: '/repo/manta' })
  ...
  it('derives workspace scope paths at scale', () => {
    ...
    const prefix = '/Users/dev/manta/workspaces/manta-monorepo/feature-'
...
describe('deriveAiVaultScopeSessionPaths', () => {
  ...
  it('deduplicates paths that differ only by separators or trailing slash', () => {
    ...
    const active = makeWorktree()
    const trailing = makeWorktree({ id: 'repo-1::a', path: '/repo/manta/' })
    const doubled = makeWorktree({ id: 'repo-1::b', path: '/repo//manta' })
