...
export type ChecksPanelHostedReviewModifierDestination = 'system-browser' | 'manta' | null

// Why: mirrors openHttpLink's routing inputs — with inverting on and Link Routing off the
// modifier now reaches Manta here, so gating the hint on openLinksInApp alone hides a live gesture.
export function resolveChecksPanelHostedReviewModifierDestination(
  ...
  // runtime id while the click still routes to Manta.
  ...
  return settings?.openLinksInAppModifierInverts === true ? 'manta' : null
}
