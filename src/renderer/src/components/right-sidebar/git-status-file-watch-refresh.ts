...
export function useGitStatusFileWatchRefresh({
  ...
  useEffect(() => {
    ...
    window.addEventListener(MANTA_WORKTREE_FILE_CHANGE_EVENT, handleFsChanged as EventListener)

    return () => {
      if (refreshTimer) {
        clearTimeout(refreshTimer)
      }
      window.removeEventListener(MANTA_WORKTREE_FILE_CHANGE_EVENT, handleFsChanged as EventListener)
    }
