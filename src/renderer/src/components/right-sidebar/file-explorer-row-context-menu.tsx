...
export function FileExplorerRowContextMenu({
  ...
  const handleOpenInMantaBrowser = useCallback(() => {
    if (!activeWorktreeId) {
      return
    }
  ...
  return (
    ...
    >
      ...
      {!node.isDirectory && activeWorktreeId && canOpenInMantaBrowser && (
        <ContextMenuItem onSelect={handleOpenInMantaBrowser}>
          <Globe />
          {translate(
