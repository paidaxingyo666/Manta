...
describe('openAiVaultSessionLogInManta', () => {
  it('authorizes the exact path and opens a permanent read-only local tab', async () => {
    const state = makeState()
    getStateMock.mockReturnValue(state)

    await openAiVaultSessionLogInManta({ filePath: LOG_PATH, executionHostId: 'local' })
  ...
  it('withholds blank, remote, and synthetic paths without authorizing', async () => {
    ...
    await openAiVaultSessionLogInManta({ filePath: '   ', executionHostId: 'local' })
    await openAiVaultSessionLogInManta({ filePath: LOG_PATH, executionHostId: 'ssh:dev-box' })
    await openAiVaultSessionLogInManta({
  ...
  it('shares one in-flight open for concurrent clicks of the same path', async () => {
    ...
    const first = openAiVaultSessionLogInManta({ filePath: LOG_PATH, executionHostId: 'local' })
    const second = openAiVaultSessionLogInManta({ filePath: LOG_PATH, executionHostId: 'local' })
    resolveAuth?.()
