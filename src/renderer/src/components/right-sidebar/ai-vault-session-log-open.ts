...
/**
 * Open a local AI Vault session log inside Manta as a permanent, read-only editor
 * tab (or activate an existing tab without reducing its authority). Reuses
 * Manta's external-file authorize + `openFile` pipeline; it never grants write
 * capability by itself and never redirects the open to a remote host.
 */
export async function openAiVaultSessionLogInManta(session: AiVaultLogSession): Promise<void> {
  ...
  if (!filePath || !canOpenAiVaultSessionLogInManta(session)) {
    return
  }
  ...
  try {
    ...
    try {
      ...
      // trust gesture. Reuses Manta's existing external R/W open grant.
