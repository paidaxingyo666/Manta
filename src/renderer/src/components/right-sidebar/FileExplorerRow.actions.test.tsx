...
describe('FileExplorerRow collapse folder action', () => {
  ...
  it('shows remote download only for desktop SSH rows and file-like Remote Host rows', () => {
    ...
    ;(globalThis as { __MANTA_WEB_CLIENT__?: boolean }).__MANTA_WEB_CLIENT__ = true
  ...
  it('shows OS file copy for single local rows and SSH file rows on desktop', () => {
    const previous = (globalThis as { __MANTA_WEB_CLIENT__?: boolean }).__MANTA_WEB_CLIENT__
    try {
      ...
      ;(globalThis as { __MANTA_WEB_CLIENT__?: boolean }).__MANTA_WEB_CLIENT__ = previous
    }
  })
  ...
  it('shows an actionable toast when remote clipboard staging is unavailable', async () => {
    ...
    expect(toastErrorMock).toHaveBeenCalledWith(
      "Could not copy the file because Manta's temporary storage is unavailable"
    )
  })
