...
describe('checks panel hosted review modifier hint destination', () => {
  it('names the system browser when a plain click already opens in Manta', () => {
    expect(resolveChecksPanelHostedReviewModifierDestination({ openLinksInApp: true }, true)).toBe(
      'system-browser'
    )
  })

  // Why: inverting is inert while links already open in Manta — both meanings of the
  // modifier land on the system browser, so checking inverts first would misname it.
  it('names the system browser when inverting is on and links already open in Manta', () => {
    expect(
      resolveChecksPanelHostedReviewModifierDestination(
  ...
  // and the only way to reach Manta from this button is undiscoverable.
  it('names Manta when inverting is on and links open externally', () => {
    expect(
      resolveChecksPanelHostedReviewModifierDestination(
  ...
  // Why: openHttpLink refuses to route a remote-owned link into Manta, and openLinksInApp
  ...
  // not suppress a hint for a click that still reaches Manta.
  ...
  // Why: openHttpLink gates routing to Manta on a worktree id, so without one the
