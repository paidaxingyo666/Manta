...
const WORKSPACE_PATH = '/repo/manta'

function session(overrides: Partial<ResumeInChatSession> = {}): ResumeInChatSession {
  return {
    agent: 'claude',
    cwd: WORKSPACE_PATH,
    filePath: '/home/dev/.claude/projects/-repo-manta/session-1.jsonl',
...
function eligibility(
  ...
  return resolveAiVaultSessionResumeInChatEligibility({
    session: session(),
    targetWorkspaceId: 'repo-1::/repo/manta',
...
describe('resolveAiVaultSessionResumeInChatEligibility', () => {
  it('offers the chat for a local Claude row in its own workspace', () => {
    expect(eligibility()).toEqual({ available: true, workspaceId: 'repo-1::/repo/manta' })
  })
  ...
  it('refuses a row already adopted into a chat before any other check', () => {
    ...
    expect(
      eligibility({
        session: {
          ...session(),
          structuredSession: { sessionId: 'claude_1', workspaceId: 'repo-1::/repo/manta' }
        }
      })
...
describe('workspace matching, which only Claude is bound by', () => {
  ...
  it('treats Windows spellings of one directory as the same workspace', () => {
    expect(
      eligibility({
        session: session({ cwd: 'C:\\Users\\Dev\\repo\\Manta\\' }),
        targetWorkspacePath: 'c:/users/dev/repo/manta'
      })
    ).toMatchObject({ available: true })
  })
})
...
describe('aiVaultSessionCwdMatchesWorkspace', () => {
  it('ignores separator, case, and a trailing slash', () => {
    expect(aiVaultSessionCwdMatchesWorkspace('C:\\repo\\Manta', 'c:/repo/manta')).toBe(true)
    expect(aiVaultSessionCwdMatchesWorkspace('/repo/manta/', '/repo/manta')).toBe(true)
    expect(aiVaultSessionCwdMatchesWorkspace(' /repo/manta ', '/repo/manta')).toBe(true)
  })

  it('never calls a missing path a match', () => {
    expect(aiVaultSessionCwdMatchesWorkspace(null, '/repo/manta')).toBe(false)
    expect(aiVaultSessionCwdMatchesWorkspace('/repo/manta', null)).toBe(false)
    expect(aiVaultSessionCwdMatchesWorkspace('', '')).toBe(false)
  })

  it('does not treat a sibling directory as the same workspace', () => {
    expect(aiVaultSessionCwdMatchesWorkspace('/repo/manta-2', '/repo/manta')).toBe(false)
  })
})
