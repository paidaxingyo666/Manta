...
export function resolveHostedReviewAuthInstruction(provider: HostedReviewProvider): string {
  ...
  if (provider === 'azure-devops') {
    return 'Set MANTA_AZURE_DEVOPS_TOKEN'
  }
  if (provider === 'gitea') {
    return 'Set MANTA_GITEA_TOKEN'
  }
...
export function resolveUnavailableCreateReviewLookupNoticeMessage(
  ...
  return `Create ${copy.shortLabel} failed: Manta could not confirm whether this branch already has a ${copy.reviewLabel}. Retry once the ${copy.providerName} lookup succeeds.`
}
