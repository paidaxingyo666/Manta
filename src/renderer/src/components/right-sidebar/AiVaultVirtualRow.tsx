...
import {
  canOpenAiVaultSessionLogInManta,
...
export function AiVaultVirtualRow({
  ...
  // Why: in-Manta View Log additionally withholds synthetic (SQLite/OpenCode)
  // identities that have no single file to open, while Reveal/CWD stay on the
  // existing local-path gate.
  const canOpenLogInManta = row.type === 'session' && canOpenAiVaultSessionLogInManta(row.session)
    ...
    >
      {row.type === 'group' ? (
        ...
        <VaultSessionRow
          ...
          onOpenLog={
            searchPathAllowed && canOpenLogInManta ? () => onOpenLog(row.session) : undefined
          }
          onRevealLog={
