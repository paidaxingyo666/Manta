...
import { isMantaWindowForegroundFocused } from '../terminal-pane/terminal-notification-pane-visibility'
...
function isViewedStructuredTab(
  ...
  if (!isMantaWindowForegroundFocused() || state.activeWorktreeId !== workspaceId) {
    return false
  }
...
export function createStructuredAttentionSurface(state: StoreSnapshot): AgentAttentionSurface {
  return {
    ...
    // Why: activeWorktreeId is in-app selection only. A backgrounded Manta still needs unread.
    isWorkspaceViewed: (workspaceId) =>
      state.activeWorktreeId === workspaceId && isMantaWindowForegroundFocused(),
