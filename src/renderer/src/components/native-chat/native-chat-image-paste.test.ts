...
describe('image paste agent map', () => {
  it('known image-capable agent attaches the temp file path', () => {
    ...
    const result = resolveImagePaste('claude', '/tmp/manta-img-123.png')
    expect(result).toEqual({ kind: 'attach', path: '/tmp/manta-img-123.png' })
  })
  ...
  it('grok attaches image paths like other vision-capable TUIs', () => {
    ...
    expect(resolveImagePaste('grok', '/tmp/manta-paste-1.png')).toEqual({
      kind: 'attach',
      path: '/tmp/manta-paste-1.png'
    })
  })
...
describe('isNativeChatPastedImagePath', () => {
  it('detects clipboard-paste temp files (so the chip shows a friendly label)', () => {
    expect(
      isNativeChatPastedImagePath(
        '/var/folders/x/manta-paste-1782775228480-c9a3c86b-1234-5678-9abc-def012345678.png'
      )
    ).toBe(true)
    // Windows-style separators resolve to the same basename.
    expect(isNativeChatPastedImagePath('C:\\Temp\\manta-paste-1-2.png')).toBe(true)
  })
