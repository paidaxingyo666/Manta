...
describe('useNativeChatComposerPaste', () => {
  ...
  it('saves on the SSH host and settles the chip on the returned remote path', async () => {
    mocks.saveClipboardImageAsTempFile.mockResolvedValue('/remote/tmp/manta-paste-1.png')
    const store = createChipStore()
    ...
    expect(store.chips).toEqual([
      {
        id: 'chip-1',
        path: '/remote/tmp/manta-paste-1.png',
  ...
  it('shows a pending chip before the save resolves', async () => {
    ...
    await act(async () => {
      resolveSave('/tmp/manta-paste-1.png')
    })
    expect(store.chips[0]).toMatchObject({ path: '/tmp/manta-paste-1.png', pending: false })
  })
  ...
  it('shows a pending chip for menu paste from the clipboard thumbnail probe', async () => {
    ...
    await act(async () => {
      resolveSave('/tmp/manta-paste-2.png')
    })
    expect(store.chips[0]).toMatchObject({ path: '/tmp/manta-paste-2.png', pending: false })
  })
  ...
  it('attaches directly when no clipboard preview was available', async () => {
    ...
    mocks.saveClipboardImageAsTempFile.mockResolvedValue('C:\\Temp\\manta-paste-3.png')
    const store = createChipStore()
    const attachResolvedPaths = vi.fn()
    const probe = await renderProbe({
      resolveAttachmentOwner: () => ({ kind: 'local' }),
    ...
    expect(store.chips).toHaveLength(0)
    expect(attachResolvedPaths).toHaveBeenCalledWith(['C:\\Temp\\manta-paste-3.png'], null)
  })
