...
function deps(
  ...
  return {
    ...
    deps: {
      ...
      destinations: overrides.destinations ?? { primary: 'system', alternate: 'manta' },
...
describe('handleNativeChatWebLink', () => {
  ...
  it('opens the destination popover on a plain click', () => {
    ...
    expect(requests[0]?.alternate?.label).toBe('Manta Browser')
  })
  ...
  it('opens the alternate destination on a shift+modifier click', () => {
    ...
    expect(mocks.openRoutedHttpLink).toHaveBeenCalledWith(
      'https://a/',
      expect.objectContaining({ forceDestination: 'manta' })
    )
  })
