...
function eligibleInventory(): SkillFreshnessInventory {
  return {
    schemaVersion: 1,
    installations: [placement('manta-cli')],
...
describe('SkillFreshnessUpdateDialog', () => {
  ...
  it('starts a background run with the eligible names', async () => {
    ...
    expect(skillsApi.startUpdateRun).toHaveBeenCalledWith(['manta-cli'])
  })
  ...
  it('shows indeterminate progress and says the run survives closing', async () => {
    ...
    expect(
      container?.querySelector('[data-skill-row="manta-cli"]')?.getAttribute('data-state-label')
    ).toBe('pending')
  })
  ...
  it('attributes failures to the names the re-scan says are still outdated', async () => {
    mocks.inventory = {
      schemaVersion: 1,
      installations: [placement('manta-cli'), placement('orchestration')],
      eligibleUpdateNames: ['manta-cli', 'orchestration'],
      scanIssues: [],
      scannedAt: 1
    }
    await renderDialog()
  ...
  it('keeps the same row elements across the whole run instead of swapping layouts', async () => {
    await renderDialog()
    await openViaRequest()
    const before = container?.querySelector('[data-skill-row="manta-cli"]')
    expect(before?.getAttribute('data-state-label')).toBe('available')

    await emitRun({ state: 'running', names: ['manta-cli'], startedAt: 1, output: '' })
    const during = container?.querySelector('[data-skill-row="manta-cli"]')
    ...
    mocks.inventory = {
      schemaVersion: 1,
      installations: [placement('manta-cli', { status: 'current', installedReleaseRevision: 2 })],
      eligibleUpdateNames: [],
      scanIssues: [],
      scannedAt: 5
    }
    await emitRun({ state: 'success', names: ['manta-cli'], finishedAt: 2, output: 'done' })
    await rerender()
    const after = container?.querySelector('[data-skill-row="manta-cli"]')
  ...
  it('collapses each skill’s locations behind its own disclosure', async () => {
    ...
    const row = container?.querySelector('[data-skill-row="manta-cli"]')
  ...
  it('shows the captured log verbatim without parsing it', async () => {
    ...
    await emitRun({
      ...
      names: ['manta-cli'],
      finishedAt: 2,
      output: 'Checking skills from source: stablyai/manta\n  ✓ Updated manta-cli'
    })

    expect(container?.querySelector('pre')?.textContent).toContain(
      'Checking skills from source: stablyai/manta'
    )
  })
  ...
  it('shows the up-to-date state once every installation is current', async () => {
    ...
    expect(container?.textContent).toContain('All installed Manta skills are up to date.')
  ...
  it('raises no row for a skill whose only finding is a project-owned copy', async () => {
    ...
    // directory. Manta only ever updates global skills, so a "Skipped" row here asserts
  ...
  it('keeps the stale-record remedy when a project copy is listed beside it', async () => {
    ...
    // runnable command with advice about a copy the user never asked Manta to update.
    mocks.inventory = {
      schemaVersion: 1,
      installations: [
  ...
  it('stays coherent while an idle re-scan is in flight', async () => {
    ...
    expect(container?.textContent).toContain('Checking installed Manta skills…')
  ...
  it('surfaces a scan error instead of a stale summary', async () => {
    ...
    expect(container?.querySelector('[data-skill-row="manta-cli"]')).not.toBeNull()
    ...
    expect(container?.querySelector('[data-skill-row="manta-cli"]')).toBeNull()
    expect(findButton('Update 1 skill')).toBeUndefined()
  })
  ...
  it('shows incomplete plugin coverage without presenting a fabricated skill copy', async () => {
    mocks.inventory = {
      schemaVersion: 1,
      installations: [
        placement('manta-cli', { status: 'current', observedPackageDigest: 'current' })
      ],
      eligibleUpdateNames: [],
      scanIssues: [
        {
    ...
    expect(container?.textContent).toContain(
      'Manta could not finish checking plugin-managed skills.'
    )
    ...
    expect(container?.textContent).not.toContain('All installed Manta skills are up to date.')
    ...
    expect(container?.textContent).not.toContain(
      '/home/.codex/plugins/cache/vendor/locked/manta-cli'
    )
  })
  ...
  // Why: Manta's own traversal bounds are not the user's to act on. Headlining them
