...
describe('groupSkillFreshness', () => {
    ...
    expect(groups[0]?.locations).toEqual([
      {
        ...
        path: '/home/.agents/skills/manta-cli',
        chip: null,
  ...
  it('raises no group when every finding is project-owned', () => {
    // Manta's updater only passes --global, so a project copy has no remedy; a row here
    // would claim Manta considered an update it could never perform.
    expect(
      groupSkillFreshness(
