...
export function SkillInstallManagementDialog({
  ...
  const load = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      setError('Manta could not inspect managed installs on this machine.')
    } finally {
      if (generation === loadGeneration.current) {
        setBusy(false)
      }
  ...
  const installVersion = async (discardLocal = false): Promise<void> => {
    ...
    try {
      ...
      setError('Manta could not verify the requested version.')
    } finally {
  ...
  const remove = async (discardLocal = false): Promise<void> => {
    ...
    try {
      ...
      setError('Manta could not safely remove this skill.')
    } finally {
      setBusy(false)
    }
  }
