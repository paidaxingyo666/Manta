 ...
 * wrong in a way the update cannot fix — an edited copy, one Manta could not read.
 *
...
export function groupSkillFreshness(
  ...
  for (const [name, entries] of byName) {
    ...
    // row would assert Manta considered an update it never could. Locations below are still
