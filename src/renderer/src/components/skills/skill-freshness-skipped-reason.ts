  ...
  switch (chip) {
    case 'newer':
      return translate(
        ...
        'This copy is a later version than the one this build of Manta ships, so Manta left it alone rather than roll it back. Updating Manta will bring the two back in line.'
      )
    case 'unrecognized':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonUnrecognized',
        'The copy here doesn’t match the official version — it may be modified, or a different skill with the same name. Manta left it out of the update so it won’t overwrite it. Remove it if you want Manta to update this skill.'
      )
    case 'read-only':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonReadOnly',
        'This copy is in a read-only location, so Manta left it out of the update. Change its permissions to let Manta update it.'
      )
    case 'inaccessible':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonInaccessible',
        'Manta couldn’t read this copy, so it left the skill out of the update.'
      )
    case 'in-a-repo':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonInRepo',
        'This is a project skill, not a global one — Manta only updates your global skills, so it left this out of the update.'
      )
    case 'plugin-cache':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonPluginCache',
        'A plugin manages this skill, so Manta left it out of the update — update the plugin instead.'
      )
    case 'external-link':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonExternalLink',
        'This copy is a shortcut pointing outside Manta’s skill folders, so Manta left it out of the update.'
      )
    case 'broken-link':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonBrokenLink',
        'This copy is a shortcut to something that no longer exists, so Manta left it out — you can safely delete it.'
      )
    case 'duplicate':
      return translate(
    ...
    case undefined:
      ...
      return skillName
        ...
        : translate(
            ...
            'Manta left this skill out of the update command.'
          )
  }
}
