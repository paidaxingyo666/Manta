...
export function DiffCommentCard({
  ...
  return (
    <div ref={cardRef} className="manta-diff-comment-card">
      <div className="manta-diff-comment-content-col">
        {/* Header Row */}
        <div className="manta-diff-comment-header">
          <div className="manta-diff-comment-meta-group">{metaText}</div>

          {/* Action buttons pill (only shown if not editing) */}
          {!editing && (
            <div
              className="manta-diff-comment-actions-pill"
              onMouseDown={(ev) => ev.stopPropagation()}
            >
              {headerActions}
              {headerActions && (url || onSubmitEdit || onDelete) && (
                <span className="manta-diff-comment-pill-divider" />
              )}
              {url && (
                <>
                  <button
                    type="button"
                    className="manta-diff-comment-pill-btn"
                    title={translate(
              ...
              {onSubmitEdit && (
                <>
                  ...
                  {onDelete && <span className="manta-diff-comment-pill-divider" />}
                </>
              )}
              {onDelete && (
                <button
                  type="button"
                  className="manta-diff-comment-pill-btn manta-diff-comment-pill-btn-danger"
                  title={translate(
        ...
        {/* Quote Block */}
        {quote ? (
          <div className="manta-diff-comment-quote">
            <div className="manta-diff-comment-quote-text">{quote}</div>
          </div>
        ) : null}
        ...
        {editing ? (
          <div className="flex flex-col gap-2 mt-1">
            <textarea
              ref={textareaRef}
              className="manta-diff-comment-popover-textarea"
              value={draft}
              onChange={(e) => {
              ...
              rows={3}
            />
            <div className="manta-diff-comment-popover-footer">
          ...
          <div className="manta-diff-comment-body">{body}</div>
        )}
      </div>
    </div>
  )
