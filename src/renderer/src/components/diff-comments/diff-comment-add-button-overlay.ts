...
export function installDiffCommentAddButtonOverlay({
  ...
  plus.type = 'button'
  plus.className = 'manta-diff-comment-add-btn'
  ...
  const rangeDrag = installDiffCommentRangeDrag({
    ...
    onDragChange: ({ dragging: isDragging, focusLine }) => {
      ...
      editorDomNode.classList.toggle('manta-diff-comment-range-dragging', isDragging)
  ...
  return {
    dispose: () => {
      ...
      rangeDrag.dispose()
      editorDomNode.classList.remove('manta-diff-comment-range-dragging')
      plus.remove()
    },
