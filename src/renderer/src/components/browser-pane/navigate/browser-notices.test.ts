...
describe('browser notice formatting', () => {
  it('formats denied permissions with safe copy', () => {
    ...
    ).toBe('this page asked for your location, and Manta denied it.')
  })
  ...
  it('names the storage permission in words rather than its raw token', () => {
    ...
    expect(notice).toBe(
      'https://example.com asked for cookie access on behalf of an embedded site, and Manta denied it.'
    )
  })
  ...
  it.each([
    ...
    ['openExternal', 'permission to open a link outside Manta'],
    ...
    ).toBe(`https://example.com asked for ${description}, and Manta denied it.`)
  })
  ...
  it('falls back to the raw permission name for anything unmapped', () => {
    ...
    ).toBe('https://example.com asked for some-future-permission, and Manta denied it.')
  })
