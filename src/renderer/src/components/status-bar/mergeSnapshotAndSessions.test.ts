...
describe('mergeSnapshotAndSessions', () => {
  ...
  it('includes browser-only workspaces in their repo', () => {
    const worktree = {
      id: 'manta::/Users/me/browser-only',
      repoId: 'manta',
    ...
    const out = mergeSnapshotAndSessions(
      ...
      [],
      baseCtx({
        repoDisplayNameById: new Map([['manta', 'MANTA']]),
  ...
  it('passes through snapshot worktrees with numeric metrics and hasLocalSamples', () => {
    const wt: WorktreeMemory = {
      worktreeId: 'manta::/Users/me/Triton',
      worktreeName: 'Triton',
      repoId: 'manta',
      repoName: 'MANTA',
      cpu: 1.5,
  ...
  it('binds a deferred SSH row so its single-row kill cannot skip confirmation', () => {
    ...
    const sessionId = 'manta::/remote/Stingray@@deferred1'
    const out = mergeSnapshotAndSessions(
      null,
      [{ id: sessionId, cwd: '', title: 'manta/Stingray', agentOwnership: 'absent' as const }],
      baseCtx({
        tabsByWorktree: { 'manta::/remote/Stingray': [makeTab('tab-ssh')] },
        deferredSshSessionIdsByTabId: { 'tab-ssh': sessionId },
        repoConnectionIdById: new Map([['manta', 'ssh-conn-1']])
      })
    )
  ...
  it('@@ parse: an SSH-style session id resolves to its worktree group', () => {
    const ds: DaemonSession[] = [
      {
        id: 'manta::/remote/Stingray@@abcd1234',
        cwd: '',
        title: 'manta/Stingray',
    ...
    expect(out[0].worktrees[0]).toMatchObject({
      worktreeId: 'manta::/remote/Stingray',
    ...
    expect(out[0].worktrees[0].sessions[0]).toMatchObject({
      sessionId: 'manta::/remote/Stingray@@abcd1234',
      hasLocalSamples: false,
      cpu: null,
      memory: null,
      bound: false
  ...
  it('warm-reattach local PTY: chip stays off when repo has no connectionId', () => {
    ...
    const ds: DaemonSession[] = [
      {
        id: 'manta::/local/Triton@@deadbeef',
        cwd: '/local/Triton',
        title: 'manta/Triton',
        agentOwnership: 'absent' as const
      }
    ...
    const ctx = baseCtx({
      repoConnectionIdById: new Map([['manta', null]])
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0]).toMatchObject({
      repoId: 'manta',
  ...
  it('tab walk wins over @@ parse when they disagree', () => {
    ...
    const ctx = baseCtx({
      tabsByWorktree: {
        'manta::/correct/path': [makeTab(tabId, 'My Tab')]
      },
      ptyIdsByTabId: { [tabId]: ['manta::/wrong/path@@feedface'] }
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0].worktrees[0].worktreeId).toBe('manta::/correct/path')
  ...
  it('treats startup deferred reattach tab ptyId wake hints as bound sessions', () => {
    ...
    const sessionId = 'manta::/Users/me/Triton@@deferred'
    const ds: DaemonSession[] = [
      {
        id: sessionId,
        cwd: '/Users/me/Triton',
    ...
    const ctx = baseCtx({
      tabsByWorktree: {
        'manta::/Users/me/Triton': [restoredTab]
      },
      ptyIdsByTabId: { [tabId]: [] }
    })
  ...
  it('local-bound interaction state: numeric metrics + bound=true + tabId set', () => {
    ...
    const ctx = baseCtx({
      tabsByWorktree: { 'manta::/Users/me/Triton': [makeTab(tabId)] },
  ...
  it('uses repoDisplayNameById to humanize new project groups when available', () => {
    const ds: DaemonSession[] = [
      { id: 'stably-ai/manta::/remote/Wt@@1', cwd: '', title: '', agentOwnership: 'absent' as const }
    ]
    const ctx = baseCtx({
      repoDisplayNameById: new Map([['stably-ai/manta', 'MANTA']])
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0].repoName).toBe('MANTA')
  })
...
it('resolves a duplicated tab id to the first tab, as the replaced scan did', () => {
  ...
  const out = mergeSnapshotAndSessions(
    makeSnapshot([wt]),
    [],
    baseCtx({ tabsByWorktree: { 'manta::/Users/me/Triton': [first, second] } })
  )
...
it('keeps tab labels scoped to their own worktree', () => {
  ...
  const out = mergeSnapshotAndSessions(
    makeSnapshot([wt]),
    [],
    baseCtx({ tabsByWorktree: { 'manta::/Users/me/Other': [makeTab('shared', 'Elsewhere')] } })
  )
...
it('attaches later daemon sessions to the first row of a duplicated worktree', () => {
  ...
  const out = mergeSnapshotAndSessions(
    ...
    [{ id: 'manta::/Users/me/Triton@@late', cwd: '', title: '', agentOwnership: 'unknown' }],
    baseCtx()
  )
