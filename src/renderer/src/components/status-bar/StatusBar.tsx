...
// Why: with a Remote Manta Server, local GlobalSettings describe this desktop, not the owner — the server snapshot wins (#7973).
...
const CLOSE_ALL_CONTEXT_MENUS_EVENT = 'manta-close-all-context-menus'
...
function StatusBarInner({ floatingTerminalOpen }: StatusBarProps): React.JSX.Element | null {
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      // Why: re-run PATH detection so a freshly-installed/removed CLI's bar appears/hides without restarting Manta.
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  ...
                  {showFloatingWorkspaceAttentionDot ? (
                    // Why: amber = Manta's "needs attention" convention; ring matches the fill so the dot reads on the icon.
                    <span
                      aria-hidden
