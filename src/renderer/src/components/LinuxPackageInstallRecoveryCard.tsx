...
function copiedNote(packageFileName: string): string {
  return translate(
    ...
    'Command copied. Quit Manta, run it in a system terminal to install {{value0}}, then reopen Manta.',
    {
...
export function LinuxPackageInstallRecoveryCard({
  ...
  const SUMMARY = translate(
    ...
    'Manta downloaded the system package. Quit Manta before finishing the update from a terminal.'
  )
  const EXPLAINER = translate(
    'auto.components.LinuxPackageInstallRecoveryCard.82c6dbea00',
    'Copy the command, quit Manta, and run it in a system terminal on the computer where Manta is installed. Reopen Manta after it finishes.'
  )
