...
type StarNagApi = {
  ...
  starManta: ReturnType<typeof vi.fn>
}
...
describe('StarNagToastHost', () => {
  ...
  beforeEach(() => {
    ...
    starNag = {
      ...
      starManta: vi.fn().mockResolvedValue(true)
    }
    shell = {
  ...
  it('renders exact onboarding toast copy and confirms only after direct star succeeds', async () => {
    ...
    expect(toastContainer.textContent).toContain(
      'If you’re enjoying Manta so far, a GitHub star helps other developers discover it.'
    )
    ...
    expect(starNag.starManta).toHaveBeenCalledTimes(1)
  ...
  it('switches to the explicit GitHub fallback when direct star fails', async () => {
    starNag.starManta.mockResolvedValueOnce(false)
    ;({ root, container } = renderHost())
  ...
  it('does not allow unresolved close while the primary action is busy', async () => {
    ...
    starNag.starManta.mockReturnValueOnce(pendingStar.promise)
    ;({ root, container } = renderHost())
