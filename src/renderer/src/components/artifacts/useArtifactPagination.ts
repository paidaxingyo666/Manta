...
export function artifactAccountIdentity(authStatus: MantaProfileAuthStatus | null): string | null {
...
function artifactRequestIsCurrent(
  ...
  return (
    ...
    artifactAccountIdentity(useAppStore.getState().mantaProfileAuthStatus) === identity
  )
}
      ...
      if (result.status === 'ok') {
        ...
        setError(
          translate(
            ...
            'Sign in to Manta again to load artifacts.'
          )
        )
      }
    } catch (loadError) {
