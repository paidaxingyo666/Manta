    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      toast.error(
        translate(
          ...
          'Sign in to Manta and try again.'
        )
      )
      return null
    }
...
async function ensureArtifactAccountConnected(): Promise<boolean> {
  ...
  if (state.mantaProfileAuthStatus?.state === 'connected') {
    return true
  }
  return (await state.connectCurrentMantaProfile())?.status === 'connected'
}

async function reconnectArtifactAccount(): Promise<boolean> {
  return (await useAppStore.getState().connectCurrentMantaProfile())?.status === 'connected'
}
  ...
  switch (code) {
    ...
    case 'too-large':
      return translate(
        ...
        'Artifacts shared from Manta must be smaller than 800 KB.'
      )
    case 'unreadable':
      return translate(
        'auto.components.artifacts.artifact-publish-flow.e2ed5acd8c',
        "Manta couldn't read this file. Open it from a workspace and try again."
      )
    case 'unsupported':
      return translate(
