...
const COPIED_NOTE =
  `Command copied. Quit Manta, run it in a system terminal to install ${PACKAGE_FILE_NAME}, ` +
  'then reopen Manta.'
const INSTRUCTIONS = {
  ok: true as const,
...
describe('LinuxPackageInstallRecoveryCard copy', () => {
  it('leads with the manual-install copy and recovery actions', () => {
    ...
    expect(screen.getByText(/Copy the command, quit Manta/)).toBeTruthy()
...
describe('LinuxPackageInstallRecoveryCard without a usable command', () => {
  it('promotes Show Package and keeps the official-release fallback', async () => {
    ...
    expect(screen.getByText(/Quit Manta before finishing the update/)).toBeTruthy()
