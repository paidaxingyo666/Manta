...
import {
  MANTA_EDITOR_REQUEST_FILE_CLOSE_EVENT,
  MANTA_EDITOR_SAVE_AND_CLOSE_EVENT,
  MANTA_EDITOR_REQUEST_CMD_SAVE_EVENT,
...
function Terminal(): React.JSX.Element | null {
  ...
  const terminalShortcutPolicy = useAppStore(
    (s) => s.settings?.terminalShortcutPolicy ?? 'manta-first'
  )
  ...
  // Why: TabBar portals into the titlebar (target created by App.tsx) so tabs share the "Manta" title row.
  ...
  const handleSaveDialogSave = useCallback(async () => {
    ...
    setSaveDialogFileId(null)
    window.dispatchEvent(new CustomEvent(MANTA_EDITOR_SAVE_AND_CLOSE_EVENT, { detail: { fileId } }))
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent): void => {
      ...
      const notifyTerminalCapture = (actionId: KeybindingActionId): void => {
        if (context !== 'terminal' || terminalShortcutPolicy !== 'manta-first') {
          return
        }
      ...
      if (!e.repeat && matchShortcut('editor.save')) {
        ...
        if (!inEditor) {
          ...
          if (requestedFileId) {
            ...
            window.dispatchEvent(
              new CustomEvent<EditorRequestCmdSaveDetail>(MANTA_EDITOR_REQUEST_CMD_SAVE_EVENT, {
