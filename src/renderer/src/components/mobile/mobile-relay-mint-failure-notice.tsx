...
export function MobileRelayMintFailureNotice({
  ...
  const reconnectRequired = useAppStore(
    (state) => state.mantaProfileAuthStatus?.state === 'reconnect-required'
  )
    ...
    : providerMissing
      ...
      : reconnectRequired
        ? translate(
            ...
            'Your Manta account session expired.'
          )
        : translate(
    ...
    : providerMissing
      ...
      : reconnectRequired
        ? translate(
            ...
            'Sign in again to use Manta Relay, or use LAN to pair over Tailscale or the same Wi‑Fi.'
          )
        : translate(
