...
import { MANTA_CLI_SKILL_NAME } from '@/lib/agent-feature-install-commands'
import {
...
function getCliActionLabel(status: CliInstallStatus | null, busy: boolean): string {
  ...
  if (isMantaCliAvailableOnPath(status)) {
    return translate(
...
export function useMobileEmulatorAgentSetupState(enabled = true): {
  ...
  } = useInstalledAgentSkill(MANTA_CLI_SKILL_NAME, {
    enabled,
  ...
  const cliEnabled = isMantaCliAvailableOnPath(cliInstallStatus)
  ...
  const recheckSetup = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const cliReady = isMantaCliAvailableOnPath(cliStatus)
      if (!mountedRef.current) {
        return
      }
      ...
      if (skillInstalled) {
        toast.message(
          translate(
            ...
            'Skill is installed. Enable the Manta CLI to finish setup.'
          )
        )
        return
      }
      if (cliReady) {
        toast.message(
          translate(
            ...
            'Manta CLI is ready. Install the skill to finish setup.'
          )
        )
        return
      }
  ...
  const handleEnableCli = useCallback(async (): Promise<void> => {
    setCliBusy(true)
    try {
      const next = await ensureMantaCliAvailableForAgentSkillTerminal({
        onStatusChange: setCliInstallStatus
      })
      if (mountedRef.current && isMantaCliAvailableOnPath(next)) {
        toast.success(
          translate(
            'auto.components.emulator.pane.use.mobile.emulator.agent.setup.state.2b519eed94',
            'Registered the Manta CLI in PATH.'
          )
        )
      }
    } finally {
