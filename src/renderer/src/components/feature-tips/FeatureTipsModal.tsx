  ...
  const handlePrimaryAction = async (): Promise<void> => {
    ...
    switch (currentTip.action) {
      case 'learn-cmd-j-palette': {
        ...
        trackCmdJPaletteFeatureTipAcknowledged(
          getMantaCliFeatureTipTelemetrySource(modalData.source)
        )
        closeModal()
        break
      }
      ...
      case 'setup-cli': {
        ...
        const canApplySetupResult = (): boolean =>
          ...
          setupRequestIdRef.current === setupRequestId
        const telemetrySource = getMantaCliFeatureTipTelemetrySource(modalData.source)
        trackMantaCliFeatureTipSetupClicked(telemetrySource)
        setPrimaryBusy(true)
        try {
          const result = await installCliFromFeatureTip(() => window.api.cli.install())
          if (result.kind === 'installed') {
            trackMantaCliFeatureTipSetupResult(telemetrySource, 'installed')
          ...
          trackMantaCliFeatureTipSetupResult(telemetrySource, 'needs_attention')
          if (!canApplySetupResult()) {
            return
          }
          toast.warning(
          ...
          const message = error instanceof Error ? error.message : 'Failed to install Manta CLI.'
          if (
            import.meta.env.DEV &&
            message.includes('Development mode uses a generated launcher for validation only')
          ) {
            trackMantaCliFeatureTipSetupResult(telemetrySource, 'dev_preview')
            if (!canApplySetupResult()) {
              return
            }
            enableOrchestrationSkillSetup()
          ...
          trackMantaCliFeatureTipSetupResult(telemetrySource, 'failed')
          if (canApplySetupResult()) {
            toast.error(message)
          }
        } finally {
