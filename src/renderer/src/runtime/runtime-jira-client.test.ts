...
describe('runtime Jira client search bounds', () => {
  ...
  it('routes isolated status and summary reads through local and paired-runtime owners', async () => {
    ...
    jiraLookupIssueSummaryLocal.mockResolvedValue({ key: 'MANTA-1' })
    runtimeCall.mockImplementation(async (args: { method: string }) => {
      ...
      return {
        ...
        ok: true,
        result:
          args.method === 'jira.readStatus' ? { connected: true, viewer: null } : { key: 'MANTA-1' },
        _meta: { runtimeId: 'remote-runtime' }
      }
    })

    await expect(jiraReadStatus(localContext)).resolves.toMatchObject({ connected: true })
    await expect(jiraLookupIssueSummary(localContext, 'MANTA-1', 'site-1')).resolves.toMatchObject({
      key: 'MANTA-1'
    })
    await expect(jiraReadStatus(runtimeContext)).resolves.toMatchObject({ connected: true })
    await expect(jiraLookupIssueSummary(runtimeContext, 'MANTA-1', 'site-1')).resolves.toMatchObject(
      { key: 'MANTA-1' }
    )
    ...
    expect(runtimeCall).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        params: { key: 'MANTA-1', siteId: 'site-1' },
        selector: 'env-1'
      })
    )
  })
  ...
  it('cancels a superseded local Jira summary read', async () => {
    ...
    const lookup = jiraLookupIssueSummary(context, 'MANTA-1', 'site-1', controller.signal)
    controller.abort()
  ...
  it('keeps ordinary remote Jira creation compatible with hosts without the user-field capability', async () => {
    runtimeCall.mockImplementation(async (args: { method: string }) => {
      ...
      return {
        id: 'rpc-1',
        ok: true,
        result: { ok: true, id: 'issue-1', key: 'MANTA-1', url: 'https://jira.example/MANTA-1' },
        _meta: { runtimeId: 'remote-runtime' }
      }
    })
    ...
    ).resolves.toMatchObject({ ok: true, key: 'MANTA-1' })
    expect(runtimeCall).toHaveBeenCalledWith(
  ...
  it('streams image-bearing issue and comment payloads from remote runtimes', async () => {
      ...
      async (args: RuntimeSubscribeArgs, callbacks: RuntimeSubscribeCallbacks) => {
        const payload =
          args.method === 'jira.getIssueStream'
            ? { key: 'MANTA-1', description: '![shot](data:image/png;base64,abc)' }
    ...
    await expect(
      jiraGetIssue({ activeRuntimeEnvironmentId: 'env-1' }, 'MANTA-1', 'site-1')
    ).resolves.toMatchObject({ key: 'MANTA-1' })
    await expect(
      jiraIssueComments({ activeRuntimeEnvironmentId: 'env-1' }, 'MANTA-1', 'site-1')
