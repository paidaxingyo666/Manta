...
describe('runtime file client', () => {
  it('streams staged directory entries through main instead of sending base64 itself', async () => {
    ...
    expect(uploads[0]?.relativePath).toMatch(/^uploads\/assets\/\.logo\.png\.manta-upload-/)
    ...
    expect(uploads[1]?.relativePath).toMatch(/^uploads\/assets\/\.large\.bin\.manta-upload-/)
  ...
  it('forwards a single staged file with the identity staging measured', async () => {
    ...
    expect(upload?.relativePath).toMatch(/^uploads\/\.large\.bin\.manta-upload-/)
  ...
  it('removes a created runtime directory import root when a nested file upload fails', async () => {
    ...
    expect(uploadRequests()[0]?.relativePath).toMatch(/^uploads\/assets\/\.logo\.png\.manta-upload-/)
    expect(runtimeEnvironmentCall).toHaveBeenLastCalledWith({
      selector: 'env-1',
      method: 'files.delete',
      params: {
