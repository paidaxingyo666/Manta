 ...
 *
 * The incident chain was: Cmd+Q leaves a packaged `manta serve` owning the
...
/** Argv macOS delivers for `open -n -a Manta` / Finder / Dock relaunch. */
const DESKTOP_RELAUNCH_ARGV = ['/Applications/Manta.app/Contents/MacOS/Manta'] as const
...
describe('#8457 headless serve promotion preserves surviving agent sessions', () => {
  ...
  it('ignores a duplicate `manta serve` launch instead of promoting the headless owner', () => {
    const serve = bootHeadlessServeOwner()
