...
describe('openHttpLink', () => {
  it('routes into Manta when openLinksInApp is on and a worktree is known', () => {
  ...
  it('routes floating workspace links into Manta without changing the active repo worktree', () => {
    storeState.settings = { openLinksInApp: true }
  ...
  it('forceInApp opens a local link in Manta when the setting is off', () => {
    storeState.settings = { openLinksInApp: false }
  ...
  it('does not force a remote link into the Manta browser', () => {
    storeState.settings = { openLinksInApp: false }
  ...
  it('labels explicit local links from the local scan instead of a merged remote port', async () => {
    ...
    registerLocalhostLabelMock.mockResolvedValue({ url: 'http://wt-local.manta.localhost:60016/' })
  ...
  it('labels localhost links from terminal output before opening the system browser', async () => {
    ...
    registerLocalhostLabelMock.mockResolvedValue({
      url: 'http://analytics.manta.localhost:60016/episodes'
    })
    ...
    expect(openUrlMock).toHaveBeenCalledWith('http://analytics.manta.localhost:60016/episodes')
  })
  ...
  it('resolves display URLs for labeled localhost links without opening them', async () => {
    ...
    registerLocalhostLabelMock.mockResolvedValue({
      url: 'http://snapstudio-main.manta.localhost:60016/'
    })

    await expect(resolveLocalhostHttpLinkDisplayUrl('http://localhost:5180/')).resolves.toBe(
      'http://snapstudio-main.manta.localhost:60016/'
    )
    expect(openUrlMock).not.toHaveBeenCalled()
    expect(createBrowserTabMock).not.toHaveBeenCalled()
  })
  ...
  it('labels a local-owned localhost link from the local scan', async () => {
    ...
    ).resolves.toBe('http://snapstudio-main.manta.localhost:60016/')
  })
})

describe('openHttpLink modifier routing', () => {
  it('forces the system browser when inverting is off and links open in Manta', () => {
  ...
  it('opens in Manta when inverting is on and links open externally', () => {
  ...
  it('opens in the system browser when inverting is on and links open in Manta', () => {
  ...
  // Why: remote-owned links must never land in a Manta tab that cannot reach them.
  it('never routes a remote source into Manta even when inverting', () => {
    storeState.settings = { openLinksInApp: false, openLinksInAppModifierInverts: true }
