...
export type HttpLinkDestination = 'manta' | 'system'
...
export function canSourceOwnerOpenInManta(
 ...
 *  cannot reach Manta's managed browser offers only the system browser. */
export function httpLinkActionDestinationsFor(
  settings: { openLinksInApp?: boolean } | null | undefined,
  sourceOwner: HttpLinkSourceOwner,
  canOpenOwnedBrowser: boolean
): HttpLinkActionDestinations {
  if (!canSourceOwnerOpenInManta(sourceOwner, canOpenOwnedBrowser)) {
...
export function httpLinkDestinationLabel(destination: HttpLinkDestination): string {
  return destination === 'manta'
    ? translate(
        'auto.components.terminal.pane.TerminalLinkActionPopover.mantaBrowser',
        'Manta Browser'
      )
    : translate(
...
export function openRoutedHttpLink(url: string, deps: RoutedHttpLinkOptions): void {
  ...
  if (deps.forceDestination) {
    openHttpLink(url, {
      ...
      forceInApp: deps.forceDestination === 'manta',
  ...
  void Promise.resolve(preferenceDecision)
    .then((openInManta) => {
      openHttpLink(url, {
        allowRemoteInApp: true,
        worktreeId: deps.worktreeId,
        forceSystemBrowser: !openInManta,
        sourceOwner
      })
    })
    .catch(() => {
