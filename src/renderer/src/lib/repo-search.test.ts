...
describe('repo-search', () => {
  ...
  it('matches display names case-insensitively', () => {
    ...
    expect(searchRepos(repos, 'MANTA').map((repo) => repo.id)).toEqual(['1'])
  })

  it('falls back to matching repo paths', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'frontend', path: '/src/team-a/manta' }),
  ...
  it('keeps display-name matches ahead of path-only matches', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'misc', path: '/src/manta-tools/misc' }),
      makeRepo({ id: '2', displayName: 'manta', path: '/src/team-a/project' })
    ]

    expect(searchRepos(repos, 'manta').map((repo) => repo.id)).toEqual(['2', '1'])
  })
