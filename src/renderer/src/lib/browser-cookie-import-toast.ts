...
function formatCookieImportWarning(warning: CookieImportWarning): string {
  ...
  if (!isHandledWireDiscriminant(code, HANDLED_WARNING_CODES)) {
    return translate(
      ...
      'The cookie import finished with a warning this version of Manta does not recognize. Update Manta to see the details, then check this profile before relying on its cookies.'
    )
  }
  switch (warning.code) {
    ...
    case 'cookies-undecryptable': {
      ...
      if (!isHandledWireDiscriminant(reason, HANDLED_UNDECRYPTABLE_REASONS)) {
        return translate(
          ...
          '{{value0}} cookies could not be decrypted and were skipped for a reason this version of Manta does not recognize. Update Manta to see the details, then try the import again.',
          { value0: warning.failedCookies }
        )
      }
...
function emitGoogleCookieImportWarning(
  ...
  // remote environment, any Manta browser locally. Client-hosted pages render on this desktop, so
  ...
  const message = !execution.executionRemoteEnvironment
    ? translate(
        ...
        'Google cookies were not imported. Open a browser in Manta with this profile, then sign into Google.'
      )
...
function emitPartitionSkippedImportWarning(summary: BrowserCookieImportSummary): void {
  ...
  toast.warning(
    translate(
      ...
      '{{value0}} cookies were not imported because their site-partition could not be read. Sign in to those sites again in Manta.',
