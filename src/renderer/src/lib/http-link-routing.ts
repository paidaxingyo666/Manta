...
export type OpenHttpLinkOptions = {
  ...
  /** Unconditional for local sources: open inside Manta regardless of settings. */
  forceInApp?: boolean
...
): { wantsManta: boolean; wantsSystemBrowser: boolean } {
  if (!modifierHeld) {
    return { wantsManta: false, wantsSystemBrowser: false }
  }
  if (!modifierInverts) {
    return { wantsManta: false, wantsSystemBrowser: true }
  }
  return { wantsManta: !openLinksInApp, wantsSystemBrowser: openLinksInApp }
}
...
export function openHttpLink(url: string, opts: OpenHttpLinkOptions = {}): void {
  ...
  const wantsManta =
    ...
    Boolean(worktreeId) &&
    (forceInApp || openLinksInApp || modifier.wantsManta)

  if (wantsManta && allowRuntimeInApp && worktreeId && sourceOwner?.kind === 'runtime') {
  ...
  if (wantsManta && sourceIsLocal && worktreeId && state) {
