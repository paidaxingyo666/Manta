...
type DiagnosticWindow = Window & { __mantaTypingDiagnostic?: TypingDiagnosticBridge }

describe('typing latency diagnostic lifecycle', () => {
  afterEach(() => {
    delete (window as DiagnosticWindow).__mantaTypingDiagnostic
    vi.restoreAllMocks()
  })
  ...
  it('releases stopped pane state while preserving the instrumented count', () => {
    ...
    const bridge = (window as DiagnosticWindow).__mantaTypingDiagnostic
    if (!bridge) {
