 ...
 *
 *   window.__mantaTypingDiagnostic.start()   // then type normally for ~20s
 *   window.__mantaTypingDiagnostic.report()  // logs + returns a JSON-safe object
 *   window.__mantaTypingDiagnostic.stop()
 *
...
function startProbe(): string {
  if (active) {
    return 'Typing diagnostic already running. Type for ~20s, then run __mantaTypingDiagnostic.report().'
  }
  cacheAppVersion()
  ...
  active = state
  lastState = state
  return `Typing diagnostic started on ${state.panes.length} pane(s). Click into the agent terminal, type normally for ~20 seconds, then run __mantaTypingDiagnostic.report().`
}
...
function stopProbe(): string {
  ...
  state.panes = []
  return 'Typing diagnostic stopped. Run __mantaTypingDiagnostic.report() to read the last samples.'
}
...
function reportProbe(): TypingLatencyReport {
  ...
  console.log('[manta] typing latency diagnostic', report)
  return report
}
...
type TypingDiagnosticWindow = Window & { __mantaTypingDiagnostic?: TypingDiagnosticBridge }
...
export function installTypingLatencyDiagnostic(): void {
  ...
  if (target.__mantaTypingDiagnostic) {
    return
  }
  target.__mantaTypingDiagnostic = { start: startProbe, stop: stopProbe, report: reportProbe }
}
