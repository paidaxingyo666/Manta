...
describe('buildAgentDraftLaunchPlan', () => {
  ...
  it('uses MANTA_PI_PREFILL env var for pi (no CLI flag exists)', () => {
    // Why: pi has no `--prefill` flag, and bracketed-paste-after-ready races
    // against pi's lengthy startup output. The Manta overlay installs an
    // `manta-prefill` extension that reads MANTA_PI_PREFILL on session_start
