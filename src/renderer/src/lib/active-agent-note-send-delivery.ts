...
const MANTA_DESKTOP_TERMINAL_CLIENT = { id: 'manta-desktop', type: 'desktop' as const }
...
export async function sendPromptWithLegacyCombinedSend(
  ...
  try {
    const { send } = await callRuntimeRpc<{ send: RuntimeTerminalSend }>(
      runtimeTarget,
      'terminal.send',
      { terminal: terminalHandle, text: prompt, enter: true, client: MANTA_DESKTOP_TERMINAL_CLIENT },
  ...
  try {
    const { send } = await callRuntimeRpc<{ send: RuntimeTerminalSend }>(
      ...
      'terminal.send',
      {
        ...
        client: MANTA_DESKTOP_TERMINAL_CLIENT
      },
      { timeoutMs: ACTIVE_AGENT_SEND_RPC_TIMEOUT_MS }
    )
    if (!send.accepted) {
