...
export function getFolderWorkspacePathStatusDescription(
  ...
  if (!isHandledPathStatusReason(status.reason)) {
    return translate(
      ...
      'Manta cannot use {{path}}, and this version does not recognize the reason. Update Manta to see the details.',
      { path: status.path }
    )
  }
  switch (status.reason) {
    case 'missing':
      return translate(
        ...
        'Manta cannot find {{path}}. Remove and re-import this folder workspace.',
        { path: status.path }
      )
    case 'not-directory':
      return translate(
    ...
    case 'ambiguous-connection':
      return translate(
        ...
        'Manta cannot tell which SSH connection owns this folder scope.'
      )
    case undefined:
    case 'unavailable':
      return translate(
        'auto.lib.folderWorkspacePathStatus.description.unavailable',
        'Manta cannot verify this folder right now. Check the runtime or SSH connection and try again.'
      )
  }
}
  ...
  if (message.startsWith('folder_workspace_path_missing:')) {
    return {
      ...
      description: translate(
        ...
        'Manta cannot find {{path}}. Remove and re-import the folder.',
        { path }
      )
    }
  }
