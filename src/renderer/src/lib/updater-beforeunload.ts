...
export function registerUpdaterBeforeUnloadBypass(): () => void {
  ...
  window.addEventListener(MANTA_RENDERER_SHUTDOWN_CHECKPOINT_ABORTED_EVENT, clearInProgress)
  ...
  return () => {
    ...
    window.removeEventListener(MANTA_RENDERER_SHUTDOWN_CHECKPOINT_ABORTED_EVENT, clearInProgress)
