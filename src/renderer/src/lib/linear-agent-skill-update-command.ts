...
export type LinearAgentSkillUpdateTarget = {
  skillName: typeof MANTA_LINEAR_SKILL_NAME | typeof LINEAR_TICKETS_SKILL_NAME
  command: string
}
...
export function getLinearAgentSkillUpdateTarget(
  ...
  const canonicalSkillInstalled = hasInstalledAgentSkill(skills, MANTA_LINEAR_SKILL_NAME, {
  ...
  return !installed || canonicalSkillInstalled || !legacySkillInstalled
    ? { skillName: MANTA_LINEAR_SKILL_NAME, command: MANTA_LINEAR_SKILL_UPDATE_COMMAND }
