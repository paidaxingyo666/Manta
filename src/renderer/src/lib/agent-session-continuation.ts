...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `Manta pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior Manta session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest Manta status hints:', ...statusHints] : []),
    '',
