...
import { MANTA_APP_RESTART_ABORTED_EVENT } from '../../../shared/updater-renderer-events'
import {
  MANTA_EDITOR_PREPARE_HOT_EXIT_EVENT,
  type EditorPrepareHotExitDetail
} from '../../../shared/editor-save-events'

const RELOAD_GUARD_KEY = 'manta:lazy-chunk-reload-attempted'
...
describe('loadLazyWithRetry recovery reload vs the dirty-editor-tab unload veto', () => {
  ...
  function installDirtyEditorTab(options: { hotExitBackupFails?: boolean } = {}): Harness {
    ...
    window.addEventListener(MANTA_EDITOR_PREPARE_HOT_EXIT_EVENT, hotExitBackup)
    vi.spyOn(window.location, 'reload').mockImplementation(() => {
      ...
      if (!accepted) {
        window.dispatchEvent(new Event(MANTA_RENDERER_UNLOAD_PREVENTED_EVENT))
      }
    })

    cleanupHarness = () => {
      window.removeEventListener('beforeunload', dirtyTabGuard)
      window.removeEventListener(MANTA_EDITOR_PREPARE_HOT_EXIT_EVENT, hotExitBackup)
      cleanupBypass()
    }
    return harness
  }
  ...
  it('refuses to reload when unsaved buffers cannot be backed up', async () => {
    ...
    window.addEventListener(MANTA_APP_RESTART_ABORTED_EVENT, restartAborted)
    const error = chunkParseError()
    ...
    window.removeEventListener(MANTA_APP_RESTART_ABORTED_EVENT, restartAborted)
  })
