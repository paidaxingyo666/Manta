...
export function shouldOpenWorkspacePortInMantaBrowser(
...
export function resolvePortOpenInMantaBrowser({
  settings,
  event,
  isMac
}: {
  ...
  return shouldOpenWorkspacePortInMantaBrowser(settings)
}
...
export async function openWorkspacePortInBrowser(args: {
  ...
  if (args.openInMantaBrowser === false && args.runtimeTarget.kind === 'local') {
    try {
