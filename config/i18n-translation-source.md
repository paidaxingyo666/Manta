...
Reduce the recurring engineering and human work required to localize Manta
...
Use gettext PO as Manta's canonical bilingual translation source, under a
constrained profile:

- `msgctxt` — the stable Manta message ID;
...
dependency. Manta must not require network access or a translation service to
...
Manta will support only the subset needed for its message model. PR B must
define and validate that subset rather than accepting arbitrary PO documents.

Each supported entry must carry:

- one stable Manta message ID (`msgctxt`), unique within the file;
...
Plurals use Manta's existing per-key convention: each i18next plural-suffixed
...
- XML is the format most likely to let a hand-edited community PR fail on a
  ...
  external editors produce whole-file diffs; Manta's translation contributions
  arrive overwhelmingly as direct catalog PRs.

If a translation provider requires XLIFF, Manta should add a deterministic
...
translator context, or review state. Meeting Manta's requirements would require
...
own platform but are not a suitable authority for Manta's macOS, Linux,
...
not become Manta's sole source of truth or a runtime dependency. Selecting one
...
to establish a format decision. Manta therefore chooses based on its own
