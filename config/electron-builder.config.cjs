...
// swap them over an installed Manta and macOS treats each build as a new app.
const isMacHourly = process.env.MANTA_MAC_HOURLY === '1'
const isMacDaily = process.env.MANTA_MAC_DAILY === '1'
const isMacAdhoc = process.env.MANTA_MAC_ADHOC === '1'
...
const isWinHourly = process.env.MANTA_WIN_HOURLY === '1'
...
const devChannelBuildVersion = isHourlyChannel
  ? process.env.MANTA_HOURLY_BUILD_VERSION
  : isDailyChannel
    ? process.env.MANTA_DAILY_BUILD_VERSION
    : isAdhocChannel
...
// Electron's loader requirements alongside Manta's headless-host dependencies.
...
// config/nsis/manta-installer-hooks.nsh, which registers the same set on Windows.
...
// @manta/windows-registry is a workspace link present on every host, so its presence proves nothing.
...
module.exports = {
  appId,
  productName: 'Manta',
  protocols: [{ name: 'Manta', schemes: ['manta'] }],
  ...
  files: [
    ...
    '!examples{,/**/*}',
    // Why: pr-evidence/ is a local e2e screenshot output (MANTA_CAPTURE_EVIDENCE);
    ...
    // Why: the Windows CLI shim ships via extraResources to resources/bin/manta.cmd
    // (beside the native resources/bin/manta.exe). Packing the source tree into
    // app.asar too lets asarUnpack:['resources/**'] extract a second copy at
    // app.asar.unpacked/resources/win32/bin/manta.cmd with no adjacent manta.exe,
  ...
  afterPack: async (context) => {
    ...
    if (context.electronPlatformName === 'darwin') {
      ...
      let commit = process.env.MANTA_BUILD_COMMIT || process.env.GITHUB_SHA || 'unknown'
      if (commit === 'unknown') {
        try {
    ...
    if (context.electronPlatformName === 'darwin') {
      await signMacComputerUseHelper(join(resourcesDir, 'Manta Computer Use.app'), context.packager)
      await signMacStandaloneHelper(
        join(resourcesDir, '..', 'MacOS', 'manta-notification-status'),
        'manta-notification-status',
        context.packager
      )
      await signMacStandaloneHelper(
        join(resourcesDir, '..', 'MacOS', 'manta-keyboard-layout'),
        'manta-keyboard-layout',
        context.packager
      )
    }
  },
  win: {
    executableName: 'Manta',
    ...
    //
    // isWinUnsigned rather than upstream's isWinDevChannel: it already includes
    // that channel and adds the fork's own case, which has no certificate at
    // all. Both must drop the name for the same reason.
    signtoolOptions: {
    ...
    extraResources: [
      ...
      {
        from: 'resources/win32/bin/manta.cmd',
        to: 'bin/manta.cmd'
      },
      {
        from: 'native/windows-cli-launcher/.build/manta.exe',
        to: 'bin/manta.exe'
      },
      {
  ...
  nsis: {
    artifactName: 'manta-windows-setup.${ext}',
    ...
    include: resolve(__dirname, 'nsis', 'manta-installer-hooks.nsh')
  },
  mac: {
    // Why rank Alternate: Manta joins Finder's "Open With" list for Markdown without claiming
    ...
    extendInfo: {
      NSAppleEventsUsageDescription:
        'Manta allows terminal-launched developer tools to automate local apps when you request it.',
      NSBluetoothAlwaysUsageDescription:
        'Manta allows terminal-launched developer tools to access Bluetooth devices when you request it.',
      NSBluetoothPeripheralUsageDescription:
        'Manta allows terminal-launched developer tools to access Bluetooth devices when you request it.',
      NSCameraUsageDescription: "Application requests access to the device's camera.",
      NSLocationUsageDescription:
        'Manta allows terminal-launched developer tools to access location when you request it.',
      NSLocalNetworkUsageDescription:
        'Manta allows terminal-launched developer tools to discover and connect to local development servers when you request it.',
      NSMicrophoneUsageDescription: "Application requests access to the device's microphone.",
      NSAudioCaptureUsageDescription:
        'Manta allows terminal-launched developer tools to capture desktop audio when you request it.',
    ...
    extraResources: [
      ...
      {
        from: 'resources/darwin/bin/manta',
        to: 'bin/manta'
      },
      {
      ...
      {
        from: 'native/computer-use-macos/.build/release/Manta Computer Use.app',
        to: 'Manta Computer Use.app'
      },
      featureWallResources
    ],
    ...
    extraFiles: [
      {
        from: 'native/notification-status-macos/.build/release/manta-notification-status',
        to: 'MacOS/manta-notification-status'
      },
      {
        from: 'native/keyboard-layout-macos/.build/release/manta-keyboard-layout',
  ...
  linux: {
    ...
    // text/markdown, so reusing that type puts Manta in the Open With list without shipping a glob
    ...
    desktop: {
      entry: {
        // Why: Electron reports WM_CLASS=manta for the visible Linux window;
        // GNOME docks need an exact match to group it with manta-ide.desktop.
        StartupWMClass: 'manta'
      }
    },
    extraResources: [
      ...commonExtraResources,
      ...
      {
        from: 'resources/linux/bin/manta-ide',
        to: 'bin/manta-ide'
      },
      {
  ...
  appImage: {
    artifactName: isLinuxArm64Release ? 'manta-linux-arm64.${ext}' : 'manta-linux.${ext}'
  },
  deb: {
    packageName: 'manta-ide',
    artifactName: 'manta-ide_${version}_${arch}.${ext}',
    // Why: xvfb lets the bundled `manta serve` CLI run browser panes on a headless
    ...
    // Why: symlink the bundled CLI onto PATH at install time so `manta-ide serve`
  ...
  rpm: {
    packageName: 'manta-ide',
    artifactName: 'manta-ide-${version}.${arch}.${ext}',
...
function chmodUnixCliLaunchers(resourcesDir, electronPlatformName) {
  ...
  for (const launcherName of ['manta', 'manta-ide']) {
...
async function signMacComputerUseHelper(helperAppPath, packager) {
  if (!existsSync(helperAppPath)) {
    if (isMacRelease) {
      throw new Error(`Missing Manta Computer Use helper app at ${helperAppPath}`)
    }
    return
  }
  const codeSigningInfo =
    isMacRelease && process.env.CSC_LINK && packager?.codeSigningInfo?.value
      ...
      : null
  const identity =
    process.env.MANTA_COMPUTER_MACOS_SIGN_IDENTITY ??
    process.env.CSC_NAME ??
    findInstalledMacSigningIdentity(codeSigningInfo?.keychainFile) ??
    (isMacRelease ? null : '-')
  if (!identity) {
    throw new Error('Missing signing identity for Manta Computer Use helper app')
  }
  // Why: TCC grants attach to this nested app's code identity. Sign it before
  // the outer Manta.app is sealed so production builds preserve that identity.
