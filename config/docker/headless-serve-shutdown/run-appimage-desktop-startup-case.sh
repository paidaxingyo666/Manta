...
appimage=${1:-/input/manta.AppImage}
...
if ((EUID == 0)); then
  ...
  if ! chown manta:manta "$state_dir"; then
    echo "FAIL: unable to hand the AppImage startup state directory to manta: $state_dir" >&2
    rm -rf -- "$state_dir" || true
    exit 1
  fi
  exec runuser --user manta --preserve-environment -- env \
...
export MANTA_STARTUP_DIAGNOSTICS=1
ulimit -c 0
