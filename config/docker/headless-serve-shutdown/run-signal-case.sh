...
app_root=${MANTA_TEST_APP_ROOT:-/artifacts/root}
signal_target_kind=${MANTA_SIGNAL_TARGET:-app}
entrypoint_kind=${MANTA_TEST_ENTRYPOINT:-app}
int_delivery=${MANTA_INT_DELIVERY:-foreground-process-group}
...
startup_timeout_seconds=${MANTA_STARTUP_TIMEOUT_SECONDS:-180}
...
state_dir=$(mktemp -d "/tmp/manta-shutdown-${signal_name}.XXXXXX")
...
case "$entrypoint_kind" in
  ...
  appimage) entrypoint=(/input/manta.AppImage --appimage-extract-and-run --no-sandbox) ;;
  launcher)
    entrypoint=("$app_root/resources/bin/manta-ide")
    ;;
...
read_ready_line() {
  sed -u -n 's/^[^{]*//p' "$stdout_log" \
    | jq --unbuffered -Rnc 'first(inputs | fromjson? | select(.type == "manta_server_ready" and .schemaVersion == 1))'
}
...
if [[ -z "$ready_line" ]]; then
  ...
  echo "FAIL: entrypoint exited or timed out before manta_server_ready" >&2
  exit 1
fi

registered_cli_verified=false
if [[ "$entrypoint_kind" == appimage ]]; then
  registered_cli="$HOME/.local/bin/manta-ide"
  expected_target="$XDG_CACHE_HOME/manta/appimage/launcher/manta-ide"
  ...
  if ! registered_help=$("$registered_cli" --help 2>&1) \
    || [[ "$registered_help" != *'Usage: manta <command>'* ]]; then
...
for shutdown_poll in {0..50}; do
  ...
  owned_residue=$(ps -eo pid=,ppid=,stat=,args= | awk -v state="$state_dir" \
    '($0 ~ state || $0 ~ /\/artifacts\/root\/manta-ide/ || $0 ~ /[X]vfb :99 /) && $0 !~ /awk -v state=/ {print}' || true)
