...
startup_timeout_seconds=${MANTA_STARTUP_TIMEOUT_SECONDS:-90}
...
ready_line=$(timeout "$startup_timeout_seconds" bash -c '
  tail --pid="$1" -n +1 -F "$2" 2>/dev/null \
    | jq --unbuffered -nc '\''first(inputs | select(.type == "manta_server_ready" and .schemaVersion == 1))'\''
' bash "$app_pid" "$stdout_log" || true)
if [[ -z "$ready_line" ]]; then
  cat "$stdout_log" "$stderr_log" >&2
  echo "FAIL: AppRun exited or timed out before manta_server_ready" >&2
  exit 1
fi
...
if [[ "$signal_target_kind" == serving-electron ]]; then
  signal_target_pid=$(awk '/\/manta-ide .* --serve / {print $1; exit}' <<<"$tree_snapshot")
