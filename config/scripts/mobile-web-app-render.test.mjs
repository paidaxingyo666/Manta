...
async function readShellCsp() {
  const source = await readFile(
    join(
      projectDir,
      'mobile/modules/manta-mobile-web-shell/android/src/main/java/expo/modules/mantamobilewebshell/MobileWebShellCsp.kt'
    ),
    'utf8'
  )
...
beforeAll(async () => {
  ...
  // the same reason and the same override shape as the mantad browser-provider job.
...
describe('the shell policy this page is tested under', () => {
  it('is the same on both platforms, so one render check covers both', async () => {
    const swift = await readFile(
      join(projectDir, 'mobile/modules/manta-mobile-web-shell/ios/MobileWebShellCsp.swift'),
      'utf8'
    )
...
describeRender('the Route A page in a real browser', () => {
  ...
  it('says to update the app when the shell that opened it named no screen', async () => {
    ...
    expect(errors).toEqual([])
    expect(text).toContain('Update Manta to open this workspace')
