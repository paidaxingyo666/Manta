...
// @manta/windows-registry is a workspace link present on every host, so it proves nothing.
...
describe('packaged runtime resources', () => {
  ...
  it('ignores member calls onto Manta methods that are themselves named require', async () => {
    ...
    async () => {
      ...
      try {
        ...
        await writeFile(
          ...
          `${JSON.stringify({ name: 'manta-compiled-output', type: 'commonjs', private: true })}\n`,
          'utf8'
        )
