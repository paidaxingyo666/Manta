...
/** A real loadable addon, so the OS holds the same lock a running Manta holds. */
...
/**
 * Stage a stale addon and keep it loaded, exactly as a running Manta does.
 *
...
// a real EPERM, and the classifier that turns that into "close running Manta"
    ...
    async () => {
      ...
      try {
        ...
        expect(result.stderr).toContain(
          'Close running Manta/Electron/dev processes for this worktree'
        )
