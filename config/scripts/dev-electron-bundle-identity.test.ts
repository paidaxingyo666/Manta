...
const BRANCH_ENV_KEYS = [
  'MANTA_DEV_DOCK_TITLE',
  'MANTA_DEV_BRANCH',
  'MANTA_DEV_INSTANCE_LABEL',
  'MANTA_DEV_WORKTREE_NAME'
] as const
...
function patchesUnder(dockTitle: string, branch: string) {
  ...
  Object.assign(process.env, {
    MANTA_DEV_DOCK_TITLE: dockTitle,
    MANTA_DEV_BRANCH: branch,
    MANTA_DEV_INSTANCE_LABEL: branch,
    MANTA_DEV_WORKTREE_NAME: branch
  })
  try {
...
describe('dev-electron-bundle-identity', () => {
  it('leaves process.env untouched, including its object identity', () => {
    ...
    patchesUnder('Manta: some-branch', 'some-branch')
  ...
  it('keeps the bundle display name in step with the name safeStorage keys off', () => {
    // Two independently hardcoded 'Manta Dev' strings: this one names the bundle (notifications,
  ...
  it('produces byte-identical patches on two different branches', () => {
    ...
    expect(patchesUnder('Manta: fix-login-crash', 'fix-login-crash')).toEqual(
      patchesUnder('Manta: perf-2', 'perf-2')
    )
    expect(patchesUnder('Manta: dev', 'main')).toEqual(
      patchesUnder('Manta: some-worktree @ feature/x', 'feature/x')
    )
  })

  it('leaks no branch, worktree, or title text into any patched value', () => {
    const branch = 'fix-login-crash'
    const worktree = 'Manta-safe-storage-lock'
    for (const patch of patchesUnder(`Manta: ${branch}`, branch)) {
