...
describe('isNsisUninstallerArtifact', () => {
  ...
  it('matches only electron-builder intermediate uninstallers', () => {
    expect(isNsisUninstallerArtifact('C:\\dist\\manta-windows-setup.__uninstaller.exe')).toBe(true)
    expect(isNsisUninstallerArtifact('/dist/manta-windows-setup.__uninstaller.exe')).toBe(true)
    expect(isNsisUninstallerArtifact('C:\\dist\\win-unpacked\\Manta.exe')).toBe(false)
    expect(isNsisUninstallerArtifact('C:\\dist\\manta-windows-setup.exe')).toBe(false)
...
describe('relayNsisUninstaller', () => {
  const writeUninstaller = (dir, contents) => {
    const filePath = join(dir, 'manta-windows-setup.__uninstaller.exe')
...
describe('signWindowsUninstallerViaSignPath', () => {
  ...
  it.each([
    ...
    ['a non-uninstaller path', { path: 'C:\\dist\\win-unpacked\\Manta.exe' }]
