...
describe('orchestration kernel', () => {
  ...
  it('classifies coordinator, dispatched worker, handoff, compatibility, and ordinary roles', () => {
    ...
    expect(squash(kernel)).toContain('Never substitute a non-Manta subagent tool')
  })
  ...
  it('teaches worker-start as the only normal-path launch and starts the wave before waiting', () => {
    ...
    expect(kernel).not.toMatch(/^MANTA terminal create/mu)
  })
  ...
  it('gives the supervised loop an exit condition for a live terminal with a dead agent', () => {
    ...
    expect(kernel).toContain('`MANTA orchestration worker-list --include-remote --json`')
  ...
  it('makes a dispatched worker read coordinator follow-ups on a cadence', () => {
    ...
    expect(kernel).toContain('`MANTA orchestration check --terminal <your_handle> --json`')
  })
...
describe('owned orchestration references', () => {
  it('routes every conditional read to exactly one shipped reference', () => {
    ...
    expect(kernel).toContain('MANTA skills get orchestration --full')
    ...
    expect(squash(kernel)).toContain(
      'run `MANTA skills get orchestration --reference references/<file>.md`'
    )
    expect(squash(kernel)).toContain(
      'If the CLI rejects `--reference`, run `MANTA skills get orchestration --full`'
    )
  ...
  it('owns worker heartbeat, ask resume, escalation, failure, and idle', () => {
    ...
    expect(squash(reference)).toContain(
      'MANTA orchestration check --terminal <worker_handle> --json'
    )
  ...
  it('owns local, folder, worktree, SSH, WSL, remote, and mixed-version placement', () => {
    ...
    expect(squash(reference)).toContain(
      'A worktree selector needs the full `<repo-id>::<path>` value Manta returned, passed as `id:<newFullWorktreeId>`; a bare repo id is not a worktree id'
    )
    ...
    expect(reference).toContain('printed `manta-ide`')
    expect(squash(reference)).toContain(
  ...
  it('names worker-list as the enumerating command and the agent-liveness authority', () => {
    ...
    expect(reference).toContain('MANTA orchestration worker-list --run <run_id> --json')
