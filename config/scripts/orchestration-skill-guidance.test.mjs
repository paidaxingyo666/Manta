...
describe('orchestration skill guidance', () => {
  ...
  it('requires Manta runtime state before claiming a worker was orchestrated', () => {
    ...
    expect(toolBoundary).toContain('create the Task with `manta orchestration task-create`')
    expect(toolBoundary).toContain('preferred `manta orchestration worker-start` composition')
    expect(toolBoundary).toContain('low-level `manta orchestration dispatch --inject` path')
    expect(toolBoundary).not.toContain('or `manta orchestration run`')
    expect(skill).toContain(
      '`coordinator-start`, `coordinator-stop`, `run`, and `run-stop` are retired scheduler commands'
    )
    expect(toolBoundary).toContain(
      'Do not substitute non-Manta subagent tools, generic agent-spawn APIs, or chat-only parallel worker features'
    )
    expect(toolBoundary).toContain('do not create Manta task/dispatch provenance')
    ...
    expect(toolBoundary).toContain('manta orchestration task-list --json')
    expect(toolBoundary).toContain('manta orchestration dispatch-show --task <task_id> --json')
    expect(toolBoundary).toContain(
  ...
  it('keeps full handoffs out of dispatch lifecycle and off the active branch base', () => {
    ...
    expect(fullHandoffs).toContain(
      'Do not run `manta orchestration task-create`, `manta orchestration dispatch --inject`, or `manta orchestration check --wait` for full handoffs.'
    )
    expect(fullHandoffs).toContain(
    ...
    expect(skill).toContain(
      '`--no-parent` only controls Manta lineage; it does not choose the Git base.'
    )
    expect(skill).toContain(
    ...
    expect(fullHandoffs).toContain(
      'Before creating a new worktree from an active feature branch, decide and state whether the desired Manta lineage is child or top-level'
    )
    expect(fullHandoffs).toContain(
  ...
  it('makes settled worker terminal release an explicit coordinator step', () => {
    ...
    expect(workerLoop).toContain(
      'manta orchestration worker-start --task <next_task_id> --terminal <handle> --json` so Manta ' +
    ...
    expect(workerLoop).toContain('manta orchestration worker-retain --dispatch <dispatch_id> --json')
    expect(workerLoop).toContain(
  ...
  it('keeps agent-first launch, handle recovery, and inbox injection distinct', () => {
    ...
    expect(agentFirstExample).not.toContain('manta terminal list')
...
describe('orchestration install stub', () => {
  it('points at the version-matched guide and preserves the safe resolver', () => {
    ...
    expect(stub).toContain('MANTA skills get orchestration')
  ...
  it('does not tell agents to mutate orchestration state before loading the guide', () => {
    ...
    expect(preGuide).not.toContain('manta orchestration task-create')
    expect(preGuide).not.toContain('manta orchestration dispatch')
  })
