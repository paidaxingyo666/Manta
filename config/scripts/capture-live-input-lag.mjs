...
// Called with an already-attached main Manta page; never launches, focuses, or reloads it.
export async function captureLiveInputLag(page, durationMs = 30_000) {
  ...
  const identity = await page.evaluate(async () => {
    if (!window.api?.app?.getIdentity) {
      throw new Error('Target is not the main Manta renderer')
    }
    if (window.__orcaLiveInputLag || window.__mantaIdleCpuTimingProbe) {
  ...
  try {
    ...
    await page.evaluate(() => {
      delete window.__mantaIdleCpuTimingProbe
    })
    timingStarted = false
