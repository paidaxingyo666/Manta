...
/** Builds a temporary Manta-style project fixture with a compiled CLI entrypoint. */
function makeProjectWithCli(
  content,
  { mode = 0o755, rootPackageType, writeOutPackageJson = true } = {}
) {
  const projectDir = mkdtempSync(path.join(tmpdir(), 'manta-cli-bin-'))
  ...
  writeFileSync(
    ...
    JSON.stringify({ bin: { manta: './out/cli/index.js' }, type: rootPackageType }),
    'utf8'
  )
...
describe('verifyPackageCliBin', () => {
  ...
  it('rejects an empty package bin target', () => {
    ...
    expect(() => verifyPackageCliBin({ projectDir })).toThrow('bin.manta target is empty')
  })

  it('rejects package bin targets without a Node shebang', () => {
    const { projectDir } = makeProjectWithCli('console.log("manta")\n')
