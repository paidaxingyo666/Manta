...
const agentSessionToken = '--manta-repro-agent-session'
...
try {
  ...
  execFileSync(
    ...
    [
      ...
      projectPath,
      '-c',
      'user.name=Manta Repro',
      '-c',
      'user.email=manta-repro@example.invalid',
      'commit',
      '--allow-empty',
      '-m',
  ...
  writeFileSync(
    path.join(profilePath, 'manta-data.json'),
    JSON.stringify({
...
async function startServer(port) {
  ...
  const env = {
    ...
    MANTA_DEV_USER_DATA_PATH: profilePath,
    MANTA_USER_DATA_PATH: profilePath,
    MANTA_REPRO_SPAWN_MARKER: spawnMarkerPath,
    MANTA_REPRO_EXIT_TRIGGER: exitTriggerPath,
    MANTA_REPRO_INPUT_MARKER: inputMarkerPath,
    MANTA_REPRO_AGENT_SESSION_TOKEN: agentSessionToken,
  ...
  return await new Promise((resolve, reject) => {
    ...
    lines.on('line', (line) => {
      try {
        ...
        if (parsed.type === 'manta_server_ready' && parsed.pairing?.url) {
