...
async function main(): Promise<void> {
  const environmentName = process.env.MANTA_PROBE_ENVIRONMENT_NAME
  if (!environmentName) {
    throw new Error('MANTA_PROBE_ENVIRONMENT_NAME is required')
  }
  ...
  const cycles = readProbeInteger('MANTA_PROBE_CYCLES', 10, 100)
  const concurrency = readProbeInteger('MANTA_PROBE_CONCURRENCY', 25, 200)
  const settleMs = readProbeInteger('MANTA_PROBE_SETTLE_MS', 250, 5_000)
  const cleanupTimeoutMs = readProbeInteger('MANTA_PROBE_CLEANUP_TIMEOUT_MS', 10_000, 30_000)
