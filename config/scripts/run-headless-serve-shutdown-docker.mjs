...
try {
  ...
  docker([
    ...
    [
      ...
      'timeout --kill-after=5s 15s /input/manta.AppImage --appimage-help > /artifacts/appimage-help.log 2>&1',
      'cd /artifacts',
      'timeout --kill-after=10s 120s /input/manta.AppImage --appimage-extract > /artifacts/appimage-extract.log 2>&1',
    ...
    for (const signal of ['INT', 'TERM']) {
      const result = docker(
        [
          ...
          '256m',
          '--name',
          `manta-headless-serve-shutdown-${entrypoint}-${signal.toLowerCase()}-${suffix}`,
          '-e',
...
function runDesktopStartupOracle({ image, appImage, platform }) {
  ...
  docker([
    ...
    '-e',
    'MANTA_STARTUP_DIAGNOSTICS=1',
    '-v',
    `${appImage}:/input/manta.AppImage:ro`,
    image,
    '/input/manta.AppImage'
  ])
}
