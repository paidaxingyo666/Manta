...
const WIN_ADHOC_ENV = {
  MANTA_WIN_ADHOC: '1',
  MANTA_ADHOC_BUILD_VERSION: '1.4.178-adhoc.20260819010203'
}
...
describe('electron-builder dev-channel identity', () => {
  ...
  it.each([
    ...
    ['adhoc', { MANTA_WIN_ADHOC: '1' }, 'manta-adhoc']
  ...
  // Why: MANTA_MAC_* gates hardened runtime, notarization, and root-level
  ...
  it('still notarizes mac dev builds', () => {
    ...
    expect(config.publish.repo).toBe('manta-adhoc')
  })
})

describe('collectDevChannelPackagingProblems', () => {
  const goodWinConfig = {
    publish: { repo: 'manta-adhoc', releaseType: 'prerelease' },
    extraMetadata: { version: '1.4.178-adhoc.20260819010203' },
    win: { verifyUpdateCodeSignature: false }
  }
  const env = { MANTA_ADHOC_BUILD_VERSION: '1.4.178-adhoc.20260819010203' }
  ...
  it('rejects a config that resolved the main repo', () => {
    const problems = collectDevChannelPackagingProblems({
      channel: 'adhoc',
      platform: 'win32',
      config: { ...goodWinConfig, publish: { repo: 'manta', releaseType: 'release' } },
      env
    })

    expect(problems.join('\n')).toContain('must publish to "manta-adhoc"')
