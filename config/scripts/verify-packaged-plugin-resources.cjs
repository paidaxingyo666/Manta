...
function hashPackagedPluginTree(root) {
  ...
  visit(root)
  const hash = createHash('sha256').update('manta-plugin-tree-v1\0')
...
function verifyPackagedPluginResources(resourcesDir) {
  ...
  readJsonFile(join(launchRoot, 'manta-marketplace.json'), 'marketplace index')
  ...
  for (const entry of index.plugins) {
    ...
    const manifest = readJsonFile(join(pluginRoot, 'manta-plugin.json'), 'plugin manifest')
