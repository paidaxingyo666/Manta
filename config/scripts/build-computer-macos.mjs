...
const binaryPath = path.join(packagePath, '.build', 'release', 'manta-computer-use-macos')
const appPath = path.join(packagePath, '.build', 'release', 'Manta Computer Use.app')
const appExecutablePath = path.join(appPath, 'Contents', 'MacOS', 'manta-computer-use-macos')
...
const bundleId = process.env.MANTA_COMPUTER_MACOS_BUNDLE_ID ?? 'cn.sh.manta.computer-use'
const displayName = 'Manta Computer Use'
...
function buildUniversalBinary() {
  const builtBinaries = universalTriples.map((triple) => {
    ...
    return path.join(packagePath, '.build', triple, 'release', 'manta-computer-use-macos')
  })
...
function codesignArgs(identity, targetPath) {
  ...
  if (process.env.MANTA_MAC_RELEASE === '1' && identity !== '-') {
...
function resolveSigningIdentity() {
  const explicitIdentity = process.env.MANTA_COMPUTER_MACOS_SIGN_IDENTITY ?? process.env.CSC_NAME
  if (explicitIdentity) {
...
<dict>
  ...
  <string>manta-computer-use-macos</string>
  ...
  <string>Manta Computer Use needs Accessibility permission to read and interact with app interfaces when you ask Manta to use apps.</string>
  <key>NSScreenCaptureUsageDescription</key>
  <string>Manta Computer Use needs Screen Recording permission to capture app windows when you ask Manta to inspect your screen.</string>
</dict>
</plist>
`
}
