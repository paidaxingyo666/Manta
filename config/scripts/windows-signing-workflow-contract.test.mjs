...
describe('Windows signing workflow contract', () => {
  ...
  it('verifies Windows inner binary signatures fail-open before publishing', () => {
    ...
    expect(steps[innerVerifyIndex].env.MANTA_WINDOWS_INNER_SIGNATURE_REQUIRED).toBe('false')
...
// Every link is asserted here the way Manta.exe and conpty_console_list.node are.
describe('Windows NSIS uninstaller signing', () => {
  ...
  it('reports the embedded uninstaller in the inner-binary evidence gate', () => {
    ...
    expect(gate.run).toContain('not signed by SignPath Foundation: Uninstall Manta.exe')
    // The uninstaller must not join the 7z payload loop, which cannot see it.
    expect(gate.run).not.toContain("$targets += 'Uninstall Manta.exe'")
  })
  ...
  it('rehearses the uninstaller leg end to end', () => {
    ...
    expect(verify.run).toContain('shipped: Uninstall Manta.exe')
  ...
  it('never lets an unreliable extract fail the rehearsal', () => {
    ...
    expect(verify.run).toContain("Get-Process -Name 'manta-terminal-daemon'")
  })
  ...
  it('confines the advisory escape hatch to elevate.exe', () => {
    ...
    for (const call of ['relayed: orca-uninstaller.exe', 'shipped: Uninstall Manta.exe']) {
