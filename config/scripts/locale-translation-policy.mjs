...
export function shouldPreserveEnglishValue(enValue, key = '') {
  ...
  if (/^https?:\/\//.test(enValue) || enValue.startsWith('manta://')) {
    return true
  }
...
function applyCjkLatinTermSpacing(localeValue, locale) {
  ...
  if (locale === 'ko') {
    ...
    // boundary — re-glue "Manta 에"/"PR 을"/"에서는" but keep "Jira 이슈"/"Manta 로고"/"agent 에뮬레이터".
...
export function repairTranslatedValue({ key, enValue, localeValue, locale }) {
  ...
  if (enValue.includes('manta://')) {
    result = result.replace(/虎鲸:\/\//g, 'manta://')
  }

  if (enValue === 'Manta' || enValue.startsWith('Manta ')) {
    result = result
      .replaceAll('虎鲸', 'Manta')
      .replaceAll('逆戟鲸', 'Manta')
      .replaceAll('シャチ', 'Manta')
  }

  if (enValue.includes('manta://')) {
    result = result.replace(/シャチ:\/\//g, 'manta://')
  }
...
export function repairCatalog(enCatalog, localeCatalog, locale) {
  ...
  if (localeCatalog.menu) {
    if (locale === 'zh') {
      if (localeCatalog.menu.exploreManta !== '探索 Manta') {
        localeCatalog.menu.exploreManta = '探索 Manta'
        repaired += 1
      }
      if (localeCatalog.menu.gettingStarted !== 'Manta 入门') {
        localeCatalog.menu.gettingStarted = 'Manta 入门'
        repaired += 1
      }
    }
    if (locale === 'ko') {
      if (localeCatalog.menu.exploreManta !== 'Manta 둘러보기') {
        localeCatalog.menu.exploreManta = 'Manta 둘러보기'
        repaired += 1
      }
      if (localeCatalog.menu.gettingStarted !== 'Manta 시작하기') {
        localeCatalog.menu.gettingStarted = 'Manta 시작하기'
        repaired += 1
      }
    }
  }
