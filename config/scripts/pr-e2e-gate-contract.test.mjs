...
describe('PR E2E gate contract', () => {
  ...
  it('maps SSH source edits onto the Docker-backed specs they can break', () => {
    // Why: the Docker-SSH specs self-skip without MANTA_E2E_SSH_DOCKER, and the only
    ...
    for (const spec of mappedSpecs) {
      ...
      if (spec !== 'tests/e2e/ssh-startup-exec-readiness.spec.ts') {
        expect(readFileSync(join(projectDir, spec), 'utf8'), spec).toContain('MANTA_E2E_SSH_DOCKER')
      }
    }
  ...
  it('routes P0 sentinels from their causal sources', () => {
    const cases = [
      ...
      ['src/main/runtime/manta-runtime-files.ts', 'tests/e2e/paired-quick-open-large-tree.spec.ts'],
      [
    ...
    expect(
      selectPrE2eSpecs([
        'src/main/runtime/manta-runtime-files.ts',
        'tests/e2e/paired-quick-open-large-tree.spec.ts'
      ])
