...
async function readRendererProbe(page) {
  return await runWithTimeout(
    'renderer probe read',
    () => page.evaluate(() => globalThis.__mantaApphangProbe?.probe ?? null),
    rendererActionTimeoutMs
  )
}
...
async function resetRendererProbe(page) {
  ...
  await runWithTimeout(
    ...
    () =>
      page.evaluate(() => {
        const probe = globalThis.__mantaApphangProbe?.probe
        if (!probe) {
          return
        }
...
async function runActivationCycle(page, target, args) {
  ...
  const marker = `MANTA_APPHANG_${target.index}_${Date.now()}`
