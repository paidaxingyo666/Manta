...
async function main() {
  ...
  let relay
  try {
    tempRoot = await mkdtemp(join(tmpdir(), 'manta-relay-watcher-fault-'))
    ...
    const socketPath =
      process.platform === 'win32'
        ? `\\\\.\\pipe\\manta-relay-watcher-fault-${process.pid}-${Date.now()}`
    ...
    daemon = spawn(
      ...
      ],
      {
        ...
        env: { ...process.env, MANTA_WATCHER_CHILD_PID_FILE: pidFile },
    ...
    const beforePtyMarker = `MANTA_PTY_BEFORE_${Date.now()}`
    ...
    const afterPtyMarker = `MANTA_PTY_AFTER_${Date.now()}`
    startIndex = relay.messageCount()
