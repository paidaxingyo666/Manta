...
export function initGitRepo(projectDir) {
  ...
  runGit(projectDir, ['config', 'user.email', 'manta-test@example.com'])
  runGit(projectDir, ['config', 'user.name', 'Manta Test'])
...
export function sharedCacheRoot(repoDir) {
  return join(repoDir, '.git', 'manta-cache', 'electron')
}

export function readSharedDistMarker(projectDir) {
  try {
    return readFileSync(join(projectDir, 'node_modules/electron/.manta-shared-dist'), 'utf8')
  } catch {
    return null
  }
}
