...
const NATIVE_MODULES = [
  ...
  ...(process.platform === 'win32'
    ? ['@manta/windows-registry', '@vscode/windows-process-tree']
    : [])
]
...
function loadNodePtyNativeModule() {
  ...
  if (requiresPatchedNodePtySourceBuild() && !isNodePtyReleaseBuildDir(native?.dir)) {
    throw new Error(
      `node-pty resolved to ${native.dir}; expected build/Release so Manta's node-pty patch is active`
    )
  }
}
...
function getPatchedNodePtyRebuildReason() {
  ...
  // Why: a loadable upstream node-pty prebuild is not enough; Manta's Unix and
