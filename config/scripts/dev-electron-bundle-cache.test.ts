...
function psLine(dir: string, appName = 'Manta: some-branch') {
...
describe('dev-electron-bundle-cache', () => {
  ...
  it('protects a live bundle whose path contains spaces', () => {
    ...
    const spaced = '/Users/me/My Projects/manta/out/electron-dev/aaaaaaaaaaaa'
    const other = '/Users/me/My Projects/manta/out/electron-dev/bbbbbbbbbbbb'
    expect(
      selectStaleDevBundleDirs({
        bundles: [settled(spaced), settled(other)],
        currentDir: C,
        processTable: psLine(spaced, 'Manta: my branch'),
        nowMs: NOW
      })
    ).toEqual([other])
  })
