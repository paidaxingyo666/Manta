...
if (!appImageArg) {
  fail('Usage: run-headless-linux-pairing-docker.mjs --appimage /path/to/manta.AppImage')
}
...
function extractAppImage(image) {
  ...
  docker([
    ...
    image,
    '-lc',
    'cp /input/manta.AppImage /artifacts/manta.AppImage && chmod +x /artifacts/manta.AppImage && cd /artifacts && ./manta.AppImage --appimage-extract >/dev/null && chmod -R a+rX /artifacts/squashfs-root'
  ])
}
...
async function validateStartupMatrix() {
  ...
  const publicEntry = await startAndWait({
    ...
    mode: 'json',
    address: '127.0.0.1',
    appPath: '/artifacts/manta.AppImage',
...
async function validateAuthenticatedPairing() {
  const server = await startAndWait({
    ...
    launch: 'direct',
    mode: 'json',
    address: 'ws://manta-pairing-server:6768/runtime?route=runtime',
    port: '6768',
    networkAlias: 'manta-pairing-server'
  })
  ...
  assert(
    ...
    'paired server did not report its Manta app version'
  )
  assert(
...
async function validateUnreachableOffer() {
  const server = await startAndWait({
    ...
    launch: 'direct',
    mode: 'json',
    address: 'manta-pairing-server:6769',
    port: '6768',
    networkAlias: 'manta-pairing-server'
  })
  const payload = readyJson(server.stdout)
...
async function startAndWait({
  ...
  const name = `manta-pairing-${suffix}-${containers.size}`
  const args = [
    'run',
    '-d',
    '--init',
    ...
    '-e',
    'MANTA_KEEP_RUNNING=1',
    '-e',
    'LIBGL_ALWAYS_SOFTWARE=1',
    '-e',
    `MANTA_READY_JSON=${mode === 'json' ? '1' : '0'}`,
    '-e',
    `MANTA_PAIRING_ADDRESS=${address}`,
    '-e',
    `MANTA_SERVE_PORT=${port}`,
    '-e',
    `MANTA_TEST_APPIMAGE=${appPath}`,
    ...(noPairing ? ['-e', 'MANTA_NO_PAIRING=1'] : []),
    '-v',
...
function hasCompleteReadyContract(stdout) {
  if (
    stdout.includes('Manta server ready\n') &&
...
function validateReady(logs, mode, expectedHost, options = {}) {
  if (mode === 'human') {
    assert(
      (logs.match(/^Manta server ready$/gm) ?? []).length === 1,
    ...
    assert(logs.includes('Pairing URL: manta://pair?code='), 'human pairing URL is missing')
    return
  }
...
function readyJsonObjects(logs) {
  const marker = '{"type":"manta_server_ready"'
  return logs
    .split(/\r?\n/)
    .map((line) => {
    ...
    .filter((value) => value?.type === 'manta_server_ready')
}
