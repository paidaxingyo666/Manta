    ...
    try {
      ...
      const result = spawnSync(process.execPath, [scriptPath, '--runtime=node'], {
        ...
        env: envWithPrependedPath(binDir, {
          MANTA_NATIVE_TEST_LOG: logPath,
          MANTA_NATIVE_TEST_MARKER: markerPath
        })
      })
...
function mkTempProject() {
  const projectDir = mkdtempSync(join(tmpdir(), 'manta-native-runtime-'))
...
exports.loadNativeModule = function loadNativeModule(nativeName) {
  const markerExists = existsSync(process.env.MANTA_NATIVE_TEST_MARKER)
  appendFileSync(
    process.env.MANTA_NATIVE_TEST_LOG,
...
exports.loadNativeModule = function loadNativeModule(nativeName) {
  const rebuilt = existsSync(process.env.MANTA_NATIVE_TEST_MARKER)
  const dir = ${JSON.stringify(nativeDir)} ??
    (rebuilt ? '../build/Release/' : '../prebuilds/' + process.platform + '-' + process.arch + '/')
  appendFileSync(process.env.MANTA_NATIVE_TEST_LOG, \`node-pty load \${nativeName} dir=\${dir}\\n\`)
  return { dir, module: {} }
}
`
  )
...
appendFileSync(process.env.MANTA_NATIVE_TEST_LOG, \`pnpm \${process.argv.slice(2).join(' ')}\\n\`)
writeFileSync(process.env.MANTA_NATIVE_TEST_MARKER, 'rebuilt')
`
  )
