  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      ...
      try {
        ...
        expect(log).toContain(join('node_modules', 'node-pty'))
        expect(log).toContain(join('node_modules', '@manta', 'windows-registry'))
      } finally {
        rmSync(projectDir, { recursive: true, force: true })
      }
    }
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      try {
        ...
        expect(result.status, result.stderr).toBe(0)
        expect(result.stderr).toContain("expected build/Release so Manta's node-pty patch is active")
        expect(readFileSync(logPath, 'utf8')).toContain('pnpm exec node-gyp rebuild\n')
      } finally {
        rmSync(projectDir, { recursive: true, force: true })
      }
...
function writeFakeWindowsRegistry(projectDir, { requiresMarker = false } = {}) {
  ...
  const registryDir = join(projectDir, 'node_modules', '@manta', 'windows-registry')
  mkdirSync(registryDir, { recursive: true })
  writeFileSync(
    join(registryDir, 'package.json'),
    '{"name":"@manta/windows-registry","version":"1.0.0","main":"index.js"}\n'
  )
