...
function parseEndpoint(contents) {
  const port = Number(/MANTA_AGENT_HOOK_PORT=['"]?(\d+)/.exec(contents)?.[1])
  const token = /MANTA_AGENT_HOOK_TOKEN=['"]?([^'"\r\n]+)/.exec(contents)?.[1]
...
async function writeEndpoint(distro, path, endpoint) {
  const contents = [
    `MANTA_AGENT_HOOK_PORT=${endpoint.port}`,
    `MANTA_AGENT_HOOK_TOKEN=${endpoint.token}`,
    'MANTA_AGENT_HOOK_ENV=benchmark',
...
async function invokeHook(distro, scriptPath, endpointPath) {
  ...
  const result = await run(
    ...
    wslArgs(distro, [
      ...
      'MANTA_TAB_ID=wsl-relay-bench',
      'MANTA_WORKTREE_ID=wsl-relay-bench',
      '/bin/sh',
      scriptPath
    ]),
...
async function main() {
  ...
  if (nodeVersion.status !== 0 || Number(nodeVersion.stdout.trim().split('.')[0]) < 18) {
    throw new Error(`WSL distro '${distro}' needs Node.js 18 or newer to run Manta's relay`)
  }
  ...
  let staller = null
  try {
    userDataDir = mkdtempSync(join(tmpdir(), 'manta-wsl-relay-bench-'))
    ...
    const benchmarkRoot = `${guestHome}/.manta-wsl/benchmarks/${instanceKey}`
    const scriptPath = `${benchmarkRoot}/.manta/agent-hooks/codex-hook.sh`
    const endpointPath = `${guestHome}/.manta-wsl/agent-hooks/instance-${instanceKey}/endpoint.env`
    cleanupPaths = [benchmarkRoot, `${guestHome}/.manta-wsl/agent-hooks/instance-${instanceKey}`]
    if (cleanupPaths.some((cleanupPath) => !cleanupPath.startsWith(`${guestHome}/.manta-wsl/`))) {
    ...
    const survivingSessionId = `manta-wsl-relay-bench-${instanceKey}`
    ...
    const createManager = async (token) => {
      ...
      return new WslHookRelayManager({
        ...
        hookCoordsEnv: () => ({
          MANTA_AGENT_HOOK_PORT: String(preferredPort),
