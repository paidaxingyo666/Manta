...
// Why: under a Git Bash setup runner, Manta exports MANTA_WORKTREE_PATH in MSYS form (/c/...), which
...
export function runInternalDevSetup({
  ...
  const setupPath = env.MANTA_INTERNAL_DEV_SETUP?.trim()
  ...
  spawnOptionalSetup(spawn, setupPath, env.MANTA_WORKTREE_PATH || cwd, platform, env)
