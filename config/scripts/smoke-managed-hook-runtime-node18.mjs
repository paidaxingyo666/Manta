...
const home = await mkdtemp(join(tmpdir(), 'manta-managed-hook-node18-'))
...
try {
  ...
  assert.match(codexHooks, /\.manta\/agent-hooks\/codex-hook\.sh/)
  assert.match(claudeSettings, /\.manta\/agent-hooks\/claude-hook\.sh/)
  await access(join(home, '.manta', 'agent-hooks', 'codex-hook.sh'), constants.X_OK)
  await access(join(home, '.manta', 'agent-hooks', 'claude-hook.sh'), constants.X_OK)
} finally {
