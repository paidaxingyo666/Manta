...
describe('manta CLI skill guidance', () => {
  it('keeps independent worktree lineage separate from Git base selection', () => {
    const skill = readSkill()

    expect(skill).toContain('`--no-parent` only controls Manta lineage')
    expect(skill).toContain('omit `--base-branch` so Manta uses the repo default base')
  ...
  it('documents non-lifecycle full handoffs and custom Codex model fallback', () => {
    ...
    expect(skill).toContain(
      'Do not use `manta orchestration task-create`, `manta orchestration dispatch --inject`, or `manta orchestration check --wait` for full handoffs.'
    )
    expect(skill).toContain(
  ...
  it('keeps browser injection guidance narrow and avoids literal secret examples', () => {
    ...
    expect(skill).toContain('`manta eval` expressions, or `manta exec` commands')
...
describe('manta CLI install stub', () => {
  ...
  it('drops the changing command reference from the installable file', () => {
    ...
    expect(stub).not.toContain('MANTA automations create')
