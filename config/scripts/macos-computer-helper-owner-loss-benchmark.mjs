...
const INTERNAL_ENV = 'MANTA_COMPUTER_HELPER_OWNER_BENCH_INTERNAL'
const EXPECTATION_ENV = 'MANTA_COMPUTER_HELPER_OWNER_BENCH_EXPECTATION'
const HELPER_RECORD_PATH_ENV = 'MANTA_COMPUTER_HELPER_OWNER_BENCH_HELPER_RECORD_PATH'
const RESULT_PATH_ENV = 'MANTA_COMPUTER_HELPER_OWNER_BENCH_RESULT_PATH'
...
const helperPath = path.join(helperAppPath, 'Contents', 'MacOS', 'manta-computer-use-macos')
...
function startSidecar() {
  ...
  const child = fork(sidecarPath, [], {
    ...
    env: {
      ...
      MANTA_COMPUTER_SIDECAR: '1',
      MANTA_COMPUTER_MACOS_HELPER_APP_PATH: helperAppPath
    }
  })
...
function runTrial(executable, expectation) {
  ...
  let trialOutput = ''
  try {
    launcherDir = mkdtempSync(path.join(tmpdir(), 'manta-helper-owner-bench-launcher-'))
    trialTempDir = mkdtempSync(path.join(path.sep, 'tmp', 'manta-owner-bench-'))
    ...
    writeFileSync(
      ...
      JSON.stringify({ name: 'manta-helper-owner-benchmark', main: 'main.cjs' })
    )
    writeFileSync(
