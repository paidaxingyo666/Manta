...
if gh api "repos/$DAILY_REPO" --jq '.full_name' >/dev/null 2>&1; then
  ...
  # as it does for manta-hourly. A private repo would 404 for every client.
  #
  ...
  gh repo create "$DAILY_REPO" \
    --public \
    --description "Daily macOS dev builds of Manta, cut from main each morning. Not a source repo." \
    --add-readme \
    --disable-issues \
    --disable-wiki ||
...
Could not do it from here${INSTALL_ID:+ (needs an Organization Owner)}. Do it in the browser:
  ...
  3. Repository access  ->  Only select repositories  ->  add $DAILY_REPO
     (keep manta-hourly and manta-adhoc selected; all dev channels use this one App)
  4. Save.
EOF
fi
