#!/usr/bin/env bash
# Capture one-shot macOS launch diagnostics for a published Manta release.
#
# Usage:
#   MANTA_DIAGNOSTIC_TAG=v1.4.42-rc.1 bash config/scripts/macos-launch-diagnostics.sh
...
if [[ -z "$TAG" ]]; then
  echo "Set MANTA_DIAGNOSTIC_TAG or pass --tag vX.Y.Z-rc.N." >&2
  exit 2
fi

ARCH="$(uname -m)"
case "$ARCH" in
  arm64) ASSET="manta-macos-arm64.dmg" ;;
  x86_64) ASSET="manta-macos-x64.dmg" ;;
  *)
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/manta-launch-diagnostics.XXXXXXXX")"
OUT_DIR="$WORK_DIR/output"
MOUNT_DIR="$WORK_DIR/mount"
APP_DIR="$WORK_DIR/Manta.app"
DMG_PATH="$WORK_DIR/$ASSET"
mkdir -p "$OUT_DIR" "$MOUNT_DIR"

if [[ -d "$HOME/Desktop" ]]; then
  ZIP_PATH="$HOME/Desktop/manta-launch-diagnostics-${TAG}-${TIMESTAMP}.zip"
else
  ZIP_PATH="$PWD/manta-launch-diagnostics-${TAG}-${TIMESTAMP}.zip"
fi

diag_log() {
  printf '[manta-diagnostics] %s\n' "$*"
}
...
write_environment_report() {
  {
    ...
    echo "$SHELL"
    echo
    echo "## current Manta processes before diagnostics"
    pgrep -fl 'Manta|manta' || true
  } >"$OUT_DIR/environment.txt"
}

ensure_no_existing_manta() {
  local existing
  existing="$(pgrep -x Manta || true)"
  if [[ -z "$existing" ]]; then
    return 0
  fi

  {
    echo "A Manta process is already running. Close Manta and run this script again."
    echo
    ps -p "$(printf '%s' "$existing" | paste -sd, -)" -o pid=,ppid=,command= || true
  } | tee "$OUT_DIR/existing-manta-process.txt" >&2
  exit 2
}
...
start_log_stream() {
  local file="$1"
  local predicate='process == "Manta" OR eventMessage CONTAINS[c] "Manta" OR eventMessage CONTAINS[c] "cn.sh.manta"'
...
latest_manta_pid_for_app() {
  pgrep -nf "$APP_DIR/Contents/MacOS/Manta" || true
}
...
wait_for_probe() {
  ...
  for i in $(seq 1 200); do
    local app_pid
    app_pid="$(latest_manta_pid_for_app)"
...
run_launchservices_probe() {
  ...
  if [[ -n "$extra_env_name" ]]; then
    open -n -W \
      ...
      --env MANTA_STARTUP_DIAGNOSTICS=trace \
      --env MANTA_STARTUP_DIAGNOSTICS_TRACE_LIMIT=30000 \
      --env MANTA_STARTUP_DIAGNOSTICS_FILE="$bootstrap_file" \
...
run_direct_exec_probe() {
  ...
  ELECTRON_ENABLE_LOGGING=1 \
    MANTA_STARTUP_DIAGNOSTICS=trace \
    MANTA_STARTUP_DIAGNOSTICS_TRACE_LIMIT=30000 \
    MANTA_STARTUP_DIAGNOSTICS_FILE="$bootstrap_file" \
    "$APP_DIR/Contents/MacOS/Manta" >"$stdout_file" 2>"$stderr_file" &
  local runner_pid="$!"
  wait_for_probe "$runner_pid" "$label"
  echo "ended_utc=$(date -u '+%Y-%m-%dT%H:%M:%SZ')" >>"$OUT_DIR/$label.meta"
  stop_log_stream "$stream_pid"
...
run_launchservices_probe "launchservices-bypass-lock" "MANTA_BYPASS_SINGLE_INSTANCE_LOCK" "1"
run_direct_exec_probe
write_system_log_snapshot
package_results
