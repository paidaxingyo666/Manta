...
describe('buildMobileWebBundle', () => {
  it('emits a content-addressed bundle whose only stable name is the entrypoint', async () => {
    ...
    try {
      ...
      expect(html).not.toContain('__MANTA_')
    } finally {
...
describe('isDirectInvocation', () => {
  ...
  it('matches a Windows entry path, which the file:// template form never does', () => {
    const scriptPath = 'C:\\manta\\config\\scripts\\build-mobile-web-bundle.mjs'
    const moduleUrl = 'file:///C:/manta/config/scripts/build-mobile-web-bundle.mjs'
    ...
    // The regression this guards: `file://${argv[1]}` yields file://C:\manta\... on Windows,
