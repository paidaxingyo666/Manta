...
const cliEntry =
  process.env.MANTA_DEV_CLI_ENTRY_PATH ?? path.join(repoRoot, 'out', 'cli', 'index.js')

if (!existsSync(cliEntry)) {
  console.error("manta-dev: CLI not built yet. Run 'pnpm run build:cli' first.")
  process.exit(1)
}
...
if (!process.env.MANTA_APP_EXECUTABLE && isRunnableFile(electronExecutable)) {
  process.env.MANTA_APP_EXECUTABLE = electronExecutable
  process.env.MANTA_APP_EXECUTABLE_NEEDS_APP_ROOT = '1'
}

// Why: headless `manta-dev serve` skips the Electron dev runner that normally installs terminal CLI shims.
prepareDevCliTerminalWrappers({
  repoRoot,
  userDataPath: process.env.MANTA_USER_DATA_PATH,
  electronExecutable: process.env.MANTA_APP_EXECUTABLE ?? electronExecutable
})
