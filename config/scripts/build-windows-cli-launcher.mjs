...
function defaultOutputPath(projectRoot) {
  return join(projectRoot, 'native', 'windows-cli-launcher', '.build', 'manta.exe')
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  if (process.platform !== 'win32') {
    ...
    // continue toward a Windows package whose declared manta.exe does not exist.
    throw new Error(
  ...
  const sourcePath = join(repoRoot, 'native', 'windows-cli-launcher', 'MantaCliLauncher.cs')
  const outputPath = readArg('--output') ?? defaultOutputPath(repoRoot)
  const compilerPath = findFrameworkCompiler(process.env)

  if (!compilerPath) {
    throw new Error('Unable to find the .NET Framework C# compiler required for manta.exe.')
  }

  mkdirSync(dirname(outputPath), { recursive: true })
  if (
    shouldReuseCompiledWindowsCliLauncher(outputPath, sourcePath, {
      reuseCached: process.env.MANTA_REUSE_WINDOWS_CLI_LAUNCHER === '1'
    })
  ) {
