...
const MUTABLE_BUILD_ENV = [
  ...
  'MANTA_MAC_ADHOC',
  'MANTA_MAC_RELEASE',
  'MANTA_HOURLY_BUILD_VERSION',
  'MANTA_DAILY_BUILD_VERSION',
  'MANTA_ADHOC_BUILD_VERSION',
  'MANTA_LOCAL_BUILD_VERSION'
]
...
const withHourlyEnv = (assert) => withEnv({ MANTA_MAC_HOURLY: '1' }, assert)
const withDailyEnv = (assert) => withEnv({ MANTA_MAC_DAILY: '1' }, assert)
const withAdhocEnv = (assert) => withEnv({ MANTA_MAC_ADHOC: '1' }, assert)
...
describe('electron-builder mac channel config', () => {
  ...
  // (cn.sh.manta.local, ad-hoc) identity would be un-installable over a real
  // Manta — the whole point of the channel.
  it('builds hourly artifacts with the release signing identity', () => {
    withHourlyEnv((config) => {
      expect(config.mac.appId).toBeUndefined()
      expect(config.appId).toBe('cn.sh.manta')
  ...
  it('notarizes hourly builds like releases, and neither locally', () => {
    ...
    withEnv({ MANTA_MAC_RELEASE: '1' }, (config) => {
      expect(config.mac.notarize).toBe(true)
    })
  ...
  it('stamps hourly packages with the hourly version', () => {
    withEnv(
      { MANTA_MAC_HOURLY: '1', MANTA_HOURLY_BUILD_VERSION: '1.4.160-hourly.202607281400' },
      (config) => {
  ...
  // real Manta through the same updater path, so the same signing and the same TCC
  ...
  it('builds adhoc artifacts with the release identity and its own repo', () => {
    withAdhocEnv((config) => {
      ...
      expect(config.mac.notarize).toBe(true)
      expect(config.forceCodeSigning).toBe(true)
      expect(config.publish).toMatchObject({ repo: 'manta-adhoc', releaseType: 'prerelease' })
    })
  })

  it('stamps adhoc packages with the adhoc version', () => {
    withEnv(
      { MANTA_MAC_ADHOC: '1', MANTA_ADHOC_BUILD_VERSION: '1.4.160-adhoc.20260728140533' },
      (config) => {
  ...
  it('builds daily artifacts with the release identity and its own repo', () => {
    withDailyEnv((config) => {
      ...
      expect(config.mac.notarize).toBe(true)
      expect(config.forceCodeSigning).toBe(true)
      expect(config.publish).toMatchObject({ repo: 'manta-daily', releaseType: 'prerelease' })
    })
  })

  it('stamps daily packages with the daily version', () => {
    withEnv(
      { MANTA_MAC_DAILY: '1', MANTA_DAILY_BUILD_VERSION: '1.4.160-daily.202607281300' },
      (config) => {
  ...
  // repos — a branch or daily build landing in manta-hourly would be offered to
