...
const tag = `manta-cli-launch-contract:ubuntu-24.04-${tagArchitecture}-${suffix}`
...
const CASES = [
  {
    ...
    expectStatus: 0,
    expectOutput: 'Usage: manta <command>',
...
try {
  if (!appImage) {
    fail(
      'Usage: run-linux-cli-launch-contract-docker.mjs --appimage /path/to/manta-linux.AppImage [--platform linux/amd64|linux/arm64]'
    )
  }
...
function stageArtifacts() {
  ...
  docker(
    [
      ...
      `${artifactVolume}:/artifacts`,
      '-v',
      `${appImage}:/input/manta-linux.AppImage:ro`,
      '--entrypoint',
      'bash',
      tag,
      '-lc',
      [
        'set -euo pipefail',
        'cp /input/manta-linux.AppImage /artifacts/manta-linux.AppImage',
        'chmod +x /artifacts/manta-linux.AppImage',
        'chown -R manta:manta /artifacts',
        // Use the AppImage runtime's no-FUSE extraction path.
        'cd /artifacts && runuser --user manta -- ./manta-linux.AppImage --appimage-extract >/dev/null',
        'test -x /artifacts/squashfs-root/resources/bin/manta-ide'
      ].join(' && ')
    ],
