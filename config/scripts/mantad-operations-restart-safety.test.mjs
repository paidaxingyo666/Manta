...
const operationsGuide = readFileSync('docs/reference/mantad-operations.md', 'utf8')
const operationsProse = operationsGuide.replace(/\s+/g, ' ')

describe('mantad operations restart safety', () => {
  ...
  it('fails closed before cgroup-wide maintenance', () => {
    ...
    expect(operationsGuide).not.toContain('sudo -Hu manta manta-ide terminal list --json')
    expect(operationsProse).toContain(
    ...
    expect(operationsProse).toContain('Manta does not yet provide an atomic census-and-stop fence')
  })
