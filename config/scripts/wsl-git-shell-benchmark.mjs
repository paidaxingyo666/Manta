...
async function main() {
  ...
  const loginPath = /__MANTA_PATH__(.*)/.exec(probeText)?.[1]?.trim()
  const gitPath = /__MANTA_GIT__(.*)/.exec(probeText)?.[1]?.trim()
  const loginHome = /__MANTA_HOME__(.*)/.exec(probeText)?.[1]?.trim()
  ...
  const outputMarker = `__MANTA_GIT_OUTPUT_${process.pid}__\n`
  ...
  const benchmarkRepo = async (repo) => {
    const branch = (
      ...
      .toString('utf8')
      .trim()
    const fixtureName = `.manta-wsl-git-shell-benchmark-${process.pid}-${randomUUID()}.txt`
  ...
  const result = {
    ...
    loginProbePreambleBytes: Buffer.byteLength(probeText.split('__MANTA_PATH__', 1)[0]),
    guestProcessShape: {
