...
const ITERATIONS = Number.parseInt(process.env.MANTA_PR_CARRY_BENCH_ITERATIONS ?? '2000', 10)
const WARMUP = Number.parseInt(process.env.MANTA_PR_CARRY_BENCH_WARMUP ?? '200', 10)

for (const [name, value] of [
  ['MANTA_PR_CARRY_BENCH_ITERATIONS', ITERATIONS],
  ['MANTA_PR_CARRY_BENCH_WARMUP', WARMUP]
]) {
...
const EQUIVALENCE_FIXTURES = [
  `noise ${'x'.repeat(400)}https://github.com/acme/manta/pull/7`,
  `https://github.com/acme/manta/pull/1${'x'.repeat(600)}`,
  `https://github.com/acme/manta/pull/1${'x'.repeat(600)}https`,
  `${'x'.repeat(1000)}https`,
  `${'x'.repeat(1000)}http`,
  'https://github.com/acme/manta/pull/7 trailing words',
  `${'y'.repeat(600)}`,
  '',
  'https://github.com/acme/manta/pull/7'
]
