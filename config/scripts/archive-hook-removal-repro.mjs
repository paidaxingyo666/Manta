 ...
 * and drives `manta worktree rm` end to end against real git worktrees on disk.
 */
...
function ok(args) {
  ...
  if (parsed.ok === false) {
    throw new Error(`manta ${args.join(' ')} failed: ${parsed.error?.code} ${parsed.error?.message}`)
  }
...
/** A throwaway git repo with one commit; optionally an manta.yaml archive hook. */
...
/** Filesystem + git truth about a worktree, read directly rather than through Manta. */
...
async function main() {
  ...
  try {
    ...
    out('\n$ manta worktree rm --worktree <id> --run-hooks     (human output)')
    ...
    out(
      '\n$ manta worktree rm --worktree <id> --force --run-hooks --json   (--force must NOT waive)'
    )
    ...
    check('Manta still resolves the worktree', shown?.id === wt1.id)
    check(
    ...
    out(`\n$ manta worktree rm --worktree <folder workspace> --force --run-hooks`)
