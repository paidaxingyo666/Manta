...
describe('static AppImage package contract', () => {
  it.each([
    ['manta-linux.AppImage', 0x3e, 1],
    ['manta-linux-arm64.AppImage', 0xb7, 'arm64']
  ...
  it.each([
    ['generic filename for an arm64 runtime and target', 'manta-linux.AppImage', 0xb7, 3],
    ['arm64 filename for an x64 runtime and target', 'manta-linux-arm64.AppImage', 0x3e, 1],
    ['generic x64 runtime for an arm64 target', 'manta-linux.AppImage', 0x3e, 3],
    ['generic arm64 runtime for an x64 target', 'manta-linux.AppImage', 0xb7, 1],
    ['arm64 artifact filename for an x64 target', 'manta-linux-arm64.AppImage', 0xb7, 1],
    ['x64 runtime under an arm64 artifact filename', 'manta-linux-arm64.AppImage', 0x3e, 3]
  ...
  it.each([undefined, 0, 'ia32'])(
    ...
    async (targetArch) => {
      await withFixture('manta-linux.AppImage', createRuntime(), (path) => {
  ...
  it('accepts PT_DYNAMIC relocation metadata without dependencies', async () => {
    ...
    await withFixture('manta-linux.AppImage', runtime, (path) => {
  ...
  it('does not scan the appended AppImage payload as outer ELF data', async () => {
    ...
    await withFixture('manta-linux.AppImage', Buffer.concat([createRuntime(), payload]), (path) => {
      expect(() => verifyStaticAppImagePackage(path, 1)).not.toThrow()
    })
  ...
  it('rejects artifact names outside the release contract before reading them', () => {
    expect(() => verifyStaticAppImagePackage('/missing/manta-preview.AppImage')).toThrow(
