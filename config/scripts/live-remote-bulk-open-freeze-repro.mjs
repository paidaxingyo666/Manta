#!/usr/bin/env node
/**
 * Live freeze repro against a running Manta desktop + paired remote runtime.
 *
 * Models bulk-open of remote sessions under multi-worktree load.
 *
 * Usage:
 *   node config/scripts/live-remote-bulk-open-freeze-repro.mjs
 *   MANTA_FREEZE_ENV=paired-remote MANTA_FREEZE_CREATE=12 MANTA_FREEZE_SWITCH_PASSES=5 \
 *     MANTA_FREEZE_PARALLEL=8 node config/scripts/live-remote-bulk-open-freeze-repro.mjs
 */
...
const switchPasses = Math.max(1, readFreezeNumberEnv('MANTA_FREEZE_SWITCH_PASSES', 3))
const parallel = Math.max(1, readFreezeNumberEnv('MANTA_FREEZE_PARALLEL', 1))
// 0 = no cap (use all live terminals). Only positive env values limit targets.
const maxSwitchTargets = Math.max(0, readFreezeNumberEnv('MANTA_FREEZE_MAX_SWITCH_TARGETS', 0))
...
const createWorktreeSpan = Math.max(1, readFreezeNumberEnv('MANTA_FREEZE_CREATE_WT_SPAN', 16))
const preFloodMs = Math.max(0, readFreezeNumberEnv('MANTA_FREEZE_PRE_FLOOD_MS', 3000))
const scratchDir = process.env.MANTA_FREEZE_SCRATCH || ''

const { mantaJsonSync, mantaJsonAsync } = createMantaRpc({ envName })
...
function sampleMantaIfPossible() {
  ...
  try {
    ...
    const out = path.join(reportDir, `manta-sample-${Date.now()}.txt`)
...
async function main() {
  ...
  const status = mantaJsonSync(['status'])
  notes.push(
  ...
  const statusProbe = mantaJsonSync(['status'], { local: true })
  let memoryProbeMs = null
  try {
    const mem = mantaJsonSync(['diagnostics', 'memory'], { local: true, timeoutMs: 120_000 })
