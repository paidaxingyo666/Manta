...
describe('vendored xterm WebGL runtime contract', () => {
  it('keeps shared WebGL atlas invalidation per renderer', () => {
    // Manta panes with the same font share one atlas, so a page merge or a clear in one
    ...
    // Manta patches in. Assert it on the resolved dependency rather than on the patch.
  ...
  it('keeps the Manta-only WebGL hunks in the generated patch', () => {
