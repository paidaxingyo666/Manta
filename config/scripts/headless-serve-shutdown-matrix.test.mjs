...
describe('packaged shutdown matrix', () => {
  it('shares extraction but isolates every entrypoint and signal', async () => {
    ...
    const extraction = commands().filter((args) =>
      args.some((arg) => arg.includes('120s /input/manta.AppImage --appimage-extract'))
    )
    ...
    for (const [index, args] of signalRuns().entries()) {
      ...
      expect(args).toContain(`MANTA_TEST_ENTRYPOINT=${entrypoint}`)
      expect(args).toContain(
        `MANTA_SIGNAL_TARGET=${entrypoint === 'appimage' ? 'serving-electron' : 'app'}`
      )
      expect(args).toContain(
        `MANTA_INT_DELIVERY=${entrypoint === 'appimage' ? 'pid' : 'foreground-process-group'}`
      )
      expect(args.at(-1)).toBe(index % 2 === 0 ? 'INT' : 'TERM')
      expect(args).toContain(`${artifact}:/input/manta.AppImage:ro`)
  ...
  it('preserves individual launcher overlay invocations', async () => {
    ...
    expect(signalRuns().every((args) => args.includes('MANTA_TEST_ENTRYPOINT=launcher'))).toBe(true)
    expect(
      commands().some((args) =>
