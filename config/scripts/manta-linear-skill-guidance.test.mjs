...
describe('orca-linear skill guidance', () => {
  ...
  it('keeps Linear guide examples off a bare manta command name', () => {
    for (const guidePath of [canonicalGuidePath, legacyGuidePath]) {
      ...
      expect(skill, guidePath).toContain(
        '`MANTA` is a placeholder for the executable you used to run `skills get`'
      )
  ...
  it('keeps the project flag surface owned by the CLI spec', () => {
    ...
    expect(spec).toContain('manta linear project list [--query <text>]')
...
describe('orca-linear install stubs', () => {
  ...
  for (const { name, stubPath, guidePath } of cases) {
    it(`points ${name} at the version-matched guide and preserves the safe resolver`, () => {
      ...
      expect(stub).toContain(`MANTA skills get ${name}`)
