...
function makeSocketPath(userDataDir) {
  ...
  if (process.platform === 'win32') {
    return `\\\\.\\pipe\\manta-daemon-smoke-${process.pid}-${randomUUID()}`
  }
...
async function main() {
  const userDataDir = mkdtempSync(join(tmpdir(), 'manta-daemon-boot-smoke-'))
  ...
  const child = fork(
    ...
    ],
    {
      ...
      env: { ...process.env, MANTA_USER_DATA_PATH: userDataDir }
    }
  )
