...
const ITERATIONS = Number(process.env.MANTA_DIFF_BLOB_BENCH_ITERATIONS ?? '10')
const WARMUP = Number(process.env.MANTA_DIFF_BLOB_BENCH_WARMUP ?? '3')

for (const [name, value] of [
  ['MANTA_DIFF_BLOB_BENCH_ITERATIONS', ITERATIONS],
  ['MANTA_DIFF_BLOB_BENCH_WARMUP', WARMUP]
]) {
