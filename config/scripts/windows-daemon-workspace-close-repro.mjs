 ...
 * graceful-then-immediate kill pair emitted when Manta closes a workspace.
...
const iterations = Number(process.env.MANTA_WINDOWS_DAEMON_CLOSE_ITERATIONS ?? 25)
...
async function main() {
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'manta-windows-daemon-close-'))
  const socketPath = `\\\\.\\pipe\\manta-daemon-close-${process.pid}-${randomUUID()}`
  ...
  const child = fork(
    ...
    ['--socket', socketPath, '--token', tokenPath, '--log-file', daemonLogPath],
    {
      ...
      windowsHide: true,
      env: { ...process.env, MANTA_USER_DATA_PATH: scratch }
    }
  )
