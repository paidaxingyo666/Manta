...
const CANDIDATE_COUNT = Number(process.env.MANTA_PENDING_MARKER_BENCH_CANDIDATES ?? '64')
const TOTAL_TICKS = Number(process.env.MANTA_PENDING_MARKER_BENCH_TICKS ?? '900')
const STEADY_TICKS = Number(process.env.MANTA_PENDING_MARKER_BENCH_STEADY_TICKS ?? '300')

for (const [name, value] of [
  ['MANTA_PENDING_MARKER_BENCH_CANDIDATES', CANDIDATE_COUNT],
  ['MANTA_PENDING_MARKER_BENCH_TICKS', TOTAL_TICKS],
  ['MANTA_PENDING_MARKER_BENCH_STEADY_TICKS', STEADY_TICKS]
]) {
...
if (STEADY_TICKS >= TOTAL_TICKS) {
  throw new Error('MANTA_PENDING_MARKER_BENCH_STEADY_TICKS must be smaller than total ticks')
}

const root = await mkdtemp(join(os.tmpdir(), 'manta-pending-marker-bench-'))
