...
const ITERATIONS = Number(process.env.MANTA_ADVERTISED_URL_BENCH_ITERATIONS ?? '10000')
const ROUNDS = Number(process.env.MANTA_ADVERTISED_URL_BENCH_ROUNDS ?? '12')
const WARMUP = Number(process.env.MANTA_ADVERTISED_URL_BENCH_WARMUP ?? '1000')

for (const [name, value] of [
  ['MANTA_ADVERTISED_URL_BENCH_ITERATIONS', ITERATIONS],
  ['MANTA_ADVERTISED_URL_BENCH_ROUNDS', ROUNDS],
  ['MANTA_ADVERTISED_URL_BENCH_WARMUP', WARMUP]
]) {
...
if (ROUNDS % 2 !== 0) {
  throw new Error('MANTA_ADVERTISED_URL_BENCH_ROUNDS must be even')
}
