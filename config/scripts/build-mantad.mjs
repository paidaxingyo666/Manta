...
if (graphErrors.length > 0) {
  ...
  if (
    smoke.error ||
    smoke.signal ||
    !/Unknown argument: --mantad-smoke-load-check/.test(smokeOutput)
  ) {
    console.error(
...
if (process.exitCode !== 1) {
  console.log(
    `[build-mantad] ok — ${(output.bytes / 1024 / 1024).toFixed(2)} MB, ${Object.keys(output.inputs).length} modules, zero electron and node:sqlite imports.`
  )
}
