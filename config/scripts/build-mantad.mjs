...
// Why beside mantad.js: the watcher runs in a forked child so a native @parcel/watcher
...
// Why beside mantad.js: mantad forks the terminal daemon so PTYs outlive the runtime process,
// and `getDaemonEntryPath()` probes the app root for this exact filename. Without it every
// mantad restart would SIGKILL every running terminal.
...
const OUT_FILE = join(OUT_DIR, 'mantad.js')
...
const output = Object.values(result.metafile.outputs).find(
  (o) => o.entryPoint === 'src/main/mantad/main.ts'
)
...
if (graphErrors.length > 0) {
  console.error(`[build-mantad] ${graphErrors.join('\n')}`)
  ...
  // smoke-loads its entries for exactly this reason, and mantad cannot join that guard
  ...
  const smoke = spawnSync(process.execPath, [OUT_FILE, '--mantad-smoke-load-check'], {
    encoding: 'utf8',
    timeout: 60_000
  })
  ...
  if (smoke.error || smoke.signal || smoke.status !== 0) {
    console.error(
      `[build-mantad] the bundle did not load under plain Node.\n` +
  ...
  // token and a PTY — `smoke:mantad-terminal` does that end to end, through mantad.
  ...
  const daemonSmoke = spawnSync(
    ...
    ],
    {
      encoding: 'utf8',
      timeout: 60_000,
      env: { ...process.env, MANTA_DAEMON_ENTRY_LOAD_CHECK: '1' }
    }
  )
  ...
  if (daemonSmoke.error || daemonSmoke.signal || daemonSmoke.status !== 0) {
    console.error(
      `[build-mantad] the daemon child did not load under plain Node.\n` +
  ...
  if (watcherFailure) {
    console.error(
      `[build-mantad] the watcher child did not run under plain Node.\n${watcherFailure}`
    )
    process.exitCode = 1
  }
}

// Why a content hash and not MANTAD_VERSION alone: the remote install directory is keyed on
...
async function smokeLoadWatcherChild() {
  const probeDir = mkdtempSync(join(tmpdir(), 'mantad-watcher-smoke-'))
