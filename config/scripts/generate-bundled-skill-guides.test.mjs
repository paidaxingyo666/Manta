...
describe('bundled skill guide generator', () => {
  ...
  it('points every guide at the executable that ran skills get', async () => {
    ...
    for (const name of CANONICAL_GUIDE_NAMES.filter((name) => name !== 'orchestration')) {
      ...
      expect(source, name).not.toContain('MANTA_CLI_COMMAND')
    }
  })
  ...
  it('projects one shared resolver fragment byte-for-byte into every stub', async () => {
    ...
    // where bare `manta` started the screen reader talking on a user's Ubuntu box.
      ...
      for (const [id, block] of blocks) {
        ...
        if (expected === null) {
          ...
          expect(projection.replace(/\s+/gu, ' '), `${name}/${id}`).toContain(
            `\`MANTA skills get ${name}\`. Beyond these commands, ask the user rather than guessing a command surface this older binary may not support.`
          )
          continue
        }
        expect(projection.split(expected), `${name}/${id}`).toHaveLength(2)
      }
      // The `MANTA` placeholder rule is stated once, in the fragment, never restated.
  ...
  // the `MANTA` placeholder rule. Red until the guide bodies drop their ladders; retiring
  // those also retires the MANTA_CLI_COMMAND/manta-dev/manta-ide assertions in
