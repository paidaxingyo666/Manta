...
const tempRoot = await mkdtemp(join(tmpdir(), 'manta-claude-spawn-'))
...
const fixtureDir = join(tempRoot, 'Profile space & ^ (paren) %MANTA_PATH_TRAP% !bang! 한글')
...
const fixtureEnv = {
  ...process.env,
  CLAUDE_CONFIG_DIR: join(fixtureDir, 'config space & ^ (paren) %MANTA_ENV_LITERAL% !bang! 한글'),
  MANTA_ARG_TRAP: 'EXPANDED_ARG',
  MANTA_PATH_TRAP: 'EXPANDED_PATH',
  MANTA_FIXTURE_CAPTURE: capturePath,
  MANTA_FIXTURE_PIDS: pidPath,
  MANTA_FIXTURE_NODE: process.execPath
}
const reportedEnv = {
  ...fixtureEnv,
  CLAUDE_CONFIG_DIR: join(reportedDir, 'config with spaces 한글'),
  MANTA_FIXTURE_CAPTURE: reportedCapturePath,
  MANTA_FIXTURE_PIDS: reportedPidPath
}
...
const fixtureSource =
  ...
  `  writeFileSync(process.env.MANTA_FIXTURE_PIDS, JSON.stringify({ child: process.pid, grandchild: grandchild.pid }))\n` +
  `  setInterval(() => {}, 1000)\n` +
  `} else {\n` +
  `  writeFileSync(process.env.MANTA_FIXTURE_CAPTURE, JSON.stringify({ argv: process.argv.slice(2), configDir: process.env.CLAUDE_CONFIG_DIR }))\n` +
  `}\n`
const shimSource = '@echo off\r\n"%MANTA_FIXTURE_NODE%" "%~dp0capture-child.cjs" %*\r\n'
...
try {
  ...
  for (const [name, segment] of Object.entries({
    ...
    percent: 'profile%MANTA_PATH_TRAP%',
    bang: 'profile!name',
    ...
    const env = {
      ...
      MANTA_FIXTURE_CAPTURE: captureFile
    }
