...
describe('electron-builder markdown file associations', () => {
  ...
  // so a macro that kills only manta-terminal-daemon.exe matches no running process. The
  ...
  it('keeps the daemon-host uninstall sweep across the include rename', async () => {
    ...
    expect(script).toMatch(/taskkill[^\n]*\/IM\s+"?manta-terminal-daemon\.exe"?/)
    ...
    expect(script).toContain('$LOCALAPPDATA\\Manta\\daemon-host')
