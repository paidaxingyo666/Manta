...
describe('headless serve shutdown PR gate', () => {
  it('reads only exact, closed systemd unit blocks', () => {
    expect(
      readSystemdUnitBlocks('# /etc/systemd/system/manta-serveXservice\n```', 'manta-serve.service')
    ).toEqual([])
    expect(() =>
      readSystemdUnitBlocks('# /etc/systemd/system/manta-serve.service\n', 'manta-serve.service')
    ).toThrow('Missing closing code fence for manta-serve.service')
    expect(() =>
  ...
  it('packages Linux artifacts before running the Docker signal oracle', () => {
    ...
    expect(shutdownStep.run).toBe(
      'node config/scripts/run-headless-serve-shutdown-docker.mjs --appimage dist/manta-linux.AppImage --all-entrypoints'
    )
  ...
  it('keeps readiness polling finite and leak-free', () => {
    ...
    expect(signalCase).toContain('startup_timeout_seconds=${MANTA_STARTUP_TIMEOUT_SECONDS:-180}')
  ...
  it('checks that a serving-electron signal target owns the ready socket', () => {
    const ssRecord =
      'LISTEN 0 128 127.0.0.1:41235 0.0.0.0:* users:(("manta-ide",pid=23,fd=7),("manta-ide",pid=25,fd=8))'
  ...
  it('runs the original AppImage desktop startup oracle before extraction and signals', () => {
    ...
    const extractionCall = shutdownDockerRunner.indexOf(
      "'timeout --kill-after=10s 120s /input/manta.AppImage --appimage-extract"
    )
  ...
  it('requires the bound AppImage to be executable before launch and extraction', () => {
    ...
    expect(shutdownDockerRunner).toContain(
      '\'test -r /input/manta.AppImage && test -x /input/manta.AppImage || { echo "FAIL: AppImage bind must be readable and executable" >&2; exit 1; }\''
    )
  })
  ...
  it('keeps owned Xvfb alive during the documented systemd graceful stop', () => {
    const serveUnits = readSystemdUnitBlocks(headlessLinuxGuide, 'manta-serve.service')
    ...
    expect(ownedXvfbUnits[0]).toMatch(/^ExecStart=.*manta-linux\.AppImage serve.*$/m)
  ...
  it('distinguishes persisted state from live work during a service restart', () => {
    ...
    expect(headlessLinuxGuide).not.toContain('sudo -Hu manta manta-ide terminal list --json')
  ...
  it('uses the registered CLI name from ordinary Linux shells', () => {
    const commandRule =
      'The registered Linux CLI command is `manta-ide`, not `orca`, to avoid shadowing the GNOME Orca screen reader.'
    const substitutionRule =
      "From an ordinary shell outside that service user's managed environment, substitute `manta-ide` for `manta` in commands below."
    const censusCommand = '`sudo -Hu manta /home/manta/.local/bin/manta-ide terminal list --json`'
    ...
    expect(headlessLinuxGuide).toContain('best-effort dispatcher at `$HOME/.local/bin/manta`')
