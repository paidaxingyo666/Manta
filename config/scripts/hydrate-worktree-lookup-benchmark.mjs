...
const ITERATIONS = Number(process.env.MANTA_HYDRATE_BENCH_ITERATIONS ?? '60')
const WARMUP = Number(process.env.MANTA_HYDRATE_BENCH_WARMUP ?? '10')
const ROUNDS = 6

for (const [name, value] of [
  ['MANTA_HYDRATE_BENCH_ITERATIONS', ITERATIONS],
  ['MANTA_HYDRATE_BENCH_WARMUP', WARMUP]
]) {
...
// Row shapes are synthetic, sized against a real manta-data.json on a heavy machine
