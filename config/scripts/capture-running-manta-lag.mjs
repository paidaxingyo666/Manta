...
import { connectOrcaMainInspector } from './manta-main-inspector-connection.mjs'
...
try {
  ...
  const before = await evaluateRenderer('window.__mantaTypingDiagnostic.report()')
  ...
  await evaluateRenderer('window.__mantaTypingDiagnostic.start()')
  echoStarted = true
  ...
  mainProfiling = false
  await evaluateRenderer('window.__mantaTypingDiagnostic.stop()')
  echoStarted = false
  const echo = await evaluateRenderer('window.__mantaTypingDiagnostic.report()')
  ...
  if (echoStarted) {
    await evaluateRenderer('window.__mantaTypingDiagnostic.stop()').catch(() => {})
  }
  if (mainProfiling) {
