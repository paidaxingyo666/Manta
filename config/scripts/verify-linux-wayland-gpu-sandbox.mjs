...
function createGitRepo() {
  const repoDir = mkdtempSync(path.join(tmpdir(), 'manta-wayland-gpu-repo-'))
...
async function runValidation(mode) {
  ...
  const userDataPath = mkdtempSync(path.join(tmpdir(), 'manta-wayland-gpu-userdata-'))
  ...
  try {
    ...
    app.process().stderr?.on('data', (chunk) => {
      ...
      if (process.env.MANTA_WAYLAND_GPU_VERBOSE === '1') {
