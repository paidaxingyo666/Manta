...
function readAppDirArg(argv) {
  ...
  if (process.platform === 'darwin') {
    return 'dist/mac-arm64/Manta.app'
  }
...
function getPackagedCliPath(appDir) {
  if (process.platform === 'darwin' || appDir.endsWith('.app')) {
    return join(appDir, 'Contents', 'Resources', 'bin', 'manta')
  }
  if (process.platform === 'win32') {
    return join(appDir, 'resources', 'bin', 'manta.exe')
  }
  return join(appDir, 'resources', 'bin', 'manta-ide')
}

const appDir = resolve(readAppDirArg(process.argv.slice(2)))
const tempRoot = await mkdtemp(join(tmpdir(), 'manta-packaged-cli-smoke-'))
...
try {
  ...
  assert(list.topics.some((topic) => topic.name === 'manta-cli'))
  assert.match((await run(['skills', 'get', 'manta-cli'])).stdout, /name: manta-cli/)
  ...
  const update = JSON.parse(
    (await run(['skills', 'update', '--skill', 'manta-cli', '--dry-run', '--json'])).stdout
  )
...
// Why: on Windows the launcher above spawns the copied Manta.exe (and its crashpad/utility children)
