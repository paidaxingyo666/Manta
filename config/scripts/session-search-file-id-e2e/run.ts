...
assert.equal(process.env.MANTA_BACKGROUND_LAUNCH, '1')
...
await build({
  ...
  bundle: true,
  packages: 'external',
  define: { MANTA_FEATURE_WALL_ENABLED: 'true' },
  plugins: [
    {
...
console.error('[file-id-child]', JSON.stringify({pid:process.pid,execPath:process.execPath,versions:process.versions,background:process.env.MANTA_BACKGROUND_LAUNCH}));\n${contents}`
          }
