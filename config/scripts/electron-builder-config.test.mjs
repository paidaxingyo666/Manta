...
describe('electron-builder config', () => {
  ...
  it('keeps plugin authoring examples out of app.asar', () => {
    ...
    for (const authoringOnly of [
      ...
      'examples/plugins/hostile-panel/manta-plugin.json',
      'examples/plugins/hello-manta/main.mjs',
      'examples/plugins/hello-manta/manta-plugin.json'
    ]) {
  ...
  it('keeps cached dev Electron bundles out of app.asar', () => {
    ...
    for (const devBundlePath of [
      'out/electron-dev/1a2b3c4d5e6f/Manta: dev.app/Contents/MacOS/Electron',
      'out/electron-dev/1a2b3c4d5e6f/manta-dev-electron-app.json'
    ]) {
  ...
  // asarUnpack:['resources/**'] to app.asar.unpacked/resources/win32/bin/manta.cmd,
  // a duplicate with no adjacent manta.exe that fails to launch (#7351).
  ...
  it('ships the mac notification-status helper in Contents/MacOS, not Resources', () => {
    ...
    expect(electronBuilderConfig.mac.extraResources).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ to: 'manta-notification-status' })])
    )
  })
  ...
  it('ships the mac keyboard-layout helper in Contents/MacOS, not Resources', () => {
    ...
    expect(electronBuilderConfig.mac.extraResources).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ to: 'manta-keyboard-layout' })])
    )
  })
  ...
  it('matches the Linux desktop entry to Electron window class', () => {
    expect(electronBuilderConfig.linux.desktop.entry.StartupWMClass).toBe('manta')
  })
  ...
  it('uses AppImage and deb as local Linux targets without changing existing artifact names', () => {
    ...
    expect(electronBuilderConfig.appImage.artifactName).toBe('manta-linux.${ext}')
    expect(electronBuilderConfig.deb.artifactName).toBe('manta-ide_${version}_${arch}.${ext}')
  ...
  it('uses a distinct AppImage name for Linux arm64 release uploads', () => {
    ...
    const original = process.env.MANTA_LINUX_ARM64_RELEASE
    try {
      delete require.cache[configPath]
      process.env.MANTA_LINUX_ARM64_RELEASE = '1'
      expect(require('../electron-builder.config.cjs').appImage.artifactName).toBe(
        'manta-linux-arm64.${ext}'
      )
    } finally {
      if (original === undefined) {
        delete process.env.MANTA_LINUX_ARM64_RELEASE
      } else {
        process.env.MANTA_LINUX_ARM64_RELEASE = original
      }
      delete require.cache[configPath]
  ...
  it('overrides packaged semver only for local macOS builds', () => {
    const configPath = require.resolve('../electron-builder.config.cjs')
    const original = process.env.MANTA_LOCAL_BUILD_VERSION
    const originalMacRelease = process.env.MANTA_MAC_RELEASE
    try {
      delete require.cache[configPath]
      delete process.env.MANTA_MAC_RELEASE
      process.env.MANTA_LOCAL_BUILD_VERSION = '1.4.159-rc.0.local.123.abc'
      ...
      if (originalMacRelease === undefined) {
        ...
        process.env.MANTA_MAC_RELEASE = originalMacRelease
      }
      if (original === undefined) {
        delete process.env.MANTA_LOCAL_BUILD_VERSION
      } else {
        process.env.MANTA_LOCAL_BUILD_VERSION = original
      }
      delete require.cache[configPath]
      require('../electron-builder.config.cjs')
    }
  ...
  it('never applies local semver to release packaging', () => {
    const configPath = require.resolve('../electron-builder.config.cjs')
    const originalLocalVersion = process.env.MANTA_LOCAL_BUILD_VERSION
    const originalMacRelease = process.env.MANTA_MAC_RELEASE
    try {
      delete require.cache[configPath]
      process.env.MANTA_LOCAL_BUILD_VERSION = '1.4.159-local.123.abc'
      process.env.MANTA_MAC_RELEASE = '1'
      expect(require('../electron-builder.config.cjs').extraMetadata).toBeUndefined()
    } finally {
      if (originalLocalVersion === undefined) {
        delete process.env.MANTA_LOCAL_BUILD_VERSION
      } else {
        process.env.MANTA_LOCAL_BUILD_VERSION = originalLocalVersion
      }
      if (originalMacRelease === undefined) {
        delete process.env.MANTA_MAC_RELEASE
      } else {
  ...
  it('uses Manta native rebuild hook instead of electron-builder default rebuild', () => {
  ...
  it('verifies packaged main runtime deps from Windows-style asar entries', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'manta-runtime-deps-'))
    try {
  ...
  it('prunes non-target node-pty architecture outputs from packaged runtime resources', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'manta-node-pty-prune-'))
    try {
  ...
  it('copies the Windows node-pty ConPTY runtime beside the rebuilt addon', async () => {
    for (const [arch, electronArch] of [
      ...
      const resourcesDir = await mkdtemp(join(tmpdir(), `manta-node-pty-conpty-${arch}-`))
      try {
        const nodePtyDir = join(resourcesDir, 'node_modules', 'node-pty')
  ...
  it('prunes non-target @parcel/watcher architecture subpackages', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'manta-parcel-watcher-prune-'))
    try {
  ...
  it('leaves unrelated @parcel/* runtime deps untouched when pruning the watcher', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'manta-parcel-watcher-prune-unrelated-'))
    try {
      const parcelDir = join(resourcesDir, 'node_modules', '@parcel')
      await mkdir(join(parcelDir, 'watcher'), { recursive: true })
      await mkdir(join(parcelDir, 'watcher-darwin-arm64'), { recursive: true })
  ...
  it('prunes type declaration artifacts from packaged runtime node_modules', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'manta-runtime-type-prune-'))
    try {
  ...
  it('prunes duplicate darwin sherpa-onnx runtime dylib aliases', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'manta-sherpa-prune-'))
    try {
  ...
  it('prunes zod TypeScript sources from packaged runtime resources', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'manta-zod-prune-'))
    try {
  ...
  it('fails when the packaged resources directory is missing', async () => {
    const root = await mkdtemp(join(tmpdir(), 'manta-electron-builder-config-'))
    try {
      await expect(
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      try {
        ...
        const launcherPath = join(resourcesDir, 'bin', 'manta-ide')
