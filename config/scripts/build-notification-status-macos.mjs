#!/usr/bin/env node
// Builds the manta-notification-status helper binary.
//
...
const bundleId = readArg('--bundle-id') ?? 'cn.sh.manta'
...
const workDir = path.join(tmpdir(), `manta-notification-status-${process.pid}`)
...
try {
  ...
  const builtBinaries = triples.map((triple) => {
    const output = path.join(workDir, `manta-notification-status-${triple}`)
    execFileSync(
      'swiftc',
      [
        '-O',
...
<dict>
  ...
  <string>manta-notification-status</string>
</dict>
</plist>
`
}
