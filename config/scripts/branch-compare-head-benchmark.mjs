...
const ITERATIONS = Number(process.env.MANTA_BRANCH_COMPARE_BENCH_ITERATIONS ?? '8')
const WARMUP = Number(process.env.MANTA_BRANCH_COMPARE_BENCH_WARMUP ?? '2')
const ROUNDS = 6

for (const [name, value] of [
  ['MANTA_BRANCH_COMPARE_BENCH_ITERATIONS', ITERATIONS],
  ['MANTA_BRANCH_COMPARE_BENCH_WARMUP', WARMUP]
]) {
...
// A short remote label is the common case (Manta's base picker emits `origin/main`); the
