...
// installed; @manta/windows-registry is a workspace link and present everywhere.
...
describe('Electron runtime package contract', () => {
  ...
  it('keeps the native Windows registry addon optional and platform-gated', () => {
    ...
    expect(packageJson.optionalDependencies['@manta/windows-registry']).toBe('workspace:*')
    ...
    expect(pnpmWorkspace.allowBuilds['@manta/windows-registry']).toBe(false)
    ...
    expect(rebuildScript).toContain("'@manta/windows-registry'")
    expect(ensureScript).toContain("process.platform === 'win32'")
    expect(ensureScript).toContain("'@manta/windows-registry'")
    if (windowsAddonsInstalled) {
      expect(packageTargets.win32).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ to: join('node_modules', '@manta', 'windows-registry') }),
  ...
  it('guards release publishing before electron-builder runs', () => {
    ...
    expect(macReleaseCommand).toContain(' && MANTA_MAC_RELEASE=1 ')
    ...
    expect(releaseCommands.get('linux-arm64')).toContain('MANTA_LINUX_ARM64_RELEASE=1')
  ...
  it('runs the macOS release build in an isolated Blacksmith workflow', () => {
    ...
    expect(publishStep.with.command).toContain('MANTA_MAC_RELEASE=1')
  ...
  it('bumps separate Homebrew casks for stable and RC desktop tags', () => {
    ...
    expect(resolveCaskStep.run).toContain('token="manta@rc"')
    expect(resolveCaskStep.run).toContain('token="manta"')
  ...
  it('keeps terminal scale perf wired to the report budget gate', () => {
    ...
    const manualProfileKnobs = [
      ['MANTA_TERMINAL_PERF_FRAME_COUNT', 'frame_count', 'MANTA_E2E_OPENCODE_FRAME_COUNT'],
      [
        'MANTA_TERMINAL_PERF_FRAME_INTERVAL_MS',
        'frame_interval_ms',
        'MANTA_E2E_OPENCODE_FRAME_INTERVAL_MS'
      ],
      [
        'MANTA_TERMINAL_PERF_PRESSURE_OUTPUT_CHARS',
        'pressure_output_chars',
        'MANTA_E2E_OPENCODE_PRESSURE_OUTPUT_CHARS'
      ],
      ['MANTA_TERMINAL_PERF_SCALE_PANES', 'scale_panes', 'MANTA_E2E_OPENCODE_SCALE_PANES'],
      [
        'MANTA_TERMINAL_PERF_SCALE_CROSS_WORKSPACE_PANES',
        'scale_cross_workspace_panes',
        'MANTA_E2E_OPENCODE_SCALE_CROSS_WORKSPACE_PANES'
      ],
      [
        'MANTA_TERMINAL_PERF_SCALE_PRESSURE_PANES',
        'scale_pressure_panes',
        'MANTA_E2E_OPENCODE_SCALE_PRESSURE_PANES'
      ],
      [
        'MANTA_TERMINAL_PERF_SCALE_HIDDEN_PRESSURE_PANES',
    ...
    expect(uploadStep.with.path).toBe('${{ env.MANTA_E2E_TERMINAL_PERF_REPORT_PATH }}')
  })
  ...
  it('keeps platform golden regressions in the manual and release workflows', () => {
    ...
    expect(releaseEvidenceJob.steps.map((step) => step.run ?? '')).toContain(
      'xvfb-run --auto-servernum env SKIP_BUILD=1 MANTA_E2E_FORWARD_APP_LOGS=1 pnpm run test:e2e:terminal-rendering-release-evidence'
    )
  })
})
