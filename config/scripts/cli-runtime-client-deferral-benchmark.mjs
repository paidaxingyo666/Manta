...
const ROUNDS = Number(process.env.MANTA_CLI_DEFER_BENCH_ROUNDS ?? '30')
const WARMUP = Number(process.env.MANTA_CLI_DEFER_BENCH_WARMUP ?? '3')

for (const [name, value] of [
  ['MANTA_CLI_DEFER_BENCH_ROUNDS', ROUNDS],
  ['MANTA_CLI_DEFER_BENCH_WARMUP', WARMUP]
]) {
...
if (ROUNDS % 2 !== 0) {
  ...
  throw new Error(`MANTA_CLI_DEFER_BENCH_ROUNDS must be even, received ${ROUNDS}`)
}
...
try {
  ...
  const isolated = { MANTA_USER_DATA_PATH: userDataPath }
  /** @type {Array<[string, string[], Record<string, string>]>} */
  const cases = [
    ['manta --help', ['--help'], {}],
    ['manta help worktree', ['help', 'worktree'], {}],
    ['manta (no args)', [], {}],
    ['unknown command', ['no-such-command'], {}],
    ['unknown flag', ['worktree', 'list', '--nope'], {}],
    ['manta agent-context --json', ['agent-context', '--json'], {}],
    ['manta status --json', ['status', '--json'], isolated],
    ['manta worktree list --json', ['worktree', 'list', '--json'], isolated]
  ]
