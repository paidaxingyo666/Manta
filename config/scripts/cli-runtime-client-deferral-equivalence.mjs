...
function run(entry, argv, env) {
  ...
  if (result.error) {
    if (result.error.code === 'ETIMEDOUT') {
      throw new Error(
        `manta ${argv.join(' ')} did not exit within ${RUN_TIMEOUT_MS} ms — it reached a blocking command`
      )
    }
    throw result.error
  }
...
function buildCases(isolatedUserData) {
  const isolated = { MANTA_USER_DATA_PATH: isolatedUserData }
  const cases = [
    ...
    [['no-such-command'], {}],
    [['no-such-command'], { MANTA_PAIRING_CODE: 'garbage' }],
    [['wrktree', 'list'], {}],
    [['agent-context'], {}],
    [['agent-context', '--json'], {}],
    ...
    [['status', '--pairing-code', 'manta://pair?code=zzzz'], isolated],
    ...
    [['status', '--json'], { ...isolated, MANTA_PAIRING_CODE: 'not-a-pairing-code' }],
    [['status', '--json'], { ...isolated, MANTA_REMOTE_PAIRING: 'not-a-pairing-code' }],
    [['status', '--json'], { ...isolated, MANTA_ENVIRONMENT: 'no-such-environment' }],
    ...
    // MANTA_USER_DATA_PATH. They are deliberately NOT invoked here. The
    ...
    [['environment', 'list', '--json'], { ...isolated, MANTA_ENVIRONMENT: 'no-such-environment' }],
    [['environment', 'list', '--json'], { ...isolated, MANTA_PAIRING_CODE: 'not-a-pairing-code' }],
    [['agent-context', '--json'], { ...isolated, MANTA_PAIRING_CODE: 'not-a-pairing-code' }],
...
//
//   1. FOREGROUND — `manta serve` runs Manta until Ctrl+C and `manta open` /
...
//   2. MUTATING — writes outside MANTA_USER_DATA_PATH (`agent hooks off` parks
...
const UNSAFE_TOKENS = new Map([
  ...
  ...MUTATING_TOKENS.map((token) => [token, 'can write outside MANTA_USER_DATA_PATH'])
])
