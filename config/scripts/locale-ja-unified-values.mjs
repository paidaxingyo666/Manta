...
export const JA_UNIFIED_VALUES = {
  ...
  'Show Manta Mobile Button': 'Manta Mobile ボタンを表示',
  Paste: '貼り付け',
  ...
  'Show the Manta Mobile button at the top of the left sidebar.':
    '左サイドバー上部に Manta Mobile ボタンを表示します。',
  'Enable Manta CLI': 'Manta CLI を有効化',
  'Registered the Manta CLI in PATH.': 'Manta CLI を PATH に登録しました。',
  ...
  'Gitea credentials are configured but could not authenticate. Check the token, API base URL, and repository permissions, then restart Manta if environment variables changed.':
    'Gitea の認証情報は設定されていますが、認証できませんでした。トークン、API ベース URL、リポジトリの権限を確認し、環境変数を変更した場合は Manta を再起動してください。',
  'Azure DevOps credentials are configured but could not authenticate. Check the token, API base URL, and repository permissions, then restart Manta if environment variables changed.':
    'Azure DevOps の認証情報は設定されていますが、認証できませんでした。トークン、API ベース URL、リポジトリの権限を確認し、環境変数を変更した場合は Manta を再起動してください。',
  'Bitbucket credentials are configured but could not authenticate. Check the token and repository permissions, then restart Manta if environment variables changed.':
    'Bitbucket の認証情報は設定されていますが、認証できませんでした。トークンとリポジトリの権限を確認し、環境変数を変更した場合は Manta を再起動してください。',
  ...
  'Manta will continue to work, but commands such as manta status, manta terminal, and orchestration are unavailable for this session.\n\n{{guidance}}\n\nCause: {{cause}}':
    'Manta は引き続き使用できますが、このセッションでは manta status や manta terminal などのコマンドとオーケストレーションを利用できません。\n\n{{guidance}}\n\n原因: {{cause}}'
}
