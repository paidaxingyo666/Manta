...
const NATIVE_MODULES = [
  'node-pty',
  'cpu-features',
  ...(rebuildPlatform === 'win32' ? ['@manta/windows-registry', '@vscode/windows-process-tree'] : [])
]
...
const forceRebuild =
  process.env.MANTA_FORCE_NATIVE_REBUILD === '1' || cliOptions.force || isCrossHostRebuild
...
try {
  ...
  // raw stack instead of the "close running Manta/Electron processes" message.
  if (
  ...
  if (isWindowsNativeLockError(err)) {
    console.error(
      '[rebuild] A Windows process appears to be using a native .node file. ' +
        'Close running Manta/Electron/dev processes for this worktree, then rerun `pnpm install` ' +
        'or `pnpm run rebuild:electron`.'
    )
    if (isPostinstall() && process.env.MANTA_STRICT_NATIVE_REBUILD !== '1') {
      console.error(
...
function continuePostinstallWithoutElectron() {
  if (!isPostinstall() || process.env.MANTA_STRICT_ELECTRON_INSTALL === '1') {
    return false
  }
  console.error(
...
function getPatchedNodePtyRebuildReason() {
  ...
  // Why: Manta patches node-pty's native Unix spawn path and Windows job-object
...
function loadNativeModule(moduleName) {
  if (moduleName === '@manta/windows-registry') {
    ...
    if (requirePatchedNodePtySourceBuild && !isNodePtyReleaseBuildDir(native.dir)) {
      throw new Error(
        'node-pty resolved to ' +
          native.dir +
          '; expected build/Release so Manta\\'s node-pty patch is active'
      )
    }
    return
  }
