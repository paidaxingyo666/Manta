; electron-builder NSIS hooks for the Manta Windows installer.
;
; electron-builder accepts exactly ONE `nsis.include` file, so every customInstall /
; customUnInstall hook Manta needs lives here.

; ---------------------------------------------------------------------------
; Markdown "Open with Manta" (issue #10138)
;
...
; uninstalling Manta would leave .md pointing at a deleted ProgID.
;
; These writes are additive only. Registering a ProgID plus an OpenWithProgids
; hint and an Applications\<exe>\SupportedTypes entry puts Manta in Explorer's
...
; (manta-terminal-daemon.exe) under %LOCALAPPDATA%\Manta\daemon-host so that app
...
!macro customUnInstall
  ${ifNot} ${isUpdated}
    nsExec::Exec 'taskkill /F /IM manta-terminal-daemon.exe'
    ; Give the OS a moment to release the image lock before removing the tree.
    Sleep 500
    RMDir /r "$LOCALAPPDATA\Manta\daemon-host"
  ${endIf}
