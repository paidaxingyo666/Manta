...
describe('the native agent-history route that hands off to the shell', () => {
  beforeEach(() => {
    ...
    dependencies.storage.set('manta:mobileWebShellEnabled', 'true')
  })
  ...
  it('renders the native panel with the flag off, which is every store build', async () => {
    dependencies.storage.set('manta:mobileWebShellEnabled', 'false')
    await renderRoute()
    expect(dependencies.routes).toEqual([])
   ...
   * `bridge-host.ts` dropped the route to null, and the page answered with "Update Manta to open
