...
describe('what init primed', () => {
  it('answers a read without waiting on the shell', async () => {
    publish({ 'manta:pins:host-1': '["wt-1"]' })
    ...
    await expect(pageAsyncStorage.getItem('manta:pins:host-1')).resolves.toBe('["wt-1"]')
    await expect(pageAsyncStorage.getItem('manta:pins:host-2')).resolves.toBeNull()
  })

  it('reads several keys at once, which is how the list asks', async () => {
    publish({ 'manta:pins:host-1': '["wt-1"]' })
    await expect(
      pageAsyncStorage.multiGet(['manta:pins:host-1', 'manta:last-visited-worktree'])
    ).resolves.toEqual([
      ['manta:pins:host-1', '["wt-1"]'],
      ['manta:last-visited-worktree', null]
    ])
  })

  it('replaces what the last page held rather than adding to it', async () => {
    publish({ 'manta:pins:host-1': '["wt-1"]' })
    publish({ 'manta:pins:host-2': '["wt-2"]' })
    await expect(pageAsyncStorage.getItem('manta:pins:host-1')).resolves.toBeNull()
  })
})

describe('a write the page makes', () => {
  it('goes to the app, and is readable here at once', async () => {
    await pageAsyncStorage.setItem('manta:pins:host-1', '["wt-1","wt-2"]')
    expect(writes).toEqual([{ key: 'manta:pins:host-1', value: '["wt-1","wt-2"]' }])
    await expect(pageAsyncStorage.getItem('manta:pins:host-1')).resolves.toBe('["wt-1","wt-2"]')
  })

  it('removes by writing null, which is what the app then deletes', async () => {
    publish({ 'manta:pins:host-1': '["wt-1"]' })
    await pageAsyncStorage.removeItem('manta:pins:host-1')
    expect(writes).toEqual([{ key: 'manta:pins:host-1', value: null }])
    await expect(pageAsyncStorage.getItem('manta:pins:host-1')).resolves.toBeNull()
  })
  ...
  it('is refused, and not kept, for a key outside the allowlist', async () => {
    ...
    await pageAsyncStorage.setItem('manta:mobileWebShellEnabled', 'true')
    expect(writes).toEqual([])
    await expect(pageAsyncStorage.getItem('manta:mobileWebShellEnabled')).resolves.toBeNull()
  })

  it('is refused, and not kept, when the shell granted no storage', async () => {
    granted = false
    await pageAsyncStorage.setItem('manta:pins:host-1', '["wt-1"]')
    await expect(pageAsyncStorage.getItem('manta:pins:host-1')).resolves.toBeNull()
  })
  ...
  it('carries each pair of a multi-write separately, and drops the ones outside the list', async () => {
    ...
    expect(writes).toEqual([{ key: 'manta:pins:host-1', value: '["wt-1"]' }])
  })
...
describe('what the page will not keep', () => {
  it("refuses another host's pinned list, so a later read cannot answer with it", async () => {
    publish({ 'manta:pins:host-1': '["mine"]' })
    await pageAsyncStorage.setItem('manta:pins:host-2', '["theirs"]')
    ...
    expect(writes).toEqual([])
    expect(await pageAsyncStorage.getItem('manta:pins:host-2')).toBeNull()
  })
  ...
  it('refuses a value over the envelope bound rather than caching what the wire will drop', async () => {
    ...
    await pageAsyncStorage.setItem('manta:last-visited-worktree', oversized)
    expect(writes).toEqual([])
    expect(await pageAsyncStorage.getItem('manta:last-visited-worktree')).toBeNull()
  })
  ...
  it('still keeps a value exactly at the bound, so the refusal above discriminates', async () => {
    ...
    await pageAsyncStorage.setItem('manta:last-visited-worktree', atBound)
    expect(writes).toEqual([{ key: 'manta:last-visited-worktree', value: atBound }])
    expect(await pageAsyncStorage.getItem('manta:last-visited-worktree')).toBe(atBound)
  })
})
