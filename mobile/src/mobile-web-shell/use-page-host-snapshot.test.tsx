...
const PINS = 'manta:pins:host-1'
const LAST_VISITED = 'manta:last-visited-worktree'
...
describe('what the shell puts on every init', () => {
  ...
  it('never carries another host key, whatever the store holds', async () => {
    ...
    doubles.store.set('manta:pins:host-2', '["theirs"]')
    const mounted = await mount()
    await act(async () => {
      mounted.view().refreshStorage()
    })
    ...
    await act(async () => {
      mounted.view().writeStorage('manta:pins:host-2', '["theirs"]')
    })
    expect(mounted.view().readStorage()).toEqual({ [PINS]: '["mine"]' })
  })
})
