 ...
 *
 * An allowlist and not a passthrough. Everything the app keeps under `manta:` is in one namespace —
...
export const PAGE_STORAGE_KEY_PREFIXES = [
  /** `manta:pins:<hostId>`: the pinned worktrees of the host whose list the page is showing. */
  'manta:pins:'
] as const
...
export function pageStorageKeysForHost(hostId: string): string[] {
  return [...PAGE_STORAGE_EXACT_KEYS, `manta:pins:${hostId}`]
}

/**
 * The allowlist narrowed to one host, which is the one every write is actually held to.
 *
 * `isPageStorageKey` answers for the shape, so `manta:pins:<any host>` passes it; a page opened for
