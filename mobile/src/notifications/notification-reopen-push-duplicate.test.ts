...
// Why this file exists: a push the OS drew while Manta was closed never runs through
...
function presentTray(entries: readonly Record<string, unknown>[]): void {
  vi.mocked(Notifications.getPresentedNotificationsAsync).mockResolvedValue(
    entries.map((manta, index) => ({
      request: {
        identifier: `tray-${index}`,
        content: { data: null },
        trigger: { type: 'push', payload: { manta } }
      }
    })) as never
  )
}
...
describe('reopen after a push the OS showed while Manta was closed', () => {
