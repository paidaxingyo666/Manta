...
// APNs nests Manta's fields beside `aps`; FCM sends them flat and stringified.
function apnsData(manta: Record<string, unknown>): unknown {
  return { aps: { alert: { title: 'Manta', body: 'Agent needs input' } }, manta }
}

function fcmData(manta: Record<string, unknown>): unknown {
  return Object.fromEntries(Object.entries(manta).map(([key, value]) => [key, String(value)]))
}
...
describe('pushNotificationRouteData', () => {
  it('routes a tap by mapping the fingerprint to the paired host id', () => {
    ...
    expect(getNotificationNavigationTarget(data, { knownHostIds: new Set(['host-1']) })).toEqual({
      ...
      sessionTarget: {
        ...
        params: { hostId: 'host-1', worktreeId: 'repo::/Users/me/manta/workspaces/feature' }
      }
    })
  })
  ...
  it('leaves a remote push unrouted when no host catalog could be read', () => {
    const data = { hostId: 'host-1', manta: { hostFingerprint, notificationId: 'agent:one' } }
  ...
  it('recognises only a provider-delivered trigger as remote', () => {
    ...
    expect(isRemotePushTrigger({ channelId: 'manta-desktop' })).toBe(false)
  ...
  it('drops a gateway payload that pairs an unresolvable fingerprint with a stray hostId', () => {
    const data = {
      hostId: 'host-1',
      manta: { hostFingerprint: '0123456789abcdef', notificationId: 'agent:one' }
    }
