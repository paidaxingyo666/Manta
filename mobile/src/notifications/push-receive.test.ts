...
describe('pushNotificationRouteData', () => {
  ...
  it('keeps local data untouched and rejects an unresolvable remote fingerprint', () => {
    ...
    expect(
      pushNotificationRouteData(
        { hostId: 'host-1', manta: { hostFingerprint: 'unknown' } },
        hosts,
        true
      )
    ).toBeNull()
...
it('uses one delivery snapshot for sound and viewing even when settings change during host lookup', async () => {
  ...
  storage.set(
    'manta:notificationDeliveryPreferences',
    JSON.stringify({
      sound: false,
  ...
  expect(
    vi
      ...
      .mock.calls.filter(([key]) => key === 'manta:notificationDeliveryPreferences')
  ).toHaveLength(1)
})
...
it.each(['apns', 'fcm'])(
  ...
  (provider) => {
    ...
    const data = provider === 'apns' ? { manta: payload } : payload
