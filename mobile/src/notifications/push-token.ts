...
// Why: the native APNs/FCM token, not an Expo push token — Manta's own gateway
...
function toMobilePushToken(raw: { type: string; data: unknown }): MobilePushToken | null {
  ...
  // Web tokens carry an object payload and no Manta gateway path; only native counts.
