...
function ProbeResult({ state, hostName }: { state: MobileWebBundleProbeState; hostName: string }) {
  ...
  if (state.status === 'failed') {
    return (
      <View style={styles.section}>
        <ProbeLine
          label={translate('m.mobile.web.bundle.probe.row.75615fd996', 'Bundle')}
          detail={state.detail}
          failed
        />
      </View>
  ...
  return (
    <View style={styles.section}>
      <ProbeLine
        label={translate('m.mobile.web.bundle.probe.row.e1ab7ffa4b', 'Host')}
        detail={hostName}
      />
      <View style={styles.separator} />
      <ProbeLine
        label={translate('m.mobile.web.bundle.probe.row.8f7d8fa787', 'Build')}
        detail={state.buildId.slice(0, 12)}
      />
      <View style={styles.separator} />
      <ProbeLine
        label={translate('m.mobile.web.bundle.probe.row.6a70855f9e', 'Assets')}
        detail={`${state.assetCount}`}
      />
      <View style={styles.separator} />
      <ProbeLine
        label={translate('m.mobile.web.bundle.probe.row.3808a6a12b', 'Bytes')}
        detail={`${state.totalBytes}`}
      />
      <View style={styles.separator} />
      <ProbeLine
        label={translate('m.mobile.web.bundle.probe.row.5423f35086', 'Elapsed')}
        detail={translate('m.mobile.web.bundle.probe.row.9e9bbd7927', '{{value0}} ms', {
          value0: state.elapsedMs
        })}
      />
    </View>
  )
