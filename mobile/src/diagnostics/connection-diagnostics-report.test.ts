...
describe('active path', () => {
  // The paired endpoint says where this phone met the host; on a relay the two
  // disagree, and a report naming only the endpoint sends readers to the wrong
  // network — the "connected node looks wrong" report.
  it('records the path actually in use alongside the paired endpoint', () => {
    const report = buildConnectionDiagnosticsReport({
      hostName: 'Desk',
      endpoint: 'ws://192.168.1.50:6768',
      activePath: 'relay',
    ...
    expect(report).toContain('Paired endpoint: 192.168.1.50:6768')
    expect(report).toContain('Active path: relay')
  })

  it('omits the line when the path is unknown', () => {
    const report = buildConnectionDiagnosticsReport({
      hostName: 'Desk',
      endpoint: 'ws://192.168.1.50:6768',
      state: 'disconnected',
    ...
    expect(report).not.toContain('Active path:')
  })
})
