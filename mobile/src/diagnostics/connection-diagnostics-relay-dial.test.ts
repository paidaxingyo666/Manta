...
// Provenance: the last two resume windows of a real diagnostics export (Manta
...
const HOST_OFFLINE_STEP =
  'Check the desktop is awake, Manta is running, and it is signed in to Manta Cloud.'
...
describe('diagnoseConnection on the other relay close codes', () => {
  ...
  it('leaves a director outage to the director branch', () => {
    ...
    expect(diagnose([appResumed(), lanOpening(), director]).likelyCause).toBe(
      'Relay service was temporarily unavailable and asked Manta to retry in 30s.'
    )
  })
})
