...
describe('readRepoIdentity', () => {
  it('parses a valid owner/repo identity', () => {
    expect(readRepoIdentity({ owner: 'octo', repo: 'manta' })).toEqual({
      owner: 'octo',
      repo: 'manta'
    })
  })
  ...
  it('preserves an Enterprise host', () => {
    expect(readRepoIdentity({ owner: 'octo', repo: 'manta', host: 'github.acme.test' })).toEqual({
      owner: 'octo',
      repo: 'manta',
  ...
  it('drops a non-record value', () => {
    ...
    expect(readRepoIdentity('octo/manta')).toBeUndefined()
  })

  it('drops a missing owner or repo', () => {
    expect(readRepoIdentity({ repo: 'manta' })).toBeUndefined()
    expect(readRepoIdentity({ owner: 'octo' })).toBeUndefined()
  })

  it('drops an empty owner or repo as malformed', () => {
    expect(readRepoIdentity({ owner: '', repo: 'manta' })).toBeUndefined()
