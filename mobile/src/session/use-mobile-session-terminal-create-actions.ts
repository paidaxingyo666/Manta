...
export function useMobileSessionTerminalCreateActions(scope: MobileSessionAttachmentsModel) {
            ...
            .then((sendResponse) => {
              ...
              showToast(
                options.successToast ??
                  translate('m.use.mobile.session.terminal.create.actions.7088f59be7', 'Notes sent')
              )
              ...
              showToast(
                options.errorToast ??
                  (err instanceof Error
                    ? err.message
                    : translate(
                        'm.use.mobile.session.terminal.create.actions.ac43c4f206',
                        "Couldn't send notes"
                      )),
                1800
              )
  ...
  function launchQuickCommand(command: TerminalQuickCommand): boolean {
    ...
    if (!launch) {
      ...
      showToast(
        translate(
          'm.use.mobile.session.terminal.create.actions.bc62534eb0',
