...
describe('useMobileNativeChatSessionOptions', () => {
  ...
  it('switches an OMP session with /manta-model <selector>', async () => {
    mount({
      agent: 'omp',
    ...
    expect(dispatchCommand).toHaveBeenCalledWith('/manta-model minimax-cn/MiniMax-M3')
    const model = api!.snapshot[0]!
  ...
  it('does not revive a stale session-start report over a newer local pick', async () => {
    ...
    // which cannot have observed the `/manta-model opus` sent after it.
