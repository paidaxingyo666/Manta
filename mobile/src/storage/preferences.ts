...
// scheduled from the live socket, which work with Manta open and share no token
// with anyone. Background push hands a native token to Apple/Google and needs
// its own explicit, default-off consent.
const REMOTE_PUSH_KEY = 'manta:remotePushEnabled'
const REMOTE_PUSH_AGENT_STATES_KEY = 'manta:remotePushAgentStates'
const REMOTE_PUSH_HOST_REGISTRATIONS_KEY = 'manta:remotePushHostRegistrations'
...
const TEXT_SCALE_KEY = 'manta:terminalTextScale'
...
const AUTOCOMPLETE_KEY = 'manta:terminalAutocompleteEnabled'
...
const TERMINAL_LIVE_INPUT_DISABLED_PREFIX = 'manta:terminalLiveInputDisabled:'
...
const SIDEBAR_WIDTH_KEY = 'manta:hostSidebarWidth'
...
const DOCK_WIDTH_KEY = 'manta:hostDockWidth'
...
export type MobileTerminalLinkOpenMode = 'manta-browser' | 'phone-browser'

const TERMINAL_LINK_OPEN_MODE_KEY = 'manta:terminalLinkOpenMode'
export const DEFAULT_TERMINAL_LINK_OPEN_MODE: MobileTerminalLinkOpenMode = 'manta-browser'
...
export async function loadTerminalLinkOpenMode(): Promise<MobileTerminalLinkOpenMode> {
  try {
    ...
    return raw === 'phone-browser' || raw === 'manta-browser' ? raw : DEFAULT_TERMINAL_LINK_OPEN_MODE
  } catch {
