...
describe('session view preference', () => {
  ...
  it('defaults to terminal and persists the chat default', async () => {
    ...
    expect(AsyncStorage.getItem).toHaveBeenCalledWith('manta:defaultSessionView')

    await saveDefaultSessionView('chat')
    expect(AsyncStorage.setItem).toHaveBeenCalledWith('manta:defaultSessionView', 'chat')
  ...
  it('loads and updates per-tab overrides under a host-and-worktree scoped key', async () => {
    ...
    expect(AsyncStorage.getItem).toHaveBeenCalledWith(
      'manta:nativeChatTabs:host%2Fone:folder%3AC%3A%5Crepo'
    )
  ...
  it('serializes default writes across callers and makes reads wait for the latest', async () => {
    ...
    expect(AsyncStorage.setItem).toHaveBeenNthCalledWith(1, 'manta:defaultSessionView', 'chat')
    expect(AsyncStorage.setItem).toHaveBeenNthCalledWith(2, 'manta:defaultSessionView', 'terminal')
  })
  ...
  it('does not globally block updates for a different host and worktree', async () => {
    ...
    expect(AsyncStorage.setItem).toHaveBeenCalledWith(
      'manta:nativeChatTabs:other-host:worktree',
  ...
  it('repairs invalid preference data on the next user mutation', async () => {
    ...
    expect(AsyncStorage.setItem).toHaveBeenCalledWith(
      'manta:nativeChatTabs:host:worktree',
...
describe('terminal autocomplete preference', () => {
  ...
  it('defaults to disabled when unset', async () => {
    ...
    expect(AsyncStorage.getItem).toHaveBeenCalledWith('manta:terminalAutocompleteEnabled')
  })
  ...
  it('persists the selected value', async () => {
    ...
    expect(AsyncStorage.setItem).toHaveBeenCalledWith('manta:terminalAutocompleteEnabled', 'true')

    await saveTerminalAutocompleteEnabled(false)

    expect(AsyncStorage.setItem).toHaveBeenCalledWith('manta:terminalAutocompleteEnabled', 'false')
  })
})
...
describe('terminal live input disabled handles preference', () => {
  ...
  it('persists disabled handles per host and worktree', async () => {
    ...
    expect(AsyncStorage.setItem).toHaveBeenCalledWith(
      'manta:terminalLiveInputDisabled:host%2Fone:folder%3AC%3A%5Crepo',
...
describe('terminal link open mode preference', () => {
  ...
  it('defaults to Manta browser when unset', async () => {
    vi.mocked(AsyncStorage.getItem).mockResolvedValue(null)

    await expect(loadTerminalLinkOpenMode()).resolves.toBe('manta-browser')
    expect(AsyncStorage.getItem).toHaveBeenCalledWith('manta:terminalLinkOpenMode')
  })
  ...
  it('falls back to Manta browser when storage cannot be read', async () => {
    vi.mocked(AsyncStorage.getItem).mockRejectedValue(new Error('storage unavailable'))
  ...
  it('persists the selected mode', async () => {
    ...
    expect(AsyncStorage.setItem).toHaveBeenCalledWith('manta:terminalLinkOpenMode', 'phone-browser')
  })
})
...
describe('hybrid shell flag', () => {
  ...
  it('reads the developer toggle in a development build', async () => {
    ...
    expect(AsyncStorage.getItem).toHaveBeenCalledWith('manta:mobileWebShellEnabled')
  })
