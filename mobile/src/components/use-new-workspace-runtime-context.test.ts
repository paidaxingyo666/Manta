...
type PublishedState = Pick<
  RuntimeContext,
  'runtimeSettings' | 'trustedMantaHooks' | 'availableProviders'
>

// A real PersistedTrustedMantaHooks record, keyed by repo id with a per-hook approval. The earlier
...
const UI_WITH_TRUST = { ui: { trustedMantaHooks: TRUSTED_HOOKS } }
...
describe('useNewWorkspaceRuntimeContext', () => {
  ...
  async function answer(settingsResult: unknown, uiResult: unknown): Promise<PublishedState> {
    ...
    await act(async () => {})
    const { runtimeSettings, trustedMantaHooks, availableProviders } = context!
    return { runtimeSettings, trustedMantaHooks, availableProviders }
  }
  ...
  it.each([
    ...
    expect(await answer(settingsResult, UI_WITH_TRUST)).toEqual({
      ...
      trustedMantaHooks: TRUSTED_HOOKS,
  ...
  // The blank, not just the absence of a throw: trustedMantaHooks gates the setup-hook approval
  ...
  it('blanks the trust an earlier host published when the next ui result is null', async () => {
    expect((await answer({ settings: SETTINGS }, UI_WITH_TRUST)).trustedMantaHooks).toEqual(
      TRUSTED_HOOKS
    )
    expect((await answer({ settings: SETTINGS }, null)).trustedMantaHooks).toEqual({})
  })
