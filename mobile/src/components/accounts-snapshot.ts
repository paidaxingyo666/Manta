  ...
  .superRefine((target, context) => {
    if (target.runtime === 'host' && target.wslDistro !== null) {
      context.addIssue({
        code: 'custom',
        message: translate(
          'm.accounts.snapshot.10884c74dd',
    ...
    if (
      ...
      context.addIssue({
        code: 'custom',
        message: translate(
          'm.accounts.snapshot.66534ee9f4',
  ...
  .superRefine((account, context) => {
    ...
    if (runtime === 'host' && account.wslDistro != null) {
      context.addIssue({
        code: 'custom',
        message: translate(
          'm.accounts.snapshot.39989a7f2a',
    ...
    if (
      ...
      context.addIssue({
        code: 'custom',
        message: translate(
          'm.accounts.snapshot.f795e4b34f',
  ...
  .superRefine((snapshot, context) => {
    if (snapshot.rateLimits.claude && snapshot.rateLimits.claude.provider !== 'claude') {
      context.addIssue({
        code: 'custom',
        message: translate(
          'm.accounts.snapshot.3967e8ed07',
    ...
    if (snapshot.rateLimits.codex && snapshot.rateLimits.codex.provider !== 'codex') {
      context.addIssue({
        code: 'custom',
        message: translate(
          'm.accounts.snapshot.37c91810fe',
    ...
    for (const [index, entry] of snapshot.rateLimits.inactiveClaudeAccounts.entries()) {
      if (entry.rateLimits && entry.rateLimits.provider !== 'claude') {
        context.addIssue({
          code: 'custom',
          message: translate(
            'm.accounts.snapshot.e0d2cb1b80',
    ...
    for (const [index, entry] of snapshot.rateLimits.inactiveCodexAccounts.entries()) {
      if (entry.rateLimits && entry.rateLimits.provider !== 'codex') {
        context.addIssue({
          code: 'custom',
          message: translate(
            'm.accounts.snapshot.0a40a0d179',
