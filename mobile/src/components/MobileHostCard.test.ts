...
describe('MobileHostCard', () => {
  ...
  it('preserves the connected worktree-catalog failure state', async () => {
    ...
    const navigationButton = renderer.root.findAllByType('Pressable')[0]
    expect(navigationButton.props.accessibilityLabel).toBe(
      'Open Desk, Connected, Manta Relay, Worktree list unavailable'
    )
    expect(
      renderer.root
        .findAllByType('Text')
  ...
  it('includes visible offline recovery guidance in the navigation label', async () => {
    ...
    const navigationButton = renderer.root.findAllByType('Pressable')[0]
    expect(navigationButton.props.accessibilityLabel).toBe(
      "Open Desk, Can't reach desktop, Update desktop Manta and sign in to connect from anywhere"
    )
  })
  ...
  it('renders the verdict detail as a second line and announces it', async () => {
      ...
      renderer = create(
        createElement(MobileHostCard, {
          ...
          verdict: {
            ...
            reason: 'never-connected',
            detail: "Check it's awake, Manta is running, and you're signed in"
          },
          path: 'lan',
          onPress: vi.fn(),
          onLongPress: vi.fn(),
    ...
    expect(texts).toContainEqual("Check it's awake, Manta is running, and you're signed in")
    expect(renderer.root.findAllByType('Pressable')[0]?.props.accessibilityLabel).toBe(
      "Open Host 1, Host 1 is offline, Check it's awake, Manta is running, and you're signed in"
    )
  })
})
