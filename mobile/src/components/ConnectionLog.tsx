...
type Props = {
  ...
  title?: string
  // Stretch to the space left below instead of capping at a panel height. The
  // pairing screens embed this under other content and need the cap; the
  // connection-log screen is nothing but the log, and the cap left the bottom
  // of the screen blank.
  fill?: boolean
}
...
export function ConnectionLog({ entries, title, fill = false }: Props) {
  ...
  return (
    <View style={[styles.container, fill && styles.containerFill]}>
      ...
      <ScrollView
        ref={scrollRef}
        style={fill ? styles.scrollFill : styles.scroll}
