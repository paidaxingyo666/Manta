...
import {
  ...
  newWorktreeBlankAgent as BLANK_TERMINAL,
...
function NewWorktreeModalContent({
  ...
  async function handleCreate(options: CreateOptions = {}) {
    ...
    try {
      ...
      if (
        ...
        !isSetupHookTrusted(trustedMantaHooks, selectedRepo.id, setupTrust.contentHash)
      ) {
        // Why: desktop prompts before running repo-owned manta.yaml setup hooks.
        ...
        setSetupTrustPrompt({
          ...
          previouslyApproved: wasSetupHookPreviouslyApproved(trustedMantaHooks, selectedRepo.id)
        })
  ...
  async function approveSetupTrust(alwaysTrust: boolean): Promise<void> {
    ...
    try {
      ...
      setTrustedMantaHooks(nextTrust)
