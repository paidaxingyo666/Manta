import { z } from 'zod'
import type { SetupRunPolicy } from '../../../src/shared/manta-yaml-hook-types'
 ...
 * handler always answers it, `null` when no manta.yaml parsed. Nullable rather than optional so the
 ...
 * there for a binary or unparsable manta.yaml. `setupRunPolicy` is a closed enum with the call
 ...
 * (src/shared/manta-yaml-hook-types.ts:1), which is what `getEffectiveSetupRunPolicy` answers, and
 ...
 * Main cast the result and read `?.trustedMantaHooks ?? {}`, which tolerated a reply of any shape,
 * so anything that is not an object decodes as absent and reaches that `?? {}` exactly as before —
 * use-new-workspace-runtime-context.ts:80 takes `ui.value?.trustedMantaHooks ?? {}`. The record
...
export const newWorkspaceUiTrustSchema = z
  .looseObject({
    ui: salvagedOptional(
      ...
      z
        .looseObject({
          trustedMantaHooks: salvagedOptional(
            'trustedMantaHooks',
