...
export function MobileFilePreviewScreen({ route }: Props) {
  ...
  const [preview, setPreview] = useState<MobileFilePreviewResult>(() =>
    route.ok
      ? {
          status: 'loading',
          message: translate('m.MobileFilePreviewScreen.22a9f89ade', 'Loading preview...')
        }
  ...
  const loadPreview = useCallback(async () => {
    ...
    if (!client || connState !== 'connected') {
      ...
      setPreview({
        status: 'waiting',
        message: translate('m.MobileFilePreviewScreen.302163baef', 'Waiting for desktop...'),
        reconnect: true
      })
      return
    }
  ...
  const meta = previewParams
    ...
    : translate('m.MobileFilePreviewScreen.efa10b0260', 'Preview')
    ...
    <View style={styles.container}>
        ...
        <View style={styles.topBar}>
          ...
          <View style={styles.titleBlock}>
            <Text style={styles.title} numberOfLines={1}>
              {title || translate('m.MobileFilePreviewScreen.efa10b0260', 'Preview')}
            </Text>
      ...
      <MobileFilePreviewBody
        ...
        onImageError={() =>
          setPreview({
            status: 'error',
            message: translate('m.MobileFilePreviewScreen.b8b5edd90b', 'Unable to load preview'),
            reconnect: false
          })
        }
        onRetry={retry}
      ...
      <ConfirmModal
        ...
        title={translate('m.MobileFilePreviewScreen.93debd0de1', 'Discard changes?')}
        message={translate('m.MobileFilePreviewScreen.bd76a56e52', 'Unsaved edits will be lost.')}
