...
const JOURNAL_STORAGE_KEY = 'manta:mobile-relay:pairing-journal:v1'
const JOURNAL_SECRET_KEY = 'manta.mobile-relay.pairing-journal.v1'
...
function requireNativeSecretStore(): void {
  if (Platform.OS === 'web') {
    throw new Error('Manta Relay pairing requires a native secret store')
  }
}
