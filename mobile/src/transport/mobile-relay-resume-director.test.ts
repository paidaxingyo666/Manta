...
const relay = {
  ...
  cellUrl: 'https://relay-old.manta.sh.cn',
  assignmentEpoch: 7,
...
describe('mobile relay resume director', () => {
  it('uses a bounded POST body and never puts the bearer in the URL', async () => {
    ...
    ).resolves.toMatchObject({ cellUrl: 'https://relay-c2.manta.sh.cn', assignmentEpoch: 8 })
    const [url, init] = fetchImpl.mock.calls[0]!
    expect(url).toBe('https://relay.manta.sh.cn/v1/resolve')
    ...
    const badTarget = vi.fn(
      async () =>
        new Response(
          JSON.stringify({
            v: 1,
            cellUrl: 'http://relay-c2.manta.sh.cn',
            assignmentEpoch: 8,
            leaseExpiresAt: 1
          })
        )
