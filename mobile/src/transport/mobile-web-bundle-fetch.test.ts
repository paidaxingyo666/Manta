...
describe('fetchMobileWebBundle', () => {
  it('pages every asset to eof and returns the verified bytes', async () => {
    const host = bundleHost({ 'index.html': '<h1>manta</h1>', 'assets/app.js': 'x=1' })
    ...
    expect(new TextDecoder().decode(fetched.assets.get('index.html'))).toBe('<h1>manta</h1>')
    expect(new TextDecoder().decode(fetched.assets.get('assets/app.js'))).toBe('x=1')
    expect(fetched.totalBytes).toBe(17)
    ...
    // 14 bytes at 4 per chunk is four requests, 3 bytes is one, plus the manifest.
