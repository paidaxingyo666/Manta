...
Five domains share `hostClientContextExposure` to mount the real host-client hooks. Manta exports
their `RpcClientCtx` from `rpc-client-context-contract.ts`; the exposure reads that public export
through the same module loader as the provider. It cannot read an imported TypeScript binding by
its source name after CommonJS transpilation. `adapter-seam.test.ts` verifies both the exported
context and the provider's import, and rejects duplicate exposure strings. The shared exposure
is part of `recorderSha256`, so editing it requires recording the entire corpus again.
...
```sh
MANTA_BACKGROUND_LAUNCH=1 RPC_FOUNDATION_RECORD=1 pnpm --dir mobile exec tsx scripts/rpc-recording.mts --record
MANTA_BACKGROUND_LAUNCH=1 pnpm --dir mobile test src/test-support/rpc-recording
```
...
MANTA_BACKGROUND_LAUNCH=1 RPC_FOUNDATION_RECORD=1 \
