...
describe('a scenario that declares its device', () => {
  ...
  it('reads the declared entry, and null for everything else', async () => {
    const { module } = store({ 'manta:key': 'declared' })
    await expect(module.getItem!('manta:key')).resolves.toBe('declared')
    await expect(module.getItem!('manta:other')).resolves.toBeNull()
  })
  ...
  it('records a write as an effect without letting a read see it', async () => {
    ...
    await module.setItem!('manta:key', 'written')
    await module.removeItem!('manta:key')
    await expect(module.getItem!('manta:key')).resolves.toBeNull()
    expect(effects).toEqual([
      { name: 'device-store.setItem', value: { key: 'manta:key', value: 'written' } },
      { name: 'device-store.removeItem', value: { key: 'manta:key' } }
    ])
  })

  it('refuses a member the declaration does not back', () => {
    const { module } = store({})
    expect(() => module.multiGet!('manta:key')).toThrow(
  ...
  it('backs a recorded write: the reset credit journals its key before the request', async () => {
    ...
    expect(write.key).toMatch(/^manta:codex-reset-credit-attempt:v1:[0-9a-f]{64}$/)
