...
// Same-named repos on different hosts (a local "manta" and a runtime "manta")
...
describe('every worktree stays visible regardless of repo.list contents', () => {
  it('shows worktrees whose repoId is missing from repo.list', () => {
    ...
    const worktrees = [
      worktree('manta-local', 'a'),
      worktree('manta-local', 'b'),
      worktree('manta-runtime', 'c')
    ]
    const repoIdsByName = new Map([['manta-runtime', 'manta-runtime']])
  ...
  it('shows every worktree when the same repo name maps to two host-specific ids', () => {
    // The exact trigger: a local "manta" and a runtime "manta". repo.list returns
    // both under the same displayName; a name-keyed map keeps only the last id.
    const worktrees = [
      worktree('manta-local', 'local-1', 'manta'),
      worktree('manta-local', 'local-2', 'manta'),
      worktree('manta-runtime', 'runtime-1', 'manta')
    ]
    // Name-keyed map collapses "manta" to a single id, as the host screen builds it.
    const repoIdsByName = new Map([['manta', 'manta-runtime']])
