...
export async function startHeadlessPairingRuntime({
  ...
  const runDir = mkdtempSync(path.join(os.tmpdir(), 'manta-mobile-run.'))
  ...
  const child = spawn(
    ...
    ['serve', '--mobile-pairing', '--pairing-address', pairingAddress, '--json'],
    {
      ...
      env: {
        ...process.env,
        MANTA_E2E_USER_DATA_DIR: userData,
        MANTA_E2E_HOME_DIR: homeDir,
        HOME: homeDir,
        USERPROFILE: homeDir
      },
...
export async function registerWorktreeForPairingRuntime(runtime, worktree, tools) {
  ...
  await tools.manta(['repo', 'add', '--path', worktree, '--json'], {
    cwd: worktree,
    env: runtime.env,
    timeout: 60000
  })
