...
#
# Everything is driven with xcrun simctl rather than `manta emulator`, because the
...
#     > /tmp/manta-emulator-boot.log 2>&1 &
#
...
APP=$(pgrep -f "CoreSimulator.*Manta.app/Manta" | head -1)
...
T0=$(date +%s)
step "[5] user returns to Manta  <-- t0, desktop still down"
