#!/usr/bin/env node
/**
 * Start Manta Mobile server and load it in the iOS emulator.
...
const MANTA_CLI = process.env.MANTA_CLI || 'manta'
...
// Execute manta CLI command
async function manta(args, options = {}) {
  const { stdout, stderr } = await execFileAsync(MANTA_CLI, args, {
...
async function getWorktree() {
  ...
  try {
    const { stdout } = await manta(['worktree', 'current', '--json'])
...
async function attachEmulator(worktree, device, runtime) {
  ...
  try {
    await manta(['emulator', 'attach', device.udid, '--worktree', worktree, '--focus', '--json'], {
      cwd: worktree,
...
function devClientUrlForMetroUrl(url) {
  return `exp+manta-mobile://expo-development-client/?url=${encodeURIComponent(url)}`
}
...
async function startMetro(worktree) {
  ...
  return new Promise((resolve, reject) => {
    ...
    rl.on('line', (line) => {
      ...
      const urlMatch = line.match(/exp\+manta-mobile:\/\/expo-development-client\/\?url=([^\s]+)/)
...
async function openPairingUrlInSimulator(pairingUrl, deviceUdid, runtime, worktree) {
  ...
  await manta(['emulator', 'tap', '0.5', '0.56', '--worktree', worktree, '--json'], {
    cwd: worktree,
    env: runtime?.env || process.env,
    timeout: 30000
  })
...
async function takeScreenshot(
  deviceUdid,
  outputPath = path.join(os.tmpdir(), 'manta-mobile-ios.png')
) {
...
// Main function
async function main() {
  log(colors.bright + 'Starting Manta Mobile in Emulator\n' + colors.reset)
  ...
  try {
    ...
    // Why: emulator helpers are worktree-scoped in Manta; attach is idempotent
