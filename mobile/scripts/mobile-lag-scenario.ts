...
const REPO_NAMES = ['manta', 'dashboard', 'mobile', 'runtime', 'docs', 'api', 'desktop', 'site']
const WORKTREE_NAMES = ['manta', 'narwhal', 'otter', 'squid', 'turtle', 'beluga', 'marlin', 'manta']
...
export function createMockRepos(count: number): MockRepo[] {
  return Array.from({ length: count }, (_, index) => {
    ...
    return {
      id: `repo-${index + 1}`,
      displayName,
      path: `/tmp/manta-mobile-repro/${displayName}`,
