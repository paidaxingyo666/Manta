...
const USER_DATA =
  process.env.MANTA_USER_DATA ?? `${process.env.HOME}/Library/Application Support/manta-dev`
...
const devices = readJson<Array<{ token: string }>>(join(USER_DATA, 'manta-devices.json'))
const token = devices[0]?.token
const keypair = readJson<{ publicKeyB64: string }>(join(USER_DATA, 'manta-e2ee-keypair.json'))
